 ...
 *
 * @file management_terminal.c
 * @author Anup Patel (anup@brainfault.org)
 * @author Himanshu Chauhan (hschauhan@nulltrace.org)
 * @brief source file of management terminal
 */
...
#define MODULE_DESC      "Management Terminal"
#define MODULE_AUTHOR    "Anup Patel"
#define MODULE_LICENSE   "GPL"
#define MODULE_IPRIORITY 0
#define MODULE_INIT      daemon_management_terminal_init
#define MODULE_EXIT      daemon_management_terminal_exit

static struct management_terminal_ctrl {
    vmm_thread_t *thread;
...
static int management_terminal_main(void *udata)
{
    size_t             cmds_len;
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (cmds_len > 0) {
                ...
                vmm_command_manager_execute_cmdstr(cdev, cmds, NULL);
            }
        }
    }
...
static int __init daemon_management_terminal_init(void)
{
    uint32_t                management_terminal_priority;
    uint32_t                management_terminal_time_slice;
    ...
    /* Retrive management_terminal time slice */
    ...
    if (vmm_device_tree_read_u32(node, "management_terminal_priority", &management_terminal_priority)) {
        management_terminal_priority = VMM_THREAD_DEF_PRIORITY;
    }

    if (vmm_device_tree_read_u32(node, "management_terminal_time_slice", &management_terminal_time_slice)) {
        management_terminal_time_slice = VMM_THREAD_DEF_TIME_SLICE;
    }

    vmm_device_tree_dref_node(node);

    /* Create management_terminal thread */
    mtctrl.thread =
        vmm_threads_create("management_terminal", &management_terminal_main, NULL, management_terminal_priority, management_terminal_time_slice);

    if (!mtctrl.thread) {
        vmm_panic("Creation of system critical thread failed.\n");
    }

    /* Start the management_terminal thread */
    vmm_threads_start(mtctrl.thread);

    return VMM_OK;
}

static void __exit daemon_management_terminal_exit(void)
{
