...
char *conf_get_default_confname(void)
{
    ...
    if (env && (snprintf(fullname, sizeof(fullname), "%s/%s", env, name) < sizeof(fullname))) {
...
int conf_write(const char *name)
{
    ...
    if (name && name[0]) {
        ...
        if (!stat(name, &st) && S_ISDIR(st.st_mode)) {
            ...
            if (snprintf(dirname, sizeof(dirname), "%s/", name) >= sizeof(dirname)) {
                return 1;
            }
    ...
    if (snprintf(newname, sizeof(newname), "%s%s", dirname, basename) >= sizeof(newname)) {
        return 1;
    }
    ...
    if (!env || !*env) {
        if (snprintf(tmpname, sizeof(tmpname), "%s.tmpconfig.%d", dirname, (int)getpid()) >= sizeof(tmpname)) {
            return 1;
        }
    ...
    fprintf(
        ...
        _("#\n"
          ...
          "%s%s"
          "#\n"),
        project, version, use_timestamp ? "# " : "", use_timestamp ? ctime(&now) : "");
    ...
    if (*tmpname) {
        ...
        if (!stat(newname, &st) && S_ISREG(st.st_mode)) {
            ...
            if (snprintf(tmp, sizeof(tmp), "%s%s.old", dirname, basename) >= sizeof(tmp)) {
                return 1;
            }
...
int conf_split_config(void)
{
    ...
    if (snprintf(path, sizeof(path), "%s/%s", tmppath, name) >= sizeof(path)) {
        return 1;
    }
...
int conf_write_autoconf(void)
{
    ...
    if (snprintf(path, sizeof(path), "%s/%s.cmd", tmppath, name) >= sizeof(path)) {
        return 1;
    }
