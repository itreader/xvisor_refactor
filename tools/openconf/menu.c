...
void menu_set_type(int type)
{
    ...
    menu_warn(
        current_entry, "type of '%s' redefined from '%s' to '%s'", sym->name ? sym->name : "<choice>", sym_type_name(sym->type), sym_type_name(type));
}
        ...
        switch (prop->type) {
            ...
            case P_RANGE:
                if (sym->type != S_INT && sym->type != S_HEX) {
                    prop_warn(
                        prop, "range is only allowed "
                              "for int or hex symbols");
                }

                if (!menu_range_valid_sym(sym, prop->expr->left.sym) || !menu_range_valid_sym(sym, prop->expr->right.sym)) {
