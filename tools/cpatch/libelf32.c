...
Elf32_File *Elf32_Open(const char *filename, int is_be)
{
    ...
    for (ph = 0; ph < elf->hdr->e_phnum; ph++) {
        if (lseek(fd, elf->hdr->e_phoff + elf->hdr->e_phentsize * ph, SEEK_SET) == -1) {
            goto free_elf_phdr;
        }
    ...
    for (sh = 0; sh < elf->hdr->e_shnum; sh++) {
        if (lseek(fd, elf->hdr->e_shoff + elf->hdr->e_shentsize * sh, SEEK_SET) == -1) {
            goto free_elf_shdr;
        }
    ...
    if (lseek(fd, elf->shdr[elf->hdr->e_shstrndx].sh_offset, SEEK_SET) == -1) {
...
int Elf32_Shdr_Read32(Elf32_File *elf, Elf32_Shdr *shdr, Elf32_Addr addr, Elf32_Word *word)
{
    ...
    if (lseek(elf->fd, shdr->sh_offset + (addr - shdr->sh_addr), SEEK_SET) == -1) {
        return -1;
    }
