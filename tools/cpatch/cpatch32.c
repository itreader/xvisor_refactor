...
uint32_t strsplit(char *str, char sep, char *tok[], uint32_t maxtok, uint32_t maxtoksz)
{
    char     delim[2] = {sep, '\0'};
    char    *token;
    uint32_t cnt = 0;
...
int main(int argc, char **argv)
{
    ...
    while (fgets(line, sizeof(line), script) != NULL) { /* read a line */
        ...
        tokcnt = strsplit(line, CPATCH32_SEPARATOR, toks, CPATCH32_MAX_TOKEN_COUNT, CPATCH32_MAX_TOKEN_SIZE);
