...
static int32_t ps2_emu_get_write_cmd(struct ps2_emu_state *s)
{
    int32_t     ret;
    irq_flags_t flags;
...
static void ps2_emu_set_write_cmd(struct ps2_emu_state *s, int32_t write_cmd)
{
    irq_flags_t flags;
...
static const unsigned char ps2_raw_keycode_set3[128] = {
    0,   8,   22,  30,  38,  37,  46,  54,  61, 62, 70, 69, 78, 85, 102, 13, 21, 29,  36,  45,  44,  53,  60,  67,  68,  77,
    84,  91,  90,  17,  28,  27,  35,  43,  52, 51, 59, 66, 75, 76, 82,  14, 18, 92,  26,  34,  33,  42,  50,  49,  58,  65,
    73,  74,  89,  126, 25,  41,  20,  7,   15, 23, 31, 39, 47, 2,  63,  71, 79, 118, 95,  108, 117, 125, 132, 107, 115, 116,
    124, 105, 114, 122, 112, 113, 127, 96,  97, 86, 94, 15, 23, 31, 39,  47, 55, 63,  71,  79,  86,  94,  8,   16,  24,  32,
    40,  48,  56,  64,  72,  80,  87,  111, 19, 25, 57, 81, 83, 92, 95,  98, 99, 100, 101, 103, 104, 106, 109, 110};
...
static void ps2_emu_keyboard_event(struct vmm_vkeyboard *vkbd, int keycode, int key)
{
    irq_flags_t              flags;
    struct ps2_emu_keyboard *k = vmm_vkeyboard_private(vkbd);

    vmm_spin_lock_irq_save(&k->lock, flags);
    __ps2_emu_keyboard_event(k, keycode);
    vmm_spin_unlock_irq_restore(&k->lock, flags);
}

struct ps2_emu_keyboard *ps2_emu_alloc_keyboard(const char *name, void (*update_irq)(void *, int), void *update_arg)
{
...
static void __ps2_emu_mouse_send_packet(struct ps2_emu_mouse *m)
{
    ...
    b = 0x08 | ((dx1 < 0) << 4) | ((dy1 < 0) << 5) | (m->mouse_buttons & 0x07);
...
static void ps2_emu_mouse_event(struct vmm_vmouse *vmou, int dx, int dy, int dz, int buttons_state)
{
    irq_flags_t           flags;
    struct ps2_emu_mouse *m = vmm_vmouse_private(vmou);

    vmm_spin_lock_irq_save(&m->lock, flags);

    /* check if deltas are recorded when disabled */
    if (!(m->mouse_status & MOUSE_STATUS_ENABLED)) {
        vmm_spin_unlock_irq_restore(&m->lock, flags);
        return;
    }
    ...
    if (m->mouse_dx == 0 && m->mouse_dy == 0 && m->mouse_dz == 0 && m->mouse_buttons == buttons_state) {
        vmm_spin_unlock_irq_restore(&m->lock, flags);
        return;
    }
    ...
    if (!(m->mouse_status & MOUSE_STATUS_REMOTE) && (ps2_emu_queue_count(&m->state) < (PS2_EMU_QUEUE_SIZE - 16))) {
        for (;;) {
            ...
            if (m->mouse_dx == 0 && m->mouse_dy == 0 && m->mouse_dz == 0) {
                break;
            }
        }
    }
...
struct ps2_emu_mouse *ps2_emu_alloc_mouse(const char *name, void (*update_irq)(void *, int), void *update_arg)
{
