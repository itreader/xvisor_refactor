...
#define MODULE_IPRIORITY          (VMM_VIRTIO_IPRIORITY + VMM_VINPUT_IPRIORITY + 1)
...
static uint64_t virtio_input_get_host_features(struct vmm_virtio_device *dev)
{
    return 1ULL << VMM_VIRTIO_F_VERSION_1 | 1UL << VMM_VIRTIO_RING_F_EVENT_IDX;
#if 0
    | 1UL << VMM_VIRTIO_RING_F_INDIRECT_DESC;
#endif
}

static void virtio_input_set_guest_features(struct vmm_virtio_device *dev, uint32_t select, uint32_t features)
{
    struct virtio_input_device *videv = dev->emu_data;
    ...
    videv->features &= ~((uint64_t)UINT_MAX << (select * 32));
    videv->features |= ((uint64_t)features << (select * 32));
}

static int virtio_input_init_vq(struct vmm_virtio_device *dev, uint32_t vq, uint32_t page_size, uint32_t align, uint32_t pfn)
{
    int                         rc;
    struct virtio_input_device *videv = dev->emu_data;
    ...
    switch (vq) {
        ...
        case VIRTIO_INPUT_STATUS_QUEUE:
            rc = vmm_virtio_queue_setup(&videv->vqs[vq], dev->guest, pfn, page_size, VIRTIO_INPUT_QUEUE_SIZE, align);
            break;
...
static int virtio_input_get_pfn_vq(struct vmm_virtio_device *dev, uint32_t vq)
{
    int                         rc;
    struct virtio_input_device *videv = dev->emu_data;
...
static int virtio_input_get_size_vq(struct vmm_virtio_device *dev, uint32_t vq)
{
    int rc;
...
static int virtio_input_set_size_vq(struct vmm_virtio_device *dev, uint32_t vq, int size)
{
...
static void __virtio_input_do_events(struct virtio_input_device *videv, struct vmm_virtio_input_event *events, int events_count)
{
    uint16_t                  head;
    int                       rc, i = 0;
    uint32_t                  iov_cnt, total_len, wr_len;
    ...
    while ((i < events_count) && vmm_virtio_queue_available(vq)) {
        ...
        if (rc) {
            vmm_printf(
                "%s: failed to get iovec for event[%d] "
                "(error %d)\n",
                __func__, i, rc);
            return;
        }
        ...
        wr_len = vmm_virtio_buf_to_iovec_write(dev, iov, iov_cnt, &events[i], sizeof(*events));
...
static void virtio_input_keyboard_event(struct vmm_vkeyboard *vkbd, int vkeycode, int vkey)
{
    irq_flags_t                   flags;
    int                           vkeyvalue, cnt = 0;
    struct vmm_virtio_input_event events[2];
    struct virtio_input_device   *videv = vmm_vkeyboard_private(vkbd);

    DPRINTF("%s: dev=%s vkeycode=0x%x vkey=%d\n", __func__, videv->vdev->name, vkeycode, vkey);

    vmm_spin_lock_irq_save(&videv->event_lock, flags);
    ...
    events[cnt].code             = vmm_cpu_to_le16((uint16_t)vkeycode);
    events[cnt].value            = vmm_cpu_to_le32((uint32_t)vkeyvalue);
    cnt++;
    ...
    vmm_spin_unlock_irq_restore(&videv->event_lock, flags);
}

static void virtio_input_mouse_event(struct vmm_vmouse *vmou, int dx, int dy, int dz, int buttons_state)
{
    uint32_t                      value;
    irq_flags_t                   flags;
    int                           buttons_change, cnt = 0;
    struct vmm_virtio_input_event events[7];
    struct virtio_input_device   *videv = vmm_vmouse_private(vmou);
    ...
    events[cnt].value = vmm_cpu_to_le32((uint32_t)dx);
    cnt++;

    events[cnt].type  = vmm_cpu_to_le16(EV_REL);
    events[cnt].code  = vmm_cpu_to_le16(REL_Y);
    events[cnt].value = vmm_cpu_to_le32((uint32_t)dy);
    cnt++;

    events[cnt].type  = vmm_cpu_to_le16(EV_REL);
    events[cnt].code  = vmm_cpu_to_le16(REL_Z);
    events[cnt].value = vmm_cpu_to_le32((uint32_t)dz);
    cnt++;
...
static void virtio_input_do_status(struct vmm_virtio_device *dev, struct virtio_input_device *videv)
{
    uint16_t                      head;
    int                           rc, ledstate, ledmask;
    uint32_t                      i, iov_cnt, total_len, rd_len;
    ...
    while (vmm_virtio_queue_available(vq)) {
        ...
        for (i = 0; i < iov_cnt; i++) {
            rd_len = vmm_virtio_iovec_to_buf_read(dev, &iov[i], 1, &event, sizeof(event));
            ...
            DPRINTF("%s: dev=%s type=0x%x code=0x%x value=0x%x\n", __func__, dev->name, event.type, event.code, event.value);
...
static int virtio_input_notify_vq(struct vmm_virtio_device *dev, uint32_t vq)
{
    int                         rc    = VMM_OK;
    struct virtio_input_device *videv = dev->emu_data;
...
static void virtio_input_status_changed(struct vmm_virtio_device *dev, uint32_t new_status)
{
    /* Nothing to do here. */
}

static void virtio_input_update_config(struct virtio_input_device *videv)
{
    uint32_t                        i;
...
static int virtio_input_read_config(struct vmm_virtio_device *dev, uint32_t offset, void *dst, uint32_t dst_len)
{
    struct virtio_input_device *videv = dev->emu_data;
    uint32_t                    i, src_len = sizeof(videv->config);
    uint8_t                    *src = (uint8_t *)&videv->config;
...
static int virtio_input_write_config(struct vmm_virtio_device *dev, uint32_t offset, void *src, uint32_t src_len)
{
    uint8_t                     data8;
    struct virtio_input_device *videv = dev->emu_data;
...
static int virtio_input_reset(struct vmm_virtio_device *dev)
{
    ...
    videv->config.subsel = 0, virtio_input_update_config(videv);
...
static int virtio_input_connect(struct vmm_virtio_device *dev, struct vmm_virtio_emulator *emu)
{
    char                        name[VMM_FIELD_NAME_SIZE];
    struct virtio_input_device *videv;

    DPRINTF("%s: dev=%s emu=%s\n", __func__, dev->name, emu->name);

    videv = vmm_zalloc(sizeof(struct virtio_input_device));
    ...
    videv->vkbd = vmm_vkeyboard_create(name, virtio_input_keyboard_event, videv);
    ...
    videv->vmou = vmm_vmouse_create(name, TRUE, virtio_input_mouse_event, videv);
