...
static void i8042_update_irq(i8042_emu_state_t *s)
{
    ...
    vmm_device_emulate_emulate_irq(s->guest, s->irq_kbd, irq_kbd_level);
    vmm_device_emulate_emulate_irq(s->guest, s->irq_mouse, irq_mouse_level);
}
...
static uint64_t kbd_read_status(void *opaque, physical_addr_t addr, unsigned size)
{
...
static void outport_write(i8042_emu_state_t *s, uint32_t val)
{
...
static void kbd_write_command(void *opaque, physical_addr_t addr, uint64_t val, unsigned size)
{
    i8042_emu_state_t *s = opaque;
...
static uint64_t kbd_read_data(void *opaque, physical_addr_t addr, unsigned size)
{
    i8042_emu_state_t *s = opaque;
    uint32_t           val;
...
static void kbd_write_data(void *opaque, physical_addr_t addr, uint64_t val, unsigned size)
{
    i8042_emu_state_t *s = opaque;
...
static uint32_t kbd_mm_readb(i8042_emu_state_t *s, physical_addr_t addr)
{
    if (addr & s->mask) {
...
static void kbd_mm_writeb(i8042_emu_state_t *s, physical_addr_t addr, uint32_t value)
{
    if (addr & s->mask) {
...
static int i8042_reg_read(i8042_emu_state_t *s, uint32_t offset, uint32_t *dst)
{
    *dst = kbd_mm_readb(s, offset);
    return VMM_OK;
}

static int i8042_reg_write(i8042_emu_state_t *s, uint32_t offset, uint32_t src_mask, uint32_t val)
{
    kbd_mm_writeb(s, offset, val);
    return VMM_OK;
}

static int i8042_emulator_read8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;

    rc              = i8042_reg_read(edev->private, offset, &regval);
...
static int i8042_emulator_read16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;
...
static int i8042_emulator_read32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst)
{
    return i8042_reg_read(edev->private, offset, dst);
}

static int i8042_emulator_write8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t src)
{
    return i8042_reg_write(edev->private, offset, 0xFFFFFF00, src);
}

static int i8042_emulator_write16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t src)
{
    return i8042_reg_write(edev->private, offset, 0xFFFF0000, src);
}

static int i8042_emulator_write32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t src)
{
    return i8042_reg_write(edev->private, offset, 0x00000000, src);
}

static int i8042_emulator_reset(vmm_emulate_device_t *edev)
{
    int                rc = VMM_OK;
    i8042_emu_state_t *s  = edev->private;
...
static int i8042_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int                rc = VMM_OK;
    char               name[64];
    i8042_emu_state_t *s;
    ...
    rc       = vmm_device_tree_read_u32_atindex(edev->node, VMM_DEVICE_TREE_INTERRUPTS_ATTR_NAME, &s->irq_kbd, 0);

    if (rc) {
        goto i8042_emulator_probe_freestate_fail;
    }

    rc = vmm_device_tree_read_u32_atindex(edev->node, VMM_DEVICE_TREE_INTERRUPTS_ATTR_NAME, &s->irq_mouse, 1);
...
static int i8042_emulator_remove(vmm_emulate_device_t *edev)
{
    int                rc = VMM_OK;
    i8042_emu_state_t *s  = edev->private;
...
static struct vmm_device_tree_nodeid i8042_emuid_table[] = {
    {
     .type       = "input",
...
static vmm_emulator_t i8042_emulator = {
    .name        = "i8042",
...
{
    return vmm_device_emulate_register_emulator(&i8042_emulator);
}

static void __exit i8042_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&i8042_emulator);
}
