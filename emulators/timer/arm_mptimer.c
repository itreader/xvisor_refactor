...
/* Note: Must hold the timer lock before calling this function. */
static uint32_t __timer_block_counter_value(struct timer_block *timer)
{
    uint32_t ret = 0;

    if (timer->control & TIMER_CTRL_ENABLE) {
        /* How much nsecs since the timer was started */
        uint64_t cval = vmm_timer_timestamp() - timer->tstamp;
        ...
        if (timer->freq == 1000000) {
            ...
            cval = udiv64(cval * timer->freq, (uint64_t)1000000000);
        }
        ...
        if (timer->control & TIMER_CTRL_ARELOAD) {
             ...
             */
            cval = umod64(cval, (uint64_t)timer->load);
            ret  = timer->load - (uint32_t)cval;
        } else {
            if (cval >= timer->count) {
                ret = 0;
            } else {
                ret = timer->count - (uint32_t)cval;
            }
        }
    }
...
static void __timer_block_reload(struct timer_block *timer)
{
    ...
    if (timer->freq == 1000000) {
        ...
        nsecs = udiv64((nsecs * 1000000000), (uint64_t)timer->freq);
    }

    vmm_timer_event_start(&timer->event, nsecs);
}

static inline void timer_block_update_irq(struct timer_block *timer, uint32_t control)
{
    if ((control & TIMER_CTRL_ENABLE) && (control & TIMER_CTRL_IE)) {
        vmm_device_emulate_emulate_per_cpu_irq(timer->mptimer->guest, timer->irq, timer->cpu, timer->status);
    } else {
        vmm_device_emulate_emulate_per_cpu_irq(timer->mptimer->guest, timer->irq, timer->cpu, 0);
    }
}

static void timer_block_event(vmm_timer_event_t *event)
{
    uint32_t            control;
    struct timer_block *timer = event->private;
...
int mptimer_reg_write(struct mptimer_state *s, uint32_t offset, uint32_t src_mask, uint32_t src)
{
    ...
    switch (offset) {
        ...
        /* Fall through.  */
        case 0x4: /* Counter.  */
            if ((timer->control & TIMER_CTRL_ENABLE) && timer->count) {
        ...
        case 0x8: /* Control.  */
            ...
            if (((old & TIMER_CTRL_ENABLE) == 0) && (src & TIMER_CTRL_ENABLE)) {
                if (timer->count == 0 && (timer->control & TIMER_CTRL_ARELOAD)) {
        ...
        case 0x14:
            ...
            } else if ((src == 0x87654321) && (timer->wdisable == 0x12345678)) {
                timer->control &= ~TIMER_CTRL_WDM;
            } else {
                timer->wdisable = 0;
            }
    ...
    struct vmm_guest *guest, vmm_emulate_device_t *edev, uint32_t num_cpu, uint32_t periphclk, uint32_t timer_irq, uint32_t wdt_irq)
{
    ...
    s->timers   = vmm_zalloc(NUM_TIMERS_PER_CPU * s->num_cpu * sizeof(struct timer_block));
    ...
    for (i = 0; i < (NUM_TIMERS_PER_CPU * num_cpu); i++) {
        ...
        INIT_TIMER_EVENT(&s->timers[i].event, &timer_block_event, &(s->timers[i]));
    }
