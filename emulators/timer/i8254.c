...
static void pit_irq_timer_update(pit_channel_state_t *s, int64_t current_time);
...
int pit_get_out(pit_channel_state_t *s, int64_t current_time)
{
    ...
    d = muldiv64(current_time - s->count_load_time, PIT_FREQ, 1000000000ul);
...
int64_t pit_get_next_transition_time(pit_channel_state_t *s, int64_t current_time)
{
    ...
    next_time = s->count_load_time + muldiv64(next_time, 1000000000ul, PIT_FREQ);
...
static int pit_get_count(pit_channel_state_t *s)
{
    ...
    d = muldiv64(vmm_timer_timestamp() - s->count_load_time, PIT_FREQ, 1000000000UL);
...
static int pit_ioport_write(pit_common_state_t *pit, uint32_t addr, uint32_t src_mask, uint32_t val)
{
    int                  channel, access;
    pit_channel_state_t *s;
            ...
            for (channel = 0; channel < 3; channel++) {
                ...
                if (val & (2 << channel)) {
                    ...
                    if (!(val & 0x10) && !s->status_latched) {
                        ...
                        s->status         = (pit_get_out(s, vmm_timer_timestamp()) << 7) | (s->rw_mode << 4) | (s->mode << 1) | s->bcd;
                        s->status_latched = 1;
                    }
                }
            }
...
static uint64_t pit_ioport_read(pit_common_state_t *pit, uint32_t addr, uint32_t *dst)
{
    int                  ret, count;
    pit_channel_state_t *s;
...
static void pit_irq_timer_update(pit_channel_state_t *s, int64_t current_time)
{
    ...
    vmm_device_emulate_emulate_irq(s->guest, s->irq, irq_level);
#ifdef DEBUG_PIT
    vmm_printf("irq_level=%d next_delay=%f\n", irq_level, (double)(expire_time - current_time) / 1000000000ull);
#endif
    s->next_transition_time = expire_time;

    if (expire_time != -1) {
        vmm_timer_event_stop(&s->irq_timer);
        vmm_timer_event_start(&s->irq_timer, (expire_time - s->count_load_time));
    } else {
        vmm_timer_event_stop(&s->irq_timer);
    }
}

static void pit_irq_timer(vmm_timer_event_t *event)
{
    pit_channel_state_t *s = event->private;
...
static int i8254_emulator_reset(vmm_emulate_device_t *edev)
{
    pit_common_state_t *s = edev->private;
...
static int i8254_emulator_read8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;

    rc              = pit_ioport_read(edev->private, offset, &regval);
...
static int i8254_emulator_read16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;
...
static int i8254_emulator_read32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst)
{
    return pit_ioport_read(edev->private, offset, dst);
}

static int i8254_emulator_write8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t src)
{
    return pit_ioport_write(edev->private, offset, 0xFFFFFF00, src);
}

static int i8254_emulator_write16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t src)
{
    return pit_ioport_write(edev->private, offset, 0xFFFF0000, src);
}

static int i8254_emulator_write32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t src)
{
    return pit_ioport_write(edev->private, offset, 0x00000000, src);
}

static int i8254_emulator_remove(vmm_emulate_device_t *edev)
{
    struct pit_common_state_t *s = edev->private;
...
static int i8254_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int                 rc = VMM_OK;
    ...
    rc = vmm_device_tree_read_u32_atindex(edev->node, VMM_DEVICE_TREE_INTERRUPTS_ATTR_NAME, &s->channels[0].irq, 0);
...
static struct vmm_device_tree_nodeid i8254_emulator_emuid_table[] = {
    {
     .type       = "pit",
...
static vmm_emulator_t i8254_emulator = {
    .name        = "i8254",
...
{
    return vmm_device_emulate_register_emulator(&i8254_emulator);
}

static void __exit i8254_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&i8254_emulator);
}
