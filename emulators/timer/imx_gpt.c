...
struct gpt_t {
    ...
    /* OCR1, 2, and 3 events */
    vmm_timer_event_t event[3];
    /* irq number */
    uint32_t          irq;

    uint32_t control;
    uint32_t output_compare[3];
    uint32_t prescaler;
    uint32_t freq;
    uint32_t status;
...
static uint32_t imx_gpt_cnt(struct gpt_t *gpt)
{
    ...
    return (uint32_t)udiv64(vmm_timer_timestamp() - gpt->offset, gpt->freq);
}
...
static void _imx_gpt_reset(struct gpt_t *gpt)
{
    ...
    gpt->control &= GPT_CR_EN | GPT_CR_ENMOD | GPT_CR_STOPEN | GPT_CR_WAITEN | GPT_CR_DBGEN;
    gpt->prescaler = 0;
    gpt->status    = 0;
    gpt->irq_ena   = 0;
...
static int imx_gpt_emulator_read(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst, uint32_t size)
{
    struct gpt_t *gpt = edev->private;
    uint16_t      reg = offset & ~0x3;
    ...
    switch (reg) {
        ...
        default:
            vmm_lwarning(NULL, "i.MX GPT read at unknown register 0x%" PRIPADDR "\n", offset);
    }
...
static void _imx_gpt_restart_timer(struct gpt_t *gpt, int timer_idx, uint32_t timestamp)
{
    uint32_t delta   = 0;
    uint64_t delta64 = 0UL;
...
static void _imx_gpt_stop_timer(struct gpt_t *gpt, int timer_idx)
{
    vmm_timer_event_stop(&gpt->event[timer_idx]);
}

static void _imx_gpt_enable(struct gpt_t *gpt, int enable)
{
    int i  = 0;
...
static void _imx_gpt_cnt_update(struct gpt_t *gpt, int cmpidx, uint32_t *cnt)
{
...
static int imx_gpt_emulator_write(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t regmask, uint32_t regval, uint32_t size)
{
    struct gpt_t *gpt = edev->private;
    uint32_t      idx = 0;
    uint32_t      cnt = 0;
    ...
    switch (reg) {
        ...
        case GPT_PR:
            gpt->prescaler = (gpt->prescaler & regmask) | (regval & regmask & GPT_PR_MASK);
            break;
        ...
        case GPT_SR:
            ...
            if (!(gpt->status & GPT_SR_MASK)) {
                vmm_device_emulate_emulate_irq(gpt->guest, gpt->irq, 0);
            }
        ...
        case GPT_IR:
            if (regval & GPT_IR_ROVIE) {
                vmm_lwarning(NULL, "Rollover interrupt not supported\n");
            }
...
static int imx_gpt_emulator_reset(vmm_emulate_device_t *edev)
{
    struct gpt_t *gpt = edev->private;

    vmm_write_lock(&gpt->lock);
    _imx_gpt_reset(edev->private);
    vmm_write_unlock(&gpt->lock);

    return VMM_OK;
}

static void imx_gpt_event(vmm_timer_event_t *event)
{
    int           i   = 0;
    struct gpt_t *gpt = event->private;
    ...
    for (i = 0; i < ARRAY_SIZE(gpt->event); ++i) {
        if (event == &gpt->event[i]) {
            ...
            if (gpt->irq_ena & (1 << i)) {
                vmm_device_emulate_emulate_irq(gpt->guest, gpt->irq, 1);
            }
...
static int imx_gpt_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int           i, rc;
    uint32_t      freq = 0;
    ...
    rc         = vmm_device_tree_read_u32_atindex(edev->node, VMM_DEVICE_TREE_INTERRUPTS_ATTR_NAME, &gpt->irq, 0);
    ...
    rc = vmm_device_tree_read_u32(edev->node, "clock-frequency", &freq);
...
static int imx_gpt_emulator_remove(vmm_emulate_device_t *edev)
{
    struct gpt_t *gpt = edev->private;
    vmm_free(gpt);
    edev->private = NULL;
...
static struct vmm_device_tree_nodeid imx_gpt_emuid_table[] = {
    {
     .type       = "timer",
...
VMM_DECLARE_EMULATOR_SIMPLE(
    imx_gpt_emulator, "imx_gpt", imx_gpt_emuid_table, VMM_DEVICE_EMULATE_LITTLE_ENDIAN, imx_gpt_emulator_probe, imx_gpt_emulator_remove,
    imx_gpt_emulator_reset, NULL, imx_gpt_emulator_read, imx_gpt_emulator_write);

static int __init imx_gpt_emulator_init(void)
{
    int rc = vmm_device_emulate_register_emulator(&imx_gpt_emulator);
    return rc;
}

static void __exit imx_gpt_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&imx_gpt_emulator);
}
