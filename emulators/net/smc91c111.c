...
{
    smc91c111_state *s = port->private;
...
{
    smc91c111_state *s = (smc91c111_state *)port->private;

    s->link_down       = !(port->flags & VMM_NETPORT_LINK_UP);
}

static int smc91c111_switch2port_xfer(struct vmm_netport *port, struct vmm_mbuf *mbuf)
{
    int              rc = VMM_OK;
    smc91c111_state *s  = port->private;
    char            *buf;
...
static int smc91c111_emulator_read8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t *dst)
{
    int              rc = VMM_OK;
    smc91c111_state *s  = (smc91c111_state *)edev->private;
...
static int smc91c111_emulator_read16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t *dst)
{
    int     rc;
    uint8_t val;
...
static int smc91c111_emulator_read32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst)
{
    int      rc;
    uint16_t val;
...
static int smc91c111_emulator_write8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t value)
{
    int              rc = VMM_OK;
    smc91c111_state *s  = (smc91c111_state *)edev->private;
...
static int smc91c111_emulator_write16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t value)
{
    int rc;
...
static int smc91c111_emulator_write32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t value)
{
    int rc;
...
static int smc91c111_emulator_reset(vmm_emulate_device_t *edev)
{
    smc91c111_state *s = edev->private;

    return smc91c111_reset(s);
}

static int smc91c111_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int                   rc = VMM_OK, i = 0;
    char                  tname[64];
    const char           *attr;
    ...
    vmm_sprintf(tname, "%s%s%s", guest->name, VMM_DEVICE_TREE_PATH_SEPARATOR_STRING, edev->node->name);
...
static int smc91c111_emulator_remove(vmm_emulate_device_t *edev)
{
    int              rc = VMM_OK;
    smc91c111_state *s  = edev->private;
...
static struct vmm_device_tree_nodeid smc91c111_emuid_table[] = {
    {
     .type       = "nic",
...
static vmm_emulator_t smc91c111_emulator = {
    .name        = "smc91c111",
...
{
    return vmm_device_emulate_register_emulator(&smc91c111_emulator);
}

static void __exit smc91c111_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&smc91c111_emulator);
}
