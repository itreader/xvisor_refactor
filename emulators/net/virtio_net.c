...
static uint64_t virtio_net_get_host_features(struct vmm_virtio_device *dev)
{
...
#endif
           | 1UL << VMM_VIRTIO_NET_F_MQ | 1UL << VMM_VIRTIO_NET_F_CTRL_VQ;
}

static void virtio_net_set_guest_features(struct vmm_virtio_device *dev, uint32_t select, uint32_t features)
{
    ...
    ndev->features &= ~((uint64_t)UINT_MAX << (select * 32));
    ndev->features |= ((uint64_t)features << (select * 32));
}

static int virtio_net_init_vq(struct vmm_virtio_device *dev, uint32_t vq, uint32_t page_size, uint32_t align, uint32_t pfn)
{
    int                    rc;
    struct virtio_net_dev *ndev = dev->emu_data;

    rc                          = vmm_virtio_queue_setup(&ndev->vqs[vq].vq, dev->guest, pfn, page_size, VIRTIO_NET_QUEUE_SIZE, align);
...
static int virtio_net_get_pfn_vq(struct vmm_virtio_device *dev, uint32_t vq)
{
    int                    rc;
    struct virtio_net_dev *ndev = dev->emu_data;
...
static int virtio_net_get_size_vq(struct vmm_virtio_device *dev, uint32_t vq)
{
    return VIRTIO_NET_QUEUE_SIZE;
}

static int virtio_net_set_size_vq(struct vmm_virtio_device *dev, uint32_t vq, int size)
{
    /* FIXME: dynamic */
    return size;
}

static void virtio_net_tx_poke(struct virtio_net_dev *ndev, uint32_t vq);
...
static void virtio_net_tx_lazy(struct vmm_netport *port, void *arg, int budget)
{
    ...
    while ((budget > 0) && vmm_virtio_queue_available(vq)) {
        ...
        if (pkt_len <= VIRTIO_NET_MTU) {
            ...
            vmm_virtio_iovec_to_buf_read(dev, &iov[1], iov_cnt - 1, M_BUFADDR(mb), pkt_len);
...
static void virtio_net_handle_ctrl(struct virtio_net_dev *ndev, uint32_t qnum)
{
    ...
    while (vmm_virtio_queue_available(vq)) {
        ...
        if ((iov_cnt < 2) || (iov[0].len < sizeof(ctrl_hdr)) || (iov[iov_cnt - 1].len < sizeof(status))) {
            vmm_printf("%s: invalid ctrl IOV\n", __func__);
            goto skip;
        }

        vmm_virtio_iovec_to_buf_read(dev, &iov[0], 1, &ctrl_hdr, sizeof(ctrl_hdr));

        switch (ctrl_hdr.class) {
            case VMM_VIRTIO_NET_CTRL_MQ:
                if ((iov_cnt < 3) || (iov[1].len < sizeof(ctrl_mq))) {
                    vmm_printf("%s: invalid ctrl mq IOV\n", __func__);
                    goto skip;
                }

                vmm_virtio_iovec_to_buf_read(dev, &iov[1], 1, &ctrl_mq, sizeof(ctrl_mq));
    ...
    skip:
        vmm_virtio_buf_to_iovec_write(dev, &iov[iov_cnt - 1], 1, &status, 1);
...
static int virtio_net_notify_vq(struct vmm_virtio_device *dev, uint32_t vq)
{
    int                    rc   = VMM_OK;
    struct virtio_net_dev *ndev = dev->emu_data;
...
static void virtio_net_status_changed(struct vmm_virtio_device *dev, uint32_t new_status)
{
    uint32_t               i, have_rx_queue = 0;
    struct virtio_net_dev *ndev = dev->emu_data;

    for (i = 0; i < ndev->max_queues; i++) {
        if (ndev->vqs[i].valid && (ndev->vqs[i].type == VIRTIO_NET_RX_QUEUE)) {
            have_rx_queue++;
        }
    }

    if (have_rx_queue && (new_status & VMM_VIRTIO_CONFIG_S_DRIVER_OK)) {
        ndev->can_receive = 1;
    } else {
        ndev->can_receive = 0;
    }
...
{
    struct virtio_net_dev *ndev = p->private;

    return ndev->can_receive;
}

static int virtio_net_switch2port_xfer(struct vmm_netport *p, struct vmm_mbuf *mb)
{
    int                       rc;
    uint16_t                  head = 0;
    uint64_t                  iov0_addr;
    ...
    if (iov_cnt == 1) {
        vmm_virtio_buf_to_iovec_write(dev, &iov[0], 1, &hdr, sizeof(hdr));
        iov0_addr = iov[0].addr;
        iov0_len  = iov[0].len;
        iov[0].addr += sizeof(hdr);
        iov[0].len -= sizeof(hdr);
        vmm_virtio_buf_to_iovec_write(dev, &iov[0], 1, M_BUFADDR(mb), pkt_len);
        vmm_virtio_queue_set_used_elem(vq, head, sizeof(hdr) + pkt_len);
        iov[0].addr = iov0_addr;
        iov[0].len  = iov0_len;
    } else if (iov_cnt > 1) {
        vmm_virtio_buf_to_iovec_write(dev, &iov[0], 1, &hdr, sizeof(hdr));
        vmm_virtio_buf_to_iovec_write(dev, &iov[1], iov_cnt - 1, M_BUFADDR(mb), pkt_len);
...
static int virtio_net_read_config(struct vmm_virtio_device *dev, uint32_t offset, void *dst, uint32_t dst_len)
{
    struct virtio_net_dev *ndev = dev->emu_data;
...
static int virtio_net_write_config(struct vmm_virtio_device *dev, uint32_t offset, void *src, uint32_t src_len)
{
    struct virtio_net_dev *ndev = dev->emu_data;
...
static int virtio_net_connect(struct vmm_virtio_device *dev, struct vmm_virtio_emulator *emu)
{
    int                    i, rc;
    const char            *attr;
    struct virtio_net_dev *ndev;
    ...
    ndev->vdev = dev;
    vmm_snprintf(ndev->name, VMM_VIRTIO_DEVICE_MAX_NAME_LEN, "%s", dev->name);
     ...
     */
    ndev->vqs                        = vmm_zalloc(sizeof(struct virtio_net_queue) * ((ndev->config.max_virtqueue_pairs * 2) + 1));
    ...
    for (i = 0; i < ndev->max_queues; i++) {
        ...
        if (i == ndev->cq) {
            ...
            if (i % 2) {
                INIT_NETPORT_LAZY(&ndev->vqs[i].lazy, ndev->port, VIRTIO_NET_TX_LAZY_BUDGET, &ndev->vqs[i], virtio_net_tx_lazy);
    ...
    if (vmm_device_tree_read_string(dev->edev->node, "switch", &attr) == VMM_OK) {
