...
static uint32_t crc32_le(uint32_t crc, unsigned char const *p, uint32_t len)
{
    int i;
...
struct lan9118_state {
    ...
    uint32_t gpt_cfg;
    uint32_t word_swap;
    uint64_t gpt_start_tstamp;
    uint64_t free_timer_start_tstamp;
    uint32_t mac_cmd;
    uint32_t mac_data;
    uint32_t afc_cfg;
    uint32_t e2p_cmd;
...
static void lan9118_update(struct lan9118_state *s)
{
    ...
    vmm_device_emulate_emulate_irq(s->guest, s->irq, level);
}
...
static uint16_t gpt_counter_value(struct lan9118_state *s)
{
    uint16_t ret = 0xffff;

    if (s->gpt_cfg & GPT_TIMER_EN) {
        /* How much nsecs since the timer was started */
        uint64_t cval = vmm_timer_timestamp() - s->gpt_start_tstamp;
        /* LAN9118 GPT has 100us granularity i.e. 10KHz */
        cval          = udiv64(cval, 100000ULL);
        /* Always in auto-reload mode */
        cval          = umod64(cval, (uint64_t)s->gpt_count);
        ret           = s->gpt_count - (uint32_t)cval;
    }

    return ret;
}

static void gpt_event(vmm_timer_event_t *event)
{
    struct lan9118_state *s = event->private;
    gpt_reload(s, 0);
...
#ifdef DEBUG_LAN9118
    char tname[30];
    DPRINTF("MAC of \"%s\" changed to [%s]\n", s->port->name, ethaddr_to_str(tname, vmm_netport_mac(s->port)));
#endif
}
...
{
    struct lan9118_state *s = (struct lan9118_state *)port->private;
...
static int lan9118_emulator_reset(vmm_emulate_device_t *edev)
{
    struct lan9118_state *s = edev->private;
    return lan9118_state_reset(s);
}

static void rx_fifo_push(struct lan9118_state *s, uint32_t val)
{
    int fifo_pos;
...
static int lan9118_filter(struct lan9118_state *s, const uint8_t *addr)
{
    int      multicast;
    uint32_t hash;
    ...
    if (multicast ? (s->mac_cr & MAC_CR_HPFILT) == 0 : (s->mac_cr & MAC_CR_HO) == 0) {
        /* Exact matching.  */
...
{
    struct lan9118_state *s = port->private;
    return ((s->mac_cr & MAC_CR_RXEN) != 0);
}

static uint32_t lan9118_receive(struct lan9118_state *s, struct vmm_mbuf *mbuf)
{
    int      fifo_len;
    int      offset;
    int      src_pos;
    ...
    uint8_t *buf  = mtod(mbuf, uint8_t *);
    uint32_t size = mbuf->m_len;
...
static int lan9118_switch2port_xfer(struct vmm_netport *port, struct vmm_mbuf *mbuf)
{
    int                   rc = VMM_OK;
    struct lan9118_state *s  = port->private;
    char                 *buf;
...
static uint32_t rx_fifo_pop(struct lan9118_state *s)
{
    int      n;
    uint32_t val;
...
static void do_tx_packet(struct lan9118_state *s)
{
    ...
    DPRINTF("LAN9118: %s pkt with srcaddr[%s]", __func__, ethaddr_to_str(tname, ether_srcmac(mtod(mbuf, uint8_t *))));
    DPRINTF(", dstaddr[%s]", ethaddr_to_str(tname, ether_dstmac(mtod(mbuf, uint8_t *))));
    DPRINTF(", ethertype: 0x%04X\n", ether_type(mtod(mbuf, uint8_t *)));
    DPRINTF("LAN9118: %s[mbuf(data: 0x%X, len: %d)] to ", __func__, txp_mbuf(s)->m_data, txp_mbuf(s)->m_len);
#endif
...
static uint32_t rx_status_fifo_pop(struct lan9118_state *s)
{
    uint32_t val;
...
static uint32_t tx_status_fifo_pop(struct lan9118_state *s)
{
    uint32_t val;
...
static void tx_fifo_push(struct lan9118_state *s, uint32_t val)
{
    int n;
    ...
    switch (s->txp->state) {
        ...
        case TX_B:
            ...
            DPRINTF("Block len:%d offset:%d pad:%d cmd %08x\n", s->txp->buffer_size, s->txp->offset, s->txp->pad, s->txp->cmd_a);
            s->txp->state = TX_DATA;
            break;
        ...
        case TX_DATA:
            ...
            if (s->txp->buffer_size <= 0 && s->txp->pad != 0) {
                ...
                while (n--) {
                    *(mtod(txp_mbuf(s), uint8_t *) + txp_mbuf_len(s)) = val & 0xff;
                    txp_mbuf_len(s)++;
                    val >>= 8;
                    s->txp->buffer_size--;
                }
...
static uint32_t do_phy_read(struct lan9118_state *s, int reg)
{
    uint32_t val;
...
static void do_phy_write(struct lan9118_state *s, int reg, uint32_t val)
{
    switch (reg) {
        case 0: /* Basic Control */
            if (val & 0x8000) {
...
static void do_mac_write(struct lan9118_state *s, int reg, uint32_t val)
{
    switch (reg) {
        case MAC_CR:
...
static uint32_t do_mac_read(struct lan9118_state *s, int reg)
{
    switch (reg) {
        case MAC_CR:
            return s->mac_cr;
        ...
        case MAC_ADDRL:
            return vmm_netport_mac(s->port)[0] | (vmm_netport_mac(s->port)[1] << 8) | (vmm_netport_mac(s->port)[2] << 16) |
                   (vmm_netport_mac(s->port)[3] << 24);
...
static int lan9118_reg_write(struct lan9118_state *s, physical_addr_t offset, uint32_t src_mask, uint32_t src)
{
    bool do_reset = FALSE;
    offset &= 0xff;
    src = src & ~src_mask;
...
static int lan9118_reg_read(struct lan9118_state *s, physical_addr_t offset, uint32_t *dst)
{
    uint64_t cval;
    ...
    switch (offset) {
        ...
        case CSR_TX_FIFO_INF:
            *dst = (s->tx_status_fifo_used << 16) | (s->tx_fifo_size - s->txp->fifo_used);
            break;
        ...
        case CSR_FREE_RUN:
            cval = udiv64((vmm_timer_timestamp() - s->free_timer_start_tstamp), (uint64_t)40);
...
static int lan9118_emulator_read8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;

    rc              = lan9118_reg_read(edev->private, offset, &regval);
...
static int lan9118_emulator_read16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;
...
static int lan9118_emulator_read32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst)
{
    return lan9118_reg_read(edev->private, offset, dst);
}

static int lan9118_emulator_write8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t src)
{
    return lan9118_reg_write(edev->private, offset, 0xFFFFFF00, src);
}

static int lan9118_emulator_write16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t src)
{
    return lan9118_reg_write(edev->private, offset, 0xFFFF0000, src);
}

static int lan9118_emulator_write32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t src)
{
    return lan9118_reg_write(edev->private, offset, 0x00000000, src);
}

static int lan9118_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int                   i, rc = VMM_OK;
    char                  tname[64];
    const char           *attr;
    ...
    if (vmm_device_tree_read_string(edev->node, "switch", &attr) == VMM_OK) {
...
static int lan9118_emulator_remove(vmm_emulate_device_t *edev)
{
    struct lan9118_state *s = edev->private;
...
static struct vmm_device_tree_nodeid lan9118_emuid_table[] = {
    {
     .type       = "nic",
...
static vmm_emulator_t lan9118_emulator = {
    .name        = "lan9118",
...
{
    return vmm_device_emulate_register_emulator(&lan9118_emulator);
}

static void __exit lan9118_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&lan9118_emulator);
}
