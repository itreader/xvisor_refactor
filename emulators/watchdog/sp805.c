...
#define sp805_error(SP805, ...)   sp805_msg(VMM_LOGLEVEL_ERROR, SP805, "Error - " __VA_ARGS__)

#define sp805_warning(SP805, ...) sp805_msg(VMM_LOGLEVEL_ERROR, SP805, "Warning - " __VA_ARGS__)

#define sp805_debug(SP805, ...)   sp805_msg(VMM_LOGLEVEL_ERROR, SP805, __VA_ARGS__)

#else /* !DEBUG */

#define sp805_error(SP805, ...)   sp805_msg(VMM_LOGLEVEL_ERROR, SP805, "Error - " __VA_ARGS__)

#define sp805_warning(SP805, ...) sp805_msg(VMM_LOGLEVEL_WARNING, SP805, "Warning - " __VA_ARGS__)
...
{
    int      rc     = VMM_OK;
    uint64_t reload = (sp805->load + 1) * 1000;
...
/* Must be called with sp805->lock held */
static uint32_t _sp805_reg_value(struct sp805_state *sp805)
{
    uint64_t load = vmm_timer_timestamp();
...
static int sp805_reg_read(struct sp805_state *sp805, physical_addr_t offset, uint32_t *dst)
{
    int ret = VMM_OK;
...
static int sp805_emulator_read8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t *dst)
{
    uint32_t val   = 0;
    int      ret   = VMM_OK;
    int      shift = (offset & 0x3) * 8;

    ret            = sp805_reg_read(edev->private, offset, &val);
...
static int sp805_emulator_read16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t *dst)
{
    uint32_t val   = 0;
    int      ret   = VMM_OK;
...
static int sp805_emulator_read32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst)
{
    uint32_t val = 0;
    int      ret = VMM_OK;
...
static int sp805_reg_write(struct sp805_state *sp805, physical_addr_t offset, uint32_t val)
{
    int ret = VMM_OK;
        ...
        case WDT_CTRL:
            if (likely(val & WDT_CTRL_INTEN)) {
                ...
                if (unlikely(sp805->irq_level)) {
                    ...
                    vmm_device_emulate_emulate_irq(sp805->guest, sp805->irq, 0);
                }
            }
...
static int sp805_emulator_write8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t src)
{
    uint32_t            val  = 0;
    uint32_t            mask = 0xFF;
    int                 rc, shift = (offset & 3) * 8;
    struct sp805_state *sp805 = edev->private;
...
static int sp805_emulator_write16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t src)
{
    uint32_t            val  = 0;
...
static int sp805_emulator_write32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t src)
{
    int                 ret   = VMM_OK;
    struct sp805_state *sp805 = edev->private;
...
static int sp805_emulator_reset(vmm_emulate_device_t *edev)
{
    struct sp805_state *sp805 = edev->private;
...
static void sp805_emulator_event(vmm_timer_event_t *evt)
{
    struct sp805_state *sp805 = evt->private;
    ...
    if (sp805->ctrl & WDT_CTRL_INTEN) {
        if (unlikely(sp805->irq_level)) {
            ...
            vmm_spin_lock(&sp805->lock);
            sp805->irq_level = 1;
            vmm_device_emulate_emulate_irq(sp805->guest, sp805->irq, 1);
            vmm_spin_unlock(&sp805->lock);
...
static int sp805_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int                 rc;
    ...
    rc = vmm_device_tree_read_u32_atindex(edev->node, VMM_DEVICE_TREE_INTERRUPTS_ATTR_NAME, &sp805->irq, 0);
...
static int sp805_emulator_remove(vmm_emulate_device_t *edev)
{
    struct sp805_state *sp805 = edev->private;
...
static struct vmm_device_tree_nodeid sp805_emuid_table[] = {
    {
     .type       = "watchdog",
...
static vmm_emulator_t sp805_emulator = {
    .name        = "sp805",
...
{
    return vmm_device_emulate_register_emulator(&sp805_emulator);
}

static void __exit sp805_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&sp805_emulator);
}
