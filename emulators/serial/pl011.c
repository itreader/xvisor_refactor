...
static int pl011_reg_write(struct pl011_state *s, uint32_t offset, uint32_t src_mask, uint32_t src)
{
    uint8_t  val;
    int      rc        = VMM_OK;
    bool     set_irq   = FALSE;
    ...
    switch (offset >> 2) {
        ...
        case 14: /* UARTIMSC */
            s->int_enabled = (s->int_enabled & src_mask) | (src & ~src_mask);
            set_irq        = TRUE;
            level          = s->int_level;
            enabled        = s->int_enabled;
            break;
...
{
    struct pl011_state *s = vmm_vserial_private(vser);
#if 0
    uint32_t rd_count;
...
static int pl011_emulator_read8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;

    rc              = pl011_reg_read(edev->private, offset, &regval);
...
static int pl011_emulator_read16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;
...
static int pl011_emulator_read32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst)
{
    return pl011_reg_read(edev->private, offset, dst);
}

static int pl011_emulator_write8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t src)
{
    return pl011_reg_write(edev->private, offset, 0xFFFFFF00, src);
}

static int pl011_emulator_write16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t src)
{
    return pl011_reg_write(edev->private, offset, 0xFFFF0000, src);
}

static int pl011_emulator_write32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t src)
{
    return pl011_reg_write(edev->private, offset, 0x00000000, src);
}

static int pl011_emulator_reset(vmm_emulate_device_t *edev)
{
    struct pl011_state *s = edev->private;
...
static int pl011_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int                 rc = VMM_OK;
    char                name[64];
    ...
    rc = vmm_device_tree_read_u32(edev->node, "fifo_size", &s->fifo_sz);
    ...
    s->vser = vmm_vserial_create(name, &pl011_vserial_can_send, &pl011_vserial_send, s->fifo_sz, s);
...
static int pl011_emulator_remove(vmm_emulate_device_t *edev)
{
    struct pl011_state *s = edev->private;
...
static struct vmm_device_tree_nodeid pl011_emuid_table[] = {
    {
     .type       = "serial",
...
{
    return vmm_device_emulate_register_emulator(&pl011_emulator);
}

static void __exit pl011_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&pl011_emulator);
}
