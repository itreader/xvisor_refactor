...
static int ns16550_reg_write(struct ns16550_state *s, uint32_t addr, uint32_t src_mask, uint32_t val, uint32_t io_width)
{
    uint8_t data8;
    int     ret        = VMM_OK;
...
static void __ns16550_recv(struct ns16550_state *s, uint32_t *dst)
{
    uint8_t data8 = 0;
...
static int ns16550_reg_read(struct ns16550_state *s, uint32_t addr, uint32_t *dst, uint32_t io_width)
{
    int  ret        = VMM_OK;
    bool update_irq = FALSE;
...
{
    bool                  ret;
    struct ns16550_state *s = vmm_vserial_private(vser);
...
static int ns16550_send(struct vmm_vserial *vser, uint8_t data)
{
    struct ns16550_state *s = vmm_vserial_private(vser);
...
static int ns16550_emulator_reset(vmm_emulate_device_t *edev)
{
    struct ns16550_state *s = edev->private;
...
static int ns16550_emulator_read8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;

    rc              = ns16550_reg_read(edev->private, offset, &regval, 1);
...
static int ns16550_emulator_read16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;

    rc              = ns16550_reg_read(edev->private, offset, &regval, 2);
...
static int ns16550_emulator_read32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst)
{
    return ns16550_reg_read(edev->private, offset, dst, 4);
}

static int ns16550_emulator_write8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t src)
{
    return ns16550_reg_write(edev->private, offset, 0xFFFFFF00, src, 1);
}

static int ns16550_emulator_write16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t src)
{
    return ns16550_reg_write(edev->private, offset, 0xFFFF0000, src, 2);
}

static int ns16550_emulator_write32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t src)
{
    return ns16550_reg_write(edev->private, offset, 0x00000000, src, 4);
}

static int ns16550_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int                   rc = VMM_OK;
    char                  name[64];
    ...
    if (rc) {
        vmm_lerror(edev->node->name, "Failed to get serial IRQ entry in guest DTS.\n");
        goto uart16550a_emulator_probe_freestate_fail;
    }

    if (vmm_device_tree_read_u32(edev->node, "reg_shift", &s->reg_shift)) {
        s->reg_shift = 0;
    }

    if (vmm_device_tree_read_u32(edev->node, "reg_io_width", &s->reg_io_width)) {
        s->reg_io_width = 1;
    }
    ...
    s->vser = vmm_vserial_create(name, &ns16550_can_send, &ns16550_send, 2048, s);

    if (!(s->vser)) {
        vmm_lerror(edev->node->name, "Failed to create vserial instance.\n");
        goto uart16550a_emulator_probe_freerbuf_fail;
    }
...
static int ns16550_emulator_remove(vmm_emulate_device_t *edev)
{
    struct ns16550_state *s = edev->private;
...
static struct vmm_device_tree_nodeid ns16550_emuid_table[] = {
    {
     .type       = "serial",
     .compatible = "ns16550a",
     },
...
static vmm_emulator_t ns16550_emulator = {
...
{
    return vmm_device_emulate_register_emulator(&ns16550_emulator);
}

static void __exit ns16550_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&ns16550_emulator);
}
