...
struct imx_state {
    ...
    uint32_t                    rdirq;
    struct fifo                *rd_fifo;
    uint16_t                    regs[(IMX21_ONEMS - UCR1) / 4 + 1];
    uint16_t                    uts;
    uint8_t                     tx;
};
...
static inline uint32_t _reg_offset_get(uint32_t reg)
{
    return (reg - UCR1) / 4;
}

static uint16_t _reg_read(const struct imx_state *s, uint32_t reg)
{
    const uint32_t offset = _reg_offset_get(reg);

    BUG_ON(offset >= ARRAY_SIZE(s->regs));
    return s->regs[offset];
}

static void _reg_write(struct imx_state *s, uint32_t reg, uint16_t val)
{
    const uint32_t offset = _reg_offset_get(reg);
...
static void _reg_set_mask(struct imx_state *s, uint32_t reg, uint16_t mask)
{
    const uint32_t offset = _reg_offset_get(reg);
...
static void _reg_clear_mask(struct imx_state *s, uint32_t reg, uint16_t mask)
{
    const uint32_t offset = _reg_offset_get(reg);
...
static void _reg_ack(struct imx_state *s, uint32_t reg, uint16_t mask)
{
    uint16_t       tmp    = 0;
    const uint32_t offset = _reg_offset_get(reg);
...
{
    vmm_device_emulate_emulate_irq(s->guest, s->rdirq, level);
}

static int _imx_reg_urxd0(struct imx_state *s, uint32_t *dst, int nb)
{
    int      i          = 0;
    uint32_t read_count = 0x0;
    uint8_t  val;
    ...
    if ((_reg_read(s, USR1) & USR1_RRDY) && (_reg_read(s, UCR1) & (UCR1_RRDYEN | UCR1_RDMAEN))) {
        return 1;
    }
...
static int imx_reg_read(struct imx_state *s, uint32_t offset, uint32_t *dst, int nb)
{
    int            rc      = VMM_OK;
    const uint32_t reg     = _reg_offset_get(offset);
    int            level   = 0;
    bool           set_irq = FALSE;
...
{
    vmm_device_emulate_emulate_irq(s->guest, s->txirq, level);
}

static int imx_reg_write(struct imx_state *s, uint32_t offset, uint32_t src_mask, uint32_t src)
{
    const uint32_t reg       = _reg_offset_get(offset);
    uint16_t       ack       = 0;
    uint16_t       usr1      = 0;
    ...
    uint16_t       val       = (uint16_t)(src & ~src_mask);
    ...
    if ((_reg_read(s, USR1) & USR1_TRDY) && (_reg_read(s, UCR1) & UCR1_TRDYEN)) {
        /*
...
static int imx_vserial_send(struct vmm_vserial *vser, uint8_t data)
{
    bool              set_irq = FALSE;
    uint32_t          rd_count;
    struct imx_state *s = vmm_vserial_private(vser);
...
static int imx_emulator_read8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;

    rc              = imx_reg_read(edev->private, offset, &regval, 1);
...
static int imx_emulator_read16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;

    rc              = imx_reg_read(edev->private, offset, &regval, 2);
...
static int imx_emulator_read32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst)
{
    return imx_reg_read(edev->private, offset, dst, 4);
}

static int imx_emulator_write8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t src)
{
    return imx_reg_write(edev->private, offset, 0xFFFFFF00, src);
}

static int imx_emulator_write16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t src)
{
    return imx_reg_write(edev->private, offset, 0xFFFF0000, src);
}

static int imx_emulator_write32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t src)
{
    return imx_reg_write(edev->private, offset, 0x00000000, src);
}

static int imx_emulator_reset(vmm_emulate_device_t *edev)
{
    struct imx_state *s = edev->private;
...
static int imx_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int               rc = VMM_OK;
    char              name[64];
    struct imx_state *s;
    ...
    rc      = vmm_device_tree_read_u32_atindex(edev->node, VMM_DEVICE_TREE_INTERRUPTS_ATTR_NAME, &s->rdirq, 0);

    if (rc) {
        goto imx_emulator_probe_freestate_fail;
    }

    rc = vmm_device_tree_read_u32_atindex(edev->node, VMM_DEVICE_TREE_INTERRUPTS_ATTR_NAME, &s->txirq, 1);
    ...
    s->vser = vmm_vserial_create(name, &imx_vserial_can_send, &imx_vserial_send, IMX_FIFO_SIZE, s);
...
static int imx_emulator_remove(vmm_emulate_device_t *edev)
{
    struct imx_state *s = edev->private;
...
static struct vmm_device_tree_nodeid imx_emuid_table[] = {
    {.type = "serial", .compatible = "fsl,imx1-uart", .data = &imx_uart_devdata[IMX1_UART]},
    {
     .type       = "serial",
...
static vmm_emulator_t imx_emulator = {
    .name        = "imx",
...
{
    return vmm_device_emulate_register_emulator(&imx_emulator);
}

static void __exit imx_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&imx_emulator);
}
