...
struct arm_sysregs {
    ...
    uint64_t          ref_100hz;
    uint64_t          ref_24mhz;
    uint32_t          mux_in_irq[2];
    uint32_t          mux_out_irq;
...
static int arm_sysregs_reg_read(struct arm_sysregs *s, uint32_t offset, uint32_t *dst)
{
    int      rc = VMM_OK;
    uint64_t tdiff;
 ...
 */
static bool vexpress_cfgctrl_read(
    struct arm_sysregs *s, uint32_t dcc, uint32_t function, uint32_t site, uint32_t position, uint32_t device, uint32_t *val)
{
    /* We don't support anything other than DCC 0, board stack position 0
     * or sites other than motherboard/daughterboard:
     */
    if (dcc != 0 || position != 0 || (site != SYS_CFG_SITE_MB && site != SYS_CFG_SITE_DB1)) {
        goto cfgctrl_unimp;
    }
...
cfgctrl_unimp:
    vmm_printf(
        "%s: Unimplemented SYS_CFGCTRL read of function "
 ...
 * @val: data to write
 */
static bool vexpress_cfgctrl_write(
    struct arm_sysregs *s, uint32_t dcc, uint32_t function, uint32_t site, uint32_t position, uint32_t device, uint32_t val)
{
    /* We don't support anything other than DCC 0, board stack position 0
     * or sites other than motherboard/daughterboard:
...
cfgctrl_unimp:
    vmm_printf(
        "%s: Unimplemented SYS_CFGCTRL write of function "
        "0x%x DCC 0x%x site 0x%x position 0x%x device 0x%x\n",
        __func__, function, dcc, site, position, device);
...
static int arm_sysregs_reg_write(struct arm_sysregs *s, uint32_t offset, uint32_t regmask, uint32_t regval)
{
    int rc = VMM_OK;
    ...
    switch (offset & ~0x3) {
        ...
        case 0x50: /* CLCD */
            switch (board_id(s)) {
                case BOARD_ID_PB926:
                    ...
                    vmm_device_emulate_emulate_irq(s->guest, s->mux_out_irq, regval & 0x3);
                    break;
        ...
        case 0xa4: /* SYS_CFGCTRL */
            ...
            if (s->sys_cfgctrl & (1 << 31)) {
                ...
                uint32_t dcc      = (s->sys_cfgctrl >> 26) & 0xF;
                uint32_t function = (s->sys_cfgctrl >> 20) & 0x3F;
                uint32_t site     = (s->sys_cfgctrl >> 16) & 0x3;
                uint32_t position = (s->sys_cfgctrl >> 12) & 0xF;
                uint32_t device   = (s->sys_cfgctrl >> 0) & 0xFFF;
                s->sys_cfgstat    = 1; /* complete */

                if (s->sys_cfgctrl & (1 << 30)) {
                    if (!vexpress_cfgctrl_write(s, dcc, function, site, position, device, s->sys_cfgdata)) {
                        s->sys_cfgstat |= 2; /* error */
                    }
                } else {
                    uint32_t val;

                    if (!vexpress_cfgctrl_read(s, dcc, function, site, position, device, &val)) {
                        s->sys_cfgstat |= 2; /* error */
                    } else {
                        s->sys_cfgdata = val;
                    }
...
static int arm_sysregs_emulator_read8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;

    rc              = arm_sysregs_reg_read(edev->private, offset, &regval);
...
static int arm_sysregs_emulator_read16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;
...
static int arm_sysregs_emulator_read32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst)
{
    return arm_sysregs_reg_read(edev->private, offset, dst);
}

static int arm_sysregs_emulator_write8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t src)
{
    return arm_sysregs_reg_write(edev->private, offset, 0xFFFFFF00, src);
}

static int arm_sysregs_emulator_write16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t src)
{
    return arm_sysregs_reg_write(edev->private, offset, 0xFFFF0000, src);
}

static int arm_sysregs_emulator_write32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t src)
{
    return arm_sysregs_reg_write(edev->private, offset, 0x00000000, src);
}

static int arm_sysregs_emulator_reset(vmm_emulate_device_t *edev)
{
    int                 i;
...
static void arm_sysregs_irq_handle(uint32_t irq, int cpu, int level, void *opaque)
{
    int                 bit;
...
struct vmm_device_emulation_irqchip arm_sysregs_irqchip = {
    .name   = "ARM_SYSREGS",
    .handle = arm_sysregs_irq_handle,
};

static int arm_sysregs_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int                 i, j, rc = VMM_OK;
    ...
    if (eid->data) {
        i                  = 0;
        s->sys_id          = ((uint32_t *)eid->data)[i++];
        s->proc_id         = ((uint32_t *)eid->data)[i++];
        s->db_num_vsensors = ((uint32_t *)eid->data)[i++];
        ...
        if (s->db_num_vsensors) {
            ...
            for (j = 0; j < s->db_num_vsensors; j++) {
                s->db_voltage[j] = ((uint32_t *)eid->data)[i++];
            }
        }

        s->db_num_clocks = ((uint32_t *)eid->data)[i++];
        ...
        if (s->db_num_clocks) {
            ...
            s->db_clock_reset = vmm_zalloc(sizeof(uint32_t) * s->db_num_clocks);
            ...
            for (j = 0; j < s->db_num_clocks; j++) {
                s->db_clock_reset[j] = ((uint32_t *)eid->data)[i++];
            }
        }
    }

    rc = vmm_device_tree_read_u32_array(edev->node, "mux_in_irq", s->mux_in_irq, array_size(s->mux_in_irq));

    if (rc) {
        goto arm_sysregs_emulator_probe_freeclock_fail;
    }

    rc = vmm_device_tree_read_u32(edev->node, "mux_out_irq", &s->mux_out_irq);

    if (rc) {
        goto arm_sysregs_emulator_probe_freeclock_fail;
    }

    vmm_device_emulate_register_irqchip(guest, s->mux_in_irq[0], &arm_sysregs_irqchip, s);
    vmm_device_emulate_register_irqchip(guest, s->mux_in_irq[1], &arm_sysregs_irqchip, s);
...
static int arm_sysregs_emulator_remove(vmm_emulate_device_t *edev)
{
    struct arm_sysregs *s = edev->private;
    ...
    vmm_device_emulate_unregister_irqchip(s->guest, s->mux_in_irq[1], &arm_sysregs_irqchip, s);
...
static uint32_t versatilepb_sysids[] = {
    /* === VersatilePB === */
...
static uint32_t realview_ebmpcore_sysids[] = {
    /* === Realview-EB-MPCore === */
...
static uint32_t realview_pba8_sysids[] = {
    /* === Realview-PB-aA8 === */
...
static uint32_t vexpress_a9_sysids[] = {
    /* === VExpress-A9 === */
...
static uint32_t vexpress_a15_sysids[] = {
    /* === VExpress-A15 === */
...
static struct vmm_device_tree_nodeid arm_sysregs_emuid_table[] = {
    {.type = "sys", .compatible = "versatilepb,arm926", .data = &versatilepb_sysids[0]},
    {.type = "sys", .compatible = "realview,eb-mpcore", .data = &realview_ebmpcore_sysids[0]},
    {.type = "sys", .compatible = "realview,pb-a8", .data = &realview_pba8_sysids[0]},
    {.type = "sys", .compatible = "vexpress,a9", .data = &vexpress_a9_sysids[0]},
    {.type = "sys", .compatible = "vexpress,a15", .data = &vexpress_a15_sysids[0]},
    {/* end of list */},
};

static vmm_emulator_t arm_sysregs_emulator = {
    .name        = "arm_sysregs",
...
{
    return vmm_device_emulate_register_emulator(&arm_sysregs_emulator);
}

static void __exit arm_sysregs_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&arm_sysregs_emulator);
}
