...
#define MODULE_IPRIORITY                (VMM_VMSG_IPRIORITY + VMM_VIRTIO_IPRIORITY + 1)
#define MODULE_INIT                     virtio_rpmsg_init
#define MODULE_EXIT                     virtio_rpmsg_exit

#define VIRTIO_RPMSG_MAX_BUFF_SIZE      512
#define VIRTIO_RPMSG_NODE_MAX_BUFF_SIZE (VIRTIO_RPMSG_MAX_BUFF_SIZE - sizeof(struct vmm_rpmsg_hdr))
...
static uint64_t virtio_rpmsg_get_host_features(struct vmm_virtio_device *dev)
{
    return 1UL << VMM_VIRTIO_RPMSG_F_NS;
}

static void virtio_rpmsg_set_guest_features(struct vmm_virtio_device *dev, uint32_t select, uint32_t features)
{
    ...
    rdev->features &= ~((uint64_t)UINT_MAX << (select * 32));
    rdev->features |= ((uint64_t)features << (select * 32));
}

static int virtio_rpmsg_init_vq(struct vmm_virtio_device *dev, uint32_t vq, uint32_t page_size, uint32_t align, uint32_t pfn)
{
    int                      rc;
    struct virtio_rpmsg_dev *rdev = dev->emu_data;
    ...
    switch (vq) {
        ...
        case VIRTIO_RPMSG_TX_QUEUE:
            rc = vmm_virtio_queue_setup(&rdev->vqs[vq], dev->guest, pfn, page_size, VIRTIO_RPMSG_QUEUE_SIZE, align);
            break;
...
static int virtio_rpmsg_get_pfn_vq(struct vmm_virtio_device *dev, uint32_t vq)
{
    int                      rc;
    struct virtio_rpmsg_dev *rdev = dev->emu_data;
...
static int virtio_rpmsg_get_size_vq(struct vmm_virtio_device *dev, uint32_t vq)
{
    int rc;
...
static int virtio_rpmsg_set_size_vq(struct vmm_virtio_device *dev, uint32_t vq, int size)
{
    /* FIXME: dynamic */
    return size;
}
...
{
    struct virtio_rpmsg_dev  *rdev = m->private;
    struct virtio_rpmsg_buf  *buf  = container_of(m, struct virtio_rpmsg_buf, msg);
...
void virtio_rpmsg_tx_msgs(struct vmm_vmsg_node *node, void *data, int budget)
{
    ...
    while ((budget > 0) && vmm_virtio_queue_available(vq)) {
        ...
        for (i = 0; i < iov_cnt; i++) {
            ...
            if ((tiov.len < sizeof(hdr)) || (VIRTIO_RPMSG_MAX_BUFF_SIZE < tiov.len)) {
                continue;
            }

            len = vmm_virtio_iovec_to_buf_read(dev, &tiov, 1, &hdr, sizeof(hdr));
            ...
            if (!tiov.len || (hdr.dst < VMM_VMSG_NODE_ADDR_MIN) || (hdr.len != tiov.len)) {
                continue;
            }
            ...
            INIT_VMSG(msg, hdr.dst, rdev->node->addr, hdr.src, &buf->data[0], hdr.len, rdev, NULL, virtio_rpmsg_free_hdr);

            len = vmm_virtio_iovec_to_buf_read(dev, &tiov, 1, msg->data, msg->len);
            ...
            DPRINTF(
                "%s: node=%s addr=0x%x src=0x%x dst=0x%x "
                "local=0x%x len=0x%zx\n",
                __func__, rdev->node->name, rdev->node->addr, msg->src, msg->dst, msg->local, msg->len);
...
static int virtio_rpmsg_notify_vq(struct vmm_virtio_device *dev, uint32_t vq)
{
    int                      rc   = VMM_OK;
    struct virtio_rpmsg_dev *rdev = dev->emu_data;
...
static void virtio_rpmsg_status_changed(struct vmm_virtio_device *dev, uint32_t new_status)
{
    struct virtio_rpmsg_dev *rdev = dev->emu_data;
...
static int virtio_rpmsg_rx_msg(
    struct virtio_rpmsg_dev *rdev, uint32_t src, uint32_t dst, uint32_t local, void *msg, uint16_t len, bool use_local_as_dst)
{
    int                       rc;
    uint16_t                  head = 0;
    uint32_t                  pos, iov_cnt = 0, total_len = 0;
    ...
    DPRINTF(
        "%s: node=%s src=0x%x dst=0x%x local=0x%x len=0x%x "
        "use_local_as_dst=%d\n",
    ...
    pos          = vmm_virtio_buf_to_iovec_write(dev, &iov[0], 1, &hdr, sizeof(hdr));
    iov[0].addr += pos;
    iov[0].len -= pos;
...
static void virtio_rpmsg_peer_up(struct vmm_vmsg_node *node, const char *peer_name, uint32_t peer_addr)
{
    int                      rc;
    struct vmm_rpmsg_ns_msg  nsmsg;
    struct virtio_rpmsg_dev *rdev = vmm_vmsg_node_private(node);
    ...
    rc = virtio_rpmsg_rx_msg(rdev, VMM_VIRTIO_RPMSG_NS_ADDR, VMM_VIRTIO_RPMSG_NS_ADDR, VMM_VIRTIO_RPMSG_NS_ADDR, &nsmsg, sizeof(nsmsg), FALSE);
...
static void virtio_rpmsg_peer_down(struct vmm_vmsg_node *node, const char *peer_name, uint32_t peer_addr)
{
    int                      rc;
    struct vmm_rpmsg_ns_msg  nsmsg;
    struct virtio_rpmsg_dev *rdev = vmm_vmsg_node_private(node);
...
static int virtio_rpmsg_recv_msg(struct vmm_vmsg_node *node, struct vmm_vmsg *msg)
{
    struct virtio_rpmsg_dev *rdev = vmm_vmsg_node_private(node);

    return virtio_rpmsg_rx_msg(rdev, msg->src, msg->dst, msg->local, msg->data, (uint32_t)msg->len, TRUE);
}

static int virtio_rpmsg_read_config(struct vmm_virtio_device *dev, uint32_t offset, void *dst, uint32_t dst_len)
{
    /* No config reads */
...
static int virtio_rpmsg_connect(struct vmm_virtio_device *dev, struct vmm_virtio_emulator *emu)
{
    uint32_t                 addr, page_count;
    ...
    if (vmm_device_tree_read_string(dev->edev->node, VMM_DEVICE_TREE_DOMAIN_ATTR_NAME, &dom_name)) {
        dom_name = NULL;
    }
    ...
    if (vmm_device_tree_read_string(dev->edev->node, VMM_DEVICE_TREE_NODE_NS_NAME_ATTR_NAME, &ns_name)) {
        rdev->node_ns_name_avail = FALSE;
    } else {
        strncpy(rdev->node_ns_name, ns_name, sizeof(rdev->node_ns_name));
        rdev->node_ns_name_avail = TRUE;
    }

    if (vmm_device_tree_read_u32(dev->edev->node, VMM_DEVICE_TREE_NODE_ADDR_ATTR_NAME, &addr)) {
        addr = VMM_VMSG_NODE_ADDR_ANY;
    }

    page_count        = VMM_SIZE_TO_PAGE(sizeof(struct virtio_rpmsg_buf) * VIRTIO_RPMSG_QUEUE_SIZE);
    rdev->tx_buf_pool = mempool_ram_create(sizeof(struct virtio_rpmsg_buf), page_count, VMM_PAGE_POOL_NORMAL);
