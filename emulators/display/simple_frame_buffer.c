...
#define MODULE_INIT                     simple_frame_buffer_emulator_init
#define MODULE_EXIT                     simple_frame_buffer_emulator_exit

#define SIMPLE_FRAME_BUFFER_MAGIC       0xa4a297a6 /* virt */
#define SIMPLE_FRAME_BUFFER_VENDOR      0x52535658 /* XVSR */
#define SIMPLE_FRAME_BUFFER_VERSION_0_1 0x00000001

struct simple_frame_buffer_state {
...
static void simple_frame_buffer_display_invalidate(struct vmm_virtual_display *vdis)
{
    vmm_virtual_display_surface_gfx_clear(vdis);
}

static int simple_frame_buffer_display_pixeldata(
...
static void simple_frame_buffer_display_update(struct vmm_virtual_display *vdis, struct vmm_surface *sf)
{
    drawfn                           *fntable;
    ...
    vmm_surface_update(
        sf, s->guest, gphys, width, height, src_width, dest_width, 0, fntable[DRAWFN_FNTABLE_INDEX(fmt, order, bppmode)], NULL, &first, &last);

    if (first >= 0) {
        vmm_virtual_display_surface_gfx_update(vdis, 0, first, width, last - first + 1);
    }
}

static int simple_frame_buffer_emulator_read(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst, uint32_t size)
{
    int                               rc = VMM_OK;
    ...
    switch (offset) {
        ...
        case 0x48:
        case 0x4c:
            *dst = (uint32_t)s->mode[(offset - 0x10) >> 2];
            break;
...
static struct vmm_virtual_display_ops simple_frame_buffer_ops = {
    .invalidate    = simple_frame_buffer_display_invalidate,
    .gfx_pixeldata = simple_frame_buffer_display_pixeldata,
    .gfx_update    = simple_frame_buffer_display_update,
};

static int simple_frame_buffer_guest_address_space_notification(vmm_notifier_block_t *nb, uint64_t evt, void *data)
{
    int                                   rc;
    ...
    struct simple_frame_buffer_state     *s     = container_of(nb, struct simple_frame_buffer_state, nb);
    ...
    rc = vmm_device_tree_read_physaddr(s->edev->node, "base", &s->fb_base);
    ...
    reg = vmm_guest_find_region(s->guest, s->fb_base, VMM_REGION_MEMORY, FALSE);

    if (!reg) {
        vmm_printf(
            "%s: guest=%s region not found for "
            "fb_base=0x%" PRIPADDR "\n",
    ...
    s->fb_base_ms    = ((uint64_t)paddr >> 32) & 0xffffffff;
...
static int simple_frame_buffer_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int         rc = VMM_OK;
    char        name[64];
    const char *str = NULL;
    ...
    struct frame_buffer_var_screeninfo info_var;
#endif
    struct simple_frame_buffer_state *s;
    ...
    s->magic   = SIMPLE_FRAME_BUFFER_MAGIC;
    s->vendor  = SIMPLE_FRAME_BUFFER_VENDOR;
    s->version = (uint32_t)((uint64_t)eid->data);

#if defined(CONFIG_FB)
    rc = vmm_device_tree_read_string(edev->node, "match_fbdev", &tmp);
    ...
    if (rc == VMM_OK) {
        ...
        info_mode = fb_find_best_mode(&info->var, &info->modelist);
...
skip_match:
    ...
    if (!str) {
        rc = vmm_device_tree_read_u32(edev->node, "width", &s->width);
        ...
        rc = vmm_device_tree_read_u32(edev->node, "height", &s->height);
        ...
        str = NULL;
        rc  = vmm_device_tree_read_string(edev->node, "mode", &str);
    ...
    rc = vmm_device_tree_read_string(edev->node, "order", &str);
    ...
    if (vmm_device_tree_read_u32(edev->node, "stride", &s->height)) {
    ...
    s->nb.notifier_call = &simple_frame_buffer_guest_address_space_notification;
    s->nb.priority      = 0;
...
simple_frame_buffer_emulator_probe_unreg_client_fail:
    vmm_guest_address_space_unregister_client(&s->nb);
simple_frame_buffer_emulator_probe_freestate_fail:
    vmm_free(s);
simple_frame_buffer_emulator_probe_fail:
    return rc;
}

static int simple_frame_buffer_emulator_remove(vmm_emulate_device_t *edev)
{
    int                               rc = VMM_OK;
    struct simple_frame_buffer_state *s  = edev->private;
...
static struct vmm_device_tree_nodeid simple_frame_buffer_emuid_table[] = {
    {
     .type       = "display",
     .compatible = "simple_frame_buffer-0.1",
     .data       = (void *)SIMPLE_FRAME_BUFFER_VERSION_0_1,
     },
    {/* end of list */},
};

VMM_DECLARE_EMULATOR_SIMPLE(
    simple_frame_buffer_emulator, "simple_frame_buffer", simple_frame_buffer_emuid_table, VMM_DEVICE_EMULATE_LITTLE_ENDIAN,
    simple_frame_buffer_emulator_probe, simple_frame_buffer_emulator_remove, simple_frame_buffer_emulator_reset, NULL,
    simple_frame_buffer_emulator_read, simple_frame_buffer_emulator_write);

static int __init simple_frame_buffer_emulator_init(void)
{
    return vmm_device_emulate_register_emulator(&simple_frame_buffer_emulator);
}

static void __exit simple_frame_buffer_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&simple_frame_buffer_emulator);
}
