...
struct pl110_state {
    ...
    uint32_t            mux_ctrl;
    uint32_t            palette8[256];
    uint32_t            palette15[256];
    uint32_t            palette16[256];
...
static void pl110_display_invalidate(struct vmm_virtual_display *vdis)
{
...
static int pl110_display_pixeldata(struct vmm_virtual_display *vdis, struct vmm_pixelformat *pf, uint32_t *rows, uint32_t *cols, physical_addr_t *pa)
{
    uint32_t            flags;
...
static void pl110_display_update(struct vmm_virtual_display *vdis, struct vmm_surface *sf)
{
    uint32_t           *palette;
    drawfn             *fntable;
    ...
    vmm_surface_update(
        sf, s->guest, gphys, cols, rows, src_width, dest_width, 0, fntable[DRAWFN_FNTABLE_INDEX(fmt, order, bppmode)], palette, &first, &last);

    if (first >= 0) {
        vmm_virtual_display_surface_gfx_update(vdis, 0, first, cols, last - first + 1);
    }
}

/* Process IRQ asserted via device emulation framework */
static void pl110_mux_in_irq_handle(uint32_t irq, int cpu, int level, void *opaque)
{
...
static void pl110_resize(struct pl110_state *s, int width, int height)
{
    ...
    if (do_gfx_resize) {
        vmm_virtual_display_surface_gfx_resize(s->vdis, width, height);
    }
}
...
static int pl110_reg_read(struct pl110_state *s, uint32_t offset, uint32_t *dst)
{
    int rc = VMM_OK;
...
static int pl110_reg_write(struct pl110_state *s, uint32_t offset, uint32_t src_mask, uint32_t src)
{
    uint32_t val, n;
...
static int pl110_emulator_read8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;

    rc              = pl110_reg_read(edev->private, offset, &regval);
...
static int pl110_emulator_read16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;
...
static int pl110_emulator_read32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst)
{
    return pl110_reg_read(edev->private, offset, dst);
}

static int pl110_emulator_write8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t src)
{
    return pl110_reg_write(edev->private, offset, 0xFFFFFF00, src);
}

static int pl110_emulator_write16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t src)
{
    return pl110_reg_write(edev->private, offset, 0xFFFF0000, src);
}

static int pl110_emulator_write32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t src)
{
    return pl110_reg_write(edev->private, offset, 0x00000000, src);
}

static int pl110_emulator_reset(vmm_emulate_device_t *edev)
{
    ...
    vmm_virtual_display_surface_gfx_clear(s->vdis);

    return VMM_OK;
}

static struct vmm_virtual_display_ops pl110_ops = {
    .invalidate    = pl110_display_invalidate,
    .gfx_pixeldata = pl110_display_pixeldata,
    .gfx_update    = pl110_display_update,
};

static struct vmm_device_emulation_irqchip pl110_mux_in_irqchip = {
    .name   = "PL110_MUX_IN",
    .handle = pl110_mux_in_irq_handle,
};

static int pl110_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int                 rc = VMM_OK;
    char                name[64];
    ...
    if (vmm_device_tree_read_u32(edev->node, "mux_in", &s->mux_in)) {
        s->mux_in = UINT_MAX;
    }
    ...
    if (s->mux_in != UINT_MAX) {
        vmm_device_emulate_register_irqchip(guest, s->mux_in, &pl110_mux_in_irqchip, s);
    }
...
static int pl110_emulator_remove(vmm_emulate_device_t *edev)
{
    int                 rc = VMM_OK;
    struct pl110_state *s  = edev->private;
    ...
    if (s->mux_in != UINT_MAX) {
        vmm_device_emulate_unregister_irqchip(s->guest, s->mux_in, &pl110_mux_in_irqchip, s);
    }
 ...
 */
static uint32_t pl110_versatile_config[] = {
    /* id0 */ 0x10,
    /* id1 */ 0x11,
    /* id2 */ 0x04,
    /* id3 */ 0x00,
...
static struct vmm_device_tree_nodeid pl110_emuid_table[] = {
    {
     .type       = "display",
...
static vmm_emulator_t pl110_emulator = {
    .name        = "pl110",
...
{
    return vmm_device_emulate_register_emulator(&pl110_emulator);
}

static void __exit pl110_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&pl110_emulator);
}
