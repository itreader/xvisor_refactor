...
#define MODULE_IPRIORITY               (VMM_VSERIAL_IPRIORITY + VMM_VIRTIO_IPRIORITY + 1)
...
static uint64_t virtio_console_get_host_features(struct vmm_virtio_device *dev)
{
    /* We support emergency write. */
    return 1UL << VMM_VIRTIO_RING_F_EVENT_IDX | 1UL << VMM_VIRTIO_CONSOLE_F_EMERG_WRITE;
}

static void virtio_console_set_guest_features(struct vmm_virtio_device *dev, uint32_t select, uint32_t features)
{
    /* No host features so, ignore it. */
}

static int virtio_console_init_vq(struct vmm_virtio_device *dev, uint32_t vq, uint32_t page_size, uint32_t align, uint32_t pfn)
{
    int                        rc;
    ...
    switch (vq) {
        ...
        case VIRTIO_CONSOLE_TX_QUEUE:
            rc = vmm_virtio_queue_setup(&cdev->vqs[vq], dev->guest, pfn, page_size, VIRTIO_CONSOLE_QUEUE_SIZE, align);
            break;
...
static int virtio_console_get_pfn_vq(struct vmm_virtio_device *dev, uint32_t vq)
{
    int                        rc;
    struct virtio_console_dev *cdev = dev->emu_data;
...
static int virtio_console_get_size_vq(struct vmm_virtio_device *dev, uint32_t vq)
{
    int rc;
...
static int virtio_console_set_size_vq(struct vmm_virtio_device *dev, uint32_t vq, int size)
{
    /* FIXME: dynamic */
    return size;
}

static int virtio_console_do_tx(struct vmm_virtio_device *dev, struct virtio_console_dev *cdev)
{
    int                      rc;
    uint8_t                  buf[8];
    uint16_t                 head = 0;
    ...
    while (vmm_virtio_queue_available(vq)) {
        ...
        for (i = 0; i < iov_cnt; i++) {
            ...
            while (tiov.len) {
                len = vmm_virtio_iovec_to_buf_read(dev, &tiov, 1, &buf, sizeof(buf));
...
static int virtio_console_notify_vq(struct vmm_virtio_device *dev, uint32_t vq)
{
    int                        rc   = VMM_OK;
    struct virtio_console_dev *cdev = dev->emu_data;
...
static void virtio_console_status_changed(struct vmm_virtio_device *dev, uint32_t new_status)
{
    /* Nothing to do here. */
}
...
static int virtio_console_vserial_send(struct vmm_vserial *vser, uint8_t data)
{
    int                        rc;
    uint16_t                   head    = 0;
    uint32_t                   iov_cnt = 0, total_len = 0;
    struct virtio_console_dev *cdev = vmm_vserial_private(vser);
    ...
    if (vmm_virtio_queue_available(vq)) {
        ...
        if (iov_cnt) {
            vmm_virtio_buf_to_iovec_write(dev, &iov[0], 1, &data, 1);
...
static int virtio_console_read_config(struct vmm_virtio_device *dev, uint32_t offset, void *dst, uint32_t dst_len)
{
    struct virtio_console_dev *cdev = dev->emu_data;
    uint8_t                    data8, *src = (uint8_t *)&cdev->config;
    uint32_t                   i, data, src_len = sizeof(cdev->config);
...
static int virtio_console_write_config(struct vmm_virtio_device *dev, uint32_t offset, void *src, uint32_t src_len)
{
    uint8_t                    data;
    struct virtio_console_dev *cdev = dev->emu_data;
...
static int virtio_console_connect(struct vmm_virtio_device *dev, struct vmm_virtio_emulator *emu)
{
    ...
    vmm_snprintf(cdev->name, VMM_VIRTIO_DEVICE_MAX_NAME_LEN, "%s", dev->name);
