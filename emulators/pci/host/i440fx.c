...
struct i440fx_state {
    ...
    vmm_notifier_block_t        guest_address_space_client;
    uint32_t                    conf_add;
    uint32_t                    conf_data;
};

static uint32_t i440fx_config_read(struct pci_class *pci_class, uint16_t reg_offset)
{
...
static int i440fx_config_write(struct pci_class *pci_class, uint16_t reg_offset, uint32_t data)
{
...
static int i440fx_reg_write(struct i440fx_state *s, uint32_t addr, uint32_t src_mask, uint32_t val)
{
    s->conf_add = val;
...
static int i440fx_reg_read(struct i440fx_state *s, uint32_t addr, uint32_t *dst, uint32_t size)
{
    uint16_t           bus, dev, func, reg_offs;
    ...
    if (s->conf_add & 0x80000000) {
        ...
        if (bus == 0 && dev == 0) {
            ...
            if (func) {
                ...
                *dst = pci_emu_config_space_read((struct pci_class *)s->controller, reg_offs, size);
            }
        } else {
...
static int i440fx_emulator_reset(vmm_emulate_device_t *edev)
{
    return VMM_OK;
}

static int i440fx_emulator_read8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;

    rc              = i440fx_reg_read(edev->private, offset, &regval, 1);
...
static int i440fx_emulator_read16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;

    rc              = i440fx_reg_read(edev->private, offset, &regval, 2);
...
static int i440fx_emulator_read32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst)
{
    return i440fx_reg_read(edev->private, offset, dst, 4);
}

static int i440fx_emulator_write8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t src)
{
    return i440fx_reg_write(edev->private, offset, 0xFFFFFF00, src);
}

static int i440fx_emulator_write16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t src)
{
    return i440fx_reg_write(edev->private, offset, 0xFFFF0000, src);
}

static int i440fx_emulator_write32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t src)
{
    return i440fx_reg_write(edev->private, offset, 0x00000000, src);
}

static int i440fx_guest_address_space_notification(vmm_notifier_block_t *nb, uint64_t evt, void *data)
{
    int                  ret = NOTIFY_DONE;
    int                  rc;
    struct i440fx_state *i440fx = container_of(nb, struct i440fx_state, guest_address_space_client);

    vmm_mutex_lock(&i440fx->lock);

    switch (evt) {
        case VMM_GUEST_ADDRESS_SPACE_EVENT_RESET:
            if ((rc = pci_emu_register_controller(i440fx->node, i440fx->guest, i440fx->controller)) != VMM_OK) {
                I440FX_LOG(LVL_ERR, "Failed to attach PCI controller.\n");
                goto _failed;
            }
...
static int i440fx_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int                  rc = VMM_OK, i;
    char                 name[64];
    ...
    s->guest_address_space_client.notifier_call = &i440fx_guest_address_space_notification;
    s->guest_address_space_client.priority      = 0;

    vmm_guest_address_space_register_client(&s->guest_address_space_client);
...
static int i440fx_emulator_remove(vmm_emulate_device_t *edev)
{
    // struct i440fx_state *s = edev->private;

    return VMM_OK;
}

static struct vmm_device_tree_nodeid i440fx_emuid_table[] = {
    {
...
static vmm_emulator_t i440fx_emulator = {
...
{
    return vmm_device_emulate_register_emulator(&i440fx_emulator);
}

static void __exit i440fx_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&i440fx_emulator);
}
