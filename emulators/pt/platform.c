...
struct platform_pt_state {
    ...
    uint32_t             irq_count;
    uint32_t            *host_irqs;
    uint32_t            *host_irqs_type;
    uint32_t            *host_irqs_cpu;
    uint32_t            *guest_irqs;
    vmm_device_t        *dev;
    vmm_iommu_domain_t  *dom;
...
static vmm_irq_return_t platform_pt_routed_irq(int irq, void *dev)
{
    int                       rc;
    bool                      found = FALSE;
    uint32_t                  i, host_irq = irq, guest_irq = 0;
     ...
     */
    rc = vmm_device_emulate_emulate_irq2(s->guest, guest_irq, 0, 1);

    if (rc) {
        vmm_printf(
            "%s: Emulate Guest=%s irq=%d "
...
static void platform_pt_notify(struct platform_pt_state *s, uint32_t guest_irq, int cpu, bool enabled)
{
    bool     found = FALSE;
    uint32_t i, host_irq = 0;
...
static void platform_pt_notify_enabled(uint32_t irq, int cpu, void *opaque)
{
    platform_pt_notify(opaque, irq, cpu, TRUE);
}

static void platform_pt_notify_disabled(uint32_t irq, int cpu, void *opaque)
{
    platform_pt_notify(opaque, irq, cpu, FALSE);
}

static struct vmm_device_emulation_irqchip platform_pt_irqchip = {
    .name            = "PLATFORM_PT",
    .notify_enabled  = platform_pt_notify_enabled,
    .notify_disabled = platform_pt_notify_disabled,
};

static int platform_pt_fault(vmm_iommu_domain_t *dom, vmm_device_t *dev, physical_addr_t iova, int flags, void *private)
{
    struct platform_pt_state *s = private;

    vmm_lerror("platform_pt", "iommu fault flags=0x%x iova=0x%" PRIPADDR "\n", flags, iova);
...
static void platform_pt_mapping_iter(
    struct vmm_guest *guest, struct vmm_region *reg, physical_addr_t guest_phys, physical_addr_t host_phys, physical_size_t size, void *private)
{
    struct platform_pt_state *s = private;

    /* Create IOMMU mapping for given guest region mapping */
    vmm_iommu_map(s->dom, guest_phys, host_phys, size, VMM_IOMMU_READ | VMM_IOMMU_WRITE);
}

static void platform_pt_region_iter(struct vmm_guest *guest, struct vmm_region *reg, void *private)
{
    struct platform_pt_state *s = private;

    /* Iterate over each mapping of guest region */
    vmm_guest_iterate_mapping(guest, reg, platform_pt_mapping_iter, s);
}

static int platform_pt_guest_address_space_notification(vmm_notifier_block_t *nb, uint64_t evt, void *data)
{
    uint32_t                              i;
    ...
    for (i = 0; i < s->irq_count; i++) {
        vmm_device_emulate_map_host2guest_irq(s->guest, s->guest_irqs[i], s->host_irqs[i]);
    }

    /* Iterate over each real ram regions of guest */
    if (s->dom) {
        vmm_guest_iterate_region(s->guest, VMM_REGION_REAL | VMM_REGION_MEMORY | VMM_REGION_IS_RAM, platform_pt_region_iter, s);
    }

    return NOTIFY_OK;
}

static int platform_pt_reset(vmm_emulate_device_t *edev)
{
    uint32_t                  i;
...
static int platform_pt_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int                       rc = VMM_OK;
    uint32_t                  i, irq_reg_count = 0;
    ...
    s->guest          = guest;
    s->irq_count      = vmm_device_tree_attrlen(edev->node, "host-interrupts");
    s->irq_count      = s->irq_count / (sizeof(uint32_t) * 3);
    s->guest_irqs     = NULL;
    s->host_irqs      = NULL;
    ...
    for (i = 0; i < s->irq_count; i++) {
        rc = vmm_device_tree_read_u32_atindex(edev->node, "host-interrupts", &s->host_irqs[i], i * 3);

        if (rc) {
            goto platform_pt_probe_cleanupirqs_fail;
        }

        rc = vmm_device_tree_read_u32_atindex(edev->node, "host-interrupts", &s->host_irqs_type[i], i * 3 + 1);

        if (rc) {
            goto platform_pt_probe_cleanupirqs_fail;
        }

        rc = vmm_device_tree_read_u32_atindex(edev->node, "host-interrupts", &s->host_irqs_cpu[i], i * 3 + 2);

        if (rc) {
            goto platform_pt_probe_cleanupirqs_fail;
        }

        rc = vmm_device_tree_read_u32_atindex(edev->node, VMM_DEVICE_TREE_INTERRUPTS_ATTR_NAME, &s->guest_irqs[i], i);

        if (rc) {
            goto platform_pt_probe_cleanupirqs_fail;
        }

        rc = vmm_host_irq_set_type(s->host_irqs[i], s->host_irqs_type[i]);

        if (rc) {
            goto platform_pt_probe_cleanupirqs_fail;
        }

        if ((s->host_irqs_cpu[i] != U32_MAX) && (s->host_irqs_cpu[i] < vmm_num_online_cpus())) {
            rc = vmm_host_irq_set_affinity(s->host_irqs[i], vmm_cpumask_of(s->host_irqs_cpu[i]), TRUE);
        ...
        rc = vmm_host_irq_register(s->host_irqs[i], s->name, platform_pt_routed_irq, s);
        ...
        rc = vmm_device_emulate_register_irqchip(s->guest, s->guest_irqs[i], &platform_pt_irqchip, s);
    ...
    iommu_device = NULL;
    vmm_device_tree_read_string(edev->node, "iommu-device", &iommu_device);
    ...
    if (iommu_device) {
        ...
        vmm_device_driver_ref_device(s->dev);

        s->dom = vmm_iommu_group_get_domain(s->dev->iommu_group);

        if (!s->dom) {
            s->dom = vmm_iommu_domain_alloc(s->name, &platform_bus, vmm_iommu_group_controller(s->dev->iommu_group), VMM_IOMMU_DOMAIN_UNMANAGED);
        }
        ...
        rc = vmm_iommu_group_attach_domain(s->dev->iommu_group, s->dom);
    ...
    s->nb.notifier_call = &platform_pt_guest_address_space_notification;
    s->nb.priority      = 0;
...
platform_pt_probe_dref_dev_fail:
    ...
    if (s->dev) {
        vmm_device_driver_dref_device(s->dev);
    }

platform_pt_probe_cleanupirqs_fail:

    for (i = 0; i < irq_reg_count; i++) {
        vmm_device_emulate_unregister_irqchip(s->guest, s->guest_irqs[i], &platform_pt_irqchip, s);
        vmm_host_irq_unregister(s->host_irqs[i], s);
        vmm_host_irq_unmark_routed(s->host_irqs[i]);
    }
...
static int platform_pt_remove(vmm_emulate_device_t *edev)
{
    uint32_t                  i;
    struct platform_pt_state *s = edev->private;
...
static struct vmm_device_tree_nodeid platform_pt_emuid_table[] = {
    {
     .type       = "pt",
     .compatible = "platform",
     },
...
static vmm_emulator_t platform_pt_emulator = {
    .name        = "platform",
...
{
    return vmm_device_emulate_register_emulator(&platform_pt_emulator);
}

static void __exit platform_pt_exit(void)
{
    vmm_device_emulate_unregister_emulator(&platform_pt_emulator);
}
