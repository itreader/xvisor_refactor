...
struct gic_cpu_state {
    ...
    uint16_t     last_active[GIC_MAX_NIRQ];
    uint8_t      priority[32];
};
...
#define GIC_GET_PRIORITY(s, irq, cpu) (((irq) < 32) ? (s)->cpu_state[cpu].priority[irq] : (s)->irq_state[irq].priority)
...
static void gic_update(struct gic_state *s)
{
    ...
    for (cpu = 0; cpu < GIC_NUM_CPU(s); cpu++) {
        ...
        vmm_write_lock_irq_save(&cpu_state->cpu_lock, cpu_flags);

        cpu_state->current_pending = 1023;

        if (!s->enabled || !cpu_state->enabled) {
            vmm_write_unlock_irq_restore(&cpu_state->cpu_lock, cpu_flags);

            if (s->is_child_pic) {
                vmm_device_emulate_emulate_per_cpu_irq(s->guest, cpu_state->parent_irq, cpu, 0);
            }
        ...
        vmm_read_lock_irq_save(&s->dist_lock, dist_flags);

        for (irq = 0; irq < GIC_NUM_IRQ(s); irq++) {
            if (GIC_TEST_ENABLED(s, irq, cm) && GIC_TEST_PENDING(s, irq, cm)) {
        ...
        vmm_read_unlock_irq_restore(&s->dist_lock, dist_flags);
        ...
        if (s->is_child_pic) {
            ...
            vmm_device_emulate_emulate_per_cpu_irq(s->guest, cpu_state->parent_irq, cpu, level);
        } else {
            vcpu = vmm_manager_guest_vcpu(s->guest, cpu);

            if (level && vcpu) {
                /* Assert irq to VCPU */
                vmm_vcpu_irq_assert(vcpu, cpu_state->parent_irq, 0x0);
            }

            if (!level && vcpu) {
                /* Deassert irq to VCPU */
                vmm_vcpu_irq_deassert(vcpu, cpu_state->parent_irq);
            }
        }
    }
}
...
static void gic_irq_handle(uint32_t irq, int cpu, int level, void *opaque)
{
    irq_flags_t       flags;
    int               cm, target;
    ...
    if (level == GIC_TEST_LEVEL(s, irq, cm)) {
        vmm_write_unlock_irq_restore(&s->dist_lock, flags);
        return;
    }
...
static uint32_t gic_acknowledge_irq(struct gic_state *s, int cpu)
{
    int                   new_irq;
    int                   cm = 1 << cpu;
    irq_flags_t           dist_flags, cpu_flags;
    ...
    if ((new_irq == 1023) || GIC_GET_PRIORITY(s, new_irq, cpu) >= cpu_state->running_priority) {
        new_irq = 1023;
        goto release_lock;
    }
    ...
    vmm_write_lock_irq_save(&s->dist_lock, dist_flags);

    /* Clear pending flags for both level and edge triggered interrupts.
     * Level triggered IRQs will be reasserted once they become inactive. */
    GIC_CLEAR_PENDING(s, new_irq, GIC_TEST_MODEL(s, new_irq) ? GIC_ALL_CPU_MASK(s) : cm);

    vmm_write_unlock_irq_restore(&s->dist_lock, dist_flags);
...
static void gic_complete_irq(struct gic_state *s, int cpu, int irq)
{
    ...
    if (!GIC_TEST_TRIGGER(s, irq) && GIC_TEST_ENABLED(s, irq, cm) && GIC_TEST_LEVEL(s, irq, cm) && (GIC_TARGET(s, irq) & cm) != 0) {
    ...
    if (irq != cpu_state->running_irq) {
        ...
        while (cpu_state->last_active[tmp] != 1023) {
            if (cpu_state->last_active[tmp] == irq) {
                cpu_state->last_active[tmp] = cpu_state->last_active[irq];
                break;
            }
        ...
        gic_set_running_irq(s, cpu, cpu_state->last_active[cpu_state->running_irq]);
        update = 2;
    }
...
static int __gic_dist_readb(struct gic_state *s, int cpu, uint32_t offset, uint8_t *dst)
{
    ...
    switch (offset >> 8) {
        ...
        case 0x1: /* Enable */
            ...
            *dst = 0;
            ...
            for (i = 0; i < 8; i++) {
                *dst |= GIC_TEST_ENABLED(s, irq + i, (1 << cpu)) ? (1 << i) : 0x0;
            }
        ...
        case 0x2: /* Pending */
            ...
            *dst = 0;
            ...
            for (i = 0; i < 8; i++) {
                *dst |= GIC_TEST_PENDING(s, irq + i, mask) ? (1 << i) : 0x0;
            }
        ...
        case 0x3: /* Active */
            ...
            *dst = 0;
            ...
            for (i = 0; i < 8; i++) {
                *dst |= GIC_TEST_ACTIVE(s, irq + i, mask) ? (1 << i) : 0x0;
            }
        ...
        case 0x1: /* Enable */
            ...
            if (!(offset & 0x80)) { /* Set-enableX */
                ...
                for (i = 0; i < 8; i++) {
                    ...
                    mask = ((irq + i) < 32) ? (1 << cpu) : GIC_TARGET(s, (irq + i));
                    cm   = ((irq + i) < 32) ? (1 << cpu) : GIC_ALL_CPU_MASK(s);
                    GIC_SET_ENABLED(s, irq + i, cm);

                    /* If a raised level triggered IRQ enabled
                     * then mark is as pending.  */
                    if (GIC_TEST_LEVEL(s, (irq + i), mask) && !GIC_TEST_TRIGGER(s, (irq + i))) {
...
static int gic_dist_read(struct gic_state *s, int cpu, uint32_t offset, uint32_t *dst)
{
    ...
    *dst = 0;
    ...
    vmm_read_unlock_irq_restore(&s->dist_lock, flags);

    return VMM_OK;
}

static int gic_dist_write(struct gic_state *s, int cpu, uint32_t offset, uint32_t src_mask, uint32_t src)
{
    ...
    if (offset == 0xF00) {
        ...
        for (i = 0; i < 4; i++) {
            if (src_mask & 0xFF) {
                if ((rc = __gic_dist_writeb(s, cpu, offset + i, src & 0xFF))) {
                    break;
                }
            }
...
static int gic_cpu_read(struct gic_state *s, uint32_t cpu, uint32_t offset, uint32_t *dst)
{
    int rc = VMM_OK;
...
static int gic_cpu_write(struct gic_state *s, uint32_t cpu, uint32_t offset, uint32_t src_mask, uint32_t src)
{
    int rc = VMM_OK;
...
int gic_reg_read(struct gic_state *s, physical_addr_t offset, uint32_t *dst)
{
    vmm_vcpu_t *vcpu;
    ...
    } else if ((offset >= s->dist.offset) && (offset < (s->dist.offset + s->dist.length))) {
...
int gic_reg_write(struct gic_state *s, physical_addr_t offset, uint32_t src_mask, uint32_t src)
{
    ...
    if ((offset >= s->cpu.offset) && (offset < (s->cpu.offset + s->cpu.length))) {
        /* Write CPU Interface */
        return gic_cpu_write(s, vcpu->subid, offset & 0xFC, src_mask, src);
    } else if ((offset >= s->dist.offset) && (offset < (s->dist.offset + s->dist.length))) {
...
static int gic_emulator_read8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t *dst)
{
    ...
    rc              = gic_reg_read(edev->private, offset, &regval);
...
static int gic_emulator_read16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;
...
static int gic_emulator_read32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst)
{
    return gic_reg_read(edev->private, offset, dst);
}

static int gic_emulator_write8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t src)
{
    return gic_reg_write(edev->private, offset, 0xFFFFFF00, src);
}

static int gic_emulator_write16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t src)
{
    return gic_reg_write(edev->private, offset, 0xFFFF0000, src);
}

static int gic_emulator_write32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t src)
{
    return gic_reg_write(edev->private, offset, 0x00000000, src);
}
...
{
    struct gic_state *s = edev->private;

    return gic_state_reset(s);
}

static uint32_t gic_configs[][14] = {
    {
     /* num_irq */ 96,
     /* base_irq */ 0,
     /* id0 */ 0x90,
...
static struct vmm_device_emulation_irqchip gic_irqchip = {
    .name   = "GIC",
    .handle = gic_irq_handle,
};

struct gic_state *gic_state_alloc(
    const char *name, struct vmm_guest *guest, enum gic_type type, uint32_t num_cpu, bool is_child_pic, uint32_t base_irq, uint32_t num_irq,
    uint32_t parent_irq)
{
    uint32_t          i;
    ...
    for (i = s->base_irq; i < (s->base_irq + s->num_irq); i++) {
        vmm_device_emulate_register_irqchip(guest, i, &gic_irqchip, s);
    }
...
int gic_state_free(struct gic_state *s)
{
    ...
    for (i = s->base_irq; i < (s->base_irq + s->num_irq); i++) {
        vmm_device_emulate_unregister_irqchip(s->guest, i, &gic_irqchip, s);
    }
...
static int gic_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    ...
    if (vmm_device_tree_read_u32(edev->node, "base_irq", &base_irq)) {
...
static int gic_emulator_remove(vmm_emulate_device_t *edev)
{
    struct gic_state *s = edev->private;
...
static struct vmm_device_tree_nodeid gic_emuid_table[] = {
    {
     .type       = "pic",
...
{
    return vmm_device_emulate_register_emulator(&gic_emulator);
}

static void __exit gic_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&gic_emulator);
}
