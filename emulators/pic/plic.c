...
struct plic_context {
    ...
    uint8_t        irq_priority_threshold;
    uint32_t       irq_enable[MAX_DEVICES / 32];
    uint32_t       irq_pending[MAX_DEVICES / 32];
    uint8_t        irq_pending_priority[MAX_DEVICES];
    uint32_t       irq_claimed[MAX_DEVICES / 32];
};
...
struct plic_state {
    ...
    /* Global IRQ state */
    vmm_spinlock_t irq_lock;
    uint8_t        irq_priority[MAX_DEVICES];
    uint32_t       irq_level[MAX_DEVICES / 32];
};

/* Note: Must be called with c->irq_lock held */
static uint32_t __plic_context_best_pending_irq(struct plic_state *s, struct plic_context *c)
{
    uint8_t  best_irq_prio = 0;
    uint32_t i, j, irq, best_irq = 0;
...
{
    uint32_t    best_irq = __plic_context_best_pending_irq(s, c);
    vmm_vcpu_t *vcpu     = vmm_manager_guest_vcpu(s->guest, c->num / 2);
...
/* Note: Must be called with c->irq_lock held */
static uint32_t __plic_context_irq_claim(struct plic_state *s, struct plic_context *c)
{
    uint32_t    best_irq = __plic_context_best_pending_irq(s, c);
    vmm_vcpu_t *vcpu     = vmm_manager_guest_vcpu(s->guest, c->num / 2);
...
static void plic_irq_handle(uint32_t irq, int cpu, int level, void *opaque)
{
    bool                 irq_marked = FALSE;
    irq_flags_t          flags, flags1;
    uint8_t              i, irq_prio, irq_word;
    uint32_t             irq_mask;
    ...
    for (i = 0; i < s->num_context; i++) {
        c = &s->contexts[i];
        vmm_spin_lock_irq_save(&c->irq_lock, flags1);
        ...
        vmm_spin_unlock_irq_restore(&c->irq_lock, flags1);
    ...
    vmm_spin_unlock_irq_restore(&s->irq_lock, flags);
}

static int plic_priority_read(struct plic_state *s, physical_addr_t offset, uint32_t *dst)
{
    irq_flags_t flags;
...
static int plic_priority_write(struct plic_state *s, physical_addr_t offset, uint32_t src_mask, uint32_t src)
{
    irq_flags_t flags;
...
static int plic_context_enable_read(struct plic_state *s, struct plic_context *c, physical_addr_t offset, uint32_t *dst)
{
    irq_flags_t flags;
    ...
    *dst = c->irq_enable[irq_word];
    vmm_spin_unlock_irq_restore(&c->irq_lock, flags);

    return VMM_OK;
}

static int plic_context_enable_write(struct plic_state *s, struct plic_context *c, physical_addr_t offset, uint32_t src_mask, uint32_t src)
{
    uint8_t     irq_prio;
    irq_flags_t flags, flags1;
    uint32_t    i, irq, irq_mask;
    uint32_t    irq_word = offset >> 2;
...
static int plic_context_read(struct plic_state *s, struct plic_context *c, physical_addr_t offset, uint32_t *dst)
{
    int         rc = VMM_OK;
    irq_flags_t flags;
...
static int plic_context_write(struct plic_state *s, struct plic_context *c, physical_addr_t offset, uint32_t src_mask, uint32_t src)
{
    uint32_t    val;
    int         rc = VMM_OK;
    irq_flags_t flags;
...
static int plic_emulator_read(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst, uint32_t size)
{
    ...
    if (PRIORITY_BASE <= offset && offset < ENABLE_BASE) {
        ...
        if (cntx < s->num_context) {
            rc = plic_context_enable_read(s, &s->contexts[cntx], offset, dst);
        }
        ...
        if (cntx < s->num_context) {
            rc = plic_context_read(s, &s->contexts[cntx], offset, dst);
        }
    }
...
static int plic_emulator_write(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t src_mask, uint32_t src, uint32_t size)
{
    ...
    if (PRIORITY_BASE <= offset && offset < ENABLE_BASE) {
        ...
        if (cntx < s->num_context) {
            rc = plic_context_enable_write(s, &s->contexts[cntx], offset, src_mask, src);
        }
    } else if (CONTEXT_BASE <= offset && offset < REG_SIZE) {
        cntx = (offset - CONTEXT_BASE) / CONTEXT_PER_HART;
        offset -= cntx * CONTEXT_PER_HART + CONTEXT_BASE;
        ...
        if (cntx < s->num_context) {
            rc = plic_context_write(s, &s->contexts[cntx], offset, src_mask, src);
        }
    }
...
static int plic_emulator_reset(vmm_emulate_device_t *edev)
{
    int                  i, j;
    irq_flags_t          flags;
...
static struct vmm_device_emulation_irqchip plic_irqchip = {
    .name   = "PLIC",
    .handle = plic_irq_handle,
};

static int plic_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    uint32_t             i;
    int                  rc = VMM_OK;
    struct plic_context *c;
    ...
    if (vmm_device_tree_read_u32(edev->node, "max_priority", &s->max_prio)) {
    ...
    for (i = s->base_irq; i < (s->base_irq + s->num_irq); i++) {
        vmm_device_emulate_register_irqchip(guest, i, &plic_irqchip, s);
    }
...
static int plic_emulator_remove(vmm_emulate_device_t *edev)
{
    uint32_t           i;
    struct plic_state *s = edev->private;
    ...
    for (i = s->base_irq; i < (s->base_irq + s->num_irq); i++) {
        vmm_device_emulate_unregister_irqchip(s->guest, i, &plic_irqchip, s);
    }
...
static struct vmm_device_tree_nodeid plic_emulator_emuid_table[] = {
    {
     .type       = "pic",
...
VMM_DECLARE_EMULATOR_SIMPLE(
    plic_emulator, "plic", plic_emulator_emuid_table, VMM_DEVICE_EMULATE_LITTLE_ENDIAN, plic_emulator_probe, plic_emulator_remove,
    plic_emulator_reset, NULL, plic_emulator_read, plic_emulator_write);

static int __init plic_emulator_init(void)
{
    return vmm_device_emulate_register_emulator(&plic_emulator);
}

static void __exit plic_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&plic_emulator);
}
