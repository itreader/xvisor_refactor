...
static void apic_get_delivery_bitmask(apic_state_t *s, uint32_t *deliver_bitmask, uint8_t dest, uint8_t dest_mode);
...
static void apic_local_deliver(apic_state_t *s, int vector)
{
    ...
    switch ((lvt >> 8) & 7) {
        ...
        case APIC_DM_FIXED:
            ...
            if ((vector == APIC_LVT_LINT0 || vector == APIC_LVT_LINT1) && (lvt & APIC_LVT_LEVEL_TRIGGER)) {
...
static void apic_bus_deliver(
    apic_state_t *apic_base, const uint32_t *deliver_bitmask, uint8_t delivery_mode, uint8_t vector_num, uint8_t trigger_mode)
{
    ...
    foreach_apic(apic_base, apic_iter, deliver_bitmask, apic_set_irq(apic_iter, vector_num, trigger_mode));
}

/* Called when slaves (IOAPIC/PIC) assert interrupt. */
void apic_deliver_irq(apic_state_t *s, uint8_t dest, uint8_t dest_mode, uint8_t delivery_mode, uint8_t vector_num, uint8_t trigger_mode)
{
...
void apic_irq_handle(uint32_t irq, int cpu, int level, void *opaque)
{
    irq_flags_t   flags;
    apic_state_t *s = opaque;
    uint8_t       dest, dest_mode, del_mode, tmode, vnum;

    vmm_spin_lock_irq_save(&s->state_lock, flags);

    SLAVE_IRQ_DECODE(level, dest, dest_mode, del_mode, vnum, tmode);

    apic_deliver_irq(s, dest, dest_mode, del_mode, vnum, tmode);

    vmm_spin_unlock_irq_restore(&s->state_lock, flags);
}

#if 0
static void apic_set_base(apic_state_t *s, uint64_t val)
{
...
static void apic_set_tpr(apic_state_t *s, uint8_t val)
{
...
static uint8_t apic_get_tpr(apic_state_t *s)
{
#if 0
...
static int apic_find_dest(apic_state_t *base, uint8_t dest)
{
...
static void apic_get_delivery_bitmask(apic_state_t *apic_base, uint32_t *deliver_bitmask, uint8_t dest, uint8_t dest_mode)
{
    if (dest_mode == 0) {
        if (dest == 0xff) {
...
static void apic_deliver(apic_state_t *s, uint8_t dest, uint8_t dest_mode, uint8_t delivery_mode, uint8_t vector_num, uint8_t trigger_mode)
{
    uint32_t      deliver_bitmask[MAX_APIC_WORDS];
    ...
    switch (delivery_mode) {
        case APIC_DM_INIT:
        {
            ...
            if (level == 0 && trig_mode == 1) {
                foreach_apic(s, apic_iter, deliver_bitmask, apic_iter->arb_id = apic_iter->id);
                return;
            }
        } break;
...
static uint32_t apic_get_current_count(apic_state_t *s)
{
    int64_t  d;
    uint32_t val;
    d = (vmm_timer_timestamp() - s->initial_count_load_time) >> s->count_shift;
...
bool apic_next_timer(apic_state_t *s, int64_t current_time)
{
    int64_t d;
...
static void apic_timer_update(apic_state_t *s, int64_t current_time)
{
...
static void apic_timer(vmm_timer_event_t *event)
{
    apic_state_t *s = (apic_state_t *)event->private;

    apic_local_deliver(s, APIC_LVT_TIMER);
    apic_timer_update(s, s->next_time);
}

static uint32_t apic_ioport_read(apic_state_t *base, physical_addr_t addr, uint32_t *dst)
{
    apic_state_t *s;
    uint32_t      val;
    int           index;
...
static int apic_ioport_write(apic_state_t *s, uint32_t addr, uint32_t src_mask, uint32_t val)
{
    ...
    switch (index) {
        ...
        case 0x30:
            s->icr[0] = val;
            apic_deliver(s, (s->icr[1] >> 24) & 0xff, (s->icr[0] >> 11) & 1, (s->icr[0] >> 8) & 7, (s->icr[0] & 0xff), (s->icr[0] >> 15) & 1);
            break;
...
static int apic_emulator_read8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;

    rc              = apic_ioport_read(edev->private, offset, &regval);
...
static int apic_emulator_read16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;
...
static int apic_emulator_read32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst)
{
    int ret_val = apic_ioport_read(edev->private, offset, dst);

    return ret_val;
}

static int apic_emulator_write8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t src)
{
    int ret_val = apic_ioport_write(edev->private, offset, 0xFFFFFF00, src);

    return ret_val;
}

static int apic_emulator_write16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t src)
{
    int ret_val = apic_ioport_write(edev->private, offset, 0xFFFF0000, src);

    return ret_val;
}

static int apic_emulator_write32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t src)
{
    int ret_val = apic_ioport_write(edev->private, offset, 0x00000000, src);

    return ret_val;
}

void cpu_set_apic_base(apic_state_t *s, uint64_t val)
{
    /* FIXME: Change the APIC base and move the region. */
}

uint64_t cpu_get_apic_base(apic_state_t *s)
{
    if (s) {
        return s->apicbase;
    }
...
{
    uint64_t base = cpu_get_apic_base(s);
...
void cpu_set_apic_tpr(apic_state_t *s, uint8_t val)
{
    if (!s) {
        return;
    }
...
uint8_t cpu_get_apic_tpr(apic_state_t *s)
{
    if (!s) {
        return 0;
    }
...
static void apic_reset_common(apic_state_t *s)
{
    ...
    bsp         = cpu_is_bsp(s);
    s->apicbase = APIC_DEFAULT_ADDRESS | (bsp ? MSR_IA32_APICBASE_BSP : 0) | MSR_IA32_APICBASE_ENABLE;
...
static int apic_emulator_remove(vmm_emulate_device_t *edev)
{
...
static struct vmm_device_emulation_irqchip apic_irqchip = {
    .name   = "APIC",
    .handle = apic_irq_handle,
};

static int apic_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int           rc = VMM_OK;
    apic_state_t *s, *e;
    int           i = 0;
    ...
    if ((rc = vmm_device_tree_read_u32(edev->node, "base_irq", &s->base_irq)) != VMM_OK) {
        APIC_LOG(ERR, "Base IRQ not defined!\n");
        goto apic_emulator_probe_freestate_fail;
    }

    if ((rc = vmm_device_tree_read_u32(edev->node, "num_irq", &s->num_irq)) != VMM_OK) {
        APIC_LOG(ERR, "Number of IRQ not defined!\n");
        goto apic_emulator_probe_freestate_fail;
    }

    for (i = s->base_irq; i < (s->base_irq + s->num_irq); i++) {
        vmm_device_emulate_register_irqchip(guest, i, &apic_irqchip, s);
    }
...
static int apic_emulator_reset(vmm_emulate_device_t *edev)
{
    apic_state_t *s = edev->private;
...
static struct vmm_device_tree_nodeid apic_emulator_emuid_table[] = {
    {
     .type       = "pic",
     .compatible = "lapic",
     },
...
static vmm_emulator_t apic_emulator = {
    .name        = "lapic",
...
{
    return vmm_device_emulate_register_emulator(&apic_emulator);
}

static void __exit apic_emulator_exit(void)
{
    vmm_device_emulate_unregister_emulator(&apic_emulator);
}
