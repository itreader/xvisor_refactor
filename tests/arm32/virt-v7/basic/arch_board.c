...
{
    basic_strcpy(
        cmdline, "root=/dev/ram rw earlyprintk "
...
void arch_board_fdt_fixup(void *fdt_addr)
{
    ...
    uint32_t i, vcpu_count = vminfo_vcpu_count(VIRT_V7_VMINFO);
    ...
    for (i = 0; i < vcpu_count; i++) {
        ...
        ret = fdt_setprop_string(fdt_addr, cpu_offset, "compatible", "arm,arm-v7");
        ...
        ret     = fdt_setprop(fdt_addr, cpu_offset, "reg", vals, sizeof(vals));
        ...
        ret = fdt_setprop_string(fdt_addr, cpu_offset, "enable-method", "psci");
    ...
    simple_frame_buffer_fdt_fixup(VIRT_V7_SIMPLE_FRAME_BUFFER, fdt_addr);
}
...
physical_addr_t arch_board_iosection_addr(int num)
{
    ...
    switch (num) {
        ...
        case 4:
            /* simple_frame_buffer */
            ret = VIRT_V7_SIMPLE_FRAME_BUFFER;
            break;
