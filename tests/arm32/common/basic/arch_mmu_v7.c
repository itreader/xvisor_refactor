...
uint32_t __attribute__((aligned(TTBL_L1TBL_SIZE))) l1[TTBL_L1TBL_SIZE / 4];
uint32_t __attribute__((aligned(TTBL_L2TBL_SIZE))) l2[TTBL_L2TBL_SIZE / 4];
uint32_t                                           l2_mapva;

uint32_t test_area_pa;
uint32_t test_area_size;

void arm_mmu_syscall(struct pt_regs *regs)
{
    uint32_t inst = 0x0;
    inst          = *((uint32_t *)regs->pc);
    inst &= 0x00FFFFFF;
...
extern void _switch_to_user_mode(uint32_t, uint32_t);

#define ARM_MMU_TEST_SWITCH_TO_USER()  _switch_to_user_mode(0x0, 0x0)
#define ARM_MMU_TEST_SWITCH_TO_SUPER() asm("svc 0x1")

uint32_t test_prefetch_abort_fs;
uint32_t test_prefetch_abort_far;
uint32_t test_prefetch_abort_result;
...
void arm_mmu_prefetch_abort(struct pt_regs *regs)
{
    ...
    if ((fs == test_prefetch_abort_fs) && (ifar == test_prefetch_abort_far)) {
...
uint32_t test_data_abort_fs;
uint32_t test_data_abort_far;
uint32_t test_data_abort_wnr;
uint32_t test_data_abort_dom;
uint32_t test_data_abort_result;

void arm_mmu_data_abort(struct pt_regs *regs)
{
    uint32_t dfsr, dfar, fs, dom, wnr;
    ...
    if ((fs == test_data_abort_fs) && (dfar == test_data_abort_far) && (wnr == test_data_abort_wnr) && (dom == test_data_abort_dom)) {
...
static void arm_mmu_section_test_iter(uint32_t free_sec0, uint32_t free_sec1, uint32_t *total, uint32_t *pass, uint32_t *fail)
{
    ...
    /* 1.1 */
    test_ptr               = (uint32_t *)(free_sec0 << TTBL_L1TBL_TTE_BASE20_SHIFT);
    test_data_abort_fs     = DFSR_FS_TRANS_FAULT_SECTION;
    test_data_abort_far    = (uint32_t)test_ptr;
    ...
    *(test_ptr)            = 0xC001BABE;
    ...
    /* 1.2 */
    test_ptr               = (uint32_t *)((free_sec0 << TTBL_L1TBL_TTE_BASE20_SHIFT) + (TTBL_L1TBL_SECTION_PAGE_SIZE / 2) - 4);
    test_data_abort_fs     = DFSR_FS_TRANS_FAULT_SECTION;
    test_data_abort_far    = (uint32_t)test_ptr;
    test_data_abort_wnr    = 1;
    test_data_abort_dom    = 0x0;
    ...
    *(test_ptr)            = 0xC001BABE;
    ...
    /* 1.3 */
    test_ptr               = (uint32_t *)((free_sec0 << TTBL_L1TBL_TTE_BASE20_SHIFT) + TTBL_L1TBL_SECTION_PAGE_SIZE - 4);
    test_data_abort_fs     = DFSR_FS_TRANS_FAULT_SECTION;
    test_data_abort_far    = (uint32_t)test_ptr;
    test_data_abort_wnr    = 1;
    test_data_abort_dom    = 0x0;
    ...
    *(test_ptr)            = 0xC001BABE;
    ...
    test_ptr      = (uint32_t *)((free_sec0 << TTBL_L1TBL_TTE_BASE20_SHIFT) + (TTBL_L1TBL_SECTION_PAGE_SIZE / 2) - 8);
    test_ptr[0]   = 0xC001BABE;
    test_ptr      = (uint32_t *)((free_sec0 << TTBL_L1TBL_TTE_BASE20_SHIFT) + TTBL_L1TBL_SECTION_PAGE_SIZE - 8);
    test_ptr[0]   = 0xC001BABE;
    test_ptr      = (uint32_t *)(free_sec1 << TTBL_L1TBL_TTE_BASE20_SHIFT);
    test_ptr[1]   = 0xD00DFEED;
    test_ptr      = (uint32_t *)((free_sec1 << TTBL_L1TBL_TTE_BASE20_SHIFT) + (TTBL_L1TBL_SECTION_PAGE_SIZE / 2) - 8);
    test_ptr[1]   = 0xD00DFEED;
    test_ptr      = (uint32_t *)((free_sec1 << TTBL_L1TBL_TTE_BASE20_SHIFT) + TTBL_L1TBL_SECTION_PAGE_SIZE - 8);
    test_ptr[1]   = 0xD00DFEED;
    test_ptr      = (uint32_t *)(free_sec0 << TTBL_L1TBL_TTE_BASE20_SHIFT);
    (*total)++;
...
static void arm_mmu_page_test_iter(uint32_t free_page0, uint32_t free_page1, uint32_t *total, uint32_t *pass, uint32_t *fail)
{
    volatile uint32_t *test_ptr = NULL;
    uint32_t           tmp, tmp1;
    ...
    /* 1. Unmapped Read/Write test */
    /* 1.1 */
    test_ptr               = (uint32_t *)(free_page0 << TTBL_L2TBL_TTE_BASE12_SHIFT);
    test_ptr               = (uint32_t *)((uint32_t)test_ptr + l2_mapva);
    ...
    *(test_ptr)            = 0xC001BABE;
    ...
    /* 1.2 */
    test_ptr               = (uint32_t *)((free_page0 << TTBL_L2TBL_TTE_BASE12_SHIFT) + (TTBL_L2TBL_SMALL_PAGE_SIZE / 2) - 4);
    test_ptr               = (uint32_t *)((uint32_t)test_ptr + l2_mapva);
    test_data_abort_fs     = DFSR_FS_TRANS_FAULT_PAGE;
    test_data_abort_far    = (uint32_t)test_ptr;
    test_data_abort_wnr    = 1;
    ...
    *(test_ptr)            = 0xC001BABE;
    ...
    /* 1.3 */
    test_ptr               = (uint32_t *)((free_page0 << TTBL_L2TBL_TTE_BASE12_SHIFT) + TTBL_L2TBL_SMALL_PAGE_SIZE - 4);
    test_ptr               = (uint32_t *)((uint32_t)test_ptr + l2_mapva);
    test_data_abort_fs     = DFSR_FS_TRANS_FAULT_PAGE;
    test_data_abort_far    = (uint32_t)test_ptr;
    test_data_abort_wnr    = 1;
    ...
    *(test_ptr)            = 0xC001BABE;
    ...
    test_ptr       = (uint32_t *)((uint32_t)test_ptr + l2_mapva);
    test_ptr[0]    = 0xC001BABE;
    test_ptr       = (uint32_t *)((free_page0 << TTBL_L2TBL_TTE_BASE12_SHIFT) + (TTBL_L2TBL_SMALL_PAGE_SIZE / 2) - 8);
    test_ptr       = (uint32_t *)((uint32_t)test_ptr + l2_mapva);
    test_ptr[0]    = 0xC001BABE;
    test_ptr       = (uint32_t *)((free_page0 << TTBL_L2TBL_TTE_BASE12_SHIFT) + TTBL_L2TBL_SMALL_PAGE_SIZE - 8);
    test_ptr       = (uint32_t *)((uint32_t)test_ptr + l2_mapva);
    test_ptr[0]    = 0xC001BABE;
    test_ptr       = (uint32_t *)(free_page1 << TTBL_L2TBL_TTE_BASE12_SHIFT);
    test_ptr       = (uint32_t *)((uint32_t)test_ptr + l2_mapva);
    test_ptr[1]    = 0xD00DFEED;
    test_ptr       = (uint32_t *)((free_page1 << TTBL_L2TBL_TTE_BASE12_SHIFT) + (TTBL_L2TBL_SMALL_PAGE_SIZE / 2) - 8);
    test_ptr       = (uint32_t *)((uint32_t)test_ptr + l2_mapva);
    test_ptr[1]    = 0xD00DFEED;
    test_ptr       = (uint32_t *)((free_page1 << TTBL_L2TBL_TTE_BASE12_SHIFT) + TTBL_L2TBL_SMALL_PAGE_SIZE - 8);
    test_ptr       = (uint32_t *)((uint32_t)test_ptr + l2_mapva);
    test_ptr[1]    = 0xD00DFEED;
    test_ptr       = (uint32_t *)(free_page0 << TTBL_L2TBL_TTE_BASE12_SHIFT);
    test_ptr       = (uint32_t *)((uint32_t)test_ptr + l2_mapva);
...
void arch_mmu_section_test(uint32_t *total, uint32_t *pass, uint32_t *fail)
{
    int      setup_required = 0;
    uint32_t ite, pos, free_sec[TEST_SECTION_COUNT];
    ...
    *fail  = 0x0;
    ...
    for (ite = 0; ite < (TTBL_L1TBL_SIZE / 4); ite++) {
        if ((l1[ite] & TTBL_L1TBL_TTE_TYPE_MASK) == TTBL_L1TBL_TTE_TYPE_FAULT) {
            free_sec[pos] = ite;
            pos++;
        }
    ...
    for (ite = 0; ite < TEST_SECTION_COUNT; ite++) {
        arm_mmu_section_test_iter(free_sec[ite], free_sec[(ite + 1) % TEST_SECTION_COUNT], total, pass, fail);
    }
...
void arch_mmu_page_test(uint32_t *total, uint32_t *pass, uint32_t *fail)
{
    int      setup_required = 0;
    uint32_t ite, pos, free_page[TEST_PAGE_COUNT];
    uint32_t sctlr = read_sctlr();
    ...
    *fail  = 0x0;
    ...
    for (ite = 0; ite < (TTBL_L2TBL_SIZE / 4); ite++) {
        if ((l2[ite] & TTBL_L2TBL_TTE_TYPE_MASK) == TTBL_L2TBL_TTE_TYPE_FAULT) {
            free_page[pos] = ite;
            pos++;
        }
    ...
    for (ite = 0; ite < TEST_PAGE_COUNT; ite++) {
        arm_mmu_page_test_iter(free_page[ite], free_page[(ite + 1) % TEST_PAGE_COUNT], total, pass, fail);
    }
...
{
    uint32_t s, sec, sec_tmpl = 0x0, sec_start = 0x0, sec_end = 0x0;
    uint32_t sctlr = read_sctlr();
    ...
    sec_start = ((uint32_t)&_code_start) & ~(TTBL_L1TBL_SECTION_PAGE_SIZE - 1);
    sec_end   = ((uint32_t)&_code_end) & ~(TTBL_L1TBL_SECTION_PAGE_SIZE - 1);
    ...
    for (s = 0; s < arch_board_iosection_count(); s++) {
        sec                                    = arch_board_iosection_addr(s) & ~(TTBL_L1TBL_SECTION_PAGE_SIZE - 1);
        l1[sec / TTBL_L1TBL_SECTION_PAGE_SIZE] = sec_tmpl | sec;
    }
    ...
    l2_mapva                                    = sec_end;
    l1[l2_mapva / TTBL_L1TBL_SECTION_PAGE_SIZE] = sec_tmpl | (uint32_t)(&l2);
