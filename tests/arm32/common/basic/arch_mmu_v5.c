...
static uint32_t __attribute__((aligned(TTBL_L1TBL_SIZE))) l1[TTBL_L1TBL_SIZE / 4];
static uint32_t __attribute__((aligned(TTBL_L2TBL_SIZE))) l2[TTBL_L2TBL_SIZE / 4];
static uint32_t                                           l2_mapva;

static uint32_t test_area_pa;
static uint32_t test_area_size;
...
extern void _switch_to_user_mode(uint32_t, uint32_t, uint32_t);

#define ARM_MMU_TEST_SWITCH_TO_USER()  _switch_to_user_mode(0x0, 0x0, 0)
#define ARM_MMU_TEST_SWITCH_TO_SUPER() asm("svc 0x1")

static uint32_t test_prefetch_abort_fs;
static uint32_t test_prefetch_abort_result;
...
static uint32_t test_data_abort_fs;
static uint32_t test_data_abort_far;
static uint32_t test_data_abort_dom;
static uint32_t test_data_abort_result;

void arm_mmu_data_abort(struct pt_regs *regs)
{
    uint32_t dfsr, dfar, fs, dom;
    ...
    if ((fs == test_data_abort_fs) && (dfar == test_data_abort_far) && (dom == test_data_abort_dom)) {
...
{
    uint32_t s, sec, sec_tmpl, sec_start, sec_end;
