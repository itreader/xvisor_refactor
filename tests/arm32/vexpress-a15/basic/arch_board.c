...
void arch_board_fdt_fixup(void *fdt_addr)
{
    ...
    rc = fdt_setprop(fdt_addr, poff, "compatible", str, basic_strlen(str) + 1);
    ...
    rc      = fdt_setprop(fdt_addr, poff, "#address-cells", vals, sizeof(uint32_t));
    ...
    vals[0] = cpu_to_fdt32(1);
    rc      = fdt_setprop(fdt_addr, poff, "#size-cells", vals, sizeof(uint32_t));
    ...
    rc = fdt_setprop(fdt_addr, noff, "compatible", str, basic_strlen(str) + 1);
    ...
    rc      = fdt_setprop(fdt_addr, noff, "reg", vals, sizeof(uint32_t) * 2);
    ...
    rc      = fdt_setprop(fdt_addr, noff, "interrupts", vals, sizeof(uint32_t) * 3);
...
{
    pl01x_init(CA15X4_UART_BASE, CA15X4_UART_TYPE, CA15X4_UART_BAUD, CA15X4_UART_INCLK);
