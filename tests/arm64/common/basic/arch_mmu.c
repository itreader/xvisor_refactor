...
void arm_sync_abort(struct pt_regs *regs)
{
    ...
    basic_printf("Bad synchronous exception @ PC: 0x%llX\n", (virtual_addr_t)regs->pc);
    basic_printf("ESR: 0x%08X (EC:0x%X, ISS:0x%X)\n", (uint32_t)esr, (uint32_t)ec, (uint32_t)iss);
    basic_printf("LR: 0x%llX, FAR: 0x%llX, PSTATE: 0x%X\n", (virtual_addr_t)regs->lr, (virtual_addr_t)far, (uint32_t)regs->pstate);
    ...
    for (ite = 0; ite < 30; ite++) {
        ...
        basic_printf("    X%02d=0x%016llx  ", ite, (uint64_t)regs->gpr[ite]);
    }
