...
#define VIRT_SIMPLE_FRAME_BUFFER (0x10002000)
...
{
    basic_strcpy(
        cmdline, "root=/dev/ram rw console=ttyS0,115200 "
...
void arch_board_fdt_fixup(void *fdt_addr)
{
    ...
    uint32_t timebase_freq = (uint32_t)vminfo_clocksource_freq(VIRT_VMINFO);
    uint32_t vcpu_count    = vminfo_vcpu_count(VIRT_VMINFO);

    vals                   = basic_malloc(sizeof(uint32_t) * 4 * vcpu_count);
    ...
    ret     = fdt_setprop(fdt_addr, cpus_offset, "timebase-frequency", vals, sizeof(uint32_t));

    if (ret < 0) {
        basic_printf(
            "Failed to set %s property of /cpus "
            "DT node\n",
            "timebase-frequency");
        return;
    }
    ...
    for (i = 0; i < vcpu_count; i++) {
        ...
        ret = fdt_setprop_string(fdt_addr, cpu_offset, "device_type", "cpu");

        if (ret < 0) {
            basic_printf(
                "Failed to set %s property of /cpus/%s "
                "DT node\n",
                "device_type", name);
            return;
        }

        ret = fdt_setprop_string(fdt_addr, cpu_offset, "compatible", "riscv");
        ...
        ret     = fdt_setprop(fdt_addr, cpu_offset, "reg", vals, sizeof(uint32_t));
...
#error "Unexpected __riscv_xlen"
        ...
        ret = fdt_setprop_string(fdt_addr, cpu_offset, "riscv,isa", isa);
        ...
        ret = fdt_setprop_string(fdt_addr, cpu_offset, "status", "okay");
        ...
        intc_offset = fdt_add_subnode(fdt_addr, cpu_offset, "interrupt-controller");
        ...
        ret = fdt_setprop_string(fdt_addr, intc_offset, "compatible", "riscv,cpu-intc");

        if (ret < 0) {
            basic_printf(
                "Failed to set %s property of "
                "/cpus/%s/interrupt-controller "
                "DT node\n",
                "compatible", name);
            return;
        ...
        ret = fdt_setprop(fdt_addr, intc_offset, "interrupt-controller", NULL, 0);
        ...
        ret     = fdt_setprop(fdt_addr, intc_offset, "#interrupt-cells", vals, sizeof(uint32_t));
        ...
        ret     = fdt_setprop(fdt_addr, intc_offset, "phandle", vals, sizeof(uint32_t));
        ...
        vals[0] = cpu_to_fdt32(100 + i);
        ret     = fdt_setprop(fdt_addr, intc_offset, "linux,phandle", vals, sizeof(uint32_t));
    ...
    plic_offset = fdt_node_offset_by_compatible(fdt_addr, -1, "riscv,plic0");

    if (plic_offset < 0) {
        return;
    }

    ret = fdt_setprop(fdt_addr, plic_offset, "interrupts-extended", vals, sizeof(uint32_t) * 4 * vcpu_count);

    if (ret < 0) {
        basic_printf(
            "Failed to set %s property of PLIC "
            "DT node\n",
            "interrupts-extended");
        return;
    }

    simple_frame_buffer_fdt_fixup(VIRT_SIMPLE_FRAME_BUFFER, fdt_addr);
}
