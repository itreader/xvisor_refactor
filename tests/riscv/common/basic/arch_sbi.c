...
static struct sbiret sbi_ecall(int ext, int fid, uint64_t arg0, uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4, uint64_t arg5)
{
    struct sbiret ret;
...
{
    sbi_ecall(SBI_EXT_0_1_REMOTE_SFENCE_VMA_ASID, 0, (uint64_t)hart_mask, start, size, asid, 0, 0);
}
...
uint64_t sbi_xvisor_isa_string(char *out_isa, uint64_t max_len)
{
    struct sbiret ret;
    uint64_t      pos = 0;
...
#error "Unexpected __riscv_xlen"
    ...
    for (i = 0; i < valid_isa_len && pos < max_len; i++) {
        ret = sbi_ecall(SBI_EXT_XVISOR, SBI_EXT_XVISOR_ISA_EXT, valid_isa_order[i] - 'a', 0, 0, 0, 0, 0);
...
void sbi_init(void)
{
    ...
    basic_printf("RISC-V SBI specification v%d.%d detected\n", (uint32_t)sbi_major_version(), (uint32_t)sbi_minor_version());
