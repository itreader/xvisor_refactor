...
static void printc(char **out, uint32_t *out_len, char ch)
{
    if (out) {
        if (*out) {
...
static int prints(char **out, uint32_t *out_len, const char *string, int width, int flags)
{
    int  pc      = 0;
    char padchar = ' ';
...
static int printi(char **out, uint32_t *out_len, long long i, int b, int sg, int width, int flags, int letbase)
{
...
static int print(char **out, uint32_t *out_len, const char *format, va_list args)
{
    ...
    for (; *format != 0; ++format) {
        if (*format == '%') {
            ...
            if ((*format == 'd') || (*format == 'i')) {
                pc += printi(out, out_len, va_arg(args, int), 10, 1, width, flags, '0');
                acnt += sizeof(int);
                continue;
            }

            if (*format == 'x') {
                pc += printi(out, out_len, va_arg(args, uint32_t), 16, 0, width, flags, 'a');
            ...
            if (*format == 'u') {
                pc += printi(out, out_len, va_arg(args, uint32_t), 10, 0, width, flags, 'a');
                acnt += sizeof(uint32_t);
                continue;
            }

            if (*format == 'p') {
                pc += printi(out, out_len, va_arg(args, uint64_t), 16, 0, width, flags, 'a');
            ...
            if (*format == 'l' && *(format + 1) == 'l') {
                ...
                if (sizeof(uint64_t) == sizeof(uint64_t)) {
                    ...
                    ((uint64_t *)&tmp)[0] = va_arg(args, uint64_t);
                    ((uint64_t *)&tmp)[1] = va_arg(args, uint64_t);
                    acnt += 2 * sizeof(uint64_t);
                }

                if (*(format + 2) == 'u') {
                    format += 2;
                    pc += printi(out, out_len, tmp, 10, 0, width, flags, 'a');
                } else if (*(format + 2) == 'x') {
                    format += 2;
                    pc += printi(out, out_len, tmp, 16, 0, width, flags, 'a');
                } else if (*(format + 2) == 'X') {
                    format += 2;
                    pc += printi(out, out_len, tmp, 16, 0, width, flags, 'A');
                } else {
                    format += 1;
                    pc += printi(out, out_len, tmp, 10, 1, width, flags, '0');
                }
                ...
                if (*(format + 1) == 'x') {
                    ...
                    pc += printi(out, out_len, va_arg(args, long), 10, 1, width, flags, '0');
                    acnt += sizeof(long);
                }
            }
...
int basic_snprintf(char *out, uint32_t out_sz, const char *format, ...)
{
    va_list args;
    int     retval;
    va_start(args, format);
