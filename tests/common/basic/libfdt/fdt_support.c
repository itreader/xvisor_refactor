...
static int fdt_fixup_stdout(void *fdt, int chosenoff)
{
    ...
    if (node >= 0) {
        ...
        if (path) {
            ...
            if (p) {
                memcpy(p, path, len);
                err = fdt_setprop(fdt, chosenoff, "linux,stdout-path", p, len);
                free(p);
            }
        } else {
            err = len;
...
int fdt_initrd(void *fdt, uint64_t initrd_start, uint64_t initrd_end, int force)
{
    int         nodeoffset;
    int         err, j, total;
    uint64_t    tmp;
...
int fdt_chosen(void *fdt, int force, char *linux_cmdline)
{
    ...
    //  if (str != NULL)
    {
        ...
        if ((path == NULL) || force) {
            err = fdt_setprop(fdt, nodeoffset, "bootargs", linux_cmdline, strlen(linux_cmdline) + 1);
...
void do_fixup_by_path_u32(void *fdt, const char *path, const char *prop, uint32_t val, int create)
{
    val = cpu_to_fdt32(val);
    do_fixup_by_path(fdt, path, prop, &val, sizeof(val), create);
}

void do_fixup_by_prop(void *fdt, const char *pname, const void *pval, int plen, const char *prop, const void *val, int len, int create)
{
    int off;
#if defined(DEBUG)
    int i;
...
void do_fixup_by_prop_u32(void *fdt, const char *pname, const void *pval, int plen, const char *prop, uint32_t val, int create)
{
    val = cpu_to_fdt32(val);
...
void do_fixup_by_compat_u32(void *fdt, const char *compat, const char *prop, uint32_t val, int create)
{
    val = cpu_to_fdt32(val);
 ...
 */
static void write_cell(uint8_t *addr, uint64_t val, int size)
{
    int shift = (size - 1) * 8;
...
int fdt_fixup_memory_banks(void *blob, uint64_t start[], uint64_t size[], int banks)
{
    ...
    uint8_t tmp[banks * 16]; /* Up to 64-bit address + 64-bit size */
    int     bank;
    ...
    err = fdt_setprop(blob, nodeoffset, "device_type", "memory", sizeof("memory"));
...
int fdt_resize(void *blob)
{
     ...
     */
    actualsize = fdt_off_device_tree_strings(blob) + fdt_size_device_tree_strings(blob) + 5 * sizeof(struct fdt_reserve_entry);

    /* Make it so the fdt ends on a page boundary */
    actualsize = ALIGN((actualsize + (((uint64_t)blob) & 0xfff)), 0x1000);
    actualsize = actualsize - ((uint64_t)blob & 0xfff);

    /* Change the fdt header to reflect the correct size */
    fdt_set_totalsize(blob, actualsize);

    /* Add the new reservation */
    ret = fdt_add_mem_rsv(blob, (uint64_t)blob, actualsize);
 ...
 */
uint32_t __flash_get_bank_size(int cs, int idx)
{
...
uint32_t flash_get_bank_size(int cs, int idx) __attribute__((weak, alias("__flash_get_bank_size")));
        ...
        while (off != -FDT_ERR_NOTFOUND) {
            ...
            if (prop) {
                ...
                for (idx = 0; idx < (len / tuple_size); idx++) {
                     ...
                     */
                    reg[2] = flash_get_bank_size(reg[0], idx);
            ...
            off = fdt_node_offset_by_compatible(blob, off, compat[i]);
        }
    }
...
int fdt_del_subnodes(const void *blob, int parent_offset)
{
    ...
    for (ndepth = 0, off = fdt_next_node(blob, parent_offset, &ndepth); (off >= 0) && (ndepth > 0); off = fdt_next_node(blob, off, &ndepth)) {
        if (ndepth == 1) {
...
int fdt_node_set_part_info(void *blob, int parent_offset, struct mtd_device *dev)
{
    list_head_t      *pentry;
 ...
 *      { "fsl,mpc5121-nfc",    MTD_DEVICE_TYPE_NAND, },
 *      { "cfi-flash",          MTD_DEVICE_TYPE_NOR,  },
 *  };
 *
...
void fdt_fixup_mtdparts(void *blob, void *node_info, int node_info_size)
{
    ...
    for (i = 0; i < node_info_size; i++) {
        ...
        while (noff != -FDT_ERR_NOTFOUND) {
            debug_printf("%s: %s, mtd dev type %d\n", fdt_get_name(blob, noff, 0), ni[i].compat, ni[i].type);
            ...
            noff = fdt_node_offset_by_compatible(blob, noff, ni[i].compat);
        }
    }
}
#endif
...
/* Debug utility */
#ifdef DEBUG
static void of_dump_addr(const char *s, const uint32_t *addr, int na)
{
    printf("%s", s);
...
struct of_bus {
    ...
    void (*count_cells)(void *blob, int parentoffset, int *addrc, int *sizec);
    uint64_t (*map)(uint32_t *addr, const uint32_t *range, int na, int ns, int pna);
    int (*translate)(uint32_t *addr, uint64_t offset, int na);
};

/* Default translator (generic bus) */
static void of_bus_default_count_cells(void *blob, int parentoffset, int *addrc, int *sizec)
{
    const uint32_t *prop;
    ...
    if (addrc) {
        ...
        if (prop) {
            *addrc = be32_to_cpup((uint32_t *)prop);
        } else {
            *addrc = 2;
        }
    }
    ...
    if (sizec) {
        ...
        if (prop) {
            *sizec = be32_to_cpup((uint32_t *)prop);
        } else {
            *sizec = 1;
        }
    }
...
static uint64_t of_bus_default_map(uint32_t *addr, const uint32_t *range, int na, int ns, int pna)
{
    uint64_t cp, s, da;
...
static int of_translate_one(
    void *blob, int parent, struct of_bus *bus, struct of_bus *pbus, uint32_t *addr, int na, int ns, int pna, const char *rprop)
{
 ...
 */
uint64_t __of_translate_address(void *blob, int node_offset, const uint32_t *in_addr, const char *rprop)
{
    int            parent;
    struct of_bus *bus, *pbus;
    uint32_t       addr[OF_MAX_ADDR_CELLS];
    int            na, ns, pna, pns;
    ...
    for (;;) {
        ...
        if (of_translate_one(blob, node_offset, bus, pbus, addr, na, ns, pna, rprop)) {
            break;
        }
 ...
 *
 */
int fdt_node_offset_by_compat_reg(void *blob, const char *compat, uint64_t compat_off)
{
    int len, off = fdt_node_offset_by_compatible(blob, -1, compat);

    while (off != -FDT_ERR_NOTFOUND) {
        uint32_t *reg = (uint32_t *)fdt_getprop(blob, off, "reg", &len);
...
int fdt_set_phandle(void *fdt, int nodeoffset, uint32_t phandle)
{
    ...
    if ((off >= 0) && (off != nodeoffset)) {
        ...
        printf("Trying to update node %s with phandle %u ", buf, phandle);
 ...
 * @nodeoffset: node to update
 */
uint32_t fdt_create_phandle(void *fdt, int nodeoffset)
{
 ...
 */
int fdt_set_node_status(void *fdt, int nodeoffset, enum fdt_status status, uint32_t error_code)
{
    char buf[16];
    int  ret = 0;
 ...
 * @error_code: optional, only used if status is FDT_STATUS_FAIL_ERROR_CODE
 */
int fdt_set_status_by_alias(void *fdt, const char *alias, enum fdt_status status, uint32_t error_code)
{
 ...
 */
int fdt_verify_alias_address(void *fdt, int anode, const char *alias, uint64_t addr)
{
    const char     *path;
    const uint32_t *reg;
    int             node, len;
    ...
    if (addr != dt_addr) {
        printf(
            "Warning: U-Boot configured device %s at address " PRu64 ",\n"
