...
void basic_cmd_wfi_test(int argc, char **argv)
{
    char     str[256];
    int      delay = 1000;
    uint64_t tstamp, tcount, wcount = 0;
...
{
    char     str[32];
    uint32_t total = 0x0, pass = 0x0, fail = 0x0;
...
{
    char     str[32];
    uint64_t irq_count, irq_delay, tstamp;
...
void basic_cmd_hexdump(int argc, char **argv)
{
    ...
    addr  = (uint32_t *)(virtual_addr_t)basic_hexstr2ulonglong(argv[1]);
...
void basic_cmd_copy(int argc, char **argv)
{
    ...
    /* Copy contents */
    if (!((virtual_addr_t)dst & 0x7) && !((virtual_addr_t)src & 0x7) && !(count & 0x7)) {
        for (i = 0; i < (count / sizeof(uint64_t)); i++) {
            ((uint64_t *)dst)[i] = ((uint64_t *)src)[i];
        }
    } else if (!((virtual_addr_t)dst & 0x3) && !((virtual_addr_t)src & 0x3) && !(count & 0x3)) {
        for (i = 0; i < (count / sizeof(uint32_t)); i++) {
            ((uint32_t *)dst)[i] = ((uint32_t *)src)[i];
        }
    } else if (!((virtual_addr_t)dst & 0x1) && !((virtual_addr_t)src & 0x1) && !(count & 0x1)) {
        for (i = 0; i < (count / sizeof(uint16_t)); i++) {
            ((uint16_t *)dst)[i] = ((uint16_t *)src)[i];
        }
    } else {
        for (i = 0; i < (count / sizeof(uint8_t)); i++) {
...
void basic_cmd_start_linux_fdt(int argc, char **argv)
{
    ...
    arch_start_linux_prep(kernel_addr, fdt_addr, initrd_addr, initrd_size);
    ...
    basic_sprintf(cfg_str, " mem=%dM", (uint32_t)(memory_size >> 20));
     ...
     *      - number of cpus
     */
    if ((err = fdt_fixup_memory_banks((void *)fdt_addr, (&meminfo[0]), (&meminfo[1]), 1))) {
    ...
    if (initrd_size) {
        if ((err = fdt_initrd((void *)fdt_addr, initrd_addr, initrd_addr + initrd_size, 1))) {
    ...
    arch_start_linux_jump(kernel_addr, fdt_addr, initrd_addr, initrd_size);
...
void basic_cmd_fdt_override_u32(int argc, char **argv)
{
    ...
    *(prop++)  = '\0';
    ...
    err = fdt_setprop(fdt, nodeoffset, prop, &val, sizeof(uint32_t));

    if (err) {
        printf(
            "*** Failed to set property \"%s\" of node \"%s\". "
            "Error: %i\n",
            prop, path, err);
        return;
    }
...
void basic_cmd_linux_memory_size(int argc, char **argv)
{
    ...
    if (argc == 2) {
        memory_size = (uint32_t)basic_hexstr2uint(argv[1]);
    }
...
#define BASIC_CMD_AUTOEXEC_BUF_SIZE 4096
    ...
    while ((len < BASIC_CMD_AUTOEXEC_BUF_SIZE) && basic_isprintable(ptr[len])) {
        len++;
    }
...
{
    uint64_t tstamp;
    uint32_t i, key_pressed, boot_delay;
    ...
    if (boot_delay == 0xffffffff) {
        ...
        while (boot_delay) {
            ...
            while ((arch_board_timer_timestamp() - tstamp) < 1000000000) {
                for (i = 0; i < 10000; i++)
                    ;
