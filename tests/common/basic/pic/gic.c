...
static inline void gic_write(uint32_t val, virtual_addr_t addr)
{
    arch_writel(val, (void *)(addr));
}

static inline uint32_t gic_read(virtual_addr_t addr)
{
    return arch_readl((void *)(addr));
}

int gic_active_irq(uint32_t gic_nr)
{
    int ret = -1;
...
int gic_eoi_irq(uint32_t gic_nr, uint32_t irq)
{
    uint32_t gic_irq;
...
int gic_mask(uint32_t gic_nr, uint32_t irq)
{
    ...
    gic_write(mask, gic_data[gic_nr].dist_base + GIC_DIST_ENABLE_CLEAR + (gic_irq / 32) * 4);

    return 0;
}

int gic_unmask(uint32_t gic_nr, uint32_t irq)
{
    uint32_t mask = 1 << (irq % 32);
    uint32_t gic_irq;
    ...
    gic_write(mask, gic_data[gic_nr].dist_base + GIC_DIST_ENABLE_SET + (gic_irq / 32) * 4);

    return 0;
}

int gic_dist_init(uint32_t gic_nr, virtual_addr_t base, uint32_t irq_start)
{
    uint32_t max_irq, i;
    ...
    for (i = 0; i < max_irq; i += 32) {
        gic_write(0xffffffff, base + GIC_DIST_ENABLE_CLEAR + i * 4 / 32);
    }
...
int gic_cpu_init(uint32_t gic_nr, virtual_addr_t base)
{
    if (GIC_MAX_NR <= gic_nr) {
        return -1;
    }
