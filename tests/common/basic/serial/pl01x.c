...
void pl01x_putc(virtual_addr_t base, uint32_t type, char ch)
{
...
bool pl01x_can_getc(virtual_addr_t base, uint32_t type)
{
    return (arch_readl((void *)(base + UART_PL01x_FR)) & UART_PL01x_FR_RXFE) ? FALSE : TRUE;
}

char pl01x_getc(virtual_addr_t base, uint32_t type)
{
    char data;
...
void pl01x_init(virtual_addr_t base, uint32_t type, uint32_t baudrate, uint32_t input_clock)
{
    uint32_t divider;
    uint32_t temp;
    uint32_t remainder;
    ...
    if (type == PL01X_TYPE_1) {
         ...
         */
        arch_writel((UART_PL011_LCRH_WLEN_8 | UART_PL011_LCRH_FEN), (void *)(base + UART_PL011_LCRH));

        /* Finally, enable the UART */
        arch_writel((UART_PL011_CR_UARTEN | UART_PL011_CR_TXE | UART_PL011_CR_RXE), (void *)(base + UART_PL011_CR));
    } else {
        /* First, disable everything */
        ...
        arch_writel(((divider & 0xf00) >> 8), (void *)(base + UART_PL010_LCRM));
        arch_writel((divider & 0xff), (void *)(base + UART_PL010_LCRL));

        /* Set the UART to be 8 bits, 1 stop bit,
         * no parity, fifo enabled */
        arch_writel((UART_PL010_LCRH_WLEN_8 | UART_PL010_LCRH_FEN), (void *)(base + UART_PL010_LCRH));

        /* Finally, enable the UART */
        arch_writel((UART_PL010_CR_UARTEN), (void *)(base + UART_PL010_CR));
    }
}
