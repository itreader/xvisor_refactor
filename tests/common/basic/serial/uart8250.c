...
static uint32_t get_reg(virtual_addr_t base, uint32_t reg_shift, uint32_t reg_width, uint32_t num)
{
    uint32_t offset = num << reg_shift;
...
static void set_reg(virtual_addr_t base, uint32_t reg_shift, uint32_t reg_width, uint32_t num, uint32_t val)
{
    uint32_t offset = num << reg_shift;
...
bool uart8250_can_getc(virtual_addr_t base, uint32_t reg_shift, uint32_t reg_width)
{
    if (!(get_reg(base, reg_shift, reg_width, UART_LSR_OFFSET) & UART_LSR_DR)) {
        return FALSE;
    }
...
char uart8250_getc(virtual_addr_t base, uint32_t reg_shift, uint32_t reg_width)
{
    while (!(get_reg(base, reg_shift, reg_width, UART_LSR_OFFSET) & UART_LSR_DR))
        ;

    return get_reg(base, reg_shift, reg_width, UART_RBR_OFFSET);
}

void uart8250_putc(virtual_addr_t base, uint32_t reg_shift, uint32_t reg_width, char ch)
{
    while ((get_reg(base, reg_shift, reg_width, UART_LSR_OFFSET) & UART_LSR_THRE) == 0)
        ;

    set_reg(base, reg_shift, reg_width, UART_THR_OFFSET, ch);
}

void uart8250_init(virtual_addr_t base, uint32_t reg_shift, uint32_t reg_width, uint32_t baudrate, uint32_t input_clock)
{
    uint16_t bdiv   = input_clock / (16 * baudrate);
    uint8_t  bdiv_l = bdiv & 0xff;
    uint8_t  bdiv_u = (bdiv >> 8) & 0xff;
