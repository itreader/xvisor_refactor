...
#define RPMSG_TEST_HEADER_LEN             (3 * sizeof(uint64_t))
...
#define RPMSG_TEST_PAYLOAD_MAX_SIZE(rmax) (RPMSG_TEST_MAX_BUFF_SIZE(rmax) - RPMSG_TEST_HEADER_LEN)
#define RPMSG_TEST_NUM_PAYLOADS(rmax)     (RPMSG_TEST_PAYLOAD_MAX_SIZE(rmax) / RPMSG_TEST_PAYLOAD_MIN_SIZE(min))

uint64_t rpmsg_timestamp_usecs(void)
{
    struct timeval  tv;
    struct timezone tz;
...
char *get_rpmsg_ept_dev_name(char *rpmsg_char_name, char *ept_name, char *ept_dev_name)
{
...
int main(int argc, char *argv[])
{
    ...
    if (is_server) {
        ...
        printf(
            "Rpmsg test started for %d rounds and "
    ...
    printf("Rpmsg test max payload size %d bytes (%d bytes)\n", (int)RPMSG_TEST_PAYLOAD_MAX_SIZE(rmax), (int)RPMSG_TEST_MAX_BUFF_SIZE(rmax));
    ...
    while (is_server) {
        r_payload->num = 0;
        bytes_rcvd     = read(fd, r_payload, RPMSG_TEST_MAX_BUFF_SIZE(rmax));
    ...
    for (i = 0; i < ntimes; i++) {
        ...
        memset(&(i_payload->data[0]), 0xA5, RPMSG_TEST_PAYLOAD_MAX_SIZE(rmax));

        usecs   = rpmsg_timestamp_usecs();

        err_cnt = 0;
        size    = (fpayload) ? fsize : RPMSG_TEST_PAYLOAD_MIN_SIZE(rmax);
        tpos = rpos = 0;
        tbytes = rbytes = 0;
        ...
        while ((tpos < pcount) || (rpos < pcount)) {
            ...
            i_payload->num   = tpos;
            i_payload->size  = size;
            i_payload->flags = (is_unidir) ? 0x0 : RPMSG_TEST_FLAG_ECHO;

            bytes_sent       = write(fd, i_payload, RPMSG_TEST_HEADER_LEN + size);
            ...
            if (is_verbose) {
                printf(
                    "Rpmsg test Tx #%ld payload %d total"
                    " %d\n",
                    i_payload->num, size, bytes_sent);
            }
        ...
        skip_tx:
            ...
            if (is_verbose) {
                printf(
                    "Rpmsg test Rx #%ld payload %ld total"
                    " %d\n",
                    r_payload->num, (is_unidir) ? 0 : r_payload->size, bytes_rcvd);
            }
        ...
        printf(
            "Rpmsg test round %d Tx %d payloads Rx %d payloads "
            "%d errors\n",
            i, tpos, rpos, err_cnt);
