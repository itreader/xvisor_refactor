 ...
 */
static uint32_t platform_msi_calc_hwirq(struct vmm_msi_descriptor *desc)
{
...
static void platform_msi_write_msg(vmm_msi_domain_t *domain, struct vmm_msi_descriptor *desc, uint32_t hirq, uint32_t hwirq, struct vmm_msi_msg *msg)
{
...
{
    struct vmm_msi_descriptor *desc, *tmp;
...

{
    ...
    if (!list_empty(dev_to_msi_list(dev))) {
        desc = list_last_entry(dev_to_msi_list(dev), struct vmm_msi_descriptor, list);
