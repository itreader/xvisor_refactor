...
static void dos_process_extended_part(vmm_block_device_t *block_device, struct dos_partition *parent)
{
    ...
    while (1) {
        ...
        addr = (parent->lba_start + rel) * block_device->block_size;
        addr += DOS_MBR_SIGN_OFFSET;
        read = vmm_block_device_read(block_device, (uint8_t *)&sign, addr, sizeof(uint16_t));
        ...
        read = vmm_block_device_read(block_device, (uint8_t *)&part[0], addr, sizeof(part));
        ...
        /* Process first entry */
        addr = parent->lba_start + rel + part[0].lba_start;
        rc   = vmm_block_device_add_child(block_device, addr, part[0].sector_count);
