...
#define ptr_align(p, a)           ((typeof(p))align((uint64_t)(p), (a)))
...
#define roundup2_order_size(x, order) (((x) & order_mask(order)) ? order_align(x, order) + order_size(order) : (x))

/** Calulate number of 2^order sized chunks required to cover size x */
#define size_to_order(x, order)       (((x) >> (order)) + (((x) & order_mask(order)) ? 1 : 0))
