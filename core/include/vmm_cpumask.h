...
/**
 * to_cpumask - convert an CONFIG_CPU_COUNT bitmap to a vmm_cpumask_t *
 * @bitmap: the bitmap
 *
 * This does the conversion, and can be used as a constant initializer.
 */
#define to_cpumask(bitmap)          \
    ((vmm_cpumask_t *)(1 ? (bitmap) \
                         : (void *)sizeof(__check_is_bitmap(bitmap))))

static inline int __check_is_bitmap(const uint64_t *bitmap)
{
    return 1;
}
...
static inline const vmm_cpumask_t *get_cpu_mask(uint32_t cpu)
{
...
extern const vmm_cpumask_t *const cpu_possible_mask;
extern const vmm_cpumask_t *const cpu_online_mask;
extern const vmm_cpumask_t *const cpu_present_mask;
extern const vmm_cpumask_t *const cpu_active_mask;
...
static inline uint32_t vmm_cpumask_check(uint32_t cpu)
{
...
static inline uint32_t vmm_cpumask_next_and(int n,
        const vmm_cpumask_t *srcp,
        const vmm_cpumask_t *andp)
{
    return n + 1;
}

/* cpu must be a valid cpu, ie 0, so there's no other choice. */
static inline uint32_t vmm_cpumask_any_but(const vmm_cpumask_t *mask,
        uint32_t cpu)
{
    return 1;
}
