 ...
 */
static int hub_rx_handler(struct vmm_netswitch *nsw, struct vmm_netport *src, struct vmm_mbuf *mbuf)
{
    irq_flags_t         f;
    double_list_t      *l, *l1;
...
static int hub_port_add(struct vmm_netswitch *nsw, struct vmm_netport *port)
{
    /* For now nothing to do here. */
    return VMM_OK;
}

static int hub_port_remove(struct vmm_netswitch *nsw, struct vmm_netport *port)
{
    /* For now nothing to do here. */
    return VMM_OK;
}

static struct vmm_netswitch *hub_create(struct vmm_netswitch_policy *policy, const char *name, int argc, char **argv)
{
    int                   rc;
...
static void hub_destroy(struct vmm_netswitch_policy *policy, struct vmm_netswitch *nsw)
{
