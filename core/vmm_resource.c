...
vmm_resource_t vmm_hostio_resource = {
    .name  = "Host IO",
    .start = 0,
...
vmm_resource_t vmm_hostmem_resource = {
    .name  = "Host Memory",
    .start = 0,
    .end   = -1,
...
static int find_next_res(vmm_resource_t *root, vmm_resource_t *res, int *res_level, char *name, bool check_flags, bool first_level_children_only)
{
    ...
    for (p = root->child; p; p = next_resource(p, sibling_only)) {
    ...
    vmm_resource_t *root, void *arg, int (*func)(const char *name, uint64_t start, uint64_t end, uint64_t flags, int level, void *arg))
{
    ...
    ret = (*func)(root->name, root->start, root->end, root->flags, level, arg);
    ...
    while ((res.start < res.end) && (!find_next_res(root, &res, &level, NULL, FALSE, FALSE))) {
        ret = (*func)(res.name, res.start, res.end, res.flags, level, arg);
...
int vmm_walk_hostmem_res(char *name, uint64_t flags, uint64_t start, uint64_t end, void *arg, int (*func)(uint64_t, uint64_t, void *))
{
    ...
    while ((res.start < res.end) && (!find_next_res(&vmm_hostmem_resource, &res, NULL, name, TRUE, FALSE))) {
...
int vmm_walk_system_ram_res(uint64_t start, uint64_t end, void *arg, int (*func)(uint64_t, uint64_t, void *))
{
    ...
    while ((res.start < res.end) && (!find_next_res(&vmm_hostmem_resource, &res, NULL, "System RAM", TRUE, TRUE))) {
        ret = (*func)(res.start, res.end, arg);
...
int vmm_walk_system_ram_range(uint64_t start_pfn, uint64_t nr_pages, void *arg, int (*func)(uint64_t, uint64_t, void *))
{
    ...
    res.start          = (uint64_t)start_pfn << VMM_PAGE_SHIFT;
    res.end            = ((uint64_t)(start_pfn + nr_pages) << VMM_PAGE_SHIFT) - 1;
    res.flags          = VMM_IORESOURCE_MEM | VMM_IORESOURCE_BUSY;
    orig_end           = res.end;

    while ((res.start < res.end) && (find_next_res(&vmm_hostmem_resource, &res, NULL, "System RAM", TRUE, TRUE) >= 0)) {
    ...
    while ((res = *p)) {
        ...
        if (res->start == start && res->end == end) {
            ...
            *p = res->sibling;
            ...
            /* adjust the start */
            ret = __adjust_resource(res, end + 1, res->end - end);
        } else if (res->start != start && res->end == end) {
            /* adjust the end */
            ret = __adjust_resource(res, res->start, start - res->start);
        } else {
...
vmm_resource_t *__vmm_devm_request_region(vmm_device_t *dev, vmm_resource_t *parent, resource_size_t start, resource_size_t n, const char *name)
{
    ...
    dr = vmm_device_resource_alloc(devm_region_release, sizeof(struct region_device_resource));
    ...
    if (res) {
        vmm_device_resource_add(dev, dr);
    } else {
        vmm_device_resource_free(dr);
    }
...
void __vmm_devm_release_region(vmm_device_t *dev, vmm_resource_t *parent, resource_size_t start, resource_size_t n)
{
    ...
    WARN_ON(vmm_device_resource_destroy(dev, devm_region_release, devm_region_match, &match_data));
}
...
static int __init reserve_setup(char *str)
{
    ...
    for (;;) {
        ...
        if (x < MAXRESERVE) {
            ...
            if (vmm_request_resource(res->start >= 0x10000 ? &vmm_hostmem_resource : &vmm_hostio_resource, res) == 0) {
                reserved = x + 1;
            }
        }
    }
...
int vmm_hostmem_map_sanity_check(resource_size_t addr, uint64_t size)
{
    ...
    for (p = p->child; p; p = r_next(p, &l)) {
        ...
        if (VMM_PFN_DOWN(p->start) <= VMM_PFN_DOWN(addr) && VMM_PFN_DOWN(p->end) >= VMM_PFN_DOWN(addr + size - 1)) {
            continue;
        }
        ...
        vmm_printf(
            "resource sanity check: requesting "
            "[mem %#010llx-%#010llx], which spans "
            "more than %s %pR\n",
            (uint64_t)addr, (uint64_t)(addr + size - 1), p->name, p);
        err = -1;
        break;
    }
...
int vmm_hostmem_is_exclusive(uint64_t addr)
{
    ...
    for (p = p->child; p; p = r_next(p, &l)) {
        ...
        if (p->flags & VMM_IORESOURCE_BUSY && p->flags & VMM_IORESOURCE_EXCLUSIVE) {
            err = 1;
            break;
        }
    }
