...
static void cmd_memory_usage(vmm_char_device_t *cdev)
{
    vmm_cdev_printf(cdev, "Usage: ");
    vmm_cdev_printf(cdev, "   memory help\n");
    vmm_cdev_printf(cdev, "   memory dump8    <phys_addr> <count> [<host_cpu>]\n");
    vmm_cdev_printf(cdev, "   memory dump16   <phys_addr> <count> [<host_cpu>]\n");
    vmm_cdev_printf(cdev, "   memory dump32   <phys_addr> <count> [<host_cpu>]\n");
    vmm_cdev_printf(cdev, "   memory iodump8  <phys_addr> <count> [<host_cpu>]\n");
    vmm_cdev_printf(cdev, "   memory iodump16 <phys_addr> <count> [<host_cpu>]\n");
    vmm_cdev_printf(cdev, "   memory iodump32 <phys_addr> <count> [<host_cpu>]\n");
    vmm_cdev_printf(cdev, "   memory crc32    <phys_addr> <count>\n");
#if CONFIG_CRYPTO_HASH_MD5
    vmm_cdev_printf(cdev, "   memory md5      <phys_addr> <count>\n");
#endif
#if CONFIG_CRYPTO_HASH_SHA256
    vmm_cdev_printf(cdev, "   memory sha256   <phys_addr> <count>\n");
#endif
    vmm_cdev_printf(
        cdev, "   memory modify8  <phys_addr> "
              "<val0> <val1> ...\n");
    vmm_cdev_printf(
        cdev, "   memory modify16 <phys_addr> "
              "<val0> <val1> ...\n");
    vmm_cdev_printf(
        cdev, "   memory modify32 <phys_addr> "
              "<val0> <val1> ...\n");
    vmm_cdev_printf(
        cdev, "   memory copy     <phys_addr> <src_phys_addr> "
              "<byte_count>\n");
}
...
static void memory_dump_func(void *arg0, void *arg1, void *arg2)
{
    ...
    vmm_char_device_t          *cdev = dreq->cdev;
    physical_addr_t             addr = dreq->addr;
    physical_addr_t             page_pa;
    uint32_t                    wsz  = dreq->wsz;
    uint32_t                    wcnt = dreq->wcnt;
    bool                        io   = dreq->io;
    bool                        page_mapped;
    int                         rc;
    uint32_t                    w;
    ...
    while (w < wcnt) {
        ...
        switch (wsz) {
            case 1:
                vmm_cdev_printf(cdev, " %02x", *((uint8_t *)(page_va + addr_offset)));
                break;

            case 2:
                vmm_cdev_printf(cdev, " %04x", *((uint16_t *)(page_va + addr_offset)));
                break;

            case 4:
                vmm_cdev_printf(cdev, " %08x", *((uint32_t *)(page_va + addr_offset)));
                break;
...
static int cmd_memory_dump(vmm_char_device_t *cdev, physical_addr_t addr, uint32_t wsz, uint32_t wcnt, bool io, int host_cpu)
{
...
static int cmd_memory_crc32(vmm_char_device_t *cdev, physical_addr_t addr, uint32_t wcnt)
{
    int             rc;
    uint32_t        w, crc = 0;
    bool            page_mapped;
    ...
    vmm_cdev_printf(cdev, "CRC32 for 0x%" PRIPADDR " - 0x%" PRIPADDR ":\n", addr, (addr + wcnt));
    ...
    while (w < wcnt) {
        if (page_pa != (addr - (addr & VMM_PAGE_MASK))) {
            if (page_mapped) {
                ...
                if (rc) {
                    vmm_cdev_printf(cdev, "Error: Failed to unmap memory.\n");
                    return rc;
                }
        ...
        addr_offset = (addr & VMM_PAGE_MASK);
        crc         = crc32_tab[(crc ^ *((uint8_t *)(page_va + addr_offset))) & 0xFF] ^ (crc >> 8);
        addr += 1;
        w++;
    }

    crc = crc ^ ~0U;
    vmm_cdev_printf(cdev, "%08x\n", crc);
...
#if CONFIG_CRYPTO_HASH_MD5
static int cmd_memory_md5(vmm_char_device_t *cdev, physical_addr_t addr, uint32_t wcnt)
{
    int                rc, i;
    ...
    vmm_cdev_printf(cdev, "MD5 digest for 0x%" PRIPADDR " - 0x%" PRIPADDR ":\n", addr, (addr + wcnt));
    ...
    md5_init(&md5c);
    md5_update(&md5c, (uint8_t *)page_va, wcnt);
    md5_final(signature, &md5c);

    for (i = 0; i < 16; i++) {
        vmm_cdev_printf(cdev, "%x", signature[i]);
    }
...
#if CONFIG_CRYPTO_HASH_SHA256
static int cmd_memory_sha256(vmm_char_device_t *cdev, physical_addr_t addr, uint32_t wcnt)
{
    int                   rc, i;
    ...
    vmm_cdev_printf(cdev, "SHA-256 digest for 0x%" PRIPADDR " - 0x%" PRIPADDR ":\n", addr, (addr + wcnt));
    ...
    sha256_init(&sha256c);
    sha256_update(&sha256c, (uint8_t *)page_va, wcnt);
...
static int cmd_memory_modify(vmm_char_device_t *cdev, physical_addr_t addr, uint32_t wsz, int valc, char **valv)
{
    int             rc, w = 0;
    bool            page_mapped;
    virtual_addr_t  page_va, addr_offset;
    ...
    while (w < valc) {
        ...
        switch (wsz) {
            case 1:
                *((uint8_t *)(page_va + addr_offset)) = (uint8_t)strtoul(valv[w], NULL, 0);
                break;

            case 2:
                *((uint16_t *)(page_va + addr_offset)) = (uint16_t)strtoul(valv[w], NULL, 0);
                break;

            case 4:
                *((uint32_t *)(page_va + addr_offset)) = (uint32_t)strtoul(valv[w], NULL, 0);
                break;
...
static int cmd_memory_copy(vmm_char_device_t *cdev, physical_addr_t daddr, physical_addr_t saddr, uint32_t bcnt)
{
    int             rc;
    uint32_t        b = 0, b2copy;
    ...
    while (b < bcnt) {
        ...
        if ((daddr & VMM_PAGE_MASK) < (saddr & VMM_PAGE_MASK)) {
            b2copy = VMM_PAGE_SIZE - (uint32_t)(saddr & VMM_PAGE_MASK);
        } else {
            b2copy = VMM_PAGE_SIZE - (uint32_t)(daddr & VMM_PAGE_MASK);
        }
    ...
    vmm_cdev_printf(cdev, "Copied %d (0x%x) bytes.\n", b, b);
...
static int cmd_memory_exec(vmm_char_device_t *cdev, int argc, char **argv)
{
    uint32_t        tmp;
    int             host_cpu;
    ...
    if (strcmp(argv[1], "dump8") == 0) {
        ...
        host_cpu = (4 < argc) ? atoi(argv[4]) : -1;
        return cmd_memory_dump(cdev, addr, 1, (uint32_t)tmp, FALSE, host_cpu);
    } else if (strcmp(argv[1], "dump16") == 0) {
        tmp      = strtoull(argv[3], NULL, 0);
        host_cpu = (4 < argc) ? atoi(argv[4]) : -1;
        return cmd_memory_dump(cdev, addr, 2, (uint32_t)tmp, FALSE, host_cpu);
    } else if (strcmp(argv[1], "dump32") == 0) {
        tmp      = strtoull(argv[3], NULL, 0);
        host_cpu = (4 < argc) ? atoi(argv[4]) : -1;
        return cmd_memory_dump(cdev, addr, 4, (uint32_t)tmp, FALSE, host_cpu);
    } else if (strcmp(argv[1], "iodump8") == 0) {
        tmp      = strtoull(argv[3], NULL, 0);
        host_cpu = (4 < argc) ? atoi(argv[4]) : -1;
        return cmd_memory_dump(cdev, addr, 1, (uint32_t)tmp, TRUE, host_cpu);
    } else if (strcmp(argv[1], "iodump16") == 0) {
        tmp      = strtoull(argv[3], NULL, 0);
        host_cpu = (4 < argc) ? atoi(argv[4]) : -1;
        return cmd_memory_dump(cdev, addr, 2, (uint32_t)tmp, TRUE, host_cpu);
    } else if (strcmp(argv[1], "iodump32") == 0) {
        tmp      = strtoull(argv[3], NULL, 0);
        host_cpu = (4 < argc) ? atoi(argv[4]) : -1;
        return cmd_memory_dump(cdev, addr, 4, (uint32_t)tmp, TRUE, host_cpu);
    } else if (strcmp(argv[1], "crc32") == 0) {
        tmp = strtoull(argv[3], NULL, 0);
        return cmd_memory_crc32(cdev, addr, (uint32_t)tmp);
#if CONFIG_CRYPTO_HASH_MD5
    } else if (strcmp(argv[1], "md5") == 0) {
        tmp = strtoull(argv[3], NULL, 0);
        return cmd_memory_md5(cdev, addr, (uint32_t)tmp);
#endif
#if CONFIG_CRYPTO_HASH_SHA256
    } else if (strcmp(argv[1], "sha256") == 0) {
        tmp = strtoull(argv[3], NULL, 0);
        return cmd_memory_sha256(cdev, addr, (uint32_t)tmp);
#endif
...
{
    return vmm_command_manager_register_cmd(&cmd_memory);
}

static void __exit cmd_memory_exit(void)
{
    vmm_command_manager_unregister_cmd(&cmd_memory);
}
