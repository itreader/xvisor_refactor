...
static void cmd_version_usage(vmm_char_device_t *cdev)
{
    vmm_cdev_printf(cdev, "Usage: ");
    vmm_cdev_printf(cdev, "   version\n");
}

static int cmd_version_exec(vmm_char_device_t *cdev, int argc, char **argv)
{
    vmm_cdev_print_version(cdev);
    return VMM_OK;
}

static vmm_command_t cmd_version = {
    .name  = "version",
...
{
    return vmm_command_manager_register_cmd(&cmd_version);
}

static void __exit cmd_version_exit(void)
{
    vmm_command_manager_unregister_cmd(&cmd_version);
}
