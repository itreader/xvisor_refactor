...
static void cmd_sleep_usage(vmm_char_device_t *cdev)
{
    vmm_cdev_printf(cdev, "Usage:\n");
    vmm_cdev_printf(cdev, "   sleep help\n");
    vmm_cdev_printf(cdev, "   sleep secs <number_of_seconds>\n");
    vmm_cdev_printf(cdev, "   sleep msecs <number_of_milliseconds>\n");
    vmm_cdev_printf(cdev, "   sleep usecs <number_of_microseconds>\n");
    vmm_cdev_printf(
        cdev, "   sleep test_secs <number_of_iterations> "
              "<seconds_per_iteration>\n");
    vmm_cdev_printf(
        cdev, "   sleep test_msecs <number_of_iterations> "
              "<milliseconds_per_iteration>\n");
    vmm_cdev_printf(
        cdev, "   sleep test_usecs <number_of_iterations> "
...
static int cmd_sleep_test(vmm_char_device_t *cdev, enum cmd_sleep_type type, int iter, int val)
{
    int      i;
    uint64_t delta, avgdelta = 0;
    ...
    for (i = 0; i < iter; i++) {
        ...
        vmm_cdev_printf(cdev, "delta %" PRId64 " nanoseconds\n", delta);
    }

    avgdelta = udiv64(avgdelta, (uint64_t)iter);
    vmm_cdev_printf(cdev, "average delta %" PRId64 " nanoseconds\n", avgdelta);
...
static int cmd_sleep_exec(vmm_char_device_t *cdev, int argc, char **argv)
{
    ...
    if (argc == 2) {
        ...
        if (strcmp(argv[1], "secs") == 0) {
            rc = cmd_sleep_normal(cdev, CMD_SLEEP_TYPE_SECS, val);
        } else if (strcmp(argv[1], "msecs") == 0) {
            rc = cmd_sleep_normal(cdev, CMD_SLEEP_TYPE_MSECS, val);
        } else if (strcmp(argv[1], "usecs") == 0) {
            rc = cmd_sleep_normal(cdev, CMD_SLEEP_TYPE_USECS, val);
        }
    } else if (argc == 4) {
        iter = atoi(argv[2]);
        val  = atoi(argv[3]);
        ...
        if (strcmp(argv[1], "test_secs") == 0) {
            rc = cmd_sleep_test(cdev, CMD_SLEEP_TYPE_SECS, iter, val);
        } else if (strcmp(argv[1], "test_msecs") == 0) {
            rc = cmd_sleep_test(cdev, CMD_SLEEP_TYPE_MSECS, iter, val);
        } else if (strcmp(argv[1], "test_usecs") == 0) {
            rc = cmd_sleep_test(cdev, CMD_SLEEP_TYPE_USECS, iter, val);
        }
    }
...
static vmm_command_t cmd_sleep = {
    .name  = "sleep",
...
{
    return vmm_command_manager_register_cmd(&cmd_sleep);
}

static void __exit cmd_sleep_exit(void)
{
    vmm_command_manager_unregister_cmd(&cmd_sleep);
}
