...
static void cmd_shutdown_usage(vmm_char_device_t *cdev)
{
    vmm_cdev_printf(cdev, "Usage: ");
    vmm_cdev_printf(cdev, "   shutdown\n");
}

static int cmd_shutdown_exec(vmm_char_device_t *cdev, int argc, char **argv)
{
...
static vmm_command_t cmd_shutdown = {
    .name  = "shutdown",
...
{
    return vmm_command_manager_register_cmd(&cmd_shutdown);
}

static void __exit cmd_shutdown_exit(void)
{
    vmm_command_manager_unregister_cmd(&cmd_shutdown);
}
