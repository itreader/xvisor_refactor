...
#ifdef CONFIG_64BIT
int arch_elf_apply_relocate(
    struct elf64_shdr *sechdrs,
#else
int arch_elf_apply_relocate(
    struct elf32_shdr *sechdrs,
#endif
    const char *strtab, uint32_t symindex, uint32_t relindex, vmm_module_t *mod)
{
    /* TODO: */
...
#ifdef CONFIG_64BIT
int arch_elf_apply_relocate_add(
    struct elf64_shdr *sechdrs,
#else
int arch_elf_apply_relocate_add(
    struct elf32_shdr *sechdrs,
#endif
    const char *strtab, uint32_t symindex, uint32_t relsec, vmm_module_t *mod)
{
    /* TODO: */
