...
extern uint64_t _bootcpu_reg0;

volatile uint64_t start_secondary_pen_release         = HARTID_INVALID;
volatile uint64_t start_secondary_smp_id              = 0x0;
physical_addr_t   __smp_logical_map[CONFIG_CPU_COUNT] = {[0 ... CONFIG_CPU_COUNT - 1] = HARTID_INVALID};
...
void smp_write_pen_release(uint64_t val)
{
    arch_smp_mb();
    start_secondary_pen_release = val;
    vmm_flush_dcache_range(
        (virtual_addr_t)&start_secondary_pen_release, (virtual_addr_t)&start_secondary_pen_release + sizeof(start_secondary_pen_release));
}

uint64_t smp_read_pen_release(void)
{
    return start_secondary_pen_release;
}

void smp_write_logical_id(uint64_t val)
{
    arch_smp_mb();
    start_secondary_smp_id = val;
    vmm_flush_dcache_range((virtual_addr_t)&start_secondary_smp_id, (virtual_addr_t)&start_secondary_smp_id + sizeof(start_secondary_smp_id));
}

uint64_t smp_read_logical_id(void)
{
    return start_secondary_smp_id;
}

static int __init smp_read_ops(vmm_device_tree_node_t *dn, int cpu)
{
...
static void __init smp_init_ops(void)
{
    ...
    count = vmm_device_tree_nidtable_count();

    for (i = 0; i < count; i++) {
        nide = vmm_device_tree_nidtable_get(i);
...
int arch_smp_map_hwid(uint32_t cpu, uint64_t *hwid)
{
...
int __init arch_smp_init_cpus(void)
{
    ...
    vmm_device_tree_node_t *dn, *cpus;

    smp_init_ops();

    cpus = vmm_device_tree_getnode(VMM_DEVICE_TREE_PATH_SEPARATOR_STRING "cpus");
    ...
    dn = NULL;
    vmm_device_tree_for_each_child(dn, cpus)
    {
        str = NULL;
        rc  = vmm_device_tree_read_string(dn, VMM_DEVICE_TREE_DEVICE_TYPE_ATTR_NAME, &str);

        if (rc || !str) {
            continue;
        }

        if (strcmp(str, VMM_DEVICE_TREE_DEVICE_TYPE_VAL_CPU)) {
            continue;
        }

        rc = vmm_device_tree_read_physaddr(dn, VMM_DEVICE_TREE_REG_ATTR_NAME, &hwid);

        if ((rc == VMM_OK) && (hwid == _bootcpu_reg0)) {
    ...
    vmm_device_tree_for_each_child(dn, cpus)
    {
        ...
        for (i = 1; (i < cpu) && (i < CONFIG_CPU_COUNT); i++) {
            if (smp_logical_map(i) == hwid) {
                vmm_printf(
                    "%s: duplicate cpu reg properties"
                    " in the DT\n",
                    dn->name);
                goto next;
            }
        ...
        if (hwid == smp_logical_map(0)) {
            if (bootcpu_valid) {
                vmm_printf(
                    "%s: duplicate boot cpu reg property"
                    " in DT\n",
                    dn->name);
                goto next;
            }
    ...
    if (cpu > CONFIG_CPU_COUNT) {
        vmm_printf(
            "%s: no. of cores (%d) greater than configured"
...
int __init arch_smp_prepare_cpus(uint32_t max_cpus)
{
    int      err;
    uint32_t cpu;
...
int __init arch_smp_start_cpu(uint32_t cpu)
{
    int rc;
