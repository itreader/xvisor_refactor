...
#define SBI_EXT_XVISOR         (SBI_EXT_FIRMWARE_START + CPU_VCPU_SBI_IMPID)

#define SBI_EXT_XVISOR_ISA_EXT 0x0

static int vcpu_sbi_xvisor_ecall(vmm_vcpu_t *vcpu, uint64_t ext_id, uint64_t func_id, uint64_t *args, struct cpu_vcpu_sbi_return *out)
{
    switch (func_id) {
        case SBI_EXT_XVISOR_ISA_EXT:
            if (args[0] < RISCV_ISA_EXT_MAX) {
                out->value = __riscv_isa_extension_available(riscv_private(vcpu)->isa, args[0]);
            } else {
