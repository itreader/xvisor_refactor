...
static const struct cpu_vcpu_sbi_extension *vcpu_sbi[] = {
    &vcpu_sbi_time, &vcpu_sbi_rfence, &vcpu_sbi_ipi, &vcpu_sbi_base, &vcpu_sbi_hsm, &vcpu_sbi_srst, &vcpu_sbi_legacy, &vcpu_sbi_xvisor,
};

const struct cpu_vcpu_sbi_extension *cpu_vcpu_sbi_find_extension(uint64_t ext_id)
{
    int i;

    for (i = 0; i < array_size(vcpu_sbi); i++) {
        if (ext_id >= vcpu_sbi[i]->extid_start && ext_id <= vcpu_sbi[i]->extid_end) {
            return vcpu_sbi[i];
        }
    }
...
int cpu_vcpu_sbi_ecall(vmm_vcpu_t *vcpu, ulong cause, arch_regs_t *regs)
{
    int                                  ret = 0;
    const struct cpu_vcpu_sbi_extension *ext;
    uint64_t                             extension_id = regs->a7;
    uint64_t                             func_id      = regs->a6;
    struct cpu_vcpu_trap                 trap         = {0};
    struct cpu_vcpu_sbi_return           out          = {.value = 0, .trap = &trap, .regs_updated = FALSE};
    bool                                 is_0_1_spec  = FALSE;
    uint64_t                             args[6];

    /* Forward SBI calls from virtual-VS mode to virtual-HS mode */
    if (riscv_nested_virt(vcpu)) {
        riscv_stats_private(vcpu)->nested_sbi++;
        trap.sepc   = regs->sepc;
