...
static int vcpu_sbi_base_ecall(vmm_vcpu_t *vcpu, uint64_t ext_id, uint64_t func_id, uint64_t *args, struct cpu_vcpu_sbi_return *out)
{
    int           ret = 0;
    struct sbiret hret;
    ...
    switch (func_id) {
        case SBI_EXT_BASE_GET_SPEC_VERSION:
            out->value = (CPU_VCPU_SBI_VERSION_MAJOR << SBI_SPEC_VERSION_MAJOR_SHIFT) | CPU_VCPU_SBI_VERSION_MINOR;
            break;
        ...
        case SBI_EXT_BASE_GET_IMP_VERSION:
            out->value = VMM_VERSION_MAJOR << 24 | VMM_VERSION_MINOR << 12 | VMM_VERSION_RELEASE;
            break;
