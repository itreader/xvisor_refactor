...
static int vcpu_sbi_hsm_ecall(vmm_vcpu_t *vcpu, uint64_t ext_id, uint64_t func_id, uint64_t *args, struct cpu_vcpu_sbi_return *out)
{
    int               rc;
    ...
    switch (func_id) {
        case SBI_EXT_HSM_HART_START:
            ...
            if (!rvcpu || (rvcpu == vcpu) || (vmm_manager_vcpu_get_state(rvcpu) != VMM_VCPU_STATE_RESET)) {
                return SBI_ERR_INVALID_PARAM;
            }

            rc = vmm_guest_physical_map(guest, args[1], 1, &outaddr, &availsz, &reg_flags);

            if (rc || !(reg_flags & (VMM_REGION_IS_RAM | VMM_REGION_IS_ROM))) {
        ...
        case SBI_EXT_HSM_HART_SUSPEND:
            if (args[0] == SBI_HSM_SUSPEND_RET_DEFAULT) {
                ...
                (args[0] == SBI_HSM_SUSPEND_NON_RET_DEFAULT) || (SBI_HSM_SUSPEND_RET_PLATFORM <= args[0] && args[0] <= SBI_HSM_SUSPEND_RET_LAST) ||
                (SBI_HSM_SUSPEND_NON_RET_PLATFORM <= args[0] && args[0] <= SBI_HSM_SUSPEND_NON_RET_LAST)) {
