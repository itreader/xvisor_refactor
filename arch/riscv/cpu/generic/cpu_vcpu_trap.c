...
{
    /* Read Guest sstatus */
    uint64_t vsstatus = csr_read(CSR_VSSTATUS);
...
void cpu_vcpu_redirect_trap(vmm_vcpu_t *vcpu, arch_regs_t *regs, struct cpu_vcpu_trap *trap)
{
    ...
    cpu_vcpu_nested_set_virt(vcpu, regs, NESTED_SET_VIRT_EVENT_TRAP, FALSE, prev_spp, gva);

    /* Update Guest HTVAL and HTINST */
    nprivate->htval  = trap->htval;
    nprivate->htinst = trap->htinst;

    /* Update Guest supervisor state */
    cpu_vcpu_redirect_smode_trap(regs, trap, prev_spp);
}

static int cpu_vcpu_stage2_map(vmm_vcpu_t *vcpu, physical_addr_t fault_addr)
{
    int             rc, rc1;
    uint32_t        reg_flags = 0x0, pg_reg_flags = 0x0;
    struct mmu_page pg;
    ...
    if (rc) {
        vmm_printf("%s: guest_phys=0x%" PRIPADDR " size=0x%" PRIPSIZE " map failed\n", __func__, inaddr, size);
        return rc;
    }
    ...
    rc = mmu_map_page(riscv_guest_private(vcpu->guest)->page_table, &pg);
    ...
    if (rc) {
         ...
         */
        memset(&pg, 0, sizeof(pg));
        rc1 = mmu_get_page(riscv_guest_private(vcpu->guest)->page_table, fault_addr, &pg);
...
static int cpu_vcpu_emulate_load(vmm_vcpu_t *vcpu, arch_regs_t *regs, physical_addr_t fault_addr, uint64_t htinst)
{
    uint8_t              data8;
    uint16_t             data16;
    uint32_t             data32;
    ...
    switch (len) {
        case 1:
            rc = vmm_device_emulate_emulate_read(vcpu, fault_addr, &data8, sizeof(data8), VMM_DEVICE_EMULATE_LITTLE_ENDIAN);
        ...
        case 2:
            rc = vmm_device_emulate_emulate_read(vcpu, fault_addr, &data16, sizeof(data16), VMM_DEVICE_EMULATE_LITTLE_ENDIAN);
        ...
        case 4:
            rc = vmm_device_emulate_emulate_read(vcpu, fault_addr, &data32, sizeof(data32), VMM_DEVICE_EMULATE_LITTLE_ENDIAN);
        ...
        case 8:
            rc = vmm_device_emulate_emulate_read(vcpu, fault_addr, &data64, sizeof(data64), VMM_DEVICE_EMULATE_LITTLE_ENDIAN);
...
static int cpu_vcpu_emulate_store(vmm_vcpu_t *vcpu, arch_regs_t *regs, physical_addr_t fault_addr, uint64_t htinst)
{
    uint8_t              data8;
    uint16_t             data16;
    uint32_t             data32;
    ...
    switch (len) {
        case 1:
            rc = vmm_device_emulate_emulate_write(vcpu, fault_addr, &data8, sizeof(data8), VMM_DEVICE_EMULATE_LITTLE_ENDIAN);
            break;

        case 2:
            rc = vmm_device_emulate_emulate_write(vcpu, fault_addr, &data16, sizeof(data16), VMM_DEVICE_EMULATE_LITTLE_ENDIAN);
            break;

        case 4:
            rc = vmm_device_emulate_emulate_write(vcpu, fault_addr, &data32, sizeof(data32), VMM_DEVICE_EMULATE_LITTLE_ENDIAN);
            break;
...
int cpu_vcpu_page_fault(vmm_vcpu_t *vcpu, arch_regs_t *regs, struct cpu_vcpu_trap *trap)
{
    ...
    if (riscv_nested_virt(vcpu)) {
        ...
        otrap.htval  = 0;
        otrap.htinst = 0;
        rc           = cpu_vcpu_nested_page_fault(vcpu, (regs->hstatus & HSTATUS_SPVP) ? TRUE : FALSE, trap, &otrap);
    ...
    reg = vmm_guest_find_region(vcpu->guest, fault_addr, VMM_REGION_VIRTUAL | VMM_REGION_MEMORY, FALSE);

    if (reg) {
        /* Emulate load/store instructions for virtual device */
        switch (trap->scause) {
            case CAUSE_LOAD_GUEST_PAGE_FAULT:
                return cpu_vcpu_emulate_load(vcpu, regs, fault_addr, trap->htinst);
...
static int truly_illegal_insn(vmm_vcpu_t *vcpu, arch_regs_t *regs, ulong insn)
{
    struct cpu_vcpu_trap trap;
...
static int truly_virtual_insn(vmm_vcpu_t *vcpu, arch_regs_t *regs, ulong insn)
{
    struct cpu_vcpu_trap trap;
...
struct system_opcode_func {
     ...
     */
    int (*func)(vmm_vcpu_t *vcpu, arch_regs_t *regs, ulong insn);
};
...
struct csr_func {
     ...
     * 5) Return == 3 to do nothing and continue
     */
    int (*rmw_func)(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint32_t csr_num, uint64_t *val, uint64_t new_val, uint64_t wr_mask);
};
...
static int csr_insn(vmm_vcpu_t *vcpu, arch_regs_t *regs, ulong insn)
{
    int                    i, rc = TRAP_RETURN_ILLEGAL_INSN;
    uint32_t               csr_num = insn >> SH_RS2;
    uint32_t               rs1_num = (insn >> SH_RS1) & MASK_RX;
...
{
    bool     next_virt;
    uint64_t vsstatus, next_sepc, next_spp;
     ...
     * and next sepc
     */
    next_virt = (riscv_nested_private(vcpu)->hstatus & HSTATUS_SPV) ? TRUE : FALSE;
    ...
    cpu_vcpu_nested_set_virt(vcpu, regs, NESTED_SET_VIRT_EVENT_SRET, next_virt, FALSE, FALSE);

    return TRAP_RETURN_CONTINUE;
}

static int wfi_insn(vmm_vcpu_t *vcpu, arch_regs_t *regs, ulong insn)
{
    /*
     * Trap from virtual-VS and virtual-VU modes should be forwarded to
     * virtual-HS mode as a virtual instruction trap.
...
static int hfence_vvma_insn(vmm_vcpu_t *vcpu, arch_regs_t *regs, ulong insn)
{
    uint32_t rs1_num = (insn >> SH_RS1) & MASK_RX;
    uint32_t rs2_num = (insn >> SH_RS2) & MASK_RX;
    uint64_t vaddr   = GET_RS1(insn, regs);
    uint32_t asid    = GET_RS2(insn, regs);
...
static int hfence_gvma_insn(vmm_vcpu_t *vcpu, arch_regs_t *regs, ulong insn)
{
    uint32_t        rs1_num = (insn >> SH_RS1) & MASK_RX;
    uint32_t        rs2_num = (insn >> SH_RS2) & MASK_RX;
    physical_addr_t gaddr   = GET_RS1(insn, regs) << 2;
    uint32_t        vmid    = GET_RS2(insn, regs);
...
static int hlv_insn(vmm_vcpu_t *vcpu, arch_regs_t *regs, ulong insn)
{
    int                  rc;
    void                *data;
    bool                 hlvx = FALSE;
     ...
     */
    if (!(regs->hstatus & HSTATUS_SPVP) && !(riscv_nested_private(vcpu)->hstatus & HSTATUS_HU)) {
        return TRAP_RETURN_ILLEGAL_INSN;
    }
    ...
    rc          = cpu_vcpu_nested_hlv(vcpu, vaddr, hlvx, data, len, &trap.scause, &trap.stval, &trap.htval);
...
static int hsv_insn(vmm_vcpu_t *vcpu, arch_regs_t *regs, ulong insn)
{
    int                  rc;
    void                *data;
    union hxv_reg_data   v;
    ...
    rc          = cpu_vcpu_nested_hsv(vcpu, vaddr, data, len, &trap.scause, &trap.stval, &trap.htval);
...
static int system_opcode_insn(vmm_vcpu_t *vcpu, arch_regs_t *regs, ulong insn)
{
    int                              i, rc = TRAP_RETURN_ILLEGAL_INSN;
...
int cpu_vcpu_general_fault(vmm_vcpu_t *vcpu, arch_regs_t *regs, struct cpu_vcpu_trap *trap)
{
    ...
    if (unlikely((trap->stval & INSN_16BIT_MASK) != INSN_16BIT_MASK)) {
        if (trap->stval == 0) {
            trap->stval = __cpu_vcpu_unpriv_read_insn(regs->sepc, &itrap);
...
int cpu_vcpu_illegal_insn_fault(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint64_t stval)
{
    uint64_t             insn = stval;
