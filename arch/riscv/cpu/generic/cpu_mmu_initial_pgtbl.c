 ...
 *
 * @file cpu_mmu_initial_page_table.c
...
struct cpu_mmu_entry_ctrl {
    ...
    arch_pte_t    *next_page_table;
    virtual_addr_t page_table_base;
};
    ...
    struct cpu_mmu_entry_ctrl *entry, virtual_addr_t map_start, virtual_addr_t map_end, virtual_addr_t pa_start, bool writeable)
{
    ...
    while (page_addr < map_end) {
        page_table = (arch_pte_t *)entry->page_table_base;
...
void __attribute__((section(".entry"))) __detect_page_table_mode(
    virtual_addr_t load_start, virtual_addr_t load_end, virtual_addr_t exec_start, virtual_addr_t exec_end)
{
#ifdef CONFIG_64BIT
    uint32_t    i, index;
    uint64_t    satp;
    ...
    virtual_addr_t load_start, virtual_addr_t load_end, virtual_addr_t exec_start, virtual_addr_t dtb_start)
{
    ...
    __detect_page_table_mode(load_start, load_end, exec_start, exec_end);
    ...
    /* Init page_table_base and next_page_table */
    entry.page_table_base = to_load_pa((virtual_addr_t)&stage1_page_table_root);
    entry.next_page_table = (arch_pte_t *)to_load_pa((virtual_addr_t)&stage1_page_table_nonroot);
