...
int riscv_node_to_hartid(vmm_device_tree_node_t *node, uint32_t *hart_id)
{
    ...
    if (hart_id) {
        rc = vmm_device_tree_read_u32(node, "reg", hart_id);
...
int riscv_isa_populate_string(uint64_t xlen, const uint64_t *isa_bitmap, char *out, size_t out_sz)
{
...
int riscv_isa_parse_string(const char *isa, uint64_t *out_xlen, uint64_t *out_bitmap, size_t out_bitmap_sz)
{
    size_t i, j, isa_len;
    ...
    *out_xlen = 0;
    ...
    while (i < isa_len) {
        ...
        while ((i < isa_len) && (isa[i] != '_') && (j < (sizeof(mstr) - 1))) {
            mstr[j++] = isa[i++];
        }
...
const uint64_t *riscv_isa_extension_host(void)
{
    return riscv_isa;
}

bool __riscv_isa_extension_available(const uint64_t *isa_bitmap, int bit)
{
    const uint64_t *bmap = (isa_bitmap) ? isa_bitmap : riscv_isa;
...
uint64_t riscv_xlen = 0;
#ifdef CONFIG_64BIT
uint64_t riscv_stage2_mode = HGATP_MODE_SV39X4;
#else
uint64_t riscv_stage2_mode = HGATP_MODE_SV32X4;
#endif
uint64_t riscv_stage2_vmid_bits   = 0;
uint64_t riscv_stage2_vmid_nested = 0;
bool     riscv_stage2_use_vmid    = false;
uint64_t riscv_timer_hz           = 0;
...
int __init arch_cpu_nascent_init(void)
{
    ...
    if (rc) {
        ...
        vmm_printf(
            "%s: Failed to read timebase-frequency from "
            "cpus node\n",
            __func__);
        return rc;
    }
    ...
    vmm_device_tree_for_each_child(dn, cpus)
    {
        ...
        isa = NULL;
        rc  = vmm_device_tree_read_string(dn, "riscv,isa", &isa);
        ...
        rc = riscv_isa_parse_string(isa, &this_xlen, this_isa, RISCV_ISA_EXT_MAX);
    ...
    if (riscv_isa_extension_available(NULL, h)) {
...
#ifdef CONFIG_64BIT
        /* Try Sv57x4 MMU mode */
        csr_write(CSR_HGATP, HGATP_VMID | (HGATP_MODE_SV57X4 << HGATP_MODE_SHIFT));
        ...
        /* Try Sv48x4 MMU mode */
        csr_write(CSR_HGATP, HGATP_VMID | (HGATP_MODE_SV48X4 << HGATP_MODE_SHIFT));
        val = csr_read(CSR_HGATP) >> HGATP_MODE_SHIFT;
