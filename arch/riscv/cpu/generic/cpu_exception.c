...
static virtual_addr_t preempt_orphan_pc = (virtual_addr_t)&arch_vcpu_preempt_orphan;

void do_error(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint64_t scause, const char *msg, int err, bool panic)
{
    ...
    cpu_vcpu_dump_exception_regs(NULL, scause, csr_read(CSR_STVAL), csr_read(CSR_HTVAL), csr_read(CSR_HTINST));
...
void do_handle_irq(arch_regs_t *regs, uint64_t cause)
{
    int rc = VMM_OK;

    vmm_scheduler_irq_enter(regs, FALSE);

    if (cause == IRQ_VS_SOFT || cause == IRQ_VS_TIMER || cause == IRQ_VS_EXT) {
        rc = cpu_vcpu_redirect_vsirq(vmm_scheduler_current_vcpu(), regs, cause);
        goto done;
    }
    ...
    if (rc) {
        do_error(vmm_scheduler_current_vcpu(), regs, cause | SCAUSE_INTERRUPT_MASK, "interrupt handling failed", rc, TRUE);
    }

    vmm_scheduler_irq_exit(regs);
}

void do_handle_trap(arch_regs_t *regs, uint64_t cause)
{
    int                  rc    = VMM_OK;
    bool                 panic = TRUE;
    ...
    if ((cause == CAUSE_STORE_PAGE_FAULT) && !(regs->hstatus & HSTATUS_SPV) && (regs->sepc == preempt_orphan_pc)) {
        regs->sepc += 4;
    ...
    if (rc) {
        ...
        riscv_stats_private(vcpu)->trap[cause]++;
    }
...
{
    extern uint64_t _handle_exception[];
    extern uint64_t _handle_hyp_exception[];
