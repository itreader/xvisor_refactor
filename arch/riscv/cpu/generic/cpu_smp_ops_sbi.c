...
static int sbi_hart_start(uint64_t hartid, uint64_t saddr, uint64_t private)
{
    struct sbiret ret;

    ret = sbi_ecall(SBI_EXT_HSM, SBI_EXT_HSM_HART_START, hartid, saddr, private, 0, 0, 0);
...
static int __init smp_sbi_cpu_init(vmm_device_tree_node_t *node, uint32_t cpu)
{
    /* For now nothing to do here. */
    return VMM_OK;
}

static int __init smp_sbi_cpu_prepare(uint32_t cpu)
{
    /* For now nothing to do here. */
    return VMM_OK;
}
...
static int __init smp_sbi_cpu_boot(uint32_t cpu)
{
    int             rc;
    ...
    /* Do SBI call */
    rc = sbi_hart_start(smp_logical_map(cpu), _start_secondary_nopen_pa, 0);
