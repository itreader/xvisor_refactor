...
struct cpu_vcpu_sbi_extension {
    uint64_t extid_start;
    uint64_t extid_end;
    int (*handle)(vmm_vcpu_t *vcpu, uint64_t ext_id, uint64_t func_id, uint64_t *args, struct cpu_vcpu_sbi_return *out);
};

const struct cpu_vcpu_sbi_extension *cpu_vcpu_sbi_find_extension(uint64_t ext_id);

int cpu_vcpu_sbi_ecall(vmm_vcpu_t *vcpu, ulong mcause, arch_regs_t *regs);
