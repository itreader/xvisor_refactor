 ...
 */
uint64_t __cpu_vcpu_unpriv_read_insn(uint64_t guest_addr, struct cpu_vcpu_trap *trap);

/* Read uint64_t from Guest memory
 * Note: This function should only be called from normal context
 */
uint64_t __cpu_vcpu_unpriv_read_ulong(uint64_t guest_addr, struct cpu_vcpu_trap *trap);
