...
void __hfence_gvma_vmid_gpa(uint64_t gpa_divby_4, uint64_t vmid);

/** Invalidate Stage2 TLBs for given VMID */
void __hfence_gvma_vmid(uint64_t vmid);

/** Invalidate Stage2 TLBs for given guest physical address */
void __hfence_gvma_gpa(uint64_t gpa_divby_4);

/** Invalidate all possible Stage2 TLBs */
void __hfence_gvma_all(void);

/** Invalidate unified TLB entries for given asid and guest virtual address */
void __hfence_vvma_asid_va(uint64_t va, uint64_t asid);

/** Invalidate unified TLB entries for given ASID for a guest*/
void __hfence_vvma_asid(uint64_t asid);

/** Invalidate unified TLB entries for a given guest virtual address */
void __hfence_vvma_va(uint64_t va);

/** Invalidate all possible Stage2 TLBs */
void __hfence_vvma_all(void);

inline void __sfence_vma_asid_va(uint64_t asid, uint64_t va)
{
    __asm__ __volatile__("sfence.vma %0, %1" : : "r"(va), "r"(asid) : "memory");
}

inline void __sfence_vma_asid(uint64_t asid)
{
    __asm__ __volatile__("sfence.vma x0, %1" : : "r"(asid) : "memory");
}
