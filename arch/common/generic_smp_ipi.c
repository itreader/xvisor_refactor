...
void arch_smp_ipi_trigger(const vmm_cpumask_t *dest)
{
...
int __cpuinit arch_smp_ipi_init(void)
{
    ...
    rc = vmm_host_irq_find(0, VMM_IRQ_STATE_IPI | VMM_IRQ_STATE_PER_CPU, &smp_ipi_irq);
    ...
    rc = vmm_host_irq_register(smp_ipi_irq, "IPI", &smp_ipi_handler, NULL);
