...
static struct clcd_panel *panels[] = {
    &vga, &xvga, &sanyo_tm38qv67a02a, &sanyo_2_5_in, &epson_l2f50113t00,
};
...
int versatile_clcd_setup_dma(struct clcd_fb *fb, uint64_t framesize)
{
    int             rc;
    uint32_t        use_dma, val[2];
    void           *screen_base;
    uint64_t        smem_len;
    ...
    if (use_dma) {
        ...
        screen_base = (void *)vmm_host_alloc_pages(VMM_SIZE_TO_PAGE(smem_len), VMM_MEMORY_READABLE | VMM_MEMORY_WRITEABLE);
        ...
        rc = vmm_device_tree_read_u32_array(fb->dev->of_node, "framebuffer", val, 2);
...
{
    vmm_host_free_pages((virtual_addr_t)fb->fb.screen_base, VMM_SIZE_TO_PAGE(fb->fb.fix.smem_len));
}
