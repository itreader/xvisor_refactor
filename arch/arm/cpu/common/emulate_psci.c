...
static uint64_t psci_affinity_mask(uint64_t affinity_level)
{
    if (affinity_level <= 3) {
...
static uint64_t psci_vcpu_suspend(vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
    /*
...
static uint64_t psci_vcpu_off(vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
...
static uint64_t psci_vcpu_on(vmm_vcpu_t *source_vcpu, arch_regs_t *regs)
{
    uint64_t    cpu_id;
    uint64_t    context_id;
    uint64_t    mpidr;
    uint64_t    target_pc;
    vmm_vcpu_t *vcpu = NULL, *tmp = NULL;
...
static uint64_t psci_vcpu_affinity_info(vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
    uint64_t    mpidr;
    uint64_t    target_affinity;
    uint64_t    target_affinity_mask;
    ...
    vmm_manager_for_each_guest_vcpu(tmp, vcpu->guest)
    {
        ...
        if (((mpidr & target_affinity_mask) == target_affinity) && (vmm_manager_vcpu_get_state(tmp) & VMM_VCPU_STATE_INTERRUPTIBLE)) {
...
static void psci_system_off(vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
    int rc;
...
static void psci_system_reset(vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
    int rc;
...
static int emulate_psci_0_2_call(vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
    uint64_t psci_fn = emulate_psci_get_reg(vcpu, regs, 0) & ~((uint32_t)0);
    uint64_t val;
...
static int emulate_psci_0_1_call(vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
    uint64_t psci_fn = emulate_psci_get_reg(vcpu, regs, 0) & ~((uint32_t)0);
    uint64_t val;
