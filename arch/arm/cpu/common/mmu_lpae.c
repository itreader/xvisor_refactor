...
{
    if ((size == TTBL_L0_BLOCK_SIZE) || (size == TTBL_L1_BLOCK_SIZE) || (size == TTBL_L2_BLOCK_SIZE) || (size == TTBL_L3_BLOCK_SIZE)) {
        return TRUE;
    }
...
void arch_mmu_pgflags_set(arch_page_flags_t *flags, int stage, uint32_t mflags)
{
    if (stage == MMU_STAGE2) {
        ...
        if ((mflags & VMM_MEMORY_CACHEABLE) && (mflags & VMM_MEMORY_BUFFERABLE)) {
...
bool arch_mmu_pte_is_valid(arch_pte_t *pte, int stage, int level)
{
    if (!level) {
        ...
        return ((*pte & TTBL_TABLE_MASK) && (*pte & TTBL_VALID_MASK)) ? TRUE : FALSE;
    }
...
{
    if (stage == MMU_STAGE2) {
        out_flags->xn      = (*pte & TTBL_STAGE2_UPPER_XN_MASK) >> TTBL_STAGE2_UPPER_XN_SHIFT;
        out_flags->cont    = (*pte & TTBL_STAGE2_UPPER_CONT_MASK) >> TTBL_STAGE2_UPPER_CONT_SHIFT;
        out_flags->af      = (*pte & TTBL_STAGE2_LOWER_AF_MASK) >> TTBL_STAGE2_LOWER_AF_SHIFT;
        out_flags->sh      = (*pte & TTBL_STAGE2_LOWER_SH_MASK) >> TTBL_STAGE2_LOWER_SH_SHIFT;
        out_flags->ap      = (*pte & TTBL_STAGE2_LOWER_HAP_MASK) >> TTBL_STAGE2_LOWER_HAP_SHIFT;
        out_flags->memattr = (*pte & TTBL_STAGE2_LOWER_MEMATTR_MASK) >> TTBL_STAGE2_LOWER_MEMATTR_SHIFT;
    } else {
        out_flags->xn     = (*pte & TTBL_STAGE1_UPPER_XN_MASK) >> TTBL_STAGE1_UPPER_XN_SHIFT;
        out_flags->pxn    = (*pte & TTBL_STAGE1_UPPER_PXN_MASK) >> TTBL_STAGE1_UPPER_PXN_SHIFT;
        out_flags->cont   = (*pte & TTBL_STAGE1_UPPER_CONT_MASK) >> TTBL_STAGE1_UPPER_CONT_SHIFT;
        out_flags->ng     = (*pte & TTBL_STAGE1_LOWER_NG_MASK) >> TTBL_STAGE1_LOWER_NG_SHIFT;
        out_flags->af     = (*pte & TTBL_STAGE1_LOWER_AF_MASK) >> TTBL_STAGE1_LOWER_AF_SHIFT;
        out_flags->sh     = (*pte & TTBL_STAGE1_LOWER_SH_MASK) >> TTBL_STAGE1_LOWER_SH_SHIFT;
        out_flags->ap     = (*pte & TTBL_STAGE1_LOWER_AP_MASK) >> TTBL_STAGE1_LOWER_AP_SHIFT;
        out_flags->ns     = (*pte & TTBL_STAGE1_LOWER_NS_MASK) >> TTBL_STAGE1_LOWER_NS_SHIFT;
        out_flags->aindex = (*pte & TTBL_STAGE1_LOWER_AINDEX_MASK) >> TTBL_STAGE1_LOWER_AINDEX_SHIFT;
    }
}
...
void arch_mmu_pte_set(arch_pte_t *pte, int stage, int level, physical_addr_t pa, arch_page_flags_t *flags)
{
    ...
    *pte = *pte & TTBL_OUTADDR_MASK;
    ...
    if (stage == MMU_STAGE2) {
        *pte |= ((uint64_t)flags->xn << TTBL_STAGE2_UPPER_XN_SHIFT) & TTBL_STAGE2_UPPER_XN_MASK;
        *pte |= ((uint64_t)flags->cont << TTBL_STAGE2_UPPER_CONT_SHIFT) & TTBL_STAGE2_UPPER_CONT_MASK;
        *pte |= ((uint64_t)flags->af << TTBL_STAGE2_LOWER_AF_SHIFT) & TTBL_STAGE1_LOWER_AF_MASK;
        *pte |= ((uint64_t)flags->sh << TTBL_STAGE2_LOWER_SH_SHIFT) & TTBL_STAGE2_LOWER_SH_MASK;
        *pte |= ((uint64_t)flags->ap << TTBL_STAGE2_LOWER_HAP_SHIFT) & TTBL_STAGE2_LOWER_HAP_MASK;
        *pte |= ((uint64_t)flags->memattr << TTBL_STAGE2_LOWER_MEMATTR_SHIFT) & TTBL_STAGE2_LOWER_MEMATTR_MASK;
    } else {
        *pte |= ((uint64_t)flags->xn << TTBL_STAGE1_UPPER_XN_SHIFT) & TTBL_STAGE1_UPPER_XN_MASK;
        *pte |= ((uint64_t)flags->pxn << TTBL_STAGE1_UPPER_PXN_SHIFT) & TTBL_STAGE1_UPPER_PXN_MASK;
        *pte |= ((uint64_t)flags->cont << TTBL_STAGE1_UPPER_CONT_SHIFT) & TTBL_STAGE1_UPPER_CONT_MASK;
        *pte |= ((uint64_t)flags->ng << TTBL_STAGE1_LOWER_NG_SHIFT) & TTBL_STAGE1_LOWER_NG_MASK;
        *pte |= ((uint64_t)flags->af << TTBL_STAGE1_LOWER_AF_SHIFT) & TTBL_STAGE1_LOWER_AF_MASK;
        *pte |= ((uint64_t)flags->sh << TTBL_STAGE1_LOWER_SH_SHIFT) & TTBL_STAGE1_LOWER_SH_MASK;
        *pte |= ((uint64_t)flags->ap << TTBL_STAGE1_LOWER_AP_SHIFT) & TTBL_STAGE1_LOWER_AP_MASK;
        *pte |= ((uint64_t)flags->ns << TTBL_STAGE1_LOWER_NS_SHIFT) & TTBL_STAGE1_LOWER_NS_MASK;
        *pte |= ((uint64_t)flags->aindex << TTBL_STAGE1_LOWER_AINDEX_SHIFT) & TTBL_STAGE1_LOWER_AINDEX_MASK;
    }
...
{
    *pte = table_pa & TTBL_OUTADDR_MASK;
    ...
    uint32_t *out_fault_flags)
{
    ...
    arch_cpu_irq_save(f);
    cpu_mmu_at_test_exec(s2_table_pa, s1_avail, s1_table_pa, addr, (flags & MMU_TEST_WRITE) ? TRUE : FALSE, &t);
    ...
    *out_fault_flags = 0;
    ...
    if (t.fault) {
        ...
        *out_fault_flags |= (flags & MMU_TEST_WRITE) ? MMU_TEST_FAULT_WRITE : MMU_TEST_FAULT_READ;
        ...
        *out_addr = addr;
        ...
        } else if (*out_fault_flags & (MMU_TEST_FAULT_READ | MMU_TEST_FAULT_WRITE)) {
...
{
    cpu_stage2_update(table_phys, vmid);
