...
static bool scu_cpu_core_is_smp(void *scu_base, uint32_t cpu)
{
...
static struct vmm_device_tree_nodeid scu_matches[] = {
...
static int __init scu_cpu_init(vmm_device_tree_node_t *node, uint32_t cpu)
{
    int             rc;
    uint32_t        ncores;
    physical_addr_t pa;
...
static int __init scu_cpu_prepare(uint32_t cpu)
{
    int             rc;
    uint32_t        val = 0;
    ...
    if (clear_addr[cpu]) {
        arch_wmb();
        val = ~0x0;
        vmm_host_memory_write(clear_addr[cpu], &val, sizeof(uint32_t), FALSE);
    }
    ...
    if (release_addr[cpu]) {
        ...
        vmm_host_memory_write(release_addr[cpu], &val, sizeof(uint32_t), FALSE);
    }

    return VMM_OK;
}

static int __init scu_cpu_boot(uint32_t cpu)
{
