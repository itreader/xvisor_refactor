...
volatile uint64_t start_secondary_pen_release         = MPIDR_INVALID;
volatile uint64_t start_secondary_smp_id              = 0x0;
physical_addr_t   __smp_logical_map[CONFIG_CPU_COUNT] = {[0 ... CONFIG_CPU_COUNT - 1] = MPIDR_INVALID};
...
void smp_write_pen_release(uint64_t val)
{
    ...
    uint64_t       size         = sizeof(start_secondary_pen_release);
...
void smp_write_logical_id(uint64_t val)
{
    ...
    uint64_t       size    = sizeof(start_secondary_smp_id);
...
static int __init smp_read_ops(vmm_device_tree_node_t *dn, int cpu)
{
    ...
    rc = vmm_device_tree_read_string(dn, VMM_DEVICE_TREE_ENABLE_METHOD_ATTR_NAME, &enable_method);
...
int __init arch_smp_init_cpus(void)
{
    ...
    vmm_device_tree_for_each_child(dn, cpus)
    {
        ...
        if ((rc == VMM_OK) && ((cpus_count < 2) || (hwid == (read_mpidr() & MPIDR_HWID_BITMASK)))) {
    ...
    if (!bootcpu_valid) {
        vmm_printf(
            "%s: DT missing boot CPU MPIDR, not enabling"
            " secondaries\n",
            __func__);
        return VMM_ENODEV;
    }
