...
static void vgic_v2_reset_state(struct vgic_hw_state *hw, enum vgic_model_type model)
{
    uint32_t i;
...
static void vgic_v2_save_state(struct vgic_hw_state *hw, enum vgic_model_type model)
{
    uint32_t i;
    ...
    for (i = 0; i < vgicp.lr_cnt; i++) {
        hw->v2.lr[i] = vmm_readl_relaxed((void *)vgicp.hctrl_va + GICH_LR0 + 4 * i);
    }
}

static void vgic_v2_restore_state(struct vgic_hw_state *hw, enum vgic_model_type model)
{
    uint32_t i;
    ...
    for (i = 0; i < vgicp.lr_cnt; i++) {
        vmm_writel_relaxed(hw->v2.lr[i], (void *)vgicp.hctrl_va + GICH_LR0 + 4 * i);
    }
}
...
static void vgic_v2_read_elrsr(uint32_t *elrsr0, uint32_t *elrsr1)
{
    *elrsr0 = vmm_readl_relaxed((void *)vgicp.hctrl_va + GICH_ELRSR0);

    if (32 < vgicp.lr_cnt) {
        *elrsr1 = vmm_readl_relaxed((void *)vgicp.hctrl_va + GICH_ELRSR1);
    } else {
        *elrsr1 = 0x0;
    }
}

static void vgic_v2_read_eisr(uint32_t *eisr0, uint32_t *eisr1)
{
    *eisr0 = vmm_readl_relaxed((void *)vgicp.hctrl_va + GICH_EISR0);

    if (32 < vgicp.lr_cnt) {
        *eisr1 = vmm_readl_relaxed((void *)vgicp.hctrl_va + GICH_EISR1);
    } else {
        *eisr1 = 0x0;
    }
}

static void vgic_v2_set_lr(uint32_t lr, struct vgic_lr *lrv, enum vgic_model_type model)
{
    uint32_t lrval = lrv->virtid & GICH_LR_VIRTUALID;
    ...
    if (lrv->flags & VGIC_LR_HW) {
        lrval |= GICH_LR_HW;
        lrval |= (lrv->physid << GICH_LR_PHYSID_SHIFT) & GICH_LR_PHYSID;
    } else {
...
static void vgic_v2_get_lr(uint32_t lr, struct vgic_lr *lrv, enum vgic_model_type model)
{
    uint32_t lrval;
    ...
    if (lrval & GICH_LR_HW) {
        ...
        lrv->physid = (lrval & GICH_LR_PHYSID) >> GICH_LR_PHYSID_SHIFT;
    } else {
...
static void vgic_v2_clear_lr(uint32_t lr)
{
    DPRINTF("%s: LR%d\n", __func__, lr);
...
static const struct vmm_device_tree_nodeid vgic_host_match[] = {
    {
...
int vgic_v2_probe(struct vgic_ops *ops, struct vgic_params *params)
{
    ...
    rc = vmm_device_tree_regaddr(node, &vgicp.hctrl_pa, 2);

    if (rc) {
        goto fail_dref;
    }

    rc = vmm_device_tree_request_regmap(node, &vgicp.hctrl_va, 2, "GIC HCTRL");

    if (rc) {
        goto fail_dref;
    }

    rc = vmm_device_tree_regaddr(node, &vgicp.vcpu_pa, 3);

    if (rc) {
        goto fail_unmap_hctrl;
    }

    rc = vmm_device_tree_regsize(node, &vgicp.vcpu_sz, 3);

    if (rc) {
        goto fail_unmap_hctrl;
    }

    rc = vmm_device_tree_request_regmap(node, &vgicp.vcpu_va, 3, "GIC VCPU");
    ...
    vmm_printf("vgic_v2: hctrl=0x%lx vcpu=0x%lx\n", (uint64_t)vgicp.hctrl_pa, (uint64_t)vgicp.vcpu_pa);
    vmm_printf("vgic_v2: lr_cnt=%d maint_irq=%d\n", vgicp.lr_cnt, vgicp.maint_irq);

    return VMM_OK;

fail_unmap_vcpu:
    vmm_device_tree_regunmap_release(node, vgicp.vcpu_va, 3);
fail_unmap_hctrl:
    vmm_device_tree_regunmap_release(node, vgicp.hctrl_va, 2);
fail_dref:
    vmm_device_tree_dref_node(node);
fail:
    return rc;
...
void vgic_v2_remove(struct vgic_ops *ops, struct vgic_params *params)
{
    ...
    vmm_device_tree_regunmap(node, vgicp.vcpu_va, 3);

    vmm_device_tree_regunmap(node, vgicp.hctrl_va, 2);
