...
#define vtr_to_nr_pri_bits(v) (((uint32_t)(v) >> 29) + 1)
...
static uint64_t vgic_v3_read_lr(uint32_t lr)
{
    switch (lr) {
        case 0:
...
static void vgic_v3_write_lr(uint32_t lr, uint64_t val)
{
    switch (lr) {
        case 0:
...
static void vgic_v3_reset_state(struct vgic_hw_state *hw, enum vgic_model_type model)
{
    uint32_t i;
...
static void vgic_v3_save_state(struct vgic_hw_state *hw, enum vgic_model_type model)
{
    uint32_t i;
...
static void vgic_v3_restore_state(struct vgic_hw_state *hw, enum vgic_model_type model)
{
    uint32_t i;
...
{
    uint32_t misr = arch_gic_read_sysreg(ICH_MISR_EL2);
    return (misr & ICH_MISR_U) ? TRUE : FALSE;
}

static void vgic_v3_enable_underflow(void)
{
    uint32_t hcr = arch_gic_read_sysreg(ICH_HCR_EL2);
...
static void vgic_v3_read_elrsr(uint32_t *elrsr0, uint32_t *elrsr1)
{
    *elrsr0 = arch_gic_read_sysreg(ICH_ELSR_EL2);
    *elrsr1 = 0x0;
}

static void vgic_v3_read_eisr(uint32_t *eisr0, uint32_t *eisr1)
{
    *eisr0 = arch_gic_read_sysreg(ICH_EISR_EL2);
    *eisr1 = 0x0;
}

static void vgic_v3_set_lr(uint32_t lr, struct vgic_lr *lrv, enum vgic_model_type model)
{
    uint64_t lrval = 0;
    ...
    lrval |= ((uint64_t)lrv->prio << ICH_LR_PRIORITY_SHIFT);
    ...
    if (lrv->flags & VGIC_LR_HW) {
        lrval |= ICH_LR_HW;
        lrval |= ((uint64_t)lrv->physid << ICH_LR_PHYS_ID_SHIFT) & ICH_LR_PHYS_ID_MASK;
    } else {
...
static void vgic_v3_get_lr(uint32_t lr, struct vgic_lr *lrv, enum vgic_model_type model)
{
    uint64_t lrval = vgic_v3_read_lr(lr);
    ...
    if (lrval & ICH_LR_HW) {
        ...
        lrv->physid = (lrval & ICH_LR_PHYS_ID_MASK) >> ICH_LR_PHYS_ID_SHIFT;
    } else {
        if (lrval & ICH_LR_EOI) {
...
static void vgic_v3_clear_lr(uint32_t lr)
{
    DPRINTF("%s: LR%d\n", __func__, lr);
...
int vgic_v3_probe(struct vgic_ops *ops, struct vgic_params *params)
{
    ...
    rc                = vmm_device_tree_regaddr(node, &vgicp.vcpu_pa, 4);

    if (!rc) {
        rc = vmm_device_tree_regsize(node, &vgicp.vcpu_sz, 4);

        if (rc) {
            goto fail_dref;
        }

        rc = vmm_device_tree_request_regmap(node, &vgicp.vcpu_va, 4, "GIC VCPU");
    ...
    vmm_printf("vgic_v3: vcpu=0x%lx GICv2 emulation %s\n", (uint64_t)vgicp.vcpu_pa, (vgicp.vcpu_mapped) ? "available" : "not available");
