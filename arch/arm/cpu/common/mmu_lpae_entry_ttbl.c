...
struct mmu_lpae_entry_ctrl {
    uint32_t       ttable_count;
    uint64_t      *next_ttbl;
    virtual_addr_t ttable_base;
};
    ...
    bool writeable)
{
    ...
    while (page_addr < map_end) {
        /* Setup level1 table */
        ttbl  = (uint64_t *)lpae_entry->ttable_base;
        ...
        if (!(ttbl[index] & TTBL_VALID_MASK)) {
            /* Update level3 table */
            ttbl[index] = (((page_addr - map_start) + pa_start) & TTBL_OUTADDR_MASK);
            ttbl[index] |= TTBL_STAGE1_LOWER_AF_MASK;
            ttbl[index] |= (writeable) ? (TTBL_AP_SRW_U << TTBL_STAGE1_LOWER_AP_SHIFT) : (TTBL_AP_SR_U << TTBL_STAGE1_LOWER_AP_SHIFT);
            ttbl[index] |= (aindex << TTBL_STAGE1_LOWER_AINDEX_SHIFT) & TTBL_STAGE1_LOWER_AINDEX_MASK;
            ttbl[index] |= TTBL_STAGE1_LOWER_NS_MASK;
            ttbl[index] |= (TTBL_SH_INNER_SHAREABLE << TTBL_STAGE1_LOWER_SH_SHIFT);
            ttbl[index] |= (TTBL_TABLE_MASK | TTBL_VALID_MASK);
        }
...
{
    uint32_t *src = (uint32_t *)dtb_start;
...
void __attribute__((section(".entry"))) _setup_initial_ttbl(
    virtual_addr_t load_start, virtual_addr_t load_end, virtual_addr_t exec_start, virtual_addr_t dtb_start)
{
    uint32_t       i;
    uint64_t      *root_ttbl;
    virtual_addr_t exec_end = exec_start + (load_end - load_start);
#ifdef CONFIG_ARCH_GENERIC_DEFTERM_EARLY
    virtual_addr_t default_terminal_early_va;
#endif
    virtual_addr_t            *dt_virt      = (virtual_addr_t *)to_load_pa((virtual_addr_t)&device_tree_virt);
    virtual_addr_t            *dt_virt_base = (virtual_addr_t *)to_load_pa((virtual_addr_t)&device_tree_virt_base);
    virtual_size_t            *dt_virt_size = (virtual_size_t *)to_load_pa((virtual_addr_t)&device_tree_virt_size);
    physical_addr_t           *dt_phys_base = (physical_addr_t *)to_load_pa((virtual_addr_t)&device_tree_phys_base);
    struct mmu_lpae_entry_ctrl lpae_entry   = {0, NULL, 0};

    /* Init ttable_base and next_ttbl */
    lpae_entry.ttable_base                  = to_load_pa((virtual_addr_t)&stage1_page_table_root);
...
#ifdef CONFIG_ARCH_GENERIC_DEFTERM_EARLY
    /* Map UART for early default_terminal
     * Note: This is for early debug purpose
     */
    default_terminal_early_va = to_exec_va((virtual_addr_t)&default_terminal_early_base);
    __setup_initial_ttbl(
        &lpae_entry, default_terminal_early_va, default_terminal_early_va + TTBL_L3_BLOCK_SIZE,
        (virtual_addr_t)CONFIG_ARCH_GENERIC_DEFTERM_EARLY_BASE_PA, AINDEX_DEVICE_nGnRE, TRUE);
#endif
     ...
     * Note: This mapping is used at runtime
     */
    __setup_initial_ttbl(&lpae_entry, exec_start, exec_end, load_start, AINDEX_NORMAL_WB, TRUE);
    ...
    *dt_virt      = *dt_virt_base + (dtb_start & (TTBL_L3_BLOCK_SIZE - 1));
    ...
    /* Map device tree */
    __setup_initial_ttbl(&lpae_entry, *dt_virt_base, *dt_virt_base + *dt_virt_size, *dt_phys_base, AINDEX_NORMAL_WB, TRUE);
}
