...
static int __noinline invoke_psci_fn_smc_raw(uint64_t func, uint64_t arg0, uint64_t arg1, uint64_t arg2)
{
    long ret;
...
static int __noinline invoke_psci_fn_smc(uint64_t func, uint64_t arg0, uint64_t arg1, uint64_t arg2)
{
    long ret;
...
{
    return invoke_psci_fn_smc(psci_function_id[PSCI_FN_CPU_SUSPEND], power_state, entry_point, 0);
}

int psci_cpu_off(uint64_t power_state)
{
    return invoke_psci_fn_smc(psci_function_id[PSCI_FN_CPU_OFF], power_state, 0, 0);
}

int psci_cpu_on(uint64_t cpuid, uint64_t entry_point)
{
    return invoke_psci_fn_smc(psci_function_id[PSCI_FN_CPU_ON], cpuid, entry_point, 0);
}

int psci_migrate(uint64_t cpuid)
{
    return invoke_psci_fn_smc(psci_function_id[PSCI_FN_MIGRATE], cpuid, 0, 0);
}
...
static int psci_0_2_init(vmm_device_tree_node_t *psci)
{
...
static int psci_0_1_init(vmm_device_tree_node_t *psci)
{
    int rc;
    ...
    rc = vmm_device_tree_read_u32(psci, "cpu_on", &psci_function_id[PSCI_FN_CPU_ON]);
    ...
    rc = vmm_device_tree_read_u32(psci, "cpu_suspend", &psci_function_id[PSCI_FN_CPU_SUSPEND]);
    ...
    rc = vmm_device_tree_read_u32(psci, "cpu_off", &psci_function_id[PSCI_FN_CPU_OFF]);
    ...
    rc = vmm_device_tree_read_u32(psci, "migrate", &psci_function_id[PSCI_FN_MIGRATE]);
...
static struct vmm_device_tree_nodeid psci_matches[] = {
...
void __init psci_init(void)
{
    int                                  rc;
    const char                          *method;
    vmm_device_tree_node_t              *psci;
    const struct vmm_device_tree_nodeid *id;
    int (*psci_init)(vmm_device_tree_node_t *);

    /* Look for node with PSCI compatible string */
    psci = vmm_device_tree_find_matching(NULL, psci_matches);
    ...
    id        = vmm_device_tree_match_node(psci_matches, psci);
    psci_init = id->data;

    /* It should have a "method" attibute equal to "smc" */
    rc        = vmm_device_tree_read_string(psci, "method", &method);
    ...
    vmm_device_tree_dref_node(psci);
}
