 ...
 */
static uint32_t scu_get_core_count(void *scu_base)
{
    return (vmm_readl(scu_base + SCU_CONFIG) & 0x03) + 1;
}

static bool smp_imx_core_is_smp(void *scu_base, uint32_t cpu)
{
...
static const struct vmm_device_tree_nodeid scu_matches[] = {
...
static const struct vmm_device_tree_nodeid src_matches[] = {{.compatible = "fsl,imx51-src"}, {.compatible = "fsl,imx6-src"}, {/* sentinel */}};

static void __init smp_imx_ops_init(void)
{
    int                     rc;
    vmm_device_tree_node_t *scu_node;
    vmm_device_tree_node_t *src_node;

    scu_node = vmm_device_tree_find_matching(NULL, scu_matches);

    if (!scu_node) {
        return;
    }

    rc = vmm_device_tree_regmap(scu_node, &scu_base, 0);
    vmm_device_tree_dref_node(scu_node);
    ...
    src_node = vmm_device_tree_find_matching(NULL, src_matches);

    if (!src_node) {
        return;
    }

    rc = vmm_device_tree_regmap(src_node, &src_base, 0);
    vmm_device_tree_dref_node(src_node);
...
static int __init smp_imx_cpu_init(vmm_device_tree_node_t *node, uint32_t cpu)
{
    uint32_t ncores;
...
static void smp_imx_set_cpu_arg(int cpu, uint32_t arg)
{
    vmm_writel(arg, (void *)src_base + SRC_GPR1 + cpu * 8 + 4);
}

extern uint8_t _start_secondary_nopen;

static int __init smp_imx_cpu_prepare(uint32_t cpu)
{
...
static int __init smp_imx_cpu_boot(uint32_t cpu)
{
