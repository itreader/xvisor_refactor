 ...
 * Note: Must be called with VGIC distributor lock held
 */
static bool __vgic_test_enabled(struct vgic_guest_state *s, uint32_t irq, uint32_t cm)
{
    uint32_t i;
 ...
 * Note: Must be called with VGIC distributor lock held
 */
static bool __vgic_test_pending(struct vgic_guest_state *s, uint32_t irq, uint32_t cm)
{
    uint32_t i;
...
#define VGIC_TEST_LEVEL(s, irq, cm)     (((s)->irq_state[irq].level & (cm)) ? TRUE : FALSE)
...
#define VGIC_TEST_TRIGGER(s, irq)       (((irq < 32) || ((s)->irq_state[irq].host_irq != UINT_MAX)) ? 1 : (s)->irq_state[irq].trigger)
#define VGIC_GET_PRIORITY(s, irq, cpu)  (((irq) < 32) ? (s)->priority1[irq][cpu] : (s)->priority2[(irq) - 32])
...
#define VGIC_TEST_EISR(eisr, lr)        ((eisr)[((lr) >> 5) & 0x1] & (1 << ((lr) & 0x1f)))
#define VGIC_SET_EISR(vs, lr)           ((eisr)[((lr) >> 5) & 0x1] |= (1 << ((lr) & 0x1f)))
#define VGIC_CLEAR_EISR(vs, lr)         ((eisr)[((lr) >> 5) & 0x1] &= ~(1 << ((lr) & 0x1f)))

#define VGIC_TEST_ELRSR(elrsr, lr)      ((elrsr)[((lr) >> 5) & 0x1] & (1 << ((lr) & 0x1f)))
#define VGIC_SET_ELRSR(vs, lr)          ((elrsr)[((lr) >> 5) & 0x1] |= (1 << ((lr) & 0x1f)))
#define VGIC_CLEAR_ELRSR(vs, lr)        ((elrsr)[((lr) >> 5) & 0x1] &= ~(1 << ((lr) & 0x1f)))
 ...
 * Note: Must be called with VGIC distributor lock held
 */
static bool __vgic_queue_irq(struct vgic_guest_state *s, struct vgic_vcpu_state *vs, uint32_t irq)
{
    ...
    if ((lr < vgich.params.lr_cnt) && VGIC_TEST_LR_USED(vs, lr)) {
 ...
 * Note: Must be called with VGIC distributor lock held
 */
static bool __vgic_queue_sgi(struct vgic_guest_state *s, struct vgic_vcpu_state *vs, uint32_t irq)
{
 ...
 * Note: Must be called with VGIC distributor lock held
 */
static bool __vgic_vcpu_irq_pending(struct vgic_guest_state *s, struct vgic_vcpu_state *vs)
{
    uint32_t i, mask;
 ...
 * Note: Must be called with VGIC distributor lock held
 */
static void __vgic_flush_vcpu_hwstate(struct vgic_guest_state *s, struct vgic_vcpu_state *vs)
{
    bool     overflow = FALSE;
    uint32_t i, irq, mask;
 ...
 * Note: Must be called with VGIC distributor lock held
 */
static void __vgic_sync_vcpu_hwstate(struct vgic_guest_state *s, struct vgic_vcpu_state *vs)
{
    ...
    for (lr = 0; lr < vgich.params.lr_cnt; lr++) {
        ...
        if (!VGIC_TEST_TRIGGER(s, irq)) {
            ...
            /* Update pending bit */
            if (VGIC_TEST_LEVEL(s, irq, cm) && (VGIC_TARGET(s, irq) & cm) != 0) {
 ...
 * Note: Must be called with VGIC distributor lock held
 */
static void __vgic_sync_and_flush_vcpu(struct vgic_guest_state *s, struct vgic_vcpu_state *vs)
{
...
static void vgic_vcpu_irq_update(vmm_vcpu_t *vcpu, bool irq_pending)
{
    if (irq_pending) {
...
static void __vgic_irq_handle(struct vgic_guest_state *s, uint32_t irq, int cpu, bool two_levels, int level0, int level1)
{
    irq_flags_t             flags;
    bool                    irq_pending;
    ...
    if (!s->enabled) {
        vmm_spin_unlock_irq_restore_lite(&s->dist_lock, flags);
        return;
    }
...
static void vgic_irq_handle2(uint32_t irq, int cpu, int level0, int level1, void *opaque)
{
...
static int __vgic_dist_readb(struct vgic_guest_state *s, int cpu, uint32_t offset, uint8_t *dst)
{
            ...
            if (!(offset & 0x80)) { /* Set-enableX */
                ...
                for (i = 0; i < 8; i++) {
                    ...
                    if (cm == VGIC_ALL_CPU_MASK(s)) {
                        vmm_device_emulate_notify_irq_enabled(s->guest, irq + i, -1);
                    } else {
                        vmm_device_emulate_notify_irq_enabled(s->guest, irq + i, cpu);
                    }

                    /* If a raised level triggered IRQ enabled
                     * then mark is as pending.
                     */
                    if (VGIC_TEST_LEVEL(s, (irq + i), mask) && !VGIC_TEST_TRIGGER(s, (irq + i))) {
                ...
                for (i = 0; i < 8; i++) {
                    ...
                    if (cm == VGIC_ALL_CPU_MASK(s)) {
                        vmm_device_emulate_notify_irq_disabled(s->guest, irq + i, -1);
                    } else {
                        vmm_device_emulate_notify_irq_disabled(s->guest, irq + i, cpu);
                    }
                }
            }
...
static int vgic_dist_write(struct vgic_guest_state *s, int cpu, uint32_t offset, uint32_t src_mask, uint32_t src)
{
    int                     rc = VMM_OK;
    ...
    if (offset == 0xF00) {
        ...
        for (i = 0; i < 4; i++) {
            if (src_mask & 0xFF) {
                if ((rc = __vgic_dist_writeb(s, cpu, offset + i, src & 0xFF))) {
                    break;
                }
            }
    ...
    /* Update VCPU irqs */
    for (i = 0; i < VGIC_NUM_CPU(s); i++) {
        vgic_vcpu_irq_update(s->vstate[i].vcpu, (update_mask & (1U << i)) ? TRUE : FALSE);
    }

    return rc;
}

static int vgic_dist_reg_read(struct vgic_guest_state *s, uint32_t offset, uint32_t *dst)
{
    vmm_vcpu_t *vcpu;
...
static int vgic_dist_reg_write(struct vgic_guest_state *s, uint32_t offset, uint32_t regmask, uint32_t regval)
{
    vmm_vcpu_t *vcpu;
...
static int vgic_dist_emulator_read8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;

    rc              = vgic_dist_reg_read(edev->private, offset, &regval);
...
static int vgic_dist_emulator_read16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t *dst)
{
    int      rc;
    uint32_t regval = 0x0;
...
static int vgic_dist_emulator_read32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t *dst)
{
    return vgic_dist_reg_read(edev->private, offset, dst);
}

static int vgic_dist_emulator_write8(vmm_emulate_device_t *edev, physical_addr_t offset, uint8_t src)
{
    return vgic_dist_reg_write(edev->private, offset, 0xFFFFFF00, src);
}

static int vgic_dist_emulator_write16(vmm_emulate_device_t *edev, physical_addr_t offset, uint16_t src)
{
    return vgic_dist_reg_write(edev->private, offset, 0xFFFF0000, src);
}

static int vgic_dist_emulator_write32(vmm_emulate_device_t *edev, physical_addr_t offset, uint32_t src)
{
    return vgic_dist_reg_write(edev->private, offset, 0x00000000, src);
}

static int vgic_dist_emulator_reset(vmm_emulate_device_t *edev)
{
    uint32_t                 i, j;
    irq_flags_t              flags;
    ...
    for (i = 0; i < VGIC_NUM_IRQ(s); i++) {
        ...
        vmm_device_emulate_notify_irq_disabled(s->guest, i, -1);
    ...
    for (i = 0; i < 16; i++) {
        ...
        vmm_device_emulate_notify_irq_enabled(s->guest, i, -1);
...
static struct vmm_device_emulation_irqchip vgic_irqchip = {
    .name             = "VGIC",
...
static struct vgic_guest_state *vgic_state_alloc(const char *name, struct vmm_guest *guest, uint32_t num_cpu, uint32_t num_irq, uint32_t parent_irq)
{
    uint32_t                 i;
    vmm_vcpu_t              *vcpu;
    ...
    for (i = 0; i < VGIC_NUM_IRQ(s); i++) {
        vmm_device_emulate_register_irqchip(guest, i, &vgic_irqchip, s);
    }

    /* Setup save/restore hooks */
    list_for_each_entry(vcpu, &guest->vcpu_list, head)
    {
        arm_vgic_setup(vcpu, vgic_save_vcpu_context, vgic_restore_vcpu_context, vgic_irq_pending, s);
    }
...
static int vgic_state_free(struct vgic_guest_state *s)
{
    ...
    for (i = 0; i < s->num_irq; i++) {
        vmm_device_emulate_unregister_irqchip(s->guest, i, &vgic_irqchip, s);
    }
...
static int vgic_dist_emulator_probe(struct vmm_guest *guest, vmm_emulate_device_t *edev, const struct vmm_device_tree_nodeid *eid)
{
    int                      rc;
    ...
    if (vmm_device_tree_read_u32(edev->node, "num_irq", &num_irq)) {
...
static int vgic_dist_emulator_remove(vmm_emulate_device_t *edev)
{
    struct vgic_guest_state *s = edev->private;
...
static struct vmm_device_tree_nodeid vgic_dist_emuid_table[] = {
    {
     .type       = "pic",
...
static vmm_emulator_t vgic_dist_emulator = {
    .name        = "vgic-dist",
...
static int vgic_cpu_emulator_reset(vmm_emulate_device_t *edev)
{
    /* Nothing to do here. */
    return VMM_OK;
}
...
static int vgic_cpu_emulator_remove(vmm_emulate_device_t *edev)
{
    /* Nothing to do here. */
    return VMM_OK;
}

static struct vmm_device_tree_nodeid vgic_cpu_emuid_table[] = {
    {
     .type       = "pic",
...
static vmm_emulator_t vgic_cpu_emulator = {
    .name        = "vgic-cpu",
...
{
    int      rc;
    uint32_t maint_irq = (uint32_t)(uint64_t)arg0;

    rc                 = vmm_host_irq_register(maint_irq, "VGIC", arg1, NULL);
...
static int __init vgic_emulator_init(void)
{
    ...
    rc = vmm_device_emulate_register_emulator(&vgic_dist_emulator);

    if (rc) {
        goto fail_unprobe;
    }

    rc = vmm_device_emulate_register_emulator(&vgic_cpu_emulator);

    if (rc) {
        goto fail_unreg_dist;
    }

    vmm_smp_ipi_async_call(cpu_online_mask, vgic_enable_maint_irq, (void *)(uint64_t)vgich.params.maint_irq, vgic_maint_irq, NULL);
...
fail_unreg_dist:
    vmm_device_emulate_unregister_emulator(&vgic_dist_emulator);
fail_unprobe:
...
static void __exit vgic_emulator_exit(void)
{
    ...
    vmm_smp_ipi_async_call(cpu_online_mask, vgic_disable_maint_irq, (void *)(uint64_t)vgich.params.maint_irq, NULL, NULL);

    vmm_device_emulate_unregister_emulator(&vgic_cpu_emulator);
