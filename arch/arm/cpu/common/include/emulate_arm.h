...
#define ARM_INST_UNCOND_OP1_MASK           0x0FF00000
#define ARM_INST_UNCOND_OP1_SHIFT          20
...
#define arm_zero_extend(imm, bits)         ((uint32_t)(imm))
...
#define arm_condition_passed(cond, regs) (((cond) == 0xE) ? TRUE : arm_condition_check((cond), (regs)))
...
static inline uint32_t arm_shift(uint32_t val, uint32_t shift_t, uint32_t shift_n, uint32_t cin)
{
    return arm_shift_c(val, shift_t, shift_n, cin, NULL);
}

static inline uint32_t arm_expand_imm_c(uint32_t imm12, uint32_t cin, uint32_t *cout)
{
    return arm_shift_c((imm12 & 0xFF), arm_shift_ror, 2 * (imm12 >> 8), cin, cout);
}

static inline uint32_t arm_expand_imm(arch_regs_t *regs, uint32_t imm12)
{
    return arm_expand_imm_c(imm12, (arm_cpsr(regs) >> CPSR_CARRY_SHIFT) & 0x1, NULL);
}
