...
void cpu_vcpu_spsr32_update(vmm_vcpu_t *vcpu, uint32_t mode, uint32_t new_spsr)
{
    struct arm_priv_sysregs *s = &arm_private(vcpu)->sysregs;
...
uint32_t cpu_vcpu_regmode32_read(arch_regs_t *regs, uint32_t mode, uint32_t reg)
{
    switch (reg) {
        case 0:
        case 1:
...
void cpu_vcpu_regmode32_write(arch_regs_t *regs, uint32_t mode, uint32_t reg, uint32_t val)
{
    switch (reg) {
        case 0:
        case 1:
...
uint64_t cpu_vcpu_reg64_read(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint32_t reg)
{
    uint64_t ret;
...
void cpu_vcpu_reg64_write(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint32_t reg, uint64_t val)
{
    /* Truncate bits[63:32] for AArch32 mode */
    if (regs->pstate & PSR_MODE32) {
...
uint64_t cpu_vcpu_reg_read(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint32_t reg)
{
    if (regs->pstate & PSR_MODE32) {
        return cpu_vcpu_regmode32_read(regs, regs->pstate & PSR_MODE32_MASK, reg & 0xF);
    } else {
        return cpu_vcpu_reg64_read(vcpu, regs, reg);
    }
}

void cpu_vcpu_reg_write(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint32_t reg, uint64_t val)
{
    if (regs->pstate & PSR_MODE32) {
        cpu_vcpu_regmode32_write(regs, regs->pstate & PSR_MODE32_MASK, reg & 0xF, val);
    } else {
...
int arch_vcpu_init(vmm_vcpu_t *vcpu)
{
    ...
    const char *attr;
    irq_flags_t flags;
    uint32_t    saved_cptr_el2, saved_hstr_el2;
    ...
    if (!vcpu->reset_count) {
         ...
         */
        arm_private(vcpu)->cpuid    = cpuid;
        /* Initialize VCPU features */
        arm_private(vcpu)->features = 0;
        ...
        INIT_SPIN_LOCK(&arm_private(vcpu)->hcr_lock);
        arm_private(vcpu)->hcr =
            (HCR_TSW_MASK | HCR_TACR_MASK | HCR_TIDCP_MASK | HCR_TSC_MASK | HCR_TWE_MASK | HCR_TWI_MASK | HCR_FB_MASK | HCR_AMO_MASK | HCR_IMO_MASK |
             HCR_FMO_MASK | HCR_SWIO_MASK | HCR_VM_MASK);

        if (!(arm_regs(vcpu)->pstate & PSR_MODE32)) {
            arm_private(vcpu)->hcr |= HCR_RW_MASK;
        }

        /* Initialize Coprocessor Trap Register */
        arm_private(vcpu)->cptr = CPTR_TTA_MASK;
        arm_private(vcpu)->cptr |= CPTR_TFP_MASK;
        /* Initialize Hypervisor System Trap Register */
        arm_private(vcpu)->hstr = 0;
    ...
    arm_private(vcpu)->hcr &= ~(HCR_VSE_MASK | HCR_VI_MASK | HCR_VF_MASK);
    vmm_spin_unlock_irq_restore(&arm_private(vcpu)->hcr_lock, flags);

    /* Set last host CPU to invalid value */
    arm_private(vcpu)->last_hcpu = 0xFFFFFFFF;
...
void arch_vcpu_switch(vmm_vcpu_t *vcpu_on_thread, vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
    ...
    if (vcpu_on_thread) {
        ...
        arm_regs(vcpu_on_thread)->pstate = regs->pstate;
    ...
    if (vcpu->is_normal) {
        ...
        vmm_spin_lock_irq_save(&arm_private(vcpu)->hcr_lock, flags);
        msr(hcr_el2, arm_private(vcpu)->hcr);
        vmm_spin_unlock_irq_restore(&arm_private(vcpu)->hcr_lock, flags);
        msr(cptr_el2, arm_private(vcpu)->cptr);
        msr(hstr_el2, arm_private(vcpu)->hstr);
...
static void __cpu_vcpu_dump_user_reg(vmm_char_device_t *cdev, arch_regs_t *regs)
{
    ...
    vmm_cdev_printf(cdev, " %11s=0x%016lx %11s=0x%08lx\n", "PC", regs->pc, "PSTATE", (regs->pstate & 0xffffffff));
    ...
    for (i = 0; i < (CPU_GPR_COUNT); i++) {
        ...
        vmm_cdev_printf(cdev, " %9s%02d=0x%016lx", "X", i, regs->gpr[i]);
    }
