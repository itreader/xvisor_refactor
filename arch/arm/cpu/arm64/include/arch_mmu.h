...
static inline void cpu_mmu_invalidate_range(virtual_addr_t start, virtual_addr_t size)
{
...
static inline void cpu_mmu_at_test_exec(
    physical_addr_t s2_table_pa, bool s1_avail, physical_addr_t s1_table_pa, virtual_addr_t addr, bool write, struct cpu_mmu_at_test_result *tp)
{
    uint64_t        par;
    uint64_t        t_vttbr_el2, t_hcr_el2;
    uint64_t        t_par_el1, t_mair_el1, t_tcr_el1, t_ttbr0_el1, t_sctlr_el1;
    extern uint64_t __mair_set, __tcr_set, v8_crval[2];
    ...
    cpu_stage2_update(s2_table_pa, 0);

    if (s1_avail) {
        msr(mair_el1, __mair_set);
        msr(tcr_el1, (__tcr_set & (TCR_T0SZ_MASK | TCR_IRGN0_MASK | TCR_ORGN0_MASK | TCR_SH0_MASK | TCR_TG0_MASK)));
        msr(ttbr0_el1, s1_table_pa);
