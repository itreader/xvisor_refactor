 ...
 */
static bool cpu_vcpu_condition_check(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint32_t iss)
{
    bool     ret = FALSE;
    uint32_t cond;
    ...
    switch (cond >> 1) {
        ...
        case 4:
            ret = (regs->pstate & PSR_CARRY_MASK) ? TRUE : FALSE;
            ret = (ret && !(regs->pstate & PSR_ZERO_MASK)) ? TRUE : FALSE;
            break;
        ...
        case 5:
            if (regs->pstate & PSR_NEGATIVE_MASK) {
                ...
                ret = (regs->pstate & PSR_OVERFLOW_MASK) ? FALSE : TRUE;
            }
 ...
 */
static void cpu_vcpu_update_itstate(vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
...
{
    int      rc = VMM_OK;
    uint32_t opc2, opc1, CRn, Rt, CRm, t;
...
int cpu_vcpu_emulate_msr_mrs_system(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint32_t il, uint32_t iss)
{
    ...
    if (read) {
        if (!cpu_vcpu_sysregs_read(vcpu, regs, iss & ISS_SYSREG_MASK, &data)) {
            rc = VMM_ERR_NOTAVAIL;
        }
        ...
        if (!cpu_vcpu_sysregs_write(vcpu, regs, iss & ISS_SYSREG_MASK, data)) {
            rc = VMM_ERR_NOTAVAIL;
        }
    }
...
int cpu_vcpu_emulate_load(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint32_t il, uint32_t iss, physical_addr_t ipa)
{
    ...
    if (regs->pstate & PSR_MODE32) { /* Aarch32 VCPU */
        ...
        if (arm_private(vcpu)->sysregs.sctlr_el1 & SCTLR_EE_MASK) {
            data_endian = VMM_DEVICE_EMULATE_BIG_ENDIAN;
        } else {
            data_endian = VMM_DEVICE_EMULATE_LITTLE_ENDIAN;
        }
    ...
    switch (sas) {
        case 0:
            rc = vmm_device_emulate_emulate_read(vcpu, ipa, &data8, sizeof(data8), data_endian);

            if (!rc) {
                if (sse) {
                    cpu_vcpu_reg64_write(vcpu, regs, srt, arm_sign_extend(data8, 8, 32));
                } else {
        ...
        case 1:
            rc = vmm_device_emulate_emulate_read(vcpu, ipa, &data16, sizeof(data16), data_endian);

            if (!rc) {
                if (sse) {
                    cpu_vcpu_reg64_write(vcpu, regs, srt, arm_sign_extend(data16, 16, 32));
                } else {
        ...
        case 2:
            rc = vmm_device_emulate_emulate_read(vcpu, ipa, &data32, sizeof(data32), data_endian);
...
int cpu_vcpu_emulate_store(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint32_t il, uint32_t iss, physical_addr_t ipa)
{
    ...
    switch (sas) {
        case 0:
            ...
            rc    = vmm_device_emulate_emulate_write(vcpu, ipa, &data8, sizeof(data8), data_endian);
            break;

        case 1:
            data16 = cpu_vcpu_reg64_read(vcpu, regs, srt);
            rc     = vmm_device_emulate_emulate_write(vcpu, ipa, &data16, sizeof(data16), data_endian);
            break;

        case 2:
            data32 = cpu_vcpu_reg64_read(vcpu, regs, srt);
            rc     = vmm_device_emulate_emulate_write(vcpu, ipa, &data32, sizeof(data32), data_endian);
            break;
