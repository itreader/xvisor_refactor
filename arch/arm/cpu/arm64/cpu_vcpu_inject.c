...
static int __cpu_vcpu_inject_und32(vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
    uint32_t old_cpsr, new_cpsr, sctlr;
    ...
    new_cpsr &= ~(CPSR_IT2_MASK | CPSR_IT1_MASK | CPSR_JAZZLE_ENABLED | CPSR_BE_ENABLED | CPSR_THUMB_ENABLED);
    ...
    cpu_vcpu_regmode32_write(regs, CPSR_MODE_UNDEFINED, 14, regs->pc - ((old_cpsr & CPSR_THUMB_ENABLED) ? 2 : 4));
...
static int __cpu_vcpu_inject_und64(vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
    uint32_t esr;
    ...
    /* Update PSTATE */
    regs->pstate = (PSR_MODE64_EL1h | PSR_ASYNC_ABORT_DISABLED | PSR_FIQ_DISABLED | PSR_IRQ_DISABLED | PSR_MODE64_DEBUG_DISABLED);
...
static int __cpu_vcpu_inject_abt32(vmm_vcpu_t *vcpu, arch_regs_t *regs, bool is_pabt, virtual_addr_t addr)
{
    uint64_t far;
    uint32_t old_cpsr, new_cpsr, sctlr, ttbcr;
    ...
    old_cpsr = new_cpsr = regs->pstate & 0xFFFFFFFFULL;
    new_cpsr &= ~CPSR_MODE_MASK;
    new_cpsr |= (CPSR_MODE_ABORT | CPSR_ASYNC_ABORT_DISABLED | CPSR_IRQ_DISABLED);
    new_cpsr &= ~(CPSR_IT2_MASK | CPSR_IT1_MASK | CPSR_JAZZLE_ENABLED | CPSR_BE_ENABLED | CPSR_THUMB_ENABLED);
    ...
    cpu_vcpu_regmode32_write(regs, CPSR_MODE_ABORT, 14, regs->pc - ((old_cpsr & CPSR_THUMB_ENABLED) ? 4 : 0));
...
static int __cpu_vcpu_inject_abt64(vmm_vcpu_t *vcpu, arch_regs_t *regs, bool is_pabt, virtual_addr_t addr)
{
    uint32_t esr, old_pstate;
...
int cpu_vcpu_inject_undef(vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
    /* Sanity checks */
    if (!vcpu || !regs) {
        return VMM_ERR_FAIL;
    ...
    if (arm_private(vcpu)->hcr & HCR_RW_MASK) {
...
int cpu_vcpu_inject_pabt(vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
    /* Sanity checks */
    if (!vcpu || !regs) {
        return VMM_ERR_FAIL;
...
int cpu_vcpu_inject_dabt(vmm_vcpu_t *vcpu, arch_regs_t *regs, virtual_addr_t addr)
{
    /* Sanity checks */
    if (!vcpu || !regs) {
        return VMM_ERR_FAIL;
