...
void do_bad_mode(arch_regs_t *regs, uint64_t mode)
{
    uint32_t    ec, il, iss;
    ...
    vmm_printf("%s: CPU%d VCPU=%s unexpected exception\n", __func__, vmm_smp_processor_id(), (vcpu) ? vcpu->name : "(NULL)");
...
void do_sync(arch_regs_t *regs, uint64_t mode)
{
    int             rc = VMM_OK;
    uint32_t        ec, il, iss;
    uint64_t        esr, far, elr;
    ...
    if (rc) {
        vmm_printf("%s: CPU%d VCPU=%s sync failed (error %d)\n", __func__, vmm_smp_processor_id(), (vcpu) ? vcpu->name : "(NULL)", rc);
        vmm_printf("%s: ESR=0x%016lx EC=0x%x IL=0x%x ISS=0x%x\n", __func__, esr, ec, il, iss);
        vmm_printf("%s: ELR=0x%016lx FAR=0x%016lx HPFAR=0x%016lx\n", __func__, elr, far, mrs(hpfar_el2));
