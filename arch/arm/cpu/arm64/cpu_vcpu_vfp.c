...
void cpu_vcpu_vfp_dump(vmm_char_device_t *cdev, vmm_vcpu_t *vcpu)
{
    ...
    vmm_cdev_printf(cdev, "VFP Feature Registers\n");
    vmm_cdev_printf(cdev, " %11s=0x%08" PRIx32 "         %11s=0x%08" PRIx32 "\n", "MVFR0_EL1", vfp->mvfr0, "MVFR1_EL1", vfp->mvfr1);
    vmm_cdev_printf(cdev, " %11s=0x%08" PRIx32 "\n", "MVFR2_EL1", vfp->mvfr2);
    vmm_cdev_printf(cdev, "VFP System Registers\n");
    vmm_cdev_printf(cdev, " %11s=0x%08" PRIx32 "         %11s=0x%08" PRIx32 "\n", "FPCR", vfp->fpcr, "FPSR", vfp->fpsr);
    ...
    for (i = 0; i < 64; i++) {
        ...
        vmm_cdev_printf(cdev, " %9s%02d=0x%016" PRIx64, "D", (i), vfp->fpregs[i]);
    }
