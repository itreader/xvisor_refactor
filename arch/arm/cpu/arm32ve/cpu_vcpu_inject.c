...
int cpu_vcpu_inject_undef(vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
    ...
    cpu_vcpu_regmode_write(vcpu, regs, CPSR_MODE_UNDEFINED, 14, regs->pc - ((old_cpsr & CPSR_THUMB_ENABLED) ? 2 : 4));
...
static int __cpu_vcpu_inject_abt(vmm_vcpu_t *vcpu, arch_regs_t *regs, bool is_pabt, virtual_addr_t addr)
{
    ...
    cpu_vcpu_regmode_write(vcpu, regs, CPSR_MODE_ABORT, 14, regs->pc - ((old_cpsr & CPSR_THUMB_ENABLED) ? 4 : 0));
