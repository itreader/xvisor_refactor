    ...
    switch (reg_num) {
        ...
        case 8:
            if (mode == CPSR_MODE_FIQ) {
                ...
                arm_private(vcpu)->bnk.gpr_fiq[reg_num - 8] = hwreg;
                return hwreg;
            } else {
                return regs->gpr[reg_num];
            }
        ...
        case 13:
            switch (mode) {
                ...
                case CPSR_MODE_SYSTEM:
                    ...
                    arm_private(vcpu)->bnk.sp_usr = hwreg;
                    return hwreg;

                case CPSR_MODE_FIQ:
                    asm volatile(" mrs     %0, SP_fiq\n\t" : "=r"(hwreg)::"memory", "cc");
                    arm_private(vcpu)->bnk.sp_fiq = hwreg;
                    return hwreg;

                case CPSR_MODE_IRQ:
                    asm volatile(" mrs     %0, SP_irq\n\t" : "=r"(hwreg)::"memory", "cc");
                    arm_private(vcpu)->bnk.sp_irq = hwreg;
                    return hwreg;

                case CPSR_MODE_SUPERVISOR:
                    asm volatile(" mrs     %0, SP_svc\n\t" : "=r"(hwreg)::"memory", "cc");
                    arm_private(vcpu)->bnk.sp_svc = hwreg;
                    return hwreg;

                case CPSR_MODE_ABORT:
                    asm volatile(" mrs     %0, SP_abt\n\t" : "=r"(hwreg)::"memory", "cc");
                    arm_private(vcpu)->bnk.sp_abt = hwreg;
                    return hwreg;

                case CPSR_MODE_UNDEFINED:
                    asm volatile(" mrs     %0, SP_und\n\t" : "=r"(hwreg)::"memory", "cc");
                    arm_private(vcpu)->bnk.sp_und = hwreg;
                    return hwreg;
        ...
        case 14:
            switch (mode) {
                ...
                case CPSR_MODE_FIQ:
                    ...
                    arm_private(vcpu)->bnk.lr_fiq = hwreg;
                    return hwreg;

                case CPSR_MODE_IRQ:
                    asm volatile(" mrs     %0, LR_irq\n\t" : "=r"(hwreg)::"memory", "cc");
                    arm_private(vcpu)->bnk.lr_irq = hwreg;
                    return hwreg;

                case CPSR_MODE_SUPERVISOR:
                    asm volatile(" mrs     %0, LR_svc\n\t" : "=r"(hwreg)::"memory", "cc");
                    arm_private(vcpu)->bnk.lr_svc = hwreg;
                    return hwreg;

                case CPSR_MODE_ABORT:
                    asm volatile(" mrs     %0, LR_abt\n\t" : "=r"(hwreg)::"memory", "cc");
                    arm_private(vcpu)->bnk.lr_abt = hwreg;
                    return hwreg;

                case CPSR_MODE_UNDEFINED:
                    asm volatile(" mrs     %0, LR_und\n\t" : "=r"(hwreg)::"memory", "cc");
                    arm_private(vcpu)->bnk.lr_und = hwreg;
                    return hwreg;
    ...
    switch (reg_num) {
        ...
        case 8:
            if (mode == CPSR_MODE_FIQ) {
                ...
                arm_private(vcpu)->bnk.gpr_fiq[reg_num - 8] = reg_val;
            } else {
                regs->gpr[reg_num] = reg_val;
            }
        ...
        case 13:
            switch (mode) {
                ...
                case CPSR_MODE_SYSTEM:
                    ...
                    arm_private(vcpu)->bnk.sp_usr = reg_val;
                    break;

                case CPSR_MODE_FIQ:
                    asm volatile(" msr     SP_fiq, %0\n\t" ::"r"(reg_val) : "memory", "cc");
                    arm_private(vcpu)->bnk.sp_fiq = reg_val;
                    break;

                case CPSR_MODE_IRQ:
                    asm volatile(" msr     SP_irq, %0\n\t" ::"r"(reg_val) : "memory", "cc");
                    arm_private(vcpu)->bnk.sp_irq = reg_val;
                    break;

                case CPSR_MODE_SUPERVISOR:
                    asm volatile(" msr     SP_svc, %0\n\t" ::"r"(reg_val) : "memory", "cc");
                    arm_private(vcpu)->bnk.sp_svc = reg_val;
                    break;

                case CPSR_MODE_ABORT:
                    asm volatile(" msr     SP_abt, %0\n\t" ::"r"(reg_val) : "memory", "cc");
                    arm_private(vcpu)->bnk.sp_abt = reg_val;
                    break;

                case CPSR_MODE_UNDEFINED:
                    asm volatile(" msr     SP_und, %0\n\t" ::"r"(reg_val) : "memory", "cc");
                    arm_private(vcpu)->bnk.sp_und = reg_val;
                    break;
        ...
        case 14:
            switch (mode) {
                ...
                case CPSR_MODE_FIQ:
                    ...
                    arm_private(vcpu)->bnk.lr_fiq = reg_val;
                    break;

                case CPSR_MODE_IRQ:
                    asm volatile(" msr     LR_irq, %0\n\t" ::"r"(reg_val) : "memory", "cc");
                    arm_private(vcpu)->bnk.lr_irq = reg_val;
                    break;

                case CPSR_MODE_SUPERVISOR:
                    asm volatile(" msr     LR_svc, %0\n\t" ::"r"(reg_val) : "memory", "cc");
                    arm_private(vcpu)->bnk.lr_svc = reg_val;
                    break;

                case CPSR_MODE_ABORT:
                    asm volatile(" msr     LR_abt, %0\n\t" ::"r"(reg_val) : "memory", "cc");
                    arm_private(vcpu)->bnk.lr_abt = reg_val;
                    break;

                case CPSR_MODE_UNDEFINED:
                    asm volatile(" msr     LR_und, %0\n\t" ::"r"(reg_val) : "memory", "cc");
                    arm_private(vcpu)->bnk.lr_und = reg_val;
                    break;
...
{
    return cpu_vcpu_regmode_read(vcpu, regs, regs->cpsr & CPSR_MODE_MASK, reg_num);
}

void cpu_vcpu_reg_write(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint32_t reg_num, uint32_t reg_val)
{
    cpu_vcpu_regmode_write(vcpu, regs, regs->cpsr & CPSR_MODE_MASK, reg_num, reg_val);
}
...
uint32_t cpu_vcpu_spsr_retrieve(vmm_vcpu_t *vcpu, uint32_t mode)
{
    ...
    switch (mode) {
        case CPSR_MODE_ABORT:
            ...
            arm_private(vcpu)->bnk.spsr_abt = hwreg;
            return hwreg;

        case CPSR_MODE_UNDEFINED:
            asm volatile(" mrs     %0, SPSR_und\n\t" : "=r"(hwreg)::"memory", "cc");
            arm_private(vcpu)->bnk.spsr_und = hwreg;
            return hwreg;

        case CPSR_MODE_SUPERVISOR:
            asm volatile(" mrs     %0, SPSR_svc\n\t" : "=r"(hwreg)::"memory", "cc");
            arm_private(vcpu)->bnk.spsr_svc = hwreg;
            return hwreg;

        case CPSR_MODE_IRQ:
            asm volatile(" mrs     %0, SPSR_irq\n\t" : "=r"(hwreg)::"memory", "cc");
            arm_private(vcpu)->bnk.spsr_irq = hwreg;
            return hwreg;

        case CPSR_MODE_FIQ:
            asm volatile(" mrs     %0, SPSR_fiq\n\t" : "=r"(hwreg)::"memory", "cc");
            arm_private(vcpu)->bnk.spsr_fiq = hwreg;
            return hwreg;
...
int cpu_vcpu_spsr_update(vmm_vcpu_t *vcpu, uint32_t mode, uint32_t new_spsr)
{
    ...
    switch (mode) {
        case CPSR_MODE_ABORT:
            ...
            arm_private(vcpu)->bnk.spsr_abt = new_spsr;
            break;

        case CPSR_MODE_UNDEFINED:
            asm volatile(" msr     SPSR_und, %0\n\t" ::"r"(new_spsr) : "memory", "cc");
            arm_private(vcpu)->bnk.spsr_und = new_spsr;
            break;

        case CPSR_MODE_SUPERVISOR:
            asm volatile(" msr     SPSR_svc, %0\n\t" ::"r"(new_spsr) : "memory", "cc");
            arm_private(vcpu)->bnk.spsr_svc = new_spsr;
            break;

        case CPSR_MODE_IRQ:
            asm volatile(" msr     SPSR_irq, %0\n\t" ::"r"(new_spsr) : "memory", "cc");
            arm_private(vcpu)->bnk.spsr_irq = new_spsr;
            break;

        case CPSR_MODE_FIQ:
            asm volatile(" msr     SPSR_fiq, %0\n\t" ::"r"(new_spsr) : "memory", "cc");
            arm_private(vcpu)->bnk.spsr_fiq = new_spsr;
            break;
...
int arch_guest_init(struct vmm_guest *guest)
{
    ...
    if (!guest->reset_count) {
        ...
        arm_guest_private(guest)->ttbl = mmu_page_table_alloc(MMU_STAGE2, -1, page_table_attr, guest->id);
        ...
        if (vmm_device_tree_read_u32(guest->node, "psci_version", &arm_guest_private(guest)->psci_version)) {
            ...
            arm_guest_private(guest)->psci_version = 1;
        }
    }
...
int arch_guest_deinit(struct vmm_guest *guest)
{
    ...
    if (guest->arch_private) {
        if ((rc = mmu_page_table_free(arm_guest_private(guest)->ttbl))) {
            return rc;
        }
...
{
    int                 rc    = VMM_OK, ite;
    uint32_t            cpuid = 0, saved_hcptr, saved_hstr;
    struct arm_private *p;
    const char         *attr;
    irq_flags_t         flags;
    uint32_t            phys_timer_irq, virt_timer_irq;
    ...
    if (!vcpu->reset_count) {
        ...
        p->hcr =
            (HCR_TAC_MASK | HCR_TSW_MASK | HCR_TIDCP_MASK | HCR_TSC_MASK | HCR_TWE_MASK | HCR_TWI_MASK | HCR_FB_MASK | HCR_AMO_MASK | HCR_IMO_MASK |
             HCR_FMO_MASK | HCR_SWIO_MASK | HCR_VM_MASK);
        p->hcptr = (HCPTR_TTA_MASK | HCPTR_TASE_MASK | HCPTR_TCP_MASK);
        p->hstr  = (HSTR_TJDBX_MASK | HSTR_TTEE_MASK | HSTR_T9_MASK | HSTR_T15_MASK);
    ...
    vmm_spin_lock_irq_save(&p->hcr_lock, flags);
    p->hcr &= ~(HCR_VA_MASK | HCR_VI_MASK | HCR_VF_MASK);
    vmm_spin_unlock_irq_restore(&p->hcr_lock, flags);
    ...
    if (arm_feature(vcpu, ARM_FEATURE_GENERIC_TIMER)) {
        if (vmm_device_tree_read_u32(vcpu->node, "gentimer_phys_irq", &phys_timer_irq)) {
            phys_timer_irq = 0;
        }

        if (vmm_device_tree_read_u32(vcpu->node, "gentimer_virt_irq", &virt_timer_irq)) {
            virt_timer_irq = 0;
        }

        rc = generic_timer_vcpu_context_init(vcpu, &arm_gentimer_context(vcpu), phys_timer_irq, virt_timer_irq);
...
{
    int      rc = VMM_OK;
    uint32_t saved_hcptr, saved_hstr;
    ...
    if (arm_feature(vcpu, ARM_FEATURE_GENERIC_TIMER)) {
        if ((rc = generic_timer_vcpu_context_deinit(vcpu, &arm_gentimer_context(vcpu)))) {
            goto done;
        }
    }
...
void arch_vcpu_switch(vmm_vcpu_t *vcpu_on_thread, vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
    ...
    if (vcpu_on_thread) {
        ...
        arm_regs(vcpu_on_thread)->pc = regs->pc;
        arm_regs(vcpu_on_thread)->lr = regs->lr;
        arm_regs(vcpu_on_thread)->sp = regs->sp;

        for (ite = 0; ite < CPU_GPR_COUNT; ite++) {
            arm_regs(vcpu_on_thread)->gpr[ite] = regs->gpr[ite];
        }

        arm_regs(vcpu_on_thread)->cpsr = regs->cpsr;

        if (vcpu_on_thread->is_normal) {
            /* Update last host CPU */
            arm_private(vcpu_on_thread)->last_hcpu = vmm_smp_processor_id();
            /* Save VGIC registers */
            arm_vgic_save(vcpu_on_thread);
            /* Save general purpose banked registers */
            cpu_vcpu_banked_regs_save(&arm_private(vcpu_on_thread)->bnk);
            /* Save VFP and SIMD context */
            cpu_vcpu_vfp_save(vcpu_on_thread);
            /* Save CP14 context */
            cpu_vcpu_cp14_save(vcpu_on_thread);
            /* Save CP15 context */
            cpu_vcpu_cp15_save(vcpu_on_thread);

            /* Save generic timer */
            if (arm_feature(vcpu_on_thread, ARM_FEATURE_GENERIC_TIMER)) {
                generic_timer_vcpu_context_save(vcpu_on_thread, arm_gentimer_context(vcpu_on_thread));
            }
        }
    }
    ...
    if (vcpu->is_normal) {
        ...
        if (arm_feature(vcpu, ARM_FEATURE_GENERIC_TIMER)) {
            generic_timer_vcpu_context_restore(vcpu, arm_gentimer_context(vcpu));
        }
        ...
        cpu_vcpu_banked_regs_restore(&arm_private(vcpu)->bnk);
        /* Restore VGIC registers */
        arm_vgic_restore(vcpu);
        /* Update hypervisor context */
        vmm_spin_lock_irq_save(&arm_private(vcpu)->hcr_lock, flags);
        write_hcr(arm_private(vcpu)->hcr);
        vmm_spin_unlock_irq_restore(&arm_private(vcpu)->hcr_lock, flags);
        write_hcptr(arm_private(vcpu)->hcptr);
        write_hstr(arm_private(vcpu)->hstr);
        /* Update hypervisor Stage2 MMU context */
        mmu_stage2_change_page_table(arm_guest_private(vcpu->guest)->ttbl);

        /* Flush TLB if moved to new host CPU */
        if (arm_private(vcpu)->last_hcpu != vmm_smp_processor_id()) {
...
void arch_vcpu_post_switch(vmm_vcpu_t *vcpu, arch_regs_t *regs)
{
    ...
    if (arm_feature(vcpu, ARM_FEATURE_GENERIC_TIMER)) {
        generic_timer_vcpu_context_post_restore(vcpu, arm_gentimer_context(vcpu));
    }
}
...
static void __cpu_vcpu_dump_user_reg(vmm_char_device_t *cdev, arch_regs_t *regs)
{
    uint32_t i;

    vmm_cdev_printf(cdev, "Core Registers\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "SP", regs->sp, "LR", regs->lr, "PC", regs->pc);
    vmm_cdev_printf(cdev, " %7s=0x%08x\n", "CPSR", regs->cpsr);
    vmm_cdev_printf(cdev, "General Purpose Registers");
    ...
    for (i = 0; i < CPU_GPR_COUNT; i++) {
        ...
        vmm_cdev_printf(cdev, " %5s%02d=0x%08x", "R", i, regs->gpr[i]);
    }
...
void arch_vcpu_regs_dump(vmm_char_device_t *cdev, vmm_vcpu_t *vcpu)
{
    ...
    vmm_cdev_printf(cdev, "Hypervisor Registers\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "HCR", p->hcr, "HCPTR", p->hcptr, "HSTR", p->hstr);
    vmm_cdev_printf(cdev, " %7s=0x%016llx\n", "VTTBR", arm_guest_private(vcpu->guest)->ttbl->table_pa);

    /* Print banked registers */
    vmm_cdev_printf(cdev, "User Mode Registers (Banked)\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x\n", "SP", p->bnk.sp_usr, "LR", arm_regs(vcpu)->lr);
    vmm_cdev_printf(cdev, "Supervisor Mode Registers (Banked)\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "SP", p->bnk.sp_svc, "LR", p->bnk.lr_svc, "SPSR", p->bnk.spsr_svc);
    vmm_cdev_printf(cdev, "Abort Mode Registers (Banked)\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "SP", p->bnk.sp_abt, "LR", p->bnk.lr_abt, "SPSR", p->bnk.spsr_abt);
    vmm_cdev_printf(cdev, "Undefined Mode Registers (Banked)\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "SP", p->bnk.sp_und, "LR", p->bnk.lr_und, "SPSR", p->bnk.spsr_und);
    vmm_cdev_printf(cdev, "IRQ Mode Registers (Banked)\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "SP", p->bnk.sp_irq, "LR", p->bnk.lr_irq, "SPSR", p->bnk.spsr_irq);
    vmm_cdev_printf(cdev, "FIQ Mode Registers (Banked)\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x", "SP", p->bnk.sp_fiq, "LR", p->bnk.lr_fiq, "SPSR", p->bnk.spsr_fiq);
    ...
    for (i = 0; i < 5; i++) {
        ...
        vmm_cdev_printf(cdev, " %5s%02d=0x%08x", "R", (i + 8), arm_private(vcpu)->bnk.gpr_fiq[i]);
    }
