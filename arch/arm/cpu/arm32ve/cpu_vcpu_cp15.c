...
{
    struct arm_priv_cp15 *cp15 = &arm_private(vcpu)->cp15;
        ...
        case 7:          /* Cache control. */
            switch (CRm) {
                ...
                case 14: /* DCCISW */
                    switch (opc2) {
                        case 2:
                            vmm_cpumask_setall(&arm_private(vcpu)->dflush_needed);
                            vmm_cpumask_clear_cpu(vmm_smp_processor_id(), &arm_private(vcpu)->dflush_needed);
                            asm volatile("mcr p15, 0, %0, c7, c14, 2" : : "r"(data));
                            break;
                ...
                case 10: /* DCCSW */
                    switch (opc2) {
                        case 2:
                            vmm_cpumask_setall(&arm_private(vcpu)->dflush_needed);
                            vmm_cpumask_clear_cpu(vmm_smp_processor_id(), &arm_private(vcpu)->dflush_needed);
                            asm volatile("mcr p15, 0, %0, c7, c10, 2" : : "r"(data));
                            break;
...
void cpu_vcpu_cp15_restore(vmm_vcpu_t *vcpu)
{
     ...
     */
    if (vmm_cpumask_test_and_clear_cpu(vmm_smp_processor_id(), &arm_private(vcpu)->dflush_needed)) {
        vmm_flush_cache_all();
    }
}
...
void cpu_vcpu_cp15_dump(vmm_char_device_t *cdev, vmm_vcpu_t *vcpu)
{
    ...
    vmm_cdev_printf(cdev, "CP15 Identification Registers\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "MIDR", cp15->c0_midr, "MPIDR", cp15->c0_mpidr, "CTR", cp15->c0_cachetype);
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "PFR0", cp15->c0_pfr0, "PFR1", cp15->c0_pfr1, "DFR0", cp15->c0_dfr0);
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "AFR0", cp15->c0_afr0, "MMFR0", cp15->c0_mmfr0, "MMFR1", cp15->c0_mmfr1);
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "MMFR2", cp15->c0_mmfr2, "MMFR3", cp15->c0_mmfr3, "ISAR0", cp15->c0_isar0);
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "ISAR1", cp15->c0_isar1, "ISAR2", cp15->c0_isar2, "ISAR3", cp15->c0_isar3);
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x", "ISAR4", cp15->c0_isar4, "ISAR5", cp15->c0_isar5, "CSSID00", cp15->c0_ccsid[0]);
    ...
    for (i = 1; i < 16; i++) {
        ...
        vmm_cdev_printf(cdev, " %5s%02d=0x%08x", "CCSID", i, cp15->c0_ccsid[i]);
    }

    vmm_cdev_printf(cdev, "\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x\n", "CLID", cp15->c0_clid, "CSSEL", cp15->c0_cssel);
    vmm_cdev_printf(cdev, "CP15 Control Registers\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "SCTLR", cp15->c1_sctlr, "CPACR", cp15->c1_cpacr, "VBAR", cp15->c12_vbar);
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x\n", "FCSEIDR", cp15->c13_fcseidr, "CNTXIDR", cp15->c13_contextidr);
    vmm_cdev_printf(cdev, "CP15 MMU Registers\n");
    vmm_cdev_printf(cdev, " %7s=0x%016llx %7s=0x%016llx\n", "TTBR0", cp15->c2_ttbr0, "TTBR1", cp15->c2_ttbr1);
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "TTBCR", cp15->c2_ttbcr, "DACR", cp15->c3_dacr, "PRRR", cp15->c10_prrr);
    vmm_cdev_printf(cdev, " %7s=0x%08x\n", "NMRR", cp15->c10_nmrr);
    vmm_cdev_printf(cdev, "CP15 Fault Status Registers\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "IFSR", cp15->c5_ifsr, "DFSR", cp15->c5_dfsr, "AIFSR", cp15->c5_aifsr);
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "ADFSR", cp15->c5_adfsr, "IFAR", cp15->c6_ifar, "DFAR", cp15->c6_dfar);
    vmm_cdev_printf(cdev, "CP15 Address Translation Registers\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%016llx\n", "PAR", cp15->c7_par, "PAR64", cp15->c7_par64);
    vmm_cdev_printf(cdev, "CP15 Cache Lockdown Registers\n");
    vmm_cdev_printf(
        cdev, " %7s=0x%08x %7s=0x%08x\n", "CILOCK", cp15->c9_insn, /* ??? */
        "CDLOCK", cp15->c9_data);                                  /* ??? */
    vmm_cdev_printf(cdev, "CP15 Performance Monitor Control Registers\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "PMCR", cp15->c9_pmcr, "PMCNTEN", cp15->c9_pmcnten, "PMOVSR", cp15->c9_pmovsr);
    vmm_cdev_printf(
        cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "PMXEVTY", cp15->c9_pmxevtyper, "PMUSREN", cp15->c9_pmuserenr, "PMINTEN", cp15->c9_pminten);
    vmm_cdev_printf(cdev, "CP15 Thread Local Storage Registers\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "TPIDURW", cp15->c13_tls1, "TPIDURO", cp15->c13_tls2, "TPIDPRW", cp15->c13_tls3);
}

int cpu_vcpu_cp15_init(vmm_vcpu_t *vcpu, uint32_t cpuid)
{
    uint32_t              i, cache_type, last_level;
    struct arm_priv_cp15 *cp15 = &arm_private(vcpu)->cp15;
    ...
    vmm_cpumask_clear(&arm_private(vcpu)->dflush_needed);
