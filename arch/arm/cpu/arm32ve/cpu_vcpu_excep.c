...
static int cpu_vcpu_stage2_map(vmm_vcpu_t *vcpu, arch_regs_t *regs, physical_addr_t fipa)
{
    int             rc, rc1;
    ...
    rc = mmu_map_page(arm_guest_private(vcpu->guest)->ttbl, &pg);
    ...
    if (rc) {
         ...
         */
        memset(&pg, 0, sizeof(pg));
        rc1 = mmu_get_page(arm_guest_private(vcpu->guest)->ttbl, fipa, &pg);
...
int cpu_vcpu_inst_abort(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint32_t il, uint32_t iss, virtual_addr_t ifar, physical_addr_t fipa)
{
