...
static bool cpu_vcpu_condition_check(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint32_t iss)
{
    ...
    switch (cond >> 1) {
        ...
        case 4:
            ret = (regs->cpsr & CPSR_CARRY_MASK) ? TRUE : FALSE;
            ret = (ret && !(regs->cpsr & CPSR_ZERO_MASK)) ? TRUE : FALSE;
            break;
        ...
        case 5:
            if (regs->cpsr & CPSR_NEGATIVE_MASK) {
                ...
                ret = (regs->cpsr & CPSR_OVERFLOW_MASK) ? FALSE : TRUE;
            }
...
int cpu_vcpu_emulate_cp0_cp13(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint32_t il, uint32_t iss)
{
    ...
    if (iss & ISS_CP0_TO_CP13_ASIMD_MASK) {
        ...
        coproc = (iss & ISS_CP0_TO_CP13_COPROC_MASK) >> ISS_CP0_TO_CP13_COPROC_SHIFT;
...
int cpu_vcpu_emulate_load(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint32_t il, uint32_t iss, physical_addr_t ipa)
{
    ...
    switch (sas) {
        case 0:
            ...
            if (!rc) {
                if (sse) {
                    cpu_vcpu_reg_write(vcpu, regs, srt, arm_sign_extend(data8, 8, 32));
                } else {
        ...
        case 1:
            ...
            if (!rc) {
                if (sse) {
                    cpu_vcpu_reg_write(vcpu, regs, srt, arm_sign_extend(data16, 16, 32));
                } else {
