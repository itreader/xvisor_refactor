...
{
    struct arm_private  *p   = arm_private(vcpu);
...
int cpu_vcpu_vfp_trap(vmm_vcpu_t *vcpu, arch_regs_t *regs, uint32_t il, uint32_t iss, bool is_asimd)
{
    struct arm_private  *p   = arm_private(vcpu);
    struct arm_priv_vfp *vfp = &p->vfp;
...
{
    uint32_t             i;
    struct arm_priv_vfp *vfp = &arm_private(vcpu)->vfp;
    ...
    vmm_cdev_printf(cdev, "VFP Identification Registers\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "FPSID", vfp->fpsid, "MVFR0", vfp->mvfr0, "MVFR1", vfp->mvfr1);
    vmm_cdev_printf(cdev, "VFP System Registers\n");
    vmm_cdev_printf(cdev, " %7s=0x%08x %7s=0x%08x %7s=0x%08x\n", "FPEXC", vfp->fpexc, "FPSCR", vfp->fpscr, "FPINST", vfp->fpinst);
    vmm_cdev_printf(cdev, " %7s=0x%08x\n", "FPINST2", vfp->fpinst2);
    vmm_cdev_printf(cdev, "VFP Data Registers");
    ...
    for (i = 0; i < 32; i++) {
        ...
        if (i < 16) {
            vmm_cdev_printf(cdev, " %5s%02d=0x%016llx", "D", (i), vfp->fpregs1[i]);
        } else {
            vmm_cdev_printf(cdev, " %5s%02d=0x%016llx", "D", (i), vfp->fpregs2[i - 16]);
        }
    }
