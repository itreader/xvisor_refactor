...
struct video_terminal_emulate *x86_vt;
#endif
...
static int __init boot_module_initrd(physical_addr_t start, physical_addr_t end)
{
     ...
     */
    rc = vmm_device_tree_setattr(node, "linux,initrd-start", &start, VMM_DEVICE_TREE_ATTRTYPE_UINT64, sizeof(start), FALSE);

    if (rc != VMM_OK) {
        goto _done;
    }

    rc = vmm_device_tree_setattr(node, "linux,initrd-end", &end, VMM_DEVICE_TREE_ATTRTYPE_UINT64, sizeof(end), FALSE);
...
static int __init boot_modules_init(void)
{
    ...
    for (i = 0; i < boot_info.mods_count; i++) {
        switch (i) {
            case 0:
                rc = boot_module_initrd(modlist->mod_start, modlist->mod_end);
...
int __init arch_board_final_init(void)
{
    ...
    /* Create VIDEO_TERMINAL_EMULATE instace from first available frame buffer
    ...
    if (info) {
        x86_vt = video_terminal_emulate_create(info->dev.name, info, NULL);
