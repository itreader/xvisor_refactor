 ...
 *****************************************************/
static inline void hpet_write(virtual_addr_t vbase, uint32_t reg_offset, uint64_t val)
{
    vmm_out_le64((uint64_t *)(vbase + reg_offset), val);
}

static inline uint64_t hpet_read(virtual_addr_t vbase, uint32_t reg_offset)
{
    return vmm_in_le64((uint64_t *)(vbase + reg_offset));
}
...
{
    uint64_t _v = hpet_read(timer->parent->vbase, HPET_GEN_CONF_BASE);
    return (_v & (0x01 << 1) ? 1 : 0);
}

static uint32_t hpet_timer_get_int_route(struct hpet_timer *timer)
{
    uint64_t _v = hpet_read(timer->parent->vbase, HPET_TIMER_N_CONF_BASE(timer->timer_id));
    debug_print("%s: int route: %x\n", __func__, (uint32_t)(_v >> 32));
    return (uint32_t)(_v >> 32);
}
...
static uint64_t hpet_read_main_counter(struct hpet_timer *timer)
{
    return hpet_read(timer->parent->vbase, HPET_GEN_MAIN_CNTR_BASE);
}

static void hpet_write_main_counter(struct hpet_timer *timer, uint64_t val)
{
...
static int hpet_initialize_timer(struct hpet_timer *timer, uint8_t dest_int, uint32_t flags)
{
    uint64_t tmr = 0;
    uint64_t _v  = hpet_read(timer->parent->vbase, HPET_TIMER_N_CONF_BASE(timer->timer_id));
...
/*****************************/
int hpet_system_clock_chip_init(void)
{
    return hpet_clock_chip_init(DEFAULT_HPET_SYS_TIMER, "hpet_clock_chip", 0);
}

int hpet_system_clocksource_init(void)
{
    return hpet_clocksource_init(DEFAULT_HPET_SYS_TIMER, "hpet_clock_src");
}
...
int __init hpet_init(void)
{
    ...
    BUG_ON(node == NULL);
    rc = vmm_device_tree_read_u32(node, VMM_DEVICE_TREE_NR_HPET_ATTR_NAME, &nr_hpet_chips);
    ...
    for (i = 0; i < nr_hpet_chips; i++) {
        ...
        for (j = 0; j < chip->nr_blocks; j++) {
            ...
            /* FIXME: read from device_tree */
            block->nr_timers = 32;

            vmm_sprintf(
                hpet_nm,
                VMM_DEVICE_TREE_PATH_SEPARATOR_STRING        VMM_DEVICE_TREE_MOTHERBOARD_NODE_NAME VMM_DEVICE_TREE_PATH_SEPARATOR_STRING
                "HPET" VMM_DEVICE_TREE_PATH_SEPARATOR_STRING VMM_DEVICE_TREE_HPET_NODE_FMT,
                j);

            node = vmm_device_tree_getnode(hpet_nm);
            BUG_ON(node == NULL);

            rc = vmm_device_tree_read_physaddr(node, VMM_DEVICE_TREE_HPET_PADDR_ATTR_NAME, &block->pbase);
            vmm_device_tree_dref_node(node);
            BUG_ON(rc != VMM_OK);
            ...
            for (k = 0; k < block->nr_timers; k++) {
                ...
                timer->is_busy     = 0;
                timer->parent      = block;
                timer->conf_cap    = hpet_read(timer->parent->vbase, HPET_GEN_CAP_ID_BASE);
    ...
    hpet_sys_timer_ops.sys_cc_init = &hpet_system_clock_chip_init;
...
static vmm_irq_return_t hpet_clock_chip_irq_handler(int irq_no, void *dev)
{
...
static void hpet_clock_chip_set_mode(vmm_clock_chip_mode_e mode, vmm_clock_chip_t *cc)
{
    ...
    switch (mode) {
        ...
        case VMM_CLOCKCHIP_MODE_ONESHOT:
            ...
            struct hpet_timer *timer = container_of(cc, struct hpet_timer, clock_chip);
            BUG_ON(timer == NULL);
            /* FIXME: N_CONF_BASE should be passed a timer id */
            uint64_t _v = hpet_read(timer->parent->vbase, HPET_TIMER_N_CONF_BASE(0));
            _v &= ~(0x01ULL << 3);
...
static int hpet_clock_chip_set_next_event(uint64_t next, vmm_clock_chip_t *cc)
{
    struct hpet_timer *timer = container_of(cc, struct hpet_timer, clock_chip);
    BUG_ON(timer == NULL);
    uint64_t res;
...
int __cpuinit hpet_clock_chip_init(timer_id_t timer_id, const char *chip_name, uint32_t target_cpu)
{
    int                rc;
    uint32_t           int_dest;
    ...
    timer->clock_chip.name   = chip_name;
    timer->clock_chip.hirq   = int_dest;
    timer->clock_chip.rating = 250;
#ifdef CONFIG_SMP
    timer->clkcip.cpumask = vmm_cpumask_of(target_cpu);
#else
    timer->clock_chip.cpumask = cpu_all_mask;
#endif
    debug_print("%s: Hpet Freq: %l Period: %l\n", __func__, timer->hpet_freq, timer->hpet_period);
    timer->clock_chip.features = VMM_CLOCKCHIP_FEAT_PERIODIC | VMM_CLOCKCHIP_FEAT_ONESHOT;
    timer->clock_chip.freq     = timer->hpet_freq;
    vmm_clocks_calc_mult_shift(&timer->clock_chip.mult, &timer->clock_chip.shift, NSEC_PER_SEC, timer->hpet_freq, 5);
    timer->clock_chip.min_delta_ns = 100000;
    debug_print("%s: Min Delts NS: %d\n", __func__, timer->clock_chip.min_delta_ns);
    debug_print("%s: mult: %l shift: %l\n", __func__, timer->clock_chip.mult, timer->clock_chip.shift);
    timer->clock_chip.max_delta_ns = vmm_clock_chip_delta2ns(0x7FFFFFFFFFFFFFFFULL, &timer->clock_chip);
    debug_print("%s: Max delta ns: %l\n", __func__, timer->clock_chip.max_delta_ns);
    timer->clock_chip.set_mode       = &hpet_clock_chip_set_mode;
    timer->clock_chip.set_next_event = &hpet_clock_chip_set_next_event;
    timer->clock_chip.private        = (void *)timer;

#ifdef CONFIG_SMP

    /* Set host irq affinity to target cpu */
    if ((rc = vmm_host_irq_set_affinity(irqno, vmm_cpumask_of(target_cpu), TRUE))) {
        return rc;
    }

#endif

    vmm_clock_chip_register(&timer->clock_chip);
    timer->armed = 0;
    rc           = vmm_host_irq_register(int_dest, chip_name, hpet_clock_chip_irq_handler, timer);
    BUG_ON(rc != VMM_OK);
 ...
 *          HPET CLOCK SOURCE            *
 *****************************************/
static uint64_t hpet_clocksource_read(vmm_clocksource_t *cs)
{
    struct hpet_timer *timer = container_of(cs, struct hpet_timer, clock_src);
    BUG_ON(timer == NULL);
...
static int hpet_clocksource_enable(vmm_clocksource_t *cs)
{
    struct hpet_timer *timer = container_of(cs, struct hpet_timer, clock_src);
    BUG_ON(timer == NULL);
...
static void hpet_clocksource_disable(vmm_clocksource_t *cs)
{
    struct hpet_timer *timer = container_of(cs, struct hpet_timer, clock_src);
    BUG_ON(timer == NULL);
    hpet_disable_main_counter(timer);
...
int __init hpet_clocksource_init(timer_id_t timer_id, const char *chip_name)
{
    ...
    timer->clock_src.name   = chip_name;
    timer->clock_src.rating = 300;
    timer->clock_src.mask   = 0xFFFFFFFFULL;
    timer->clock_src.freq   = timer->hpet_freq;
    vmm_clocks_calc_mult_shift(&timer->clock_src.mult, &timer->clock_src.shift, timer->hpet_freq, NSEC_PER_SEC, 5);
    debug_print("%s: Mult 0x%x shift: 0x%x\n", __func__, timer->clock_src.mult, timer->clock_src.shift);
    timer->clock_src.read    = &hpet_clocksource_read;
    timer->clock_src.disable = &hpet_clocksource_disable;
    timer->clock_src.enable  = &hpet_clocksource_enable;
    timer->clock_src.private = (void *)timer;
    ...
    t1 = hpet_clocksource_read(&timer->clock_src);
    t2 = hpet_clocksource_read(&timer->clock_src);
    ...
    return vmm_clocksource_register(&timer->clock_src);
}
