...
static void pci_fixup_via_northbridge_bug(struct pci_dev *d)
{
    ...
    } else if (d->device == PCI_DEVICE_ID_VIA_8363_0 && (d->revision == VIA_8363_KL133_REVISION_ID || d->revision == VIA_8363_KM133_REVISION_ID)) {
...
static int quirk_pcie_aspm_read(struct pci_bus *bus, uint32_t devfn, int where, int size, uint32_t *value)
{
 ...
 */
static int quirk_pcie_aspm_write(struct pci_bus *bus, uint32_t devfn, int where, int size, uint32_t value)
{
    uint8_t offset;
...
static void pcie_rootport_aspm_quirk(struct pci_dev *pdev)
{
     ...
     * hot-plug driver.
     */
    if ((pdev->device < PCI_DEVICE_ID_INTEL_MCH_PA) || (pdev->device > PCI_DEVICE_ID_INTEL_MCH_PC1)) {
        return;
    }
    ...
    if (list_empty(&pbus->devices)) {
        ...
        list_for_each_entry(dev, &pbus->devices, bus_list)
            ...
            quirk_aspm_offset[GET_INDEX(pdev->device, dev->devfn)] = dev->pcie_cap + PCI_EXP_LNKCTL;
...
static void pci_fixup_msi_k8t_onboard_sound(struct pci_dev *dev)
{
    ...
    if (val & 0x40) {
        ...
        if (val & 0x40) {
            dev_info(
                &dev->dev, "Detected MSI K8T Neo2-FIR; "
 ...
 */
static uint16_t toshiba_line_size;
...
static void pci_post_fixup_toshiba_ohci1394(struct pci_dev *dev)
{
    ...
    pci_read_config_byte(dev, PCI_INTERRUPT_LINE, (uint8_t *)&dev->irq);
