...
static struct pci_root_info *x86_find_pci_root_info(int bus)
{
    ...
    list_for_each_entry(info, &pci_root_infos, list) if (info->busn.start == bus) return info;
...
void x86_pci_root_bus_resources(int bus, list_head_t *resources)
{
    struct pci_root_info          *info = x86_find_pci_root_info(bus);
...
void update_res(struct pci_root_info *info, resource_size_t start, resource_size_t end, uint64_t flags, int merge)
{
    struct resource     *res;
    struct pci_root_res *root_res;
