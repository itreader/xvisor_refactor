...
static struct pcibios_fwaddrmap *pcibios_fwaddrmap_lookup(struct pci_dev *dev)
{
    ...
    list_for_each_entry(map, &pcibios_fwaddrmappings, list) if (map->dev == dev) return map;

    return NULL;
}

static void pcibios_save_fw_addr(struct pci_dev *dev, int idx, resource_size_t fw_addr)
{
    uint64_t                  flags;
    struct pcibios_fwaddrmap *map;
    ...
    spin_lock_irq_save(&pcibios_fwaddrmap_lock, flags);
    map = pcibios_fwaddrmap_lookup(dev);

    if (!map) {
        spin_unlock_irq_restore(&pcibios_fwaddrmap_lock, flags);
...
static int skip_isa_ioresource_align(struct pci_dev *dev)
{
    ...
    if ((pci_probe & PCI_CAN_SKIP_ISA_ALIGN) && !(dev->bus->bridge_ctl & PCI_BRIDGE_CTL_ISA)) {
        return 1;
    }
...
static void pcibios_allocate_bus_resources(struct pci_bus *bus)
{
    ...
    list_for_each_entry(child, &bus->children, node) pcibios_allocate_bus_resources(child);
}
