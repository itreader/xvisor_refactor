...
uint32_t pci_probe = PCI_PROBE_BIOS | PCI_PROBE_CONF1 | PCI_PROBE_CONF2 | PCI_PROBE_MMCONF;

uint32_t   pci_early_dump_regs;
...
static int pci_read(struct pci_bus *bus, uint32_t devfn, int where, int size, uint32_t *value)
{
    return raw_pci_read(pci_domain_nr(bus), bus->number, devfn, where, size, value);
}

static int pci_write(struct pci_bus *bus, uint32_t devfn, int where, int size, uint32_t value)
{
...
{
    uint8_t *d = (uint8_t *)dm + 4;

    if (dm->type != 0xB1) {
        return;
    }

    switch (((*(uint32_t *)d) >> 9) & 0x03) {
        case 0x00:
...
static int __init assign_all_busses(const struct dmi_system_id *d)
{
    ...
    printk(
        KERN_INFO "%s detected: enabling PCI bus# renumbering"
                  " (pci=assign-busses)\n",
        d->ident);
    return 0;
}
...
extern void x86_pci_root_bus_resources(int bus, list_head_t *resources);
...
uint32_t pcibios_assign_all_busses(void)
{
