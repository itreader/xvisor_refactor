...
static inline void set_interrupt_gate(uint8_t intrno, physical_addr_t addr, uint8_t ist)
{
    set_idt_gate_handler(intrno, addr, IDT_GATE_TYPE_INTERRUPT, ist);
}

static inline void set_trap_gate(uint8_t trapno, physical_addr_t addr, uint8_t ist)
{
...
{
    extern uint8_t _ist_stacks_start;
    int            i;
    uint32_t      *tss_stacks  = (uint32_t *)(&init_tss->ist1_lo);
    uint64_t       stack_start = (uint64_t)&_ist_stacks_start;

    for (i = 0; i < (2 * NR_IST_STACKS); i += 2) {
        debug_print("stack[%d]: %lx%lx\n", i, stack_start >> 32, stack_start & 0xFFFFFFFFUL);
        tss_stacks[i]     = (uint32_t)(stack_start & 0xFFFFFFFFUL);
        tss_stacks[i + 1] = (uint32_t)((stack_start >> 32) & 0xFFFFFFFFUL);
...
static void install_tss_64_descriptor(struct tss_64 *init_tss)
{
    ...
    uint32_t           tss_seg_sel    = VMM_TSS_SEG_SEL;
    struct tss64_desc *tss_64_desc    = &__xvisor_tss_64_desc;
    ...
    tss_64_desc->tb.bits.tss_base4    = ((tss_base >> 32) & 0xFFFFFFFFUL);
...
{
    uint32_t i;
    uint64_t user_irq_base = VIRT_TO_PHYS(__IRQ_32);
