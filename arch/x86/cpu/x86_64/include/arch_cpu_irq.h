...
static inline uint64_t __arch_save_flags(void)
{
    uint64_t flags;
...
static inline void __arch_restore_flags(uint64_t flags)
{
...
#ifndef __ASSEMBLY__
static inline int arch_irqs_disabled_flags(uint64_t flags)
{
 ...
 */
#define arch_cpu_wait_for_irq() asm volatile("hlt\n");
