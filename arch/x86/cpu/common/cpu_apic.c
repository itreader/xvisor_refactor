...
void trace_timer(uint64_t next, uint64_t tsc, uint64_t cc, uint64_t esr, uint64_t lvt)
{
...
void trace_timer_irq(uint64_t tsc, uint64_t cc, uint64_t init)
{
...
static uint32_t is_lapic_present(void)
{
    uint32_t a, b, c, d;
...
static inline uint32_t lapic_read(virtual_addr_t base)
{
    return vmm_readl((void *)base);
}

static inline void lapic_write(virtual_addr_t base, uint32_t val)
{
    vmm_writel(val, (void *)base);
}

static uint32_t ioapic_read(virtual_addr_t ioa_base, uint32_t reg)
{
    vmm_writel((reg & 0xff), (void *)(ioa_base + IOAPIC_IOREGSEL));
    return vmm_readl((void *)(ioa_base + IOAPIC_IOWIN));
}

static void ioapic_write(virtual_addr_t ioa_base, uint8_t reg, uint32_t val)
{
...
#ifdef DEBUG_IOAPIC
static uint64_t ioapic_read_irt_entry(virtual_addr_t ioapic_addr, int pin)
{
    uint8_t  hia = IOAPIC_REDIR_TABLE + pin * 2;
    uint8_t  loa = hia++;
    uint64_t val;

    uint32_t hi = ioapic_read(ioapic_addr, hia);
    uint32_t lo = ioapic_read(ioapic_addr, loa);

    val         = (uint64_t)(((uint64_t)hi) << 32 | (lo & 0xFFFFFFFFUL));

    return val;
}
#endif

static int ioapic_write_irt_entry(virtual_addr_t ioapic_addr, int pin, uint64_t entry)
{
    uint8_t  hia = IOAPIC_REDIR_TABLE + pin * 2;
    uint8_t  loa = hia++;
    uint32_t lo  = (uint32_t)(entry & 0xFFFFFFFFUL);
    uint32_t hi  = (uint32_t)((entry >> 32) & 0xFFFFFFFFUL);
...
static int ioapic_route_irq_to_vector(struct cpu_ioapic *ioapic, uint32_t irq, uint32_t vector)
{
...
void ioapic_set_id(uint32_t addr, uint32_t id)
{
...
int detect_ioapics(uint32_t *nr_ioapics)
{
    uint32_t                val;
    int                     ret = VMM_OK;
    ...
    *nr_ioapics = 1;
    ...
    while (n < val) {
        vmm_sprintf(
            apic_nm,
            VMM_DEVICE_TREE_PATH_SEPARATOR_STRING        VMM_DEVICE_TREE_MOTHERBOARD_NODE_NAME VMM_DEVICE_TREE_PATH_SEPARATOR_STRING
            "APIC" VMM_DEVICE_TREE_PATH_SEPARATOR_STRING VMM_DEVICE_TREE_IOAPIC_NODE_FMT,
            n);

        node = vmm_device_tree_getnode(apic_nm);
        BUG_ON(node == NULL);

        ret = vmm_device_tree_read_physaddr(node, VMM_DEVICE_TREE_IOAPIC_PADDR_ATTR_NAME, &io_apic[n].paddr);
        vmm_device_tree_dref_node(node);
        ...
        io_apic[n].pins  = ((ioapic_read(io_apic[n].vaddr, IOAPIC_VERSION) & 0xff0000) >> 16) + 1;
...
static int setup_ioapic_irq_route(struct cpu_ioapic *ioapic, uint32_t irq, uint32_t vector)
{
...
#define IOAPIC_IRQ_TO_VECTOR(_ioapic_id, _irq) ((IOAPIC_IRQ_BASE * (_ioapic_id + 1)) + _irq)
...
static int setup_ioapic(void)
{
    ...
    for (nr = 0; nr < nioapics; nr++) {
        for (i = 0; i < NR_IOAPIC_IRQ; i++) {
            setup_ioapic_irq_route(&io_apic[nr], i, IOAPIC_IRQ_TO_VECTOR(nr, i));
        }
    }
...
/******************************/
static void lapic_set_timer_count(uint32_t count, int periodic);
static void lapic_stop_timer(void);

struct lapic_timer {
    uint8_t                  timer_name[APIC_NAME_LEN];
    uint32_t                 timer_cpu;
    uint32_t                 freq_khz;
    uint32_t                 armed;
...
static vmm_irq_return_t lapic_clock_chip_irq_handler(int irq_no, void *dev)
{
    ...
    if (unlikely(!timer->clock_chip.event_handler)) {
        return VMM_IRQ_NONE;
    }

    /* call the event handler */
    timer->clock_chip.event_handler(&timer->clock_chip);

    return VMM_IRQ_HANDLED;
}

static void lapic_clock_chip_set_mode(vmm_clock_chip_mode_e mode, vmm_clock_chip_t *cc)
{
    BUG_ON(cc == NULL);
...
static int lapic_arm_timer(struct lapic_timer *timer)
{
    uint32_t lvt;
    uint32_t flags;

    vmm_spin_lock_irq_save(&this_cpu(lapic).lock, flags);
...
static __unused int lapic_disarm_timer(struct lapic_timer *timer)
{
    uint32_t lvt;
    uint32_t flags;
...
#ifdef CONFIG_USE_DEADLINE_TSC
static int lapic_set_deadline(uint64_t next)
{
    uint64_t res;
    uint32_t nr_tries = 5;
...
static int lapic_set_icr(uint64_t next)
{
...
static int lapic_clock_chip_set_next_event(uint64_t next, vmm_clock_chip_t *cc)
{
    struct lapic_timer *timer = container_of(cc, struct lapic_timer, clock_chip);
    uint32_t            flags;
    int                 rc;
    ...
    trace_timer(
        next, get_tsc_serialized(), lapic_read(LAPIC_TIMER_CCR(this_cpu(lapic).vbase)), lapic_read(LAPIC_ESR(this_cpu(lapic).vbase)),
...
int __cpuinit lapic_clock_chip_init(void)
{
    int      rc, irq;
    uint32_t lvt;

    lapic_sys_timer.clock_chip.name     = "lapic_clock_chip";
    lapic_sys_timer.clock_chip.hirq     = IRQ_VECTOR_TO_IRQ(LAPIC_TIMER_IRQ_VECTOR);
    lapic_sys_timer.clock_chip.rating   = 350;
    lapic_sys_timer.clock_chip.cpumask  = cpu_all_mask;
    lapic_sys_timer.clock_chip.features = (VMM_CLOCKCHIP_FEAT_PERIODIC | VMM_CLOCKCHIP_FEAT_ONESHOT);
    ...
    vmm_clocks_calc_mult_shift(
        &lapic_sys_timer.clock_chip.mult, &lapic_sys_timer.clock_chip.shift, NSEC_PER_SEC, (lapic_sys_timer.freq_khz * 1000), 5);
    lapic_sys_timer.clock_chip.min_delta_ns = 10000000;
    lapic_sys_timer.clock_chip.max_delta_ns = vmm_clock_chip_delta2ns(0x7FFFFFFFUL, &lapic_sys_timer.clock_chip);

    vmm_printf("%s: LAPIC Freq: %ukhz Max Delta NS: %lu\n", __func__, lapic_sys_timer.freq_khz, lapic_sys_timer.clock_chip.max_delta_ns);

    lapic_sys_timer.clock_chip.set_mode       = &lapic_clock_chip_set_mode;
    lapic_sys_timer.clock_chip.set_next_event = &lapic_clock_chip_set_next_event;
    lapic_sys_timer.clock_chip.private        = (void *)&lapic_sys_timer;

    vmm_clock_chip_register(&lapic_sys_timer.clock_chip);

    rc = vmm_host_irq_register(IRQ_VECTOR_TO_IRQ(LAPIC_TIMER_IRQ_VECTOR), "lapic_clock_chip", lapic_clock_chip_irq_handler, &lapic_sys_timer);
    BUG_ON(rc != VMM_OK);

    return VMM_OK;
}

static uint64_t lapic_clocksource_read(vmm_clocksource_t *cs)
{
 ...
 */
static int lapic_clocksource_enable(vmm_clocksource_t *cs)
{
    return VMM_OK;
}

static void lapic_clocksource_disable(vmm_clocksource_t *cs)
{
    return;
}
...
int __cpuinit lapic_clocksource_init(void)
{
    ...
    lapic_sys_timer.clock_src.name   = "lapic_clock_src";
    lapic_sys_timer.clock_src.rating = 400;
    lapic_sys_timer.clock_src.mask   = 0xFFFFFFFFUL;

    vmm_clocks_calc_mult_shift(&lapic_sys_timer.clock_src.mult, &lapic_sys_timer.clock_src.shift, (lapic_sys_timer.freq_khz * 1000), NSEC_PER_SEC, 5);

    lapic_sys_timer.clock_src.read    = &lapic_clocksource_read;
    lapic_sys_timer.clock_src.disable = &lapic_clocksource_disable;
    lapic_sys_timer.clock_src.enable  = &lapic_clocksource_enable;
    lapic_sys_timer.clock_src.private = (void *)(&lapic_sys_timer);

    rc                                = vmm_clocksource_register(&lapic_sys_timer.clock_src);

    return rc;
}

static uint32_t __init pit_calibrate_tsc(void)
{
    cycles_t start, end;
    uint64_t pit_tick_rate = 1193182UL; /* 1.193182 Mhz */
...
uint32_t __init lapic_calibrate_timer(void)
{
    const uint32_t tick_count = 100000000;
    ...
    } while (((tsc_now - tsc_start) < tick_count) && ((apic_start - apic_now) < tick_count));

    apic_Hz = (apic_start - apic_now) * 1000L * cpu_info.tsc_khz / (tsc_now - tsc_start);
...
int __init lapic_timer_init(void)
{
    struct x86_system_timer_ops lapic_sys_timer_ops;
    struct cpu_lapic           *lapic = NULL;

    lapic_sys_timer_ops.sys_cc_init   = &lapic_clock_chip_init;
    ...
    vmm_snprintf((char *)&lapic_sys_timer.timer_name[0], APIC_NAME_LEN, "LAPIC-%d", vmm_smp_processor_id());
