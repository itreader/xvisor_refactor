...
static inline void gather_cpu_brandinfo(struct cpuinfo_x86 *cpu_info)
{
    ...
    cpuid(
        CPUID_EXTENDED_BRANDSTRING, (uint32_t *)&cpu_info->name_string[0], (uint32_t *)&cpu_info->name_string[4],
        (uint32_t *)&cpu_info->name_string[8], (uint32_t *)&cpu_info->name_string[12]);

    cpuid(
        CPUID_EXTENDED_BRANDSTRINGMORE, (uint32_t *)&cpu_info->name_string[16], (uint32_t *)&cpu_info->name_string[20],
        (uint32_t *)&cpu_info->name_string[24], (uint32_t *)&cpu_info->name_string[28]);

    cpuid(
        CPUID_EXTENDED_BRANDSTRINGEND, (uint32_t *)&cpu_info->name_string[32], (uint32_t *)&cpu_info->name_string[36],
        (uint32_t *)&cpu_info->name_string[40], (uint32_t *)&cpu_info->name_string[44]);
}
...
static inline void gather_intel_cacheinfo(struct cpuinfo_x86 *cpu_info)
{
    ...
    uint32_t       trace = 0, l1i = 0, l1d = 0, l2 = 0, l3 = 0; /* Cache sizes */

    cpuid(CPUID_BASE_FEATURES, (uint32_t *)&regs[0], (uint32_t *)&regs[1], (uint32_t *)&regs[2], (uint32_t *)&regs[3]);
    n = regs[0] & 0xff;
...
void indentify_cpu(void)
{
    ...
    cpuid(
        CPUID_BASE_LFUNCSTD, (uint32_t *)&tmp, (uint32_t *)&cpu_info.vendor_string[0], (uint32_t *)&cpu_info.vendor_string[8],
