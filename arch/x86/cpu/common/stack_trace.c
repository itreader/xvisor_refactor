 ...
 *
 * Portions of this file are derived from arch/x86/kernel/stack_trace.c
 * in linux source.
 *
 */
#include <arch_regs.h>
...
static void __save_stack_address(void *data, uint64_t addr, bool reliable)
{
...
static void save_stack_address(void *data, uint64_t addr, int reliable)
{
    return __save_stack_address(data, addr, reliable);
}

static const struct stack_trace_ops save_stack_ops = {
