...
#define EXT_DEVICE_NAME_LEN 256
...
struct ioapic_ext_irq_device {
    char ext_dev_name[EXT_DEVICE_NAME_LEN];
    ...
    int (*irq_set_type)(void *data, uint32_t flow_type);
    vmm_irq_return_t (*irq_handler)(uint32_t irq_no, void *data);
    void         *data;
    double_list_t head;
};
...
int ioapic_route_pin_to_irq(uint32_t pin, uint32_t irqno);

/** Attach an extern device to given IRQ line. */
int ioapic_set_ext_irq_device(uint32_t irqno, struct ioapic_ext_irq_device *device, void *data);
