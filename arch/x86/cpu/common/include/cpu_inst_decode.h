...
typedef union mod_rm {
    ...
    struct {
        uint32_t src : 3; /* src reg */
        uint32_t dst : 3; /* dst reg */
        uint32_t mod : 2;
    } f;
} mod32_rm_t;
...
typedef struct {
    ...
    union {
        struct {
            uint32_t op_size;  /* size of operation */
            uint32_t src_type; /* type of source operand (mem or register) */
            uint32_t dst_type;
            uint64_t src_addr;
            uint64_t dst_addr;
        } gen_mov;
        ...
        struct {
            ...
            uint32_t offs; /* offset in segment */
        } lja;
    } inst;
} x86_decoded_inst_t;

int x86_decode_inst(struct vcpu_hw_context *context, x86_inst inst, x86_decoded_inst_t *dinst);
