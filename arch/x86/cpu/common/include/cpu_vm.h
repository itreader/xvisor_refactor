...
struct vcpu_hw_context {
    uint32_t            instruction_error; /* !!NOTE!!: This has to be first variable */
    uint32_t            sign;              /* 8-byte align */
    struct cpuinfo_x86 *cpuinfo;
    struct vmcb        *vmcb;
    struct vmcs        *vmcs;
    vmm_vcpu_t         *assoc_vcpu; /**< vCPU associated to this hardware context */
    uint64_t            g_regs[NR_GUEST_REGS];
    uint64_t            g_cr0;      /* till g_cr3 is what guest sees */
    uint64_t            g_cr1;
    uint64_t            g_cr2;
    uint64_t            g_cr3;
    uint64_t            g_cr4;
    ...
    uint64_t            g_cr8;
    uint64_t            g_rip;
    uint64_t            vmx_last_exit_qualification;

    uint32_t           asid;
    uint64_t           eptp;
    uint64_t           vmcs_state;
    uint64_t           n_cr3;            /* [Note] When #VMEXIT occurs with
    ...
    uint32_t              host_msr_count;
    struct vmx_msr_entry *host_msr_area;

    int             itc_flag;       /* flags specifying which interceptions were
                               registered for this vm. */
    int             itc_skip_flag;
    uint64_t        guest_start_pc; /* Guest will start execution from here (comes from DTS) */
...
#define GUEST_DEVICE_MMIO_REGION (VMM_REGION_MEMORY | VMM_REGION_VIRTUAL)
...
#define x86_vcpu_hw_context(vcpu)             ((struct vcpu_hw_context *)(((struct x86_vcpu_priv *)((vcpu)->arch_private))->hw_context))

extern void print_page_errorcode(uint64_t errcode);

extern physical_addr_t cpu_create_vcpu_intercept_table(size_t size, virtual_addr_t *table_vaddr);
...
extern void            enable_ioport_intercept(struct vcpu_hw_context *context, uint32_t ioport);
extern void            disable_ioport_intercept(struct vcpu_hw_context *context, uint32_t ioport);
