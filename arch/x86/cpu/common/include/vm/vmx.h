...
#define cpu_has_vmx_ept_wl4_supported         (vmx_ept_vpid_cap & VMX_EPT_WALK_LENGTH_4_SUPPORTED)
...
#define cpu_has_vmx_invept                    (vmx_ept_vpid_cap & VMX_EPT_INVEPT_INSTRUCTION)
#define cpu_has_vmx_ept_invept_single_context (vmx_ept_vpid_cap & VMX_EPT_INVEPT_SINGLE_CONTEXT)
#define cpu_has_vmx_ept_invept_all_context    (vmx_ept_vpid_cap & VMX_EPT_INVEPT_ALL_CONTEXT)
...
#define cpu_has_vmx_vpid_invvpid_individual_addr                 (vmx_ept_vpid_cap & VMX_VPID_INVVPID_INDIVIDUAL_ADDR)
#define cpu_has_vmx_vpid_invvpid_single_context                  (vmx_ept_vpid_cap & VMX_VPID_INVVPID_SINGLE_CONTEXT)
#define cpu_has_vmx_vpid_invvpid_single_context_retaining_global (vmx_ept_vpid_cap & VMX_VPID_INVVPID_SINGLE_CONTEXT_RETAINING_GLOBAL)
...
static inline int __vmptrld(uint64_t addr)
{
    int rc = 0;
    ...
    asm volatile("1: vmptrld (%1) \n\t"
                 ...
                 ".previous\n"
                 : "=a"(rc)
                 : "c"(&addr), "0"(0), "i"(-VMX_FAIL_INVALID), "i"(-VMX_FAIL_VALID), "i"(-VMX_FAIL_UD_GF)
                 : "memory", "cc");

    return rc;
}

static inline uint64_t __vmptrst(void)
{
    uint64_t addr = 0;
    int      rc   = VMM_OK;
...
static inline int __vmpclear(uint64_t addr)
{
    int rc = VMM_OK;
...
static inline int __vmread(uint64_t field, uint64_t *value)
{
    uint64_t edx;
    int      rc = VMM_OK;
    ...
    asm volatile("1: vmread %2, %1\n\t"
                 ...
                 ".previous\n"
                 : "=r"(rc), "=r"(edx)
                 : "r"(field), "i"(-VMX_FAIL_INVALID), "i"(-VMX_FAIL_VALID), "i"(-VMX_FAIL_UD_GF), "0"(0)
                 : "memory", "cc");
...
static inline int __vmwrite(uint64_t field, uint64_t value)
{
    int rc = VMM_OK;
    asm volatile("1: vmwrite %1, %2\n"
                 ...
                 ".previous\n"
                 : "=qm"(rc)
                 : "r"(value), "r"(field), "i"(-VMX_FAIL_INVALID), "i"(-VMX_FAIL_VALID), "i"(-VMX_FAIL_UD_GF)
                 : "memory");

    return rc;
}

static inline uint64_t __vmread_safe(uint64_t field, int *error)
{
    uint64_t ecx;
    int      rc;
...
static inline uint64_t vmr(uint64_t field)
{
    int      rc;
    uint64_t val;
...
static inline int __vm_set_bit(uint64_t field, uint32_t bit)
{
    uint64_t value;
    int      rc = VMM_OK;
...
static inline int __vm_clear_bit(uint64_t field, uint32_t bit)
{
    uint64_t value;
    int      rc = VMM_OK;
