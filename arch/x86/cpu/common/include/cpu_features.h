...
#define DECLARE_CPUID_BASE_FEAT_REG_MASK(_f, _r, _m)      __CPUID_FEAT_MASK(CPUID_FEAT_BASE_##_f##_##_r##_##_m##_BIT)
#define DECLARE_CPUID_EXTD_FEAT_REG_MASK(_f, _r, _m)      __CPUID_FEAT_MASK(CPUID_FEAT_EXTENDED_##_f##_##_r##_##_m##_BIT)

/* To be used in side enum */
#define DECLARE_CPUID_BASE_FEAT_BIT_W_NUM(_f, _r, _b, _n) CPUID_FEAT_BASE_##_f##_##_r##_##_b##_BIT = (_n)
#define DECLARE_CPUID_BASE_FEAT_BIT_ENUM(_f, _r, _b)      CPUID_FEAT_BASE_##_f##_##_r##_##_b##_BIT
#define DECLARE_CPUID_EXTD_FEAT_BIT_W_NUM(_f, _r, _b, _n) CPUID_FEAT_EXTENDED_##_f##_##_r##_##_b##_BIT = (_n)
#define DECLARE_CPUID_EXTD_FEAT_BIT_ENUM(_f, _r, _b)      CPUID_FEAT_EXTENDED_##_f##_##_r##_##_b##_BIT
...
struct cpuinfo_x86 {
    ...
    uint8_t  model;
    uint8_t  stepping;
    uint8_t  vendor_string[PROCESSOR_VENDOR_ID_LEN];
    uint8_t  name_string[PROCESSOR_NAME_STRING_LEN];
    uint8_t  virt_bits;
    uint8_t  phys_bits;
    uint8_t  cpuid_level;
    uint8_t  l1_dcache_size;
    uint8_t  l1_dcache_line_size;
    uint8_t  l1_icache_size;
    uint8_t  l1_icache_line_size;
    uint16_t l2_cache_size;
    uint16_t l2_cache_line_size;
    uint16_t l3_cache_size;
    uint8_t  hw_virt_available;
    uint8_t  hw_nested_paging;
    uint8_t  decode_assist;
    uint32_t hw_nr_asids;
    uint32_t tsc_khz; /* calibrated TSC frequency */
    uint32_t lapic_khz;
...
static inline uint64_t cpu_read_msr(uint32_t msr)
{
    ...
    return (uint64_t)(((uint64_t)d << 32) | (a & 0xFFFFFFFFUL));
}

static inline void cpu_read_msr32(uint32_t msr, uint32_t *high, uint32_t *low)
{
    uint32_t a, d;
...
static inline void cpu_write_msr32(uint32_t msr, uint32_t high, uint32_t low)
{
    asm volatile("wrmsr\n\t" ::"a"(low), "d"(high), "c"(msr));
}
