...
{
    struct x86_guest_priv *private = vmm_zalloc(sizeof(struct x86_guest_private));
...
int arch_guest_add_region(struct vmm_guest *guest, struct vmm_region *region)
{
    ...
    if (region->flags & VMM_REGION_IO) {
        ...
        for (i = start; i < end; i++) {
            ...
            {
                enable_ioport_intercept(x86_vcpu_private(vcpu)->hw_context, i);
            }

            vmm_read_unlock_irq_restore_lite(&guest->vcpu_lock, flags);
        }
    } else if ((region->flags & VMM_REGION_MEMORY) && (region->flags & VMM_REGION_REAL) && (region->flags & VMM_REGION_IS_RAM)) {
        struct x86_guest_priv *private = x86_guest_private(guest);
...
int arch_guest_del_region(struct vmm_guest *guest, struct vmm_region *region)
{
    ...
    if (region->flags & VMM_REGION_IO) {
        ...
        for (i = start; i < end; i++) {
            ...
            list_for_each_entry(vcpu, &guest->vcpu_list, head)
            {
                disable_ioport_intercept(x86_vcpu_private(vcpu)->hw_context, i);
            }
...
static void guest_cmos_init(struct vmm_guest *guest)
{
    int val;
    struct x86_guest_priv *private = x86_guest_private(guest);
    struct cmos_rtc_state *s       = private->rtc_cmos;

    /* memory size */
    /* base memory (first MiB) */
    val                            = min((int)(private->tot_ram_sz / 1024), 640);

    s->rtc_cmos_write(s, RTC_REG_BASE_MEM_LO, val);
    s->rtc_cmos_write(s, RTC_REG_BASE_MEM_HI, val >> 8);

    /* extended memory (next 64MiB) */
    if (private->tot_ram_sz > 1024 * 1024) {
        val = (private->tot_ram_sz - 1024 * 1024) / 1024;
    } else {
        val = 0;
    }
    ...
    if (private->tot_ram_sz > 16 * 1024 * 1024) {
        val = (private->tot_ram_sz - 16 * 1024 * 1024) / 65536;
    } else {
        val = 0;
    }
...
{
    uint32_t tcr3 = (uint32_t)context->vmcb->cr3;
    ...
    /* page directory entry */
    pte_addr = (((uint32_t)(pde & ~0xfff)) + (4 * ((vaddr >> 12) & 0x3ff)));
...
int purge_guest_shadow_pagetable(struct vcpu_hw_context *context)
{
    ...
    memset(context->shadow32_pg_list, 0, NR_32BIT_PGLIST_PAGES * VMM_PAGE_SIZE);
...
virtual_addr_t get_free_page_for_pagemap(struct vcpu_hw_context *context, physical_addr_t *page_phys)
{
...
int create_guest_shadow_map(
    struct vcpu_hw_context *context, virtual_addr_t vaddr, physical_addr_t paddr, size_t size, uint32_t pdprot, uint32_t pgprot)
{
    ...
    temp     = (union page32 *)((uint64_t)(pde_addr->paddr << PAGE_SHIFT));
...
int update_guest_shadow_pgprot(struct vcpu_hw_context *context, virtual_addr_t vaddr, uint32_t level, uint32_t pgprot)
{
    union page32    pte;
    union page32   *pde_addr, *temp;
...
int purge_guest_shadow_map(struct vcpu_hw_context *context, virtual_addr_t vaddr, size_t size)
{
    return VMM_OK;
}

int lookup_guest_pagetable(
    struct vcpu_hw_context *context, physical_addr_t fault_addr, physical_addr_t *lookedup_addr, union page32 *pde, union page32 *pte)
{
    union page32 pd, pt;
    ...
    ptindex = (((uint32_t)fault_addr) >> 12) & 0x3ff;
...
void invalidate_shadow_entry(struct vcpu_hw_context *context, virtual_addr_t invl_va)
{
    union page32    pde, pte;
    union page32   *pde_addr, *temp;
    physical_addr_t pte_addr;
...
void mark_guest_interrupt_pending(struct vcpu_hw_context *context, uint32_t intno)
{
    struct x86_vcpu_priv *vcpu_private = NULL;
    irq_flags_t           flags;

    vcpu_private = x86_vcpu_private(context->assoc_vcpu);

    vmm_spin_lock_irq_save(&vcpu_private->lock, flags);
    ...
    vmm_spin_unlock_irq_restore(&vcpu_private->lock, flags);
}

void inject_guest_interrupt(struct vcpu_hw_context *context, uint32_t intno)
{
    struct x86_vcpu_priv *vcpu_private = NULL;
    irq_flags_t           flags;
...
void inject_guest_exception(struct vcpu_hw_context *context, uint32_t exception)
{
    struct x86_vcpu_priv *vcpu_private = NULL;
    irq_flags_t           flags;
...
void invalidate_guest_tlb(struct vcpu_hw_context *context, uint32_t inval_va)
{
