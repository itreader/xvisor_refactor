 ...
 */
static uint32_t __unused __read_mostly ple_gap                   = 41;
static uint32_t __unused __read_mostly ple_window                = 4096;

static uint32_t vmx_basic_msr_low  __read_mostly;
static uint32_t vmx_basic_msr_high __read_mostly;
static uint32_t vmx_misc_msr_low   __read_mostly;
static uint32_t vmx_misc_msr_high  __read_mostly;

/* Dynamic (run-time adjusted) execution control flags. */
uint32_t vmx_pin_based_exec_control  __read_mostly;
uint32_t vmx_pin_based_exec_default1 __read_mostly;
uint32_t vmx_pin_based_exec_default0 __read_mostly;

uint32_t vmx_cpu_based_exec_control  __read_mostly;
uint32_t vmx_cpu_based_exec_default1 __read_mostly;
uint32_t vmx_cpu_based_exec_default0 __read_mostly;

uint32_t vmx_secondary_exec_control  __read_mostly;
uint32_t vmx_secondary_exec_default1 __read_mostly;
uint32_t vmx_secondary_exec_default0 __read_mostly;

uint32_t vmx_vmexit_control  __read_mostly;
uint32_t vmx_vmexit_default1 __read_mostly;
uint32_t vmx_vmexit_default0 __read_mostly;

uint32_t vmx_vmentry_control  __read_mostly;
uint32_t vmx_vmentry_default1 __read_mostly;
uint32_t vmx_vmentry_default0 __read_mostly;
uint32_t cpu_has_vmx_ept_2mb  __read_mostly;

uint64_t vmx_ept_vpid_cap               __read_mostly;
uint32_t vmx_on_size                    __read_mostly;
uint8_t cpu_has_vmx_ins_outs_instr_info __read_mostly;
uint32_t vmxon_region_size              __read_mostly;
uint32_t vmxon_region_nr_pages          __read_mostly;

uint32_t vmcs_revision_id __read_mostly;
...
{
    cpu_read_msr32(MSR_IA32_VMX_BASIC, &vmx_basic_msr_high, &vmx_basic_msr_low);

    cpu_read_msr32(MSR_IA32_VMX_MISC, &vmx_misc_msr_high, &vmx_misc_msr_low);
    ...
    vmxon_region_size = VMM_ROUNDUP2_PAGE_SIZE(vmx_basic_msr_high & 0x1ffful);
    ...
    if (!(vmx_basic_msr_high & (1u << 23))) {
        /* PIN BASED CONTROL */
        cpu_read_msr32(MSR_IA32_VMX_PINBASED_CTLS, &vmx_pin_based_exec_default1, &vmx_pin_based_exec_default0);

        /* PROCESSOR(CPU) BASED CONTROL */
        cpu_read_msr32(MSR_IA32_VMX_PROCBASED_CTLS, &vmx_cpu_based_exec_default1, &vmx_cpu_based_exec_default0);

        /* VMEXIT CONTROL */
        cpu_read_msr32(MSR_IA32_VMX_EXIT_CTLS, &vmx_vmexit_default1, &vmx_vmexit_default0);

        /* VMENTRY CONTROL */
        cpu_read_msr32(MSR_IA32_VMX_ENTRY_CTLS, &vmx_vmentry_default1, &vmx_vmentry_default0);
    } else { /* if the 55 bit is 1 */
        /* PIN BASED CONTROL */
        cpu_read_msr32(MSR_IA32_VMX_TRUE_PINBASED_CTLS, &vmx_pin_based_exec_default1, &vmx_pin_based_exec_default0);

        /* PROCESSOR(CPU) BASED CONTROL */
        cpu_read_msr32(MSR_IA32_VMX_TRUE_PROCBASED_CTLS, &vmx_cpu_based_exec_default1, &vmx_cpu_based_exec_default0);

        /* VMEXIT CONTROL */
        cpu_read_msr32(MSR_IA32_VMX_TRUE_EXIT_CTLS, &vmx_vmexit_default1, &vmx_vmexit_default0);

        /* VMENTRY CONTROL */
        cpu_read_msr32(MSR_IA32_VMX_TRUE_ENTRY_CTLS, &vmx_vmentry_default1, &vmx_vmentry_default0);
    }

    /* detect EPT and VPID capability */
    if (vmx_cpu_based_exec_default1 & CPU_BASED_ACTIVATE_SECONDARY_CONTROLS) {
        cpu_read_msr32(MSR_IA32_VMX_PROCBASED_CTLS2, &vmx_secondary_exec_default1, &vmx_secondary_exec_default0);

        /* The IA32_VMX_EPT_VPID_CAP MSR exists only when EPT or VPID available */
        if (vmx_secondary_exec_default1 & (SECONDARY_EXEC_ENABLE_EPT | SECONDARY_EXEC_ENABLE_VPID)) {
...
struct vmcs *alloc_vmcs(void)
{
    ...
    vmcs = (struct vmcs *)vmm_host_alloc_pages(
        1, /* number of pages */
        VMM_MEMORY_FLAGS_IO);
...
void *alloc_vmx_on_region(void)
{
    ...
    vmcs = (void *)vmm_host_alloc_pages(
        vmxon_region_nr_pages, /* number of pages */
        VMM_MEMORY_FLAGS_IO);
...
struct vmcs *current_vmcs(physical_addr_t *phys)
{
    ...
    if (!vmcs_phys || vmcs_phys == 0xFFFFFFFFFFFFFFFFULL) {
        X86_DEBUG_LOG(
            vmcs, LVL_ERR,
            "%s: There is not active(current) VMCS on this "
            "logical processor.\n",
            __func__);
        return NULL;
    }
...
void set_pin_based_exec_controls(void)
{
    uint32_t pin_controls[] = {
        PIN_BASED_EXT_INTR_MASK, PIN_BASED_NMI_EXITING, PIN_BASED_VIRTUAL_NMIS, PIN_BASED_PREEMPTION_TIMER, PIN_BASED_PROCESS_INTRS};
    uint32_t pin_controls_processed =
        (PIN_BASED_EXT_INTR_MASK | PIN_BASED_NMI_EXITING | PIN_BASED_VIRTUAL_NMIS | PIN_BASED_PREEMPTION_TIMER | PIN_BASED_PROCESS_INTRS);
    uint32_t vmx_pin_based_control = 0;
    uint32_t i                     = 0;
    uint32_t nr_pin_controls       = sizeof(pin_controls) / sizeof(uint32_t);

    /* Set the single settings */
    vmx_pin_based_control |= (vmx_pin_based_exec_default1 & vmx_pin_based_exec_default0);
    pin_controls_processed &= ~(vmx_pin_based_exec_default1 & vmx_pin_based_exec_default0);
...
{
    uint32_t proc_controls[] = {CPU_BASED_VIRTUAL_INTR_PENDING, CPU_BASED_USE_TSC_OFFSETING, CPU_BASED_HLT_EXITING,
                                CPU_BASED_INVLPG_EXITING,       CPU_BASED_MWAIT_EXITING,     CPU_BASED_RDPMC_EXITING,
                                CPU_BASED_RDTSC_EXITING,        CPU_BASED_CR3_LOAD_EXITING,  CPU_BASED_CR3_STORE_EXITING,
                                CPU_BASED_CR8_LOAD_EXITING,     CPU_BASED_CR8_STORE_EXITING, CPU_BASED_TPR_SHADOW,
                                CPU_BASED_VIRTUAL_NMI_PENDING,  CPU_BASED_MOV_DR_EXITING,    CPU_BASED_UNCOND_IO_EXITING,
                                CPU_BASED_ACTIVATE_IO_BITMAP,   CPU_BASED_MONITOR_TRAP_FLAG, CPU_BASED_ACTIVATE_MSR_BITMAP,
                                CPU_BASED_MONITOR_EXITING,      CPU_BASED_PAUSE_EXITING,     CPU_BASED_ACTIVATE_SECONDARY_CONTROLS};
    uint32_t proc_controls_processed =
        (CPU_BASED_VIRTUAL_INTR_PENDING | CPU_BASED_USE_TSC_OFFSETING | CPU_BASED_HLT_EXITING | CPU_BASED_INVLPG_EXITING | CPU_BASED_MWAIT_EXITING |
         CPU_BASED_RDPMC_EXITING | CPU_BASED_RDTSC_EXITING | CPU_BASED_CR3_LOAD_EXITING | CPU_BASED_CR3_STORE_EXITING | CPU_BASED_CR8_LOAD_EXITING |
         CPU_BASED_CR8_STORE_EXITING | CPU_BASED_TPR_SHADOW | CPU_BASED_VIRTUAL_NMI_PENDING | CPU_BASED_MOV_DR_EXITING | CPU_BASED_UNCOND_IO_EXITING |
         CPU_BASED_ACTIVATE_IO_BITMAP | CPU_BASED_MONITOR_TRAP_FLAG | CPU_BASED_ACTIVATE_MSR_BITMAP | CPU_BASED_MONITOR_EXITING |
         CPU_BASED_PAUSE_EXITING | CPU_BASED_ACTIVATE_SECONDARY_CONTROLS);
    uint32_t vmx_proc_based_control     = 0;
    uint32_t vmx_proc_secondary_control = 0;
    uint32_t i                          = 0;
    uint32_t nr_proc_controls           = sizeof(proc_controls) / sizeof(uint32_t);

    /* Set the single settings */
    vmx_proc_based_control |= (vmx_cpu_based_exec_default1 & vmx_cpu_based_exec_default0);
    proc_controls_processed &= ~(vmx_cpu_based_exec_default1 & vmx_cpu_based_exec_default0);
...
void set_vmx_entry_exec_controls(void)
{
    ...
    uint32_t entry_controls_processed =
        (VM_ENTRY_LOAD_DEBUG_CONTROLS | VM_ENTRY_IA32E_MODE | VM_ENTRY_SMM | VM_ENTRY_DEACT_DUAL_MONITOR | VM_ENTRY_LOAD_PERF_GLOBAL_CTRL |
         VM_ENTRY_LOAD_GUEST_PAT | VM_ENTRY_LOAD_GUEST_EFER | VM_ENTRY_LOAD_GUEST_BNDCFGS | VM_ENTRY_CONCEAL_VMX_PT);
    uint32_t vmx_entry_control = 0;
    uint32_t i                 = 0;
    uint32_t nr_entry_controls = sizeof(entry_controls) / sizeof(uint32_t);

    /* Set the single settings */
    vmx_entry_control |= (vmx_vmentry_default1 & vmx_vmentry_default0);
    entry_controls_processed &= ~(vmx_vmentry_default1 & vmx_vmentry_default0);
...
{
    uint32_t exit_controls[] = {VM_EXIT_SAVE_DBG_CTRL,      VM_EXIT_IA32E_MODE,    VM_EXIT_LOAD_PERF_GLOBAL_CTRL, VM_EXIT_ACK_INTR_ON_EXIT,
                                VM_EXIT_SAVE_GUEST_PAT,     VM_EXIT_LOAD_HOST_PAT, VM_EXIT_SAVE_GUEST_EFER,       VM_EXIT_LOAD_HOST_EFER,
                                VM_EXIT_SAVE_PREEMPT_TIMER, VM_EXIT_CLEAR_BNDCFGS, VM_EXIT_CONCEAL_VMX_PT};
    uint32_t exit_controls_processed =
        (VM_EXIT_SAVE_DBG_CTRL | VM_EXIT_IA32E_MODE | VM_EXIT_LOAD_PERF_GLOBAL_CTRL | VM_EXIT_ACK_INTR_ON_EXIT | VM_EXIT_SAVE_GUEST_PAT |
         VM_EXIT_LOAD_HOST_PAT | VM_EXIT_SAVE_GUEST_EFER | VM_EXIT_LOAD_HOST_EFER | VM_EXIT_SAVE_PREEMPT_TIMER | VM_EXIT_CLEAR_BNDCFGS |
         VM_EXIT_CONCEAL_VMX_PT);
    uint32_t vmx_exit_control = 0;
    uint32_t i                = 0;
    uint32_t nr_exit_controls = sizeof(exit_controls) / sizeof(uint32_t);

    /* Set the single settings */
    vmx_exit_control |= (vmx_vmexit_default1 & vmx_vmexit_default0);
    exit_controls_processed &= ~(vmx_vmexit_default1 & vmx_vmexit_default0);
...
{
    int      rc      = VMM_OK;
    uint32_t vcpu_id = context->assoc_vcpu->subid = 1;
    ...
    /* A and B - 4K each */
    context->icept_table.io_table_phys = cpu_create_vcpu_intercept_table(VMM_SIZE_TO_PAGE(8 << 10), &context->icept_table.io_table_virt);
    ...
    context->icept_table.msr_table_phys = cpu_create_vcpu_intercept_table(VMM_SIZE_TO_PAGE(4 << 10), &context->icept_table.msr_table_virt);
...
void vmx_disable_intercept_for_msr(struct vcpu_hw_context *context, uint32_t msr)
{
    uint64_t *msr_bitmap = (uint64_t *)context->icept_table.msr_table_virt;
...
int vmx_read_guest_msr(struct vcpu_hw_context *context, uint32_t msr, uint64_t *val)
{
    uint32_t                    i, msr_count = context->msr_count;
...
int vmx_write_guest_msr(struct vcpu_hw_context *context, uint32_t msr, uint64_t val)
{
    uint32_t              i, msr_count = context->msr_count;
...
int vmx_add_guest_msr(struct vcpu_hw_context *context, uint32_t msr)
{
    uint32_t              i, msr_count = context->msr_count;
    struct vmx_msr_entry *msr_area = context->msr_area;
    ...
    if (msr_area == NULL) {
        ...
        __vmwrite(VM_EXIT_MSR_STORE_ADDR, (uint64_t)msr_area);
        __vmwrite(VM_ENTRY_MSR_LOAD_ADDR, (uint64_t)msr_area);
    }
...
int vmx_add_host_load_msr(struct vcpu_hw_context *context, uint32_t msr)
{
    uint32_t              i, msr_count = context->host_msr_count;
    ...
    if (msr_area == NULL) {
        ...
        __vmwrite(VM_EXIT_MSR_LOAD_ADDR, (uint64_t)msr_area);
    }
...
static void __unused vmx_dump_sel(char *name, uint32_t selector)
{
    ...
    vmm_printf("%s: sel=0x%" PRIx32 ", attr=0x%" PRIx32 ", limit=0x%" PRIx32 ", base=0x%" PRIx64 "\n", name, sel, attr, limit, base);
}

static void __unused vmx_dump_sel2(char *name, uint32_t lim)
{
    uint32_t limit;
    uint64_t base;
    limit = vmr(lim);
...
void vmcs_dump(struct vcpu_hw_context *context)
{
    ...
    vmm_printf(
        "CR0: actual=0x%016llx, shadow=0x%016llx, gh_mask=%016llx\n", (uint64_t)vmr(GUEST_CR0), (uint64_t)vmr(CR0_READ_SHADOW),
        (uint64_t)vmr(CR0_GUEST_HOST_MASK));
    vmm_printf(
        "CR4: actual=0x%016llx, shadow=0x%016llx, gh_mask=%016llx\n", (uint64_t)vmr(GUEST_CR4), (uint64_t)vmr(CR4_READ_SHADOW),
        (uint64_t)vmr(CR4_GUEST_HOST_MASK));
    vmm_printf("CR3: actual=0x%016llx, target_count=%d\n", (uint64_t)vmr(GUEST_CR3), (int)vmr(CR3_TARGET_COUNT));
    vmm_printf("     target0=%016llx, target1=%016llx\n", (uint64_t)vmr(CR3_TARGET_VALUE0), (uint64_t)vmr(CR3_TARGET_VALUE1));
    vmm_printf("     target2=%016llx, target3=%016llx\n", (uint64_t)vmr(CR3_TARGET_VALUE2), (uint64_t)vmr(CR3_TARGET_VALUE3));
    vmm_printf("RSP = 0x%016llx, RIP = 0x%016llx\n", (uint64_t)vmr(GUEST_RSP), (uint64_t)vmr(GUEST_RIP));
    vmm_printf("RFLAGS=0x%016llx DR7 = 0x%016llx\n", (uint64_t)vmr(GUEST_RFLAGS), (uint64_t)vmr(GUEST_DR7));
    vmm_printf(
        "Sysenter RSP=%016llx CS:RIP=%04x:%016llx\n", (uint64_t)vmr(GUEST_SYSENTER_ESP), (int)vmr(GUEST_SYSENTER_CS),
        (uint64_t)vmr(GUEST_SYSENTER_EIP));
    ...
    vmm_printf("Fixed CR0: Fixed0:%llx Fixed1: %llx\n", (uint64_t)vmx_cr0_fixed0, (uint64_t)vmx_cr0_fixed1);
    vmm_printf("Fixed CR4: Fixed0:%llx Fixed1: %llx\n", (uint64_t)vmx_cr4_fixed0, (uint64_t)vmx_cr4_fixed1);
    vmm_printf("Exit Reason=%08x Exit Qualification=%08x\n", (uint32_t)vmr(VM_EXIT_REASON), (uint32_t)vmr(EXIT_QUALIFICATION));
    vmm_printf("Guest PAT = 0x%08x%08x\n", (uint32_t)vmr(GUEST_IA32_PAT_HIGH), (uint32_t)vmr(GUEST_IA32_PAT));
    x = (uint64_t)vmr(TSC_OFFSET_HIGH) << 32;
    x |= (uint32_t)vmr(TSC_OFFSET);
    vmm_printf("TSC Offset = %016llx\n", x);
    x = (uint64_t)vmr(GUEST_IA32_DEBUGCTL_HIGH) << 32;
    x |= (uint32_t)vmr(GUEST_IA32_DEBUGCTL);
    vmm_printf("DebugCtl=%016llx DebugExceptions=%016llx\n", x, (uint64_t)vmr(GUEST_PENDING_DBG_EXCEPTIONS));
    vmm_printf("Interruptibility=%04x ActivityState=%04x\n", (int)vmr(GUEST_INTERRUPTIBILITY_INFO), (int)vmr(GUEST_ACTIVITY_STATE));
    vmm_printf("VMENTRY Controls: 0x%08x, VMEXIT Controls: 0x%08x\n", (uint32_t)vmr(VM_ENTRY_CONTROLS), (uint32_t)vmr(VM_EXIT_CONTROLS));
    vmm_printf("*** Host State ***\n");
    vmm_printf("RSP = 0x%016llx  RIP = 0x%016llx\n", (uint64_t)vmr(HOST_RSP), (uint64_t)vmr(HOST_RIP));
    vmm_printf(
        "CS=%04x DS=%04x ES=%04x FS=%04x GS=%04x SS=%04x TR=%04x\n", (uint16_t)vmr(HOST_CS_SELECTOR), (uint16_t)vmr(HOST_DS_SELECTOR),
        (uint16_t)vmr(HOST_ES_SELECTOR), (uint16_t)vmr(HOST_FS_SELECTOR), (uint16_t)vmr(HOST_GS_SELECTOR), (uint16_t)vmr(HOST_SS_SELECTOR),
        (uint16_t)vmr(HOST_TR_SELECTOR));
    vmm_printf(
        "FSBase=%016llx GSBase=%016llx TRBase=%016llx\n", (uint64_t)vmr(HOST_FS_BASE), (uint64_t)vmr(HOST_GS_BASE), (uint64_t)vmr(HOST_TR_BASE));
    vmm_printf("GDTBase=%016llx IDTBase=%016llx\n", (uint64_t)vmr(HOST_GDTR_BASE), (uint64_t)vmr(HOST_IDTR_BASE));
    vmm_printf("CR0=%016llx CR3=%016llx CR4=%016llx\n", (uint64_t)vmr(HOST_CR0), (uint64_t)vmr(HOST_CR3), (uint64_t)vmr(HOST_CR4));
    vmm_printf(
        "Sysenter RSP=%016llx CS:RIP=%04x:%016llx\n", (uint64_t)vmr(HOST_IA32_SYSENTER_ESP), (int)vmr(HOST_IA32_SYSENTER_CS),
        (uint64_t)vmr(HOST_IA32_SYSENTER_EIP));
    vmm_printf("Host PAT = 0x%08x%08x\n", (uint32_t)vmr(HOST_IA32_PAT_HIGH), (uint32_t)vmr(HOST_IA32_PAT));

    vmm_printf("*** Control State ***\n");
    vmm_printf(
        "PinBased=%08x CPUBased=%08x SecondaryExec=%08x\n", (uint32_t)vmr(PIN_BASED_VM_EXEC_CONTROL), (uint32_t)vmr(CPU_BASED_VM_EXEC_CONTROL),
    ...
    vmm_printf(
        "VMEntry: intr_info=%08x errcode=%08x ilen=%08x\n", (uint32_t)vmr(VM_ENTRY_INTR_INFO_FIELD), (uint32_t)vmr(VM_ENTRY_EXCEPTION_ERROR_CODE),
        (uint32_t)vmr(VM_ENTRY_INSTRUCTION_LEN));
    vmm_printf(
        "VMExit: intr_info=%08x errcode=%08x ilen=%08x\n", (uint32_t)vmr(VM_EXIT_INTR_INFO), (uint32_t)vmr(VM_EXIT_INTR_ERROR_CODE),
        (uint32_t)vmr(VM_ENTRY_INSTRUCTION_LEN));
    vmm_printf("        reason=%08x qualification=%08x\n", (uint32_t)vmr(VM_EXIT_REASON), (uint32_t)vmr(EXIT_QUALIFICATION));
    vmm_printf("IDTVectoring: info=%08x errcode=%08x\n", (uint32_t)vmr(IDT_VECTORING_INFO_FIELD), (uint32_t)vmr(IDT_VECTORING_ERROR_CODE));
