...
extern uint32_t     vmxon_region_nr_pages;
extern uint32_t     vmcs_revision_id;
...
static int __vmcs_run(struct vcpu_hw_context *context, bool resume)
{
    ...
    __asm__ __volatile__(
        ...
        "7:sti\n\t"
        :
        : [resume] "m"(resume), "d"((uint64_t)HOST_RSP), [context] "c"(context),
          "b"((uint64_t)HOST_RIP), [rax] "i"(offsetof(struct vcpu_hw_context, g_regs[GUEST_REGS_RAX])),
