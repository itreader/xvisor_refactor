...
typedef struct ept_trace_entry {
    uint64_t g_phys;
    uint64_t h_phys;
    uint64_t pml4_index : 10;
    uint64_t pdpt_index : 10;
    uint64_t pde_index  : 10;
    uint64_t pt_index   : 10;
    uint64_t pg_sz      : 24;
    uint32_t pg_prot;
} ept_trace_entry_t;
...
static void add_ept_trace_point(
    uint64_t gphys, uint64_t hphys, uint32_t pml4, uint32_t pdpt, uint32_t pde, uint32_t pt, uint32_t size, uint32_t pg_prot)
{
...
static void add_ept_trace_point(uint64_t gphys, uint64_t hphys, uint32_t pml4, uint32_t pdpt, uint32_t pde, uint32_t pg_prot) {}
#endif /* EPT Trace */

static inline uint32_t ept_pml4_index(physical_addr_t gphys)
{
    gphys &= PHYS_ADDR_BIT_MASK;
    return ((gphys >> 39) & 0x1fful);
}

static inline uint32_t ept_pdpt_index(physical_addr_t gphys)
{
    gphys &= PHYS_ADDR_BIT_MASK;
    return ((gphys >> 30) & 0x1fful);
}

static inline uint32_t ept_pd_index(physical_addr_t gphys)
{
    gphys &= PHYS_ADDR_BIT_MASK;
    return ((gphys >> 21) & 0x1fful);
}

static inline uint32_t ept_pt_index(physical_addr_t gphys)
{
    gphys &= PHYS_ADDR_BIT_MASK;
    return ((gphys >> 12) & 0x1fful);
}

static inline void invalidate_ept(int type, struct invept_desc *desc)
{
    ...
    if (likely(cpu_has_vmx_invept)) {
        ...
        if (unlikely(type == INVEPT_ALL_CONTEXT && !cpu_has_vmx_ept_invept_all_context)) {
            X86_DEBUG_LOG(ept, LVL_DEBUG, "EPT all context flush not supported\n");
            return;
        }

        if (unlikely(type == INVEPT_SINGLE_CONTEXT && !cpu_has_vmx_ept_invept_single_context)) {
            X86_DEBUG_LOG(ept, LVL_DEBUG, "EPT single context flush not supported\n");
            return;
        }

        asm volatile("invept (%0), %1\n\t" ::"D"(type), "S"(desc) : "memory", "cc");
    } else {
...
static void decode_ept_entry(enum ept_level level, void *entry, physical_addr_t *paddr, uint32_t *pg_prot)
{
    ept_pml4e_t *pml4e;
    ept_pdpte_t *pdpte;
    ept_pde_t   *pde;
...
{
    uint32_t           pml4_index = ept_pml4_index(gphys);
    uint32_t           pdpt_index = ept_pdpt_index(gphys);
    uint32_t           pd_index   = ept_pd_index(gphys);
    uint32_t           pt_index   = ept_pt_index(gphys);
    ept_pml4e_t       *pml4e;
    ept_pdpte_t       *pdpte;
    ept_pde_t         *pde;
    ept_pte_t         *pte;
    uint64_t          *pml4 = (uint64_t *)context->n_cr3;
    ...
    X86_DEBUG_LOG(
        ept, LVL_DEBUG, "pml4: 0x%" PRIx32 " pdpt: 0x%" PRIx32 " pd: 0x%" PRIx32 " pt: 0x%" PRIx32 "\n", pml4_index, pdpt_index, pd_index, pt_index);
    ...
    pdpte                     = (ept_pdpte_t *)(&((uint64_t *)virt)[pdpt_index]);
    ...
    phys = e_phys = e_pg_prot = 0;
    pde                       = (ept_pde_t *)(&((uint64_t *)virt)[pd_index]);
    ...
    e_phys = e_pg_prot = 0;
    pte                = (ept_pte_t *)(&((uint64_t *)virt)[pt_index]);
...
int setup_ept(struct vcpu_hw_context *context)
{
    ...
    eptp->val = 0;
    eptp->bits.mt =
        (vmx_ept_vpid_cap & (0x01UL << 8)     ? 0 /* UC */
