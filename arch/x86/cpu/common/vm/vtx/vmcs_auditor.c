...
BxExceptionInfo exceptions_info[32] = {
    ...
    {BX_ET_BENIGN,       BX_EXCEPTION_CLASS_FAULT, 0},
    /* BP */
    {BX_ET_BENIGN,       BX_EXCEPTION_CLASS_TRAP,  0},
    /* OF */
    {BX_ET_BENIGN,       BX_EXCEPTION_CLASS_TRAP,  0},
    /* BR */
    {BX_ET_BENIGN,       BX_EXCEPTION_CLASS_FAULT, 0},
    ...
    {BX_ET_BENIGN,       BX_EXCEPTION_CLASS_FAULT, 0},
    /* DF */
    {BX_ET_DOUBLE_FAULT, BX_EXCEPTION_CLASS_FAULT, 1},
    /* 09 */
    {BX_ET_BENIGN,       BX_EXCEPTION_CLASS_FAULT, 0},
    /* TS */
    {BX_ET_CONTRIBUTORY, BX_EXCEPTION_CLASS_FAULT, 1},
    /* NP */
    {BX_ET_CONTRIBUTORY, BX_EXCEPTION_CLASS_FAULT, 1},
    /* SS */
    {BX_ET_CONTRIBUTORY, BX_EXCEPTION_CLASS_FAULT, 1},
    ...
    {BX_ET_CONTRIBUTORY, BX_EXCEPTION_CLASS_FAULT, 1},
    /* PF */
    {BX_ET_PAGE_FAULT,   BX_EXCEPTION_CLASS_FAULT, 1},
    /* 15 */
    {BX_ET_BENIGN,       BX_EXCEPTION_CLASS_FAULT, 0},
    /* MF */
    {BX_ET_BENIGN,       BX_EXCEPTION_CLASS_FAULT, 0},
    /* AC */
    {BX_ET_BENIGN,       BX_EXCEPTION_CLASS_FAULT, 1},
    /* MC */
    {BX_ET_BENIGN,       BX_EXCEPTION_CLASS_ABORT, 0},
    /* XM */
    {BX_ET_BENIGN,       BX_EXCEPTION_CLASS_FAULT, 0},
    /* VE */
    {BX_ET_PAGE_FAULT,   BX_EXCEPTION_CLASS_FAULT, 0},
    /* 21 */
    {BX_ET_BENIGN,       BX_EXCEPTION_CLASS_FAULT, 0},
    /* 22 */
    {BX_ET_BENIGN,       BX_EXCEPTION_CLASS_FAULT, 0},
...
int audit_vmcs(BOOLEAN IsVMResume, UINT64 RevisionID, UINT64 VMXON_Pointer)
{
    ...
    return CheckVMXState(
        vm, IsVMResume, VMXON_Pointer, RevisionID, _vmx_pin_vmexec_ctrl_supported_bits, _vmx_proc_vmexec_ctrl_supported_bits,
        _vmx_vmexec_ctrl2_supported_bits, _vmx_vmexit_ctrl_supported_bits, _vmx_vmentry_ctrl_supported_bits, _vmx_ept_vpid_cap_supported_bits,
        _vmx_vmfunc_supported_bits, _cr0_suppmask_0, _cr0_suppmask_1, _cr4_suppmask_0, _cr4_suppmask_1);
}
...
void init_vmx_extensions_bitmask()
{
    ...
    if (x86_64_enabled) {
        features_bitmask |= BX_VMX_TPR_SHADOW | BX_VMX_APIC_VIRTUALIZATION | BX_VMX_WBINVD_VMEXIT;

#if BX_SUPPORT_VMX >= 2
        features_bitmask |= BX_VMX_PREEMPTION_TIMER | BX_VMX_PAT | BX_VMX_EFER | BX_VMX_EPT | BX_VMX_VPID | BX_VMX_UNRESTRICTED_GUEST |
                            BX_VMX_DESCRIPTOR_TABLE_EXIT | BX_VMX_X2APIC_VIRTUALIZATION | BX_VMX_PAUSE_LOOP_EXITING | BX_VMX_EPT_ACCESS_DIRTY |
                            BX_VMX_VINTR_DELIVERY | BX_VMX_VMCS_SHADOWING | BX_VMX_EPTP_SWITCHING | BX_VMX_EPT_EXCEPTION;
...
bx_bool set_segment_ar_data(bx_segment_reg_t *seg, bx_bool valid, Bit16u raw_selector, bx_address base, Bit32u limit_scaled, Bit16u ar_data)
{
    ...
    vmm_printf(
        "%s: AR Data: 0x%x Present: %d DPL: %d Segment: %d Type: %d Valid %d\n", __func__, ar_data, d->p, d->dpl, d->segment, d->type, d->valid);
...
VMX_error_code VMenterLoadCheckVmControls(VMCS_CACHE *vm)
{
    ...
    vm->vmexec_ctrls2 = __vmread_safe(VMCS_32BIT_CONTROL_PROCESSOR_BASED_VMEXEC_CONTROLS, &error);

    if (vm->vmexec_ctrls2 & (VMX_VM_EXEC_CTRL2_SECONDARY_CONTROLS)) {
        vm->vmexec_ctrls3 = __vmread_safe(VMCS_32BIT_CONTROL_SECONDARY_VMEXEC_CONTROLS, &error);

    } else {
        vm->vmexec_ctrls3 = 0;
    }

    vm->vm_exceptions_bitmap = __vmread_safe(VMCS_32BIT_CONTROL_EXECUTION_BITMAP, &error);
    vm->vm_pf_mask           = __vmread_safe(VMCS_32BIT_CONTROL_PAGE_FAULT_ERR_CODE_MASK, &error);
    vm->vm_pf_match          = __vmread_safe(VMCS_32BIT_CONTROL_PAGE_FAULT_ERR_CODE_MATCH, &error);
    ...
    vm->vm_cr3_target_cnt    = __vmread_safe(VMCS_32BIT_CONTROL_CR3_TARGET_COUNT, &error);

    for (int n = 0; n < VMX_CR3_TARGET_MAX_CNT; n++) {
        if (n == 0) {
            vm->vm_cr3_target_value[n] = __vmread_safe(VMCS_CR3_TARGET0 + 2 * n, &error);
        }

        if (n == 1) {
            vm->vm_cr3_target_value[n] = __vmread_safe(VMCS_CR3_TARGET1 + 2 * n, &error);
        }

        if (n == 2) {
            vm->vm_cr3_target_value[n] = __vmread_safe(VMCS_CR3_TARGET2 + 2 * n, &error);
        }

        if (n == 3) {
            vm->vm_cr3_target_value[n] = __vmread_safe(VMCS_CR3_TARGET3, &error);
        }
    }
     ...
     */
    extern uint32_t vmx_pin_based_exec_default1;
    extern uint32_t vmx_pin_based_exec_default0;
    extern uint32_t vmx_cpu_based_exec_default0;
    extern uint32_t vmx_cpu_based_exec_default1;
    extern uint32_t vmx_secondary_exec_default0;
    extern uint32_t vmx_secondary_exec_default1;
    ...
    if (vm->vmexec_ctrls1 & ~vmx_pin_based_exec_default1) {
        vmm_printf(
            "\nVMFAIL: VMCS EXEC CTRL: VMX pin-based controls allowed 1-settings(CTRL: 0x%08x DEF: 0x%08x)", vm->vmexec_ctrls1,
    ...
    if (vm->vmexec_ctrls2 & VMX_VM_EXEC_CTRL2_IO_BITMAPS) {
        vm->io_bitmap_addr[0] = __vmread_safe(VMCS_64BIT_CONTROL_IO_BITMAP_A, &error);
        vm->io_bitmap_addr[1] = __vmread_safe(VMCS_64BIT_CONTROL_IO_BITMAP_B, &error);
    ...
    if (vm->vmexec_ctrls2 & VMX_VM_EXEC_CTRL2_MSR_BITMAPS) {
        ...
        vm->msr_bitmap_addr = __vmread_safe(VMCS_64BIT_CONTROL_MSR_BITMAPS, &error);
    ...
    if (vm->vmexec_ctrls2 & VMX_VM_EXEC_CTRL2_TPR_SHADOW) {
        ...
        if (vm->vmexec_ctrls3 &
            (VMX_VM_EXEC_CTRL3_VIRTUALIZE_X2APIC_MODE | VMX_VM_EXEC_CTRL3_VIRTUALIZE_APIC_REGISTERS | VMX_VM_EXEC_CTRL3_VIRTUAL_INT_DELIVERY)) {
     ...
     */
    extern uint32_t vmx_vmexit_default1;
    extern uint32_t vmx_vmexit_default0;
     ...
     */
    extern uint32_t vmx_vmentry_default1;
    extern uint32_t vmx_vmentry_default0;
...
Bit32u VMenterLoadCheckGuestState(VMCS_CACHE *vm, Bit64u *qualification, UINT64 VMXON_Pointer, INT32 RevisionID)
{
    ...
    *qualification = VMENTER_ERR_NO_ERROR;
    ...
    for (n = 0; n < 6; n++) {
        ...
        set_segment_ar_data(&guest.sregs[n], !invalid, (Bit16u)selector, base, limit, (Bit16u)ar);
    ...
    if (set_segment_ar_data(&guest.ldtr, !ldtr_invalid, (Bit16u)ldtr_selector, ldtr_base, ldtr_limit, (Bit16u)(ldtr_ar))) {
        // ldtr is valid
    ...
    set_segment_ar_data(&guest.tr, !tr_invalid, (Bit16u)tr_selector, tr_base, tr_limit, (Bit16u)(tr_ar));
    ...
    guest.sysenter_cs_msr   = __vmread_safe(VMCS_32BIT_GUEST_IA32_SYSENTER_CS_MSR, &error);
    ...
    if ((guest.interruptibility_state & BX_VMX_INTERRUPTS_BLOCKED_BY_STI) && (guest.interruptibility_state & BX_VMX_INTERRUPTS_BLOCKED_BY_MOV_SS)) {
        vmm_printf("\nVMENTER FAIL: VMCS guest interruptibility state broken");
        return VMX_VMEXIT_VMENTRY_FAILURE_GUEST_STATE;
    }
