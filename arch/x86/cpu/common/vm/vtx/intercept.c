...
static inline int vmx_handle_guest_realmode_page_fault(struct vcpu_hw_context *context)
{
    int               rc;
    uint32_t          flags;
     ...
     */
    rc = vmm_guest_physical_map(guest, (gla & PAGE_MASK), PAGE_SIZE, &hphys_addr, &availsz, &flags);
...
static inline int vmx_handle_guest_protected_mode_page_fault(struct vcpu_hw_context *context)
{
    ...
    rc = vmm_guest_physical_map(guest, (fault_gphys & PAGE_MASK), PAGE_SIZE, &hphys_addr, &availsz, &flags);
    ...
    return ept_create_pte_map(context, fault_gphys, hphys_addr, PAGE_SIZE, (EPT_PROT_READ | EPT_PROT_WRITE | EPT_PROT_EXEC_S));
}

static inline int guest_in_real_mode(struct vcpu_hw_context *context)
{
...
void vmx_handle_cpuid(struct vcpu_hw_context *context)
{
    ...
    switch (context->g_regs[GUEST_REGS_RAX]) {
        case CPUID_BASE_LFUNCSTD:
            ...
            X86_DEBUG_LOG(
                vtx_intercept, LVL_DEBUG, "RAX: 0x%" PRIx32 " RBX: 0x%" PRIx32 " RCX: 0x%" PRIx32 " RDX: 0x%" PRIx32 "\n", func->resp_eax,
                func->resp_ebx, func->resp_ecx, func->resp_edx);
            break;
        ...
        case CPUID_BASE_FEATURES:
            ...
            context->g_regs[GUEST_REGS_RDX] = func->resp_edx;
            X86_DEBUG_LOG(
                vtx_intercept, LVL_DEBUG, "CPUID: 0x%" PRIx64 " RAX: 0x%" PRIx32 " RBX: 0x%" PRIx32 " RCX: 0x%" PRIx32 " RDX: 0x%" PRIx32 "\n",
        ...
        case CPUID_EXTENDED_ADDR_BITS:
            func = &private->extended_funcs[context->g_regs[GUEST_REGS_RAX] - CPUID_EXTENDED_LFUNCEXTD];
            X86_DEBUG_LOG(
                vtx_intercept, LVL_DEBUG, "CPUID: 0x%" PRIx64 ": EAX: 0x%" PRIx32 " EBX: 0x%" PRIx32 " ECX: 0x%" PRIx32 " EDX: 0x%" PRIx32 "\n",
                context->g_regs[GUEST_REGS_RAX], func->resp_eax, func->resp_ebx, func->resp_ecx, func->resp_edx);
            context->g_regs[GUEST_REGS_RAX] = func->resp_eax;
            context->g_regs[GUEST_REGS_RBX] = func->resp_ebx;
            context->g_regs[GUEST_REGS_RCX] = func->resp_ecx;
...
static inline int vmx_handle_io_instruction_exit(struct vcpu_hw_context *context)
{
    ...
    if (ioe.bits.direction == 0) {
        if (ioe.bits.port == 0x80) {
            ...
            wval = (uint32_t)context->g_regs[GUEST_REGS_RAX];

            if (vmm_device_emulate_emulate_iowrite(context->assoc_vcpu, ioe.bits.port, &wval, io_sz, VMM_DEVICE_EMULATE_NATIVE_ENDIAN) != VMM_OK) {
                X86_DEBUG_LOG(
                    vtx_intercept, LVL_ERR,
                    "Failed to emulate OUT instruction in"
                    " guest.\n");
                goto guest_bad_fault;
            }
        }
        ...
        if (vmm_device_emulate_emulate_ioread(context->assoc_vcpu, ioe.bits.port, &wval, io_sz, VMM_DEVICE_EMULATE_NATIVE_ENDIAN) != VMM_OK) {
            X86_DEBUG_LOG(
                vtx_intercept, LVL_ERR,
...
static inline int vmx_handle_crx_exit(struct vcpu_hw_context *context)
{
    ...
    if (crx_eq.bits.type == 0) {
        switch (crx_eq.bits.cr_num) {
            case 0:
                gcr0 = GUEST_CRx_FILTER(0, (context->g_regs[crx_eq.bits.reg] | X86_CR0_ET | X86_CR0_CD | X86_CR0_NW));
                gcr0 &= ~X86_CR0_PG;
                __vmwrite(GUEST_CR0, gcr0);
                VMX_GUEST_CR0(context) = (X86_CR0_ET | X86_CR0_CD | X86_CR0_NW | context->g_regs[crx_eq.bits.reg]);
            ...
            case 3:
                ...
                X86_DEBUG_LOG(
                    vtx_intercept, LVL_DEBUG, "Moving %d register (value: 0x%" PRIx64 ") to CR3\n", crx_eq.bits.reg,
            ...
            case 4:
                ...
                X86_DEBUG_LOG(
                    vtx_intercept, LVL_DEBUG, "Moving %d register (value: 0x%" PRIx64 ") to CR4\n", crx_eq.bits.reg,
                    context->g_regs[crx_eq.bits.reg]);
                break;
...
int vmx_handle_vmexit(struct vcpu_hw_context *context, uint32_t exit_reason)
{
    switch (exit_reason) {
