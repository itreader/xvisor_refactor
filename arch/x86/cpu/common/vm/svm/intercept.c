...
static int guest_read_gva(struct vcpu_hw_context *context, uint32_t vaddr, uint8_t *where, uint32_t size)
{
    physical_addr_t gphys;

    if (gva_to_gpa(context, vaddr, &gphys)) {
        X86_DEBUG_LOG(
            svm_intercept, LVL_ERR,
...
static int guest_read_fault_inst(struct vcpu_hw_context *context, x86_inst *g_ins)
{
    ...
    /* FIXME: Should we always do cacheable memory access here ?? */
    if (vmm_guest_memory_read(guest, rip_phys, g_ins, sizeof(x86_inst), TRUE) < sizeof(x86_inst)) {
        X86_DEBUG_LOG(
            svm_intercept, LVL_ERR,
 ...
 */
static inline void handle_guest_resident_page_fault(struct vcpu_hw_context *context)
{
    physical_addr_t fault_gphys = context->vmcb->exitinfo2;
    physical_addr_t lookedup_gphys;
    union page32    pte, pte1, pde, pde1;
    uint32_t        prot, prot1, pdprot, pdprot1;

    X86_DEBUG_LOG(
        svm_intercept, LVL_DEBUG,
        "Resident page fault exit info 1: 0x%lx "
        "2: 0x%lx rip: 0x%lx\n",
        context->vmcb->exitinfo1, context->vmcb->exitinfo2, context->vmcb->rip);

    if (lookup_guest_pagetable(context, fault_gphys, &lookedup_gphys, &pde, &pte)) {
        /* Lazy TLB flush by guest? */
        X86_DEBUG_LOG(
            svm_intercept, LVL_ERR,
            "ERROR: No entry in guest page table in "
            "protection fault! Arrrgh! (Guest virtual: 0x%lx)\n",
            fault_gphys);
        goto guest_bad_fault;
    }

    if (lookup_shadow_pagetable(context, fault_gphys, &lookedup_gphys, &pde1, &pte1)) {
        X86_DEBUG_LOG(
            svm_intercept, LVL_ERR,
            "ERROR: No entry in shadow page table? "
    ...
    if (pdprot != pdprot1) {
        if (update_guest_shadow_pgprot(context, fault_gphys, GUEST_PG_LVL_1, pdprot) != VMM_OK) {
            X86_DEBUG_LOG(
                svm_intercept, LVL_ERR,
                "ERROR: Could not update level 2 "
    ...
    if (prot != prot1) {
        if (update_guest_shadow_pgprot(context, fault_gphys, GUEST_PG_LVL_2, prot) != VMM_OK) {
            X86_DEBUG_LOG(
                svm_intercept, LVL_ERR,
                "ERROR: Could not update level 1 "
...
static inline void handle_guest_realmode_page_fault(struct vcpu_hw_context *context, physical_addr_t fault_gphys, physical_size_t hphys_addr)
{
    if (create_guest_shadow_map(context, fault_gphys, hphys_addr, PAGE_SIZE, 0x3, 0x3) != VMM_OK) {
        X86_DEBUG_LOG(
            svm_intercept, LVL_ERR,
            "ERROR: Failed to create map in"
...
static inline void emulate_guest_mmio_io(struct vcpu_hw_context *context, x86_decoded_inst_t *dinst)
{
    uint64_t        guestrd;
    uint64_t        index;
    physical_addr_t fault_gphys = context->vmcb->exitinfo2;

    if (gva_to_gpa(context, dinst->inst.gen_mov.src_addr, (physical_addr_t *)&dinst->inst.gen_mov.src_addr) != VMM_OK) {
    ...
    if (dinst->inst.gen_mov.dst_addr >= RM_REG_AX && dinst->inst.gen_mov.dst_addr < RM_REG_MAX) {
        ...
        X86_DEBUG_LOG(
            svm_intercept, LVL_ERR,
            "Memory to memory move instruction not "
            "supported.\n");
        goto guest_bad_fault;
    }
    ...
    } else if (dinst->inst.gen_mov.src_addr >= RM_REG_AX && dinst->inst.gen_mov.src_addr < RM_REG_MAX) {
        if (dinst->inst.gen_mov.dst_addr == RM_REG_AX) {
...
static inline void handle_guest_mmio_fault(struct vcpu_hw_context *context)
{
    x86_inst           ins;
    ...
    if (unlikely(dinst.inst_type != INST_TYPE_MOV)) {
        X86_DEBUG_LOG(svm_intercept, LVL_ERR, "IO Fault in guest without a move instruction!\n");
        goto guest_bad_fault;
    }
...
static inline void handle_guest_protected_mem_rw(struct vcpu_hw_context *context)
{
    int               rc;
    struct vmm_guest *guest       = context->assoc_vcpu->guest;
    physical_addr_t   fault_gphys = context->vmcb->exitinfo2;
     ...
     */
    if (lookup_guest_pagetable(context, fault_gphys, &lookedup_gphys, &pde, &pte) != VMM_OK) {
        X86_DEBUG_LOG(
            svm_intercept, LVL_DEBUG,
            "ERROR: No page table entry "
     ...
     * programmed.
     */
    if ((PagePresent(&pte) && PageReadOnly(&pte)) || (PagePresent(&pde) && PageReadOnly(&pde))) {
    ...
    rc = vmm_guest_physical_map(guest, lookedup_gphys, PAGE_SIZE, &hphys_addr, &availsz, &flags);
    ...
    if (flags & VMM_REGION_REAL) {
        if (create_guest_shadow_map(context, lookedup_gphys, hphys_addr, PAGE_SIZE, pdprot, prot) != VMM_OK) {
            X86_DEBUG_LOG(
                svm_intercept, LVL_ERR,
                "ERROR: Failed to create map in"
                "guest's shadow page table.\n"
...
{
    uint64_t gdt_entry;
    uint32_t guest_gdt_base = context->g_regs[GUEST_REGS_RBX];
    int      i;

    vmm_printf("GDT Base: 0x%x\n", guest_gdt_base);

    for (i = 0; i < 2; i++) {
        guest_read_gva(context, guest_gdt_base, (uint8_t *)&gdt_entry, sizeof(gdt_entry));
    ...
    switch (context->vmcb->exitcode) {
        case VMEXIT_EXCEPTION_PF:
            ...
            if (!(context->g_cr0 & X86_CR0_PG)) {
                 ...
                 */
                rc = vmm_guest_physical_map(guest, fault_gphys, PAGE_SIZE, &hphys_addr, &availsz, &flags);

                if (rc) {
                    X86_DEBUG_LOG(
                        svm_intercept, LVL_ERR,
                        "ERROR: No region mapped to "
                ...
                if (flags & (VMM_REGION_REAL | VMM_REGION_ALIAS)) {
                    handle_guest_realmode_page_fault(context, fault_gphys, hphys_addr);
                    return;
                }
        ...
        default:
            X86_DEBUG_LOG(
                svm_intercept, LVL_ERR, "Unhandled exception %s (rIP: 0x%" PRIADDR ")\n", exception_names[context->vmcb->exitcode - 0x40],
                context->vmcb->rip);
            goto guest_bad_fault;
    }
    ...
    if (context->cpuinfo->decode_assist) {
        ...
        if (likely(dinst.inst_type == INST_TYPE_MOV_CR)) {
            switch (dinst.inst.crn_mov.src_reg) {
                ...
                default:
                    X86_DEBUG_LOG(svm_intercept, LVL_ERR, "Unknown CR 0x%" PRIx64 " read by guest\n", dinst.inst.crn_mov.src_reg);
                    goto guest_bad_fault;
            }
...
void __handle_crN_write(struct vcpu_hw_context *context)
{
    ...
    if (context->cpuinfo->decode_assist) {
            ...
            switch (dinst.inst.crn_mov.dst_reg) {
                case RM_REG_CR0:
                    if (!dinst.inst.crn_mov.src_reg) {
                        ...
                        bits_set       = (~context->g_cr0 & context->g_regs[sreg]);
                        bits_clrd      = (context->g_cr0 & ~context->g_regs[sreg]);
                        context->g_cr0 = context->g_regs[sreg];
                    }
                ...
                default:
                    X86_DEBUG_LOG(svm_intercept, LVL_ERR, "Write to CR%d not supported.\n", (int)(dinst.inst.crn_mov.dst_reg - RM_REG_CR0));
                    goto guest_bad_fault;
            }
        ...
        asm volatile("str %0\n" : "=r"(htr));
        X86_DEBUG_LOG(svm_intercept, LVL_DEBUG, "GW: CR0= 0x%" PRIx64 " HCR0: 0x%" PRIx64 "\n", context->g_cr0, context->vmcb->cr0);
        X86_DEBUG_LOG(svm_intercept, LVL_DEBUG, "TR: 0x%" PRIx64 " HTR: 0x%" PRIx64 "\n", *((uint64_t *)&context->vmcb->tr), htr);
    }
...
{
    uint32_t io_port    = (context->vmcb->exitinfo1 >> 16);
    uint8_t  in_inst    = (context->vmcb->exitinfo1 & (0x1 << 0));
    uint8_t  str_op     = (context->vmcb->exitinfo1 & (0x1 << 2));
    uint8_t  rep_access = (context->vmcb->exitinfo1 & (0x1 << 3));
    uint8_t  op_size    = (context->vmcb->exitinfo1 & (0x1 << 4) ? 8 : ((context->vmcb->exitinfo1 & (0x1 << 5)) ? 16 : 32));
    uint8_t  seg_num    = (context->vmcb->exitinfo1 >> 10) & 0x7;
    uint32_t guest_rd   = 0;
    uint32_t wval;

    X86_DEBUG_LOG(svm_intercept, LVL_VERBOSE, "RIP: 0x%" PRIx64 " exitinfo1: 0x%" PRIx64 "\n", context->vmcb->rip, context->vmcb->exitinfo1);
    X86_DEBUG_LOG(
        svm_intercept, LVL_VERBOSE,
...
{
    struct x86_vcpu_priv *private = x86_vcpu_private(context->assoc_vcpu);
    struct cpuid_response *func;

    switch (context->vmcb->rax) {
        case CPUID_BASE_LFUNCSTD:
            func                            = &private->standard_funcs[CPUID_BASE_LFUNCSTD];
        ...
        case CPUID_BASE_FEATURES:
            func                            = &private->standard_funcs[CPUID_BASE_FEATURES];
            context->vmcb->rax              = func->resp_eax;
            context->g_regs[GUEST_REGS_RBX] = func->resp_ebx;
            context->g_regs[GUEST_REGS_RCX] = func->resp_ecx;
            context->g_regs[GUEST_REGS_RDX] = func->resp_edx;
        ...
        case CPUID_EXTENDED_L2_CACHE_TLB_IDENTIFIER:
            func                            = &private->extended_funcs[context->vmcb->rax - CPUID_EXTENDED_LFUNCEXTD];
            context->vmcb->rax              = func->resp_eax;
            context->g_regs[GUEST_REGS_RBX] = func->resp_ebx;
            context->g_regs[GUEST_REGS_RCX] = func->resp_ecx;
            context->g_regs[GUEST_REGS_RDX] = func->resp_edx;
...
void handle_vcpuexit(struct vcpu_hw_context *context)
{
    ...
    switch (context->vmcb->exitcode) {
        ...
        default:
            X86_DEBUG_LOG(
                svm_intercept, LVL_ERR, "#VMEXIT: Unhandled exit code: (0x%x:%d)\n", (uint32_t)context->vmcb->exitcode,
                (uint32_t)context->vmcb->exitcode);
...
void svm_dump_guest_state(struct vcpu_hw_context *context)
{
    ...
    vmm_printf(
        "R08: 0x%" PRIx64 " R09: 0x%" PRIx64 " R10: 0x%" PRIx64 " R11: 0x%" PRIx64 "\n", context->g_regs[GUEST_REGS_R8],
        context->g_regs[GUEST_REGS_R9], context->g_regs[GUEST_REGS_R10], context->g_regs[GUEST_REGS_R10]);
    vmm_printf(
        "R12: 0x%" PRIx64 " R13: 0x%" PRIx64 " R14: 0x%" PRIx64 " R15: 0x%" PRIx64 "\n", context->g_regs[GUEST_REGS_R12],
        context->g_regs[GUEST_REGS_R13], context->g_regs[GUEST_REGS_R14], context->g_regs[GUEST_REGS_R15]);
    vmm_printf(
        "RSP: 0x%" PRIx64 " RBP: 0x%" PRIx64 " RDI: 0x%" PRIx64 " RSI: 0x%" PRIx64 "\n", context->vmcb->rsp, context->g_regs[GUEST_REGS_RBP],
        context->g_regs[GUEST_REGS_RDI], context->g_regs[GUEST_REGS_RSI]);
    vmm_printf("RIP: 0x%" PRIx64 "\n\n", context->vmcb->rip);
    vmm_printf(
        "CR0: 0x%" PRIx64 " CR2: 0x%" PRIx64 " CR3: 0x%" PRIx64 " CR4: 0x%" PRIx64 "\n", context->vmcb->cr0, context->vmcb->cr2, context->vmcb->cr3,
        context->vmcb->cr4);
