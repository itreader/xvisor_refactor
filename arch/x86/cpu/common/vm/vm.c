...
physical_addr_t cpu_create_vcpu_intercept_table(size_t size, virtual_addr_t *table_vaddr)
{
...
void enable_ioport_intercept(struct vcpu_hw_context *context, uint32_t ioport)
{
    uint32_t byte_offset = ioport >> 3;
    uint8_t  port_offset = ioport & 0x7;

    uint8_t *iop_base    = (uint8_t *)(context->icept_table.io_table_virt + byte_offset);
    *iop_base |= (0x1 << port_offset);
}

void disable_ioport_intercept(struct vcpu_hw_context *context, uint32_t ioport)
{
    uint32_t byte_offset = ioport >> 3;
    uint8_t  port_offset = ioport & 0x7;
...
int cpu_init_vcpu_hw_context(struct cpuinfo_x86 *cpuinfo, struct vcpu_hw_context *context)
{
    ...
    context->shadow32_pg_list = (union page32 *)vmm_host_alloc_pages(NR_32BIT_PGLIST_PAGES, VMM_MEMORY_FLAGS_NORMAL);
    ...
    context->icept_table.io_table_phys = cpu_create_vcpu_intercept_table(IO_INTCPT_TBL_SZ, &context->icept_table.io_table_virt);
    ...
    context->icept_table.msr_table_phys = cpu_create_vcpu_intercept_table(MSR_INTCPT_TBL_SZ, &context->icept_table.msr_table_virt);
...
_error:
    ...
    if (context->shadow32_pg_list) {
        vmm_host_free_pages((virtual_addr_t)context->shadow32_pg_list, NR_32BIT_PGLIST_PAGES * PAGE_SIZE);
    }

    if (context->shadow_pgt) {
        mmu_page_table_free(&host_page_table_ctl, context->shadow_pgt);
    }
