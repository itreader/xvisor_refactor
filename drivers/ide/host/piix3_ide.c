...
#define TO_BDF(__b, __d, __f) ((__b << 16) | (__d << 11) | (__f << 8))

#define PIIX3_BDF(__dev)      (TO_BDF(__dev.bus, __dev.device, __dev.function))

static int piix3_ide_probe(vmm_device_t *dev)
{
    ...
    for (j = 0; j < array_size(piix3_ide_devices); j++) {
        ...
        if ((config_val >> 16) != 0xFFFF) {
            ...
            for (i = 0; i < MAX_IDE_DRIVES; i++) {
                ...
                vmm_printf(
                    " Found %s Drive %dMB - [%s %s] %s\n", (const char *[]){"ATA", "ATAPI"}[drive->type], drive->size / 1024 / 2,
...
static vmm_driver_t piix3_ide_driver = {
    .name        = "piix3_ide",
...
{
    return vmm_device_driver_register_driver(&piix3_ide_driver);
}

static void piix3_ide_exit(void)
{
    vmm_device_driver_unregister_driver(&piix3_ide_driver);
}
