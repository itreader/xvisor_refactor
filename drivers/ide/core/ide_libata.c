...
static void ide_write(struct ide_channel *channel, uint8_t reg, uint8_t data)
{
    int port;
...
static uint8_t ide_read(struct ide_channel *channel, uint8_t reg)
{
    uint8_t result = 0; /* Assume it will work. */
...
unsigned char ide_print_error(struct ide_drive *drive, unsigned char err)
{
    ...
    vmm_printf(
        ...
        (const char *[]){"Master", "Slave"}[drive->drive], drive->model);

    return err;
}

static void ide_read_buffer(struct ide_channel *channel, uint8_t reg, void *buffer, uint32_t quads)
{
...
static uint8_t ide_polling(struct ide_channel *channel, uint32_t advanced_check)
{
    int i;
...
static uint8_t ide_ata_access(struct ide_drive *drive, uint8_t direction, bool use_dma, uint64_t lba, uint32_t numsects, void *buffer)
{
    uint8_t             lba_mode;                /* 0: CHS, 1:LBA28, 2: LBA48 */
    uint8_t             dma = (use_dma) ? 1 : 0; /* 0: No DMA, 1: DMA */
    uint8_t             cmd;
    uint8_t             lba_io[6];
    struct ide_channel *channel  = drive->channel;
    uint32_t            slavebit = drive->drive;
...
static int ide_atapi_read(struct ide_drive *drive, uint32_t lba, uint32_t numsects, void *buffer)
{
    struct ide_channel *channel  = drive->channel;
    uint32_t            slavebit = drive->drive;
    uint32_t            bus      = channel->base;
...
uint32_t ide_write_sectors(struct ide_drive *drive, uint64_t lba, uint32_t numsects, const void *buffer)
{
    ...
    /* Read in PIO Mode through Polling & IRQs: */
        ...
        uint8_t err = 1; /* Device Fault */

        if (drive->type == IDE_ATA) {
            err = ide_ata_access(drive, ATA_WRITE, FALSE, lba, numsects, (void *)buffer);
        }
...
uint32_t ide_read_sectors(struct ide_drive *drive, uint64_t lba, uint32_t numsects, void *buffer)
{
    ...
    /* Read in PIO Mode through Polling & IRQs: */
        ...
        if (drive->type == IDE_ATA) {
            err = ide_ata_access(drive, ATA_READ, FALSE, lba, numsects, buffer);
...
int ide_initialize(struct ide_host_controller *controller)
{
    ...
    for (i = 0; i < MAX_IDE_CHANNELS; i++) {
        for (j = 0; j < MAX_IDE_DRIVES_PER_CHAN; j++) {
            ...
            /* Select Drive: */
            ide_write(&controller->ide_channels[i], ATA_REG_HDDEVSEL, 0xA0 | (j << 4));
            ...
            if (err != 0) {
                uint8_t cl = ide_read(&controller->ide_channels[i], ATA_REG_LBA1);
                uint8_t ch = ide_read(&controller->ide_channels[i], ATA_REG_LBA2);
                ...
                ide_write(&controller->ide_channels[i], ATA_REG_COMMAND, ATA_CMD_IDENTIFY_PACKET);
            ...
            controller->ide_drives[count].cmd_set            = *((uint32_t *)(ide_buf + ATA_IDENT_COMMANDSETS));
            ...
            if (controller->ide_drives[count].cmd_set & (1 << 26)) {
                ...
                controller->ide_drives[count].size          = *((uint32_t *)(ide_buf + ATA_IDENT_MAX_LBA_EXT));
                controller->ide_drives[count].lba48_enabled = 1;
            } else {
                /* Device uses CHS or 28-bit Addressing: */
                controller->ide_drives[count].size          = *((uint32_t *)(ide_buf + ATA_IDENT_MAX_LBA));
                controller->ide_drives[count].lba48_enabled = 0;
            }

            controller->ide_drives[count].block_size = 512;

            if (type == IDE_ATAPI) {
                /* CDROM have block size of 2048 */
                controller->ide_drives[count].block_size = 2048;
                /* Force 650MB */
