...
static vmm_driver_t syscon_driver;
...
static struct syscon *of_syscon_register(vmm_device_tree_node_t *np)
{
    int                  ret;
    irq_flags_t          flags;
    ...
    ret = vmm_device_tree_regaddr(np, &pa, 0);
     ...
     */
    ret = vmm_device_tree_read_u32(np, "reg-io-width", &reg_io_width);
    ...
    syscon_config.max_register = size - reg_io_width;
    ...
    vmm_spin_unlock_irq_restore(&syscon_list_slock, flags);
...
static void of_syscon_unregister(struct syscon *syscon)
{
    ...
    vmm_device_tree_regunmap_release(syscon->np, (virtual_addr_t)syscon->base, 0);

    vmm_device_tree_dref_node(syscon->np);

    vmm_free(syscon);
}

static struct syscon *node_to_syscon(vmm_device_tree_node_t *np)
{
    irq_flags_t    flags;
...
struct regmap *syscon_node_to_regmap(vmm_device_tree_node_t *np)
{
...
{
    vmm_device_tree_node_t *syscon_np;
    struct regmap          *regmap;
    ...
    vmm_device_tree_dref_node(syscon_np);
...
{
    return !strcmp(dev->name, (const char *)data) && (dev->driver == &syscon_driver);
}
...
struct regmap *syscon_regmap_lookup_by_phandle(vmm_device_tree_node_t *np, const char *property)
{
    ...
    if (property) {
        syscon_np = vmm_device_tree_parse_phandle(np, property, 0);
    } else {
        syscon_np = np;
    }
...
static int syscon_probe(vmm_device_t *dev)
{
    ...
    vmm_device_driver_set_data(dev, syscon);

    return VMM_OK;
}

static int syscon_remove(vmm_device_t *dev)
{
    struct syscon *syscon = vmm_device_driver_get_data(dev);
...
static const struct vmm_device_tree_nodeid syscon_match[] = {
    {
     .compatible = "syscon",
     },
    {},
...
{
    return vmm_device_driver_register_driver(&syscon_driver);
}

static void __exit syscon_exit(void)
{
    vmm_device_driver_unregister_driver(&syscon_driver);
}
