...
static vmm_device_t *vexpress_sysreg_dev;
static int           vexpress_master_site;

void vexpress_flags_set(uint32_t data)
{
    vmm_writel(~0, vexpress_sysreg_base + SYS_FLAGSCLR);
    vmm_writel(data, vexpress_sysreg_base + SYS_FLAGSSET);
}

uint32_t vexpress_get_procid(int site)
{
    if (site == VEXPRESS_SITE_MASTER) {
        site = vexpress_master_site;
    }

    return vmm_readl(vexpress_sysreg_base + (site == VEXPRESS_SITE_DB1 ? SYS_PROCID0 : SYS_PROCID1));
}

uint32_t vexpress_get_hbi(int site)
{
    uint32_t id;
...
static void vexpress_sysreg_find_prop(vmm_device_tree_node_t *node, const char *name, uint32_t *val)
{
    while (node) {
        if (vmm_device_tree_read_u32(node, name, val) == VMM_OK) {
            return;
        }
...
unsigned __vexpress_get_site(vmm_device_t *dev, vmm_device_tree_node_t *node)
{
    uint32_t site = 0;
...
static vmm_timer_event_t              vexpress_sysreg_config_timer;
static uint32_t                      *vexpress_sysreg_config_data;
static int                            vexpress_sysreg_config_tries;

static void *vexpress_sysreg_config_func_get(vmm_device_t *dev, vmm_device_tree_node_t *node)
{
    ...
    if (node) {
        vexpress_sysreg_find_prop(node, "arm,vexpress,site", &site);
        vexpress_sysreg_find_prop(node, "arm,vexpress,position", &position);
        vexpress_sysreg_find_prop(node, "arm,vexpress,dcc", &dcc);
        err = vmm_device_tree_read_u32_array(node, "arm,vexpress-sysreg,func", func_device, array_size(func_device));
    }
    ...
    config_func->template |= SYS_CFGCTRL_SITE(site == VEXPRESS_SITE_MASTER ? vexpress_master_site : site);
    config_func->template |= SYS_CFGCTRL_POSITION(position);
    config_func->device |= func_device[1];

    DPRINTF("%s: func 0x%p = 0x%x, %d\n", vexpress_sysreg_dev->name, config_func, config_func->template, config_func->device);
...
static int vexpress_sysreg_config_func_exec(void *func, int offset, bool write, uint32_t *data)
{
    int                                 status;
...
static void vexpress_sysreg_config_complete(vmm_timer_event_t *ev)
{
    ...
    if (vexpress_sysreg_config_data) {
        *vexpress_sysreg_config_data = vmm_readl(vexpress_sysreg_base + SYS_CFGDATA);
        DPRINTF("%s: read data %x\n", vexpress_sysreg_dev->name, *vexpress_sysreg_config_data);
...
void vexpress_sysreg_setup(vmm_device_tree_node_t *node)
{
    if (WARN_ON(!vexpress_sysreg_base)) {
        return;
    }
    ...
    vexpress_sysreg_config_bridge = vexpress_config_bridge_register(node, &vexpress_sysreg_config_bridge_info);
...
void __init vexpress_sysreg_of_early_init(void)
{
    ...
    node = vmm_device_tree_find_compatible(NULL, NULL, "arm,vexpress-sysreg");

    if (node) {
        err = vmm_device_tree_request_regmap(node, &base_va, 0, "VExpress Sysreg");
                                    ...
                                    unsigned offset)
{
    ...
    uint32_t reg_value = readl(vexpress_sysreg_base + gpio->reg);
...
static int vexpress_sysreg_probe(vmm_device_t *dev)
{
    int            err;
    virtual_addr_t base_va;

    if (!vexpress_sysreg_base) {
        err = vmm_device_tree_request_regmap(dev->of_node, &base_va, 0, "VExpress Sysreg");
...
static int vexpress_sysreg_remove(vmm_device_t *dev)
{
    /* Nothing to do here for now. */
    return VMM_OK;
}

static const struct vmm_device_tree_nodeid vexpress_sysreg_match[] = {
    {
...
static vmm_driver_t vexpress_sysreg_driver = {
...
static int __init vexpress_sysreg_init(void)
{
     ...
     */
    return vmm_device_driver_register_driver(&vexpress_sysreg_driver);
}

static void __exit vexpress_sysreg_exit(void)
{
    vmm_device_driver_unregister_driver(&vexpress_sysreg_driver);
}
