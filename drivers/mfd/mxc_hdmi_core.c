...
struct mxc_hdmi_data {
    vmm_device_t     *dev;
    uint64_t __iomem *reg_base;
};

static void __iomem       *hdmi_base;
static struct clk         *isfr_clock;
static struct clk         *iahb_clock;
static spinlock_t          irq_spinlock;
static spinlock_t          edid_spinlock;
static uint32_t            sample_rate;
static uint64_t            pixel_clock_rate;
static struct clk         *pixel_clock;
static int                 hdmi_ratio;
int                        mxc_hdmi_ipu_id;
int                        mxc_hdmi_disp_id;
static struct mxc_edid_cfg hdmi_core_edid_cfg;
static int                 hdmi_core_init;
static uint32_t            hdmi_dma_running;
static uint32_t            hdmi_cable_state;
static uint32_t            hdmi_blank_state;
static uint32_t            hdmi_abort_state;
...
uint32_t hdmi_set_cable_state(uint32_t state)
{
    ...
    spin_lock_irq_save(&hdmi_cable_state_lock, flags);
    hdmi_cable_state = state;
    spin_unlock_irq_restore(&hdmi_cable_state_lock, flags);
...
uint32_t hdmi_set_blank_state(uint32_t state)
{
    ...
    spin_lock_irq_save(&hdmi_blank_state_lock, flags);
    hdmi_blank_state = state;
    spin_unlock_irq_restore(&hdmi_blank_state_lock, flags);
...
static void hdmi_audio_abort_stream(struct snd_pcm_substream *substream)
{
    ...
    snd_pcm_stream_lock_irq_save(substream, flags);
    ...
    snd_pcm_stream_unlock_irq_restore(substream, flags);
}
#endif
...
#if 0
    uint64_t flags;
    spin_lock_irq_save(&hdmi_audio_lock, flags);

    if (hdmi_audio_stream_playback)
        hdmi_audio_abort_stream(hdmi_audio_stream_playback);

    spin_unlock_irq_restore(&hdmi_audio_lock, flags);
#endif
    return 0;
}
...
int check_hdmi_state(void)
{
    ...
    spin_lock_irq_save(&hdmi_cable_state_lock, flags1);
    spin_lock_irq_save(&hdmi_blank_state_lock, flags2);

    ret = hdmi_cable_state && hdmi_blank_state;

    spin_unlock_irq_restore(&hdmi_blank_state_lock, flags2);
    spin_unlock_irq_restore(&hdmi_cable_state_lock, flags1);
...
int mxc_hdmi_register_audio(struct snd_pcm_substream *substream)
{
    ...
    if (substream && check_hdmi_state()) {
        spin_lock_irq_save(&hdmi_audio_lock, flags1);
        ...
        hdmi_abort_state = 0;
        spin_unlock_irq_restore(&hdmi_audio_lock, flags1);
    } else
        ret = -EINVAL;
...
void hdmi_mask_writeb(uint8_t data, uint32_t reg, uint8_t shift, uint8_t mask)
{
    uint8_t value = hdmi_readb(reg) & ~mask;
...
uint32_t hdmi_read4(uint32_t reg)
{
    ...
    return (hdmi_readb(reg + 3) << 24) | (hdmi_readb(reg + 2) << 16) | (hdmi_readb(reg + 1) << 8) | hdmi_readb(reg);
}
...
static void initialize_hdmi_ih_mutes(void)
{
    ...
    ih_mute = hdmi_readb(HDMI_IH_MUTE) | HDMI_IH_MUTE_MUTE_WAKEUP_INTERRUPT | HDMI_IH_MUTE_MUTE_ALL_INTERRUPT;
...
static void hdmi_set_clock_regenerator_n(uint32_t value)
{
    uint8_t val;
...
static void hdmi_set_clock_regenerator_cts(uint32_t cts)
{
    uint8_t val;
    ...
    hdmi_writeb(((cts >> 16) & HDMI_AUD_CTS3_AUDCTS19_16_MASK) | HDMI_AUD_CTS3_CTS_MANUAL, HDMI_AUD_CTS3);
}

static uint32_t hdmi_compute_n(uint32_t freq, uint64_t pixel_clock, uint32_t ratio)
{
    uint32_t n = (128 * freq) / 1000;

    switch (freq) {
        case 32000:
            if (pixel_clock == 25174000) {
                n = (ratio == 150) ? 9152 : 4576;
            } else if (pixel_clock == 27020000) {
                n = (ratio == 150) ? 8192 : 4096;
            } else if (pixel_clock == 74170000 || pixel_clock == 148350000) {
                n = 11648;
            } else if (pixel_clock == 297000000) {
        ...
        case 44100:
            if (pixel_clock == 25174000) {
                n = 7007;
            } else if (pixel_clock == 74170000) {
                n = 17836;
            } else if (pixel_clock == 148350000) {
        ...
        case 88200:
            n = hdmi_compute_n(44100, pixel_clock, ratio) * 2;
            break;

        case 96000:
            n = hdmi_compute_n(48000, pixel_clock, ratio) * 2;
            break;

        case 176400:
            n = hdmi_compute_n(44100, pixel_clock, ratio) * 4;
            break;

        case 192000:
            n = hdmi_compute_n(48000, pixel_clock, ratio) * 4;
            break;
...
static uint32_t hdmi_compute_cts(uint32_t freq, uint64_t pixel_clock, uint32_t ratio)
{
    uint32_t cts = 0;

    switch (freq) {
        case 32000:
            if (pixel_clock == 297000000) {
                cts = 222750;
                break;
            } else if (pixel_clock == 25174000) {
                cts = 28125;
                break;
            }
...
static void hdmi_set_clock_regenerator(void)
{
    uint32_t clock_n, clock_cts;

    clock_n   = hdmi_compute_n(sample_rate, pixel_clock_rate, hdmi_ratio);
    clock_cts = hdmi_compute_cts(sample_rate, pixel_clock_rate, hdmi_ratio);

    if (clock_cts == 0) {
        pr_debug("%s: pixel clock not supported: %d\n", __func__, (int)pixel_clock_rate);
        return;
    }

    pr_debug(
        "%s: samplerate=%d  ratio=%d  pixelclk=%d  N=%d  cts=%d\n", __func__, sample_rate, hdmi_ratio, (int)pixel_clock_rate, clock_n, clock_cts);

    hdmi_set_clock_regenerator_cts(clock_cts);
    hdmi_set_clock_regenerator_n(clock_n);
}

static int hdmi_core_get_of_property(vmm_device_t *dev)
{
    int      err;
...
void hdmi_init_clock_regenerator(void)
{
    if (pixel_clock_rate == 0) {
        pixel_clock_rate = 74250000;
        hdmi_set_clock_regenerator();
    }
}

EXPORT_SYMBOL(hdmi_init_clock_regenerator);

void hdmi_clock_regenerator_update_pixel_clock(uint32_t pixclock)
{

    /* Translate pixel clock in ps (pico seconds) to Hz  */
    pixel_clock_rate = PICOS2KHZ(pixclock) * 1000UL;
    hdmi_set_clock_regenerator();
}

EXPORT_SYMBOL(hdmi_clock_regenerator_update_pixel_clock);
...
void hdmi_set_edid_cfg(struct mxc_edid_cfg *cfg)
{
    ...
    spin_lock_irq_save(&edid_spinlock, flags);
    memcpy(&hdmi_core_edid_cfg, cfg, sizeof(struct mxc_edid_cfg));
    spin_unlock_irq_restore(&edid_spinlock, flags);
}
...
static int mxc_hdmi_core_probe(vmm_device_t *dev)
{
    ...
    ret                           = vmm_device_tree_request_regmap(dev->of_node, &base_va, 0, "MXC HDMI Core");
    ...
    isfr_clock = devm_clock_get(dev, "hdmi_isfr");

    if (IS_ERR(isfr_clock)) {
        ret = PTR_ERR(isfr_clock);
        dev_err(dev, "Unable to get HDMI isfr clk: %d\n", ret);
        goto fail;
    }

    ret = clock_prepare_enable(isfr_clock);
    ...
    pr_debug("%s isfr_clock:%lu\n", __func__, clock_get_rate(isfr_clock));

    iahb_clock = devm_clock_get(dev, "hdmi_iahb");

    if (IS_ERR(iahb_clock)) {
        ret = PTR_ERR(iahb_clock);
        dev_err(dev, "Unable to get HDMI iahb clk: %d\n", ret);
        goto eclkg2;
    }

    ret = clock_prepare_enable(iahb_clock);
    ...
    clock_disable_unprepare(isfr_clock);
    clock_disable_unprepare(iahb_clock);

    /* Replace platform data coming in with a local struct */
    vmm_device_driver_set_data(dev, hdmi_data);

    return ret;

eclke2:
    clock_put(iahb_clock);
eclkg2:
    clock_disable_unprepare(isfr_clock);
eclke:
    clock_put(isfr_clock);
fail:
...
static int __exit mxc_hdmi_core_remove(vmm_device_t *dev)
{
    return VMM_OK;
}
...
static vmm_driver_t mxc_hdmi_core_driver = {
    .name        = "mxc_hdmi_core",
...
{
    return vmm_device_driver_register_driver(&mxc_hdmi_core_driver);
}

static void __exit mxc_hdmi_core_exit(void)
{
    vmm_device_driver_unregister_driver(&mxc_hdmi_core_driver);
}
