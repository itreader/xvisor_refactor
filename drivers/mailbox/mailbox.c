...
static void txdone_hrtimer(vmm_timer_event_t *hrtimer);
...
static void txdone_hrtimer(vmm_timer_event_t *hrtimer)
{
    struct mbox_controller *mbox = container_of(hrtimer, struct mbox_controller, poll_hrt);
    ...
    if (resched) {
        vmm_timer_event_start(hrtimer, (uint64_t)mbox->txpoll_period * 1000000ULL);
    }
}
...
void mbox_chan_txdone(struct mbox_chan *chan, int r)
{
    if (unlikely(!(chan->txdone_method & TXDONE_BY_IRQ))) {
        vmm_lerror(chan->mbox->dev->name, "Controller can't run the TX ticker\n");
        return;
    }
...
void mbox_client_txdone(struct mbox_chan *chan, int r)
{
    if (unlikely(!(chan->txdone_method & TXDONE_BY_ACK))) {
        vmm_lerror(chan->mbox->dev->name, "Client can't run the TX ticker\n");
        return;
    }
...
int mbox_send_message(struct mbox_chan *chan, void *mssg)
{
    ...
    if (t < 0) {
        vmm_lerror(chan->mbox->dev->name, "Try increasing MBOX_TX_QUEUE_LEN\n");
        return t;
    }
    ...
    if (chan->cl->tx_block && chan->active_req) {
        ...
        if (!chan->cl->tx_tout) { /* wait forever */
            ...
            wait = (uint64_t)chan->cl->tx_tout * 1000000ULL;
        }
...
{
    vmm_device_t                       *dev = cl->dev;
    struct mbox_controller             *mbox;
    struct vmm_device_tree_phandle_args spec;
    ...
    vmm_device_tree_dref_node(spec.np);
...
{
    vmm_device_tree_node_t      *np = cl->dev->of_node;
