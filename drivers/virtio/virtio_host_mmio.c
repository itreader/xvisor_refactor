...
#define to_virtio_host_mmio_device(_plat_dev) container_of(_plat_dev, struct virtio_host_mmio_device, vdev)
...
static uint64_t vm_get_features(struct virtio_host_device *vdev)
{
    struct virtio_host_mmio_device *vm_dev = to_virtio_host_mmio_device(vdev);
    uint64_t                        features;
...
static int vm_finalize_features(struct virtio_host_device *vdev)
{
    ...
    if (vm_dev->version == 2 && !__virtio_host_test_bit(vdev, VMM_VIRTIO_F_VERSION_1)) {
        vmm_lerror(
    ...
    vmm_writel((uint32_t)(vdev->features >> 32), vm_dev->base + VMM_VIRTIO_MMIO_DRIVER_FEATURES);

    vmm_writel(0, vm_dev->base + VMM_VIRTIO_MMIO_DRIVER_FEATURES_SEL);
    vmm_writel((uint32_t)vdev->features, vm_dev->base + VMM_VIRTIO_MMIO_DRIVER_FEATURES);

    return 0;
}

static void vm_get(struct virtio_host_device *vdev, unsigned offset, void *buf, unsigned len)
{
    struct virtio_host_mmio_device *vm_dev = to_virtio_host_mmio_device(vdev);
...
static void vm_set(struct virtio_host_device *vdev, unsigned offset, const void *buf, unsigned len)
{
    struct virtio_host_mmio_device *vm_dev = to_virtio_host_mmio_device(vdev);
    void                           *base   = vm_dev->base + VMM_VIRTIO_MMIO_CONFIG;
    uint8_t                         b;
...
static uint32_t vm_generation(struct virtio_host_device *vdev)
{
    struct virtio_host_mmio_device *vm_dev = to_virtio_host_mmio_device(vdev);
    ...
    if (vm_dev->version == 1) {
        ...
        return vmm_readl(vm_dev->base + VMM_VIRTIO_MMIO_CONFIG_GENERATION);
    }
}

static uint8_t vm_get_status(struct virtio_host_device *vdev)
{
    struct virtio_host_mmio_device *vm_dev = to_virtio_host_mmio_device(vdev);
...
static void vm_set_status(struct virtio_host_device *vdev, uint8_t status)
{
    struct virtio_host_mmio_device *vm_dev = to_virtio_host_mmio_device(vdev);
...
{
    struct virtio_host_mmio_device *vm_dev = to_virtio_host_mmio_device(vq->vdev);
...
static vmm_irq_return_t vm_interrupt(int irq, void *opaque)
{
    ...
    if (likely(status & VMM_VIRTIO_MMIO_INT_VRING)) {
        vmm_spin_lock_irq_save(&vm_dev->lock, flags);
        list_for_each_entry(info, &vm_dev->vqs, node) ret |= virtio_host_queue_interrupt(irq, info->vq);
        vmm_spin_unlock_irq_restore(&vm_dev->lock, flags);
    }
...
{
    struct virtio_host_mmio_device  *vm_dev = to_virtio_host_mmio_device(vq->vdev);
    struct virtio_host_mmio_vq_info *info   = vq->private;
    uint64_t                         flags;
    uint32_t                         index = vq->index;
...
static void vm_del_vqs(struct virtio_host_device *vdev)
{
    ...
    list_for_each_entry_safe(vq, n, &vdev->vqs, head) vm_del_vq(vq);
}

static struct virtio_host_queue *vm_setup_vq(struct virtio_host_device *vdev, unsigned index, virtio_host_queue_callback_t callback, const char *name)
{
    struct virtio_host_mmio_device  *vm_dev = to_virtio_host_mmio_device(vdev);
    struct virtio_host_mmio_vq_info *info;
    struct virtio_host_queue        *vq;
    ...
    /* Activate the queue */
    vmm_writel(virtio_host_queue_get_vring_size(vq), vm_dev->base + VMM_VIRTIO_MMIO_QUEUE_NUM);

    if (vm_dev->version == 1) {
        vmm_writel(VMM_PAGE_SIZE, vm_dev->base + VMM_VIRTIO_MMIO_QUEUE_ALIGN);
        vmm_writel(virtio_host_queue_get_desc_addr(vq) >> VMM_PAGE_SHIFT, vm_dev->base + VMM_VIRTIO_MMIO_QUEUE_PFN);
    } else {
        uint64_t addr = virtio_host_queue_get_desc_addr(vq);

        vmm_writel((uint32_t)addr, vm_dev->base + VMM_VIRTIO_MMIO_QUEUE_DESC_LOW);
        vmm_writel((uint32_t)(addr >> 32), vm_dev->base + VMM_VIRTIO_MMIO_QUEUE_DESC_HIGH);

        addr = virtio_host_queue_get_avail_addr(vq);
        vmm_writel((uint32_t)addr, vm_dev->base + VMM_VIRTIO_MMIO_QUEUE_AVAIL_LOW);
        vmm_writel((uint32_t)(addr >> 32), vm_dev->base + VMM_VIRTIO_MMIO_QUEUE_AVAIL_HIGH);

        addr = virtio_host_queue_get_used_addr(vq);
        vmm_writel((uint32_t)addr, vm_dev->base + VMM_VIRTIO_MMIO_QUEUE_USED_LOW);
        vmm_writel((uint32_t)(addr >> 32), vm_dev->base + VMM_VIRTIO_MMIO_QUEUE_USED_HIGH);
...
static int virtio_host_mmio_probe(vmm_device_t *dev)
{
    int                             ret = 0;
    uint64_t                        magic;
    virtual_addr_t                  base;
    ...
    vm_dev->irq = vmm_device_tree_irq_parse_map(dev->of_node, 0);
    ...
    if ((ret = vmm_host_irq_register(vm_dev->irq, dev->name, vm_interrupt, vm_dev))) {
        vmm_lerror(dev->name, "Failed to register IRQ handler: %d\n", ret);
        goto fail;
    }

    ret = vmm_device_tree_request_regmap(dev->of_node, &base, 0, "VIRTIO_HOST_MMIO");

    if (ret) {
        vmm_lerror(dev->name, "Failed to map regs: %d\n", ret);
        goto fail_unreg_irq;
    }
    ...
    vm_dev->vdev.id.device = vmm_readl(vm_dev->base + VMM_VIRTIO_MMIO_DEVICE_ID);
    ...
    vm_dev->vdev.id.vendor = vmm_readl(vm_dev->base + VMM_VIRTIO_MMIO_VENDOR_ID);

    if (vm_dev->version == 1) {
        vmm_writel(VMM_PAGE_SIZE, vm_dev->base + VMM_VIRTIO_MMIO_GUEST_PAGE_SIZE);
    }

    vmm_device_driver_set_data(dev, vm_dev);

    vmm_linfo(dev->name, "VirtIO host MMIO device v%ld\n", vm_dev->version);

    ret = virtio_host_add_device(&vm_dev->vdev, &virtio_host_mmio_config_ops, dev);

    if (ret) {
        vmm_lerror(dev->name, "Failed to register VirtIO host device!\n");
        goto fail_unreg_base;
    }
...
fail_unreg_base:
    vmm_device_tree_regunmap_release(dev->of_node, (virtual_addr_t)vm_dev->base, 0);
fail_unreg_irq:
...
static int virtio_host_mmio_remove(vmm_device_t *dev)
{
    struct virtio_host_mmio_device *vm_dev = vmm_device_driver_get_data(dev);
...
static struct vmm_device_tree_nodeid virtio_host_mmio_devid_table[] = {
    {.compatible = "virtio,mmio"},
    {/* end of list */},
};

static vmm_driver_t virtio_host_mmio_driver = {
...
{
    return vmm_device_driver_register_driver(&virtio_host_mmio_driver);
}

static void __exit virtio_host_mmio_exit(void)
{
    vmm_device_driver_unregister_driver(&virtio_host_mmio_driver);
}
