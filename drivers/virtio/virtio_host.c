...
static inline void virtio_store_mb(bool weak_barriers, uint16_t *p, uint16_t v)
{
    *p = v;
    arch_mb();
}
...
static physical_addr_t virtio_map_one(const struct virtio_host_queue *vq, struct virtio_host_iovec *iv, enum vmm_dma_direction direction)
{
    int             rc;
    physical_addr_t pa;
    ...
    return vmm_dma_map((virtual_addr_t)iv->buf, (virtual_size_t)iv->buf_len, direction);
}

static void virtio_unmap_one(const struct virtio_host_queue *vq, struct vmm_vring_desc *desc)
{
    uint16_t flags;
    ...
    vmm_dma_unmap(
        virtio64_to_cpu(vq->vdev, desc->addr), virtio32_to_cpu(vq->vdev, desc->len),
        (flags & VMM_VRING_DESC_F_WRITE) ? DMA_FROM_DEVICE : DMA_TO_DEVICE);
}

void virtio_host_queue_dump_vring(struct virtio_host_queue *vq)
{
    uint16_t     *last_idx;
    vmm_device_t *dev = &vq->vdev->dev;

    vmm_linfo(dev->name, "desc=0x%" PRIPADDR "\n", vq->vring.desc_pa);
    last_idx = (uint16_t *)&vq->vring.avail->ring[vq->vring.num];
    vmm_linfo(
        dev->name,
        "avail=0x%" PRIPADDR " flags=0x%x "
        "idx=0x%x last_idx=0x%x\n",
        vq->vring.avail_pa, virtio16_to_cpu(vq->vdev, vq->vring.avail->flags), virtio16_to_cpu(vq->vdev, vq->vring.avail->idx),
        virtio16_to_cpu(vq->vdev, *last_idx));
    last_idx = (uint16_t *)&vq->vring.used->ring[vq->vring.num];
    vmm_linfo(
        dev->name,
        "used=0x%" PRIPADDR " flags=0x%x "
        "idx=0x%x last_idx=0x%x\n",
        vq->vring.used_pa, virtio16_to_cpu(vq->vdev, vq->vring.used->flags), virtio16_to_cpu(vq->vdev, vq->vring.used->idx),
        virtio16_to_cpu(vq->vdev, *last_idx));
}
    ...
    struct virtio_host_queue *vq, struct virtio_host_iovec *ivs[], uint32_t total_ivs, uint32_t out_ivs, uint32_t in_ivs, void *data)
{
    ...
    uint32_t                  i, n, avail, descs_used, prev = 0;
    int                       head;
    ...
    for (n = 0; n < out_ivs; n++) {
        ...
        desc[i].flags = cpu_to_virtio16(vq->vdev, VMM_VRING_DESC_F_NEXT);
    ...
    for (; n < (out_ivs + in_ivs); n++) {
        ...
        desc[i].flags = cpu_to_virtio16(vq->vdev, VMM_VRING_DESC_F_NEXT | VMM_VRING_DESC_F_WRITE);
        desc[i].addr  = cpu_to_virtio64(vq->vdev, addr);
        desc[i].len   = cpu_to_virtio32(vq->vdev, iv->buf_len);
        prev          = i;
        i             = virtio16_to_cpu(vq->vdev, desc[i].next);
...
bool virtio_host_queue_kick_prepare(struct virtio_host_queue *vq)
{
    ...
    if (vq->event) {
        needs_kick = vmm_vring_need_event(virtio16_to_cpu(vq->vdev, virtio_avail_event(&vq->vring)), new, old);
    } else {
        needs_kick = !(vq->vring.used->flags & cpu_to_virtio16(vq->vdev, VMM_VRING_USED_F_NO_NOTIFY));
    }
...
{
    uint32_t i;
    uint16_t nextflag         = cpu_to_virtio16(vq->vdev, VMM_VRING_DESC_F_NEXT);
...
void *virtio_host_queue_get_buf(struct virtio_host_queue *vq, uint32_t *len)
{
    ...
    *len      = virtio32_to_cpu(vq->vdev, vq->vring.used->ring[last_used].len);
    ...
    if (!(vq->avail_flags_shadow & VMM_VRING_AVAIL_F_NO_INTERRUPT)) {
        virtio_store_mb(vq->weak_barriers, &virtio_used_event(&vq->vring), cpu_to_virtio16(vq->vdev, vq->last_used_idx));
    }
    ...
    virtio_host_queue_callback_t callback, const char *name)
{
    ...
    vq = vmm_zalloc(sizeof(*vq) + vring.num * sizeof(struct virtio_host_desc_state));
    ...
    vq->indirect           = virtio_host_has_feature(vdev, VMM_VIRTIO_RING_F_INDIRECT_DESC);
    vq->event              = virtio_host_has_feature(vdev, VMM_VIRTIO_RING_F_EVENT_IDX);
    vq->broken             = FALSE;
    vq->free_head          = 0;
    ...
    memset(vq->desc_state, 0, vring.num * sizeof(struct virtio_host_desc_state));

    return vq;
}

static void *virtio_host_alloc_queue(struct virtio_host_device *vdev, size_t size, physical_addr_t *dma_handle)
{
    if (virtio_use_dma_api(vdev)) {
        ...
        queue = vmm_page_pool_alloc(VMM_PAGE_POOL_NORMAL, VMM_SIZE_TO_PAGE(size));
...
static void virtio_host_free_queue(struct virtio_host_device *vdev, size_t size, void *queue, physical_addr_t dma_handle)
{
    if (virtio_use_dma_api(vdev)) {
        vmm_dma_free(queue);
    } else {
    ...
    virtio_host_queue_callback_t callback, const char *name)
{
    ...
    if (num & (num - 1)) {
        vmm_lerror(vdev->dev.name, "Bad virtio host queue length %u\n", num);
        return NULL;
    }

    /* Try to get a single page. You are my only hope! */
    queue = virtio_host_alloc_queue(vdev, vmm_vring_size(num, vring_align), &dma_addr);
    ...
    vq = __virtio_host_new_queue(index, vring, vdev, weak_barriers, notify, callback, name);

    if (!vq) {
        virtio_host_free_queue(vdev, queue_size_in_bytes, queue, dma_addr);
        return NULL;
    }
...
{
    virtio_host_free_queue(vq->vdev, vq->vring_size, vq->vring.desc, vq->vring_dma_base);
    list_del(&vq->head);
    vmm_free(vq);
}
...
{
    struct virtio_host_driver *vdrv = to_virtio_host_driver(vdev->dev.driver);
...
void virtio_host_config_changed(struct virtio_host_device *vdev)
{
    ...
    vmm_spin_lock_irq_save(&vdev->config_lock, flags);
...
static int virtio_host_match_device(const struct virtio_host_device_id *ids, struct virtio_host_device *vdev)
{
    while (ids->device) {
        if ((ids->device == VMM_VIRTIO_ID_ANY || ids->device == vdev->id.device) &&
            (ids->vendor == VMM_VIRTIO_ID_ANY || ids->vendor == vdev->id.vendor)) {
            return 1;
        }
...
static int virtio_host_bus_match(vmm_device_t *dev, vmm_driver_t *drv)
{
...
static int virtio_host_driver_probe(vmm_device_t *dev)
{
    ...
    uint64_t                   driver_features_legacy;
    ...
    for (i = 0; i < vdrv->feature_table_size; i++) {
        uint32_t f = vdrv->feature_table[i];
        BUG_ON(f >= 64);
    ...
    if (vdrv->feature_table_legacy) {
        ...
        for (i = 0; i < vdrv->feature_table_size_legacy; i++) {
            uint32_t f = vdrv->feature_table_legacy[i];
            BUG_ON(f >= 64);
...
static int virtio_host_driver_remove(vmm_device_t *dev)
{
    struct virtio_host_device *vdev = to_virtio_host_device(dev);
    struct virtio_host_driver *vdrv = to_virtio_host_driver(dev->driver);
...
static vmm_bus_t virtio_host_bus = {
    .name   = "virtio_host",
...
int virtio_host_add_device(struct virtio_host_device *vdev, const struct virtio_host_config_ops *config, vmm_device_t *parent)
{
    ...
    vmm_device_driver_initialize_device(&vdev->dev);
    vdev->dev.bus    = &virtio_host_bus;
    vdev->dev.parent = parent;
    vmm_snprintf(vdev->dev.name, sizeof(vdev->dev.name), "virtio%u", vdev->index);
    ...
    err = vmm_device_driver_register_device(&vdev->dev);
out:
...
void virtio_host_remove_device(struct virtio_host_device *vdev)
{
    ...
    vmm_device_driver_unregister_device(&vdev->dev);
...
{
    return vmm_device_driver_register_bus(&virtio_host_bus);
}

static void __exit virtio_host_exit(void)
{
    vmm_device_driver_unregister_bus(&virtio_host_bus);
}
