 ...
 */
int _ipu_csi_mclock_set(struct ipu_soc *ipu, uint32_t pixel_clock, uint32_t csi)
{
    uint32_t temp;
    uint32_t div_ratio;

    div_ratio = udiv32(clock_get_rate(ipu->ipu_clock), pixel_clock) - 1;

    if (div_ratio > 0xFF || div_ratio < 0) {
        dev_dbg(ipu->dev, "value of pixel_clock extends normal range\n");
        return -EINVAL;
    }

    temp = ipu_csi_read(ipu, csi, CSI_SENS_CONF);
    temp &= ~CSI_SENS_CONF_DIVRATIO_MASK;
    ipu_csi_write(ipu, csi, temp | (div_ratio << CSI_SENS_CONF_DIVRATIO_SHIFT), CSI_SENS_CONF);
...
int32_t ipu_csi_init_interface(struct ipu_soc *ipu, uint16_t width, uint16_t height, uint32_t pixel_fmt, ipu_csi_signal_cfg_t cfg_param)
{
    ...
    data |= cfg_param.data_width << CSI_SENS_CONF_DATA_WIDTH_SHIFT | cfg_param.data_fmt << CSI_SENS_CONF_DATA_FMT_SHIFT |
            cfg_param.data_pol << CSI_SENS_CONF_DATA_POL_SHIFT | cfg_param.Vsync_pol << CSI_SENS_CONF_VSYNC_POL_SHIFT |
            cfg_param.Hsync_pol << CSI_SENS_CONF_HSYNC_POL_SHIFT | cfg_param.pixclock_pol << CSI_SENS_CONF_PIX_CLK_POL_SHIFT |
            cfg_param.ext_vsync << CSI_SENS_CONF_EXT_VSYNC_SHIFT | cfg_param.clock_mode << CSI_SENS_CONF_SENS_PRTCL_SHIFT |
            cfg_param.pack_tight << CSI_SENS_CONF_PACK_TIGHT_SHIFT | cfg_param.force_eof << CSI_SENS_CONF_FORCE_EOF_SHIFT |
    ...
    /* Set CCIR registers */
    ipu_csi_write(ipu, csi, (cfg_param.data_width == IPU_CSI_DATA_WIDTH_10) ? 0x03FF00000 : 0x00FF0000, CSI_CCIR_CODE_3);

    if (cfg_param.clock_mode == IPU_CSI_CLK_MODE_CCIR656_PROGRESSIVE) {
        /*
    ...
    } else if (cfg_param.clock_mode == IPU_CSI_CLK_MODE_CCIR656_INTERLACED) {
        if (width == 720 && height <= 525) {
            ...
            dev_err(
                ipu->dev, "Unsupported CCIR656 interlaced "
                          "video mode\n");
        ...
        (cfg_param.clock_mode == IPU_CSI_CLK_MODE_CCIR1120_PROGRESSIVE_DDR) || (cfg_param.clock_mode == IPU_CSI_CLK_MODE_CCIR1120_PROGRESSIVE_SDR) ||
        (cfg_param.clock_mode == IPU_CSI_CLK_MODE_CCIR1120_INTERLACED_DDR) || (cfg_param.clock_mode == IPU_CSI_CLK_MODE_CCIR1120_INTERLACED_SDR)) {
        ipu_csi_write(ipu, csi, (6 << 3) | (4 << 16), CSI_CCIR_CODE_1);
        _ipu_csi_ccir_err_detection_enable(ipu, csi);
    } else if ((cfg_param.clock_mode == IPU_CSI_CLK_MODE_GATED_CLK) || (cfg_param.clock_mode == IPU_CSI_CLK_MODE_NONGATED_CLK)) {
        ipu_csi_write(ipu, csi, (6 << 3) | (4 << 16), CSI_CCIR_CODE_1);
...
int32_t ipu_csi_get_sensor_protocol(struct ipu_soc *ipu, uint32_t csi)
{
    int ret;
    _ipu_get(ipu);
    ret = (ipu_csi_read(ipu, csi, CSI_SENS_CONF) & CSI_SENS_CONF_SENS_PRTCL_MASK) >> CSI_SENS_CONF_SENS_PRTCL_SHIFT;
    _ipu_put(ipu);
    return ret;
}
...
{
    /* Return immediately if there is no csi_clock to manage */
    if (ipu->csi_clock[csi] == NULL) {
        return 0;
    }

    if (flag) {
        clock_enable(ipu->csi_clock[csi]);

        if (wait == true) {
            msleep(10);
        }
    } else {
        clock_disable(ipu->csi_clock[csi]);
    }
 ...
 * @param       csi          csi 0 or csi 1
 */
void _ipu_csi_set_test_generator(
    struct ipu_soc *ipu, bool active, uint32_t r_value, uint32_t g_value, uint32_t b_value, uint32_t pix_clock, uint32_t csi)
{
    uint32_t temp;
    ...
    if (active == false) {
        ...
        _ipu_csi_mclock_set(ipu, pix_clock, csi);

        temp &= ~(CSI_TEST_GEN_R_MASK | CSI_TEST_GEN_G_MASK | CSI_TEST_GEN_B_MASK);
        temp |= CSI_TEST_GEN_MODE_EN;
        temp |= (r_value << CSI_TEST_GEN_R_SHIFT) | (g_value << CSI_TEST_GEN_G_SHIFT) | (b_value << CSI_TEST_GEN_B_SHIFT);
        ipu_csi_write(ipu, csi, temp, CSI_TST_CTRL);
    }
}
...
int _ipu_csi_set_skip_isp(struct ipu_soc *ipu, uint32_t skip, uint32_t max_ratio, uint32_t csi)
{
    ...
    temp |= (max_ratio << CSI_MAX_RATIO_SKIP_ISP_SHIFT) | (skip << CSI_SKIP_ISP_SHIFT);
...
int _ipu_csi_set_skip_smfc(struct ipu_soc *ipu, uint32_t skip, uint32_t max_ratio, uint32_t id, uint32_t csi)
{
    ...
    temp = ipu_csi_read(ipu, csi, CSI_SKIP);
    temp &= ~(CSI_MAX_RATIO_SKIP_SMFC_MASK | CSI_ID_2_SKIP_MASK | CSI_SKIP_SMFC_MASK);
    temp |= (max_ratio << CSI_MAX_RATIO_SKIP_SMFC_SHIFT) | (id << CSI_ID_2_SKIP_SHIFT) | (skip << CSI_SKIP_SMFC_SHIFT);
    ipu_csi_write(ipu, csi, temp, CSI_SKIP);
