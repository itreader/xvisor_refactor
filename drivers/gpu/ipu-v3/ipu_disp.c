...
{
    uint32_t dmfc_gen1 = ipu_dmfc_read(ipu, DMFC_GENERAL1);
...
{
    uint32_t dmfc_wr_chan = ipu_dmfc_read(ipu, DMFC_WR_CHAN);
    uint32_t dmfc_dp_chan = ipu_dmfc_read(ipu, DMFC_DP_CHAN);
    int      dmfc_bs      = 0;
...
{
    uint32_t reg;
    reg = (access_size << DI_DW_GEN_ACCESS_SIZE_OFFSET) | (component_size << DI_DW_GEN_COMPONENT_SIZE_OFFSET);
    ipu_di_write(ipu, di, reg, DI_DW_GEN(wave_gen));
}

static void _ipu_di_data_pin_config(struct ipu_soc *ipu, int di, int wave_gen, int di_pin, int set, int up, int down)
{
    uint32_t reg;
...
static void _ipu_di_sync_config(
    struct ipu_soc *ipu, int di, int wave_gen, int run_count, int run_src, int offset_count, int offset_src, int repeat_count, int cnt_clr_src,
    int cnt_polarity_gen_en, int cnt_polarity_clr_src, int cnt_polarity_trigger_src, int cnt_up, int cnt_down)
{
    uint32_t reg;

    if ((run_count >= 0x1000) || (offset_count >= 0x1000) || (repeat_count >= 0x1000) || (cnt_up >= 0x200) || (cnt_down >= 0x200)) {
...
static void _ipu_dc_map_link(
    struct ipu_soc *ipu, int current_map, int base_map_0, int buf_num_0, int base_map_1, int buf_num_1, int base_map_2, int buf_num_2)
{
...
static void _ipu_dc_map_config(struct ipu_soc *ipu, int map, int byte_num, int offset, int mask)
{
...
static void _ipu_dc_map_clear(struct ipu_soc *ipu, int map)
{
    ...
    ipu_dc_write(ipu, reg & ~(0xFFFF << (16 * (map & 0x1))), DC_MAP_CONF_PTR(map));
}

static void _ipu_dc_write_tmpl(struct ipu_soc *ipu, int word, uint32_t opcode, uint32_t operand, int map, int wave, int glue, int sync, int stop)
{
    uint32_t reg;
...
static void _ipu_dc_link_event(struct ipu_soc *ipu, int chan, int event, int addr, int priority)
{
    uint32_t reg;
    ...
    if (event < DC_EVEN_UGDE0) {
        ...
        if ((event - DC_EVEN_UGDE0) & 0x1) {
            ...
            if (priority) {
                chan = (chan >> 1) + ((((chan & 0x1) + ((chan & 0x2) >> 1))) | (chan >> 3));
            } else {
                chan = 0x7;
            }
...
#define mask_a(a) ((uint32_t)(a) & 0x3FF)
#define mask_b(b) ((uint32_t)(b) & 0x3FFF)
...
static struct dp_csc_param_t dp_csc_array[CSC_NUM][CSC_NUM] = {
    ...
    {{0, 0},
     ...
     {0, 0},
     {DP_COM_CONF_CSC_DEF_BG, (void *)&ycbcr2rgb_coeff}                                                                              },
    {{0, 0},                                               {DP_COM_CONF_CSC_DEF_FG, (void *)&ycbcr2rgb_coeff}, {0, 0}, {0, 0}, {0, 0}},
    {{DP_COM_CONF_CSC_DEF_FG, (void *)&rgb2ycbcr_coeff},   {0, 0},                                             {0, 0}, {0, 0}, {0, 0}},
    {{DP_COM_CONF_CSC_DEF_FG, (void *)&rgb2ycbcr_coeff},   {DP_COM_CONF_CSC_DEF_FG, (void *)&ycbcr2rgb_coeff}, {0, 0}, {0, 0}, {0, 0}}
};

void __ipu_dp_csc_setup(struct ipu_soc *ipu, int dp, struct dp_csc_param_t dp_csc_param, bool srm_mode_update)
{
    uint32_t reg;
    const int(*coeff)[5][3];
    ...
    if (coeff) {
        ipu_dp_write(ipu, mask_a((*coeff)[0][0]) | (mask_a((*coeff)[0][1]) << 16), DP_CSC_A_0(dp));
        ipu_dp_write(ipu, mask_a((*coeff)[0][2]) | (mask_a((*coeff)[1][0]) << 16), DP_CSC_A_1(dp));
        ipu_dp_write(ipu, mask_a((*coeff)[1][1]) | (mask_a((*coeff)[1][2]) << 16), DP_CSC_A_2(dp));
        ipu_dp_write(ipu, mask_a((*coeff)[2][0]) | (mask_a((*coeff)[2][1]) << 16), DP_CSC_A_3(dp));
        ipu_dp_write(ipu, mask_a((*coeff)[2][2]) | (mask_b((*coeff)[3][0]) << 16) | ((*coeff)[4][0] << 30), DP_CSC_0(dp));
        ipu_dp_write(ipu, mask_b((*coeff)[3][1]) | ((*coeff)[4][1] << 14) | (mask_b((*coeff)[3][2]) << 16) | ((*coeff)[4][2] << 30), DP_CSC_1(dp));
    }
...
int _ipu_dp_init(struct ipu_soc *ipu, ipu_channel_t channel, uint32_t in_pixel_fmt, uint32_t out_pixel_fmt)
{
    int      in_fmt, out_fmt;
    int      dp;
    int      partial = false;
    ...
    if (ipu->color_key_4rgb && (reg & DP_COM_CONF_GWCKE) &&
        (((ipu->fg_csc_type == RGB2YUV) && (ipu->bg_csc_type == YUV2YUV)) || ((ipu->fg_csc_type == YUV2YUV) && (ipu->bg_csc_type == RGB2YUV)) ||
         ((ipu->fg_csc_type == YUV2YUV) && (ipu->bg_csc_type == YUV2YUV)) || ((ipu->fg_csc_type == YUV2RGB) && (ipu->bg_csc_type == YUV2RGB)))) {
        int      red, green, blue;
        int      y, u, v;
    ...
    if ((dc_chan == 1) || (dc_chan == 5)) {
        if (interlaced) {
            ...
            if (di) {
                ...
                if ((pixel_fmt == IPU_PIX_FMT_YUYV) || (pixel_fmt == IPU_PIX_FMT_UYVY) || (pixel_fmt == IPU_PIX_FMT_YVYU) ||
...
int _ipu_disp_chan_is_interlaced(struct ipu_soc *ipu, ipu_channel_t channel)
{
    if (channel == MEM_DC_SYNC) {
        return !!(ipu_dc_read(ipu, DC_WR_CH_CONF_1) & DC_WR_CH_CONF_FIELD_MODE);
    } else if ((channel == MEM_BG_SYNC) || (channel == MEM_FG_SYNC)) {
        return !!(ipu_dc_read(ipu, DC_WR_CH_CONF_5) & DC_WR_CH_CONF_FIELD_MODE);
    }
...
void _ipu_dp_dc_enable(struct ipu_soc *ipu, ipu_channel_t channel)
{
    ...
    clock_prepare_enable(ipu->pixel_clock[di]);
}
...
void _ipu_dp_dc_disable(struct ipu_soc *ipu, ipu_channel_t channel, bool swap)
{
    ...
    if (channel == MEM_DC_SYNC) {
        ...
        if (ipu_is_channel_busy(ipu, MEM_BG_SYNC)) {
            ipu_cm_write(ipu, IPUIRQ_2_MASK(IPU_IRQ_DP_SF_END), IPUIRQ_2_STATREG(IPU_IRQ_DP_SF_END));

            while ((ipu_cm_read(ipu, IPUIRQ_2_STATREG(IPU_IRQ_DP_SF_END)) & IPUIRQ_2_MASK(IPU_IRQ_DP_SF_END)) == 0) {
                msleep(2);
                timeout -= 2;
 ...
 *
 * @param       pixel_clock       Desired pixel clock frequency in Hz.
 *
...
int32_t ipu_init_sync_panel(
    struct ipu_soc *ipu, int disp, uint32_t pixel_clock, uint16_t width, uint16_t height, uint32_t pixel_fmt, uint16_t h_start_width,
    uint16_t h_sync_width, uint16_t h_end_width, uint16_t v_start_width, uint16_t v_sync_width, uint16_t v_end_width, uint32_t v_to_h_sync,
    ipu_di_signal_cfg_t sig)
{
    ...
    uint32_t    div, rounded_pixel_clock;
    uint32_t    h_total, v_total;
    int         map;
    int         ret;
    struct clk *ldb_di0_clock, *ldb_di1_clock;
    ...
    di_parent = clock_get_parent(ipu->di_clock_sel[disp]);
    ...
    ldb_di0_clock = clock_get(ipu->dev, "ldb_di0");

    if (IS_ERR(ldb_di0_clock)) {
        dev_err(ipu->dev, "clock_get di0 failed");
        return PTR_ERR(ldb_di0_clock);
    }

    ldb_di1_clock = clock_get(ipu->dev, "ldb_di1");

    if (IS_ERR(ldb_di1_clock)) {
        dev_err(ipu->dev, "clock_get di1 failed");
        return PTR_ERR(ldb_di1_clock);
    }

    if (ldb_di0_clock == di_parent || ldb_di1_clock == di_parent) {
        /* if di clk parent is tve/ldb, then keep it;*/
        dev_dbg(ipu->dev, "use special clk parent\n");
        ret = clock_set_parent(ipu->pixel_clock_sel[disp], ipu->di_clock[disp]);
        ...
        clock_put(ldb_di0_clock);
        clock_put(ldb_di1_clock);
    } else {
        /* try ipu clk first*/
        dev_dbg(ipu->dev, "try ipu internal clk\n");
        ret = clock_set_parent(ipu->pixel_clock_sel[disp], ipu->ipu_clock);
        ...
        rounded_pixel_clock = clock_round_rate(ipu->pixel_clock[disp], pixel_clock);
         ...
         */
        if (!sig.int_clock &&
            ((rounded_pixel_clock >= pixel_clock + pixel_clock / 200) || (rounded_pixel_clock <= pixel_clock - pixel_clock / 200))) {
            dev_dbg(ipu->dev, "try ipu ext di clk\n");

            rounded_pixel_clock = clock_round_rate(ipu->di_clock[disp], pixel_clock);
            ret                 = clock_set_rate(ipu->di_clock[disp], rounded_pixel_clock);

            if (ret) {
                dev_err(ipu->dev, "set di clk rate error:%d\n", ret);
                return ret;
            }
            ...
            if (ret) {
                dev_err(ipu->dev, "set pixel clk parent error:%d\n", ret);
                return ret;
            }
        }
    }
    ...
    ret = clock_set_rate(ipu->pixel_clock[disp], rounded_pixel_clock);
    ...
    div = udiv64(clock_get_rate(clock_get_parent(ipu->pixel_clock_sel[disp])), rounded_pixel_clock);
    ...
    if (sig.interlaced) {
        if (g_ipu_hw_rev >= IPU_V3DEX) {
            ...
            /* Setup internal HSYNC waveform */
            _ipu_di_sync_config(ipu, disp, 1, h_total - 1, DI_SYNC_CLK, 0, DI_SYNC_NONE, 0, DI_SYNC_NONE, 0, DI_SYNC_NONE, DI_SYNC_NONE, 0, 0);

            field1_offset = v_sync_width + v_start_width + height / 2 + v_end_width;
            ...
            /* Field 1 VSYNC waveform */
            _ipu_di_sync_config(
                ipu, disp, 2, v_total - 1, 1, field0_offset, field0_offset ? 1 : DI_SYNC_NONE, 0, DI_SYNC_NONE, 0, DI_SYNC_NONE, DI_SYNC_NONE, 0, 4);

            /* Setup internal HSYNC waveform */
            _ipu_di_sync_config(ipu, disp, 3, h_total - 1, DI_SYNC_CLK, 0, DI_SYNC_NONE, 0, DI_SYNC_NONE, 0, DI_SYNC_NONE, DI_SYNC_NONE, 0, 4);

            /* Active Field ? */
            _ipu_di_sync_config(
                ipu, disp, 4, field0_offset ? field0_offset : field1_offset - 2, 1, v_start_width + v_sync_width, 1, 2, 2, 0, DI_SYNC_NONE,
                DI_SYNC_NONE, 0, 0);

            /* Active Line */
            _ipu_di_sync_config(ipu, disp, 5, 0, 1, 0, DI_SYNC_NONE, height / 2, 4, 0, DI_SYNC_NONE, DI_SYNC_NONE, 0, 0);

            /* Field 0 VSYNC waveform */
            _ipu_di_sync_config(ipu, disp, 6, v_total - 1, 1, 0, DI_SYNC_NONE, 0, DI_SYNC_NONE, 0, DI_SYNC_NONE, DI_SYNC_NONE, 0, 0);

            /* DC VSYNC waveform */
            vsync_cnt = 7;
            _ipu_di_sync_config(ipu, disp, 7, 0, 1, field1_offset, field1_offset ? 1 : DI_SYNC_NONE, 1, 2, 0, DI_SYNC_NONE, DI_SYNC_NONE, 0, 0);

            /* active pixel waveform */
            _ipu_di_sync_config(
                ipu, disp, 8, 0, DI_SYNC_CLK, h_sync_width + h_start_width, DI_SYNC_CLK, width, 5, 0, DI_SYNC_NONE, DI_SYNC_NONE, 0, 0);

            /* ??? */
            _ipu_di_sync_config(ipu, disp, 9, v_total - 1, 2, 0, DI_SYNC_NONE, 0, DI_SYNC_NONE, 6, DI_SYNC_NONE, DI_SYNC_NONE, 0, 0);

            reg = ipu_di_read(ipu, disp, DI_SW_GEN1(9));
            reg |= 0x8000;
            ipu_di_write(ipu, disp, reg, DI_SW_GEN1(9));

            ipu_di_write(ipu, disp, v_sync_width + v_start_width + v_end_width + height / 2 - 1, DI_SCR_CONF);
        }
        ...
        _ipu_di_sync_config(
            ipu, disp, DI_SYNC_HSYNC, h_total - 1, DI_SYNC_CLK, div * v_to_h_sync, DI_SYNC_CLK, 0, DI_SYNC_NONE, 1, DI_SYNC_NONE, DI_SYNC_CLK, 0,
            h_sync_width * 2);
        /* Setup VSYNC waveform */
        vsync_cnt = DI_SYNC_VSYNC;
        _ipu_di_sync_config(
            ipu, disp, DI_SYNC_VSYNC, v_total - 1, DI_SYNC_INT_HSYNC, 0, DI_SYNC_NONE, 0, DI_SYNC_NONE, 1, DI_SYNC_NONE, DI_SYNC_INT_HSYNC, 0,
            v_sync_width * 2);
        ipu_di_write(ipu, disp, v_total - 1, DI_SCR_CONF);

        /* Setup active data waveform to sync with DC */
        _ipu_di_sync_config(
            ipu, disp, 4, 0, DI_SYNC_HSYNC, v_sync_width + v_start_width, DI_SYNC_HSYNC, height, DI_SYNC_VSYNC, 0, DI_SYNC_NONE, DI_SYNC_NONE, 0, 0);
        _ipu_di_sync_config(ipu, disp, 5, 0, DI_SYNC_CLK, h_sync_width + h_start_width, DI_SYNC_CLK, width, 4, 0, DI_SYNC_NONE, DI_SYNC_NONE, 0, 0);

        /* set VGA delayed hsync/vsync no matter VGA enabled */
        if (disp) {
            /* couter 7 for VGA delay HSYNC */
            _ipu_di_sync_config(
                ipu, disp, 7, h_total - 1, DI_SYNC_CLK, 18, DI_SYNC_CLK, 0, DI_SYNC_NONE, 1, DI_SYNC_NONE, DI_SYNC_CLK, 0, h_sync_width * 2);

            /* couter 8 for VGA delay VSYNC */
            _ipu_di_sync_config(
                ipu, disp, 8, v_total - 1, DI_SYNC_INT_HSYNC, 1, DI_SYNC_INT_HSYNC, 0, DI_SYNC_NONE, 1, DI_SYNC_NONE, DI_SYNC_INT_HSYNC, 0,
                v_sync_width * 2);
        }
    ...
    if ((pixel_fmt == IPU_PIX_FMT_YUYV) || (pixel_fmt == IPU_PIX_FMT_UYVY) || (pixel_fmt == IPU_PIX_FMT_YVYU) || (pixel_fmt == IPU_PIX_FMT_VYUY)) {
        di_gen |= 0x00020000;
    }
    ...
    ipu_di_write(ipu, disp, (--vsync_cnt << DI_VSYNC_SEL_OFFSET) | 0x00000002, DI_SYNC_AS_GEN);
...
int ipu_init_async_panel(struct ipu_soc *ipu, int disp, int type, uint32_t cycle_time, uint32_t pixel_fmt, ipu_adc_sig_cfg_t sig)
{
    int      map;
    uint32_t ser_conf = 0;
    uint32_t div;
    uint32_t di_clock = clock_get_rate(ipu->ipu_clock);

    /* round up cycle_time, then calcalate the divider using scaled math */
    cycle_time += udiv64(1000000000UL, di_clock) - 1;
    div = (cycle_time * (di_clock / 256UL)) / (1000000000UL / 256UL);
    ...
    if (type == IPU_PANEL_SERIAL) {
        ipu_di_write(ipu, disp, (div << 24) | ((sig.ifc_width - 1) << 4), DI_DW_GEN(ASYNC_SER_WAVE));

        _ipu_di_data_pin_config(ipu, disp, ASYNC_SER_WAVE, DI_PIN_CS, 0, 0, (div * 2) + 1);
        _ipu_di_data_pin_config(ipu, disp, ASYNC_SER_WAVE, DI_PIN_SER_CLK, 1, div, div * 2);
        _ipu_di_data_pin_config(ipu, disp, ASYNC_SER_WAVE, DI_PIN_SER_RS, 2, 0, 0);
...
int32_t ipu_disp_set_global_alpha(struct ipu_soc *ipu, ipu_channel_t channel, bool enable, uint8_t alpha)
{
    ...
    if (channel == MEM_BG_SYNC || channel == MEM_BG_ASYNC0 || channel == MEM_BG_ASYNC1) {
        bg_chan = true;
    } else {
        bg_chan = false;
    }
    ...
    if (enable) {
        ...
        ipu_dp_write(ipu, reg | ((uint32_t)alpha << 24), DP_GRAPH_WIND_CTRL(flow));
...
int32_t ipu_disp_set_color_key(struct ipu_soc *ipu, ipu_channel_t channel, bool enable, uint32_t color_key)
{
    ...
    /* Transform color key from rgb to yuv if CSC is enabled */
    if (((ipu->fg_csc_type == RGB2YUV) && (ipu->bg_csc_type == YUV2YUV)) || ((ipu->fg_csc_type == YUV2YUV) && (ipu->bg_csc_type == RGB2YUV)) ||
        ((ipu->fg_csc_type == YUV2YUV) && (ipu->bg_csc_type == YUV2YUV)) || ((ipu->fg_csc_type == YUV2RGB) && (ipu->bg_csc_type == YUV2RGB))) {
...
int32_t ipu_disp_set_gamma_correction(struct ipu_soc *ipu, ipu_channel_t channel, bool enable, int constk[], int slopek[])
{
    ...
    for (i = 0; i < 4; i++) {
        ipu_dp_write(
            ipu, (slopek[4 * i] & 0xff) | ((slopek[4 * i + 1] & 0xff) << 8) | ((slopek[4 * i + 2] & 0xff) << 16) | ((slopek[4 * i + 3] & 0xff) << 24),
            DP_GAMMA_S(flow, i));
    }
...
int32_t ipu_disp_set_window_pos(struct ipu_soc *ipu, ipu_channel_t channel, int16_t x_pos, int16_t y_pos)
{
    int ret;
...
int32_t _ipu_disp_get_window_pos(struct ipu_soc *ipu, ipu_channel_t channel, int16_t *x_pos, int16_t *y_pos)
{
    uint32_t reg;
    uint32_t flow = 0;
...
int32_t ipu_disp_get_window_pos(struct ipu_soc *ipu, ipu_channel_t channel, int16_t *x_pos, int16_t *y_pos)
{
    int ret;
...
void ipu_disp_direct_write(struct ipu_soc *ipu, ipu_channel_t channel, uint32_t value, uint32_t offset)
{
