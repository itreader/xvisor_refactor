...
static inline void _ipu_ch_params_set_packing(
    struct ipu_ch_param *p, int red_width, int red_offset, int green_width, int green_offset, int blue_width, int blue_offset, int alpha_width,
    int alpha_offset)
{
...
static inline void _ipu_ch_param_dump(struct ipu_soc *ipu, int ch)
{
    ...
    /* struct ipu_ch_param *p = ipu_ch_param_addr(ipu, ch); */
    dev_dbg(
        ipu->dev, "ch %d word 0 - %08X %08X %08X %08X %08X\n", ch, p->word[0].data[0], p->word[0].data[1], p->word[0].data[2], p->word[0].data[3],
        p->word[0].data[4]);
    dev_dbg(
        ipu->dev, "ch %d word 1 - %08X %08X %08X %08X %08X\n", ch, p->word[1].data[0], p->word[1].data[1], p->word[1].data[2], p->word[1].data[3],
        p->word[1].data[4]);
...
static inline void _ipu_ch_param_init(
    struct ipu_soc *ipu, int ch, uint32_t pixel_fmt, uint32_t width, uint32_t height, uint32_t stride, uint32_t u, uint32_t v, uint32_t uv_stride,
    dma_addr_t addr0, dma_addr_t addr1, dma_addr_t addr2)
{
    uint32_t            u_offset = 0;
    uint32_t            v_offset = 0;
    int32_t             sub_ch   = 0;
    ...
    if (addr0 % 8) {
        dev_warn(ipu->dev, "IDMAC%d's EBA0 is not 8-byte aligned\n", ch);
    }

    if (addr1 % 8) {
        dev_warn(ipu->dev, "IDMAC%d's EBA1 is not 8-byte aligned\n", ch);
    }
    ...
    if (u_offset / 8 > 0x3fffff) {
        dev_warn(ipu->dev, "IDMAC%d's U offset exceeds IPU limitation\n", ch);
    }

    if (v_offset / 8 > 0x3fffff) {
        dev_warn(ipu->dev, "IDMAC%d's V offset exceeds IPU limitation\n", ch);
    }

    if (u_offset % 8) {
        dev_warn(ipu->dev, "IDMAC%d's U offset is not 8-byte aligned\n", ch);
    }

    if (v_offset % 8) {
        dev_warn(ipu->dev, "IDMAC%d's V offset is not 8-byte aligned\n", ch);
    }
...
static inline void _ipu_ch_param_set_burst_size(struct ipu_soc *ipu, uint32_t ch, uint16_t burst_pixels)
{
    ...
    ipu_ch_param_mod_field_io(ipu_ch_param_addr(ipu, ch), 1, 78, 7, burst_pixels - 1);
    ...
    ipu_ch_param_mod_field_io(ipu_ch_param_addr(ipu, sub_ch), 1, 78, 7, burst_pixels - 1);
};
...
static inline void _ipu_ch_param_set_buffer(struct ipu_soc *ipu, uint32_t ch, int bufNum, dma_addr_t phyaddr)
{
    ...
    ipu_ch_param_mod_field_io(ipu_ch_param_addr(ipu, ch), 1, 29 * bufNum, 29, phyaddr / 8);
};
...
static inline void _ipu_ch_param_set_alpha_buffer_memory(struct ipu_soc *ipu, uint32_t ch)
{
    ...
    switch (ch) {
        ...
        default:
            dev_err(
                ipu->dev, "unsupported correlative channel of local "
                          "alpha channel\n");
            return;
    }
...
static inline void _ipu_ch_param_set_interlaced_scan(struct ipu_soc *ipu, uint32_t ch)
{
    ...
    if (stride % 8) {
        dev_warn(ipu->dev, "IDMAC%d's ILO is not 8-byte aligned\n", ch);
    }

    ipu_ch_param_mod_field_io(ipu_ch_param_addr(ipu, ch), 1, 58, 20, stride / 8);

    if (sub_ch > 0) {
        ipu_ch_param_mod_field_io(ipu_ch_param_addr(ipu, sub_ch), 1, 58, 20, stride / 8);
    }
    ...
    if (sub_ch > 0) {
        ipu_ch_param_mod_field_io(ipu_ch_param_addr(ipu, sub_ch), 1, 102, 14, stride - 1);
    }
};
...
static inline void _ipu_ch_offset_update(
    struct ipu_soc *ipu, int ch, uint32_t pixel_fmt, uint32_t width, uint32_t height, uint32_t stride, uint32_t u, uint32_t v, uint32_t uv_stride,
    uint32_t vertical_offset, uint32_t horizontal_offset)
{
    ...
    switch (pixel_fmt) {
        ...
        case IPU_PIX_FMT_YUV420P:
            ...
            u_offset =
                stride * (height - vertical_offset - 1) + (stride - horizontal_offset) + (uv_stride * vertical_offset / 2) + horizontal_offset / 2;
            v_offset = u_offset + (uv_stride * height / 2);
            u_fix =
                u ? (u + (uv_stride * vertical_offset / 2) + (horizontal_offset / 2) - (stride * vertical_offset) - (horizontal_offset)) : u_offset;
            v_fix =
                v ? (v + (uv_stride * vertical_offset / 2) + (horizontal_offset / 2) - (stride * vertical_offset) - (horizontal_offset)) : v_offset;
        ...
        case IPU_PIX_FMT_YVU422P:
            ...
            v_offset = stride * (height - vertical_offset - 1) + (stride - horizontal_offset) + (uv_stride * vertical_offset) + horizontal_offset / 2;
            u_offset = v_offset + uv_stride * height;
            u_fix    = u ? (u + (uv_stride * vertical_offset) + horizontal_offset / 2 - (stride * vertical_offset) - (horizontal_offset)) : u_offset;
            v_fix    = v ? (v + (uv_stride * vertical_offset) + horizontal_offset / 2 - (stride * vertical_offset) - (horizontal_offset)) : v_offset;
            break;
        ...
        case IPU_PIX_FMT_YUV422P:
            ...
            u_offset = stride * (height - vertical_offset - 1) + (stride - horizontal_offset) + (uv_stride * vertical_offset) + horizontal_offset / 2;
        ...
        case IPU_PIX_FMT_YUV444P:
            uv_stride = stride;
            u_offset  = stride * (height - vertical_offset - 1) + (stride - horizontal_offset) + (uv_stride * vertical_offset) + horizontal_offset;
            v_offset  = u_offset + uv_stride * height;
            u_fix     = u ? (u + (uv_stride * vertical_offset) + horizontal_offset - (stride * vertical_offset) - (horizontal_offset)) : u_offset;
            v_fix     = v ? (v + (uv_stride * vertical_offset) + horizontal_offset - (stride * vertical_offset) - (horizontal_offset)) : v_offset;
            break;

        case IPU_PIX_FMT_NV12:
            uv_stride = stride;
            u_offset = stride * (height - vertical_offset - 1) + (stride - horizontal_offset) + (uv_stride * vertical_offset / 2) + horizontal_offset;
            u_fix    = u ? (u + (uv_stride * vertical_offset / 2) + horizontal_offset - (stride * vertical_offset) - (horizontal_offset)) : u_offset;
...
static inline void _ipu_ch_param_set_bandmode(struct ipu_soc *ipu, uint32_t ch, uint32_t band_height)
{
    ...
    ipu_ch_param_set_field_io(ipu_ch_param_addr(ipu, ch), 0, 114, 3, band_height - 1);
    sub_ch = __ipu_ch_get_third_buf_cpmem_num(ch);
    ...
    ipu_ch_param_set_field_io(ipu_ch_param_addr(ipu, sub_ch), 0, 114, 3, band_height - 1);

    dev_dbg(ipu->dev, "BNDM 0x%x, ", ipu_ch_param_read_field_io(ipu_ch_param_addr(ipu, ch), 0, 114, 3));
}
