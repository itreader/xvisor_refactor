...
#include <asm/sizes.h>
#include <drv/frame_buffer.h> /* For FB_CLASS_IPRIORITY */
#include <drv/reset.h>
...
{
    return (
        ((dma_chan == 8) || (dma_chan == 9) || (dma_chan == 10) || (dma_chan == 13) || (dma_chan == 21) || (dma_chan == 23) || (dma_chan == 27) ||
         (dma_chan == 28)) &&
...
{
    return ((dma_chan == 23) || (dma_chan == 24) || (dma_chan == 28) || (dma_chan == 41));
}
...
static inline int _ipu_is_sync_irq(uint32_t irq)
{
    ...
    return (
        (reg_num == 1) || (reg_num == 2) || (reg_num == 3) || (reg_num == 4) || (reg_num == 7) || (reg_num == 8) || (reg_num == 11) ||
        (reg_num == 12) || (reg_num == 13) || (reg_num == 14) || (reg_num == 15));
}
...
static const char *const pixel_clock_0[]           = {"ipu1_pclock_0", "ipu2_pclock_0"};
static const char *const pixel_clock_1[]           = {"ipu1_pclock_1", "ipu2_pclock_1"};
static const char *const pixel_clock_0_sel[]       = {"ipu1_pclk0_sel", "ipu2_pclk0_sel"};
static const char *const pixel_clock_1_sel[]       = {"ipu1_pclk1_sel", "ipu2_pclk1_sel"};
static const char *const pixel_clock_0_div[]       = {"ipu1_pclk0_div", "ipu2_pclk0_div"};
static const char *const pixel_clock_1_div[]       = {"ipu1_pclk1_div", "ipu2_pclk1_div"};
static const char *const ipu_pixel_clock_sel[2][3] = {
    {"ipu1", "ipu1_di0", "ipu1_di1"},
    {"ipu2", "ipu2_di0", "ipu2_di1"}
};

static int ipu_clock_setup_enable(struct ipu_soc *ipu, struct ipu_pltfm_data *pdata)
{
    struct clk *clk;
    int         ret;

    dev_dbg(ipu->dev, "ipu_clock = %lu\n", clock_get_rate(ipu->ipu_clock));

    clk = clock_register_mux_pix_clock(
        ipu->dev, pixel_clock_0_sel[pdata->id], (const char **)ipu_pixel_clock_sel[pdata->id], ARRAY_SIZE(ipu_pixel_clock_sel[pdata->id]), 0,
        pdata->id, 0, 0);

    if (VMM_IS_ERR_OR_NULL(clk)) {
        dev_err(ipu->dev, "clock_register mux di0 failed");
        return VMM_PTR_ERR(clk);
    }

    ipu->pixel_clock_sel[0] = clk;
    clk                     = clock_register_mux_pix_clock(
        ipu->dev, pixel_clock_1_sel[pdata->id], (const char **)ipu_pixel_clock_sel[pdata->id], ARRAY_SIZE(ipu_pixel_clock_sel[pdata->id]), 0,
        pdata->id, 1, 0);

    if (VMM_IS_ERR_OR_NULL(clk)) {
        dev_err(ipu->dev, "clock_register mux di1 failed");
        return VMM_PTR_ERR(clk);
    }

    ipu->pixel_clock_sel[1] = clk;

    clk                     = clock_register_div_pix_clock(ipu->dev, pixel_clock_0_div[pdata->id], pixel_clock_0_sel[pdata->id], 0, pdata->id, 0, 0);
    ...
    clk = clock_register_div_pix_clock(ipu->dev, pixel_clock_1_div[pdata->id], pixel_clock_1_sel[pdata->id], CLK_SET_RATE_PARENT, pdata->id, 1, 0);
    ...
    ipu->pixel_clock[0] =
        clock_register_gate_pix_clock(ipu->dev, pixel_clock_0[pdata->id], pixel_clock_0_div[pdata->id], CLK_SET_RATE_PARENT, pdata->id, 0, 0);

    if (VMM_IS_ERR_OR_NULL(ipu->pixel_clock[0])) {
        dev_err(ipu->dev, "clk register di0 gate failed");
        return VMM_PTR_ERR(ipu->pixel_clock[0]);
    }

    ipu->pixel_clock[1] =
        clock_register_gate_pix_clock(ipu->dev, pixel_clock_1[pdata->id], pixel_clock_1_div[pdata->id], CLK_SET_RATE_PARENT, pdata->id, 1, 0);

    if (VMM_IS_ERR_OR_NULL(ipu->pixel_clock[1])) {
        dev_err(ipu->dev, "clk register di1 gate failed");
        return VMM_PTR_ERR(ipu->pixel_clock[1]);
    }

    ret = clock_set_parent(ipu->pixel_clock_sel[0], ipu->ipu_clock);
    ...
    ret = clock_set_parent(ipu->pixel_clock_sel[1], ipu->ipu_clock);
    ...
    ipu->di_clock[0] = devm_clock_get(ipu->dev, "di0");

    if (VMM_IS_ERR_OR_NULL(ipu->di_clock[0])) {
        dev_err(ipu->dev, "clock_get di0 failed");
        return VMM_PTR_ERR(ipu->di_clock[0]);
    }

    ipu->di_clock[1] = devm_clock_get(ipu->dev, "di1");

    if (VMM_IS_ERR_OR_NULL(ipu->di_clock[1])) {
        dev_err(ipu->dev, "clock_get di1 failed");
        return VMM_PTR_ERR(ipu->di_clock[1]);
    }

    ipu->di_clock_sel[0] = devm_clock_get(ipu->dev, "di0_sel");

    if (VMM_IS_ERR_OR_NULL(ipu->di_clock_sel[0])) {
        dev_err(ipu->dev, "clock_get di0_sel failed");
        return VMM_PTR_ERR(ipu->di_clock_sel[0]);
    }

    ipu->di_clock_sel[1] = devm_clock_get(ipu->dev, "di1_sel");

    if (VMM_IS_ERR_OR_NULL(ipu->di_clock_sel[1])) {
        dev_err(ipu->dev, "clock_get di1_sel failed");
        return VMM_PTR_ERR(ipu->di_clock_sel[1]);
    }
...
void _ipu_get(struct ipu_soc *ipu)
{
    ...
    ret = clock_enable(ipu->ipu_clock);
...
{
    clock_disable(ipu->ipu_clock);
}

void ipu_disable_hsp_clock(struct ipu_soc *ipu)
{
    _ipu_put(ipu);
}
...
static const struct vmm_device_tree_nodeid imx_ipuv3_device_tree_ids[] = {
    {
     .compatible = "fsl,imx6q-ipu",
     .data       = &imx_ipu_type[IMX6Q_IPU],
     },
    {/* sentinel */                            }
};
MODULE_DEVICE_TABLE(of, imx_ipuv3_device_tree_ids);
 ...
 */
static int ipu_probe(vmm_device_t *dev)
{
    ...
    ipu->irq_sync = vmm_device_tree_irq_parse_map(dev->of_node, 0);
    ...
    ipu->cm_reg       = devm_ioremap(dev, ipu_base + IPU_CM_REG_BASE, VMM_PAGE_SIZE);
    ipu->ic_reg       = devm_ioremap(dev, ipu_base + IPU_IC_REG_BASE, VMM_PAGE_SIZE);
    ipu->idmac_reg    = devm_ioremap(dev, ipu_base + IPU_IDMAC_REG_BASE, VMM_PAGE_SIZE);
    /* DP Registers are accessed thru the SRM */
    ipu->dp_reg       = devm_ioremap(dev, ipu_base + IPU_SRM_REG_BASE, VMM_PAGE_SIZE);
    ipu->dc_reg       = devm_ioremap(dev, ipu_base + IPU_DC_REG_BASE, VMM_PAGE_SIZE);
    ipu->dmfc_reg     = devm_ioremap(dev, ipu_base + IPU_DMFC_REG_BASE, VMM_PAGE_SIZE);
    ipu->di_reg[0]    = devm_ioremap(dev, ipu_base + IPU_DI0_REG_BASE, VMM_PAGE_SIZE);
    ipu->di_reg[1]    = devm_ioremap(dev, ipu_base + IPU_DI1_REG_BASE, VMM_PAGE_SIZE);
    ipu->smfc_reg     = devm_ioremap(dev, ipu_base + IPU_SMFC_REG_BASE, VMM_PAGE_SIZE);
    ipu->csi_reg[0]   = devm_ioremap(dev, ipu_base + IPU_CSI0_REG_BASE, VMM_PAGE_SIZE);
    ipu->csi_reg[1]   = devm_ioremap(dev, ipu_base + IPU_CSI1_REG_BASE, VMM_PAGE_SIZE);
    ipu->cpmem_base   = devm_ioremap(dev, ipu_base + IPU_CPMEM_REG_BASE, SZ_128K);
    ipu->tpmem_base   = devm_ioremap(dev, ipu_base + IPU_TPM_REG_BASE, SZ_64K);
    ipu->dc_tmpl_reg  = devm_ioremap(dev, ipu_base + IPU_DC_TMPL_REG_BASE, SZ_128K);
    ipu->vdi_reg      = devm_ioremap(dev, ipu_base + IPU_VDI_REG_BASE, VMM_PAGE_SIZE);
    ipu->disp_base[1] = devm_ioremap(dev, ipu_base + IPU_DISP1_BASE, SZ_4K);
    ...
    ipu->ipu_clock = devm_clock_get(ipu->dev, "bus");

    if (VMM_IS_ERR_OR_NULL(ipu->ipu_clock)) {
        dev_err(ipu->dev, "clock_get ipu failed");
        return VMM_PTR_ERR(ipu->ipu_clock);
    }

    /* ipu_clock is always prepared */
    ret = clock_prepare_enable(ipu->ipu_clock);
    ...
    vmm_device_driver_set_data(dev, ipu);
    ...
    ret = ipu_clock_setup_enable(ipu, pltfm_data);
...
int ipu_remove(vmm_device_t *dev)
{
    struct ipu_soc *ipu = vmm_device_driver_get_data(dev);

    unregister_ipu_device(ipu, ipu->pdata->id);

    clock_put(ipu->ipu_clock);
    ...
    switch (channel) {
        ...
        case CSI_MEM3:
            ...
            if (params->csi_mem.interlaced) {
                ipu->chan_is_interlaced[channel_2_dma(channel, IPU_OUTPUT_BUFFER)] = true;
            } else {
                ipu->chan_is_interlaced[channel_2_dma(channel, IPU_OUTPUT_BUFFER)] = false;
            }
            ...
            if (params->csi_mem.mipi.en) {
                ipu_conf |= (1 << (IPU_CONF_CSI0_DATA_SOURCE_OFFSET + params->csi_mem.csi));
                _ipu_smfc_init(ipu, channel, params->csi_mem.mipi.vc, params->csi_mem.csi);
                _ipu_csi_set_mipi_di(ipu, params->csi_mem.mipi.vc, params->csi_mem.mipi.id, params->csi_mem.csi);
            } else {
                ipu_conf &= ~(1 << (IPU_CONF_CSI0_DATA_SOURCE_OFFSET + params->csi_mem.csi));
        ...
        case CSI_PRP_ENC_MEM:
            ...
            if (params->csi_prp_enc_mem.mipi.en) {
                ipu_conf |= (1 << (IPU_CONF_CSI0_DATA_SOURCE_OFFSET + params->csi_prp_enc_mem.csi));
                _ipu_csi_set_mipi_di(ipu, params->csi_prp_enc_mem.mipi.vc, params->csi_prp_enc_mem.mipi.id, params->csi_prp_enc_mem.csi);
            } else {
                ipu_conf &= ~(1 << (IPU_CONF_CSI0_DATA_SOURCE_OFFSET + params->csi_prp_enc_mem.csi));
            }
        ...
        case CSI_PRP_VF_MEM:
            ...
            if (params->csi_prp_vf_mem.mipi.en) {
                ipu_conf |= (1 << (IPU_CONF_CSI0_DATA_SOURCE_OFFSET + params->csi_prp_vf_mem.csi));
                _ipu_csi_set_mipi_di(ipu, params->csi_prp_vf_mem.mipi.vc, params->csi_prp_vf_mem.mipi.id, params->csi_prp_vf_mem.csi);
            } else {
                ipu_conf &= ~(1 << (IPU_CONF_CSI0_DATA_SOURCE_OFFSET + params->csi_prp_vf_mem.csi));
            }
        ...
        case MEM_PRP_VF_MEM:
            if (params->mem_prp_vf_mem.graphics_combine_en) {
                ...
                bad_pixfmt     = _ipu_ch_param_bad_alpha_pos(in_g_pixel_fmt);
        ...
        case MEM_DC_SYNC:
            ...
            _ipu_dc_init(ipu, 1, params->mem_dc_sync.di, params->mem_dc_sync.interlaced, params->mem_dc_sync.out_pixel_fmt);
        ...
        case MEM_BG_SYNC:
            ...
            _ipu_dp_init(ipu, channel, params->mem_dp_bg_sync.in_pixel_fmt, params->mem_dp_bg_sync.out_pixel_fmt);
            _ipu_dc_init(ipu, 5, params->mem_dp_bg_sync.di, params->mem_dp_bg_sync.interlaced, params->mem_dp_bg_sync.out_pixel_fmt);
        ...
        case MEM_FG_SYNC:
            _ipu_dp_init(ipu, channel, params->mem_dp_fg_sync.in_pixel_fmt, params->mem_dp_fg_sync.out_pixel_fmt);
...
void ipu_uninit_channel(struct ipu_soc *ipu, ipu_channel_t channel)
{
    ...
    if (idma_is_set(ipu, IDMAC_CHA_EN, in_dma) || idma_is_set(ipu, IDMAC_CHA_EN, out_dma)) {
        dev_err(ipu->dev, "Channel %d is not disabled, disable first\n", IPU_CHAN_ID(channel));
        mutex_unlock(&ipu->mutex_lock);
        return;
    }
    ...
    if (_ipu_is_primary_disp_chan(in_dma)) {
        clock_disable_unprepare(ipu->pixel_clock[ipu->dc_di_assignment[dc_chan]]);
    }
 ...
 * @return      Returns 0 on success or negative error code on fail
 */
int32_t ipu_init_channel_buffer(
    struct ipu_soc *ipu, ipu_channel_t channel, ipu_buffer_t type, uint32_t pixel_fmt, uint16_t width, uint16_t height, uint32_t stride,
    ipu_rotate_mode_t rot_mode, dma_addr_t phyaddr_0, dma_addr_t phyaddr_1, dma_addr_t phyaddr_2, uint32_t u, uint32_t v)
{
    uint32_t reg;
    uint32_t dma_chan;
    uint32_t burst_size;
    ...
    if ((_ipu_is_ic_graphic_chan(dma_chan) || _ipu_is_dp_graphic_chan(dma_chan)) && (ipu->thrd_chan_en[IPU_CHAN_ID(channel)] == true)) {
    ...
    } else if (_ipu_is_ic_graphic_chan(dma_chan) && ipu_pixel_format_has_alpha(pixel_fmt)) {
    ...
    if (_ipu_disp_chan_is_interlaced(ipu, channel) || ipu->chan_is_interlaced[dma_chan]) {
        _ipu_ch_param_set_interlaced_scan(ipu, dma_chan);
    }

    if (_ipu_is_ic_chan(dma_chan) || _ipu_is_irt_chan(dma_chan) || _ipu_is_vdi_out_chan(dma_chan)) {
        burst_size = _ipu_ch_param_get_burst_size(ipu, dma_chan);
        _ipu_ic_idma_init(ipu, dma_chan, width, height, burst_size, rot_mode);
    ...
    if (phyaddr_2 && g_ipu_hw_rev >= IPU_V3DEX) {
        ...
        /* Reset to buffer 0 */
        ipu_cm_write(ipu, tri_cur_buf_mask(dma_chan), IPU_CHA_TRIPLE_CUR_BUF(dma_chan));
    } else {
        reg = ipu_cm_read(ipu, IPU_CHA_TRB_MODE_SEL(dma_chan));
        reg &= ~idma_mask(dma_chan);
        ipu_cm_write(ipu, reg, IPU_CHA_TRB_MODE_SEL(dma_chan));
        ...
        /* Reset to buffer 0 */
        ipu_cm_write(ipu, idma_mask(dma_chan), IPU_CHA_CUR_BUF(dma_chan));
    }
 ...
 */
int32_t ipu_update_channel_buffer(struct ipu_soc *ipu, ipu_channel_t channel, ipu_buffer_t type, uint32_t bufNum, dma_addr_t phyaddr)
{
    uint32_t reg;
    int      ret      = 0;
 ...
 *              fail.
 */
int32_t ipu_set_channel_bandmode(struct ipu_soc *ipu, ipu_channel_t channel, ipu_buffer_t type, uint32_t band_height)
{
    uint32_t reg;
    int      ret      = 0;
    uint32_t dma_chan = channel_2_dma(channel, type);
...
int32_t ipu_update_channel_offset(
    struct ipu_soc *ipu, ipu_channel_t channel, ipu_buffer_t type, uint32_t pixel_fmt, uint16_t width, uint16_t height, uint32_t stride, uint32_t u,
    uint32_t v, uint32_t vertical_offset, uint32_t horizontal_offset)
{
    int      ret      = 0;
    uint32_t dma_chan = channel_2_dma(channel, type);
    uint64_t lock_flags;
 ...
 * @return      Returns 0 on success or negative error code on fail
 */
int32_t ipu_select_buffer(struct ipu_soc *ipu, ipu_channel_t channel, ipu_buffer_t type, uint32_t bufNum)
{
    uint32_t dma_chan = channel_2_dma(channel, type);
    uint64_t lock_flags;
    ...
    if (bufNum == 0) {
        ipu_cm_write(ipu, idma_mask(dma_chan), IPU_CHA_BUF0_RDY(dma_chan));
    } else if (bufNum == 1) {
        ipu_cm_write(ipu, idma_mask(dma_chan), IPU_CHA_BUF1_RDY(dma_chan));
    } else {
        ipu_cm_write(ipu, idma_mask(dma_chan), IPU_CHA_BUF2_RDY(dma_chan));
    }
...
int32_t ipu_select_multi_vdi_buffer(struct ipu_soc *ipu, uint32_t bufNum)
{
    ...
    uint32_t mask_bit = idma_mask(channel_2_dma(MEM_VDI_PRP_VF_MEM_P, IPU_INPUT_BUFFER)) | idma_mask(dma_chan) |
...
static int proc_src_sel[]  = {0, 6, 7, 6, 7, 8, 5, NA, NA, NA, NA, NA, NA, NA, NA, 1, 2, 3, 4, 7, 8, NA, 8, NA};
static int disp_src_sel[]  = {0, 6, 7, 8, 3, 4, 5, NA, NA, NA, NA, NA, NA, NA, NA, 1, NA, 2, NA, 3, 4, 4, 4, 4};
...
int32_t ipu_link_channels(struct ipu_soc *ipu, ipu_channel_t src_ch, ipu_channel_t dest_ch)
{
    ...
    switch (src_ch) {
        case CSI_MEM0:
            ...
            fs_proc_flow3 |= proc_dest_sel[IPU_CHAN_ID(dest_ch)] << FS_SMFC0_DEST_SEL_OFFSET;
            break;

        case CSI_MEM1:
            fs_proc_flow3 &= ~FS_SMFC1_DEST_SEL_MASK;
            fs_proc_flow3 |= proc_dest_sel[IPU_CHAN_ID(dest_ch)] << FS_SMFC1_DEST_SEL_OFFSET;
            break;

        case CSI_MEM2:
            fs_proc_flow3 &= ~FS_SMFC2_DEST_SEL_MASK;
            fs_proc_flow3 |= proc_dest_sel[IPU_CHAN_ID(dest_ch)] << FS_SMFC2_DEST_SEL_OFFSET;
            break;

        case CSI_MEM3:
            fs_proc_flow3 &= ~FS_SMFC3_DEST_SEL_MASK;
            fs_proc_flow3 |= proc_dest_sel[IPU_CHAN_ID(dest_ch)] << FS_SMFC3_DEST_SEL_OFFSET;
            break;

        case CSI_PRP_ENC_MEM:
            fs_proc_flow2 &= ~FS_PRPENC_DEST_SEL_MASK;
            fs_proc_flow2 |= proc_dest_sel[IPU_CHAN_ID(dest_ch)] << FS_PRPENC_DEST_SEL_OFFSET;
            break;

        case CSI_PRP_VF_MEM:
            fs_proc_flow2 &= ~FS_PRPVF_DEST_SEL_MASK;
            fs_proc_flow2 |= proc_dest_sel[IPU_CHAN_ID(dest_ch)] << FS_PRPVF_DEST_SEL_OFFSET;
            break;

        case MEM_PP_MEM:
            fs_proc_flow2 &= ~FS_PP_DEST_SEL_MASK;
            fs_proc_flow2 |= proc_dest_sel[IPU_CHAN_ID(dest_ch)] << FS_PP_DEST_SEL_OFFSET;
            break;

        case MEM_ROT_PP_MEM:
            fs_proc_flow2 &= ~FS_PP_ROT_DEST_SEL_MASK;
            fs_proc_flow2 |= proc_dest_sel[IPU_CHAN_ID(dest_ch)] << FS_PP_ROT_DEST_SEL_OFFSET;
            break;
        ...
        case MEM_ROT_ENC_MEM:
            ...
            fs_proc_flow2 |= proc_dest_sel[IPU_CHAN_ID(dest_ch)] << FS_PRPENC_ROT_DEST_SEL_OFFSET;
            break;
        ...
        case MEM_ROT_VF_MEM:
            ...
            fs_proc_flow2 |= proc_dest_sel[IPU_CHAN_ID(dest_ch)] << FS_PRPVF_ROT_DEST_SEL_OFFSET;
            break;
    ...
    switch (dest_ch) {
        case MEM_PP_MEM:
            ...
            if (MEM_VDOA_MEM == src_ch) {
                ...
                fs_proc_flow1 |= proc_src_sel[IPU_CHAN_ID(src_ch)] << FS_PP_SRC_SEL_OFFSET;
            }
        ...
        case MEM_ROT_PP_MEM:
            ...
            fs_proc_flow1 |= proc_src_sel[IPU_CHAN_ID(src_ch)] << FS_PP_ROT_SRC_SEL_OFFSET;
            break;

        case MEM_PRP_ENC_MEM:
            fs_proc_flow1 &= ~FS_PRP_SRC_SEL_MASK;
            fs_proc_flow1 |= proc_src_sel[IPU_CHAN_ID(src_ch)] << FS_PRP_SRC_SEL_OFFSET;
            break;

        case MEM_ROT_ENC_MEM:
            fs_proc_flow1 &= ~FS_PRPENC_ROT_SRC_SEL_MASK;
            fs_proc_flow1 |= proc_src_sel[IPU_CHAN_ID(src_ch)] << FS_PRPENC_ROT_SRC_SEL_OFFSET;
            break;
        ...
        case MEM_ROT_VF_MEM:
            ...
            fs_proc_flow1 |= proc_src_sel[IPU_CHAN_ID(src_ch)] << FS_PRPVF_ROT_SRC_SEL_OFFSET;
            break;

        case MEM_DC_SYNC:
            fs_disp_flow1 &= ~FS_DC1_SRC_SEL_MASK;
            fs_disp_flow1 |= disp_src_sel[IPU_CHAN_ID(src_ch)] << FS_DC1_SRC_SEL_OFFSET;
            break;

        case MEM_BG_SYNC:
            fs_disp_flow1 &= ~FS_DP_SYNC0_SRC_SEL_MASK;
            fs_disp_flow1 |= disp_src_sel[IPU_CHAN_ID(src_ch)] << FS_DP_SYNC0_SRC_SEL_OFFSET;
            break;

        case MEM_FG_SYNC:
            fs_disp_flow1 &= ~FS_DP_SYNC1_SRC_SEL_MASK;
            fs_disp_flow1 |= disp_src_sel[IPU_CHAN_ID(src_ch)] << FS_DP_SYNC1_SRC_SEL_OFFSET;
            break;

        case MEM_DC_ASYNC:
            fs_disp_flow1 &= ~FS_DC2_SRC_SEL_MASK;
            fs_disp_flow1 |= disp_src_sel[IPU_CHAN_ID(src_ch)] << FS_DC2_SRC_SEL_OFFSET;
            break;

        case MEM_BG_ASYNC0:
            fs_disp_flow1 &= ~FS_DP_ASYNC0_SRC_SEL_MASK;
            fs_disp_flow1 |= disp_src_sel[IPU_CHAN_ID(src_ch)] << FS_DP_ASYNC0_SRC_SEL_OFFSET;
            break;

        case MEM_FG_ASYNC0:
            fs_disp_flow1 &= ~FS_DP_ASYNC1_SRC_SEL_MASK;
            fs_disp_flow1 |= disp_src_sel[IPU_CHAN_ID(src_ch)] << FS_DP_ASYNC1_SRC_SEL_OFFSET;
            break;
...
int32_t ipu_enable_channel(struct ipu_soc *ipu, ipu_channel_t channel)
{
    ...
    if ((ipu->sec_chan_en[IPU_CHAN_ID(channel)]) && ((channel == MEM_PP_MEM) || (channel == MEM_PRP_VF_MEM) || (channel == MEM_VDI_PRP_VF_MEM))) {
        sec_dma = channel_2_dma(channel, IPU_GRAPH_IN_BUFFER);
        reg     = ipu_idmac_read(ipu, IDMAC_CHA_EN(sec_dma));
        ipu_idmac_write(ipu, reg | idma_mask(sec_dma), IDMAC_CHA_EN(sec_dma));
    }

    if ((ipu->thrd_chan_en[IPU_CHAN_ID(channel)]) && ((channel == MEM_PP_MEM) || (channel == MEM_PRP_VF_MEM))) {
    ...
    } else if ((ipu->thrd_chan_en[IPU_CHAN_ID(channel)]) && ((channel == MEM_BG_SYNC) || (channel == MEM_FG_SYNC))) {
        thrd_dma = channel_2_dma(channel, IPU_ALPHA_IN_BUFFER);
        reg      = ipu_idmac_read(ipu, IDMAC_CHA_EN(thrd_dma));
        ipu_idmac_write(ipu, reg | idma_mask(thrd_dma), IDMAC_CHA_EN(thrd_dma));
        reg = ipu_idmac_read(ipu, IDMAC_SEP_ALPHA);
    ...
    if ((channel == MEM_DC_SYNC) || (channel == MEM_BG_SYNC) || (channel == MEM_FG_SYNC)) {
    ...
    if (_ipu_is_ic_chan(in_dma) || _ipu_is_ic_chan(out_dma) || _ipu_is_irt_chan(in_dma) || _ipu_is_irt_chan(out_dma) ||
 ...
 *
 */
int32_t ipu_check_buffer_ready(struct ipu_soc *ipu, ipu_channel_t channel, ipu_buffer_t type, uint32_t bufNum)
{
    uint32_t dma_chan = channel_2_dma(channel, type);
    uint32_t reg;
    uint64_t lock_flags;
 ...
 *
 */
void _ipu_clear_buffer_ready(struct ipu_soc *ipu, ipu_channel_t channel, ipu_buffer_t type, uint32_t bufNum)
{
    ...
    if (bufNum == 0) {
        ...
        ipu_cm_write(ipu, idma_mask(dma_ch), IPU_CHA_BUF1_RDY(dma_ch));
    } else {
        ipu_cm_write(ipu, idma_mask(dma_ch), IPU_CHA_BUF2_RDY(dma_ch));
    }

    ipu_cm_write(ipu, 0x0, IPU_GPR); /* write one to set */
}

void ipu_clear_buffer_ready(struct ipu_soc *ipu, ipu_channel_t channel, ipu_buffer_t type, uint32_t bufNum)
{
    uint64_t lock_flags;
...
int32_t ipu_disable_channel(struct ipu_soc *ipu, ipu_channel_t channel, bool wait_for_stop)
{
    ...
    if ((channel == MEM_BG_SYNC) || (channel == MEM_FG_SYNC) || (channel == MEM_DC_SYNC)) {
        ...
        if ((channel == MEM_FG_SYNC) && (ipu_is_channel_busy(ipu, MEM_BG_SYNC))) {
            ...
            ipu_cm_write(ipu, IPUIRQ_2_MASK(IPU_IRQ_BG_SYNC_EOF), IPUIRQ_2_STATREG(IPU_IRQ_BG_SYNC_EOF));

            while ((ipu_cm_read(ipu, IPUIRQ_2_STATREG(IPU_IRQ_BG_SYNC_EOF)) & IPUIRQ_2_MASK(IPU_IRQ_BG_SYNC_EOF)) == 0) {
                vmm_msleep(10);
                timeout -= 10;
    ...
    } else if (wait_for_stop && !_ipu_is_smfc_chan(out_dma) && channel != CSI_PRP_VF_MEM && channel != CSI_PRP_ENC_MEM) {
        while (idma_is_set(ipu, IDMAC_CHA_BUSY, in_dma) || idma_is_set(ipu, IDMAC_CHA_BUSY, out_dma) ||
               (ipu->sec_chan_en[IPU_CHAN_ID(channel)] && idma_is_set(ipu, IDMAC_CHA_BUSY, sec_dma)) ||
               (ipu->thrd_chan_en[IPU_CHAN_ID(channel)] && idma_is_set(ipu, IDMAC_CHA_BUSY, thrd_dma))) {
            ...
            if (ipu->sec_chan_en[IPU_CHAN_ID(channel)] && idma_is_set(ipu, IDMAC_CHA_BUSY, sec_dma)) {
                irq = sec_dma;
            }

            if (ipu->thrd_chan_en[IPU_CHAN_ID(channel)] && idma_is_set(ipu, IDMAC_CHA_BUSY, thrd_dma)) {
                irq = thrd_dma;
            }
            ...
            while (((ipu_cm_read(ipu, IPUIRQ_2_STATREG(irq)) & IPUIRQ_2_MASK(irq)) == 0) && (idma_is_set(ipu, IDMAC_CHA_BUSY, irq))) {
                vmm_udelay(10);
                timeout -= 10;
    ...
    if (idma_is_valid(in_dma)) {
        ...
        ipu_cm_write(ipu, tri_cur_buf_mask(in_dma), IPU_CHA_TRIPLE_CUR_BUF(in_dma));
    }
    ...
    if (idma_is_valid(out_dma)) {
        ...
        ipu_cm_write(ipu, tri_cur_buf_mask(out_dma), IPU_CHA_TRIPLE_CUR_BUF(out_dma));
    }
...
int ipu_enable_irq(struct ipu_soc *ipu, uint32_t irq)
{
    ...
    spin_lock_irq_save(&ipu->int_reg_spin_lock, lock_flags);
...
out:
    spin_unlock_irq_restore(&ipu->int_reg_spin_lock, lock_flags);
 ...
 */
int ipu_request_irq(
    struct ipu_soc *ipu, uint32_t irq, vmm_irq_return_t (*handler)(int, void *), uint32_t irq_flags, const char *devname, void *dev_id)
{
    uint32_t reg;
    uint64_t lock_flags;
    int      ret = 0;
...
uint32_t ipu_get_cur_buffer_idx(struct ipu_soc *ipu, ipu_channel_t channel, ipu_buffer_t type)
{
    ...
    if ((reg & idma_mask(dma_chan)) && _ipu_is_trb_chan(dma_chan)) {
        ...
        return (reg & tri_cur_buf_mask(dma_chan)) >> tri_cur_buf_shift(dma_chan);
    } else {
...
uint32_t _ipu_channel_status(struct ipu_soc *ipu, ipu_channel_t channel)
{
    ...
    switch (channel) {
        ...
        case MEM_ROT_VF_MEM:
            stat = (task_stat_reg & TSTAT_VF_ROT_MASK) >> TSTAT_VF_ROT_OFFSET;
            break;
        ...
        case MEM_ROT_ENC_MEM:
            stat = (task_stat_reg & TSTAT_ENC_ROT_MASK) >> TSTAT_ENC_ROT_OFFSET;
            break;
        ...
        case MEM_ROT_PP_MEM:
            stat = (task_stat_reg & TSTAT_PP_ROT_MASK) >> TSTAT_PP_ROT_OFFSET;
            break;
...
int32_t ipu_swap_channel(struct ipu_soc *ipu, ipu_channel_t from_ch, ipu_channel_t to_ch)
{
    ...
    ipu_cm_write(ipu, tri_cur_buf_mask(from_dma), IPU_CHA_TRIPLE_CUR_BUF(from_dma));
...
static const struct dev_pm_ops ipu_pm_ops = {SET_RUNTIME_PM_OPS(ipu_runtime_suspend, ipu_runtime_resume, NULL)
 ...
 */
static vmm_driver_t mxcipu_driver = {
    .name        = "imx-ipuv3",
    .match_table = imx_ipuv3_device_tree_ids,
#ifdef CONFIG_PM
    .pm = &ipu_pm_ops,
#endif
    .probe  = ipu_probe,
...
{
    return vmm_device_driver_register_driver(&mxcipu_driver);
}

static void __exit ipu_gen_uninit(void)
{
    vmm_device_driver_unregister_driver(&mxcipu_driver);
}
