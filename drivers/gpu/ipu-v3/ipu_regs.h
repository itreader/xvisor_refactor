...
#define IPU_MAX_VDI_IN_WIDTH        ({ g_ipu_hw_rev >= 3 ? (968) : (720); })
#define IPU_DISP0_BASE              0x00000000
#define IPU_MCU_T_DEFAULT           8
#define IPU_DISP1_BASE              ({ g_ipu_hw_rev < 4 ? (IPU_MCU_T_DEFAULT << 25) : (0x00000000); })
...
#define IPU_CPMEM_REG_BASE          ({ g_ipu_hw_rev >= 4 ? (0x00100000) : (0x01000000); })
#define IPU_LUT_REG_BASE            0x01020000
#define IPU_SRM_REG_BASE            ({ g_ipu_hw_rev >= 4 ? (0x00140000) : (0x01040000); })
#define IPU_TPM_REG_BASE            ({ g_ipu_hw_rev >= 4 ? (0x00160000) : (0x01060000); })
#define IPU_DC_TMPL_REG_BASE        ({ g_ipu_hw_rev >= 4 ? (0x00180000) : (0x01080000); })
 ...
 */
#define IPU_CHA_TRB_MODE_SEL(ch)    IPU_CM_REG({ g_ipu_hw_rev >= 2 ? (0x0178 + 4 * ((ch) / 32)) : (0x012C); })
#define IPU_CHA_TRIPLE_CUR_BUF(ch)  IPU_CM_REG({ g_ipu_hw_rev >= 2 ? (0x0258 + 4 * (((ch) * 2) / 32)) : (0x012C); })
#define IPU_CHA_BUF2_RDY(ch)        IPU_CM_REG({ g_ipu_hw_rev >= 2 ? (0x0288 + 4 * ((ch) / 32)) : (0x012C); })
#define IPU_CHA_CUR_BUF(ch)         IPU_CM_REG({ g_ipu_hw_rev >= 2 ? (0x023C + 4 * ((ch) / 32)) : (0x0124 + 4 * ((ch) / 32)); })
#define IPU_ALT_CUR_BUF0            IPU_CM_REG({ g_ipu_hw_rev >= 2 ? (0x0244) : (0x012C); })
#define IPU_ALT_CUR_BUF1            IPU_CM_REG({ g_ipu_hw_rev >= 2 ? (0x0248) : (0x0130); })
#define IPU_SRM_STAT                IPU_CM_REG({ g_ipu_hw_rev >= 2 ? (0x024C) : (0x0134); })
#define IPU_PROC_TASK_STAT          IPU_CM_REG({ g_ipu_hw_rev >= 2 ? (0x0250) : (0x0138); })
#define IPU_DISP_TASK_STAT          IPU_CM_REG({ g_ipu_hw_rev >= 2 ? (0x0254) : (0x013C); })
#define IPU_CHA_BUF0_RDY(ch)        IPU_CM_REG({ g_ipu_hw_rev >= 2 ? (0x0268 + 4 * ((ch) / 32)) : (0x0140 + 4 * ((ch) / 32)); })
#define IPU_CHA_BUF1_RDY(ch)        IPU_CM_REG({ g_ipu_hw_rev >= 2 ? (0x0270 + 4 * ((ch) / 32)) : (0x0148 + 4 * ((ch) / 32)); })
#define IPU_ALT_CHA_BUF0_RDY(ch)    IPU_CM_REG({ g_ipu_hw_rev >= 2 ? (0x0278 + 4 * ((ch) / 32)) : (0x0158 + 4 * ((ch) / 32)); })
#define IPU_ALT_CHA_BUF1_RDY(ch)    IPU_CM_REG({ g_ipu_hw_rev >= 2 ? (0x0280 + 4 * ((ch) / 32)) : (0x0160 + 4 * ((ch) / 32)); })

#define IPU_INT_CTRL(n)             IPU_CM_REG(0x003C + 4 * ((n) - 1))
#define IPU_INT_STAT(n)             IPU_CM_REG({ g_ipu_hw_rev >= 2 ? (0x0200 + 4 * ((n) - 1)) : (0x00E8 + 4 * ((n) - 1)); })
...
#define IDMAC_CH_LOCK_EN_1          IPU_IDMAC_REG({ g_ipu_hw_rev >= 2 ? (0x0024) : 0; })
#define IDMAC_CH_LOCK_EN_2          IPU_IDMAC_REG({ g_ipu_hw_rev >= 2 ? (0x0028) : (0x0024); })
#define IDMAC_SUB_ADDR_0            IPU_IDMAC_REG({ g_ipu_hw_rev >= 2 ? (0x002C) : (0x0028); })
#define IDMAC_SUB_ADDR_1            IPU_IDMAC_REG({ g_ipu_hw_rev >= 2 ? (0x0030) : (0x002C); })
#define IDMAC_SUB_ADDR_2            IPU_IDMAC_REG({ g_ipu_hw_rev >= 2 ? (0x0034) : (0x0030); })
/*
 * IPUv3D doesn't support IDMAC_SUB_ADDR_3 and IDMAC_SUB_ADDR_4,
 * so point them to readonly IDMAC_CHA_BUSY1 for IPUv3D.
 */
#define IDMAC_SUB_ADDR_3            IPU_IDMAC_REG({ g_ipu_hw_rev >= 2 ? (0x0038) : (0x0040); })
#define IDMAC_SUB_ADDR_4            IPU_IDMAC_REG({ g_ipu_hw_rev >= 2 ? (0x003C) : (0x0040); })
#define IDMAC_BAND_EN(ch)           IPU_IDMAC_REG({ g_ipu_hw_rev >= 2 ? (0x0040 + 4 * ((ch) / 32)) : (0x0034 + 4 * ((ch) / 32)); })
#define IDMAC_CHA_BUSY(ch)          IPU_IDMAC_REG({ g_ipu_hw_rev >= 2 ? (0x0100 + 4 * ((ch) / 32)) : (0x0040 + 4 * ((ch) / 32)); })
