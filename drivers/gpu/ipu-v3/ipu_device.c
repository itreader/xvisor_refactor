...
struct ipu_channel_tabel {
    vmm_mutex_t lock;
    uint8_t     used[MXC_IPU_MAX_NUM][MAX_PP_CH];
    uint8_t     vdoa_used;
};

static vmm_class_t             *ipu_class = NULL;
...
struct stripe_setting {
    ...
    uint32_t i_left_pos;
    uint32_t i_right_pos;
    uint32_t i_top_pos;
    uint32_t i_bottom_pos;
    uint32_t o_left_pos;
    uint32_t o_right_pos;
    uint32_t o_top_pos;
    uint32_t o_bottom_pos;
    uint32_t rl_split_line;
    uint32_t ud_split_line;
};
...
#define SPLIT_MASK               0xF
    ...
    uint32_t ov_alpha_off;
    uint32_t ov_alpha_stride;
...
static uint32_t tiled_filed_size(struct ipu_task_entry *t)
{
    uint32_t field_size;
...
static bool only_ic(uint8_t mode)
{
    mode = mode & IPU_PREPROCESS_MODE_MASK;
    return ((mode == IC_MODE) || (mode == VDI_MODE));
}

static bool only_rot(uint8_t mode)
{
    mode = mode & IPU_PREPROCESS_MODE_MASK;
    return (mode == ROT_MODE);
}

static bool ic_and_rot(uint8_t mode)
{
    mode = mode & IPU_PREPROCESS_MODE_MASK;
...
uint32_t fmt_to_bpp(uint32_t pixelformat)
{
    uint32_t bpp;
...
static int soc_max_in_width(uint32_t is_vdoa)
{
...
static void update_offset(uint32_t fmt,
...
static int update_split_setting(struct ipu_task_entry *t, bool vdi_split)
{
    ...
    if (t->set.split_mode & RL_SPLIT) {
        ...
        ret = ipu_calc_stripes_sizes(iw,
                                     ow,
                                     max_width,
                                     (((uint64_t)1) << 32), /* 32bit for fractional*/
                                     1, /* equal stripes */
                                     t->input.format,
                                     t->output.format,
                                     &left_stripe,
...
{
    uint32_t vdi_size;
    uint32_t vdi_save_lines;
    uint32_t stripe_mode;
    uint32_t task_no;
    uint32_t i, offset_addr;
    uint32_t line_size;
...
out:
    ...
    for (j = 0; j < size; j++) {
        ...
        spin_lock_irq_save(&ipu_task_list_lock, flags);
        ...
        spin_unlock_irq_restore(&ipu_task_list_lock, flags);
...
static inline int find_task(struct ipu_task_entry **t, int thread_id)
{
    ...
    uint64_t flags;
    struct ipu_task_entry *tsk;
    list_head_t *task_list = &ipu_task_list;
...
int register_ipu_device(struct ipu_soc *ipu, int id)
{
    ...
    if (!ipu_class) {
...
#endif /* 0 */
        ipu_class = vmm_zalloc(sizeof(vmm_class_t));
        ...
        vmm_device_driver_register_class(ipu_class);
...
void unregister_ipu_device(struct ipu_soc *ipu, int id)
{
    ...
    if (ipu_class) {
        ...
        device_destroy(ipu_class, MKDEV(major, 0));
#endif /* 0 */
        vmm_device_driver_unregister_class(ipu_class);
#if 0
        unregister_chrdev(major, "mxc_ipu");
        major = 0;
#endif /* 0 */
