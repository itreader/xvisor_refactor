...
{
    uint32_t ufstat = vmm_in_le32((void *)(base + S3C2410_UFSTAT));
...
uint8_t samsung_lowlevel_getc(virtual_addr_t base)
{
    uint8_t data;
...
{
    uint32_t ufcon = vmm_in_le32((void *)(base + S3C2410_UFCON));
    ...
    if (ufcon & S3C2410_UFCON_FIFOMODE) {
        ...
        uint32_t utrstat = vmm_in_le32((void *)(base + S3C2410_UTRSTAT));

        return (utrstat & S3C2410_UTRSTAT_TXE) ? TRUE : FALSE;
    }
}

void samsung_lowlevel_putc(virtual_addr_t base, uint8_t ch)
{
...
void samsung_lowlevel_init(virtual_addr_t base, bool skip_baudrate_config, uint32_t baudrate, uint32_t input_clock)
{
    uint32_t divider;
    uint32_t temp;
    uint32_t remainder;
    ...
    if (!skip_baudrate_config) {
        ...
        vmm_out_le16((void *)(base + S3C2410_UBRDIV), (uint16_t)divider);
        vmm_out_8((void *)(base + S3C2443_DIVSLOT), (uint8_t)remainder);
    }

    /* Set the UART to be 8 bits, 1 stop bit, no parity */
    vmm_out_le32((void *)(base + S3C2410_ULCON), S3C2410_LCON_CS8 | S3C2410_LCON_PNONE);
...
static vmm_irq_return_t samsung_irq_handler(int irq_no, void *dev)
{
    ...
    if (data & S3C64XX_UINTM_RXD_MSK) {
        ...
        while (samsung_lowlevel_can_getc(port->base)) {
            uint8_t ch = samsung_lowlevel_getc(port->base);
...
static uint32_t samsung_tx(struct serial *p, uint8_t *src, size_t len)
{
    uint32_t             i;
    struct samsung_port *port = serial_tx_private(p);
...
static int samsung_driver_probe(vmm_device_t *dev)
{
    ...
    rc = vmm_device_tree_request_regmap(dev->of_node, &port->base, 0, "Samsung UART");
    ...
    if ((rc = vmm_host_irq_register(port->irq, dev->name, samsung_irq_handler, port))) {
        goto free_reg;
    }

    /* Call low-level init function */
    samsung_lowlevel_init(port->base, port->skip_baudrate_config, port->baudrate, port->input_clock);
...
static int samsung_driver_remove(vmm_device_t *dev)
{
    struct samsung_port *port = dev->private;
...
static struct vmm_device_tree_nodeid samsung_devid_table[] = {
    {.compatible = "samsung,exynos4210-uart"},
    {/* end of list */},
};

static vmm_driver_t samsung_driver = {
    .name        = "samsung_serial",
...
{
    return vmm_device_driver_register_driver(&samsung_driver);
}

static void __exit samsung_driver_exit(void)
{
    vmm_device_driver_unregister_driver(&samsung_driver);
}
