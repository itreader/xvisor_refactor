...
{
    uint32_t status = vmm_readl((void *)(base + IMX21_UTS));
...
uint8_t imx_lowlevel_getc(virtual_addr_t base)
{
    uint8_t data;
...
void imx_lowlevel_putc(virtual_addr_t base, uint8_t ch)
{
...
void imx_lowlevel_init(virtual_addr_t base, bool skip_baudrate_config, uint32_t baudrate, uint32_t input_clock)
{
    uint32_t temp = vmm_readl((void *)(base + UCR1));
    uint32_t divider;
    ...
    vmm_writel(
        temp & ~(UCR3_RXDSEN | UCR3_DTREN | UCR3_FRAERREN | UCR3_TIMEOUTEN | UCR3_AIRINTEN | UCR3_AWAKEN | UCR3_DTRDEN), (void *)(base + UCR3));

    /* disable all UCR4 related interrupts */
    temp = vmm_readl((void *)(base + UCR4));
    vmm_writel(temp & ~(UCR4_DREN | UCR4_TCEN | UCR4_ENIRI | UCR4_WKEN | UCR4_BKEN | UCR4_OREN), (void *)(base + UCR4));
    ...
    /* Enable FIFOs */
    temp = vmm_readl((void *)(base + UCR2));
    vmm_writel(temp | UCR2_SRST | UCR2_RXEN | UCR2_TXEN, (void *)(base + UCR2));
}
...
static void imx_rxint(struct imx_port *port)
{
    while (imx_lowlevel_can_getc(port->base)) {
        uint8_t ch = imx_lowlevel_getc(port->base);
...
static vmm_irq_return_t imx_irq_handler(int irq, void *dev_id)
{
    ...
    sts &= USR1_PARITYERR | USR1_RTSD | USR1_ESCF | USR1_FRAMERR | USR1_TIMEOUT | USR1_AIRINT | USR1_AWAKE;
...
static uint32_t imx_tx(struct serial *p, uint8_t *src, size_t len)
{
    uint32_t         i;
    struct imx_port *port = serial_tx_private(p);
...
static int imx_driver_probe(vmm_device_t *dev)
{
    ...
    rc = vmm_device_tree_request_regmap(dev->of_node, &port->base, 0, "i.MX UART");
    ...
    /* Setup clocks */
    clock_ipg  = of_clock_get(dev->of_node, 0);
    clock_uart = of_clock_get(dev->of_node, 1);

    if (!VMM_IS_ERR_OR_NULL(clock_ipg)) {
        rc = clock_prepare_enable(clock_ipg);
    ...
    if (!VMM_IS_ERR_OR_NULL(clock_uart)) {
        rc = clock_prepare_enable(clock_uart);

        if (rc) {
            goto clock_disable_unprepare_ipg;
        }

        old_rate = clock_get_rate(clock_uart);

        if (clock_set_rate(clock_uart, port->input_clock)) {
            vmm_printf(
    ...
    if ((rc = vmm_host_irq_register(port->irq, dev->name, imx_irq_handler, port))) {
        goto clock_old_rate;
    }

    /* Call low-level init function */
    imx_lowlevel_init(port->base, port->skip_baudrate_config, port->baudrate, port->input_clock);
...
clock_old_rate:
    ...
    if (!VMM_IS_ERR_OR_NULL(clock_uart)) {
        if (old_rate) {
            clock_set_rate(clock_uart, old_rate);
        }
    }

clock_disable_unprepare_uart:

    if (!VMM_IS_ERR_OR_NULL(clock_uart)) {
        clock_disable_unprepare(clock_uart);
    }

clock_disable_unprepare_ipg:

    if (!VMM_IS_ERR_OR_NULL(clock_ipg)) {
        clock_disable_unprepare(clock_ipg);
    }
...
static int imx_driver_remove(vmm_device_t *dev)
{
    int              rc   = VMM_OK;
    struct imx_port *port = dev->private;
...
static struct vmm_device_tree_nodeid imx_devid_table[] = {
    {.compatible = "freescale,imx-uart"},
    {/* end of list */},
};

static vmm_driver_t imx_driver = {
    .name        = "imx_serial",
...
{
    return vmm_device_driver_register_driver(&imx_driver);
}

static void __exit imx_driver_exit(void)
{
    vmm_device_driver_unregister_driver(&imx_driver);
}
