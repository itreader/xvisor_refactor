...
uint8_t pl011_lowlevel_getc(virtual_addr_t base)
{
    uint16_t data;
...
void pl011_lowlevel_putc(virtual_addr_t base, uint8_t ch)
{
...
void pl011_lowlevel_init(virtual_addr_t base, bool skip_baudrate_config, uint32_t baudrate, uint32_t input_clock)
{
    uint32_t divider;
    uint32_t temp;
    uint32_t remainder;
    ...
    if (!skip_baudrate_config) {
        ...
        vmm_out_le16((void *)(base + UART_PL011_IBRD), (uint16_t)divider);
        vmm_out_8((void *)(base + UART_PL011_FBRD), (uint8_t)fraction);
    }

    /* Set the UART to be 8 bits, 1 stop bit,
     * no parity, fifo enabled
     */
    vmm_out_8((void *)(base + UART_PL011_LCRH), UART_PL011_LCRH_WLEN_8 | UART_PL011_LCRH_FEN);
    ...
    vmm_out_8((void *)(base + UART_PL011_IFLS), (uint8_t)temp);

    /* Finally, enable the UART */
    vmm_out_le16((void *)(base + UART_PL011_CR), UART_PL011_CR_UARTEN | UART_PL011_CR_TXE | UART_PL011_CR_RXE);
}
...
static uint32_t pl011_tx(struct serial *p, uint8_t *src, size_t len)
{
    uint32_t           i;
    struct pl011_port *port = serial_tx_private(p);
...
static int pl011_driver_probe(vmm_device_t *dev)
{
    int                rc;
    ...
    rc = vmm_device_tree_request_regmap(dev->of_node, &port->base, 0, "PL011 UART");
    ...
    if ((rc = vmm_host_irq_register(port->irq, dev->name, pl011_irq_handler, port))) {
        goto free_reg;
    }

    /* Call low-level init function */
    pl011_lowlevel_init(port->base, skip_baudrate_config, port->baudrate, port->input_clock);
...
static int pl011_driver_remove(vmm_device_t *dev)
{
    struct pl011_port *port = dev->private;
...
static struct vmm_device_tree_nodeid pl011_devid_table[] = {
    {.compatible = "arm,pl011"},
    {/* end of list */},
};

static vmm_driver_t pl011_driver = {
    .name        = "pl011_serial",
...
{
    return vmm_device_driver_register_driver(&pl011_driver);
}

static void __exit pl011_driver_exit(void)
{
    vmm_device_driver_unregister_driver(&pl011_driver);
}
