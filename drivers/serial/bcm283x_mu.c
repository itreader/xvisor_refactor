...
uint8_t bcm283x_mu_lowlevel_getc(virtual_addr_t base)
{
...
void bcm283x_mu_lowlevel_putc(virtual_addr_t base, uint8_t ch)
{
...
void bcm283x_mu_lowlevel_init(virtual_addr_t base, bool skip_baudrate_config, uint32_t baudrate, uint32_t input_clock)
{
    uint32_t val;
    uint32_t divider;
...
static uint32_t bcm283x_mu_tx(struct serial *p, uint8_t *src, size_t len)
{
    uint32_t                i;
    struct bcm283x_mu_port *port = serial_tx_private(p);
...
static int bcm283x_mu_driver_probe(vmm_device_t *dev)
{
    int                     rc;
    ...
    rc = vmm_device_tree_request_regmap(dev->of_node, &port->base, 0, "BCM283x MINIUART");
    ...
    if ((rc = vmm_host_irq_register(port->irq, dev->name, bcm283x_mu_irq_handler, port))) {
        goto free_reg;
    }

    /* Call low-level init function */
    bcm283x_mu_lowlevel_init(port->base, port->skip_baudrate_config, port->baudrate, port->input_clock);
...
static int bcm283x_mu_driver_remove(vmm_device_t *dev)
{
    struct bcm283x_mu_port *port = dev->private;
...
static struct vmm_device_tree_nodeid bcm283x_mu_devid_table[] = {
    {.compatible = "brcm,bcm283x-mu"},
    {/* end of list */},
};

static vmm_driver_t bcm283x_mu_driver = {
...
{
    return vmm_device_driver_register_driver(&bcm283x_mu_driver);
}

static void __exit bcm283x_mu_driver_exit(void)
{
    vmm_device_driver_unregister_driver(&bcm283x_mu_driver);
}
