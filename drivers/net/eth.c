 ...
 * size (sizeof_private).  A 32-byte (not bit) alignment is enforced for
 * this private data area.
 */

struct net_device *alloc_etherdev_mqs(int sizeof_private, uint32_t txqs, uint32_t rxqs)
{
    static int         eth_num = 0;
    struct net_device *ndev    = NULL;

    if (1 != txqs && 1 != rxqs) {
        vmm_lwarning(NULL, "Multi-queue network is not supported yet.");
    }

    ndev = alloc_etherdev(sizeof_private);
