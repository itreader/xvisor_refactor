...
#define MODULE_IPRIORITY                  (VMM_NET_CLASS_IPRIORITY + USB_CORE_IPRIORITY + 1)
...
struct usb_net_device {
    ...
    int      have_hwaddr;
    uint32_t mac_cr;      /* MAC control register value */
    size_t   rx_urb_size; /* maximum USB URB size */

    vmm_thread_t     *rx_thread;
 ...
 */
static int smsc95xx_write_reg(struct usb_device *udev, uint32_t index, uint32_t data)
{
    int                          rc, len;
    uint32_t __cacheline_aligned tmpbuf;

    tmpbuf = vmm_cpu_to_le32(data);

    rc     = usb_control_msg(
        udev, usb_sndctrlpipe(udev, 0), USB_VENDOR_REQUEST_WRITE_REGISTER, USB_DIR_OUT | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0, index, &tmpbuf,
        sizeof(data), &len, USB_CTRL_SET_TIMEOUT);
...
static int smsc95xx_read_reg(struct usb_device *udev, uint32_t index, uint32_t *data)
{
    int                          len;
    uint32_t __cacheline_aligned tmpbuf;

    usb_control_msg(
        udev, usb_rcvctrlpipe(udev, 0), USB_VENDOR_REQUEST_READ_REGISTER, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0, index, &tmpbuf,
        sizeof(*data), &len, USB_CTRL_GET_TIMEOUT);
    *data = tmpbuf;
...
{
    uint64_t timeout = 100000 + vmm_timer_timestamp();
    uint32_t val;
...
static void smsc95xx_mdio_write(struct usb_device *udev, int phy_id, int idx, int regval)
{
    uint32_t val, addr;
...
static int smsc95xx_phy_initialize(struct usb_device *udev, struct usb_net_device *ndev)
{
    smsc95xx_mdio_write(udev, ndev->phy_id, MII_BMCR, BMCR_RESET);
    smsc95xx_mdio_write(udev, ndev->phy_id, MII_ADVERTISE, ADVERTISE_ALL | ADVERTISE_CSMA | ADVERTISE_PAUSE_CAP | ADVERTISE_PAUSE_ASYM);

    /* read to clear */
    smsc95xx_mdio_read(udev, ndev->phy_id, PHY_INT_SRC);

    smsc95xx_mdio_write(udev, ndev->phy_id, PHY_INT_MASK, PHY_INT_MASK_DEFAULT_);
...
static int smsc95xx_write_hwaddr_common(struct usb_device *udev, struct usb_net_device *dev, unsigned char *enetaddr)
{
    uint32_t addr_lo = get_unaligned((uint32_t *)&enetaddr[0]);
    uint32_t addr_hi = get_unaligned((uint16_t *)&enetaddr[4]);
...
static int smsc95xx_set_csums(struct usb_device *udev, int use_tx_csum, int use_rx_csum)
{
    uint32_t read_buf;
...
static void smsc95xx_set_multicast(struct usb_net_device *private)
{
    /* disable */
...
/* starts the TX path */
static void smsc95xx_start_tx_path(struct usb_device *udev, struct usb_net_device *private)
{
    uint32_t reg_val;
...
static int smsc95xx_init_common(struct usb_device *udev, struct usb_net_device *dev, unsigned char *enetaddr)
{
    int      ret;
    uint32_t write_buf;
    uint32_t read_buf;
...
#define _NET_IP_ALIGN 0
    ...
    write_buf = LED_GPIO_CFG_SPD_LED | LED_GPIO_CFG_LNK_LED | LED_GPIO_CFG_FDX_LED;
    ...
    /* Init Rx. Set Vlan */
    write_buf = (uint32_t)ETH_P_8021Q;
    ...
    do {
        link_detected = smsc95xx_mdio_read(udev, dev->phy_id, MII_BMSR) & BMSR_LSTATUS;
...
static int smsc95xx_send_common(struct usb_net_device *ndev, void *packet, int length)
{
    ...
    err = usb_bulk_msg(
        ndev->udev, usb_sndbulkpipe(ndev->udev, ndev->ep_out), (void *)msg, length + sizeof(tx_cmd_a) + sizeof(tx_cmd_b), &actual_len,
        USB_BULK_SEND_TIMEOUT);
    DPRINTF("Tx: len = %u, actual = %u, err = %d\n", (uint32_t)(length + sizeof(tx_cmd_a) + sizeof(tx_cmd_b)), (uint32_t)actual_len, err);
...
static int smsc95xx_recv(struct usb_net_device *ndev, int *rx_len)
{
    struct sk_buff             *skb;
    uint8_t                    *rdptr;
    uint8_t __cacheline_aligned recv_buf[RX_URB_SIZE];
    ...
    err = usb_bulk_msg(ndev->udev, usb_rcvbulkpipe(ndev->udev, ndev->ep_in), (void *)recv_buf, RX_URB_SIZE, &actual_len, USB_BULK_RECV_TIMEOUT);
    ...
    while (actual_len > 0) {
        ...
        *rx_len    = packet_len;
        ...
        if (packet_len) {
            ...
            skb_reserve(skb, 2);
            rdptr = (uint8_t *)skb_put(skb, packet_len - 4);
...
static int smsc95xx_worker(void *data)
{
    ...
    struct usb_net_device *nd   = ndev->net_private;
    int                    rx_len;
...
/* ethtool ops */
static void smsc95xx_get_drvinfo(struct net_device *dev, struct ethtool_drvinfo *info)
{
...
{
    struct usb_net_device *nd  = (struct usb_net_device *)dev->net_private;
    int                    ret = smsc95xx_init_common(nd->udev, dev->net_private, dev->dev_addr);
...
static int smsc95xx_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
    int rc = VMM_OK;
    ;
    ...
    for (int i = 0; i < intf->no_of_ep; i++) {
        ...
        if ((intf->ep_desc[i].bmAttributes & USB_ENDPOINT_XFERTYPE_MASK) == USB_ENDPOINT_XFER_BULK) {
            if (intf->ep_desc[i].bEndpointAddress & USB_DIR_IN) {
                unet->ep_in = intf->ep_desc[i].bEndpointAddress & USB_ENDPOINT_NUMBER_MASK;
            } else {
                unet->ep_out = intf->ep_desc[i].bEndpointAddress & USB_ENDPOINT_NUMBER_MASK;
            }
        }

        /* is it an interrupt endpoint? */
        if ((intf->ep_desc[i].bmAttributes & USB_ENDPOINT_XFERTYPE_MASK) == USB_ENDPOINT_XFER_INT) {
            unet->ep_int      = intf->ep_desc[i].bEndpointAddress & USB_ENDPOINT_NUMBER_MASK;
            unet->irqinterval = intf->ep_desc[i].bInterval;
        }
    }

    DPRINTF("%s Endpoints In %d Out %d Int %d\n", __func__, unet->ep_in, unet->ep_out, unet->ep_int);
...
#endif
    unet->rx_thread       = vmm_threads_create("smsc95xx_rx", smsc95xx_worker, nd, VMM_THREAD_DEF_PRIORITY, VMM_THREAD_DEF_TIME_SLICE);
