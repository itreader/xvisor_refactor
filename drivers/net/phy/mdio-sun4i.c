...
{
    struct sun4i_mdio_data *data = bus->private;
...
static int sun4i_mdio_write(struct mii_bus *bus, int mii_id, int regnum, uint16_t value)
{
    struct sun4i_mdio_data *data = bus->private;
    uint64_t                timeout_jiffies;
...
static int sun4i_mdio_probe(vmm_device_t *pdev)
{
    ...
    if ((ret = vmm_device_tree_request_regmap(np, &reg_addr, 0, "Sun4i MDIO"))) {
...
static int sun4i_mdio_remove(vmm_device_t *pdev)
{
...
static struct vmm_device_tree_nodeid sun4i_mdio_device_tree_ids[] = {
    {.compatible = "allwinner,sun4i-mdio"},
    {/* end of list */},
};

static vmm_driver_t sun4i_mdio_driver = {
    .probe       = sun4i_mdio_probe,
    .remove      = sun4i_mdio_remove,
    .name        = "sun4i-mdio",
    .match_table = sun4i_mdio_device_tree_ids,
};

static int __init sun4i_mdio_driver_init(void)
{
    return vmm_device_driver_register_driver(&sun4i_mdio_driver);
}

static void __exit sun4i_mdio_driver_exit(void)
{
    vmm_device_driver_unregister_driver(&sun4i_mdio_driver);
}
