...
static int phy_attach_direct(struct net_device *dev, struct phy_device *phydev, uint32_t flags, phy_interface_t interface);
...
static int phy_needs_fixup(struct phy_device *phydev, struct phy_fixup *fixup)
{
    ...
    if ((fixup->phy_uid & fixup->phy_uid_mask) != (phydev->phy_id & fixup->phy_uid_mask)) {
 ...
 *
 */
static int get_phy_c45_ids(struct mii_bus *bus, int addr, uint32_t *phy_id, struct phy_c45_device_ids *c45_ids)
{
    int       phy_reg;
    int       i, reg_addr;
 ...
 *
 */
static int get_phy_id(struct mii_bus *bus, int addr, uint32_t *phy_id, bool is_c45, struct phy_c45_device_ids *c45_ids)
{
    int phy_reg;
...
int phy_driver_register(struct phy_driver *new_driver)
{
    ...
    if (!strlen(new_driver->driver.name)) {
        strncpy(new_driver->driver.name, new_driver->name, sizeof(new_driver->driver.name));
    }
