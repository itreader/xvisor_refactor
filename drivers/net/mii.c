...
int mii_ethtool_gset(struct mii_if_info *mii, struct ethtool_cmd *ecmd)
{
    ...
    if (bmcr & BMCR_ANENABLE) {
        ...
        if (mii->supports_gmii) {
            ecmd->advertising |= mii_ctrl1000_to_ethtool_adv_t(ctrl1000);
        }
        ...
        ethtool_cmd_speed_set(
            ecmd, ((bmcr & BMCR_SPEED1000 && (bmcr & BMCR_SPEED100) == 0) ? SPEED_1000 : ((bmcr & BMCR_SPEED100) ? SPEED_100 : SPEED_10)));
...
int mii_ethtool_sset(struct mii_if_info *mii, struct ethtool_cmd *ecmd)
{
    ...
    if (ecmd->autoneg == AUTONEG_ENABLE) {
        ...
        if ((ecmd->advertising & (ADVERTISED_10baseT_Half | ADVERTISED_10baseT_Full | ADVERTISED_100baseT_Half | ADVERTISED_100baseT_Full |
                                  ADVERTISED_1000baseT_Half | ADVERTISED_1000baseT_Full)) == 0) {
            return -EINVAL;
        }
        ...
        if (mii->supports_gmii) {
            tmp2 |= ethtool_adv_to_mii_ctrl1000_t(ecmd->advertising);
        }
 ...
 */
uint32_t mii_check_media(struct mii_if_info *mii, uint32_t ok_to_print, uint32_t init_media)
{
    ...
    new_carrier = (uint32_t)mii_link_ok(mii);
...
int generic_mii_ioctl(struct mii_if_info *mii_if,
                      ...
                      uint32_t *duplex_chg_out)
{
    int rc = 0;
    uint32_t duplex_changed = 0;
