...
struct net_device *netdev_alloc(const char *name)
{
    ...
    if (strlcpy(ndev->name, name, sizeof(ndev->name)) >= sizeof(ndev->name)) {
        vmm_free(ndev);
        return NULL;
    }
...
static int netdev_register_port(struct net_device *ndev)
{
    ...
    ndev->nsw_private = port;

    vmm_netport_register(port);

    if (dev) {
        if (dev->of_node && vmm_device_tree_read_string(dev->of_node, "switch", &attr) == VMM_OK) {
...
int register_netdev(struct net_device *ndev)
{
    ...
    if (ndev->netdev_ops && ndev->netdev_ops->ndo_init) {
        ...
        if (rc != VMM_OK) {
            vmm_printf(
                "%s: Device %s Failed during initializaion"
                "with err %d!!!!\n",
...
{
    struct net_device *dev = (struct net_device *)port->private;
...
int netdev_switch2port_xfer(struct vmm_netport *port, struct vmm_mbuf *mbuf)
{
    ...
    if (mbuf->m_next) {
        ...
        len = min(dev->mtu, (uint32_t)mbuf->m_pktlen);
        buf = vmm_malloc(len);
...
struct net_device *alloc_etherdev(int sizeof_private)
{
    ...
    ndev->private = (void *)vmm_zalloc(sizeof_private);

    if (!ndev->private) {
        vmm_printf("%s Failed to allocate ndev->private of size %d\n", __func__, sizeof_private);
        vmm_free(ndev);
        return NULL;
    }
