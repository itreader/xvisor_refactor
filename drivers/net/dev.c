...
void netif_napi_add(struct net_device *dev, struct napi_struct *napi, int (*poll)(struct napi_struct *, int), int weight)
{
    ...
    if (weight > NAPI_POLL_WEIGHT) {
        pr_err_once(
            "netif_napi_add() called with weight %d on "
            "device %s\n",
            weight, dev->name);
    }
    ...
    napi->dev  = dev;
    napi->poll = poll;
    INIT_NETPORT_LAZY(&napi->lazy, port, netdev_budget, NULL, lazy_xfer2napi_poll);
}
...
{
    struct vmm_netport *port = n->dev->nsw_private;
