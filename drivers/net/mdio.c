...
int mdio45_probe(struct mdio_if_info *mdio, int prtad)
{
    ...
    for (mmd = 1; mmd <= 5; mmd++) {
        ...
        if (stat2 < 0 || (stat2 & MDIO_STAT2_DEVPRST) != MDIO_STAT2_DEVPRST_VAL) {
            continue;
        }
 ...
 */
int mdio_set_flag(const struct mdio_if_info *mdio, int prtad, int devad, uint16_t addr, int mask, bool sense)
{
 ...
 */
int mdio45_links_ok(const struct mdio_if_info *mdio, uint32_t mmd_mask)
{
    int devad, reg;

    if (!mmd_mask) {
        /* Use absence of XGMII faults in lieu of link state */
        reg = mdio->mdio_read(mdio->dev, mdio->prtad, MDIO_MMD_PHYXS, MDIO_STAT2);
    ...
    for (devad = 0; mmd_mask; devad++) {
        if (mmd_mask & (1 << devad)) {
            ...
            mdio->mdio_read(mdio->dev, mdio->prtad, devad, MDIO_STAT1);

            if (devad == MDIO_MMD_PMAPMD || devad == MDIO_MMD_PCS || devad == MDIO_MMD_PHYXS || devad == MDIO_MMD_DTEXS) {
                mdio->mdio_read(mdio->dev, mdio->prtad, devad, MDIO_STAT2);
            }

            /* Check the current status and fault flags */
            reg = mdio->mdio_read(mdio->dev, mdio->prtad, devad, MDIO_STAT1);

            if (reg < 0 || (reg & (MDIO_STAT1_FAULT | MDIO_STAT1_LSTATUS)) != MDIO_STAT1_LSTATUS) {
                return false;
            }
        }
    }
...
int mdio45_nway_restart(const struct mdio_if_info *mdio)
{
    ...
    mdio_set_flag(mdio, mdio->prtad, MDIO_MMD_AN, MDIO_CTRL1, MDIO_AN_CTRL1_RESTART, true);
    return 0;
}

EXPORT_SYMBOL(mdio45_nway_restart);

static uint32_t mdio45_get_an(const struct mdio_if_info *mdio, uint16_t addr)
{
    uint32_t result = 0;
    int      reg;
 ...
 */
void mdio45_ethtool_gset_npage(const struct mdio_if_info *mdio, struct ethtool_cmd *ecmd, uint32_t npage_adv, uint32_t npage_lpa)
{
    int      reg;
    uint32_t speed;
    ...
    ecmd->mdio_support = mdio->mode_support & (MDIO_SUPPORTS_C45 | MDIO_SUPPORTS_C22);

    reg                = mdio->mdio_read(mdio->dev, mdio->prtad, MDIO_MMD_PMAPMD, MDIO_CTRL2);
    ...
    switch (reg & MDIO_PMA_CTRL2_TYPE) {
        ...
        case MDIO_PMA_CTRL2_10BT:
            ...
            reg             = mdio->mdio_read(mdio->dev, mdio->prtad, MDIO_MMD_PMAPMD, MDIO_SPEED);
            ...
            if (reg & MDIO_PMA_SPEED_1000) {
                ecmd->supported |= (SUPPORTED_1000baseT_Full | SUPPORTED_1000baseT_Half);
            }

            if (reg & MDIO_PMA_SPEED_100) {
                ecmd->supported |= (SUPPORTED_100baseT_Full | SUPPORTED_100baseT_Half);
            }

            if (reg & MDIO_PMA_SPEED_10) {
                ecmd->supported |= (SUPPORTED_10baseT_Full | SUPPORTED_10baseT_Half);
            }
        ...
        case MDIO_PMA_CTRL2_1000BKX:
            ...
            reg             = mdio->mdio_read(mdio->dev, mdio->prtad, MDIO_MMD_PMAPMD, MDIO_PMA_EXCEPTION_TABLE);

            if (reg & MDIO_PMA_EXCEPTION_TABLE_10GBKX4) {
                ecmd->supported |= SUPPORTED_10000baseKX4_Full;
            }

            if (reg & MDIO_PMA_EXCEPTION_TABLE_10GBKR) {
                ecmd->supported |= SUPPORTED_10000baseKR_Full;
            }

            if (reg & MDIO_PMA_EXCEPTION_TABLE_1000BKX) {
                ecmd->supported |= SUPPORTED_1000baseKX_Full;
            }

            reg = mdio->mdio_read(mdio->dev, mdio->prtad, MDIO_MMD_PMAPMD, MDIO_PMA_10GBR_FECABLE);
    ...
    if (mdio->mmds & MDIO_DEVS_AN) {
        ...
        reg = mdio->mdio_read(mdio->dev, mdio->prtad, MDIO_MMD_AN, MDIO_CTRL1);

        if (reg & MDIO_AN_CTRL1_ENABLE) {
            ecmd->autoneg = AUTONEG_ENABLE;
            ecmd->advertising |= ADVERTISED_Autoneg | mdio45_get_an(mdio, MDIO_AN_ADVERTISE) | npage_adv;
        } else {
    ...
    if (ecmd->autoneg) {
        uint32_t modes   = 0;
        int      an_stat = mdio->mdio_read(mdio->dev, mdio->prtad, MDIO_MMD_AN, MDIO_STAT1);

        /* If AN is complete and successful, report best common
         * mode, otherwise report best advertised mode. */
        if (an_stat & MDIO_AN_STAT1_COMPLETE) {
            ecmd->lp_advertising = mdio45_get_an(mdio, MDIO_AN_LPA) | npage_lpa;
        ...
        if (modes & (ADVERTISED_10000baseT_Full | ADVERTISED_10000baseKX4_Full | ADVERTISED_10000baseKR_Full)) {
            speed        = SPEED_10000;
            ecmd->duplex = DUPLEX_FULL;
        } else if (modes & (ADVERTISED_1000baseT_Full | ADVERTISED_1000baseT_Half | ADVERTISED_1000baseKX_Full)) {
            speed        = SPEED_1000;
            ecmd->duplex = !(modes & ADVERTISED_1000baseT_Half);
        } else if (modes & (ADVERTISED_100baseT_Full | ADVERTISED_100baseT_Half)) {
            speed        = SPEED_100;
        ...
        reg          = mdio->mdio_read(mdio->dev, mdio->prtad, MDIO_MMD_PMAPMD, MDIO_CTRL1);
        speed        = (((reg & MDIO_PMA_CTRL1_SPEED1000) ? 100 : 1) * ((reg & MDIO_PMA_CTRL1_SPEED100) ? 100 : 10));
        ecmd->duplex = (reg & MDIO_CTRL1_FULLDPLX || speed == SPEED_10000);
    }

    ethtool_cmd_speed_set(ecmd, speed);

    /* 10GBASE-T MDI/MDI-X */
    if (ecmd->port == PORT_TP && (ethtool_cmd_speed(ecmd) == SPEED_10000)) {
        switch (mdio->mdio_read(mdio->dev, mdio->prtad, MDIO_MMD_PMAPMD, MDIO_PMA_10GBT_SWAPPOL)) {
...
void mdio45_ethtool_spauseparam_an(const struct mdio_if_info *mdio, const struct ethtool_pauseparam *ecmd)
{
    ...
    old_adv = mdio->mdio_read(mdio->dev, mdio->prtad, MDIO_MMD_AN, MDIO_AN_ADVERTISE);
    adv =
        ((old_adv & ~(ADVERTISE_PAUSE_CAP | ADVERTISE_PAUSE_ASYM)) |
         mii_advertise_flowctrl((ecmd->rx_pause ? FLOW_CTRL_RX : 0) | (ecmd->tx_pause ? FLOW_CTRL_TX : 0)));

    if (adv != old_adv) {
        mdio->mdio_write(mdio->dev, mdio->prtad, MDIO_MMD_AN, MDIO_AN_ADVERTISE, adv);
...
{
    int prtad, devad;
    uint16_t addr = mii_data->reg_num;
