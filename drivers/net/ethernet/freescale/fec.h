...
struct bufdesc {
    ...
    uint64_t       cbd_bufaddr; /* Buffer address */
};
#else
struct bufdesc {
    unsigned short cbd_sc;      /* Control and status info */
...
#define FEC_R_DES_START(X)       ((X == 1) ? FEC_R_DES_START_1 : ((X == 2) ? FEC_R_DES_START_2 : FEC_R_DES_START_0))
#define FEC_X_DES_START(X)       ((X == 1) ? FEC_X_DES_START_1 : ((X == 2) ? FEC_X_DES_START_2 : FEC_X_DES_START_0))
#define FEC_R_DES_ACTIVE(X)      ((X == 1) ? FEC_R_DES_ACTIVE_1 : ((X == 2) ? FEC_R_DES_ACTIVE_2 : FEC_R_DES_ACTIVE_0))
#define FEC_X_DES_ACTIVE(X)      ((X == 1) ? FEC_X_DES_ACTIVE_1 : ((X == 2) ? FEC_X_DES_ACTIVE_2 : FEC_X_DES_ACTIVE_0))
...
#define IDLE_SLOPE(X)            ((X == 1) ? (IDLE_SLOPE_1 & IDLE_SLOPE_MASK) : (IDLE_SLOPE_2 & IDLE_SLOPE_MASK))
#define RCMR_MATCHEN             (0x1 << 16)
#define RCMR_CMP_CFG(v, n)       ((v & 0x7) << (n << 2))
#define RCMR_CMP_1               (RCMR_CMP_CFG(0, 0) | RCMR_CMP_CFG(1, 1) | RCMR_CMP_CFG(2, 2) | RCMR_CMP_CFG(3, 3))
#define RCMR_CMP_2               (RCMR_CMP_CFG(4, 0) | RCMR_CMP_CFG(5, 1) | RCMR_CMP_CFG(6, 2) | RCMR_CMP_CFG(7, 3))
 ...
 * the register clock and the 1588 clock. The 1588 ts_clock is fixed to 25Mhz,
...
struct fec_enet_private {
    ...
    struct clk *clock_ref;
    struct clk *clock_enet_out;
    struct clk *clock_ptp;

    bool         ptp_clock_on;
    struct mutex ptp_clock_mutex;
    ...
    uint32_t total_tx_ring_size;
    uint32_t total_rx_ring_size;
    ...
    uint64_t              last_overflow_check;
    spinlock_t            tmreg_lock;
    struct cyclecounter   cc;
    struct timecounter    tc;
    int                   rx_hwtstamp_filter;
    uint32_t              base_incval;
    ...
    uint32_t rx_pkts_itr;
    uint32_t rx_time_itr;
    uint32_t tx_pkts_itr;
    uint32_t tx_time_itr;
    uint32_t itr_clock_rate;
