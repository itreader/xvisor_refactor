...
{
    struct emac_board_info *db = netdev_private(dev);
    uint32_t                reg_val;
...
static void emac_handle_link_change(struct net_device *dev)
{
    ...
    if (phydev->link) {
        if (db->speed != phydev->speed) {
            spin_lock_irq_save(&db->lock, flags);
            db->speed = phydev->speed;
            emac_update_speed(dev);
            spin_unlock_irq_restore(&db->lock, flags);
            status_change = 1;
        }
...
static int emac_mdio_probe(struct net_device *dev)
{
    ...
    db->phy_dev                = of_phy_connect(db->ndev, db->phy_node, &emac_handle_link_change, 0, db->phy_interface);
...
static void emac_outblock_32bit(void __iomem *reg, void *data, int count)
{
    writesl(reg, data, round_up(count, 4) / 4);
}

static void emac_inblock_32bit(void __iomem *reg, void *data, int count)
{
...
{
    struct emac_board_info *dm     = netdev_private(dev);
...
static uint32_t emac_setup(struct net_device *ndev)
{
    struct emac_board_info *db = netdev_private(ndev);
    uint32_t                reg_val;

    /* set up TX */
    reg_val = readl(db->membase + EMAC_TX_MODE_REG);

    writel(reg_val | EMAC_TX_MODE_ABORTED_FRAME_EN, db->membase + EMAC_TX_MODE_REG);

    /* set up RX */
    reg_val = readl(db->membase + EMAC_RX_CTL_REG);

    writel(
        reg_val | EMAC_RX_CTL_PASS_LEN_OOR_EN | EMAC_RX_CTL_ACCEPT_UNICAST_EN | EMAC_RX_CTL_DA_FILTER_EN | EMAC_RX_CTL_ACCEPT_MULTICAST_EN |
    ...
    writel(reg_val | EMAC_MAC_CTL0_RX_FLOW_CTL_EN | EMAC_MAC_CTL0_TX_FLOW_CTL_EN, db->membase + EMAC_MAC_CTL0_REG);
    ...
    /* set up IPGR */
    writel((EMAC_MAC_IPGR_IPG1 << 8) | EMAC_MAC_IPGR_IPG2, db->membase + EMAC_MAC_IPGR_REG);

    /* set up Collison window */
    writel((EMAC_MAC_CLRT_COLLISION_WINDOW << 8) | EMAC_MAC_CLRT_RM, db->membase + EMAC_MAC_CLRT_REG);
...
static uint32_t emac_powerup(struct net_device *ndev)
{
    struct emac_board_info *db = netdev_private(ndev);
    uint32_t                reg_val;
    ...
    writel(ndev->dev_addr[0] << 16 | ndev->dev_addr[1] << 8 | ndev->dev_addr[2], db->membase + EMAC_MAC_A1_REG);
    writel(ndev->dev_addr[3] << 16 | ndev->dev_addr[4] << 8 | ndev->dev_addr[5], db->membase + EMAC_MAC_A0_REG);
...
static void emac_init_device(struct net_device *dev)
{
    ...
    writel(reg_val | EMAC_CTL_RESET | EMAC_CTL_TX_EN | EMAC_CTL_RX_EN, db->membase + EMAC_CTL_REG);
...
static int emac_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    emac_outblock_32bit(db->membase + EMAC_TX_IO_DATA_REG, skb_data(skb), skb_len(skb));
    ...
    if (channel == 0) {
        ...
        writel(readl(db->membase + EMAC_TX_CTL0_REG) | 1, db->membase + EMAC_TX_CTL0_REG);
        ...
        /* start translate from fifo to phy */
        writel(readl(db->membase + EMAC_TX_CTL1_REG) | 1, db->membase + EMAC_TX_CTL1_REG);
 ...
 */
static void emac_tx_done(struct net_device *dev, struct emac_board_info *db, uint32_t tx_status)
{
...
static void emac_rx(struct net_device *dev)
{
    ...
    uint32_t                reg_val;
    uint32_t                rxhdr, rxstatus, rxcount, rxlen;
    ...
    while (1) {
        ...
        emac_inblock_32bit(db->membase + EMAC_RX_IO_DATA_REG, &rxhdr, sizeof(rxhdr));
        ...
        if (good_packet && skb) {
            skb_reserve(skb, 2);
            rdptr = (uint8_t *)skb_put(skb, rxlen - 4);
            ...
            emac_inblock_32bit(db->membase + EMAC_RX_IO_DATA_REG, rdptr, rxlen);
 ...
 */
static int emac_probe(vmm_device_t *pdev)
{
    ...
    if ((ret = vmm_device_tree_request_regmap(np, &reg_addr, 0, "Sun4i EMAC"))) {
    ...
    ndev->base_addr = (uint64_t)db->membase;
    ...
    db->clk = clock_get(pdev, NULL);

    if (IS_ERR(db->clk)) {
        goto out;
    }

    clock_prepare_enable(db->clk);

    db->phy_node = vmm_device_tree_parse_phandle(np, "phy", 0);
    ...
    dev_info(pdev, "%s: at %p, IRQ %d MAC: ", ndev->name, db->membase, ndev->irq);
    print_mac_address_fmt(ndev->dev_addr);
...
static int emac_remove(vmm_device_t *pdev)
{
...
static struct vmm_device_tree_nodeid emac_of_match[] = {
    {.compatible = "allwinner,sun4i-emac"},
    {/* end of list */},
};

static vmm_driver_t emac_driver = {
    .name        = "sun4i-emac",
...
{
    return vmm_device_driver_register_driver(&emac_driver);
}

static void __exit emac_driver_exit(void)
{
    vmm_device_driver_unregister_driver(&emac_driver);
}
