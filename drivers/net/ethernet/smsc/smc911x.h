...
#ifdef SMC_DYNAMIC_BUS_CONFIG
static inline uint32_t SMC_inl(struct smc911x_local *lp, int reg)
{
...
static inline void SMC_outl(uint32_t value, struct smc911x_local *lp, int reg)
{
    void __iomem *ioaddr = lp->base + reg;
...
static inline void SMC_insl(struct smc911x_local *lp, int reg, void *addr, uint32_t count)
{
    void __iomem *ioaddr = lp->base + reg;
...
static inline void SMC_outsl(struct smc911x_local *lp, int reg, void *addr, uint32_t count)
{
    void __iomem *ioaddr = lp->base + reg;
 ...
 * mechanism exits
 */
#define SMC_DMA_REQUEST(dev, handler) pxa_request_dma(dev->name, DMA_PRIO_LOW, handler, dev)
...
#ifdef SMC_insl
#undef SMC_insl
#define SMC_insl(lp, r, p, l) smc_pxa_dma_insl(lp, lp->physaddr, r, lp->rxdma, p, l)

static inline void smc_pxa_dma_insl(struct smc911x_local *lp, uint64_t physaddr, int reg, int dma, uint8_t *buf, int len)
{
    /* 64 bit alignment is required for memory to memory DMA */
    if ((long)buf & 4) {
        *((uint32_t *)buf) = SMC_inl(lp, reg);
        buf += 4;
        len--;
    }
    ...
    DCMD(dma)  = (DCMD_INCTRGADDR | DCMD_BURST32 | DCMD_WIDTH4 | DCMD_ENDIRQEN | (DCMD_LENGTH & rx_dmalen));
...
#ifdef SMC_outsl
#undef SMC_outsl
#define SMC_outsl(lp, r, p, l) smc_pxa_dma_outsl(lp, lp->physaddr, r, lp->txdma, p, l)

static inline void smc_pxa_dma_outsl(struct smc911x_local *lp, uint64_t physaddr, int reg, int dma, uint8_t *buf, int len)
{
    /* 64 bit alignment is required for memory to memory DMA */
    if ((long)buf & 4) {
        SMC_outl(*((uint32_t *)buf), lp, reg);
        buf += 4;
        len--;
    }
    ...
    DCMD(dma)  = (DCMD_INCSRCADDR | DCMD_BURST32 | DCMD_WIDTH4 | DCMD_ENDIRQEN | (DCMD_LENGTH & tx_dmalen));
    DCSR(dma)  = DCSR_NODESC | DCSR_RUN;
}
#endif
...
#define PHY_INT_SRC_ANEG_LP_ACK_      ((uint16_t)0x0008)
#define PHY_INT_SRC_PAR_DET_FAULT_    ((uint16_t)0x0004)
#define PHY_INT_SRC_ANEG_PGRX_        ((uint16_t)0x0002)
...
#define PHY_INT_MASK_ANEG_LP_ACK_     ((uint16_t)0x0008)
#define PHY_INT_MASK_PAR_DET_FAULT_   ((uint16_t)0x0004)
#define PHY_INT_MASK_ANEG_PGRX_       ((uint16_t)0x0002)

#define PHY_SPECIAL                   ((uint32_t)31)
#define PHY_SPECIAL_ANEG_DONE_        ((uint16_t)0x1000)
#define PHY_SPECIAL_RES_              ((uint16_t)0x0040)
#define PHY_SPECIAL_RES_MASK_         ((uint16_t)0x0FE1)
