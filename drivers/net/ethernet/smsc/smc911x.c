...
static const char version[] = "smc911x.c: v1.0 04-16-2005 by Dustin McIntire <dustin@sensoria.com>\n";
...
#if SMC_DEBUG_PKTS > 0
static void PRINT_PKT(uint8_t *buf, int length)
{
    int i;
    int remainder;
    int lines;
...
{
    struct smc911x_local *lp = netdev_private(dev);
    uint32_t              reg, timeout = 0, resets = 1, irq_cfg;
    uint64_t              flags;
    ...
    spin_lock_irq_save(&lp->lock, flags);
    SMC_SET_INT_EN(lp, 0);
    spin_unlock_irq_restore(&lp->lock, flags);
...
static void smc911x_enable(struct net_device *dev)
{
    ...
    mask = INT_EN_TDFA_EN_ | INT_EN_TSFL_EN_ | INT_EN_RSFL_EN_ | INT_EN_GPT_INT_EN_ | INT_EN_RXDFH_INT_EN_ | INT_EN_RXE_EN_ | INT_EN_PHY_INT_EN_;
...
{
    struct smc911x_local *lp = netdev_private(dev);
    uint32_t              fifo_count, timeout, reg;
...
static void smc911x_hardware_send_pkt(struct net_device *dev)
{
    ...
    buf  = (char *)((uint32_t)(skb_data(skb)) & ~0xF);
    len  = (skb_len(skb) + 0xF + ((uint32_t)skb_data(skb) & 0xF)) & ~0xF;
    cmdA = (1 << 24) | (((uint32_t)skb_data(skb) & 0xF) << 16) | TX_CMD_A_INT_FIRST_SEG_ | TX_CMD_A_INT_LAST_SEG_ | skb_len(skb);

#else
    buf  = (unsigned char *)((uint32_t)skb_data(skb) & ~0x3);
    len  = (skb_len(skb) + 3 + ((uint32_t)skb_data(skb) & 3)) & ~0x3;
    cmdA = (((uint32_t)skb_data(skb) & 0x3) << 16) | TX_CMD_A_INT_FIRST_SEG_ | TX_CMD_A_INT_LAST_SEG_ | skb_len(skb);
...
static void smc911x_tx(struct net_device *dev)
{
    ...
    while (((SMC_GET_TX_FIFO_INF(lp) & TX_FIFO_INF_TSUSED_) >> 16) != 0) {
        DBG(SMC_DEBUG_TX, "%s: Tx stat FIFO used 0x%04x\n", dev->name, (SMC_GET_TX_FIFO_INF(lp) & TX_FIFO_INF_TSUSED_) >> 16);
        tx_status = SMC_GET_TX_STS_FIFO(lp);
        dev->stats.tx_packets++;
        dev->stats.tx_bytes += tx_status >> 16;
        DBG(SMC_DEBUG_TX, "%s: Tx FIFO tag 0x%04x status 0x%04x\n", dev->name, (tx_status & 0xffff0000) >> 16, tx_status & 0x0000ffff);

        /* count Tx errors, but ignore lost carrier errors when in
         * full-duplex mode */
        if ((tx_status & TX_STS_ES_) && !(lp->ctl_rfduplx && !(tx_status & 0x00000306))) {
            dev->stats.tx_errors++;
        }
        ...
        if ((tx_status & (TX_STS_LOC_ | TX_STS_NO_CARR_)) && !lp->ctl_rfduplx) {
 ...
 */
static void smc911x_phy_write(struct net_device *dev, int phyaddr, int phyreg, int phydata)
{
    struct smc911x_local *lp = netdev_private(dev);
        ...
        case CHIP_9217:
            ...
            if (cfg & HW_CFG_EXT_PHY_DET_) {
                ...
                for (phyaddr = 1; phyaddr < 32; ++phyaddr) {
                    ...
                    if (id1 != 0x0000 && id1 != 0xffff && id1 != 0x8000 && id2 != 0x0000 && id2 != 0xffff && id2 != 0x8000) {
...
{
    struct smc911x_local *lp      = container_of(work, struct smc911x_local, phy_configure);
     ...
     */
    SMC_SET_PHY_INT_MASK(lp, phyaddr, PHY_INT_MASK_ENERGY_ON_ | PHY_INT_MASK_ANEG_COMP_ | PHY_INT_MASK_REMOTE_FAULT_ | PHY_INT_MASK_LINK_DOWN_);
...

{
    ...
    struct smc911x_local *lp  = netdev_private(dev);
    uint32_t              status, mask, timeout;
    uint32_t              rx_overrun = 0, cr, pkts;
    uint64_t              flags;
    ...
    if ((SMC_GET_IRQ_CFG(lp) & (INT_CFG_IRQ_INT_ | INT_CFG_IRQ_EN_)) != (INT_CFG_IRQ_INT_ | INT_CFG_IRQ_EN_)) {
        spin_unlock_irq_restore(&lp->lock, flags);
        return IRQ_NONE;
    }
    ...
    do {
        ...
        if ((status & INT_STS_RSFL_) || rx_overrun) {
            ...
            if (pkts != 0) {
                ...
                if (lp->rxdma_active) {
                    DBG(SMC_DEBUG_RX | SMC_DEBUG_DMA, "%s: RX DMA active\n", dev->name);
        ...
        if (status & INT_STS_GPT_INT_) {
            DBG(SMC_DEBUG_RX, "%s: IRQ_CFG 0x%08x FIFO_INT 0x%08x RX_CFG 0x%08x\n", dev->name, SMC_GET_IRQ_CFG(lp), SMC_GET_FIFO_INT(lp),
                SMC_GET_RX_CFG(lp));
            DBG(SMC_DEBUG_RX,
                "%s: Rx Stat FIFO Used 0x%02x "
                "Data FIFO Used 0x%04x Stat FIFO 0x%08x\n",
                dev->name, (SMC_GET_RX_FIFO_INF(lp) & 0x00ff0000) >> 16, SMC_GET_RX_FIFO_INF(lp) & 0xffff, SMC_GET_RX_STS_FIFO_PEEK(lp));
            SMC_SET_GPT_CFG(lp, GPT_CFG_TIMER_EN_ | 10000);
...
#ifdef SMC_USE_DMA
static void smc911x_tx_dma_irq(int dma, void *data)
{
    ...
    if (lp->pending_tx_skb != NULL) {
        ...
        DBG(SMC_DEBUG_TX | SMC_DEBUG_DMA, "%s: No pending Tx packets. DMA disabled\n", dev->name);
        spin_lock_irq_save(&lp->lock, flags);
        lp->txdma_active = 0;
    ...
    DBG(SMC_DEBUG_TX | SMC_DEBUG_DMA, "%s: TX DMA irq completed\n", dev->name);
}

static void smc911x_rx_dma_irq(int dma, void *data)
{
    struct net_device    *dev    = (struct net_device *)data;
    uint64_t              ioaddr = dev->base_addr;
    struct smc911x_local *lp     = netdev_private(dev);
...
{
    struct smc911x_local *lp = netdev_private(dev);
    uint32_t multicast_table[2];
    uint32_t mcr, update_multicast = 0;
    uint64_t flags;
 ...
 */
static int smc911x_open(struct net_device *dev)
{
    struct smc911x_local *lp = netdev_private(dev);
 ...
 * Ethtool support
 */
static int smc911x_ethtool_getsettings(struct net_device *dev, struct ethtool_cmd *cmd)
{
    struct smc911x_local *lp = netdev_private(dev);
    int                   ret, status;
    uint64_t              flags;
    ...
    if (lp->phy_type != 0) {
        ...
        cmd->supported = SUPPORTED_10baseT_Half | SUPPORTED_10baseT_Full | SUPPORTED_TP | SUPPORTED_AUI;
        ...
        cmd->duplex = (status & (PHY_SPECIAL_SPD_10FULL_ | PHY_SPECIAL_SPD_100FULL_)) ? DUPLEX_FULL : DUPLEX_HALF;
        ret         = 0;
    }
...
static int smc911x_ethtool_setsettings(struct net_device *dev, struct ethtool_cmd *cmd)
{
    struct smc911x_local *lp = netdev_private(dev);
    int                   ret;
    uint64_t              flags;
    ...
    if (lp->phy_type != 0) {
        ...
        if (cmd->autoneg != AUTONEG_DISABLE || cmd->speed != SPEED_10 || (cmd->duplex != DUPLEX_HALF && cmd->duplex != DUPLEX_FULL) ||
...
static void smc911x_ethtool_getdrvinfo(struct net_device *dev, struct ethtool_drvinfo *info)
{
...
static uint32_t smc911x_ethtool_getmsglevel(struct net_device *dev)
{
    struct smc911x_local *lp = netdev_private(dev);
    return lp->msg_enable;
}

static void smc911x_ethtool_setmsglevel(struct net_device *dev, uint32_t level)
{
    struct smc911x_local *lp = netdev_private(dev);
    lp->msg_enable           = level;
}
...
static int smc911x_ethtool_getregslen(struct net_device *dev)
{
    ...
    return (((E2P_CMD - ID_REV) / 4 + 1) + (WUCSR - MAC_CR) + 1 + 32) * sizeof(uint32_t);
}

static void smc911x_ethtool_getregs(struct net_device *dev, struct ethtool_regs *regs, void *buf)
{
    struct smc911x_local *lp = netdev_private(dev);
    uint64_t              flags;
    uint32_t              reg, i, j = 0;
...
static inline int smc911x_ethtool_write_eeprom_cmd(struct net_device *dev, int cmd, int addr)
{
    struct smc911x_local *lp = netdev_private(dev);
    int                   ret;
    ...
    SMC_SET_E2P_CMD(lp, E2P_CMD_EPC_BUSY_ | ((cmd) & (0x7 << 28)) | ((addr) & 0xFF));
    return 0;
}

static inline int smc911x_ethtool_read_eeprom_byte(struct net_device *dev, uint8_t *data)
{
    struct smc911x_local *lp = netdev_private(dev);
    int                   ret;
...
static inline int smc911x_ethtool_write_eeprom_byte(struct net_device *dev, uint8_t data)
{
    struct smc911x_local *lp = netdev_private(dev);
    int                   ret;
...
static int smc911x_ethtool_geteeprom(struct net_device *dev, struct ethtool_eeprom *eeprom, uint8_t *data)
{
    uint8_t eebuf[SMC911X_EEPROM_LEN];
    int     i, ret;
...
static int smc911x_ethtool_seteeprom(struct net_device *dev, struct ethtool_eeprom *eeprom, uint8_t *data)
{
    int i, ret;
...
static int smc911x_probe(struct net_device *dev)
{
    ...
    switch (lp->tx_fifo_kb) {
        ...
        default:
            PRINTK("%s: ERROR -- no AFC_CFG setting found", dev->name);
            break;
    }

    DBG(SMC_DEBUG_MISC | SMC_DEBUG_TX | SMC_DEBUG_RX, "%s: tx_fifo %d rx_fifo %d afc_cfg 0x%08x\n", CARDNAME, lp->tx_fifo_size, lp->rx_fifo_size,
        lp->afc_cfg);
    ...
    if (retval == VMM_OK) {
        ...
        printk("%s: %s (rev %d) IRQ %02X", dev->name, version_string, lp->revision, dev->irq);
...
static int smc911x_driver_probe(vmm_device_t *dev)
{
    int                   rc = VMM_OK;
    ...
    lp         = netdev_private(ndev);
    lp->netdev = ndev;

    rc         = vmm_device_tree_request_regmap(dev->of_node, &addr, 0, "SMC911x");
    ...
    DBG(SMC_DEBUG_MISC, "vmm_device_tree_regmap success at address 0x%02X\n", addr);
    lp->base  = (void *)addr;
    ...
    if (vmm_device_tree_getattr(dev->of_node, "smsc,irq-active-high")) {
...
free_ioreamp_mem:
    vmm_device_tree_regunmap_release(dev->of_node, addr, 0);
free_ndev:

    if (ndev->private) {
        vmm_free(ndev->private);
    }
...
static int smc911x_driver_remove(vmm_device_t *dev)
{
    int                rc   = VMM_OK;
    struct net_device *ndev = (struct net_device *)dev->private;
...
static struct vmm_device_tree_nodeid smc911x_devid_table[] = {
    {.compatible = "smc911x"},
    {/* end of list */},
};

static vmm_driver_t smc911x_driver = {
    .name        = "smc911x_driver",
...
{
    return vmm_device_driver_register_driver(&smc911x_driver);
}

static void __exit smc911x_driver_exit(void)
{
    vmm_device_driver_unregister_driver(&smc911x_driver);
}
