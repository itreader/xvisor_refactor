 ...
 */
static const char version[] = "smc91x.c: v1.1, sep 22 2004 by Nicolas Pitre <nico@fluxnic.net>\n";
...
{
    struct smc_local *lp     = netdev_private(dev);
...
static inline void smc_rcv(struct net_device *dev)
{
    struct smc_local *lp     = netdev_private(dev);
    void __iomem     *ioaddr = lp->base;
    uint32_t          packet_number, status, packet_len;
 ...
 */
static void smc_hardware_send_pkt(uint64_t data)
{
    struct net_device *dev    = (struct net_device *)data;
    struct smc_local  *lp     = netdev_private(dev);
    void __iomem      *ioaddr = lp->base;
    struct sk_buff    *skb;
    uint32_t           packet_no, len;
    unsigned char     *buf;
    uint64_t           flags;
...
static int smc_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    struct smc_local *lp     = netdev_private(dev);
    void __iomem     *ioaddr = lp->base;
    uint32_t          numPages, poll_count, status;
    uint64_t          flags;
    ...
    if (!poll_count) {
         ...
         * immediately.
         */
        smc_hardware_send_pkt((uint64_t)dev);
    }
...
static void smc_tx(struct net_device *dev)
{
    struct smc_local *lp     = netdev_private(dev);
    void __iomem     *ioaddr = lp->base;
    uint32_t          saved_packet, packet_no, tx_status, pkt_len;
    ...
    if (tx_status & (ES_LATCOL | ES_16COL)) {
        ...
        if (!(dev->stats.tx_window_errors & 63) && net_ratelimit()) {
            printk(
                KERN_INFO "%s: unexpectedly large number of "
...
static void smc_mii_out(struct net_device *dev, uint32_t val, int bits)
{
    struct smc_local *lp     = netdev_private(dev);
    void __iomem     *ioaddr = lp->base;
    uint32_t          mii_reg, mask;
...
static uint32_t smc_mii_in(struct net_device *dev, int bits)
{
    struct smc_local *lp     = netdev_private(dev);
    void __iomem     *ioaddr = lp->base;
    uint32_t          mii_reg, mask, val;
 ...
 */
static void smc_phy_write(struct net_device *dev, int phyaddr, int phyreg, int phydata)
{
    struct smc_local *lp     = netdev_private(dev);
    void __iomem     *ioaddr = lp->base;
...
{
    struct smc_local  *lp      = container_of(work, struct smc_local, phy_configure);
     ...
     */
    smc_phy_write(
        dev, phyaddr, PHY_MASK_REG,
        PHY_INT_LOSSSYNC | PHY_INT_CWRD | PHY_INT_SSD | PHY_INT_ESD | PHY_INT_RPOL | PHY_INT_JAB | PHY_INT_SPDDET | PHY_INT_DPLXDET);
...
static void smc_10bt_check_media(struct net_device *dev, int init)
{
    struct smc_local *lp     = netdev_private(dev);
    void __iomem     *ioaddr = lp->base;
    uint32_t          old_carrier, new_carrier;
...
static int smc_open(struct net_device *dev)
{
    ...
    lp->rpc_cur_mode = RPC_DEFAULT | lp->cfg.leda << RPC_LSXA_SHFT | lp->cfg.ledb << RPC_LSXB_SHFT;
...
static uint32_t smc_ethtool_getmsglevel(struct net_device *dev)
{
    struct smc_local *lp = netdev_private(dev);
    return lp->msg_enable;
}

static void smc_ethtool_setmsglevel(struct net_device *dev, uint32_t level)
{
    struct smc_local *lp = netdev_private(dev);
    lp->msg_enable = level;
}

static int smc_write_eeprom_word(struct net_device *dev, uint16_t addr, uint16_t word)
{
    uint16_t ctl;
    struct smc_local *lp = netdev_private(dev);
    void __iomem *ioaddr = lp->base;
...
static int smc_read_eeprom_word(struct net_device *dev, uint16_t addr, uint16_t *word)
{
    uint16_t ctl;
    struct smc_local *lp = netdev_private(dev);
    void __iomem *ioaddr = lp->base;
...
#endif
/*
 * Function: smc_probe(uint64_t ioaddr)
 *
 * Purpose:
 ...
 * o  GRAB the region
 */
static int __devinit smc_probe(struct net_device *dev, void __iomem *ioaddr, uint64_t irq_flags)
{
    struct smc_local *lp              = netdev_private(dev);
    static int        version_printed = 0;
    int               retval;
    uint32_t          val, revision_register;
    ...
    if ((val & 0xFF00) != 0x3300) {
        if ((val & 0xFF) == 0x33) {
            printk(
                KERN_WARNING "%s: Detected possible byte-swapped interface"
                             " at IOADDR %p\n",
                CARDNAME, ioaddr);
        }
    ...
    if (((uint64_t)ioaddr & (0x3e0 << SMC_IO_SHIFT)) != val) {
    ...
    tasklet_init(&lp->tx_task, smc_hardware_send_pkt, (uint64_t)dev);
    ...
    if (lp->cfg.flags & SMC91X_USE_DMA) {
        int dma = pxa_request_dma(dev->name, DMA_PRIO_LOW, smc_pxa_dma_irq, NULL);
    ...
    if (retval == 0) {
        ...
        printk("%s: %s (rev %d) at %p IRQ %d", dev->name, version_string, revision_register & 0x0f, lp->base, dev->irq);

        if (dev->dma != (unsigned char)-1) {
            printk(" DMA %d", dev->dma);
        }

        printk("%s%s\n", lp->cfg.flags & SMC91X_NOWAIT ? " [nowait]" : "", THROTTLE_TX_PKTS ? " [throttle_tx]" : "");
...
static int smc_enable_device(struct platform_device *pdev)
{
    ...
    struct smc_local *lp = netdev_private(ndev);
    uint64_t flags;
                              ...
                              struct net_device *ndev)
{
    ...
    struct smc_local *lp __maybe_unused = netdev_private(ndev);
...
static int __devinit smc_drv_probe(struct platform_device *pdev)
{
    ...
    uint64_t irq_flags = SMC_IRQ_FLAGS;
    int ret;
 ...
 *  anything else, error
 */
static int __devinit smc_drv_probe(vmm_device_t *pdev)
{
...
#endif
    ret = vmm_device_tree_request_regmap(pdev->of_node, &reg_addr, 0, "SMC91x");
...
static int smc_drv_remove(vmm_device_t *dev)
{
    int                rc   = VMM_OK;
...
static struct vmm_device_tree_nodeid smc_devid_table[] = {
    {.compatible = "smc91x"},
    {/* end of list */},
};

static vmm_driver_t smc_driver = {
    .name        = "smc_driver",
...
{
    return vmm_device_driver_register_driver(&smc_driver);
}

static void __exit smc91x_driver_exit(void)
{
    vmm_device_driver_unregister_driver(&smc_driver);
}
