...
#if defined(CONFIG_ARCH_LUBBOCK) || defined(CONFIG_MACH_MAINSTONE) || defined(CONFIG_MACH_ZYLONITE) || defined(CONFIG_MACH_LITTLETON) ||             \
    defined(CONFIG_MACH_ZYLONITE2) || defined(CONFIG_ARCH_VIPER) || defined(CONFIG_MACH_STARGATE2)
...
static inline void SMC_outw(uint16_t val, void __iomem *ioaddr, int reg)
{
...
#define SMC_inw(a, r)     in_be16((volatile uint16_t *)((a) + (r)))
#define SMC_outw(v, a, r) out_be16((volatile uint16_t *)((a) + (r)), v)
#define SMC_insw(a, r, p, l)                                                                                                                         \
    do {                                                                                                                                             \
        uint64_t  __port = (a) + (r);                                                                                                                \
        uint16_t *__p    = (uint16_t *)(p);                                                                                                          \
        int       __l    = (l);                                                                                                                      \
        insw(__port, __p, __l);                                                                                                                      \
        while (__l > 0) {                                                                                                                            \
            *__p = swab16(*__p);                                                                                                                     \
...
#elif defined(CONFIG_MACH_LOGICPD_PXA270) || defined(CONFIG_MACH_NOMADIK_8815NHK)
...
#elif defined(CONFIG_ARCH_INNOKOM) || defined(CONFIG_ARCH_PXA_IDP) || defined(CONFIG_ARCH_RAMSES) || defined(CONFIG_ARCH_PCM027)
...
#define SMC_inb(a, r)         inb(((uint32_t)a) + (r))
#define SMC_inw(a, r)         inw(((uint32_t)a) + (r))
#define SMC_outb(v, a, r)     outb(v, ((uint32_t)a) + (r))
#define SMC_outw(v, a, r)     outw(v, ((uint32_t)a) + (r))
#define SMC_insw(a, r, p, l)  insw(((uint32_t)a) + (r), p, l)
#define SMC_outsw(a, r, p, l) outsw(((uint32_t)a) + (r), p, l)
...
#elif defined(CONFIG_MACH_LPD79520) || defined(CONFIG_MACH_LPD7A400) || defined(CONFIG_MACH_LPD7A404)
...
#define SMC_inw(a, r)                                                                                                                                \
    ({                                                                                                                                               \
        unsigned short v = readw((void *)((a) + (r)));                                                                                               \
        LPD7X_IOBARRIER;                                                                                                                             \
        v;                                                                                                                                           \
    })
...
static inline void LPD7_SMC_insw(unsigned char *a, int r, unsigned char *p, int l)
{
...
static inline void LPD7_SMC_outsw(unsigned char *a, int r, unsigned char *p, int l)
{
    unsigned short *ps = (unsigned short *)p;
...
#ifdef SMC_insl
#undef SMC_insl
#define SMC_insl(a, r, p, l) smc_pxa_dma_insl(a, lp, r, dev->dma, p, l)

static inline void smc_pxa_dma_insl(void __iomem *ioaddr, struct smc_local *lp, int reg, int dma, uint8_t *buf, int len)
{
    uint64_t   physaddr = lp->physaddr;
    dma_addr_t dmabuf;
    ...
    if ((long)buf & 4) {
        *((uint32_t *)buf) = SMC_inl(ioaddr, reg);
        buf += 4;
        len--;
    }
    ...
    DCMD(dma)  = (DCMD_INCTRGADDR | DCMD_BURST32 | DCMD_WIDTH4 | (DCMD_LENGTH & len));
...
#ifdef SMC_insw
#undef SMC_insw
#define SMC_insw(a, r, p, l) smc_pxa_dma_insw(a, lp, r, dev->dma, p, l)

static inline void smc_pxa_dma_insw(void __iomem *ioaddr, struct smc_local *lp, int reg, int dma, uint8_t *buf, int len)
{
    uint64_t   physaddr = lp->physaddr;
    dma_addr_t dmabuf;
    ...
    /* 64 bit alignment is required for memory to memory DMA */
    while ((long)buf & 6) {
        *((uint16_t *)buf) = SMC_inw(ioaddr, reg);
        buf += 2;
        len--;
    }
    ...
    DTADR(dma) = dmabuf;
    DSADR(dma) = physaddr + reg;
    DCMD(dma)  = (DCMD_INCTRGADDR | DCMD_BURST32 | DCMD_WIDTH2 | (DCMD_LENGTH & len));
    DCSR(dma)  = DCSR_NODESC | DCSR_RUN;
...
static void smc_pxa_dma_irq(int dma, void *dummy)
{
    DCSR(dma) = 0;
}
...
#define SMC_GET_PN(lp)           (SMC_8BIT(lp) ? (SMC_inb(ioaddr, PN_REG(lp))) : (SMC_inw(ioaddr, PN_REG(lp)) & 0xFF))
...
#define SMC_GET_AR(lp)     (SMC_8BIT(lp) ? (SMC_inb(ioaddr, AR_REG(lp))) : (SMC_inw(ioaddr, PN_REG(lp)) >> 8))

#define SMC_GET_TXFIFO(lp) (SMC_8BIT(lp) ? (SMC_inb(ioaddr, TXFIFO_REG(lp))) : (SMC_inw(ioaddr, TXFIFO_REG(lp)) & 0xFF))

#define SMC_GET_RXFIFO(lp) (SMC_8BIT(lp) ? (SMC_inb(ioaddr, RXFIFO_REG(lp))) : (SMC_inw(ioaddr, TXFIFO_REG(lp)) >> 8))

#define SMC_GET_INT(lp)    (SMC_8BIT(lp) ? (SMC_inb(ioaddr, INT_REG(lp))) : (SMC_inw(ioaddr, INT_REG(lp)) & 0xFF))
...
#define SMC_GET_INT_MASK(lp) (SMC_8BIT(lp) ? (SMC_inb(ioaddr, IM_REG(lp))) : (SMC_inw(ioaddr, INT_REG(lp)) >> 8))
...
#define SMC_PUT_PKT_HDR(lp, status, length)                                                                                                          \
    do {                                                                                                                                             \
        if (SMC_32BIT(lp))                                                                                                                           \
            SMC_outl((status) | (length) << 16, ioaddr, DATA_REG(lp));                                                                               \
        else {                                                                                                                                       \
...
#define SMC_GET_PKT_HDR(lp, status, length)                                                                                                          \
    do {                                                                                                                                             \
        if (SMC_32BIT(lp)) {                                                                                                                         \
            uint32_t __val = SMC_inl(ioaddr, DATA_REG(lp));                                                                                          \
            (status)       = __val & 0xffff;                                                                                                         \
            (length)       = __val >> 16;                                                                                                            \
        } else {                                                                                                                                     \
...
#define SMC_PUSH_DATA(lp, p, l)                                                                                                                      \
    do {                                                                                                                                             \
        if (SMC_32BIT(lp)) {                                                                                                                         \
            ...
            if (__len >= 2 && (uint64_t)__ptr & 2) {                                                                                                 \
                __len -= 2;                                                                                                                          \
                SMC_outw(*(uint16_t *)__ptr, ioaddr, DATA_REG(lp));                                                                                  \
                __ptr += 2;                                                                                                                          \
            }                                                                                                                                        \
    ...
    do {                                                                                                                                             \
        if (SMC_32BIT(lp)) {                                                                                                                         \
            ...
            if ((uint64_t)__ptr & 2) {                                                                                                               \
                ...
                __ptr -= 2;                                                                                                                          \
                __len += 2;                                                                                                                          \
                SMC_SET_PTR(lp, 2 | PTR_READ | PTR_RCV | PTR_AUTOINC);                                                                               \
            }                                                                                                                                        \
            if (SMC_CAN_USE_DATACS && lp->datacs)                                                                                                    \
                __ioaddr = lp->datacs;                                                                                                               \
            __len += 2;                                                                                                                              \
