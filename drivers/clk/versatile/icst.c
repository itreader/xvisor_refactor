...
uint64_t icst_hz(const struct icst_params *p, struct icst_vco vco)
{
...
struct icst_vco icst_hz_to_vco(const struct icst_params *p, uint64_t freq)
{
    struct icst_vco vco = {.s = 1, .v = p->vd_max, .r = p->rd_max};
    uint64_t        f;
    uint32_t        i = 0, rd, best = (uint32_t)-1;
