...
static uint64_t vexpress_osc_recalc_rate(struct clock_hw *hw, uint64_t parent_rate)
{
...
static long vexpress_osc_round_rate(struct clock_hw *hw, uint64_t rate, uint64_t *parent_rate)
{
    struct vexpress_osc *osc = to_vexpress_osc(hw);
...
static int vexpress_osc_set_rate(struct clock_hw *hw, uint64_t rate, uint64_t parent_rate)
{
    struct vexpress_osc *osc = to_vexpress_osc(hw);
...
static struct clock_ops vexpress_osc_ops = {
...
struct clk *__init vexpress_osc_setup(struct device *dev)
{
    ...
    return clock_register(NULL, &osc->hw);
}
...
void __init vexpress_osc_of_setup(struct device_node *node)
{
    ...
    if (of_property_read_u32_array(node, "freq-range", range, ARRAY_SIZE(range)) == 0) {
    ...
    clk              = clock_register(NULL, &osc->hw);
