...
static uint8_t rockchip_muxgrf_get_parent(struct clock_hw *hw)
{
    struct rockchip_muxgrf_clock *mux  = to_muxgrf_clock(hw);
    uint32_t                      mask = GENMASK(mux->width - 1, 0);
    uint32_t                      val;
...
static int rockchip_muxgrf_set_parent(struct clock_hw *hw, uint8_t index)
{
    struct rockchip_muxgrf_clock *mux  = to_muxgrf_clock(hw);
    uint32_t                      mask = GENMASK(mux->width + mux->shift - 1, mux->shift);
    uint32_t                      val;
...
static const struct clock_ops rockchip_muxgrf_clock_ops = {
...
struct clk *rockchip_clock_register_muxgrf(
    const char *name, const char *const *parent_names, uint8_t num_parents, int flags, struct regmap *regmap, int reg, int shift, int width,
    int mux_flags)
{
    ...
    init.ops              = &rockchip_muxgrf_clock_ops;
    ...
    clk                   = clock_register(NULL, &muxgrf_clock->hw);
