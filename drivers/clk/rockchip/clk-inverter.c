...
static int rockchip_inv_get_phase(struct clock_hw *hw)
{
...
static int rockchip_inv_set_phase(struct clock_hw *hw, int degrees)
{
    struct rockchip_inv_clock *inv_clock = to_inv_clock(hw);
    uint32_t                   val;
    ...
    if (degrees % 180 == 0) {
        ...
        pr_err("%s: unsupported phase %d for %s\n", __func__, degrees, clock_hw_get_name(hw));
        return -EINVAL;
    }

    if (inv_clock->flags & ROCKCHIP_INVERTER_HIWORD_MASK) {
        writel(HIWORD_UPDATE(val, INVERTER_MASK, inv_clock->shift), inv_clock->reg);
    } else {
        uint64_t flags;
        uint32_t reg;

        spin_lock_irq_save(inv_clock->lock, flags);
        ...
        spin_unlock_irq_restore(inv_clock->lock, flags);
    }

    return 0;
}

static const struct clock_ops rockchip_inv_clock_ops = {
    ...
    const char *name, const char *const *parent_names, uint8_t num_parents, void __iomem *reg, int shift, int flags, spinlock_t *lock)
{
    ...
    init.ops           = &rockchip_inv_clock_ops;
    ...
    clk                = clock_register(NULL, &inv_clock->hw);
