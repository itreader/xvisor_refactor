...
#define ARM_SMMU_OPT_SECURE_CFG_ACCESS (1 << 0)
    ...
    uint32_t  num_context_irqs;
    uint32_t *irqs;

    uint16_t              num_mapping_groups;
    uint16_t              streamid_mask;
    uint16_t              smr_mask_mask;
    struct arm_smmu_smr  *smrs;
    struct arm_smmu_s2cr *s2crs;
    vmm_mutex_t           stream_map_mutex;

    uint32_t num_s2_context_banks;
    uint32_t num_context_banks;
    uint32_t pgshift;
    DECLARE_BITMAP(context_map, ARM_SMMU_MAX_CBS);
    struct arm_smmu_cb *cbs;
    atomic_t            irptndx;

    uint32_t cavium_id_base; /* Specific to Cavium */
...
struct arm_smmu_domain {
    ...
    struct io_page_table_ops      *page_table_ops;
    const struct iommu_gather_ops *tlb_ops;
    struct arm_smmu_cfg            cfg;
    enum arm_smmu_domain_stage     stage;
    vmm_mutex_t                    init_mutex; /* Protects smmu pointer */
...
static struct arm_smmu_domain *to_smmu_domain(vmm_iommu_domain_t *dom)
{
    return container_of(dom, struct arm_smmu_domain, domain);
}

static struct arm_smmu_device *to_smmu_device(vmm_iommu_controller_t *ct)
{
...
static void parse_driver_options(struct arm_smmu_device *smmu)
{
    ...
    do {
        if (vmm_device_tree_getattr(smmu->node, arm_smmu_options[i].prop)) {
...
static void arm_smmu_write_s2cr(struct arm_smmu_device *smmu, int idx)
{
    ...
    uint32_t              reg  = (s2cr->type & S2CR_TYPE_MASK) << S2CR_TYPE_SHIFT | (s2cr->cbndx & S2CR_CBNDX_MASK) << S2CR_CBNDX_SHIFT |
...
static int arm_smmu_find_sme(struct arm_smmu_device *smmu, uint16_t id, uint16_t mask)
{
    ...
    for (i = 0; i < smmu->num_mapping_groups; ++i) {
         ...
         */
        if ((mask & smrs[i].mask) == mask && !((id ^ smrs[i].id) & ~smrs[i].mask)) {
            return i;
        }
...
static void arm_smmu_master_free_smes(struct arm_smmu_archdata *archdata)
{
    ...
    for (i = 0; i < archdata->num_sid; i++) {
        if ((archdata->sids[i].sme > -1) && arm_smmu_free_sme(smmu, archdata->sids[i].sme)) {
...
static int __arm_smmu_alloc_bitmap(uint64_t *map, int start, int end)
{
    int idx;
...
static void __arm_smmu_free_bitmap(uint64_t *map, int idx)
{
    clear_bit(idx, map);
}

/* Wait for any pending TLB invalidations to complete */
static void __arm_smmu_tlb_sync(struct arm_smmu_device *smmu, void *sync, void *status)
{
    uint32_t spin_cnt, delay;
    ...
    vmm_lerror(smmu->node->name, "TLB sync timed out -- SMMU may be deadlocked\n");
}
...
static void arm_smmu_tlb_sync_global(struct arm_smmu_device *smmu)
{
    ...
    vmm_spin_lock_irq_save(&smmu->global_sync_lock, flags);
    __arm_smmu_tlb_sync(smmu, base + ARM_SMMU_GR0_sTLBGSYNC, base + ARM_SMMU_GR0_sTLBGSTATUS);
    vmm_spin_unlock_irq_restore(&smmu->global_sync_lock, flags);
}
...
static void arm_smmu_tlb_sync_context(void *cookie)
{
    ...
    vmm_spin_lock_irq_save(&smmu_domain->cb_lock, flags);
    __arm_smmu_tlb_sync(smmu, base + ARM_SMMU_CB_TLBSYNC, base + ARM_SMMU_CB_TLBSTATUS);
    vmm_spin_unlock_irq_restore(&smmu_domain->cb_lock, flags);
}
...
static void arm_smmu_tlb_inv_range_nosync(uint64_t iova, size_t size, size_t granule, bool leaf, void *cookie)
{
    struct arm_smmu_domain *smmu_domain = cookie;
    struct arm_smmu_cfg    *cfg         = &smmu_domain->cfg;
 ...
 */
static void arm_smmu_tlb_inv_vmid_nosync(uint64_t iova, size_t size, size_t granule, bool leaf, void *cookie)
{
    struct arm_smmu_domain *smmu_domain = cookie;
...
static vmm_irq_return_t arm_smmu_context_fault(int irq, void *dev)
{
    ...
    vmm_lerror(smmu->node->name, "Unhandled context fault: fsr=0x%x, iova=0x%08lx, fsynr=0x%x, cb=%d\n", fsr, iova, fsynr, cfg->cbndx);
...
{
    uint32_t                gfsr, gfsynr0, gfsynr1, gfsynr2;
    ...
    vmm_lerror_once(smmu->node->name, "Unexpected global fault, this could be serious\n");
    vmm_lerror_once(smmu->node->name, "\tGFSR 0x%08x, GFSYNR0 0x%08x, GFSYNR1 0x%08x, GFSYNR2 0x%08x\n", gfsr, gfsynr0, gfsynr1, gfsynr2);
...
static void arm_smmu_init_context_bank(struct arm_smmu_domain *smmu_domain, struct io_page_table_cfg *page_table_cfg)
{
    struct arm_smmu_cfg *cfg    = &smmu_domain->cfg;
    ...
    if (stage1) {
        if (cfg->fmt == ARM_SMMU_CTX_FMT_AARCH32_S) {
            cb->tcr[0] = page_table_cfg->arm_v7s_cfg.tcr;
        } else {
            cb->tcr[0] = page_table_cfg->arm_lpae_s1_cfg.tcr;
            cb->tcr[1] = page_table_cfg->arm_lpae_s1_cfg.tcr >> 32;
        ...
        cb->tcr[0] = page_table_cfg->arm_lpae_s2_cfg.vtcr;
    }

    /* TTBRs */
    if (stage1) {
        if (cfg->fmt == ARM_SMMU_CTX_FMT_AARCH32_S) {
            cb->ttbr[0] = page_table_cfg->arm_v7s_cfg.ttbr[0];
            cb->ttbr[1] = page_table_cfg->arm_v7s_cfg.ttbr[1];
        } else {
            cb->ttbr[0] = page_table_cfg->arm_lpae_s1_cfg.ttbr[0];
            cb->ttbr[0] |= (uint64_t)cfg->asid << TTBRn_ASID_SHIFT;
            cb->ttbr[1] = page_table_cfg->arm_lpae_s1_cfg.ttbr[1];
            cb->ttbr[1] |= (uint64_t)cfg->asid << TTBRn_ASID_SHIFT;
        }
    } else {
        cb->ttbr[0] = page_table_cfg->arm_lpae_s2_cfg.vttbr;
    }

    /* MAIRs (stage-1 only) */
    if (stage1) {
        if (cfg->fmt == ARM_SMMU_CTX_FMT_AARCH32_S) {
            cb->mair[0] = page_table_cfg->arm_v7s_cfg.prrr;
            cb->mair[1] = page_table_cfg->arm_v7s_cfg.nmrr;
        } else {
            cb->mair[0] = page_table_cfg->arm_lpae_s1_cfg.mair[0];
            cb->mair[1] = page_table_cfg->arm_lpae_s1_cfg.mair[1];
        }
    }
}
...
static void arm_smmu_write_context_bank(struct arm_smmu_device *smmu, int idx)
{
     ...
     */
    if (stage1) {
        reg |= (CBAR_S1_BPSHCFG_NSH << CBAR_S1_BPSHCFG_SHIFT) | (CBAR_S1_MEMATTR_WB << CBAR_S1_MEMATTR_SHIFT);
...
static int arm_smmu_init_domain_context(vmm_iommu_domain_t *domain, struct arm_smmu_device *smmu)
{
    uint32_t                  irq;
    int                       start, ret = 0;
    uint64_t                  ias, oas;
    struct io_page_table_ops *page_table_ops;
    struct io_page_table_cfg  page_table_cfg;
    enum io_page_table_fmt    fmt;
    struct arm_smmu_domain   *smmu_domain = to_smmu_domain(domain);
    struct arm_smmu_cfg      *cfg         = &smmu_domain->cfg;
     ...
     * the decision into the io-page_table code where it arguably belongs,
    ...
    if (IS_ENABLED(CONFIG_IOMMU_IO_PGTABLE_ARMV7S) && !IS_ENABLED(CONFIG_64BIT) && !IS_ENABLED(CONFIG_ARM_LPAE) &&
        (smmu->features & ARM_SMMU_FEAT_FMT_AARCH32_S) && (smmu_domain->stage == ARM_SMMU_DOMAIN_S1)) {
        cfg->fmt = ARM_SMMU_CTX_FMT_AARCH32_S;
    }

    if ((IS_ENABLED(CONFIG_64BIT) || cfg->fmt == ARM_SMMU_CTX_FMT_NONE) &&
        (smmu->features & (ARM_SMMU_FEAT_FMT_AARCH64_64K | ARM_SMMU_FEAT_FMT_AARCH64_16K | ARM_SMMU_FEAT_FMT_AARCH64_4K))) {
    ...
    ret = __arm_smmu_alloc_bitmap(smmu->context_map, start, smmu->num_context_banks);
    ...
    page_table_cfg = (struct io_page_table_cfg){
    ...
    if (smmu->features & ARM_SMMU_FEAT_COHERENT_WALK) {
        page_table_cfg.quirks = IO_PGTABLE_QUIRK_NO_DMA;
    }
    ...
    arm_smmu_init_context_bank(smmu_domain, &page_table_cfg);
    ...
    ret = vmm_host_irq_register(irq, "arm-smmu-context-fault", arm_smmu_context_fault, domain);

    if (ret < 0) {
        vmm_lerror(smmu->node->name, "failed to request context IRQ %d (%u)\n", cfg->irptndx, irq);
    ...
    smmu_domain->page_table_ops = page_table_ops;
    return 0;
...
static void arm_smmu_destroy_domain_context(vmm_iommu_domain_t *domain)
{
    struct arm_smmu_domain *smmu_domain = to_smmu_domain(domain);
    struct arm_smmu_device *smmu        = smmu_domain->smmu;
    struct arm_smmu_cfg    *cfg         = &smmu_domain->cfg;
    ...
    free_io_page_table_ops(smmu_domain->page_table_ops);
    __arm_smmu_free_bitmap(smmu->context_map, cfg->cbndx);
}

static int arm_smmu_find_sids(struct arm_smmu_device *mmu, vmm_device_t *dev, struct arm_smmu_sid *sids, uint32_t num_sid)
{
    uint32_t i;
...
static physical_addr_t arm_smmu_iova_to_phys_hard(vmm_iommu_domain_t *domain, physical_addr_t iova)
{
    struct arm_smmu_domain   *smmu_domain = to_smmu_domain(domain);
    struct arm_smmu_device   *smmu        = smmu_domain->smmu;
    struct arm_smmu_cfg      *cfg         = &smmu_domain->cfg;
    ...
    if ((tmp & ATSR_ACTIVE) && !try) {
        vmm_lerror(smmu->node->name, "iova to phys timed out. Falling back to software table walk.\n");
...
static physical_addr_t arm_smmu_iova_to_phys(vmm_iommu_domain_t *domain, physical_addr_t iova)
{
    struct arm_smmu_domain   *smmu_domain = to_smmu_domain(domain);
    struct io_page_table_ops *ops         = smmu_domain->page_table_ops;
    ...
    if ((smmu_domain->smmu->features & ARM_SMMU_FEAT_TRANS_OPS) && (smmu_domain->stage == ARM_SMMU_DOMAIN_S1)) {
...
static int arm_smmu_map(vmm_iommu_domain_t *domain, physical_addr_t iova, physical_addr_t paddr, size_t size, int prot)
{
    int                       ret;
    struct arm_smmu_domain   *smmu_domain = to_smmu_domain(domain);
    struct io_page_table_ops *ops         = smmu_domain->page_table_ops;
...
static size_t arm_smmu_unmap(vmm_iommu_domain_t *domain, physical_addr_t iova, size_t size)
{
    size_t                    ret;
    struct arm_smmu_domain   *smmu_domain = to_smmu_domain(domain);
    struct io_page_table_ops *ops         = smmu_domain->page_table_ops;
...
static void arm_smmu_domain_free(vmm_iommu_domain_t *domain)
{
    struct arm_smmu_domain *smmu_domain = to_smmu_domain(domain);
...
static vmm_iommu_domain_t *arm_smmu_domain_alloc(uint32_t type, vmm_iommu_controller_t *ctrl)
{
    int                     ret;
    struct arm_smmu_domain *smmu_domain;
    struct arm_smmu_device *smmu = to_smmu_device(ctrl);

    if (type != VMM_IOMMU_DOMAIN_UNMANAGED && type != VMM_IOMMU_DOMAIN_IDENTITY) {
        return NULL;
    }
    ...
    if (ret) {
        vmm_lerror(smmu->node->name, "Failed to init SMMU context bank (error %d)\n", ret);
        vmm_free(smmu_domain);
        return NULL;
    }
...
static int arm_smmu_attach_device(vmm_iommu_domain_t *domain, vmm_device_t *dev)
{
    struct arm_smmu_archdata *archdata = dev->iommu_private;
    struct arm_smmu_device   *smmu     = archdata->smmu;
    int                       i, ret = VMM_OK;
    ...
    for (i = 0; i < archdata->num_sid; ++i) {
        ret = arm_smmu_find_sme(smmu, archdata->sids[i].sid, archdata->sids[i].mask);
...
static void arm_smmu_detach_device(vmm_iommu_domain_t *domain, vmm_device_t *dev)
{
    struct arm_smmu_archdata *archdata = dev->iommu_private;
    struct arm_smmu_device   *smmu     = archdata->smmu;
...
static int arm_smmu_add_device(vmm_device_t *dev)
{
...
static void arm_smmu_remove_device(vmm_device_t *dev)
{
    struct arm_smmu_archdata *archdata = dev->iommu_private;
    struct arm_smmu_device   *smmu     = archdata->smmu;
...
static vmm_iommu_ops_t arm_smmu_ops = {
...
{
    uint64_t                size;
    vmm_device_tree_node_t *node     = smmu->node;
    void                   *gr0_base = ARM_SMMU_GR0(smmu);
    uint32_t                id;
    ...
    if (cttw_fw != cttw_reg) {
        vmm_linfo(
            node->name, "arm-smmu: (IDR0.CTTW overridden by "
                        "FW configuration)\n");
    }
    ...
    if (smmu->cb_base != gr0_base + size) {
        vmm_lwarning(
            node->name,
            "%s: SMMU address space size (0x%lx)"
    ...
    if (smmu->model == CAVIUM_SMMUV2) {
        smmu->cavium_id_base = arch_atomic_add_return(&cavium_smmu_context_count, smmu->num_context_banks);
        smmu->cavium_id_base -= smmu->num_context_banks;
        vmm_linfo(
            node->name, "arm-smmu: enabling workaround for "
    ...
    if (smmu->features & (ARM_SMMU_FEAT_FMT_AARCH32_L | ARM_SMMU_FEAT_FMT_AARCH64_4K)) {
...
static int arm_smmu_init(vmm_device_tree_node_t *node, enum arm_smmu_arch_version version, enum arm_smmu_implementation model)
{
    int                     ret = VMM_OK, i;
    irq_flags_t             flags;
    virtual_addr_t          va;
    ...
    if (vmm_device_tree_is_dma_coherent(node)) {
        smmu->features |= ARM_SMMU_FEAT_COHERENT_WALK;
    }

    ret = vmm_device_tree_request_regmap(node, &va, 0, "SMMU");
    ...
    ret        = vmm_device_tree_regsize(node, &pa, 0);
    ...
    ret          = vmm_device_tree_regsize(node, &size, 0);
    ...
    if (vmm_device_tree_read_u32(node, "#global-interrupts", &global_irqs)) {
        vmm_lerror(
            node->name,
    ...
    num_irqs = vmm_device_tree_irq_count(node);
    ...
    for (i = 0; i < num_irqs; ++i) {
        int irq = vmm_device_tree_irq_parse_map(node, i);
    ...
    if (smmu->version == ARM_SMMU_V2 && smmu->num_context_banks != smmu->num_context_irqs) {
        vmm_lerror(
            node->name,
    ...
    for (i = 0; i < smmu->num_global_irqs; ++i) {
        ret = vmm_host_irq_register(smmu->irqs[i], "arm-smmu-global-fault", arm_smmu_global_fault, smmu);

        if (ret) {
            vmm_lerror(
                node->name,
                "%s: failed to register global "
                "irq%d (%u)\n",
    ...
    vmm_spin_lock_irq_save(&smmu_devices_lock, flags);
    list_add_tail(&smmu->list, &smmu_devices);
    vmm_spin_unlock_irq_restore(&smmu_devices_lock, flags);
...
fail_free_irqs:
    vmm_free(smmu->irqs);
fail_unmap_regs:
    vmm_device_tree_regunmap_release(node, (virtual_addr_t)smmu->base, 0);
fail_free_smmu:
    vmm_device_tree_dref_node(smmu->node);
    vmm_free(smmu);
fail:
    return ret;
}

static int arm_smmu_v1_init(vmm_device_tree_node_t *node)
{
    return arm_smmu_init(node, ARM_SMMU_V1, GENERIC_SMMU);
}

static int arm_smmu_v2_init(vmm_device_tree_node_t *node)
{
    return arm_smmu_init(node, ARM_SMMU_V2, GENERIC_SMMU);
}

static int arm_smmu_401_init(vmm_device_tree_node_t *node)
{
    return arm_smmu_init(node, ARM_SMMU_V1_64K, GENERIC_SMMU);
}

static int arm_smmu_500_init(vmm_device_tree_node_t *node)
{
    return arm_smmu_init(node, ARM_SMMU_V2, ARM_MMU500);
}

static int cavium_smmu_v2_init(vmm_device_tree_node_t *node)
{
