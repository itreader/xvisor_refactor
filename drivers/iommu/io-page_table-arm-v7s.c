...
typedef uint32_t arm_v7s_iopte;

static bool selftest_running = FALSE;

struct arm_v7s_io_page_table {
...
/* Struct accessors */
#define io_page_table_to_data(x)     container_of((x), struct arm_v7s_io_page_table, iop)
...
#define ARM_V7S_TABLE_SIZE(lvl)      (ARM_V7S_PTES_PER_LVL(lvl) * sizeof(arm_v7s_iopte))
...
#define ARM_V7S_PTE_AP_UNPRIVATE       ARM_V7S_ATTR_AP1
...
#define ARM_V7S_TTBR_IRGN_ATTR(attr)   ((((attr) & 0x1) << 6) | (((attr) & 0x2) >> 1))
...
static void *__arm_v7s_alloc_table(int lvl, struct arm_v7s_io_page_table *data)
{
    size_t         p, size = ARM_V7S_TABLE_SIZE(lvl);
    virtual_addr_t table = 0x0;

    if (lvl == 1) {
        table = vmm_host_alloc_aligned_pages(VMM_SIZE_TO_PAGE(size), ilog2(size), VMM_MEMORY_FLAGS_NORMAL_NOCACHE);
    } else if (lvl == 2) {
        table = vmm_host_alloc_pages(VMM_SIZE_TO_PAGE(size), VMM_MEMORY_FLAGS_NORMAL_NOCACHE);
    } else {
        return NULL;
    }

    for (p = 0; p < (VMM_SIZE_TO_PAGE(size) * VMM_PAGE_SIZE); p += 4) {
        *(uint32_t *)((void *)table + p) = 0x0;
    }

    return (void *)table;
}

static void __arm_v7s_free_table(void *table, int lvl, struct arm_v7s_io_page_table *data)
{
...
static void __arm_v7s_pte_sync(arm_v7s_iopte *ptep, int num_entries, struct io_page_table_cfg *cfg)
{
    arch_smp_wmb();
}

static void __arm_v7s_set_pte(arm_v7s_iopte *ptep, arm_v7s_iopte pte, int num_entries, struct io_page_table_cfg *cfg)
{
    int i;
...
static arm_v7s_iopte arm_v7s_prot_to_pte(int prot, int lvl, struct io_page_table_cfg *cfg)
{
    ...
    if (ap) {
        pte |= ARM_V7S_PTE_AF | ARM_V7S_PTE_AP_UNPRIVATE;
...
static arm_v7s_iopte arm_v7s_pte_to_cont(arm_v7s_iopte pte, int lvl)
{
    if (lvl == 1) {
        ...
        pte |= (xn << ARM_V7S_CONT_PAGE_XN_SHIFT) | (tex << ARM_V7S_CONT_PAGE_TEX_SHIFT) | ARM_V7S_PTE_TYPE_CONT_PAGE;
    }
...
static arm_v7s_iopte arm_v7s_cont_to_pte(arm_v7s_iopte pte, int lvl)
{
    if (lvl == 1) {
        ...
        arm_v7s_iopte tex = pte & (ARM_V7S_CONT_PAGE_TEX_MASK << ARM_V7S_CONT_PAGE_TEX_SHIFT);

        pte ^= xn | tex | ARM_V7S_PTE_TYPE_CONT_PAGE;
        pte |= (xn >> ARM_V7S_CONT_PAGE_XN_SHIFT) | (tex >> ARM_V7S_CONT_PAGE_TEX_SHIFT) | ARM_V7S_PTE_TYPE_PAGE;
    }
...
static int __arm_v7s_unmap(struct arm_v7s_io_page_table *, physical_addr_t, size_t, int, arm_v7s_iopte *);

static int arm_v7s_init_pte(
    struct arm_v7s_io_page_table *data, physical_addr_t iova, physical_addr_t paddr, int prot, int lvl, int num_entries, arm_v7s_iopte *ptep)
{
...
static int __arm_v7s_map(
    struct arm_v7s_io_page_table *data, physical_addr_t iova, physical_addr_t paddr, size_t size, int prot, int lvl, arm_v7s_iopte *ptep)
{
    int                       rc;
    physical_addr_t           pa;
    struct io_page_table_cfg *cfg = &data->iop.cfg;
    ...
    if (num_entries) {
        return arm_v7s_init_pte(data, iova, paddr, prot, lvl, num_entries, ptep);
    }
...
static int arm_v7s_map(struct io_page_table_ops *ops, physical_addr_t iova, physical_addr_t paddr, size_t size, int prot)
{
    struct arm_v7s_io_page_table *data = io_page_table_ops_to_data(ops);
    ...
    if (iop->cfg.quirks & IO_PGTABLE_QUIRK_TLBI_ON_MAP) {
        io_page_table_tlb_add_flush(iop, iova, size, ARM_V7S_BLOCK_SIZE(2), FALSE);
...
static void arm_v7s_free_page_table(struct io_page_table *iop)
{
    struct arm_v7s_io_page_table *data = io_page_table_to_data(iop);
    int                           i;
...
static void arm_v7s_split_cont(struct arm_v7s_io_page_table *data, physical_addr_t iova, int idx, int lvl, arm_v7s_iopte *ptep)
{
    struct io_page_table *iop = &data->iop;
    arm_v7s_iopte         pte;
    size_t                size = ARM_V7S_BLOCK_SIZE(lvl);
...
static int arm_v7s_split_block_unmap(struct arm_v7s_io_page_table *data, physical_addr_t iova, size_t size, arm_v7s_iopte *ptep)
{
    physical_addr_t block_start, block_end, block_size;
    physical_addr_t block_paddr;
    arm_v7s_iopte   table = 0;
    int             prot  = arm_v7s_pte_to_prot(*ptep, 1);

    block_size            = ARM_V7S_BLOCK_SIZE(1);
    block_start           = iova & ARM_V7S_LVL_MASK(1);
    block_end             = block_start + ARM_V7S_BLOCK_SIZE(1);
    block_paddr           = *ptep & ARM_V7S_LVL_MASK(1);
    ...
    for (; block_start < block_end; block_start += size, block_paddr += size) {
        ...
        tablep = &table - ARM_V7S_LVL_IDX(block_start, 1);

        if (__arm_v7s_map(data, block_start, block_paddr, size, prot, 1, tablep) < 0) {
            if (table) {
...
static int __arm_v7s_unmap(struct arm_v7s_io_page_table *data, physical_addr_t iova, size_t size, int lvl, arm_v7s_iopte *ptep)
{
    ...
    if (num_entries) {
        size_t block_size = ARM_V7S_BLOCK_SIZE(lvl);
        ...
        for (i = 0; i < num_entries; i++) {
            if (ARM_V7S_PTE_IS_TABLE(pte[i], lvl)) {
                ...
                io_page_table_tlb_add_flush(iop, iova, block_size, ARM_V7S_BLOCK_SIZE(lvl + 1), FALSE);
                io_page_table_tlb_sync(iop);
                ptep = iopte_deref(pte[i], lvl);
                __arm_v7s_free_table(ptep, lvl + 1, data);
            } else {
                io_page_table_tlb_add_flush(iop, iova, block_size, block_size, TRUE);
            }
         ...
         */
        return arm_v7s_split_block_unmap(data, iova, size, ptep);
    }
...
static int arm_v7s_unmap(struct io_page_table_ops *ops, physical_addr_t iova, size_t size)
{
    struct arm_v7s_io_page_table *data = io_page_table_ops_to_data(ops);
    size_t                        unmapped;
...
static physical_addr_t arm_v7s_iova_to_phys(struct io_page_table_ops *ops, physical_addr_t iova)
{
    struct arm_v7s_io_page_table *data = io_page_table_ops_to_data(ops);
...
static struct io_page_table *arm_v7s_alloc_page_table(struct io_page_table_cfg *cfg, void *cookie)
{
    int                           rc;
    physical_addr_t               pa;
    struct arm_v7s_io_page_table *data;

    if (cfg->ias > ARM_V7S_ADDR_BITS || cfg->oas > ARM_V7S_ADDR_BITS) {
        return NULL;
    }

    if (cfg->quirks & ~(IO_PGTABLE_QUIRK_ARM_NS | IO_PGTABLE_QUIRK_NO_PERMS | IO_PGTABLE_QUIRK_TLBI_ON_MAP | IO_PGTABLE_QUIRK_ARM_MTK_4GB)) {
        return NULL;
    }

    /* If ARM_MTK_4GB is enabled, the NO_PERMS is also expected. */
    if (cfg->quirks & IO_PGTABLE_QUIRK_ARM_MTK_4GB && !(cfg->quirks & IO_PGTABLE_QUIRK_NO_PERMS)) {
        return NULL;
    }
     ...
     */
    cfg->arm_v7s_cfg.prrr = ARM_V7S_PRRR_TR(1, ARM_V7S_PRRR_TYPE_DEVICE) | ARM_V7S_PRRR_TR(4, ARM_V7S_PRRR_TYPE_NORMAL) |
                            ARM_V7S_PRRR_TR(7, ARM_V7S_PRRR_TYPE_NORMAL) | ARM_V7S_PRRR_DS0 | ARM_V7S_PRRR_DS1 | ARM_V7S_PRRR_NS1 |
                            ARM_V7S_PRRR_NOS(7);
    cfg->arm_v7s_cfg.nmrr = ARM_V7S_NMRR_IR(7, ARM_V7S_RGN_WBWA) | ARM_V7S_NMRR_OR(7, ARM_V7S_RGN_WBWA);
    ...
    /* TTBRs */
    cfg->arm_v7s_cfg.ttbr[0] =
        ((arm_v7s_iopte)pa) | ARM_V7S_TTBR_S | ARM_V7S_TTBR_NOS | ARM_V7S_TTBR_IRGN_ATTR(ARM_V7S_RGN_NC) | ARM_V7S_TTBR_ORGN_ATTR(ARM_V7S_RGN_NC);
...
struct io_page_table_init_fns io_page_table_arm_v7s_init_fns = {
    .alloc = arm_v7s_alloc_page_table,
    .free  = arm_v7s_free_page_table,
};
...
static int __init arm_v7s_do_selftests(void)
{
    ...
    ops              = alloc_io_page_table_ops(ARM_V7S, &cfg, &cfg);
    ...
    while (i < loopnr) {
        ...
        if (ops->iova_to_phys(ops, iova_start + size + 42) != (size + 42)) {
            return __FAIL(ops);
        }
