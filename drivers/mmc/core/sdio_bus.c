...
{
    if (id->class != (uint8_t)SDIO_ANY_ID && id->class != func->class) {
        return NULL;
    }

    if (id->vendor != (uint16_t)SDIO_ANY_ID && id->vendor != func->vendor) {
        return NULL;
    }

    if (id->device != (uint16_t)SDIO_ANY_ID && id->device != func->device) {
        return NULL;
    }
...
static const struct sdio_device_id *sdio_match_device(struct sdio_func *func, struct sdio_driver *drv)
{
...
static int sdio_bus_match(vmm_device_t *dev, vmm_driver_t *drv)
{
...
static int sdio_bus_probe(vmm_device_t *dev)
{
...
static int sdio_bus_remove(vmm_device_t *dev)
{
    struct sdio_driver *drv;
    struct sdio_func   *func;
    int                 ret = 0;
...
static void sdio_release_device(vmm_device_t *ddev)
{
    /* TODO : Nothing to do here as of now */
    return;
}

static void sdio_release_func(vmm_device_t *dev)
{
...
struct sdio_func *sdio_alloc_func(struct mmc_card *card)
{
    ...
    vmm_device_driver_initialize_device(&func->dev);
...
int sdio_add_func(struct sdio_func *func)
{
    ...
    ret = vmm_device_driver_register_device(&func->dev);
...
void sdio_remove_func(struct sdio_func *func)
{
    ...
    vmm_device_driver_unregister_device(&func->dev);
}
