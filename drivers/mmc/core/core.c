...
static uint32_t mmc_host_count;
 ...
 */
uint32_t mmc_align_data_size(struct mmc_card *card, uint32_t size)
{
    /*
...
int mmc_set_clock(struct mmc_host *host, uint32_t clock, bool disable)
{
    ...
    host->ios.clock_disable = disable;
    return __mmc_set_ios(host);
}

int mmc_set_bus_width(struct mmc_host *host, uint32_t width)
{
...
int mmc_send_status(struct mmc_host *host, struct mmc_card *card, int timeout)
{
    struct mmc_cmd cmd;
    int            err, retries = 5;
#ifdef CONFIG_MMC_TRACE
...
static int __mmc_detect_card_removed(struct mmc_host *host)
{
    ...
    if (host->card->block_device) {
        vmm_block_device_unregister(host->card->block_device);
    }
...
static void mmc_host_poll(vmm_block_request_queue_t *brq, void *private)
{
    struct mmc_host *host = private;
...
static void mmc_host_poll_event_handler(vmm_timer_event_t *ev)
{
    struct mmc_host *host = container_of(ev, struct mmc_host, poll_ev);

    vmm_block_request_queue_queue_work(host->brq, mmc_host_poll, host);
}

static int mmc_block_request_queue_read(vmm_block_request_queue_t *brq, vmm_request_t *r, void *private)
{
    uint32_t         cnt;
    int              rc   = VMM_OK;
    struct mmc_host *host = private;
...
static int mmc_block_request_queue_write(vmm_block_request_queue_t *brq, vmm_request_t *r, void *private)
{
    uint32_t         cnt;
    int              rc   = VMM_OK;
    struct mmc_host *host = private;
...
static int mmc_block_request_queue_abort(vmm_block_request_queue_t *brq, vmm_request_t *r, void *private)
{
    /* Nothing to do here. */
    return VMM_OK;
}

static void mmc_block_request_queue_flush(vmm_block_request_queue_t *brq, void *private)
{
    /* Nothing to do here. */
}
...
static vmm_block_request_queue_ops_t mmc_rq_ops = {
    ...
    .write = mmc_block_request_queue_write,
    .abort = mmc_block_request_queue_abort,
    .flush = mmc_block_request_queue_flush};
...
int mmc_add_host(struct mmc_host *host)
{
    ...
    host->brq = vmm_block_request_queue_create(name, 128, FALSE, &mmc_rq_ops, host);
...
void mmc_remove_host(struct mmc_host *host)
{
    ...
    vmm_block_request_queue_destroy(host->brq);
    host->brq = NULL;
...
static int __init mmc_core_init(void)
{
    ...
    rc = vmm_device_driver_register_bus(&sdio_bus_type);
...
static void __exit mmc_core_exit(void)
{
    ...
    rc = vmm_device_driver_unregister_bus(&sdio_bus_type);
