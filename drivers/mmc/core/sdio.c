...
static int cis_tpl_parse(
    struct mmc_card *card, struct sdio_func *func, const char *tpl_descr, const struct cis_tpl *tpl, int tpl_count, unsigned char code,
...
static int cistpl_funce_common(struct mmc_card *card, struct sdio_func *func, const unsigned char *buf, unsigned size)
{
...
static int cistpl_funce_func(struct mmc_card *card, struct sdio_func *func, const unsigned char *buf, unsigned size)
{
    unsigned vsn;
    unsigned min_size;
    ...
    /* TPLFE_MAX_BLK_SIZE */
    func->max_blocksize = buf[12] | (buf[13] << 8);
...
static int sdio_read_cis(struct mmc_card *card, struct sdio_func *func)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        ret = mmc_io_rw_direct(card, 0, 0, SDIO_FBR_BASE(fn) + SDIO_FBR_CIS + i, 0, &x);
...
static int sdio_read_cccr(struct mmc_card *card, uint32_t ocr)
{
    int           ret;
    int           cccr_vsn;
...
static int sdio_init_func(struct mmc_card *card, uint32_t fn)
{
    ...
    if (!(card->quirks & MMC_QUIRK_NONSTD_SDIO)) {
        ...
        func->vendor        = func->card->cis.vendor;
        func->device        = func->card->cis.device;
        func->max_blocksize = func->card->cis.blksize;
    }
