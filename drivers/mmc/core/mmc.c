...
static const char *mmc_mode_name(enum mmc_bus_mode mode)
{
    static const char *const names[] = {
        [MMC_LEGACY] = "MMC legacy (25MHz)",    [SD_LEGACY] = "SD Legacy (25MHz)",    [MMC_HS] = "MMC High Speed (26MHz)",
        [SD_HS] = "SD High Speed (50MHz)",      [UHS_SDR12] = "UHS SDR12 (25MHz)",    [UHS_SDR25] = "UHS SDR25 (50MHz)",
        [UHS_SDR50] = "UHS SDR50 (100MHz)",     [UHS_SDR104] = "UHS SDR104 (208MHz)", [UHS_DDR50] = "UHS DDR50 (50MHz)",
        [MMC_HS_52] = "MMC High Speed (52MHz)", [MMC_DDR_52] = "MMC DDR52 (52MHz)",   [MMC_HS_200] = "HS200 (200MHz)",
...
static uint32_t mmc_mode2freq(struct mmc_card *card, enum mmc_bus_mode mode)
{
    static const int freqs[] = {
        [MMC_LEGACY] = 25000000,  [SD_LEGACY] = 25000000,   [MMC_HS] = 26000000,      [SD_HS] = 50000000,      [MMC_HS_52] = 52000000,
        [MMC_DDR_52] = 52000000,  [UHS_SDR12] = 25000000,   [UHS_SDR25] = 50000000,   [UHS_SDR50] = 100000000, [UHS_DDR50] = 50000000,
        [UHS_SDR104] = 208000000, [MMC_HS_200] = 200000000, [MMC_HS_400] = 200000000,
    };
...
static int mmc_select_mode(struct mmc_card *card, enum mmc_bus_mode mode)
{
    ...
    DPRINTF("selecting mode %s (freq : %d MHz)\n", mmc_mode_name(mode), card->tran_speed / 1000000);
    return 0;
}

static int mmc_bus_width(uint32_t cap)
{
...
static uint32_t __mmc_write_blocks(struct mmc_host *host, struct mmc_card *card, uint64_t start, uint32_t blkcnt, const void *src)
{
    struct mmc_cmd  cmd;
    struct mmc_data data;
    int             timeout = 1000;

    DPRINTF("%s: start=0x%llx blkcnt=%d\n", __func__, (uint64_t)start, blkcnt);
     ...
     */
    if (!(host->caps2 & MMC_CAP2_AUTO_CMD12) && !mmc_host_is_spi(host) && blkcnt > 1) {
...
uint32_t __mmc_sd_bwrite(struct mmc_host *host, struct mmc_card *card, uint64_t start, uint32_t blkcnt, const void *src)
{
...
static uint32_t __mmc_read_blocks(struct mmc_host *host, struct mmc_card *card, void *dst, uint64_t start, uint32_t blkcnt)
{
    struct mmc_cmd  cmd;
    struct mmc_data data;
...
uint32_t __mmc_sd_bread(struct mmc_host *host, struct mmc_card *card, uint64_t start, uint32_t blkcnt, void *dst)
{
    uint32_t cur, blocks_todo = blkcnt;
...
static int __sd_switch(struct mmc_host *host, int mode, int group, uint8_t value, uint8_t *resp)
{
    struct mmc_cmd  cmd;
    struct mmc_data data;
...
retry_scr:
    ...
    while (timeout--) {
        err = __sd_switch(host, SD_SWITCH_CHECK, 0, 1, (uint8_t *)switch_status);
...
static int __sd_select_bus_width(struct mmc_host *host, struct mmc_card *card, int w)
{
    int            err;
    struct mmc_cmd cmd;
...
static int __sd_set_card_speed(struct mmc_host *host, struct mmc_card *card, enum mmc_bus_mode mode)
{
    int      err, speed;
    uint32_t switch_status[16];
...
static int __sd_read_ssr(struct mmc_host *host, struct mmc_card *card)
{
    static const uint32_t sd_au_size[] = {
        0,           SZ_16K / 512, SZ_32K / 512, SZ_64K / 512,          SZ_128K / 512, SZ_256K / 512,          SZ_512K / 512, SZ_1M / 512,
        SZ_2M / 512, SZ_4M / 512,  SZ_8M / 512,  (SZ_8M + SZ_4M) / 512, SZ_16M / 512,  (SZ_16M + SZ_8M) / 512, SZ_32M / 512,  SZ_64M / 512,
    };
    int             err, i;
    struct mmc_cmd  cmd;
    uint32_t        ssr[16];
...
static const struct mode_width_tuning sd_modes_by_pref[] = {
    {.mode = UHS_SDR104, .widths = MMC_CAP_MODE_4BIT | MMC_CAP_MODE_1BIT, .tuning = MMC_CMD_SEND_TUNING_BLOCK},
    {
     .mode   = UHS_SDR50,
...
{
    int                             err;
    uint32_t                        widths[] = {MMC_CAP_MODE_4BIT, MMC_CAP_MODE_1BIT};
    ...
    for_each_sd_mode_by_pref(caps, mwt)
    {
        ...
        DPRINTF("Trying mode %s (at %d MHz)\n", mmc_mode_name(mwt->mode), mmc_mode2freq(card, mwt->mode) / 1000000);
        ...
        for (w = widths; w < widths + array_size(widths); w++) {
            ...
            err = __sd_select_bus_width(host, card, mmc_bus_width(*w));
...
static int __mmc_send_ext_csd(struct mmc_host *host, uint8_t *ext_csd)
{
    struct mmc_cmd  cmd;
    struct mmc_data data;
...
static int __mmc_get_capabilities(struct mmc_host *host, struct mmc_card *card)
{
    uint8_t *ext_csd = &card->ext_csd[0];
    uint8_t  cardtype;
...
static int __mmc_set_card_speed(struct mmc_host *host, struct mmc_card *card, enum mmc_bus_mode mode)
{
    int     err;
    int     speed_bits;
    uint8_t test_csd[MMC_MAX_BLOCK_LEN];
 ...
 * as expected.
 */
static int __mmc_read_and_compare_ext_csd(struct mmc_host *host, struct mmc_card *card)
{
    int            err;
    const uint8_t *ext_csd = &card->ext_csd[0];
    uint8_t        test_csd[MMC_MAX_BLOCK_LEN];
    ...
    if (ext_csd[EXT_CSD_PARTITIONING_SUPPORT] == test_csd[EXT_CSD_PARTITIONING_SUPPORT] &&
        ext_csd[EXT_CSD_HC_WP_GRP_SIZE] == test_csd[EXT_CSD_HC_WP_GRP_SIZE] && ext_csd[EXT_CSD_REV] == test_csd[EXT_CSD_REV] &&
...
static int __mmc_set_lowest_voltage(struct mmc_host *host, uint8_t ext_csd_cardtype, enum mmc_bus_mode mode, uint32_t allowed_mask)
{
    ...
    switch (mode) {
        case MMC_HS_400:
        case MMC_HS_200:
            if (ext_csd_cardtype & (EXT_CSD_CARD_TYPE_HS200_1_8V | EXT_CSD_CARD_TYPE_HS400_1_8V)) {
                card_mask |= MMC_SIGNAL_VOLTAGE_180;
            }

            if (ext_csd_cardtype & (EXT_CSD_CARD_TYPE_HS200_1_2V | EXT_CSD_CARD_TYPE_HS400_1_2V)) {
...
static const struct mode_width_tuning mmc_modes_by_pref[] = {
    {.mode = MMC_HS_400, .widths = MMC_CAP_MODE_8BIT, .tuning = MMC_CMD_SEND_TUNING_BLOCK_HS200},
    {.mode = MMC_HS_200, .widths = MMC_CAP_MODE_8BIT | MMC_CAP_MODE_4BIT, .tuning = MMC_CMD_SEND_TUNING_BLOCK_HS200},
    {
     .mode   = MMC_DDR_52,
    ...
    {
     .mode   = MMC_HS_52,
     .widths = MMC_CAP_MODE_8BIT | MMC_CAP_MODE_4BIT | MMC_CAP_MODE_1BIT,
     },
    {
     .mode   = MMC_HS,
     .widths = MMC_CAP_MODE_8BIT | MMC_CAP_MODE_4BIT | MMC_CAP_MODE_1BIT,
...
static int __mmc_select_hs400(struct mmc_host *host, struct mmc_card *card)
{
    ...
    err = __mmc_switch(host, card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_BUS_WIDTH, EXT_CSD_BUS_WIDTH_8 | EXT_CSD_DDR_FLAG);
...
static int __mmc_select_mode_and_width(struct mmc_host *host, struct mmc_card *card)
{
    ...
    {
        for_each_supported_width(card_caps & mwt->widths, mmc_is_mode_ddr(mwt->mode), ecbw)
        {
            enum mmc_voltage old_voltage;
            DPRINTF(
                "Trying mode %s width %d (at %d MHz)\n", mmc_mode_name(mwt->mode), mmc_bus_width(ecbw->cap),
                mmc_mode2freq(card, mwt->mode) / 1000000);

            old_voltage = host->ios.signal_voltage;
            err         = __mmc_set_lowest_voltage(host, card->ext_csd_cardtype, mwt->mode, MMC_ALL_SIGNAL_VOLTAGE);
            ...
            /* configure the bus width (card + host) */
            err = __mmc_switch(host, card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_BUS_WIDTH, ecbw->ext_csd_bits & ~EXT_CSD_DDR_FLAG);
            ...
            if (mwt->mode == MMC_HS_400) {
                ...
                if (ecbw->ext_csd_bits & EXT_CSD_DDR_FLAG) {
                    err = __mmc_switch(host, card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_BUS_WIDTH, ecbw->ext_csd_bits);
        ...
        error:
            ...
            __mmc_switch(host, card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_BUS_WIDTH, EXT_CSD_BUS_WIDTH_1);
...
static int __mmc_startup_v4(struct mmc_host *host, struct mmc_card *card)
{
    ...
    bool                  part_completed;
    uint8_t              *ext_csd        = &card->ext_csd[0];
    static const uint32_t mmc_versions[] = {MMC_VERSION_4,    MMC_VERSION_4_1, MMC_VERSION_4_2, MMC_VERSION_4_3, MMC_VERSION_4_4,
                                            MMC_VERSION_4_41, MMC_VERSION_4_5, MMC_VERSION_5_0, MMC_VERSION_5_1};
    ...
    if (card->version >= MMC_VERSION_4_2) {
         ...
         * than 2GB
         */
        capacity = ext_csd[EXT_CSD_SEC_CNT] << 0 | ext_csd[EXT_CSD_SEC_CNT + 1] << 8 | ext_csd[EXT_CSD_SEC_CNT + 2] << 16 |
     ...
     */
    part_completed     = !!(ext_csd[EXT_CSD_PARTITION_SETTING] & EXT_CSD_PARTITION_SETTING_COMPLETED);
    ...
    if (part_completed && (ext_csd[EXT_CSD_PARTITIONING_SUPPORT] & ENHNCD_SUPPORT)) {
    ...
    if (part_completed) {
        card->enh_user_size = (ext_csd[EXT_CSD_ENH_SIZE_MULT + 2] << 16) + (ext_csd[EXT_CSD_ENH_SIZE_MULT + 1] << 8) + ext_csd[EXT_CSD_ENH_SIZE_MULT];
        ...
        card->enh_user_start = (ext_csd[EXT_CSD_ENH_START_ADDR + 3] << 24) + (ext_csd[EXT_CSD_ENH_START_ADDR + 2] << 16) +
                               (ext_csd[EXT_CSD_ENH_START_ADDR + 1] << 8) + ext_csd[EXT_CSD_ENH_START_ADDR];
    ...
    if (has_parts) {
        err = __mmc_switch(host, card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_ERASE_GROUP_DEF, 1);
    ...
    if (ext_csd[EXT_CSD_ERASE_GROUP_DEF] & 0x01) {
        ...
        card->erase_grp_size = ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE] * 1024;
        ...
        if (card->high_capacity && part_completed) {
            capacity = (ext_csd[EXT_CSD_SEC_CNT]) | (ext_csd[EXT_CSD_SEC_CNT + 1] << 8) | (ext_csd[EXT_CSD_SEC_CNT + 2] << 16) |
                       (ext_csd[EXT_CSD_SEC_CNT + 3] << 24);
            capacity *= MMC_MAX_BLOCK_LEN;
            card->capacity_user = capacity;
        }
    ...
    card->hc_wp_grp_size = 1024 * ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE] * ext_csd[EXT_CSD_HC_WP_GRP_SIZE];
...
static int __mmc_startup(struct mmc_host *host, struct mmc_card *card)
{
    ...
    cmd.cmdidx    = mmc_host_is_spi(host) ? MMC_CMD_SEND_CID : MMC_CMD_ALL_SEND_CID; /* cmd not supported in spi */
...
static int __sd_send_op_cond(struct mmc_host *host, struct mmc_card *card)
{
    ...
    do {
         ...
         * specified.
         */
        cmd.cmdarg      = mmc_host_is_spi(host) ? 0 : (host->voltages & 0xff8000);
...
int mmc_send_op_cond(struct mmc_host *host, struct mmc_card *card)
{
    ...
    do {
        cmd.cmdidx    = MMC_CMD_SEND_OP_COND;
        cmd.resp_type = MMC_RSP_R3;
        cmd.cmdarg    = (mmc_host_is_spi(host) ? 0 : (host->voltages & (cmd.response[0] & OCR_VOLTAGE_MASK)) | (cmd.response[0] & OCR_ACCESS_MODE));
...
int __mmc_sd_attach(struct mmc_host *host)
{
    ...
    block_device = card->block_device;
    ...
    for (i = 0; i < 6; i++) {
        if (!vmm_is_printable(str[i])) {
            str[i] = '\0';
        }
    }

    vmm_snprintf(block_device->name, sizeof(block_device->name), "mmc%d", host->host_num);
    vmm_snprintf(
        block_device->desc, sizeof(block_device->desc),
        "%s-%d.%d Manufacturer=%06x Serial=%04x%04x "
        "Product=%c%c%c%c%c%c Rev=%d.%d",
        IS_SD(host->card) ? "SD" : "MMC", EXTRACT_SDMMC_MAJOR_VERSION(host->card->version), EXTRACT_SDMMC_MINOR_VERSION(host->card->version),
        (card->cid[0] >> 24), (card->cid[2] & 0xffff), ((card->cid[3] >> 16) & 0xffff), str[0], str[1], str[2], str[3], str[4], str[5],
        ((card->cid[2] >> 20) & 0xf), ((card->cid[2] >> 16) & 0xf));
    block_device->dev.parent = host->dev;
    block_device->flags      = VMM_BLOCK_DEVICE_RW;

    if (card->read_bl_len < card->write_bl_len) {
        block_device->block_size = card->write_bl_len;
    } else {
        block_device->block_size = card->read_bl_len;
    }

    block_device->start_lba  = 0;
    block_device->num_blocks = udiv64(card->capacity, block_device->block_size);

    /* Setup request queue for block device instance */
    block_device->rq         = vmm_block_request_queue_to_rq(host->brq);

    /* Register block device instance */
    rc                       = vmm_block_device_register(card->block_device);
    ...
    /* Print banner */
    vmm_linfo(block_device->name, "using %s mode and %dbit bus-width\n", host->card->mode_name, mmc_bus_width(host->card->caps));
    vmm_linfo(block_device->name, "capacity %" PRIu64 " blocks of %d bytes\n", block_device->num_blocks, block_device->block_size);
    vmm_linfo(block_device->name, "%s\n", block_device->desc);

    rc = VMM_OK;
    goto detect_done;

detect_freebdev_fail:
    vmm_block_device_free(host->card->block_device);
