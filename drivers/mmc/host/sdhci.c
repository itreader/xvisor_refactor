...
static void sdhci_clear_set_irqs(struct sdhci_host *host, uint32_t clear, uint32_t set)
{
    uint32_t ier;
...
static void sdhci_unmask_irqs(struct sdhci_host *host, uint32_t irqs)
{
    sdhci_clear_set_irqs(host, 0, irqs);
}

static void sdhci_mask_irqs(struct sdhci_host *host, uint32_t irqs)
{
...
{
    struct sdhci_host *host = mmc_private(mmc);
...
static int sdhci_transfer_dma(struct sdhci_host *host, struct mmc_data *data)
{
    int      rc      = VMM_OK;
    uint64_t timeout = 100000000LL;
...
static int sdhci_transfer_data(struct sdhci_host *host, struct mmc_data *data, uint32_t start_addr)
{
    uint32_t stat, rdy, mask, timeout, block = 0;
...
int sdhci_send_command(struct mmc_host *mmc, struct mmc_cmd *cmd, struct mmc_data *data)
{
    bool               present;
    uint32_t           mask, flags, mode;
    int                ret = 0, trans_bytes = 0;
    uint32_t           retry = 10000, stat = 0;
    uint64_t           timeout;
    ...
    if (data != 0) {
        ...
        if (host->sdhci_caps & SDHCI_CAN_DO_SDMA) {
            ...
            sdhci_unmask_irqs(host, SDHCI_INT_ADMA_ERROR | SDHCI_INT_ACMD12ERR | SDHCI_INT_DATA_TIMEOUT | SDHCI_INT_DMA_END);
            ...
            vmm_flush_cache_range((virtual_addr_t)host->aligned_buffer, (virtual_addr_t)host->aligned_buffer + trans_bytes);
        }
    ...
    if (!ret && data) {
        if (host->sdhci_caps & SDHCI_CAN_DO_SDMA) {
            ...
            uint32_t start_addr = (virtual_addr_t)data->dest;
    ...
    if (!ret) {
        if ((host->sdhci_caps & SDHCI_CAN_DO_SDMA) && (data->flags == MMC_DATA_READ)) {
...
static int sdhci_set_clock(struct mmc_host *mmc, uint32_t clock)
{
    struct sdhci_host *host = (struct sdhci_host *)mmc->private;
    uint32_t           div, clk, timeout;
...
static void sdhci_set_power(struct sdhci_host *host, uint16_t power)
{
    uint8_t pwr = 0;
...
static int sdhci_init_card(struct mmc_host *mmc, struct mmc_card *card)
{
    ...
    if (host->quirks & SDHCI_QUIRK_BROKEN_CARD_DETECTION) {
        ...
        sdhci_writel(host, SDHCI_CTRL_CD_TEST_INS | SDHCI_CTRL_CD_TEST, SDHCI_HOST_CONTROL);
...
static vmm_irq_return_t sdhci_irq_handler(int irq_no, void *dev)
{
    ...
    if (intmask & (SDHCI_INT_CARD_INSERT | SDHCI_INT_CARD_REMOVE)) {
        uint32_t present = sdhci_readl(host, SDHCI_PRESENT_STATE) & SDHCI_CARD_PRESENT;
         ...
         * platforms though.
         */
        sdhci_mask_irqs(host, present ? SDHCI_INT_CARD_INSERT : SDHCI_INT_CARD_REMOVE);
        sdhci_unmask_irqs(host, present ? SDHCI_INT_CARD_REMOVE : SDHCI_INT_CARD_INSERT);
    ...
    if (intmask & (SDHCI_INT_DATA_MASK | SDHCI_INT_DMA_END)) {
        ...
        sdhci_writel(host, intmask & (SDHCI_INT_DATA_MASK | SDHCI_INT_DMA_END), SDHCI_INT_STATUS);
...
struct sdhci_host *sdhci_alloc_host(vmm_device_t *dev, int extra)
{
    struct mmc_host   *mmc;
...
int sdhci_add_host(struct sdhci_host *host)
{
    ...
    if (host->quirks & SDHCI_QUIRK_REG32_RW) {
        host->sdhci_version = sdhci_readl(host, SDHCI_HOST_VERSION - 2) >> 16;
    } else {
    ...
    if (SDHCI_GET_VERSION(host) >= SDHCI_SPEC_300) {
        ...
        host->clock_mul    = (host->sdhci_caps_1 & SDHCI_CLOCK_MUL_MASK) >> SDHCI_CLOCK_MUL_SHIFT;
    } else {
        host->sdhci_caps_1 = 0;
        host->clock_mul    = 0;
    }
    ...
    if (host->max_clock == 0) {
        if (SDHCI_GET_VERSION(host) >= SDHCI_SPEC_300) {
            host->max_clock = (host->sdhci_caps & SDHCI_CLOCK_V3_BASE_MASK) >> SDHCI_CLOCK_BASE_SHIFT;
        } else {
            host->max_clock = (host->sdhci_caps & SDHCI_CLOCK_BASE_MASK) >> SDHCI_CLOCK_BASE_SHIFT;
        }

        host->max_clock *= 1000000;

        if (host->clock_mul) {
            host->max_clock *= host->clock_mul;
        }
    }
    ...
    if (host->min_clock == 0) {
        if (SDHCI_GET_VERSION(host) >= SDHCI_SPEC_300) {
            host->min_clock = host->max_clock / SDHCI_MAX_DIV_SPEC_300;
        } else {
            host->min_clock = host->max_clock / SDHCI_MAX_DIV_SPEC_200;
        }
    }
    ...
    mmc->caps = MMC_CAP_MODE_HS | MMC_CAP_MODE_HS_52MHz | MMC_CAP_MODE_4BIT;
    ...
    if (!(mmc->voltages & MMC_VDD_165_195) || (host->quirks2 & SDHCI_QUIRK2_NO_1_8_V)) {
        host->sdhci_caps_1 &= ~(SDHCI_SUPPORT_SDR104 | SDHCI_SUPPORT_SDR50 | SDHCI_SUPPORT_DDR50);
    }

    if (host->sdhci_caps_1 & (SDHCI_SUPPORT_SDR104 | SDHCI_SUPPORT_SDR50 | SDHCI_SUPPORT_DDR50)) {
    ...
    if (host->sdhci_caps & SDHCI_CAN_DO_SDMA) {
        ...
        host->aligned_buffer = (uint8_t *)vmm_dma_malloc(VMM_SIZE_TO_PAGE(SDHCI_DMA_MAX_BUF) * VMM_PAGE_SIZE);
        ...
        if ((host->quirks & SDHCI_QUIRK_32BIT_DMA_ADDR) && (((virtual_addr_t)host->aligned_buffer) & 0x7)) {
            vmm_printf(
    ...
    if (host->irq > 0) {
        if ((rc = vmm_host_irq_register(host->irq, mmc_hostname(mmc), sdhci_irq_handler, host))) {
            goto free_host_buffer;
        }
    } else {
    ...
    vmm_linfo(
        mmc_hostname(mmc), "SDHCI controller %s at 0x%lx irq %d [%s]\n", ver, (uint64_t)iopaddr, host->irq,
        (host->sdhci_caps & SDHCI_CAN_DO_SDMA) ? "DMA" : "PIO");

    rc = mmc_add_host(mmc);

    if (rc) {
        vmm_lerror(mmc_hostname(mmc), "MMC add host failed (error %d)\n", rc);
        goto free_host_irq;
    }
