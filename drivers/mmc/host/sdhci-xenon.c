...
static int xenon_mmc_phy_init(struct sdhci_host *host)
{
    ...
    uint32_t clock                   = private->clock_freq;
    uint32_t time;
    uint32_t var;
    ...
    if ((private->timing == MMC_TIMING_UHS_SDR50) || (private->timing == MMC_TIMING_UHS_SDR25) || (private->timing == MMC_TIMING_UHS_SDR12) ||
        (private->timing == MMC_TIMING_SD_HS) || (private->timing == MMC_TIMING_LEGACY)) {
...
static void armada_3700_soc_pad_voltage_set(struct sdhci_host *host)
{
    ...
    if (private->pad_type == SOC_PAD_FIXED_1_8V) {
        vmm_writel(ARMADA_3700_SOC_PAD_1_8V, private->pad_ctrl_reg);
    } else if (private->pad_type == SOC_PAD_SD) {
        vmm_writel(ARMADA_3700_SOC_PAD_3_3V, private->pad_ctrl_reg);
    }
}
...
static void xenon_mmc_phy_set(struct sdhci_host *host)
{
     ...
     */
    if ((private->timing == MMC_TIMING_MMC_HS400) || (private->timing == MMC_TIMING_MMC_HS200) || (private->timing == MMC_TIMING_UHS_SDR50) ||
        (private->timing == MMC_TIMING_UHS_SDR104) || (private->timing == MMC_TIMING_UHS_DDR50) || (private->timing == MMC_TIMING_UHS_SDR25) ||
        (private->timing == MMC_TIMING_MMC_DDR52)) {
        var = sdhci_readl(host, EMMC_PHY_TIMING_ADJUST);
...
static void xenon_mmc_enable_slot(struct sdhci_host *host, uint8_t slot)
{
    uint32_t var;
...
static void xenon_mmc_enable_parallel_tran(struct sdhci_host *host, uint8_t slot)
{
    uint32_t var;
...
static void xenon_mmc_disable_tuning(struct sdhci_host *host, uint8_t slot)
{
    uint32_t var;
...
{
    struct xenon_sdhci_priv *private = sdhci_private(host);
    uint32_t speed                   = host->mmc->card->tran_speed;
    int      pwr_18v                 = 0;

    if ((sdhci_readb(host, SDHCI_POWER_CONTROL) & ~SDHCI_POWER_ON) == SDHCI_POWER_180) {
        pwr_18v = 1;
    }
    ...
    if (IS_SD(host->mmc->card)) {
        ...
        if (pwr_18v) {
            if (host->mmc->card->ddr_mode) {
                private->timing = MMC_TIMING_UHS_DDR50;
            } else if (speed <= 25000000) {
                private->timing = MMC_TIMING_UHS_SDR25;
            } else {
                private->timing = MMC_TIMING_UHS_SDR50;
            }
        } else {
            if (speed <= 25000000) {
                private->timing = MMC_TIMING_LEGACY;
            } else {
                private->timing = MMC_TIMING_SD_HS;
            }
        }
    } else {
        /* eMMC */
        if (host->mmc->card->ddr_mode) {
            ...
            private->timing = MMC_TIMING_MMC_HS;
        }
    }
...
static int xenon_sdhci_driver_probe(vmm_device_t *dev)
{
    int                rc;
    uint32_t           tmp;
    struct sdhci_host *host;
    struct xenon_sdhci_priv *private;
    ...
    private = sdhci_private(host);

    rc      = vmm_device_tree_request_regmap(dev->of_node, &private->base, 0, "XENON SDHCI");

    if (rc) {
        goto free_host;
    }

    rc = vmm_device_tree_request_regmap(dev->of_node, &private->pad_base, 1, "XENON SDHCI PAD");

    if (!rc) {
        private->pad_ctrl_reg = (void *)private->pad_base;
    } else {
        private->pad_ctrl_reg = NULL;
        rc                    = VMM_OK;
    }
    ...
    private->clk = clock_get(dev, NULL);

    if (VMM_IS_ERR_OR_NULL(private->clk)) {
        rc = VMM_PTR_ERR(private->clk);
        goto free_reg;
    }

    private->clock_freq = clock_get_rate(private->clk);

    /* Set name, irq, and register base */
    host->hw_name       = dev->name;
    host->irq           = (private->irq) ? private->irq : -1;
    host->ioaddr        = (void *)private->base;
     ...
     */
    if (private->pad_ctrl_reg) {
        armada_3700_soc_pad_voltage_set(host);
    }

    /* Set MMC capabilities of SDHCI host */
    host->mmc_caps = MMC_CAP_MODE_HS | MMC_CAP_MODE_HS_52MHz | MMC_CAP_MODE_DDR_52MHz;
    rc             = vmm_device_tree_read_u32(dev->of_node, "bus-width", &tmp);
    ...
    /* Set max clock */
    host->max_clock        = private->clock_freq;
...
free_clock:
    clock_put(private->clk);
free_reg:

    if (private->pad_ctrl_reg) {
        vmm_device_tree_regunmap_release(dev->of_node, private->pad_base, 1);
    }

    vmm_device_tree_regunmap_release(dev->of_node, private->base, 0);
free_host:
...
static int xenon_sdhci_driver_remove(vmm_device_t *dev)
{
...
static struct vmm_device_tree_nodeid xenon_sdhci_devid_table[] = {
    {
...
static vmm_driver_t xenon_sdhci_driver = {
    .name        = "xenon_sdhci",
...
{
    return vmm_device_driver_register_driver(&xenon_sdhci_driver);
}

static void __exit xenon_sdhci_driver_exit(void)
{
    vmm_device_driver_unregister_driver(&xenon_sdhci_driver);
}
