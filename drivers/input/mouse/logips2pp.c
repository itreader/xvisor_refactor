    ...
    if ((packet[0] & 0x48) == 0x48 && (packet[1] & 0x02) == 0x02) {
        ...
        switch ((packet[1] >> 4) | (packet[0] & 0x30)) {
            ...
            case 0x0d: /* Mouse extra info */
                ...
                input_report_rel(dev, packet[2] & 0x80 ? REL_HWHEEL : REL_WHEEL, (int)(packet[2] & 8) - (int)(packet[2] & 7));
            ...
            case 0x0f: /* TouchPad extra info */
                ...
                input_report_rel(dev, packet[2] & 0x08 ? REL_HWHEEL : REL_WHEEL, (int)((packet[2] >> 4) & 8) - (int)((packet[2] >> 4) & 7));
                packet[0] = packet[2] | 0x08;
                break;

            default:
                psmouse_dbg(psmouse, "Received PS2++ packet #%x, but don't know how to handle.\n", (packet[1] >> 4) | (packet[0] & 0x30));
                break;
        }
    } else {
...
static void ps2pp_set_resolution(struct psmouse *psmouse, uint32_t resolution)
{
    if (resolution > 400) {
        struct ps2dev *ps2dev = &psmouse->ps2dev;
...
static const struct ps2pp_info *get_model_info(unsigned char model)
{
    static const struct ps2pp_info ps2pp_list[] = {
        ...
        {15,  PS2PP_KIND_MX,       /* MX1000 */
         PS2PP_WHEEL | PS2PP_SIDE_BTN | PS2PP_TASK_BTN | PS2PP_EXTRA_BTN | PS2PP_NAV_BTN | PS2PP_HWHEEL},
        {40,  0,                   PS2PP_SIDE_BTN                                                                                  },
        {41,  0,                   PS2PP_SIDE_BTN                                                                                  },
        {42,  0,                   PS2PP_SIDE_BTN                                                                                  },
        {43,  0,                   PS2PP_SIDE_BTN                                                                                  },
        ...
        {61,  PS2PP_KIND_MX,       /* MX700 */
         PS2PP_WHEEL | PS2PP_SIDE_BTN | PS2PP_TASK_BTN | PS2PP_EXTRA_BTN | PS2PP_NAV_BTN               },
        ...
        {114, PS2PP_KIND_MX,       /* MX310 */
         PS2PP_WHEEL | PS2PP_SIDE_BTN | PS2PP_TASK_BTN | PS2PP_EXTRA_BTN                               }
    };
    int i;
...
static void ps2pp_set_model_properties(struct psmouse *psmouse, const struct ps2pp_info *model_info, bool using_ps2pp)
{
    input_device_t *input_device = psmouse->dev;

    if (model_info->features & PS2PP_SIDE_BTN) {
        __set_bit(BTN_SIDE, input_device->keybit);
    }

    if (model_info->features & PS2PP_EXTRA_BTN) {
        __set_bit(BTN_EXTRA, input_device->keybit);
    }

    if (model_info->features & PS2PP_TASK_BTN) {
        __set_bit(BTN_TASK, input_device->keybit);
    }

    if (model_info->features & PS2PP_NAV_BTN) {
        __set_bit(BTN_FORWARD, input_device->keybit);
        __set_bit(BTN_BACK, input_device->keybit);
    }

    if (model_info->features & PS2PP_WHEEL) {
        __set_bit(REL_WHEEL, input_device->relbit);
    }

    if (model_info->features & PS2PP_HWHEEL) {
        __set_bit(REL_HWHEEL, input_device->relbit);
    }
...
int ps2pp_init(struct psmouse *psmouse, bool set_properties)
{
    ...
    if (model_info) {
        ...
        if (model_info->kind == PS2PP_KIND_TP3) { /* Touch Pad 3 */
            ...
            if (!ps2_command(ps2dev, param, 0x13d1) && param[0] == 0x06 && param[1] == 0x00 && param[2] == 0x14) {
                use_ps2pp = true;
            }
            ...
            if ((param[0] & 0x78) == 0x48 && (param[1] & 0xf3) == 0xc2 && (param[2] & 0x03) == ((param[1] >> 2) & 3)) {
    ...
    if (set_properties) {
        ...
        if (use_ps2pp) {
            ...
            if (model_info->kind != PS2PP_KIND_TP3) {
                ...
                if (error) {
                    psmouse_err(psmouse, "failed to create smartscroll sysfs attribute, error: %d\n", error);
                    return -1;
                }
            }
        }
