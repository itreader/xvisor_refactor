...
{
    static uint32_t ps2_packet_cnt;
    static uint32_t ps2_last_second;
    uint32_t        jiffies_msec;
    ...
    psmouse_dbg(
        ...
        "abs_x: %d, abs_y: %d\n",
        jiffies_msec, packet_type, packet[0], packet[1], packet[2], packet[3], abs_x, abs_y);
...
static void fsp_set_slot(input_device_t *dev, int slot, bool active, uint32_t x, uint32_t y)
{
...
static psmouse_ret_t fsp_process_byte(struct psmouse *psmouse)
{
    ...
    switch (psmouse->packet[0] >> FSP_PKT_TYPE_SHIFT) {
        case FSP_PKT_TYPE_ABS:
            ...
            if ((packet[0] == 0x48 || packet[0] == 0x49) && packet[1] == 0 && packet[2] == 0) {
                /*
            ...
            if (packet[0] & FSP_PB0_MFMC) {
                ...
                /* SFAC packet */
                if ((packet[0] & (FSP_PB0_LBTN | FSP_PB0_PHY_BTN)) == FSP_PB0_LBTN) {
...
static int fsp_activate_protocol(struct psmouse *psmouse)
{
    ...
    if (pad->ver < FSP_VER_STL3888_C0) {
        ...
        if (fsp_reg_read(psmouse, FSP_REG_SYSCTL5, &val)) {
            psmouse_err(psmouse, "Unable to read SYSCTL5 register.\n");
            return -EIO;
        }
...
#endif
            psmouse_err(psmouse, "Unable to retrieve number of buttons.\n");
            return -EIO;
        }
        ...
        if (fsp_reg_write(psmouse, FSP_REG_SYSCTL5, val)) {
            psmouse_err(psmouse, "Unable to set up required mode bits.\n");
            return -EIO;
        }
        ...
        if (fsp_opc_tag_enable(psmouse, true)) {
            psmouse_warn(psmouse, "Failed to enable OPC tag mode.\n");
        }
        ...
        if (fsp_reg_write(
                psmouse, FSP_REG_SWC1, FSP_BIT_SWC1_EN_ABS_1F | FSP_BIT_SWC1_EN_ABS_2F | FSP_BIT_SWC1_EN_FUP_OUT | FSP_BIT_SWC1_EN_ABS_CON)) {
            psmouse_err(psmouse, "Unable to enable absolute coordinates output.\n");
            return -EIO;
        }
    }
...
{
    struct fsp_data *private;
    int ver, rev, sn = 0;
    int error;

    if (fsp_get_version(psmouse, &ver) || fsp_get_revision(psmouse, &rev)) {
        return -ENODEV;
    }
    ...
    psmouse_info(psmouse, "Finger Sensing Pad, hw: %d.%d.%d, sn: %x, sw: %s\n", ver >> 4, ver & 0x0F, rev, sn, fsp_drv_ver);

    psmouse->private = private = kzalloc(sizeof(struct fsp_data), GFP_KERNEL);
    ...
    if (error) {
        psmouse_err(psmouse, "Failed to create sysfs attributes (%d)", error);
        goto err_out;
    }
