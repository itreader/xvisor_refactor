...
static void copy_abs(input_device_t *dev, uint32_t dst, uint32_t src)
{
...
int input_mt_init_slots(input_device_t *dev, uint32_t num_slots, uint32_t flags)
{
    ...
    if (flags & INPUT_MT_DIRECT) {
        __set_bit(INPUT_PROPERTIES_DIRECT, dev->propbit);
    }

    if (flags & INPUT_MT_TRACK) {
        uint32_t n2 = num_slots * num_slots;
...
static int input_mt_set_matrix(struct input_mt *mt, const struct input_mt_pos *pos, int num_pos)
{
...
static void input_mt_set_slots(struct input_mt *mt, int *slots, int num_pos)
{
    struct input_mt_slot *s;
    int                  *w = mt->red, *p;
...
int input_mt_assign_slots(input_device_t *dev, int *slots, const struct input_mt_pos *pos, int num_pos)
{
    struct input_mt *mt = dev->mt;
    int              nrc;
