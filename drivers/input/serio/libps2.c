...
int ps2_sendbyte(struct ps2dev *ps2dev, unsigned char byte, int timeout)
{
    ...
    if (serio_write(ps2dev->serio, byte) == 0) {
        wait_event_timeout(ps2dev->wait, !(ps2dev->flags & PS2_FLAG_ACK), msecs_to_jiffies(timeout));
    }
...
void ps2_drain(struct ps2dev *ps2dev, int maxbytes, int timeout)
{
    ...
    wait_event_timeout(ps2dev->wait, !(ps2dev->flags & PS2_FLAG_CMD), msecs_to_jiffies(timeout));
...
int __ps2_command(struct ps2dev *ps2dev, unsigned char *param, int command)
{
    ...
    timeout = wait_event_timeout(ps2dev->wait, !(ps2dev->flags & PS2_FLAG_CMD1), timeout);
