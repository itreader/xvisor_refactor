...
static int serio_match_port(const struct serio_device_id *ids, struct serio *serio)
{
    while (ids->type || ids->proto) {
        if ((ids->type == SERIO_ANY || ids->type == serio->id.type) && (ids->proto == SERIO_ANY || ids->proto == serio->id.proto) &&
            (ids->extra == SERIO_ANY || ids->extra == serio->id.extra) && (ids->id == SERIO_ANY || ids->id == serio->id.id)) {
            return 1;
        }
...
static void serio_find_driver(struct serio *serio)
{
    ...
    if (error < 0) {
        dev_warn(&serio->dev, "device_attach() failed for %s (%s), error: %d\n", serio->phys, serio->name, error);
    }
}
...
static struct serio_event *serio_get_event(void)
{
    ...
    spin_lock_irq_save(&serio_event_lock, flags);

    if (!list_empty(&serio_event_list)) {
        event = list_first_entry(&serio_event_list, struct serio_event, node);
        list_del_init(&event->node);
    }

    spin_unlock_irq_restore(&serio_event_lock, flags);
    return event;
}
...
static void serio_remove_duplicate_events(void *object, enum serio_event_type type)
{
...
static void serio_handle_event(vmm_work_t *work)
{
...
static int serio_queue_event(void *object, enum serio_event_type event_type)
{
    uint64_t            flags;
    struct serio_event *event;
    int                 retval = 0;
    ...
    do {
        ...
        if (!error) {
            ...
            if (!list_empty(&s->children)) {
                s = list_first_entry(&s->children, struct serio, child_node);
                continue;
            }
        }
        ...
        while (s != root) {
            ...
            if (!list_is_last(&s->child_node, &parent->children)) {
                s = list_entry(s->child_node.next, struct serio, child_node);
                break;
            }
...
int __serio_register_driver(struct serio_driver *drv)
{
    ...
    if (strlcpy(drv->driver.name, drv->name, sizeof(drv->driver.name)) >= sizeof(drv->driver.name)) {
        return -EOVERFLOW;
    }
...
irqreturn_t serio_interrupt(struct serio *serio, unsigned char data, uint32_t dfl)
{
    irq_flags_t flags;
    irqreturn_t ret = IRQ_NONE;

    spin_lock_irq_save(&serio->lock, flags);
    ...
    spin_unlock_irq_restore(&serio->lock, flags);
