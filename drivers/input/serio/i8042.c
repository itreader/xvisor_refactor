...
static vmm_device_t *i8042_platform_device;

static irqreturn_t i8042_interrupt(int irq, void *dev_id);
static bool (*i8042_platform_filter)(unsigned char data, unsigned char str, struct serio *serio);
...
int i8042_install_filter(bool (*filter)(unsigned char data, unsigned char str, struct serio *serio))
{
    ...
    spin_lock_irq_save(&i8042_lock, flags);
...
out:
    spin_unlock_irq_restore(&i8042_lock, flags);
    return ret;
}
...
static int __i8042_command(unsigned char *param, int command)
{
    ...
    for (i = 0; i < ((command >> 8) & 0xf); i++) {
        ...
        if (command == I8042_CMD_AUX_LOOP && !(i8042_read_status() & I8042_STR_AUXDATA)) {
            dbg("     -- i8042 (auxerr)\n");
            return -1;
        }
...
static int i8042_aux_write(struct serio *serio, unsigned char c)
{
    ...
    return i8042_command(&c, port->mux == -1 ? I8042_CMD_AUX_SEND : I8042_CMD_MUX_SEND + port->mux);
}
...
static bool i8042_filter(unsigned char data, unsigned char str, struct serio *serio)
{
    if (unlikely(i8042_suppress_kbd_ack)) {
        if ((~str & I8042_STR_AUXDATA) && (data == 0xfa || data == 0xfe)) {
...
static irqreturn_t i8042_interrupt(int irq, void *dev_id)
{
    ...
    dbg("%02x <- i8042 (interrupt, %d, %d%s%s)\n", data, port_no, irq, dfl & SERIO_PARITY ? ", bad parity" : "",
...
static int __init i8042_toggle_aux(bool on)
{
    ...
    if (i8042_command(&param, on ? I8042_CMD_AUX_ENABLE : I8042_CMD_AUX_DISABLE)) {
        return -1;
    }
...
static int __init i8042_check_aux(void)
{
    ...
    if (retval || param != 0x5a) {
        ...
        if (i8042_command(&param, I8042_CMD_AUX_TEST) || (param && param != 0xfa && param != 0xff)) {
            return -1;
        }
    ...
    if (request_irq(I8042_AUX_IRQ, i8042_aux_test_irq, IRQF_SHARED, "i8042", i8042_platform_device)) {
        goto out;
    }
    ...
    if (wait_for_completion_timeout(&i8042_aux_irq_delivered, msecs_to_jiffies(250)) == 0) {
        /*
...
static void __init i8042_register_ports(void)
{
    ...
    for (i = 0; i < I8042_NUM_PORTS; i++) {
        if (i8042_ports[i].serio) {
            printk(
                KERN_INFO "serio: %s at %#lx,%#lx irq %d\n", i8042_ports[i].serio->name, (uint64_t)I8042_DATA_REG, (uint64_t)I8042_COMMAND_REG,
                i8042_ports[i].irq);
...
static int __init i8042_setup_aux(void)
{
    ...
    error = request_irq(I8042_AUX_IRQ, i8042_interrupt, IRQF_SHARED, "i8042", i8042_platform_device);
...
static int i8042_driver_probe(vmm_device_t *dev)
{
    int error;
...
static int i8042_driver_remove(vmm_device_t *dev)
{
...
static struct vmm_device_tree_nodeid def_i8042_devid_table[] = {
    {.compatible = "i8042"},
    {/* end of list */},
};

static vmm_driver_t i8042_driver = {
    .name        = "i8042_driver",
...
static int __init i8042_init(void)
{
    ...
    return vmm_device_driver_register_driver(&i8042_driver);
...
{
    vmm_device_driver_unregister_driver(&i8042_driver);
    i8042_platform_exit();
