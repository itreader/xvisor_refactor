...
static int goldfish_rtc_read_alarm(struct rtc_device *dev, struct rtc_wkalrm *alrm)
{
    uint64_t             rtc_alarm;
    uint64_t             rtc_alarm_low;
    uint64_t             rtc_alarm_high;
    void                *base;
...
static int goldfish_rtc_set_alarm(struct rtc_device *dev, struct rtc_wkalrm *alrm)
{
    struct goldfish_rtc *rtcdrv;
    uint64_t             rtc_alarm64;
    uint64_t             rtc_status_reg;
    void                *base;
...
static int goldfish_rtc_alarm_irq_enable(struct rtc_device *dev, uint32_t enabled)
{
    void                *base;
    struct goldfish_rtc *rtcdrv;
...
static int goldfish_rtc_probe(vmm_device_t *dev)
{
    int                  rc;
    ...
    rc = vmm_device_tree_request_regmap(dev->of_node, &reg_base, 0, "Goldfish RTC");
    ...
    if ((rc = vmm_host_irq_register(rtcdrv->irq, dev->name, goldfish_rtc_interrupt, rtcdrv))) {
        goto free_reg;
    }

    rtcdrv->rtc = rtc_device_register(dev, dev->name, &goldfish_rtc_ops, rtcdrv);
    ...
    vmm_device_driver_set_data(dev, rtcdrv);
...
free_reg:
    vmm_device_tree_regunmap_release(dev->of_node, (virtual_addr_t)rtcdrv->base, 0);
free_data:
    vmm_free(rtcdrv);
free_nothing:
    return rc;
...
static int goldfish_rtc_remove(vmm_device_t *dev)
{
...
static struct vmm_device_tree_nodeid goldfish_rtc_of_match[] = {
    {.compatible = "google,goldfish-rtc"},
    {/* end of list */},
};

static vmm_driver_t goldfish_rtc_driver = {
    .name        = "goldfish_rtc",
...
{
    return vmm_device_driver_register_driver(&goldfish_rtc_driver);
}

static void __exit goldfish_rtc_driver_exit(void)
{
    vmm_device_driver_unregister_driver(&goldfish_rtc_driver);
}
