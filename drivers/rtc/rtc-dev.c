...
int rtc_device_sync_wall_clock(struct rtc_device *rdev)
{
    ...
    utc_tz.tz_minutes_greenwich = 0;
    utc_tz.tz_dsttime           = 0;

    tv.tv_sec  = vmm_wall_clock_mktime(tm.tm_year + 1900, tm.tm_month + 1, tm.tm_month_of_day, tm.tm_hour, tm.tm_minute, tm.tm_second);
    tv.tv_nsec = 0;

    if ((rc = vmm_wall_clock_set_timeofday(&tv, &utc_tz))) {
        return rc;
    }
...
int rtc_device_sync_device(struct rtc_device *rdev)
{
    ...
    rtc_time_to_tm((uint64_t)tv.tv_sec, &tm);
...
struct rtc_device *rtc_device_register(vmm_device_t *parent, const char *name, const struct rtc_class_ops *ops, void *private)
{
    int                ret;
    ...
    vmm_device_driver_set_data(&rdev->dev, rdev);
    rdev->ops     = ops;
    rdev->private = private;

    ret           = vmm_device_driver_register_device(&rdev->dev);
...
struct rtc_device *rtc_device_find(const char *name)
{
    ...
    dev = vmm_device_driver_class_find_device_by_name(&rtc_class, name);
...
static int __rtc_device_iterate(vmm_device_t *dev, void *data)
{
    struct rtc_device_iterate_priv *p    = data;
    struct rtc_device              *rdev = vmm_device_driver_get_data(dev);

    return p->fn(rdev, p->data);
}

int rtc_device_iterate(struct rtc_device *start, void *data, int (*fn)(struct rtc_device *rdev, void *data))
{
...
{
    return vmm_device_driver_register_class(&rtc_class);
}

static void __exit rtc_device_exit(void)
{
    vmm_device_driver_unregister_class(&rtc_class);
}
