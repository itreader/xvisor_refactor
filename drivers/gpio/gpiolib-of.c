...
static int of_gpiochip_find_and_xlate(struct gpio_chip *gc, void *data)
{
    ...
    if ((gc->of_node != gg_data->gpiospec.np) || (gc->of_gpio_n_cells != gg_data->gpiospec.args_count) || (!gc->of_xlate)) {
        return false;
    }
...
struct gpio_desc *of_get_named_gpiod_flags(struct device_node *np, const char *propname, int index, enum of_gpio_flags *flags)
{
     ...
     */
    struct gg_data gg_data = {.flags = flags, .out_gpio = ERR_PTR(-EPROBE_DEFER)};
    int            ret;
 ...
 */
int of_gpio_simple_xlate(struct gpio_chip *gc, const struct of_phandle_args *gpiospec, uint32_t *flags)
{
    /*
...
static void of_gpiochip_add_pin_range(struct gpio_chip *chip)
{
    ...
    for (;; index++) {
        ...
        if (pinspec.args[2]) {
            ...
            ret = gpiochip_add_pin_range(chip, pinctrl_dev_get_devname(pctldev), pinspec.args[0], pinspec.args[1], pinspec.args[2]);
