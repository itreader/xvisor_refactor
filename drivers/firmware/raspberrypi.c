 ...
 */
static int rpi_firmware_transaction(struct rpi_firmware *fw, uint32_t chan, uint32_t data)
{
    uint32_t message = MBOX_MSG(chan, data);
    int      ret;
    ...
    if (ret >= 0) {
        ...
        vmm_lerror(fw->cl.dev->name, "mbox_send_message returned %d\n", ret);
    }
...
int rpi_firmware_property_list(struct rpi_firmware *fw, void *data, size_t tag_size)
{
    ...
    for (i = 0; i < (tag_size / 4); i++) {
        buf[2 + i] = ((uint32_t *)data)[i];
    }

    buf[size / 4 - 1] = RPI_FIRMWARE_PROPERTY_END;
    arch_smp_wmb();

    ret = rpi_firmware_transaction(fw, MBOX_CHAN_PROPERTY, (uint32_t)buf_pa);

    arch_smp_rmb();

    for (i = 0; i < (tag_size / 4); i++) {
        ((uint32_t *)data)[i] = buf[2 + i];
    }
 ...
 */
int rpi_firmware_property(struct rpi_firmware *fw, uint32_t tag, void *tag_data, size_t buf_size)
{
    /* Single tags are very small (generally 8 bytes), so the
     * stack should be safe.
     */
    uint8_t                                  data[buf_size + sizeof(struct rpi_firmware_property_tag_header)];
    struct rpi_firmware_property_tag_header *header = (struct rpi_firmware_property_tag_header *)data;
    int                                      ret;

    header->tag           = tag;
    header->buf_size      = buf_size;
    header->req_resp_size = 0;
    memcpy(data + sizeof(struct rpi_firmware_property_tag_header), tag_data, buf_size);

    ret = rpi_firmware_property_list(fw, &data, sizeof(data));
    memcpy(tag_data, data + sizeof(struct rpi_firmware_property_tag_header), buf_size);
...
{
    uint32_t packet;
    int      ret = rpi_firmware_property(fw, RPI_FIRMWARE_GET_FIRMWARE_REVISION, &packet, sizeof(packet));

    if (ret == 0) {
        vmm_time_info_t tm;

        vmm_wall_clock_mkinfo(packet, 0, &tm);

        vmm_linfo(
            fw->cl.dev->name, "Attached to firmware from %04ld-%02d-%02d %02d:%02d\n", tm.tm_year + 1900, tm.tm_month + 1, tm.tm_month_of_day,
            tm.tm_hour, tm.tm_minute);
    }
}

static int rpi_firmware_probe(vmm_device_t *dev)
{
    ...
    vmm_device_driver_set_data(dev, fw);
...
static int rpi_firmware_remove(vmm_device_t *dev)
{
    struct rpi_firmware *fw = vmm_device_driver_get_data(dev);
 ...
 */
struct rpi_firmware *rpi_firmware_get(vmm_device_tree_node_t *firmware_node)
{
    vmm_device_t *dev = vmm_platform_find_device_by_node(firmware_node);
...
static vmm_driver_t rpi_firmware_driver = {
...
{
    return vmm_device_driver_register_driver(&rpi_firmware_driver);
}

static void __exit rpi_firmware_exit(void)
{
    vmm_device_driver_unregister_driver(&rpi_firmware_driver);
}
