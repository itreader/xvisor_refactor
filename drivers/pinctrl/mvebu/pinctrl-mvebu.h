...
struct mvebu_mpp_ctrl {
    ...
    uint8_t     npins;
    unsigned   *pins;
    int (*mpp_get)(struct mvebu_mpp_ctrl_data *data, unsigned pid, uint64_t *config);
    int (*mpp_set)(struct mvebu_mpp_ctrl_data *data, unsigned pid, uint64_t config);
...
#define MPP_FUNC_CTRL(_idl, _idh, _name, _func)                                                                                                      \
    {                                                                                                                                                \
        .name = _name, .pid = _idl, .npins = _idh - _idl + 1, .pins = (unsigned[_idh - _idl + 1]){}, .mpp_get = _func##_get, .mpp_set = _func##_set, \
        .mpp_gpio_req = NULL, .mpp_gpio_dir = NULL,                                                                                                  \
    }
...
#define MPP_VAR_FUNCTION(_val, _name, _subname, _mask) _MPP_VAR_FUNCTION(_val, _name, _subname, _mask)
#else
...
int mvebu_mmio_mpp_ctrl_get(struct mvebu_mpp_ctrl_data *data, unsigned pid, uint64_t *config);
int mvebu_mmio_mpp_ctrl_set(struct mvebu_mpp_ctrl_data *data, unsigned pid, uint64_t config);
int mvebu_regmap_mpp_ctrl_get(struct mvebu_mpp_ctrl_data *data, unsigned pid, uint64_t *config);
int mvebu_regmap_mpp_ctrl_set(struct mvebu_mpp_ctrl_data *data, unsigned pid, uint64_t config);
