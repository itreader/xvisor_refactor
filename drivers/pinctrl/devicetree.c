...
/**
 * struct pinctrl_device_tree_map - mapping table chunk parsed from device tree
 * @node: list node for struct pinctrl's @dt_maps field
 * @pctldev: the pin controller that allocated this struct, and will free it
 * @maps: the mapping table entries
 */
struct pinctrl_device_tree_map {
    list_head_t         node;
...
static void dt_free_map(pinctrl_device_t *pctldev, struct pinctrl_map *map, unsigned num_maps)
{
    if (pctldev) {
...
void pinctrl_device_tree_free_maps(struct pinctrl *p)
{
    struct pinctrl_device_tree_map *dt_map, *n1;
...
static int dt_remember_or_free_map(struct pinctrl *p, const char *statename, pinctrl_device_t *pctldev, struct pinctrl_map *map, unsigned num_maps)
{
    int                             i;
    struct pinctrl_device_tree_map *dt_map;
...
pinctrl_device_t *of_pinctrl_get(struct device_node *np)
{
    return get_pinctrl_dev_from_of_node(np);
}

static int dt_to_map_one_config(struct pinctrl *p, pinctrl_device_t *hog_pctldev, const char *statename, struct device_node *np_config)
{
...
bool pinctrl_device_tree_has_hogs(pinctrl_device_t *pctldev)
{
...
int pinctrl_device_tree_to_map(struct pinctrl *p, pinctrl_device_t *pctldev)
{
...
#else
    error = of_property_read_u32(np->parent, cells_name, (uint32_t *)&cells_size);
#endif
...
#else
        error = of_property_read_u32(np->parent->parent, cells_name, (uint32_t *)&cells_size);
#endif
 ...
 */
static int pinctrl_get_list_and_count(const struct device_node *np, const char *list_name, const __be32 **list, int *cells_size, int *nr_elements)
{
    int size;
 ...
 * within the controller and a number of uint32_t entries specified for each
...
int pinctrl_count_index_with_args(const struct device_node *np, const char *list_name)
{
    ...
    error = pinctrl_get_list_and_count(np, list_name, &list, &nr_cells, &size);
 ...
 */
static int pinctrl_copy_args(const struct device_node *np, const __be32 *list, int index, int nr_cells, int nr_elem, struct of_phandle_args *out_args)
{
    int i;
...
int pinctrl_parse_index_with_args(const struct device_node *np, const char *list_name, int index, struct of_phandle_args *out_args)
{
    ...
    error = pinctrl_get_list_and_count(np, list_name, &list, &nr_cells, &nr_elem);

    if (error || !nr_cells) {
        return error;
    }

    error = pinctrl_copy_args(np, list, index, nr_cells, nr_elem, out_args);
