 ...
 * @domain: irq_domain of the gpio bank
...
struct rockchip_pin_bank {
    ...
    uint32_t                    toggle_edge_mode;
    uint32_t                    recalced_mask;
    uint32_t                    route_mask;
};
...
struct rockchip_pin_ctrl {
    ...
    uint32_t                           niomux_recalced;
    struct rockchip_mux_route_data    *iomux_routes;
    uint32_t                           niomux_routes;

    int (*ctrl_data_re_init)(struct rockchip_pin_ctrl *ctrl);

    int (*soc_data_init)(struct rockchip_pinctrl *info);

    void (*pull_calc_reg)(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit);
    enum rockchip_pin_drv_type (*drv_calc_reg)(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit);
    enum rockchip_pin_extra_drv_type (*drv_calc_extra_reg)(
        struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit);
    int (*schmitt_calc_reg)(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit);
    int (*slew_rate_calc_reg)(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit);
};
...
static inline const struct rockchip_pin_group *pinctrl_name_to_group(const struct rockchip_pinctrl *info, const char *name)
{
    int i;
...
static int rockchip_get_groups_count(pinctrl_device_t *pctldev)
{
...
static const char *rockchip_get_group_name(pinctrl_device_t *pctldev, unsigned selector)
{
    struct rockchip_pinctrl *info = pinctrl_dev_get_drvdata(pctldev);
...
static int rockchip_get_group_pins(pinctrl_device_t *pctldev, unsigned selector, const unsigned **pins, unsigned *npins)
{
    struct rockchip_pinctrl *info = pinctrl_dev_get_drvdata(pctldev);
...
static int rockchip_device_tree_node_to_map(pinctrl_device_t *pctldev, struct device_node *np, struct pinctrl_map **map, unsigned *num_maps)
{
    struct rockchip_pinctrl         *info = pinctrl_dev_get_drvdata(pctldev);
...
static void rockchip_device_tree_free_map(pinctrl_device_t *pctldev, struct pinctrl_map *map, unsigned num_maps) {}

static const struct pinctrl_ops rockchip_pctrl_ops = {
    .get_groups_count = rockchip_get_groups_count,
    .get_group_name   = rockchip_get_group_name,
    .get_group_pins   = rockchip_get_group_pins,
    .dt_node_to_map   = rockchip_device_tree_node_to_map,
    .dt_free_map      = rockchip_device_tree_free_map,
};
...
static struct rockchip_mux_recalced_data rk3308_mux_recalced_data[] = {
    {.num = 1, .pin = 14, .reg = 0x28, .bit = 12, .mask = 0xf},
    {.num = 1, .pin = 15, .reg = 0x2c, .bit = 0,  .mask = 0x3},
    {.num = 1, .pin = 18, .reg = 0x30, .bit = 4,  .mask = 0xf},
    {.num = 1, .pin = 19, .reg = 0x30, .bit = 8,  .mask = 0xf},
    {.num = 1, .pin = 20, .reg = 0x30, .bit = 12, .mask = 0xf},
    {.num = 1, .pin = 21, .reg = 0x34, .bit = 0,  .mask = 0xf},
    {.num = 1, .pin = 22, .reg = 0x34, .bit = 4,  .mask = 0xf},
    {.num = 1, .pin = 23, .reg = 0x34, .bit = 8,  .mask = 0xf},
    {.num = 3, .pin = 12, .reg = 0x68, .bit = 8,  .mask = 0xf},
    {.num = 3, .pin = 13, .reg = 0x68, .bit = 12, .mask = 0xf},
};
...
static void rockchip_get_recalced_mux(struct rockchip_pin_bank *bank, int pin, int *reg, uint8_t *bit, int *mask)
{
...
static bool rockchip_get_mux_route(struct rockchip_pin_bank *bank, int pin, int mux, uint32_t *reg, uint32_t *value)
{
    struct rockchip_pinctrl        *info = bank->drvdata;
    struct rockchip_pin_ctrl       *ctrl = info->ctrl;
...
static int rockchip_get_mux(struct rockchip_pin_bank *bank, int pin)
{
    ...
    regmap   = (bank->iomux[iomux_num].type & IOMUX_SOURCE_PMU) ? info->regmap_pmu : info->regmap_base;
...
static int rockchip_set_mux(struct rockchip_pin_bank *bank, int pin, int mux)
{
    ...
    int                      reg, ret, mask, mux_type;
    uint8_t                  bit;
    uint32_t                 data, rmask, route_reg, route_val;
    ...
    if (bank->route_mask & BIT(pin)) {
        if (rockchip_get_mux_route(bank, pin, mux, &route_reg, &route_val)) {
...
static void px30_calc_pull_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
...
static void rv1108_calc_pull_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
...
static int rv1108_calc_schmitt_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
    int                      pins_per_reg;
...
static void rk1808_calc_pull_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
...
static enum rockchip_pin_drv_type rk1808_calc_drv_reg_and_bit(
    struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
...
static int rk1808_calc_schmitt_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
...
static void rk2928_calc_pull_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
...
static void rk3128_calc_pull_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
...
static void rk3188_calc_pull_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
...
static void rk3288_calc_pull_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
...
static void rk3228_calc_pull_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
...
static void rk3308_calc_pull_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
...
static void rk3366_calc_pull_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
...
static void rk3368_calc_pull_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
...
static void rk3399_calc_pull_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
    ...
    struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    ...
    *reg = bank->drv[drv_num].offset;
    ...
    if ((bank->drv[drv_num].drv_type == DRV_TYPE_IO_1V8_3V0_AUTO) || (bank->drv[drv_num].drv_type == DRV_TYPE_IO_3V3_ONLY)) {
        *bit = (pin_num % 8) * 3;
    } else {
        *bit = (pin_num % 8) * 2;
    }
...
static int rockchip_get_drive_perpin(struct rockchip_pin_bank *bank, int pin_num)
{
    ...
    uint32_t                  data, temp, rmask_bits;
    uint8_t                   bit, extra_bit;
    int                       drv_type;
    ...
    switch (drv_type) {
        ...
        case DRV_TYPE_IO_WIDE_LEVEL:
             ...
             */
            ctrl->drv_calc_extra_reg(bank, pin_num, &extra_regmap, &extra_reg, &extra_bit);
...
static int rockchip_set_drive_perpin(struct rockchip_pin_bank *bank, int pin_num, int strength)
{
    ...
    struct regmap            *regmap, *extra_regmap;
    int                       reg, ret, i;
    uint32_t                  data, temp, rmask, rmask_bits;
    uint8_t                   bit, extra_bit;
    ...
    if (drv_type == DRV_TYPE_IO_WIDE_LEVEL) {
        ...
        for (i = 0; i < ARRAY_SIZE(rockchip_perpin_drv_list[drv_type]); i++) {
            if (rockchip_perpin_drv_list[drv_type][i] < 0) {
                ...
                break;
            } else if (rockchip_perpin_drv_list[drv_type][i] == strength) {
                ret = i;
                break;
            }
        }
    ...
    switch (drv_type) {
        ...
        case DRV_TYPE_IO_WIDE_LEVEL:
            ...
            extra_value    = ((strength - rockchip_perpin_drv_list[drv_type][ret])) >> 1;
            rmask_bits     = RK3288_DRV_BITS_PER_PIN;
             ...
             * P channel are the same.
             */
            extra_drv_type = ctrl->drv_calc_extra_reg(bank, pin_num, &extra_regmap, &extra_reg, &extra_bit);
...
static int px30_calc_schmitt_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
    int                      pins_per_reg;
...
static int rk3308_calc_schmitt_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
...
static int rk3328_calc_schmitt_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
...
static int px30_calc_slew_rate_reg_and_bit(struct rockchip_pin_bank *bank, int pin_num, struct regmap **regmap, int *reg, uint8_t *bit)
{
    struct rockchip_pinctrl *info = bank->drvdata;
    int                      pins_per_reg;
...
static int rockchip_pmx_get_funcs_count(pinctrl_device_t *pctldev)
{
    struct rockchip_pinctrl *info = pinctrl_dev_get_drvdata(pctldev);
...
static const char *rockchip_pmx_get_func_name(pinctrl_device_t *pctldev, unsigned selector)
{
    struct rockchip_pinctrl *info = pinctrl_dev_get_drvdata(pctldev);
...
static int rockchip_pmx_get_groups(pinctrl_device_t *pctldev, unsigned selector, const char *const **groups, unsigned *const num_groups)
{
    struct rockchip_pinctrl *info = pinctrl_dev_get_drvdata(pctldev);
...
static int rockchip_pmx_set(pinctrl_device_t *pctldev, unsigned selector, unsigned group)
{
    struct rockchip_pinctrl          *info = pinctrl_dev_get_drvdata(pctldev);
    const uint32_t                   *pins = info->groups[group].pins;
...
static int rockchip_gpio_get_direction(struct gpio_chip *chip, unsigned offset)
{
    ...
    ret = clock_enable(bank->clk);

    if (ret < 0) {
        dev_err(bank->drvdata->dev, "failed to enable clock for bank %s\n", bank->name);
        return ret;
    }

    data = readl_relaxed(bank->reg_base + GPIO_SWPORT_DDR);
    clock_disable(bank->clk);
 ...
 */
static int _rockchip_pmx_gpio_set_direction(struct gpio_chip *chip, int pin, bool input)
{
    struct rockchip_pin_bank *bank;
    int                       ret;
    uint64_t                  flags;
    ...
    clock_enable(bank->clk);
    raw_spin_lock_irq_save(&bank->slock, flags);
    ...
    raw_spin_unlock_irq_restore(&bank->slock, flags);
    clock_disable(bank->clk);

    return 0;
}

static int rockchip_pmx_gpio_set_direction(pinctrl_device_t *pctldev, struct pinctrl_gpio_range *range, unsigned offset, bool input)
{
    ...
    return _rockchip_pmx_gpio_set_direction(chip, offset - chip->base, input);
}
...
static int rockchip_pinconf_set(pinctrl_device_t *pctldev, uint32_t pin, uint64_t *configs, unsigned num_configs)
{
    struct rockchip_pinctrl  *info = pinctrl_dev_get_drvdata(pctldev);
    ...
    for (i = 0; i < num_configs; i++) {
        ...
        switch (param) {
            ...
            case PIN_CONFIG_OUTPUT:
                ...
                rc = _rockchip_pmx_gpio_set_direction(&bank->gpio_chip, pin - bank->pin_base, false);
...
static int rockchip_pinconf_get(pinctrl_device_t *pctldev, uint32_t pin, uint64_t *config)
{
    struct rockchip_pinctrl  *info  = pinctrl_dev_get_drvdata(pctldev);
    struct rockchip_pin_bank *bank  = pin_to_bank(info, pin);
...
static int rockchip_pinctrl_parse_groups(struct device_node *np, struct rockchip_pin_group *grp, struct rockchip_pinctrl *info, uint32_t index)
{
    struct rockchip_pin_bank *bank;
    int                       size;
    const __be32             *list;
    ...
    grp->pins  = devm_kzalloc(info->dev, grp->npins * sizeof(uint32_t), GFP_KERNEL);
    grp->data  = devm_kzalloc(info->dev, grp->npins * sizeof(struct rockchip_pin_config), GFP_KERNEL);
    ...
    for (i = 0, j = 0; i < size; i += 4, j++) {
        ...
        ret       = pinconf_generic_parse_device_tree_config(np_config, NULL, &grp->data[j].configs, &grp->data[j].nconfigs);
...
static int rockchip_pinctrl_parse_functions(struct device_node *np, struct rockchip_pinctrl *info, uint32_t index)
{
    struct device_node        *child;
    struct rockchip_pmx_func  *func;
    struct rockchip_pin_group *grp;
    int                        ret;
    static uint32_t            grp_index;
    uint32_t                   i = 0;
...
static int rockchip_pinctrl_parse_dt(struct platform_device *pdev, struct rockchip_pinctrl *info)
{
    ...
    info->functions = devm_kzalloc(pdev, info->nfunctions * sizeof(struct rockchip_pmx_func), GFP_KERNEL);

    if (!info->functions) {
        return -EINVAL;
    }

    info->groups = devm_kzalloc(pdev, info->ngroups * sizeof(struct rockchip_pin_group), GFP_KERNEL);
...
static int rockchip_pinctrl_register(struct platform_device *pdev, struct rockchip_pinctrl *info)
{
    ...
    pindesc           = devm_kzalloc(pdev, sizeof(*pindesc) * info->ctrl->nr_pins, GFP_KERNEL);
    ...
    for (bank = 0, k = 0; bank < info->ctrl->nr_banks; bank++) {
        ...
        for (pin = 0; pin < pin_bank->nr_pins; pin++, k++) {
            ...
            vmm_snprintf((char *)pdesc->name, len, "%s-%d", pin_bank->name, pin);
#endif
            pdesc++;
        }
    }
 ...
 * interface.
 */
static int rockchip_gpio_direction_output(struct gpio_chip *gc, unsigned offset, int value)
{
...
static void rockchip_irq_demux(struct irq_desc *desc)
{
    ...
    while (pend) {
        ...
        if (bank->toggle_edge_mode & BIT(irq)) {
            uint32_t data, data_old, polarity;
            uint64_t flags;
...
static int rockchip_irq_set_type(struct irq_data *d, uint32_t type)
{
...
#endif
static int rockchip_interrupts_register(struct platform_device *pdev, struct rockchip_pinctrl *info)
{
#if 0
    struct rockchip_pin_ctrl *ctrl = info->ctrl;
    struct rockchip_pin_bank *bank = ctrl->pin_banks;
    uint32_t clr = IRQ_NOREQUEST | IRQ_NOPROBE | IRQ_NOAUTOEN;
...
static int rockchip_gpiolib_register(struct platform_device *pdev, struct rockchip_pinctrl *info)
{
    struct rockchip_pin_ctrl *ctrl = info->ctrl;
    struct rockchip_pin_bank *bank = ctrl->pin_banks;
    struct gpio_chip         *gc;
...
static int rockchip_gpiolib_unregister(struct platform_device *pdev, struct rockchip_pinctrl *info)
{
    struct rockchip_pin_ctrl *ctrl = info->ctrl;
    struct rockchip_pin_bank *bank = ctrl->pin_banks;
    int                       i;
...
static int rockchip_get_bank_data(struct rockchip_pin_bank *bank, struct rockchip_pinctrl *info)
{
#if 0
    struct resource res;
    void __iomem *base;
...
#else
    err = vmm_device_tree_request_regmap(bank->of_node, (virtual_addr_t *)&bank->reg_base, 0, "Rockchip Pinctrl");
    ...
    bank->clk = of_clock_get(bank->of_node, 0);

    if (IS_ERR(bank->clk)) {
        return PTR_ERR(bank->clk);
    }

    return clock_prepare(bank->clk);
}

static const struct of_device_id rockchip_pinctrl_device_tree_match[];
...
static struct rockchip_pin_ctrl *rockchip_pinctrl_get_soc_data(struct rockchip_pinctrl *d, struct platform_device *pdev)
{
    ...
    match = of_match_node(rockchip_pinctrl_device_tree_match, node);
    ...
    for (i = 0; i < ctrl->nr_banks; ++i, ++bank) {
        ...
        for (j = 0; j < 4; j++) {
             ...
             */
            inc = (iom->type & (IOMUX_WIDTH_4BIT | IOMUX_WIDTH_3BIT | IOMUX_8WIDTH_2BIT)) ? 8 : 4;
...
static uint32_t rk3288_grf_gpio6c_iomux;
...
{
    struct rockchip_pinctrl *info = dev_get_drvdata(dev);
    int                      ret  = regmap_write(info->regmap_base, RK3288_GRF_GPIO6C_IOMUX, rk3288_grf_gpio6c_iomux | GPIO6C6_SEL_WRITE_ENABLE);
...
#if 0
static SIMPLE_DEVICE_PM_OPS(rockchip_pinctrl_dev_pm_ops, rockchip_pinctrl_suspend,
...
static struct rockchip_pin_bank px30_pin_banks[] = {
    PIN_BANK_IOMUX_FLAGS(0, 32, "gpio0", IOMUX_SOURCE_PMU, IOMUX_SOURCE_PMU, IOMUX_SOURCE_PMU, IOMUX_SOURCE_PMU),
...
static struct rockchip_pin_bank rk3066a_pin_banks[] = {
    PIN_BANK(0, 32, "gpio0"), PIN_BANK(1, 32, "gpio1"), PIN_BANK(2, 32, "gpio2"),
    PIN_BANK(3, 32, "gpio3"), PIN_BANK(4, 32, "gpio4"), PIN_BANK(6, 16, "gpio6"),
};
...
static struct rockchip_pin_bank rk3288_pin_banks[] = {
    PIN_BANK_IOMUX_FLAGS(
        0, 24, "gpio0", IOMUX_SOURCE_PMU | IOMUX_WRITABLE_32BIT, IOMUX_SOURCE_PMU | IOMUX_WRITABLE_32BIT, IOMUX_SOURCE_PMU | IOMUX_WRITABLE_32BIT,
        IOMUX_UNROUTED),
    PIN_BANK_IOMUX_FLAGS(1, 32, "gpio1", IOMUX_UNROUTED, IOMUX_UNROUTED, IOMUX_UNROUTED, 0),
    PIN_BANK_IOMUX_FLAGS(2, 32, "gpio2", 0, 0, 0, IOMUX_UNROUTED),
    PIN_BANK_IOMUX_FLAGS(3, 32, "gpio3", 0, 0, 0, IOMUX_WIDTH_4BIT),
    PIN_BANK_IOMUX_FLAGS(4, 32, "gpio4", IOMUX_WIDTH_4BIT, IOMUX_WIDTH_4BIT, 0, 0),
    PIN_BANK_IOMUX_FLAGS(5, 32, "gpio5", IOMUX_UNROUTED, 0, 0, IOMUX_UNROUTED),
    PIN_BANK_IOMUX_FLAGS(6, 32, "gpio6", 0, 0, 0, IOMUX_UNROUTED),
    PIN_BANK_IOMUX_FLAGS(7, 32, "gpio7", 0, 0, IOMUX_WIDTH_4BIT, IOMUX_UNROUTED),
    PIN_BANK(8, 16, "gpio8"),
};
...
static struct rockchip_pin_bank rk3308_pin_banks[] = {
    PIN_BANK_IOMUX_FLAGS(0, 32, "gpio0", IOMUX_8WIDTH_2BIT, IOMUX_8WIDTH_2BIT, IOMUX_8WIDTH_2BIT, IOMUX_8WIDTH_2BIT),
    PIN_BANK_IOMUX_FLAGS(1, 32, "gpio1", IOMUX_8WIDTH_2BIT, IOMUX_8WIDTH_2BIT, IOMUX_8WIDTH_2BIT, IOMUX_8WIDTH_2BIT),
    PIN_BANK_IOMUX_FLAGS(2, 32, "gpio2", IOMUX_8WIDTH_2BIT, IOMUX_8WIDTH_2BIT, IOMUX_8WIDTH_2BIT, IOMUX_8WIDTH_2BIT),
    PIN_BANK_IOMUX_FLAGS(3, 32, "gpio3", IOMUX_8WIDTH_2BIT, IOMUX_8WIDTH_2BIT, IOMUX_8WIDTH_2BIT, IOMUX_8WIDTH_2BIT),
    PIN_BANK_IOMUX_FLAGS(4, 32, "gpio4", IOMUX_8WIDTH_2BIT, IOMUX_8WIDTH_2BIT, IOMUX_8WIDTH_2BIT, IOMUX_8WIDTH_2BIT),
};
...
static struct rockchip_pin_bank rk3328_pin_banks[] = {
    ...
    PIN_BANK_IOMUX_FLAGS(3, 32, "gpio3", IOMUX_WIDTH_3BIT, IOMUX_WIDTH_3BIT, 0, 0),
};
...
static struct rockchip_pin_bank rk3366_pin_banks[] = {
    PIN_BANK_IOMUX_DRV_FLAGS(
        0, 32, "gpio0", IOMUX_SOURCE_PMU, IOMUX_SOURCE_PMU, IOMUX_SOURCE_PMU, IOMUX_SOURCE_PMU, DRV_TYPE_IO_NARROW_LEVEL, DRV_TYPE_IO_NARROW_LEVEL,
        DRV_TYPE_IO_NARROW_LEVEL, DRV_TYPE_IO_NARROW_LEVEL),
    PIN_BANK_IOMUX_FLAGS_OFFSET_DRV_FLAGS(
        1, 32, "gpio1", IOMUX_SOURCE_PMU, IOMUX_SOURCE_PMU, IOMUX_SOURCE_PMU, IOMUX_SOURCE_PMU, 0x30, 0x34, 0x38, 0x3c, DRV_TYPE_IO_NARROW_LEVEL,
        DRV_TYPE_IO_NARROW_LEVEL, DRV_TYPE_IO_NARROW_LEVEL, DRV_TYPE_IO_NARROW_LEVEL),
    PIN_BANK_DRV_FLAGS(2, 32, "gpio2", DRV_TYPE_IO_WIDE_LEVEL, DRV_TYPE_IO_NARROW_LEVEL, DRV_TYPE_IO_WIDE_LEVEL, DRV_TYPE_IO_NARROW_LEVEL),
    PIN_BANK_DRV_FLAGS(3, 32, "gpio3", DRV_TYPE_IO_NARROW_LEVEL, DRV_TYPE_IO_NARROW_LEVEL, DRV_TYPE_IO_NARROW_LEVEL, DRV_TYPE_IO_NARROW_LEVEL),
    PIN_BANK_DRV_FLAGS(4, 32, "gpio4", DRV_TYPE_IO_NARROW_LEVEL, DRV_TYPE_IO_NARROW_LEVEL, DRV_TYPE_IO_NARROW_LEVEL, DRV_TYPE_IO_NARROW_LEVEL),
    PIN_BANK_DRV_FLAGS(5, 32, "gpio5", DRV_TYPE_IO_NARROW_LEVEL, DRV_TYPE_IO_NARROW_LEVEL, DRV_TYPE_IO_NARROW_LEVEL, DRV_TYPE_IO_NARROW_LEVEL),
};
...
static struct rockchip_pin_bank rk3399_pin_banks[] = {
    PIN_BANK_IOMUX_FLAGS_DRV_FLAGS_OFFSET_PULL_FLAGS(
        ...
        DRV_TYPE_IO_DEFAULT, DRV_TYPE_IO_DEFAULT, 0x80, 0x88, -1, -1, PULL_TYPE_IO_1V8_ONLY, PULL_TYPE_IO_1V8_ONLY, PULL_TYPE_IO_DEFAULT,
        PULL_TYPE_IO_DEFAULT),
    PIN_BANK_IOMUX_FLAGS_DRV_FLAGS_OFFSET(
        1, 32, "gpio1", IOMUX_SOURCE_PMU, IOMUX_SOURCE_PMU, IOMUX_SOURCE_PMU, IOMUX_SOURCE_PMU, DRV_TYPE_IO_1V8_OR_3V0, DRV_TYPE_IO_1V8_OR_3V0,
        DRV_TYPE_IO_1V8_OR_3V0, DRV_TYPE_IO_1V8_OR_3V0, 0xa0, 0xa8, 0xb0, 0xb8),
    PIN_BANK_DRV_FLAGS_PULL_FLAGS(
        2, 32, "gpio2", DRV_TYPE_IO_1V8_OR_3V0, DRV_TYPE_IO_1V8_OR_3V0, DRV_TYPE_IO_1V8_ONLY, DRV_TYPE_IO_1V8_ONLY, PULL_TYPE_IO_DEFAULT,
        PULL_TYPE_IO_DEFAULT, PULL_TYPE_IO_1V8_ONLY, PULL_TYPE_IO_1V8_ONLY),
    PIN_BANK_DRV_FLAGS(3, 32, "gpio3", DRV_TYPE_IO_3V3_ONLY, DRV_TYPE_IO_3V3_ONLY, DRV_TYPE_IO_3V3_ONLY, DRV_TYPE_IO_1V8_OR_3V0),
    PIN_BANK_DRV_FLAGS(4, 32, "gpio4", DRV_TYPE_IO_1V8_OR_3V0, DRV_TYPE_IO_1V8_3V0_AUTO, DRV_TYPE_IO_1V8_OR_3V0, DRV_TYPE_IO_1V8_OR_3V0),
};
...
static const struct of_device_id rockchip_pinctrl_device_tree_match[] = {
    ...
    {.compatible = "rockchip,rk3128-pinctrl", .data = &rk3128_pin_ctrl},
    ...
    {.compatible = "rockchip,rk3366-pinctrl", .data = &rk3366_pin_ctrl},
...
static struct platform_driver rockchip_pinctrl_driver = {
    ...
    .driver = {
        ...
        .of_match_table = rockchip_pinctrl_device_tree_match,
    },
};
#else
static vmm_driver_t rockchip_pinctrl_driver = {
    .probe       = rockchip_pinctrl_probe,
    .name        = "rockchip-pinctrl",
    .match_table = rockchip_pinctrl_device_tree_match,
};
#endif
...
#else
    return vmm_device_driver_register_driver(&rockchip_pinctrl_driver);
#endif
}
#if 0
