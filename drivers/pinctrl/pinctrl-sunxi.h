...
static inline uint32_t sunxi_dlevel_reg(uint16_t pin)
{
    uint8_t  bank   = pin / PINS_PER_BANK;
    uint32_t offset = bank * BANK_MEM_SIZE;
...
static inline uint32_t sunxi_dlevel_offset(uint16_t pin)
{
    uint32_t pin_num = pin % DLEVEL_PINS_PER_REG;
    return pin_num * DLEVEL_PINS_BITS;
}

static inline uint32_t sunxi_pull_reg(uint16_t pin)
{
    uint8_t  bank   = pin / PINS_PER_BANK;
    uint32_t offset = bank * BANK_MEM_SIZE;
...
static inline uint32_t sunxi_pull_offset(uint16_t pin)
{
    uint32_t pin_num = pin % PULL_PINS_PER_REG;
    return pin_num * PULL_PINS_BITS;
}

static inline uint32_t sunxi_irq_cfg_reg(uint16_t irq)
{
    uint8_t reg = irq / IRQ_CFG_IRQ_PER_REG;
...
static inline uint32_t sunxi_irq_ctrl_reg(uint16_t irq)
{
    uint8_t reg = irq / IRQ_CTRL_IRQ_PER_REG;
...
static inline uint32_t sunxi_irq_status_reg(uint16_t irq)
{
    uint8_t reg = irq / IRQ_STATUS_IRQ_PER_REG;
