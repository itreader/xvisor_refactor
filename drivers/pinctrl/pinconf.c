...
int pinconf_check_ops(pinctrl_device_t *pctldev)
{
    ...
    if (!ops->pin_config_set && !ops->pin_config_group_set) {
        dev_err(pctldev->dev, "pinconf has to be able to set a pins config\n");
        return -EINVAL;
    }
...
int pinconf_validate_map(const struct pinctrl_map *map, int i)
{
    ...
    if (!map->data.configs.num_configs || !map->data.configs.configs) {
...
int pinconf_map_to_setting(const struct pinctrl_map *map, struct pinctrl_setting *setting)
{
    ...
    switch (setting->type) {
        case PIN_MAP_TYPE_CONFIGS_PIN:
            ...
            if (pin < 0) {
                dev_err(pctldev->dev, "could not map pin config for \"%s\"", map->data.configs.group_or_pin);
                return pin;
            }
        ...
        case PIN_MAP_TYPE_CONFIGS_GROUP:
            ...
            if (pin < 0) {
                dev_err(pctldev->dev, "could not map group config for \"%s\"", map->data.configs.group_or_pin);
                return pin;
            }
...
int pinconf_apply_setting(const struct pinctrl_setting *setting)
{
    ...
    switch (setting->type) {
        case PIN_MAP_TYPE_CONFIGS_PIN:
            ...
            ret = ops->pin_config_set(pctldev, setting->data.configs.group_or_pin, setting->data.configs.configs, setting->data.configs.num_configs);
        ...
        case PIN_MAP_TYPE_CONFIGS_GROUP:
            ...
            ret = ops->pin_config_group_set(
                pctldev, setting->data.configs.group_or_pin, setting->data.configs.configs, setting->data.configs.num_configs);
...
static void pinconf_show_config(struct seq_file *s, pinctrl_device_t *pctldev, uint64_t *configs, unsigned num_configs)
{
    ...
    for (i = 0; i < num_configs; i++) {
        ...
        if (confops && confops->pin_config_config_dbg_show) {
            confops->pin_config_config_dbg_show(pctldev, s, configs[i]);
        } else {
...
void pinconf_show_setting(struct seq_file *s, const struct pinctrl_setting *setting)
{
    ...
    switch (setting->type) {
        case PIN_MAP_TYPE_CONFIGS_PIN:
            desc = pin_desc_get(setting->pctldev, setting->data.configs.group_or_pin);
            seq_printf(s, "pin %s (%d)", desc->name, setting->data.configs.group_or_pin);
            break;

        case PIN_MAP_TYPE_CONFIGS_GROUP:
            seq_printf(s, "group %s (%d)", pctlops->get_group_name(pctldev, setting->data.configs.group_or_pin), setting->data.configs.group_or_pin);
            break;
...
static void pinconf_dump_pin(pinctrl_device_t *pctldev, struct seq_file *s, int pin)
{
    const struct pinconf_ops *ops = pctldev->desc->confops;
...
static void pinconf_dump_group(pinctrl_device_t *pctldev, struct seq_file *s, unsigned selector, const char *gname)
{
    const struct pinconf_ops *ops = pctldev->desc->confops;
...
static int pinconf_dbg_config_print(struct seq_file *s, void *d)
{
    ...
    if (!found) {
        ...
        seq_printf(
            s, "Use: modify config_pin <devname> "
               "<state> <pinname> <value>\n");
        goto exit;
    }
    ...
    pinconf_show_config(s, pctldev, found->data.configs.configs, found->data.configs.num_configs);
 ...
 */
static ssize_t pinconf_dbg_config_write(struct file *file, const char __user *user_buf, size_t count, loff_t *ppos)
{
    struct pinctrl_maps              *maps_node;
    const struct pinctrl_map         *map;
    const struct pinctrl_map         *found = NULL;
    ...
    if (confops && confops->pin_config_dbg_parse_modify) {
        ...
        for (i = 0; i < configs->num_configs; i++) {
            confops->pin_config_dbg_parse_modify(pctldev, config, &configs->configs[i]);
        }
    }
...
{
    debugfs_create_file("pinconf-pins", S_IFREG | S_IRUGO, devroot, pctldev, &pinconf_pins_ops);
    debugfs_create_file("pinconf-groups", S_IFREG | S_IRUGO, devroot, pctldev, &pinconf_groups_ops);
