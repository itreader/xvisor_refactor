...
int pci_for_each_dma_alias(struct pci_dev *pdev, int (*fn)(struct pci_dev *pdev, uint16_t alias, void *data), void *data)
{
     ...
     */
    if (unlikely(pdev->dev_flags & PCI_DEVICE_FLAGS_DMA_ALIAS_DEVFN)) {
        ret = fn(pdev, PCI_DEVID(pdev->bus->number, pdev->dma_alias_devfn), data);
        ...
        if (pci_is_pcie(tmp)) {
            switch (pci_pcie_type(tmp)) {
                ...
                case PCI_EXP_TYPE_PCI_BRIDGE:
                    ret = fn(tmp, PCI_DEVID(tmp->subordinate->number, PCI_DEVFN(0, 0)), data);
                ...
                case PCI_EXP_TYPE_PCIE_BRIDGE:
                    ret = fn(tmp, PCI_DEVID(tmp->bus->number, tmp->devfn), data);
            ...
            if (tmp->dev_flags & PCI_DEVICE_FLAG_PCIE_BRIDGE_ALIAS) {
                ret = fn(tmp, PCI_DEVID(tmp->subordinate->number, PCI_DEVFN(0, 0)), data);
            } else {
                ret = fn(tmp, PCI_DEVID(tmp->bus->number, tmp->devfn), data);
            }
...
static struct pci_dev *pci_get_dev_by_id(const struct pci_device_id *id, struct pci_dev *from)
{
    ...
    dev = bus_find_device(&pci_bus_type, dev_start, (void *)id, match_pci_dev_by_id);
