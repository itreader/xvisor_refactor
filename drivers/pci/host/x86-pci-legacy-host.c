...
static int x86_legacy_probe(vmm_device_t *dev)
{
    pci_subsys_init();
...
static struct vmm_device_tree_nodeid x86_legacy_pci_controller_devid_table[] = {
    {.type = "pci", .compatible = "x86-legacy"},
    {/* end of list */},
};

static vmm_driver_t x86_pci_legacy_driver = {
...
static int __init x86_legacy_init(void)
{
    ...
    return vmm_device_driver_register_driver(&x86_pci_legacy_driver);
}

static void __exit x86_legacy_exit(void)
{
    vmm_device_driver_unregister_driver(&x86_pci_legacy_driver);
}
