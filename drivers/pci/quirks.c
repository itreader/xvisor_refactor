...
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_AMD, PCI_DEVICE_ID_AMD_NL_USB, quirk_amd_nl_class);
...
static void piix4_io_quirk(struct pci_dev *dev, const char *name, uint32_t port, uint32_t enable)
{
    uint32_t devres;
    uint32_t mask, size, base;
...
static void piix4_mem_quirk(struct pci_dev *dev, const char *name, uint32_t port, uint32_t enable)
{
    uint32_t devres;
    uint32_t mask, size, base;
...
static void quirk_ich4_lpc_acpi(struct pci_dev *dev)
{
    ...
    if (enable & ICH4_ACPI_EN) {
        quirk_io_region(dev, ICH_PMBASE, 128, PCI_BRIDGE_RESOURCES, "ICH4 ACPI/GPIO/TCO");
    }

    pci_read_config_byte(dev, ICH4_GPIO_CNTL, &enable);

    if (enable & ICH4_GPIO_EN) {
        quirk_io_region(dev, ICH4_GPIOBASE, 64, PCI_BRIDGE_RESOURCES + 1, "ICH4 GPIO");
    }
}
...
static void ich6_lpc_acpi_gpio(struct pci_dev *dev)
{
    ...
    if (enable & ICH6_ACPI_EN) {
        quirk_io_region(dev, ICH_PMBASE, 128, PCI_BRIDGE_RESOURCES, "ICH6 ACPI/GPIO/TCO");
    }

    pci_read_config_byte(dev, ICH6_GPIO_CNTL, &enable);

    if (enable & ICH6_GPIO_EN) {
        quirk_io_region(dev, ICH6_GPIOBASE, 64, PCI_BRIDGE_RESOURCES + 1, "ICH6 GPIO");
    }
}
...
static void quirk_vt82c586_acpi(struct pci_dev *dev)
{
    if (dev->revision & 0x10) {
        quirk_io_region(dev, 0x48, 256, PCI_BRIDGE_RESOURCES, "vt82c586 ACPI");
    }
}
...
static void quirk_vt82c686_acpi(struct pci_dev *dev)
{
    ...
    quirk_io_region(dev, 0x70, 128, PCI_BRIDGE_RESOURCES + 1, "vt82c686 HW-mon");
...
static void quirk_via_vlink(struct pci_dev *dev)
{
    ...
    /* Internal device ? */
    if (dev->bus->number != 0 || PCI_SLOT(dev->devfn) > via_vlink_dev_hi || PCI_SLOT(dev->devfn) < via_vlink_dev_lo) {
        return;
    }
...
{
    pdev->dev_flags |= PCI_DEVICE_FLAGS_NO_D3;
}
...
static void quirk_disable_broadcom_boot_interrupt(struct pci_dev *dev)
{
    ...
    pci_write_config_dword(dev, BC_HT1000_FEATURE_REG, pci_config_dword | BC_HT1000_PIC_REGS_ENABLE);
...
static void quirk_disable_amd_813x_boot_interrupt(struct pci_dev *dev)
{
    ...
    if ((dev->revision == AMD_813X_REV_B1) || (dev->revision == AMD_813X_REV_B2)) {
        return;
    }
...
static void quirk_plx_pci9050(struct pci_dev *dev)
{
    ...
    for (bar = 0; bar <= 1; bar++) {
        if (pci_resource_len(dev, bar) == 0x80 && (pci_resource_start(dev, bar) & 0x80)) {
...
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_PLX, PCI_DEVICE_ID_PLX_9050, quirk_plx_pci9050);
/*
...
{
    uint32_t num_parallel = (dev->subsystem_device & 0xf0) >> 4;
    uint32_t num_serial   = dev->subsystem_device & 0xf;
    ...
    switch (dev->device) {
        case PCI_DEVICE_ID_NETMOS_9835:
            ...
            if (dev->subsystem_vendor == PCI_VENDOR_ID_IBM && dev->subsystem_device == 0x0299) {
                return;
            }
        ...
        case PCI_DEVICE_ID_NETMOS_9855:
            if (num_parallel) {
                dev_info(
                    ...
                    num_parallel, num_serial);
                dev->class = (PCI_CLASS_COMMUNICATION_OTHER << 8) | (dev->class & 0xff);
            }
    }
}
...
static void quirk_via_cx700_pci_parking_caching(struct pci_dev *dev)
{
    ...
    struct pci_dev *p = pci_get_device(PCI_VENDOR_ID_VIA, PCI_DEVICE_ID_VIA_8235_USB_2, NULL);
    uint8_t         b;
...
static void quirk_brcm_570x_limit_vpd(struct pci_dev *dev)
{
     ...
     */
    if ((dev->device == PCI_DEVICE_ID_NX2_5706) || (dev->device == PCI_DEVICE_ID_NX2_5706S) || (dev->device == PCI_DEVICE_ID_NX2_5708) ||
        (dev->device == PCI_DEVICE_ID_NX2_5708S) || ((dev->device == PCI_DEVICE_ID_NX2_5709) && (dev->revision & 0xf0) == 0x0)) {
        if (dev->vpd) {
            dev->vpd->len = 0x80;
        }
    }
...
static int msi_ht_cap_enabled(struct pci_dev *dev)
{
    ...
    while (pos && ttl--) {
        ...
        if (pci_read_config_byte(dev, pos + HT_MSI_FLAGS, &flags) == 0) {
            dev_info(&dev->dev, "Found %s HT MSI Mapping\n", flags & HT_MSI_FLAGS_ENABLE ? "enabled" : "disabled");
...
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_SERVERWORKS, PCI_DEVICE_ID_SERVERWORKS_HT1000_PXB, ht_enable_msi_mapping);

DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_AMD, PCI_DEVICE_ID_AMD_8132_BRIDGE, ht_enable_msi_mapping);
...
static void nvenet_msi_disable(struct pci_dev *dev)
{
    ...
    if (board_name && (strstr(board_name, "P5N32-SLI PREMIUM") || strstr(board_name, "P5N32-E SLI"))) {
        dev_info(&dev->dev, "Disabling msi for MCP55 NIC on P5N32-SLI\n");
        dev->no_msi = 1;
    }
}

DECLARE_PCI_FIXUP_EARLY(PCI_VENDOR_ID_NVIDIA, PCI_DEVICE_ID_NVIDIA_NVENET_15, nvenet_msi_disable);
...
DECLARE_PCI_FIXUP_EARLY(PCI_VENDOR_ID_NVIDIA, PCI_DEVICE_ID_NVIDIA_MCP55_BRIDGE_V0, nvbridge_check_legacy_irq_routing);

DECLARE_PCI_FIXUP_EARLY(PCI_VENDOR_ID_NVIDIA, PCI_DEVICE_ID_NVIDIA_MCP55_BRIDGE_V4, nvbridge_check_legacy_irq_routing);
...
static int is_end_of_ht_chain(struct pci_dev *dev)
{
    ...
    ctrl_off = ((flags >> 10) & 1) ? PCI_HT_CAP_SLAVE_CTRL0 : PCI_HT_CAP_SLAVE_CTRL1;
...
static void nv_ht_enable_msi_mapping(struct pci_dev *dev)
{
    ...
    if (host_bridge == dev && is_end_of_ht_chain(host_bridge) && host_bridge_with_leaf(host_bridge)) {
        goto out;
    }
...
{
    dev->dev_flags |= PCI_DEVICE_FLAGS_MSI_INTX_DISABLE_BUG;
}
...
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_BROADCOM, PCI_DEVICE_ID_TIGON3_5780, quirk_msi_intx_disable_bug);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_BROADCOM, PCI_DEVICE_ID_TIGON3_5780S, quirk_msi_intx_disable_bug);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_BROADCOM, PCI_DEVICE_ID_TIGON3_5714, quirk_msi_intx_disable_bug);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_BROADCOM, PCI_DEVICE_ID_TIGON3_5714S, quirk_msi_intx_disable_bug);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_BROADCOM, PCI_DEVICE_ID_TIGON3_5715, quirk_msi_intx_disable_bug);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_BROADCOM, PCI_DEVICE_ID_TIGON3_5715S, quirk_msi_intx_disable_bug);
...
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_ATTANSIC, 0x1062, quirk_msi_intx_disable_bug);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_ATTANSIC, 0x1063, quirk_msi_intx_disable_bug);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_ATTANSIC, 0x2060, quirk_msi_intx_disable_bug);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_ATTANSIC, 0x2062, quirk_msi_intx_disable_bug);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_ATTANSIC, 0x1073, quirk_msi_intx_disable_bug);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_ATTANSIC, 0x1083, quirk_msi_intx_disable_bug);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_ATTANSIC, 0x1090, quirk_msi_intx_disable_qca_bug);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_ATTANSIC, 0x1091, quirk_msi_intx_disable_qca_bug);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_ATTANSIC, 0x10a0, quirk_msi_intx_disable_qca_bug);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_ATTANSIC, 0x10a1, quirk_msi_intx_disable_qca_bug);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_ATTANSIC, 0xe091, quirk_msi_intx_disable_qca_bug);
...
static void ricoh_mmc_fixup_r5c832(struct pci_dev *dev)
{
     ...
     */
    if (dev->device == PCI_DEVICE_ID_RICOH_R5CE822 || dev->device == PCI_DEVICE_ID_RICOH_R5CE823) {
...
static void quirk_intel_ntb(struct pci_dev *dev)
{
    ...
    dev->resource[2].end = dev->resource[2].start + ((uint64_t)1 << val) - 1;
    ...
    dev->resource[4].end = dev->resource[4].start + ((uint64_t)1 << val) - 1;
}

DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_INTEL, 0x0e08, quirk_intel_ntb);
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_INTEL, 0x0e0d, quirk_intel_ntb);

static ktime_t fixup_debug_start(struct pci_dev *dev, void (*fn)(struct pci_dev *dev))
{
...
static void fixup_debug_report(struct pci_dev *dev, ktime_t calltime, void (*fn)(struct pci_dev *dev))
{
    ...
    if (initcall_debug) {
        ...
        duration = (uint64_t)ktime_to_ns(delta) >> 10;
...
DECLARE_PCI_FIXUP_HEADER(
    0x1814, 0x0601, /* Ralink RT2800 802.11n PCI */
 ...
 */
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_REALTEK, 0x8169, quirk_broken_intx_masking);
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_MELLANOX, PCI_ANY_ID, quirk_broken_intx_masking);
...
static void quirk_apple_poweroff_thunderbolt(struct pci_dev *dev)
{
     ...
     */
    if (ACPI_FAILURE(acpi_get_handle(bridge, "DSB0.NHI0.SXIO", &SXIO)) || ACPI_FAILURE(acpi_get_handle(bridge, "DSB0.NHI0.SXFP", &SXFP)) ||
...
DECLARE_PCI_FIXUP_SUSPEND_LATE(PCI_VENDOR_ID_INTEL, 0x1547, quirk_apple_poweroff_thunderbolt);
...
static void quirk_apple_wait_for_thunderbolt(struct pci_dev *dev)
{
    ...
    if (nhi->vendor != PCI_VENDOR_ID_INTEL || (nhi->device != 0x1547 && nhi->device != 0x156c) || nhi->subsystem_vendor != 0x2222 ||
...
DECLARE_PCI_FIXUP_RESUME_EARLY(PCI_VENDOR_ID_INTEL, 0x1547, quirk_apple_wait_for_thunderbolt);
DECLARE_PCI_FIXUP_RESUME_EARLY(PCI_VENDOR_ID_INTEL, 0x156d, quirk_apple_wait_for_thunderbolt);
#endif
...
static void pci_do_fixups(struct pci_dev *dev, struct pci_fixup *f, struct pci_fixup *end)
{
    ...
    for (; f < end; f++) {
        if ((f->class == (uint32_t)(dev->class >> f->class_shift) || f->class == (uint32_t)PCI_ANY_ID) &&
            (f->vendor == dev->vendor || f->vendor == (uint16_t)PCI_ANY_ID) && (f->device == dev->device || f->device == (uint16_t)PCI_ANY_ID)) {
...
static int reset_chelsio_generic_dev(struct pci_dev *dev, int probe)
{
    ...
    pci_write_config_word(dev, PCI_COMMAND, old_command | PCI_COMMAND_MASTER);
    ...
    if ((msix_flags & PCI_MSIX_FLAGS_ENABLE) == 0) {
        pci_write_config_word(dev, dev->msix_cap + PCI_MSIX_FLAGS, msix_flags | PCI_MSIX_FLAGS_ENABLE | PCI_MSIX_FLAGS_MASKALL);
    }
...
static const struct pci_dev_reset_methods pci_dev_reset_methods[] = {
    {PCI_VENDOR_ID_INTEL, PCI_DEVICE_ID_INTEL_82599_SFP_VF, reset_intel_82599_sfp_virtfn},
...
int pci_dev_specific_reset(struct pci_dev *dev, int probe)
{
    ...
    for (i = pci_dev_reset_methods; i->reset; i++) {
        if ((i->vendor == dev->vendor || i->vendor == (uint16_t)PCI_ANY_ID) && (i->device == dev->device || i->device == (uint16_t)PCI_ANY_ID)) {
...
static void quirk_dma_func0_alias(struct pci_dev *dev)
{
    if (PCI_FUNC(dev->devfn) != 0) {
        ...
        dev->dev_flags |= PCI_DEVICE_FLAGS_DMA_ALIAS_DEVFN;
    }
}
...
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_MARVELL_EXT, 0x9130, quirk_dma_func1_alias);
/* https://bugzilla.kernel.org/show_bug.cgi?id=42679#c47 + c57 */
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_MARVELL_EXT, 0x9172, quirk_dma_func1_alias);
/* https://bugzilla.kernel.org/show_bug.cgi?id=42679#c59 */
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_MARVELL_EXT, 0x917a, quirk_dma_func1_alias);
/* https://bugzilla.kernel.org/show_bug.cgi?id=42679#c46 */
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_MARVELL_EXT, 0x91a0, quirk_dma_func1_alias);
/* https://bugzilla.kernel.org/show_bug.cgi?id=42679#c49 */
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_MARVELL_EXT, 0x9230, quirk_dma_func1_alias);
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_TTI, 0x0642, quirk_dma_func1_alias);
/* https://bugs.gentoo.org/show_bug.cgi?id=497630 */
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_JMICRON, PCI_DEVICE_ID_JMICRON_JMB388_ESD, quirk_dma_func1_alias);
...
{
    if (!pci_is_root_bus(pdev->bus) && pdev->hdr_type == PCI_HEADER_TYPE_BRIDGE && !pci_is_pcie(pdev) && pci_is_pcie(pdev->bus->self) &&
        pci_pcie_type(pdev->bus->self) != PCI_EXP_TYPE_PCI_BRIDGE) {
        pdev->dev_flags |= PCI_DEVICE_FLAG_PCIE_BRIDGE_ALIAS;
    }
}

/* ASM1083/1085, https://bugzilla.kernel.org/show_bug.cgi?id=44881#c46 */
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_ASMEDIA, 0x1080, quirk_use_pcie_bridge_dma_alias);
 ...
 */
static int pci_quirk_amd_sb_acs(struct pci_dev *dev, uint16_t acs_flags)
{
#ifdef CONFIG_ACPI
 ...
 */
static const uint16_t pci_quirk_intel_pch_acs_ids[] = {
    /* Ibexpeak PCH */
    0x3b42,
    0x3b43,
    0x3b44,
...
static int pci_quirk_intel_pch_acs(struct pci_dev *dev, uint16_t acs_flags)
{
    uint16_t flags = dev->dev_flags & PCI_DEVICE_FLAGS_ACS_ENABLED_QUIRK ? INTEL_PCH_ACS_FLAGS : 0;
...
static int pci_quirk_mf_endpoint_acs(struct pci_dev *dev, uint16_t acs_flags)
{
    /*
     ...
     */
    acs_flags &= ~(PCI_ACS_SV | PCI_ACS_TB | PCI_ACS_RR | PCI_ACS_CR | PCI_ACS_UF | PCI_ACS_DT);
...
static const struct pci_dev_acs_enabled {
    uint16_t vendor;
    uint16_t device;
    int (*acs_enabled)(struct pci_dev *dev, uint16_t acs_flags);
} pci_dev_acs_enabled[] = {
...
static int pci_quirk_enable_intel_lpc_acs(struct pci_dev *dev)
{
    ...
    rcba_mem = ioremap_nocache(rcba & INTEL_LPC_RCBA_MASK, PAGE_ALIGN(INTEL_UPDCR_REG));
...
static int pci_quirk_enable_intel_pch_acs(struct pci_dev *dev)
{
    ...
    dev->dev_flags |= PCI_DEVICE_FLAGS_ACS_ENABLED_QUIRK;
