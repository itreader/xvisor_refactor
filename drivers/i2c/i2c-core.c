...
static int i2c_device_probe(struct device *dev)
{
    ...
    status = of_clock_set_defaults(dev->of_node, false);
...
static int i2c_device_remove(vmm_device_t *dev)
{
    struct i2c_client *client = i2c_verify_client(dev);
    struct i2c_driver *driver;
    int                status = 0;
...
static void i2c_dev_set_name(struct i2c_adapter *adap, struct i2c_client *client)
{
 ...
 * i2c_unregister_device(); or NULL to indicate an error.
 */
struct i2c_client *i2c_new_dummy(struct i2c_adapter *adapter, uint16_t address)
{
 ...
 * doing anyway.
 */
static inline uint32_t i2c_adapter_depth(struct i2c_adapter *adapter)
{
    uint32_t depth = 0;
...
static int i2c_do_add_adapter(struct i2c_driver *driver, struct i2c_adapter *adap)
{
    ...
    if (driver->attach_adapter) {
        ...
        dev_warn(
            &adap->dev, "Please use another way to instantiate "
                        "your i2c_client\n");
...
void i2c_clients_command(struct i2c_adapter *adap, uint32_t cmd, void *arg)
{
 ...
 * @count: How many bytes to write, must be less than 64k since msg.len is uint16_t
 *
 ...
 * @count: How many bytes to read, must be less than 64k since msg.len is uint16_t
 *
...
static uint8_t i2c_smbus_msg_pec(uint8_t pec, struct i2c_msg *msg)
{
...
static int i2c_smbus_check_pec(uint8_t cpec, struct i2c_msg *msg)
{
...
int32_t i2c_smbus_read_byte(const struct i2c_client *client)
{
    ...
    status = i2c_smbus_xfer(client->adapter, client->addr, client->flags, I2C_SMBUS_READ, 0, I2C_SMBUS_BYTE, &data);
...
int32_t i2c_smbus_read_byte_data(const struct i2c_client *client, uint8_t command)
{
    ...
    status = i2c_smbus_xfer(client->adapter, client->addr, client->flags, I2C_SMBUS_READ, command, I2C_SMBUS_BYTE_DATA, &data);
    return (status < 0) ? status : data.byte;
}
...
int32_t i2c_smbus_read_block_data(const struct i2c_client *client, uint8_t command, uint8_t *values)
{
    ...
    status = i2c_smbus_xfer(client->adapter, client->addr, client->flags, I2C_SMBUS_READ, command, I2C_SMBUS_BLOCK_DATA, &data);
...
static int32_t i2c_smbus_xfer_emulated(
 ...
 */
int32_t i2c_smbus_xfer(
    struct i2c_adapter *adapter, uint16_t addr, unsigned short flags, char read_write, uint8_t command, int protocol, union i2c_smbus_data *data)
{
    uint64_t orig_jiffies;
    ...
    trace_smbus_read(adapter, addr, flags, read_write, command, protocol);
...
VMM_DECLARE_MODULE("I2C-Bus main module", "Jimmy Durand Wesolowski", "GPL", I2C_IPRIORITY, i2c_init, i2c_exit);
