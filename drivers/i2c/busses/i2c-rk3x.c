 ...
 */
struct i2c_spec_values {
    uint64_t min_hold_start_ns;
    uint64_t min_low_ns;
    uint64_t min_high_ns;
    uint64_t min_setup_start_ns;
    uint64_t max_data_hold_ns;
    uint64_t min_data_setup_ns;
    uint64_t min_setup_stop_ns;
    uint64_t min_hold_buffer_ns;
};
...
struct rk3x_i2c_soc_data {
    int grf_offset;
    int (*calc_timings)(uint64_t, struct i2c_timings *, struct rk3x_i2c_calced_timings *);
};
 ...
 * @clock_rate_nb: i2c clk rate change notify
...
struct rk3x_i2c {
    ...
    struct clk           *clk;
    struct clk           *pclk;
    struct notifier_block clock_rate_nb;
...
static inline void i2c_writel(struct rk3x_i2c *i2c, uint32_t value, uint32_t offset)
{
    writel(value, i2c->regs + offset);
}

static inline uint32_t i2c_readl(struct rk3x_i2c *i2c, uint32_t offset)
{
...
{
    uint32_t val = i2c_readl(i2c, REG_CON) & REG_CON_TUNING_MASK;
...
{
    uint32_t len = i2c->msg->len - i2c->processed;
    uint32_t con;
...
static void rk3x_i2c_handle_start(struct rk3x_i2c *i2c, uint32_t ipd)
{
...
static void rk3x_i2c_handle_write(struct rk3x_i2c *i2c, uint32_t ipd)
{
    if (!(ipd & REG_INT_MBTF)) {
        rk3x_i2c_stop(i2c, -EIO);
...
static void rk3x_i2c_handle_read(struct rk3x_i2c *i2c, uint32_t ipd)
{
    uint32_t i;
    uint32_t len = i2c->msg->len - i2c->processed;
#if 0
...
static void rk3x_i2c_handle_stop(struct rk3x_i2c *i2c, uint32_t ipd)
{
    uint32_t con;
 ...
 */
static const struct i2c_spec_values *rk3x_i2c_get_spec(uint32_t speed)
{
    if (speed <= 100000) {
 ...
 *
 * @clock_rate: I2C input clock rate
 ...
 */
static int rk3x_i2c_v0_calc_timings(uint64_t clock_rate, struct i2c_timings *t, struct rk3x_i2c_calced_timings *t_calc)
{
    uint64_t min_low_ns, min_high_ns;
    uint64_t max_low_ns, min_total_ns;

    uint64_t clock_rate_khz, scl_rate_khz;

    uint64_t min_low_div, min_high_div;
    uint64_t max_low_div;

    uint64_t min_div_for_hold, min_total_div;
    uint64_t extra_div, extra_low_div, ideal_low_div;

    uint64_t                      data_hold_buffer_ns = 50;
     ...
     */
    min_high_ns      = max(min_high_ns, DIV_ROUND_UP((t->scl_rise_ns + spec->min_setup_start_ns) * 1000, 875));
    min_high_ns      = max(min_high_ns, DIV_ROUND_UP((t->scl_rise_ns + spec->min_setup_start_ns + t->sda_fall_ns + spec->min_high_ns), 2));
    ...
    clock_rate_khz   = DIV_ROUND_UP(clock_rate, 1000);
     ...
     */
    min_total_div    = DIV_ROUND_UP(clock_rate_khz, scl_rate_khz * 8);

    /* These are the min dividers needed for min hold times. */
    min_low_div      = DIV_ROUND_UP(clock_rate_khz * min_low_ns, 8 * 1000000);
    min_high_div     = DIV_ROUND_UP(clock_rate_khz * min_high_ns, 8 * 1000000);
     ...
     */
    max_low_div      = clock_rate_khz * max_low_ns / (8 * 1000000);
    ...
    if (min_div_for_hold > min_total_div) {
         ...
         */
        ideal_low_div = DIV_ROUND_UP(clock_rate_khz * min_low_ns, scl_rate_khz * 8 * min_total_ns);
 ...
 * tSU;sto = (8h * p + 1) * T;
 */
static int rk3x_i2c_v1_calc_timings(uint64_t clock_rate, struct i2c_timings *t, struct rk3x_i2c_calced_timings *t_calc)
{
    uint64_t min_low_ns, min_high_ns;
    ...
    for (sda_update_cfg = 3; sda_update_cfg > 0; sda_update_cfg--) {
        max_hold_data_ns  = DIV_ROUND_UP((sda_update_cfg * (t_calc->div_low) + 1) * 1000000, clock_rate_khz);
        min_setup_data_ns = DIV_ROUND_UP(((8 - sda_update_cfg) * (t_calc->div_low) + 1) * 1000000, clock_rate_khz);
    ...
    stp_sta_cfg        = DIV_ROUND_UP(clock_rate_khz * min_setup_start_ns - 1000000, 8 * 1000000 * (t_calc->div_high));

    /* calculate setup stop config */
    min_setup_stop_ns  = t->scl_rise_ns + spec->min_setup_stop_ns;
    stp_sto_cfg        = DIV_ROUND_UP(clock_rate_khz * min_setup_stop_ns - 1000000, 8 * 1000000 * (t_calc->div_high));

    t_calc->tuning     = REG_CON_SDA_CFG(--sda_update_cfg) | REG_CON_STA_CFG(--stp_sta_cfg) | REG_CON_STO_CFG(--stp_sto_cfg);
...
static void rk3x_i2c_adapt_div(struct rk3x_i2c *i2c, uint64_t clock_rate)
{
    struct i2c_timings            *t = &i2c->t;
    struct rk3x_i2c_calced_timings calc;
#if defined(DEV_DEBUG)
    uint64_t t_low_ns, t_high_ns;
#endif
    uint64_t flags;
    uint32_t val;
    int      ret;

    ret = i2c->soc_data->calc_timings(clock_rate, t, &calc);
    WARN_ONCE(ret != 0, "Could not reach SCL freq %u", t->bus_freq_hz);

    clock_enable(i2c->pclk);

    spin_lock_irq_save(&i2c->lock, flags);
    val = i2c_readl(i2c, REG_CON);
    val &= ~REG_CON_TUNING_MASK;
    val |= calc.tuning;
    i2c_writel(i2c, val, REG_CON);
    i2c_writel(i2c, (calc.div_high << 16) | (calc.div_low & 0xffff), REG_CLKDIV);
    spin_unlock_irq_restore(&i2c->lock, flags);

    clock_disable(i2c->pclk);

#if defined(DEV_DEBUG)
    t_low_ns  = div_u64(((uint64_t)calc.div_low + 1) * 8 * 1000000000, clock_rate);
    t_high_ns = div_u64(((uint64_t)calc.div_high + 1) * 8 * 1000000000, clock_rate);
    dev_dbg(i2c->dev, "CLK %lukhz, Req %uns, Act low %lluns high %lluns\n", clock_rate / 1000, 1000000000 / t->bus_freq_hz, t_low_ns, t_high_ns);
#endif
}

/**
 * rk3x_i2c_clock_notifier_cb - Clock rate change callback
 ...
 */
static int rk3x_i2c_clock_notifier_cb(struct notifier_block *nb, uint64_t event, void *data)
{
    struct clock_notifier_data    *ndata = data;
    struct rk3x_i2c               *i2c   = container_of(nb, struct rk3x_i2c, clock_rate_nb);
    struct rk3x_i2c_calced_timings calc;
...
static int rk3x_i2c_setup(struct rk3x_i2c *i2c, struct i2c_msg *msgs, int num)
{
    ...
    if (num >= 2 && msgs[0].len < 4 && !(msgs[0].flags & I2C_M_RD) && (msgs[1].flags & I2C_M_RD)) {
        uint32_t reg_addr = 0;
        int      i;
...
static int rk3x_i2c_restart_notify(struct notifier_block *this, uint64_t mode, void *cmd)
{
...
static __maybe_unused int rk3x_i2c_resume_noirq(struct device *dev)
{
    ...
    rk3x_i2c_adapt_div(i2c, clock_get_rate(i2c->clk));
...
static uint32_t rk3x_i2c_func(struct i2c_adapter *adap)
{
...
static const struct of_device_id rk3x_i2c_match[] = {
    {.compatible = "rockchip,rv1108-i2c", .data = (void *)&rv1108_soc_data},
    ...
    {.compatible = "rockchip,rk3328-i2c", .data = (void *)&rk3328_soc_data},
    {.compatible = "rockchip,rk3399-i2c", .data = (void *)&rk3399_soc_data},
    {},
};
MODULE_DEVICE_TABLE(of, rk3x_i2c_match);

static int rk3x_i2c_probe(vmm_device_t *dev)
{
...
#else
    ret = vmm_device_tree_request_regmap(dev->of_node, &mem, 0, "RK3X I2C");
    ...
    if (i2c->soc_data->grf_offset >= 0) {
        ...
        if (IS_ERR(grf)) {
            dev_err(dev, "rk3x-i2c needs 'rockchip,grf' property\n");
            return PTR_ERR(grf);
        }
    ...
    ret = devm_request_irq(dev, irq, rk3x_i2c_irq,
                           0, dev_name(dev), i2c);
#else
    irq = vmm_device_tree_irq_parse_map(dev->of_node, 0);

    if (!irq) {
        dev_err(dev, "No IRQ resource\n");
        vmm_device_tree_regunmap_release(dev->of_node, (virtual_addr_t)i2c->regs, 0);
        return -ENODEV;
    }

    ret = request_irq(irq, rk3x_i2c_irq, IRQF_SHARED, dev_name(dev), i2c);
#endif
    ...
    if (i2c->soc_data->calc_timings == rk3x_i2c_v0_calc_timings) {
        ...
        i2c->clk  = devm_clock_get(dev, NULL);
        i2c->pclk = i2c->clk;
    } else {
        i2c->clk  = devm_clock_get(dev, "i2c");
        i2c->pclk = devm_clock_get(dev, "pclk");
    }
    ...
    i2c->clock_rate_nb.notifier_call = rk3x_i2c_clock_notifier_cb;
    ret                              = clock_notifier_register(i2c->clk, &i2c->clock_rate_nb);
    ...
    clock_rate = clock_get_rate(i2c->clk);
    rk3x_i2c_adapt_div(i2c, clock_rate);

    ret = i2c_add_adapter(&i2c->adap);

    if (ret < 0) {
        dev_err(dev, "Could not register adapter\n");
        goto err_clock_notifier;
    }
...
err_clock_notifier:
    clock_notifier_unregister(i2c->clk, &i2c->clock_rate_nb);
err_pclk:
    clock_unprepare(i2c->pclk);
err_clock:
    clock_unprepare(i2c->clk);
    return ret;
}

static int rk3x_i2c_remove(vmm_device_t *dev)
{
...
#else
static vmm_driver_t rk3x_i2c_driver = {
    .name        = "rk3c-i2c",
    .probe       = rk3x_i2c_probe,
    .remove      = rk3x_i2c_remove,
...
{
    return vmm_device_driver_register_driver(&rk3x_i2c_driver);
}

static void __exit rk3x_i2c_driver_exit(void)
{
    vmm_device_driver_unregister_driver(&rk3x_i2c_driver);
}

VMM_DECLARE_MODULE("Rockchip RK3xxx I2C Bus driver", "Max Schwarz", "GPLv2", (I2C_IPRIORITY + 1), rk3x_i2c_driver_init, rk3x_i2c_driver_exit);
#endif
