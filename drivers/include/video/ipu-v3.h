...
typedef struct {
    ...
    unsigned int_clock       : 1;
    unsigned interlaced      : 1;
    unsigned odd_field_first : 1;
    unsigned clksel_en       : 1;
    unsigned clkidle_en      : 1;
    unsigned data_pol        : 1; /* true = inverted */
    unsigned clock_pol       : 1; /* true = rising edge */
...
typedef struct {
    ...
    unsigned clock_mode     : 3;
    unsigned ext_vsync      : 1;
    unsigned Vsync_pol      : 1;
    unsigned Hsync_pol      : 1;
    unsigned pixclock_pol   : 1;
    unsigned data_pol       : 1;
    unsigned sens_clock_src : 1;
...
int32_t ipu_init_channel_buffer(
    ...
    ipu_rotate_mode_t rot_mode, dma_addr_t phyaddr_0, dma_addr_t phyaddr_1, dma_addr_t phyaddr_2, uint32_t u_offset, uint32_t v_offset);
...
struct stripe_param {
    uint32_t input_width;   /* width of the input stripe */
    uint32_t output_width;  /* width of the output stripe */
    uint32_t input_column;  /* the first column on the input stripe */
    uint32_t output_column; /* the first column on the output stripe */
    uint32_t idr;
...
int ipu_calc_stripes_sizes(
    const uint32_t input_frame_width, uint32_t output_frame_width, const uint32_t maximal_stripe_width, const uint64_t cirr,
    const uint32_t equal_stripes, uint32_t input_pixelformat, uint32_t output_pixelformat, struct stripe_param *left, struct stripe_param *right);
...
int32_t ipu_disp_set_color_key(struct ipu_soc *ipu, ipu_channel_t channel, bool enable, uint32_t colorKey);
...
int32_t ipu_csi_init_interface(struct ipu_soc *ipu, uint16_t width, uint16_t height, uint32_t pixel_fmt, ipu_csi_signal_cfg_t sig);
...
static inline int32_t ipu_csi_enable_mclock_if(struct ipu_soc *ipu, int src, uint32_t csi, bool flag, bool wait)
{
    return ipu_csi_enable_mclk(ipu, csi, flag, wait);
}

int ipu_csi_read_mclock_flag(void);
...
void ipu_csi_window_size_crop(
    struct ipu_soc *ipu, uint32_t swidth, uint32_t sheight, uint32_t width, uint32_t height, uint32_t left, uint32_t top, uint32_t csi);
