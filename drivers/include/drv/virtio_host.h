 ...
 */
int virtio_host_queue_add_iovecs(struct virtio_host_queue *vq, struct virtio_host_iovec *ivs[], uint32_t out_ivs, uint32_t in_ivs, void *data);
 ...
 */
void *virtio_host_queue_get_buf(struct virtio_host_queue *vq, uint32_t *len);
...
uint64_t virtio_host_queue_get_desc_addr(struct virtio_host_queue *vq);
...
struct virtio_host_queue *virtio_host_create_queue(
    ...
    virtio_host_queue_callback_t callback, const char *name);
...
struct virtio_host_config_ops {
    void (*get)(struct virtio_host_device *vdev, unsigned offset, void *buf, unsigned len);
    void (*set)(struct virtio_host_device *vdev, unsigned offset, const void *buf, unsigned len);
    uint32_t (*generation)(struct virtio_host_device *vdev);
    uint8_t (*get_status)(struct virtio_host_device *vdev);
    void (*set_status)(struct virtio_host_device *vdev, uint8_t status);
    void (*reset)(struct virtio_host_device *vdev);
    int (*find_vqs)(
        struct virtio_host_device *vdev, unsigned nvqs, struct virtio_host_queue **vqs, virtio_host_queue_callback_t *callbacks, char **names);
    void (*del_vqs)(struct virtio_host_device *vdev);
    uint64_t (*get_features)(struct virtio_host_device *vdev);
...
#define to_virtio_host_device(d) container_of(d, struct virtio_host_device, dev)
...
struct virtio_host_driver {
    ...
    const uint32_t                     *feature_table_legacy;
    uint32_t                            feature_table_size_legacy;
...
#define to_virtio_host_driver(d) container_of(d, struct virtio_host_driver, drv)
 ...
 */
static inline bool __virtio_host_test_bit(const struct virtio_host_device *vdev, uint32_t fbit)
{
 ...
 * @fbit: the feature bit
 */
static inline void __virtio_host_set_bit(struct virtio_host_device *vdev, uint32_t fbit)
{
    /* Did you forget to fix assumptions on max features? */
    BUG_ON(fbit >= 64);
 ...
 * @fbit: the feature bit
 */
static inline void __virtio_host_clear_bit(struct virtio_host_device *vdev, uint32_t fbit)
{
    /* Did you forget to fix assumptions on max features? */
    BUG_ON(fbit >= 64);
 ...
 * @fbit: the feature bit
 */
static inline bool virtio_host_has_feature(const struct virtio_host_device *vdev, uint32_t fbit)
{
 ...
 * @vdev: the device
 */
static inline bool virtio_host_has_iommu_quirk(const struct virtio_host_device *vdev)
{
    /*
 ...
 */
static inline void virtio_host_device_ready(struct virtio_host_device *vdev)
{
 ...
 */
static inline void virtio_host_device_reset(struct virtio_host_device *vdev)
{
    vdev->config->reset(vdev);
}

static inline const char *virtio_host_bus_name(struct virtio_host_device *vdev)
{
 ...
 *
 */
static inline int virtio_host_queue_set_affinity(struct virtio_host_queue *vq, int cpu)
{
...
static inline uint16_t __virtio16_to_cpu(bool little_endian, uint16_t val)
{
    if (little_endian) {
...
static inline uint16_t __cpu_to_virtio16(bool little_endian, uint16_t val)
{
    if (little_endian) {
...
static inline uint32_t __virtio32_to_cpu(bool little_endian, uint32_t val)
{
    if (little_endian) {
...
static inline uint32_t __cpu_to_virtio32(bool little_endian, uint32_t val)
{
    if (little_endian) {
...
static inline uint64_t __virtio64_to_cpu(bool little_endian, uint64_t val)
{
    if (little_endian) {
...
static inline uint64_t __cpu_to_virtio64(bool little_endian, uint64_t val)
{
    if (little_endian) {
...
static inline bool virtio_host_is_little_endian(struct virtio_host_device *vdev)
{
    return virtio_host_has_feature(vdev, VMM_VIRTIO_F_VERSION_1) || virtio_host_legacy_is_little_endian();
}

static inline uint16_t virtio16_to_cpu(struct virtio_host_device *vdev, uint16_t val)
{
    return __virtio16_to_cpu(virtio_host_is_little_endian(vdev), val);
}

static inline uint16_t cpu_to_virtio16(struct virtio_host_device *vdev, uint16_t val)
{
    return __cpu_to_virtio16(virtio_host_is_little_endian(vdev), val);
}

static inline uint32_t virtio32_to_cpu(struct virtio_host_device *vdev, uint32_t val)
{
    return __virtio32_to_cpu(virtio_host_is_little_endian(vdev), val);
}

static inline uint32_t cpu_to_virtio32(struct virtio_host_device *vdev, uint32_t val)
{
    return __cpu_to_virtio32(virtio_host_is_little_endian(vdev), val);
}

static inline uint64_t virtio64_to_cpu(struct virtio_host_device *vdev, uint64_t val)
{
    return __virtio64_to_cpu(virtio_host_is_little_endian(vdev), val);
}

static inline uint64_t cpu_to_virtio64(struct virtio_host_device *vdev, uint64_t val)
{
...
static inline void __virtio_cread_many(struct virtio_host_device *vdev, uint32_t offset, void *buf, size_t count, size_t bytes)
{
    uint32_t old, gen = vdev->config->generation ? vdev->config->generation(vdev) : 0;
    int      i;
...
static inline void virtio_cread_bytes(struct virtio_host_device *vdev, uint32_t offset, void *buf, size_t len)
{
    __virtio_cread_many(vdev, offset, buf, len, 1);
}

static inline uint8_t virtio_cread8(struct virtio_host_device *vdev, uint32_t offset)
{
    uint8_t ret;
...
static inline void virtio_cwrite8(struct virtio_host_device *vdev, uint32_t offset, uint8_t val)
{
    vdev->config->set(vdev, offset, &val, sizeof(val));
}

static inline uint16_t virtio_cread16(struct virtio_host_device *vdev, uint32_t offset)
{
    uint16_t ret;
    vdev->config->get(vdev, offset, &ret, sizeof(ret));
...
static inline void virtio_cwrite16(struct virtio_host_device *vdev, uint32_t offset, uint16_t val)
{
    val = cpu_to_virtio16(vdev, val);
    vdev->config->set(vdev, offset, &val, sizeof(val));
}

static inline uint32_t virtio_cread32(struct virtio_host_device *vdev, uint32_t offset)
{
    uint32_t ret;
    vdev->config->get(vdev, offset, &ret, sizeof(ret));
...
static inline void virtio_cwrite32(struct virtio_host_device *vdev, uint32_t offset, uint32_t val)
{
    val = cpu_to_virtio32(vdev, val);
    vdev->config->set(vdev, offset, &val, sizeof(val));
}

static inline uint64_t virtio_cread64(struct virtio_host_device *vdev, uint32_t offset)
{
    uint64_t ret;
...
static inline void virtio_cwrite64(struct virtio_host_device *vdev, uint32_t offset, uint64_t val)
{
    val = cpu_to_virtio64(vdev, val);
    vdev->config->set(vdev, offset, &val, sizeof(val));
}

static inline int virtio_host_find_vqs(
    struct virtio_host_device *vdev, unsigned nvqs, struct virtio_host_queue **vqs, virtio_host_queue_callback_t *cbs, char **names)
{
...
int virtio_host_add_device(struct virtio_host_device *vdev, const struct virtio_host_config_ops *config, vmm_device_t *parent);
