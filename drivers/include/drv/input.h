...
#define INPUT_DEVICE_CLASS_IPRIORITY INPUT_IPRIORITY
...
#define INPUT_PROPERTIES_POINTER   0x00 /* needs a pointer */
#define INPUT_PROPERTIES_DIRECT    0x01 /* direct input devices */
#define INPUT_PROPERTIES_BUTTONPAD 0x02 /* has button(s) under pad */
#define INPUT_PROPERTIES_SEMI_MT   0x03 /* touch rectangle only */

#define INPUT_PROPERTIES_MAX       0x1f
#define INPUT_PROPERTIES_CNT       (INPUT_PROPERTIES_MAX + 1)
...
/**
 * input_device_t - represents an input device
...
struct input_device {
    ...
    uint64_t propbit[BITS_TO_LONGS(INPUT_PROPERTIES_CNT)];

    uint64_t evbit[BITS_TO_LONGS(EV_CNT)];
    uint64_t keybit[BITS_TO_LONGS(KEY_CNT)];
    uint64_t relbit[BITS_TO_LONGS(REL_CNT)];
    uint64_t absbit[BITS_TO_LONGS(ABS_CNT)];
    uint64_t mscbit[BITS_TO_LONGS(MSC_CNT)];
    uint64_t ledbit[BITS_TO_LONGS(LED_CNT)];
    uint64_t sndbit[BITS_TO_LONGS(SND_CNT)];
    uint64_t ffbit[BITS_TO_LONGS(FF_CNT)];
    uint64_t swbit[BITS_TO_LONGS(SW_CNT)];

    uint32_t hint_events_per_packet;

    uint32_t keycodemax;
    uint32_t keycodesize;
    void    *keycode;

    int (*setkeycode)(input_device_t *dev, const struct input_keymap_entry *ke, uint32_t *old_keycode);
    int (*getkeycode)(input_device_t *dev, struct input_keymap_entry *ke);

    uint32_t          repeat_key;
    vmm_timer_event_t repeat_ev;
    ...
    uint64_t key[BITS_TO_LONGS(KEY_CNT)];
    uint64_t led[BITS_TO_LONGS(LED_CNT)];
    uint64_t snd[BITS_TO_LONGS(SND_CNT)];
    uint64_t sw[BITS_TO_LONGS(SW_CNT)];

    int (*open)(input_device_t *dev);
    void (*close)(input_device_t *dev);
    int (*flush)(input_device_t *dev);
    int (*event)(input_device_t *dev, uint32_t type, uint32_t code, int value);
...
struct input_handler {
    ...
    bool          connected;
    double_list_t conn_head[EV_CNT];

    const char *name;

    uint64_t evbit[BITS_TO_LONGS(EV_CNT)];

    int (*event)(struct input_handler *ihnd, input_device_t *dev, uint32_t type, uint32_t code, int value);

    void *private;
};

static inline void *input_get_drvdata(input_device_t *dev)
{
    return dev->private;
}

static inline void input_set_drvdata(input_device_t *dev, void *data)
{
    dev->private = data;
}
 ...
 *
 * Returns prepared input_device_t or NULL.
 */
...
static inline void input_report_key(input_device_t *dev, uint32_t code, int value)
{
    input_event(dev, EV_KEY, code, !!value);
}

/** Report EV_REL from input device */
static inline void input_report_rel(input_device_t *dev, uint32_t code, int value)
{
    input_event(dev, EV_REL, code, value);
}

/** Report EV_ABS from input device */
static inline void input_report_abs(input_device_t *dev, uint32_t code, int value)
{
    input_event(dev, EV_ABS, code, value);
}

/** Report EV_FF_STATUS from input device */
static inline void input_report_ff_status(input_device_t *dev, uint32_t code, int value)
{
    input_event(dev, EV_FF_STATUS, code, value);
}

/** Report EV_SW from input device */
static inline void input_report_switch(input_device_t *dev, uint32_t code, int value)
{
    input_event(dev, EV_SW, code, !!value);
}

/** Report SYNC event */
static inline void input_sync(input_device_t *dev)
{
    input_event(dev, EV_SYN, SYN_REPORT, 0);
}

/** Report MultiTouch SYNC event */
static inline void input_mt_sync(input_device_t *dev)
{
 ...
 */
static inline void input_set_events_per_packet(input_device_t *dev, int n_events)
{
 ...
 * methods expect scancodes to be represented as 'uint32_t'.
 */
