...
#define interface_set_data(_i, _p) vmm_device_driver_set_data(&(_i)->dev, (_p))
#define interface_get_data(_i)     vmm_device_driver_get_data(&(_i)->dev)
...
struct usb_device {
    ...
    uint8_t               devnum; /* Device number on USB bus */
    uint16_t              bus_mA;
    ...
    /* Device Descriptor */
    struct usb_device_descriptor descriptor __attribute__((aligned(USB_DMA_MINALIGN)));
...
struct usb_device_id {
    ...
    /* not matched against */
    uint64_t driver_info;
};

/* Some useful macros to use to create struct usb_device_id */
#define USB_DEVICE_ID_MATCH_VENDOR             0x0001
#define USB_DEVICE_ID_MATCH_PRODUCT            0x0002
#define USB_DEVICE_ID_MATCH_DEVICE_LO          0x0004
#define USB_DEVICE_ID_MATCH_DEVICE_HI          0x0008
#define USB_DEVICE_ID_MATCH_DEVICE_CLASS       0x0010
#define USB_DEVICE_ID_MATCH_DEVICE_SUBCLASS    0x0020
#define USB_DEVICE_ID_MATCH_DEVICE_PROTOCOL    0x0040
...
#define USB_DEVICE_ID_MATCH_DEVICE_RANGE       (USB_DEVICE_ID_MATCH_DEVICE_LO | USB_DEVICE_ID_MATCH_DEVICE_HI)
#define USB_DEVICE_ID_MATCH_DEVICE_AND_VERSION (USB_DEVICE_ID_MATCH_DEVICE | USB_DEVICE_ID_MATCH_DEVICE_RANGE)
#define USB_DEVICE_ID_MATCH_DEVICE_INFO        (USB_DEVICE_ID_MATCH_DEVICE_CLASS | USB_DEVICE_ID_MATCH_DEVICE_SUBCLASS | USB_DEVICE_ID_MATCH_DEVICE_PROTOCOL)
...
struct usb_driver {
    ...
    uint32_t no_dynamic_id             : 1;
    uint32_t supports_autosuspend      : 1;
    uint32_t disable_hub_initiated_lpm : 1;
    uint32_t soft_unbind               : 1;
};
 ...
 * Note: This can be called from any context.
 */
bool usb_match_one_id(struct usb_interface *intf, const struct usb_device_id *id);
 ...
 * Note: This can be called from any context.
 */
const struct usb_device_id *usb_match_interface(struct usb_interface *intf, struct usb_driver *drv);
 ...
 * Note: This can be called from any context.
 */
int usb_add_dynid(struct usb_driver *driver, uint32_t idVendor, uint32_t idProduct, uint32_t bInterfaceClass);
 ...
 * Note: This can be called from any context.
 */
int usb_del_dynid(struct usb_driver *driver, uint32_t idVendor, uint32_t idProduct);
 ...
 * Note: This should be called from Thread (or Orphan) context.
 */
int usb_pre_reset_driver(struct usb_interface *intf, struct usb_driver *drv);
 ...
 * Note: This should be called from Thread (or Orphan) context.
 */
int usb_post_reset_driver(struct usb_interface *intf, struct usb_driver *drv);
...
static inline void usb_fill_control_urb(
    struct urb *urb, struct usb_device *dev, uint32_t pipe, uint8_t *setup_packet, void *transfer_buffer, int buffer_length,
...
static inline void usb_fill_bulk_urb(
    struct urb *urb, struct usb_device *dev, uint32_t pipe, void *transfer_buffer, int buffer_length, usb_complete_t complete_fn, void *context)
{
    urb->dev                    = dev;
    urb->pipe                   = pipe;
    urb->transfer_buffer        = transfer_buffer;
...
#define usb_create_pipe(dev, endpoint)     (((dev)->devnum << 8) | ((endpoint) << 15) | (dev)->maxpacketsize)
#define usb_default_pipe(dev)              ((dev)->speed << 26)
#define usb_sndctrlpipe(dev, endpoint)     ((USB_PIPE_CONTROL << 30) | usb_create_pipe(dev, endpoint))
#define usb_rcvctrlpipe(dev, endpoint)     ((USB_PIPE_CONTROL << 30) | usb_create_pipe(dev, endpoint) | USB_DIR_IN)
#define usb_sndisocpipe(dev, endpoint)     ((USB_PIPE_ISOCHRONOUS << 30) | usb_create_pipe(dev, endpoint))
#define usb_rcvisocpipe(dev, endpoint)     ((USB_PIPE_ISOCHRONOUS << 30) | usb_create_pipe(dev, endpoint) | USB_DIR_IN)
#define usb_sndbulkpipe(dev, endpoint)     ((USB_PIPE_BULK << 30) | usb_create_pipe(dev, endpoint))
#define usb_rcvbulkpipe(dev, endpoint)     ((USB_PIPE_BULK << 30) | usb_create_pipe(dev, endpoint) | USB_DIR_IN)
#define usb_sndintpipe(dev, endpoint)      ((USB_PIPE_INTERRUPT << 30) | usb_create_pipe(dev, endpoint))
#define usb_rcvintpipe(dev, endpoint)      ((USB_PIPE_INTERRUPT << 30) | usb_create_pipe(dev, endpoint) | USB_DIR_IN)
#define usb_snddefctrl(dev)                ((USB_PIPE_CONTROL << 30) | usb_default_pipe(dev))
#define usb_rcvdefctrl(dev)                ((USB_PIPE_CONTROL << 30) | usb_default_pipe(dev) | USB_DIR_IN)
...
int usb_control_msg(
    struct usb_device *dev, uint32_t pipe, uint8_t request, uint8_t requesttype, uint16_t value, uint16_t index, void *data, uint16_t size,
 ...
 */
void usb_register_notify(vmm_notifier_block_t *nb);
