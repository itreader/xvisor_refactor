...
struct usb_hcd {
     ...
     * housekeeping
     */
    double_list_t head;          /* hcd is-a bus */
    ...
    uint64_t       devicemap[USB_MAX_DEVICE / (8 * sizeof(uint64_t))];
...
#define HCD_DEAD(hcd)           ((hcd)->flags & (1U << HCD_FLAG_DEAD))
    ...
    uint32_t rh_registered : 1; /* is root hub registered? */
    uint32_t rh_pollable   : 1; /* may we poll the root hub? */
    uint32_t msix_enabled  : 1; /* driver has MSI-X enabled? */

    /* The next flag is a stopgap, to be removed when all the HCDs
     * support the new root-hub polling mechanism. */
    uint32_t uses_new_polling   : 1;
    uint32_t wireless           : 1;     /* Wireless USB HCD */
    uint32_t authorized_default : 1;
    uint32_t has_tt             : 1;     /* Integrated TT in root hub */

    uint32_t        irq;                 /* irq allocated */
    void           *regs;                /* device memory/io */
    physical_addr_t resource_addr_start; /* memory/io resource start */
    physical_size_t resource_addr_len;   /* memory/io resource length */
...
#define HC_IS_SUSPENDED(state) ((state) & __SUSPEND)
     ...
     * this structure.
     */
    uint64_t hcd_priv[0] __attribute__((aligned(sizeof(int64_t))));
};
 ...
 */
static inline void *usb_hcd_private(struct usb_hcd *hcd)
{
 ...
 * (and is done using urb->hcprivate).  It also released all HCD locks;
 ...
 */
int usb_add_hcd(struct usb_hcd *hcd, uint32_t irqnum, uint64_t irqflags);
