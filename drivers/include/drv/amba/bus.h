...
static inline uint32_t amba_periphid(vmm_device_t *dev)
{
    uint32_t       pid = 0x0, val;
    ...
    if (vmm_device_tree_read_u32(dev->of_node, AMBA_PERIPHID_ATTR_NAME, &pid)) {
        if (vmm_device_tree_regmap(dev->of_node, &dev_base, 0)) {
            return 0;
        }
        ...
        pid |= (val & 0xFF) << 24;
        vmm_device_tree_regunmap(dev->of_node, dev_base, 0);
        vmm_device_tree_setattr(dev->of_node, AMBA_PERIPHID_ATTR_NAME, &pid, VMM_DEVICE_TREE_ATTRTYPE_UINT32, sizeof(pid), FALSE);
    }
