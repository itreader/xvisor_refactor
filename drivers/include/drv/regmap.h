...
struct regmap_access_table {
    ...
    uint32_t                   n_yes_ranges;
    const struct regmap_range *no_ranges;
    uint32_t                   n_no_ranges;
};
...
typedef int (*regmap_hw_reg_read)(void *context, uint32_t reg, uint32_t *val);
typedef int (*regmap_hw_reg_write)(void *context, uint32_t reg, uint32_t val);
typedef int (*regmap_hw_reg_update_bits)(void *context, uint32_t reg, uint32_t mask, uint32_t val);
 ...
 */
#define regmap_init(dev, bus, bus_context, config) __regmap_init(dev, bus, bus_context, config)
int regmap_attach_dev(vmm_device_t *dev, struct regmap *map, const struct regmap_config *config);

/**
 * regmap_init_mmio_clock() - Initialise register map with register clock
 *
 * @dev: Device that will be interacted with
 * @clock_id: register clock consumer ID
 ...
 */
#define devm_regmap_init(dev, bus, bus_context, config)          __devm_regmap_init(dev, bus, bus_context, config)

/**
 * devm_regmap_init_mmio_clock() - Initialise managed register map with clock
 *
 * @dev: Device that will be interacted with
 * @clock_id: register clock consumer ID
 * @regs: Pointer to memory-mapped IO region
 ...
 */
#define devm_regmap_init_mmio_clock(dev, clock_id, regs, config) __devm_regmap_init_mmio_clock(dev, clock_id, regs, config)
 ...
 * device management code.
 */
#define devm_regmap_init_mmio(dev, regs, config)                 devm_regmap_init_mmio_clock(dev, NULL, regs, config)
...
static inline bool regmap_reg_in_range(uint32_t reg, const struct regmap_range *range)
{
