...
#define SDHCI_GET_VENDOR(host)               (((host)->sdhci_version & SDHCI_VENDOR_VER_MASK) >> SDHCI_VENDOR_VER_SHIFT)
#define SDHCI_GET_VERSION(host)              ((host)->sdhci_version & SDHCI_SPEC_VER_MASK)
...
struct sdhci_ops {
    ...
    uint32_t (*read_l)(struct sdhci_host *host, int reg);
    uint16_t (*read_w)(struct sdhci_host *host, int reg);
    uint8_t (*read_b)(struct sdhci_host *host, int reg);
    void (*write_l)(struct sdhci_host *host, uint32_t val, int reg);
    void (*write_w)(struct sdhci_host *host, uint16_t val, int reg);
    void (*write_b)(struct sdhci_host *host, uint8_t val, int reg);
#endif
    void (*set_ios_post)(struct sdhci_host *host);
    void (*set_control_reg)(struct sdhci_host *host);
    void (*set_clock)(struct sdhci_host *host, uint32_t div);
    void (*set_delay)(struct sdhci_host *host);
    uint32_t (*get_wp)(struct sdhci_host *host);
};

struct sdhci_host {
    const char      *hw_name;   /* controller name */
    struct mmc_host *mmc;       /* underlying mmc_host instance */
    vmm_device_t    *dev;       /* underlying device instance */
    void            *ioaddr;    /* pointer to registers */
    int              irq;       /* less than zero means no interrupt */
    uint32_t         quirks;    /* quirks or hacks */
    uint32_t         quirks2;   /* quirks or hacks */
    uint32_t         mmc_caps;  /* forced mmc_host capablities */
    uint32_t         clock;     /* input clock */
    uint32_t         clock_mul;
    uint32_t         max_clock; /* max output clock */
    uint32_t         min_clock; /* min output clock */
    uint32_t         voltages;  /* forced mmc_host voltages */
    struct sdhci_ops ops;       /* controller operations */

    uint32_t sdhci_version;
    uint32_t sdhci_caps;
    uint32_t sdhci_caps_1;

    /* struct mmc_request *mrq; /\* associated request *\/ */
    struct mmc_cmd *cmd;             /* Current command */

    void            *aligned_buffer; /* Used when DMA address has to be 8-byte aligned */
    vmm_completion_t wait_command;
    vmm_completion_t wait_dma;
...
static inline void *sdhci_private(struct sdhci_host *host)
{
