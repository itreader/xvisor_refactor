...
#define request_resource_conflict(root, new)              vmm_request_resource_conflict(root, new)
#define request_resource(root, new)                       vmm_request_resource(root, new)
#define release_resource(new)                             vmm_release_resource(new)
#define release_child_resources(new)                      vmm_release_child_resources(new)
#define reserve_region_with_split(root, start, end, name) vmm_reserve_region_with_split(root, start, end, name)
...
#define resource_alignment(res)                         vmm_resource_alignment(res)
#define resource_size(res)                              vmm_resource_size(res)
#define resource_type(res)                              vmm_resource_type(res)
#define resource_contains(r1, r2)                       vmm_resource_contains(r1, r2)

/* Convenience shorthand with allocation */
#define request_region(start, n, name)                  vmm_request_region(start, n, name)
#define request_muxed_region(start, n, name)            vmm_request_muxed_region(start, n, name)
#define __request_mem_region(start, n, name, excl)      __vmm_request_mem_region(start, n, name, excl)
#define request_mem_region(start, n, name)              vmm_request_mem_region(start, n, name)
#define request_mem_region_exclusive(start, n, name)    vmm_request_mem_region_exclusive(start, n, name)
#define rename_region(region, newname)                  vmm_rename_region(region, newname)

#define __request_region(parent, start, n, name, flags) __vmm_request_region(parent, start, n, name, flags)

/* Compatibility cruft */
#define release_region(start, n)                        vmm_release_region(start, n)
#define check_mem_region(start, n)                      vmm_check_mem_region(start, n)
#define release_mem_region(start, n)                    vmm_release_mem_region(start, n)

#define __check_region(parent, start, size)             __vmm_check_region(parent, start, size)
#define __release_region(parent, start, size)           __vmm_release_region(parent, start, size)

#ifdef CONFIG_MEMORY_HOTREMOVE
#define release_mem_region_adjustable(parent, start, size) vmm_release_mem_region_adjustable(parent, start, size)
#endif

#define devm_request_resource(dev, root, new)                 vmm_devm_request_resource(dev, root, new)
#define devm_release_resource(dev, new)                       vmm_devm_release_resource(dev, new)

#define devm_request_region(dev, start, n, name)              vmm_devm_request_region(dev, start, n, name)
#define devm_request_mem_region(dev, start, n, name)          vmm_devm_request_mem_region(dev, start, n, name)

#define __devm_request_region(dev, parent, start, n, name)    __vmm_devm_request_region(dev, parent, start, n, name)

#define devm_release_region(dev, start, n)                    vmm_devm_release_region(dev, start, n)
#define devm_release_mem_region(dev, start, n)                vmm_devm_release_mem_region(dev, start, n)

#define __devm_release_region(dev, parent, start, n)          __vmm_devm_release_region(dev, parent, start, n)
#define iomem_map_sanity_check(addr, size)                    vmm_hostmem_map_sanity_check(addr, size)
#define iomem_is_exclusive(addr)                              vmm_hostmem_is_exclusive(addr)

#define walk_system_ram_range(start_pfn, nr_pages, arg, func) vmm_walk_system_ram_range(start_pfn, nr_pages, arg, func)
#define walk_system_ram_res(start, end, arg, func)            vmm_walk_system_ram_res(start, end, arg, func)
#define walk_iomem_res(name, flags, start, end, arg, func)    vmm_walk_hostmem_res(name, flags, start, end, arg, func)

#define resource_overlaps(r1, r2)                             vmm_resource_overlaps(r1, r2)
