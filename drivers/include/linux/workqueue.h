...
#define create_singlethread_workqueue(name) vmm_workqueue_create(name, VMM_THREAD_DEF_PRIORITY)
#define destroy_workqueue(wait_queue)       vmm_workqueue_destroy(wait_queue);
#define flush_workqueue(wait_queue)         vmm_workqueue_flush(wait_queue)
#define flush_scheduled_work(wk)

static inline int schedule_delayed_work(struct delayed_work *work, uint64_t delay)
{
...
static inline int queue_delayed_work(struct workqueue_struct *wait_queue, struct delayed_work *work, uint64_t delay)
{
    uint64_t nsecs = delay;
    nsecs          = nsecs * (1000000000 / HZ);
    return vmm_workqueue_schedule_delayed_work(wait_queue, work, nsecs);
}
