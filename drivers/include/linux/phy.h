...
struct mii_bus {
    ...
    int (*write)(struct mii_bus *bus, int phy_id, int regnum, uint16_t val);
...
int                mdiobus_read(struct mii_bus *bus, int addr, uint32_t regnum);
int                mdiobus_write(struct mii_bus *bus, int addr, uint32_t regnum, uint16_t val);
...
struct phy_driver {
     ...
     * the PHY framework.
     */
    void (*write_mmd_indirect)(struct phy_device *dev, int ptrad, int devnum, int regnum, uint32_t val);
 ...
 */
static inline int phy_read(struct phy_device *phydev, uint32_t regnum)
{
 ...
 * to conclude the operation.
 */
static inline int phy_write(struct phy_device *phydev, uint32_t regnum, uint16_t val)
{
...
static inline int phy_mii_ioctl(struct phy_device *phydev, struct ifreq *ifr, int cmd)
{
    return 0;
}
...
int phy_register_fixup(const char *bus_id, uint32_t phy_uid, uint32_t phy_uid_mask, int (*run)(struct phy_device *));
int phy_register_fixup_for_id(const char *bus_id, int (*run)(struct phy_device *));
int phy_register_fixup_for_uid(uint32_t phy_uid, uint32_t phy_uid_mask, int (*run)(struct phy_device *));
