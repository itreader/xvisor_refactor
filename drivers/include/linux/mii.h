...
struct mii_if_info {
    ...
    uint32_t full_duplex   : 1; /* is full duplex? */
    uint32_t force_media   : 1; /* is autoneg. disabled? */
    uint32_t supports_gmii : 1; /* are GMII registers supported? */
...
extern uint32_t mii_check_media(struct mii_if_info *mii, uint32_t ok_to_print, uint32_t init_media);
extern int      generic_mii_ioctl(struct mii_if_info *mii_if, struct mii_ioctl_data *mii_data, int cmd, uint32_t *duplex_changed);
 ...
 */
static inline uint32_t mii_nway_result(uint32_t negotiated)
{
    uint32_t ret;
 ...
 */
static inline uint32_t mii_duplex(uint32_t duplex_lock, uint32_t negotiated)
{
    if (duplex_lock) {
        return 1;
    }
