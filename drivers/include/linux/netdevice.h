...
struct net_device {
    ...
    void                   *nsw_private; /* VMM virtual packet switching layer
                                          * specific private data.
                                          */
    void                   *net_private; /* VMM specific private data -
...
static inline void netdev_set_private(struct net_device *ndev, void *private)
{
    if (ndev && private) {
        ndev->private = private;
    }
}

static inline void *netdev_private(struct net_device *ndev)
{
    if (ndev) {
        return ndev->private;
    }
...
{
    struct vmm_netport *port = dev->nsw_private;
...
static inline struct netdev_queue *netdev_get_tx_queue(struct net_device *dev, uint32_t index)
{
    return &dev->_tx;
}
...
struct net_device *alloc_netdev_mqs(
    int sizeof_private, const char *name, unsigned char name_assign_type, void (*setup)(struct net_device *), uint32_t txqs, uint32_t rxqs);
...
#define netdev_err(ndev, msg...)  netdev_msg(VMM_LOGLEVEL_ERROR, ndev, msg)
