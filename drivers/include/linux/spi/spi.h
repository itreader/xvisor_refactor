...
extern struct spi_master *spi_busnum_to_master(uint16_t busnum);
 ...
 */
static inline void spi_message_init_with_transfers(struct spi_message *m, struct spi_transfer *xfers, uint32_t num_xfers)
{
    uint32_t i;
...
static inline struct spi_message *spi_message_alloc(unsigned ntrans, gfp_t flags)
{
    ...
    m = kzalloc(sizeof(struct spi_message) + ntrans * sizeof(struct spi_transfer), flags);
 ...
 */
static inline int spi_sync_transfer(struct spi_device *spi, struct spi_transfer *xfers, uint32_t num_xfers)
{
...
static inline ssize_t spi_w8r16(struct spi_device *spi, uint8_t cmd)
{
    ...
    status = spi_write_then_read(spi, &cmd, 1, (uint8_t *)&result, 2);
 ...
 *
 */
static inline ssize_t spi_w8r16be(struct spi_device *spi, uint8_t cmd)
 ...
 */
extern struct spi_device *spi_alloc_device(struct spi_master *master);
...
static inline void spi_unregister_device(struct spi_device *spi)
{
    if (spi) {
