...
static inline void acpi_preset_companion(struct device *dev,
        struct acpi_device *parent, uint64_t addr)
{
...
void __acpi_unmap_table(char *map, uint64_t size);
...
int acpi_table_parse(char *id, acpi_table_table_handler handler);
int __init acpi_table_parse_entries(char *id, uint64_t table_size,
                                    int entry_id,
...
extern int sbf_port;
extern uint64_t acpi_realmode_flags;

int acpi_register_gsi (struct device *dev, uint32_t gsi, int triggering, int polarity);
int acpi_gsi_to_irq (uint32_t gsi, uint32_t *irq);
int acpi_isa_irq_to_gsi (unsigned isa_irq, uint32_t *gsi);
...
typedef void (*wmi_notify_handler) (uint32_t value, void *context);

extern acpi_status wmi_evaluate_method(const char *guid, uint8_t instance,
                                       uint32_t method_id,
                                       const struct acpi_buffer *in,
                                       struct acpi_buffer *out);
extern acpi_status wmi_query_block(const char *guid, uint8_t instance,
                                   struct acpi_buffer *out);
extern acpi_status wmi_set_block(const char *guid, uint8_t instance,
                                 const struct acpi_buffer *in);
extern acpi_status wmi_install_notify_handler(const char *guid,
        wmi_notify_handler handler, void *data);
extern acpi_status wmi_remove_notify_handler(const char *guid);
extern acpi_status wmi_get_event_data(uint32_t event, struct acpi_buffer *out);
...
bool acpi_dev_resource_ext_address_space(struct acpi_resource *ares,
        struct resource *res);
uint64_t acpi_dev_irq_flags(uint8_t triggering, uint8_t polarity, uint8_t shareable);
...
void acpi_dev_free_resource_list(list_head_t *list);
int acpi_dev_get_resources(struct acpi_device *adev, list_head_t *list,
...
acpi_status acpi_str_to_uuid(char *str, uint8_t *uuid);
...
#if defined(CONFIG_ACPI_HOTPLUG_CPU) && defined(CONFIG_ACPI_HOTPLUG_MEMORY) && defined(CONFIG_ACPI_CONTAINER)
...
extern int acpi_nvs_register(uint64_t start, uint64_t size);

extern int acpi_nvs_for_each_region(int (*func)(uint64_t, uint64_t, void *),
                                    void *data);
...
static inline int acpi_nvs_register(uint64_t start, uint64_t size)
{
    return 0;
}

static inline int acpi_nvs_for_each_region(int (*func)(uint64_t, uint64_t, void *), void *data)
{
    return 0;
}
...
#if 0  /* FIXME: CONFIG_ACPI */
void acpi_os_set_prepare_sleep(int (*func)(uint8_t sleep_state,
                               uint32_t pm1a_ctrl,  uint32_t pm1b_ctrl));

acpi_status acpi_os_prepare_sleep(uint8_t sleep_state,
                                  uint32_t pm1a_control, uint32_t pm1b_control);

void acpi_os_set_prepare_extended_sleep(int (*func)(uint8_t sleep_state,
                                        uint32_t val_a,  uint32_t val_b));

acpi_status acpi_os_prepare_extended_sleep(uint8_t sleep_state,
        uint32_t val_a, uint32_t val_b);
...
__printf(3, 4) void __acpi_handle_debug(struct _ddebug *descriptor, acpi_handle handle, const char *fmt, ...);
#else
#define __acpi_handle_debug(descriptor, handle, fmt, ...) acpi_handle_printk(KERN_DEBUG, handle, fmt, ##__VA_ARGS__);
#endif
