...
static inline struct sk_buff *__alloc_skb(uint32_t size, uint8_t dummy_priority, int ext_how)
{
    struct vmm_mbuf *m;
    MGETHDR(m, 0, 0);
...
static inline struct sk_buff *alloc_skb(uint32_t size, uint8_t dummy_priority)
{
...
static inline unsigned char *__skb_put(struct sk_buff *skb, uint32_t len)
{
 ...
 */
static inline unsigned char *skb_put(struct sk_buff *skb, uint32_t len)
{
 ...
 */
static inline unsigned char *skb_push(struct sk_buff *skb, uint32_t len)
{
    skb_data(skb) -= len;
    skb_len(skb) += len;
    skb->m_pktlen += len;
 ...
 * allocation flags.
 */
static inline struct sk_buff *__dev_alloc_skb(uint32_t length, int how)
{
    struct sk_buff *skb;
 ...
 */
static inline struct sk_buff *dev_alloc_skb(uint32_t length)
{
...
#define __netdev_alloc_skb(dev, length, flags) __dev_alloc_skb(length, flags)

#define skb_checksum_none_assert(skb)

static inline void skb_copy_from_linear_data(const struct sk_buff *skb, void *to, const uint32_t len)
{
...
static inline uint16_t skb_get_queue_mapping(const struct sk_buff *skb)
{
    return 0;
}
