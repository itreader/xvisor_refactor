...
extern int      pm_schedule_suspend(struct device *dev, uint32_t delay);
extern int      __pm_runtime_set_status(struct device *dev, uint32_t status);
...
extern uint64_t pm_runtime_autosuspend_expiration(struct device *dev);
extern void     pm_runtime_update_max_time_suspended(struct device *dev, int64_t delta_ns);

static inline bool pm_children_suspended(struct device *dev)
{
    return dev->power.ignore_children || !atomic_read(&dev->power.child_count);
}
...
static inline int pm_schedule_suspend(struct device *dev, uint32_t delay)
{
    return -ENOSYS;
}

static inline int __pm_runtime_set_status(struct device *dev, uint32_t status)
{
    return 0;
}
...
static inline uint64_t pm_runtime_autosuspend_expiration(struct device *dev)
{
    return 0;
}
