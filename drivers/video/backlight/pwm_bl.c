...
struct pwm_bl_data {
    ...
    int (*check_fb)(struct device *, struct frame_buffer_info *);
...
{
    uint32_t lth = pb->lth_brightness;
    int      duty_cycle;
...
static int pwm_backlight_update_status(struct backlight_device *bl)
{
    ...
    if (bl->props.power != FB_BLANK_UNBLANK || bl->props.fb_blank != FB_BLANK_UNBLANK || bl->props.state & BL_CORE_FBBLANK) {
        brightness = 0;
    }
...
static int pwm_backlight_check_fb(struct backlight_device *bl, struct frame_buffer_info *info)
{
    struct pwm_bl_data *pb = bl_get_data(bl);
...
static int pwm_backlight_parse_dt(struct device *dev, struct platform_pwm_backlight_data *data)
{
    ...
    data->max_brightness = length / sizeof(uint32_t);
...
static struct vmm_device_tree_nodeid pwm_backlight_of_match[] = {{.compatible = "pwm-backlight"}, {}};

static int pwm_backlight_probe(vmm_device_t *dev)
{
    ...
    bl                   = backlight_device_register(dev_name(dev), dev, pb, &pwm_backlight_ops, &props);
    ...
    vmm_device_driver_set_data(dev, bl);
...
static int pwm_backlight_remove(vmm_device_t *dev)
{
    struct backlight_device *bl = vmm_device_driver_get_data(dev);
    struct pwm_bl_data      *pb = bl_get_data(bl);
...
#if 0
static void pwm_backlight_shutdown(vmm_device_t *dev)
{
    struct backlight_device *bl = vmm_device_driver_get_data(dev);
    struct pwm_bl_data *pb = bl_get_data(bl);
...
static int pwm_backlight_suspend(vmm_device_t *dev)
{
    struct backlight_device *bl = vmm_device_driver_get_data(dev);
    struct pwm_bl_data      *pb = bl_get_data(bl);
...
static vmm_driver_t pwm_backlight_driver = {
    .name = "pwm-backlight",
#if 0
...
{
    return vmm_device_driver_register_driver(&pwm_backlight_driver);
}

static void __exit pwm_backlight_driver_exit(void)
{
    vmm_device_driver_unregister_driver(&pwm_backlight_driver);
}
