...
static const unsigned char edid_v1_header[] = {0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00};
...
{
    if ((block[0] == 0x00) && (block[1] == 0x00) && (block[2] == 0x00) && (block[3] == 0xff) && (block[4] == 0x00)) {
        return 1;
    } else {
        return 0;
    }
...
{
    if ((block[0] == 0x00) && (block[1] == 0x00) && (block[2] == 0x00) && (block[3] == 0xfe) && (block[4] == 0x00)) {
        return 1;
    } else {
        return 0;
    }
...
{
    if ((block[0] == 0x00) && (block[1] == 0x00) && (block[2] == 0x00) && (block[3] == 0xfd) && (block[4] == 0x00)) {
        return 1;
    } else {
        return 0;
    }
...
{
    if ((block[0] == 0x00) && (block[1] == 0x00) && (block[2] == 0x00) && (block[3] == 0xfc) && (block[4] == 0x00)) {
        return 1;
    } else {
        return 0;
    }
...
static int check_edid(unsigned char *edid)
{
    ...
    for (i = 0; i < array_size(brokendb); i++) {
        if (!strncmp((char *)manufacturer, (char *)brokendb[i].manufacturer, 4) && brokendb[i].model == model) {
            fix = brokendb[i].fix;
            break;
        }
    }
...
static void fix_edid(unsigned char *edid, int fix)
{
    ...
    switch (fix) {
        ...
        case FBMON_FIX_TIMINGS:
            ...
            for (i = 0; i < 4; i++) {
                if (!(edid_is_serial_block(b) || edid_is_ascii_block(b) || edid_is_monitor_block(b) || edid_is_timing_block(b))) {
                    b[0]  = 0x00;
                    b[1]  = 0x00;
                    b[2]  = 0x00;
                    b[3]  = 0xfd;
...
static void get_dpms_capabilities(unsigned char flags, struct fb_monspecs *specs)
{
    ...
    DPRINTK(
        "      DPMS: Active %s, Suspend %s, Standby %s\n", (flags & DPMS_ACTIVE_OFF) ? "yes" : "no", (flags & DPMS_SUSPEND) ? "yes" : "no",
...
static void calc_mode_timings(int xres, int yres, int refresh, struct fb_videomode *mode)
{
    ...
    var = vmm_zalloc(sizeof(struct frame_buffer_var_screeninfo));
...
static int get_std_timing(unsigned char *block, struct fb_videomode *mode)
{
    ...
    for (i = 0; i < VESA_MODEDB_SIZE; i++) {
        if (vesa_modes[i].xres == xres && vesa_modes[i].yres == yres && vesa_modes[i].refresh == refresh) {
            *mode = vesa_modes[i];
...
static void get_detailed_timing(unsigned char *block, struct fb_videomode *mode)
{
    ...
    mode->refresh = udiv32(PIXEL_CLOCK, (H_ACTIVE + H_BLANKING) * (V_ACTIVE + V_BLANKING));
...
static int fb_get_monitor_limits(unsigned char *edid, struct fb_monspecs *specs)
{
    ...
    if (retval) {
        ...
        for (i = 0; i < num_modes; i++) {
            ...
            htotal   = mode->xres + mode->right_margin + mode->hsync_len + mode->left_margin;
            vtotal   = mode->yres + mode->lower_margin + mode->vsync_len + mode->upper_margin;
    ...
    DPRINTK(
        "           H: %d-%dKHz V: %d-%dHz DCLK: %dMHz\n", specs->hfmin / 1000, specs->hfmax / 1000, specs->vfmin, specs->vfmax,
...
void fb_edid_add_monspecs(unsigned char *edid, struct fb_monspecs *specs)
{
    ...
    int                  num = 0, i;
    uint8_t              svd[64], edt[(128 - 4) / DETAILED_TIMING_DESCRIPTION_SIZE];
    uint8_t              pos = 4, svd_n = 0;
    ...
    if (edid[0] != 0x2 || edid[2] < 4 || edid[2] > 128 - DETAILED_TIMING_DESCRIPTION_SIZE) {
        return;
    }
    ...
    while (pos < edid[2]) {
        uint8_t len = edid[pos] & 0x1f, type = (edid[pos] >> 5) & 7;
        pr_debug("Data block %u of %u bytes\n", type, len);

        if (type == 2) {
            for (i = pos; i < pos + len; i++) {
                uint8_t idx  = edid[pos + i] & 0x7f;
                svd[svd_n++] = idx;
    ...
    for (i = 0; i < (128 - edid[2]) / DETAILED_TIMING_DESCRIPTION_SIZE; i++, block += DETAILED_TIMING_DESCRIPTION_SIZE) {
        if (PIXEL_CLOCK != 0) {
            edt[num++] = block - edid;
        }
    }
    ...
    m = vmm_zalloc((specs->modedb_len + num + svd_n) * sizeof(struct fb_videomode));
    ...
    for (i = specs->modedb_len + num; i < specs->modedb_len + num + svd_n; i++) {
        ...
        if (!idx || idx > 63) {
            ...
            pr_debug("Adding SVD #%d: %ux%u@%u\n", idx, m[i].xres, m[i].yres, m[i].refresh);
        }
    }
 ...
 *                           1000000
 */
static uint32_t fb_get_vblank(uint32_t hfreq)
{
    uint32_t vblank;
 ...
 *
 */
static uint32_t fb_get_hblank_by_hfreq(uint32_t hfreq, uint32_t xres)
{
    uint32_t c_val, m_val, duty_cycle, hblank;
 ...
 */
static uint32_t fb_get_hblank_by_dclk(uint32_t dclk, uint32_t xres)
{
    uint32_t duty_cycle, h_period, hblank;
...
static uint32_t fb_get_hfreq(uint32_t vfreq, uint32_t yres)
{
    uint32_t divisor, hfreq;
 ...
 * @var: pointer to frame_buffer_var_screeninfo
 * @info: pointer to frame_buffer_info
 *
 * DESCRIPTION:
...
int fb_get_mode(int flags, uint32_t val, struct frame_buffer_var_screeninfo *var, struct frame_buffer_info *info)
{
    ...
    uint32_t             interlace = 1, dscan = 1;
    uint32_t             hfmin, hfmax, vfmin, vfmax, dclkmin, dclkmax, err = 0;
     ...
     * for 640x480@60
     */
    if (!info || !info->monspecs.hfmax || !info->monspecs.vfmax || !info->monspecs.dclkmax || info->monspecs.hfmax < info->monspecs.hfmin ||
    ...
    if (err || (!(flags & FB_IGNOREMON) && (timings->vfreq < vfmin || timings->vfreq > vfmax || timings->hfreq < hfmin || timings->hfreq > hfmax ||
        ...
        var->upper_margin = udiv32((timings->vblank * interlace), dscan) - (var->vsync_len + var->lower_margin);
    }
...
{
    uint32_t hfreq, vfreq, htotal, vtotal, pixclock;
    uint32_t hfmin, hfmax, vfmin, vfmax, dclkmin, dclkmax;

    /*
     * If monspecs are invalid, use values that are enough
     * for 640x480@60
     */
    if (!info->monspecs.hfmax || !info->monspecs.vfmax || !info->monspecs.dclkmax || info->monspecs.hfmax < info->monspecs.hfmin ||
        info->monspecs.vfmax < info->monspecs.vfmin || info->monspecs.dclkmax < info->monspecs.dclkmin) {
        hfmin   = 29000;
        hfmax   = 30000;
        vfmin   = 60;
