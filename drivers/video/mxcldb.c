...
struct ldb_data {
    ...
    struct ldb_setting {
        struct clk *di_clock;
        struct clk *ldb_di_clock;
        struct clk *div_3_5_clock;
        struct clk *div_sel_clock;
        bool        active;
        bool        clock_en;
        int         ipu;
        int         di;
...
{
    return (
        (pixel_fmt == IPU_PIX_FMT_RGB24) || (pixel_fmt == IPU_PIX_FMT_BGR24) || (pixel_fmt == IPU_PIX_FMT_LVDS666) ||
        (pixel_fmt == IPU_PIX_FMT_RGB666) || (pixel_fmt == IPU_PIX_FMT_BGR666));
}
...
static int ldb_get_of_property(vmm_device_t *dev, struct fsl_mxc_ldb_platform_data *plat_data)
{
    vmm_device_tree_node_t *np = dev->of_node;
    int                     err;
    uint32_t                ipu_id, disp_id;
    uint32_t                sec_ipu_id, sec_disp_id;
    char                   *mode;
    uint32_t                ext_ref;

    err = vmm_device_tree_read_string(np, "mode", (const char **)&mode);
    ...
    err = vmm_device_tree_read_u32(np, "ext_ref", &ext_ref);
    ...
    err = vmm_device_tree_read_u32(np, "ipu_id", &ipu_id);
    ...
    err = vmm_device_tree_read_u32(np, "disp_id", &disp_id);
    ...
    err = vmm_device_tree_read_u32(np, "sec_ipu_id", &sec_ipu_id);
    ...
    err = vmm_device_tree_read_u32(np, "sec_disp_id", &sec_disp_id);
...
static int find_ldb_setting(struct ldb_data *ldb, struct frame_buffer_info *fbi)
{
    char *id_di[] = {
        "DISP3 BG",
        "DISP3 BG - DI1",
    ...
    for (i = 0; i < 2; i++) {
        if (ldb->setting[i].active) {
            memset(id, 0, 16);
            memcpy(id, id_di[ldb->setting[i].di], strlen(id_di[ldb->setting[i].di]));
...
static int ldb_disp_setup(struct mxc_dispdrv_handle *disp, struct frame_buffer_info *fbi)
{
    uint32_t         reg, val;
    uint32_t         pixel_clock, rounded_pixel_clock;
    struct clk      *ldb_clock_parent;
    ...
    if (fbi->var.sync & FB_SYNC_VERT_HIGH_ACT) {
        if (di == 0) {
            reg = (reg & ~LDB_DI0_VS_POL_MASK) | LDB_DI0_VS_POL_ACT_HIGH;
        } else {
            reg = (reg & ~LDB_DI1_VS_POL_MASK) | LDB_DI1_VS_POL_ACT_HIGH;
        }
    } else {
        if (di == 0) {
            reg = (reg & ~LDB_DI0_VS_POL_MASK) | LDB_DI0_VS_POL_ACT_LOW;
        } else {
            reg = (reg & ~LDB_DI1_VS_POL_MASK) | LDB_DI1_VS_POL_ACT_LOW;
        }
    }
    ...
    /* clk setup */
    if (ldb->setting[setting_idx].clock_en) {
        clock_disable_unprepare(ldb->setting[setting_idx].ldb_di_clock);
    }

    pixel_clock      = (PICOS2KHZ(fbi->var.pixclock)) * 1000UL;

    ldb_clock_parent = clock_get_parent(ldb->setting[setting_idx].div_3_5_clock);

    if (VMM_IS_ERR_OR_NULL(ldb_clock_parent)) {
        dev_err(ldb->dev, "get ldb di parent clk fail\n");
        return VMM_PTR_ERR(ldb_clock_parent);
    }

    if ((ldb->mode == LDB_SPL_DI0) || (ldb->mode == LDB_SPL_DI1)) {
        ret = clock_set_rate(ldb_clock_parent, pixel_clock * 7 / 2);
    } else {
        ret = clock_set_rate(ldb_clock_parent, pixel_clock * 7);
    }
    ...
    rounded_pixel_clock = clock_round_rate(ldb->setting[setting_idx].ldb_di_clock, pixel_clock);
    dev_dbg(ldb->dev, "pixel_clock:%d, rounded_pixel_clock:%d\n", pixel_clock, rounded_pixel_clock);
    ret = clock_set_rate(ldb->setting[setting_idx].ldb_di_clock, rounded_pixel_clock);
    ...
    ret = clock_prepare_enable(ldb->setting[setting_idx].ldb_di_clock);
    ...
    if (!ldb->setting[setting_idx].clock_en) {
        ldb->setting[setting_idx].clock_en = true;
    }

    return 0;
}

int ldb_fb_event(vmm_notifier_block_t *nb, uint64_t val, void *v)
{
    struct ldb_data          *ldb   = container_of(nb, struct ldb_data, nb);
    struct fb_event          *event = v;
    struct frame_buffer_info *fbi   = event->info;
    int                       index;
    uint32_t                  data;
    ...
    if (!fbi->mode) {
        dev_warn(ldb->dev, "LDB: can not find mode for xres=%d, yres=%d\n", fbi->var.xres, fbi->var.yres);

        if (ldb->setting[index].clock_en) {
            clock_disable(ldb->setting[index].ldb_di_clock);
            ldb->setting[index].clock_en = false;
    ...
    switch (val) {
        case FB_EVENT_BLANK:
        {
            if (*((int *)event->data) == FB_BLANK_UNBLANK) {
                if (!ldb->setting[index].clock_en) {
                    clock_enable(ldb->setting[index].ldb_di_clock);
                    ldb->setting[index].clock_en = true;
                }
            } else {
                if (ldb->setting[index].clock_en) {
                    clock_disable(ldb->setting[index].ldb_di_clock);
                    ...
                    data &= ~ldb->setting[index].ch_mask;
                    writel(data, ldb->control_reg);
                    dev_dbg(ldb->dev, "LDB blank, control reg:0x%x\n", readl(ldb->control_reg));
                }
            }
...
static int ldb_ipu_ldb_route(int ipu, int di, struct ldb_data *ldb, int channel)
{
    ...
    if (mode < LDB_SIN0) {
        ...
        reg |= (ROUTE_IPU_DI(ipu, di) << LVDS0_MUX_CTL_OFFS) | (ROUTE_IPU_DI(ipu, di) << LVDS1_MUX_CTL_OFFS);
        dev_dbg(ldb->dev, "Dual/Split mode both channels route to IPU%d-DI%d\n", ipu, di);
    } else {
        shift = LVDS0_MUX_CTL_OFFS + channel * LVDS_MUX_CTL_WIDTH;
        reg &= ~(LVDS_MUX_CTL_MASK << shift);
        reg |= ROUTE_IPU_DI(ipu, di) << shift;
        dev_dbg(ldb->dev, "channel %d route to IPU%d-DI%d\n", channel, ipu, di);
    }
...
const static unsigned char lvds_enables[] = {
    [LDB_SPL_DI0]  = LDB_SPLIT_MODE_EN | LDB_CH0_MODE_EN_TO_DI0 | LDB_CH1_MODE_EN_TO_DI0,
    [LDB_SPL_DI1]  = LDB_SPLIT_MODE_EN | LDB_CH0_MODE_EN_TO_DI1 | LDB_CH1_MODE_EN_TO_DI1,
...
static int ldb_disp_init(struct mxc_dispdrv_handle *disp, struct mxc_dispdrv_setting *setting)
{
    int                               ret       = 0, i;
    struct ldb_data                  *ldb       = mxc_dispdrv_getdata(disp);
    ...
    int                               mode;
    char                              di_clock[]      = "ipu1_di0_sel";
    char                              ldb_clock[]     = "ldb_di0";
    char                              div_3_5_clock[] = "di0_div_3_5";
    char                              div_sel_clock[] = "di0_div_sel";
    ...
    if (!valid_mode(setting->if_fmt)) {
        dev_warn(
            ldb->dev, "Input pixel format not valid"
                      " use default RGB666\n");
        setting->if_fmt = IPU_PIX_FMT_RGB666;
    }

    if (!ldb->inited) {
        ret = vmm_device_tree_request_regmap(ldb->dev->of_node, (virtual_addr_t *)&ldb->reg, 0, "MXC LDB");
        ...
        if (ldb->mode < LDB_SIN0) {
            reg_clear |= LDB_CH0_MASKS | LDB_CH1_MASKS | LDB_SPLIT_MODE_EN;
        ...
        if ((setting->disp_id == ldb->setting[0].di) && (setting->dev_id == ldb->setting[0].ipu)) {
            dev_err(
                ldb->dev, "Err: for second ldb disp in"
    ...
    if (ldb->mode >= LDB_SIN0) {
        int lvds_ch_disp = (is_imx6_ldb(plat_data)) ? lvds_channel : setting->disp_id;

        reg_clear |= ((lvds_channel ? LDB_CH1_MASKS : LDB_CH0_MASKS) | LDB_SPLIT_MODE_EN);
        reg_set |= lvds_enables[LDB_SIN0 + ((lvds_channel << 1) | lvds_ch_disp)];

        if (bits_per_pixel(setting->if_fmt) == 24) {
            reg_set |= (lvds_channel ? LDB_DATA_WIDTH_CH1_24 : LDB_DATA_WIDTH_CH0_24);
        }

        ch_mask = lvds_channel ? LDB_CH1_MODE_MASK : LDB_CH0_MODE_MASK;
    }
    ...
    /* clock setting */
    ldb_clock[6]                           = '0' + lvds_channel;
    div_3_5_clock[2]                       = '0' + lvds_channel;
    div_sel_clock[2]                       = '0' + lvds_channel;
    ldb->setting[setting_idx].ldb_di_clock = clock_get(ldb->dev, ldb_clock);

    if (VMM_IS_ERR_OR_NULL(ldb->setting[setting_idx].ldb_di_clock)) {
        ...
        return VMM_PTR_ERR(ldb->setting[setting_idx].ldb_di_clock);
    }

    di_clock[3]                        = '1' + setting->dev_id;
    di_clock[7]                        = '0' + setting->disp_id;
    ldb->setting[setting_idx].di_clock = clock_get(ldb->dev, di_clock);

    if (VMM_IS_ERR_OR_NULL(ldb->setting[setting_idx].di_clock)) {
        ...
        return VMM_PTR_ERR(ldb->setting[setting_idx].di_clock);
    }

    dev_dbg(ldb->dev, "ldb_clock to di clk: %s -> %s\n", ldb_clock, di_clock);

    ldb->setting[setting_idx].div_3_5_clock = clock_get(ldb->dev, div_3_5_clock);

    if (VMM_IS_ERR_OR_NULL(ldb->setting[setting_idx].div_3_5_clock)) {
        ...
        return VMM_PTR_ERR(ldb->setting[setting_idx].div_3_5_clock);
    }

    ldb->setting[setting_idx].div_sel_clock = clock_get(ldb->dev, div_sel_clock);

    if (VMM_IS_ERR_OR_NULL(ldb->setting[setting_idx].div_sel_clock)) {
        ...
        return VMM_PTR_ERR(ldb->setting[setting_idx].div_sel_clock);
    }
    ...
    ret = fb_find_mode(&setting->fbi->var, setting->fbi, setting->dft_mode_str, ldb_modedb, ldb_modedb_sz, NULL, setting->default_bpp);
    ...
    INIT_LIST_HEAD(&setting->fbi->modelist);
    {
        ...
        frame_buffer_var_to_videomode(&m, &setting->fbi->var);
        ...
        if (0) {
            pr_info(
                "%s:r=%d, x=%d, y=%d, p=%d, l=%d, r=%d, upper=%d, lower=%d, h=%d, v=%d\n", __func__, m.refresh, m.xres, m.yres, m.pixclock,
        ...
        for (i = 0; i < ldb_modedb_sz; i++) {
            if (!fb_mode_is_equal(&m, &ldb_modedb[i])) {
                ...
                fb_add_videomode(&ldb_modedb[i], &setting->fbi->modelist);
            }
        }
    }
...
static int ldb_post_disp_init(struct mxc_dispdrv_handle *disp, int ipu_id, int disp_id)
{
    struct ldb_data *ldb         = mxc_dispdrv_getdata(disp);
    ...
    ret = clock_set_parent(ldb->setting[setting_idx].di_clock, ldb->setting[setting_idx].ldb_di_clock);

    if (ret) {
        dev_err(
            ldb->dev, "fail to set ldb_di clk as "
...
static void ldb_disp_deinit(struct mxc_dispdrv_handle *disp)
{
    ...
    for (i = 0; i < 2; i++) {
        clock_disable(ldb->setting[i].ldb_di_clock);
        clock_put(ldb->setting[i].ldb_di_clock);
        clock_put(ldb->setting[i].div_3_5_clock);
        clock_put(ldb->setting[i].div_sel_clock);
    }
...
static const struct of_device_id imx_ldb_device_tree_ids[] = {
    {
     .compatible = "fsl,imx6q-ldb",
     .data       = &imx_ldb_devtype[IMX6_LDB],
     },
    {/* sentinel */                            }
};
 ...
 */
static int ldb_probe(vmm_device_t *dev)
{
    int                                  ret = 0;
    struct ldb_data                     *ldb;
    struct fsl_mxc_ldb_platform_data    *plat_data;
    ...
    vmm_device_driver_set_data(dev, ldb);

    dev_dbg(dev, "%s exit\n", __func__);
    return ret;
}

static int ldb_remove(vmm_device_t *dev)
{
    struct ldb_data *ldb = vmm_device_driver_get_data(dev);
...
static vmm_driver_t mxcldb_driver = {
    .name        = "mxc_ldb",
    .match_table = imx_ldb_device_tree_ids,
    .probe       = ldb_probe,
    .remove      = ldb_remove,
#if 0
    .suspend = ldb_suspend,
...
{
    return vmm_device_driver_register_driver(&mxcldb_driver);
}

static void __exit ldb_uninit(void)
{
    vmm_device_driver_unregister_driver(&mxcldb_driver);
}
