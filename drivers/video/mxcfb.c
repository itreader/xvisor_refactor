...
static void *dma_alloc_attrs(
    vmm_device_t __unused *dev, virtual_size_t size, physical_addr_t *dma_handle, uint32_t __unused flags, uint32_t __unused *attrs)
{
...
static void dma_free_attrs(
    vmm_device_t __unused *dev, virtual_size_t __unused size, void *cpu_addr, physical_addr_t __unused dma_handle, uint32_t __unused *attrs)
{
...
struct mxcframe_buffer_info {
    ...
    bool             mode_found;
    vmm_completion_t flip_complete;
    vmm_completion_t alpha_flip_complete;
    vmm_completion_t vsync_complete;

    void                     *ipu;
    struct frame_buffer_info *ovfbi;

    struct mxc_dispdrv_handle *dispdrv;

    struct frame_buffer_var_screeninfo cur_var;
};
...
static inline int bitfield_is_equal(struct frame_buffer_bitfield f1, struct frame_buffer_bitfield f2)
{
    return !memcmp(&f1, &f2, sizeof(f1));
}

static int pixfmt_to_var(uint32_t pixfmt, struct frame_buffer_var_screeninfo *var)
{
    int i, ret = -1;
...
static int bpp_to_var(int bpp, struct frame_buffer_var_screeninfo *var)
{
    uint32_t pixfmt = 0;
...
static int check_var_pixfmt(struct frame_buffer_var_screeninfo *var)
{
    int i, ret = -1;

    for (i = 0; i < ARRAY_SIZE(mxcfb_pfmts); i++) {
        if (bitfield_is_equal(var->red, mxcfb_pfmts[i].red) && bitfield_is_equal(var->green, mxcfb_pfmts[i].green) &&
            bitfield_is_equal(var->blue, mxcfb_pfmts[i].blue) && bitfield_is_equal(var->transp, mxcfb_pfmts[i].transp) &&
            var->bits_per_pixel == mxcfb_pfmts[i].bpp) {
            ret = 0;
            break;
        }
...
static uint32_t fbi_to_pixfmt(struct frame_buffer_info *fbi)
{
    int      i;
    uint32_t pixfmt = 0;
    ...
    for (i = 0; i < ARRAY_SIZE(mxcfb_pfmts); i++) {
        if (bitfield_is_equal(fbi->var.red, mxcfb_pfmts[i].red) && bitfield_is_equal(fbi->var.green, mxcfb_pfmts[i].green) &&
            bitfield_is_equal(fbi->var.blue, mxcfb_pfmts[i].blue) && bitfield_is_equal(fbi->var.transp, mxcfb_pfmts[i].transp)) {
...
static struct frame_buffer_info *found_registered_fb(ipu_channel_t ipu_ch, int ipu_id)
{
    int                          i;
    struct mxcframe_buffer_info *mxc_fbi = NULL;
    struct frame_buffer_info    *dev_fbi = NULL;
    struct frame_buffer_info    *fbi     = NULL;

    for (i = 0; i < fb_count(); i++) {
        dev_fbi = fb_get(i);
        mxc_fbi = (struct mxcframe_buffer_info *)dev_fbi->par;

        if ((mxc_fbi->ipu_ch == ipu_ch) && (mxc_fbi->ipu_id == ipu_id)) {
            fbi = dev_fbi;
            break;
        }
    }
...
static int              mxcfb_blank(int blank, struct frame_buffer_info *info);
static int              mxcfb_map_video_memory(struct frame_buffer_info *fbi);
static int              mxcfb_unmap_video_memory(struct frame_buffer_info *fbi);
 ...
 */
static int mxcfb_set_fix(struct frame_buffer_info *info)
{
    struct frame_buffer_fix_screeninfo *fix = &info->fix;
    struct frame_buffer_var_screeninfo *var = &info->var;
...
static int _setup_disp_channel1(struct frame_buffer_info *fbi)
{
...
static int _setup_disp_channel2(struct frame_buffer_info *fbi)
{
    int                          retval  = 0;
    struct mxcframe_buffer_info *mxc_fbi = (struct mxcframe_buffer_info *)fbi->par;
    int                          fb_stride;
    uint64_t                     base;
    uint32_t                     fr_xoff, fr_yoff, fr_w, fr_h;
    ...
    if (!(fbi->var.vmode & FB_VMODE_YWRAP)) {
        ...
        fr_h    = fbi->var.yres;
        base += fbi->fix.line_length * fbi->var.yres * udiv32(fbi->var.yoffset, fbi->var.yres);
    } else {
    ...
    retval = ipu_init_channel_buffer(
        mxc_fbi->ipu, mxc_fbi->ipu_ch, IPU_INPUT_BUFFER, fbi_to_pixfmt(fbi), fbi->var.xres, fbi->var.yres, fb_stride, fbi->var.rotate, base, base,
        fbi->var.accel_flags & FB_ACCEL_DOUBLE_FLAG ? 0 : base, 0, 0);

    if (retval) {
        dev_err(&fbi->dev, "ipu_init_channel_buffer error %d\n", retval);
        return retval;
    }

    /* update u/v offset */
    ipu_update_channel_offset(mxc_fbi->ipu, mxc_fbi->ipu_ch, IPU_INPUT_BUFFER, fbi_to_pixfmt(fbi), fr_w, fr_h, fr_w, 0, 0, fr_yoff, fr_xoff);

    if (mxc_fbi->alpha_chan_en) {
        retval = ipu_init_channel_buffer(
            mxc_fbi->ipu, mxc_fbi->ipu_ch, IPU_ALPHA_IN_BUFFER, IPU_PIX_FMT_GENERIC, fbi->var.xres, fbi->var.yres, fbi->var.xres, fbi->var.rotate,
            mxc_fbi->alpha_phy_addr1, mxc_fbi->alpha_phy_addr0, 0, 0, 0);
...
static bool mxcfb_need_to_set_par(struct frame_buffer_info *fbi)
{
    struct mxcframe_buffer_info *mxc_fbi = fbi->par;

    if ((fbi->var.activate & FB_ACTIVATE_FORCE) && (fbi->var.activate & FB_ACTIVATE_MASK) == FB_ACTIVATE_NOW) {
        return true;
    }
    ...
    return !!memcmp(&mxc_fbi->cur_var, &fbi->var, sizeof(struct frame_buffer_var_screeninfo));
}
 ...
 * @param       info     framebuffer information pointer
 */
static int mxcfb_set_par(struct frame_buffer_info *fbi)
{
    int                          retval = 0;
    uint32_t                     mem_len, alpha_mem_len;
    ipu_di_signal_cfg_t          sig_cfg;
    struct mxcframe_buffer_info *mxc_fbi  = (struct mxcframe_buffer_info *)fbi->par;

    int16_t                      ov_pos_x = 0, ov_pos_y = 0;
    int                          ov_pos_ret   = 0;
    struct mxcframe_buffer_info *mxc_fbi_fg   = NULL;
    bool                         ovfbi_enable = false;

    if (mxc_fbi->ovfbi) {
        mxc_fbi_fg = (struct mxcframe_buffer_info *)mxc_fbi->ovfbi->par;
    }
    ...
    if (ovfbi_enable) {
        ov_pos_ret = ipu_disp_get_window_pos(mxc_fbi_fg->ipu, mxc_fbi_fg->ipu_ch, &ov_pos_x, &ov_pos_y);

        if (ov_pos_ret < 0) {
            dev_err(
                &fbi->dev,
                "Get overlay pos failed, "
                "dispdrv:%s.\n",
    ...
    if (mxc_fbi->first_set_par && mxc_fbi->late_init) {
        ipu_disable_hsp_clock(mxc_fbi->ipu);
    }
    ...
    if (mxc_fbi->alpha_chan_en) {
        ...
        if ((!mxc_fbi->alpha_phy_addr0 && !mxc_fbi->alpha_phy_addr1) || (alpha_mem_len > mxc_fbi->alpha_mem_len)) {
            if (mxc_fbi->alpha_phy_addr0) {
                dma_free_coherent(&fbi->dev, mxc_fbi->alpha_mem_len, mxc_fbi->alpha_virt_addr0, mxc_fbi->alpha_phy_addr0);
            }
            ...
            mxc_fbi->alpha_virt_addr0 = dma_alloc_coherent(&fbi->dev, alpha_mem_len, &mxc_fbi->alpha_phy_addr0, GFP_DMA | GFP_KERNEL);

            mxc_fbi->alpha_virt_addr1 = dma_alloc_coherent(&fbi->dev, alpha_mem_len, &mxc_fbi->alpha_phy_addr1, GFP_DMA | GFP_KERNEL);

            if (mxc_fbi->alpha_virt_addr0 == NULL || mxc_fbi->alpha_virt_addr1 == NULL) {
                dev_err(
                    &fbi->dev, "mxcfb: dma alloc for"
                               " alpha buffer failed.\n");
    ...
    if (!mxc_fbi->overlay) {
        ...
        if (mxc_fbi->ipu_int_clock) {
            sig_cfg.int_clock = true;
        }
        ...
        dev_dbg(&fbi->dev, "pixclock = %u Hz\n", (uint32_t)(PICOS2KHZ(fbi->var.pixclock) * 1000UL));

        if (ipu_init_sync_panel(
                mxc_fbi->ipu, mxc_fbi->ipu_di, (PICOS2KHZ(fbi->var.pixclock)) * 1000UL, fbi->var.xres, fbi->var.yres, out_pixel_fmt,
    ...
    if (ovfbi_enable) {
        if (ov_pos_ret >= 0) {
            ipu_disp_set_window_pos(mxc_fbi_fg->ipu, mxc_fbi_fg->ipu_ch, ov_pos_x, ov_pos_y);
        }
...
static int mxcfb_set_smem(struct frame_buffer_info *fbi, uint64_t start, uint32_t len)
{
    int                          retval = 0;
    uint32_t                     alpha_mem_len;
    ipu_di_signal_cfg_t          sig_cfg;
    ...
    if (fbi->var.xres_virtual * fbi->var.bits_per_pixel / 8 * fbi->var.yres_virtual > len) {
        return -EINVAL;
    }
...
static int _swap_channels(struct frame_buffer_info *fbi_from, struct frame_buffer_info *fbi_to, bool both_on)
{
    int                          retval, tmp;
    ipu_channel_t                old_ch;
    struct frame_buffer_info    *ovfbi;
    struct mxcframe_buffer_info *mxc_fbi_from = (struct mxcframe_buffer_info *)fbi_from->par;
    struct mxcframe_buffer_info *mxc_fbi_to   = (struct mxcframe_buffer_info *)fbi_to->par;
...
int swap_channels(struct frame_buffer_info *fbi_from)
{
    int                          i;
    int                          swap_mode;
    ipu_channel_t                ch_to;
    struct mxcframe_buffer_info *mxc_fbi_from = (struct mxcframe_buffer_info *)fbi_from->par;
    struct frame_buffer_info    *fbi_to       = NULL;
    struct mxcframe_buffer_info *mxc_fbi_to;
    ...
    mxc_fbi_to = (struct mxcframe_buffer_info *)fbi_to->par;
 ...
 * @param       info     framebuffer information pointer
 */
static int mxcfb_check_var(struct frame_buffer_var_screeninfo *var, struct frame_buffer_info *info)
{
    uint32_t                     vtotal;
    uint32_t                     htotal;
    struct mxcframe_buffer_info *mxc_fbi = (struct mxcframe_buffer_info *)info->par;
    ...
    if (mxc_fbi->ipu_ch == MEM_FG_SYNC) {
        struct frame_buffer_info *fbi_tmp;
    ...
    if ((var->bits_per_pixel != 32) && (var->bits_per_pixel != 24) && (var->bits_per_pixel != 16) && (var->bits_per_pixel != 12) &&
    ...
    if (var->pixclock < 1000) {
        ...
        dev_dbg(&info->dev, "pixclock set for 60Hz refresh = %u ps\n", var->pixclock);
    }
...
static inline u_int _chan_to_field(u_int chan, struct frame_buffer_bitfield *bf)
{
    chan &= 0xffff;
    chan >>= 16 - bf->length;
...
static int mxcfb_setcolreg(u_int regno, u_int red, u_int green, u_int blue, u_int trans, struct frame_buffer_info *fbi)
{
    uint32_t val;
    int      ret = 1;
    ...
    switch (fbi->fix.visual) {
        case FB_VISUAL_TRUECOLOR:
             ...
             */
            if (regno < 16) {
                uint32_t *pal = fbi->pseudo_palette;
 ...
 */
static int mxcfb_ioctl(struct frame_buffer_info *fbi, uint32_t cmd, uint64_t arg)
{
    int                          retval  = 0;
    ...
    switch (cmd) {
        case MXCFB_SET_GBL_ALPHA:
        {
            ...
            if (ga.enable) {
                dev_dbg(&fbi->dev, "Set global alpha of %s to %d\n", fbi->fix.id, ga.alpha);
            }
        ...
        case MXCFB_SET_LOC_ALPHA:
        {
            ...
            if (la.enable && !la.alpha_in_pixel) {
                ...
                if (fbi_tmp) {
                    ((struct mxcframe_buffer_info *)(fbi_tmp->par))->alpha_chan_en = false;
                }
            } else {
                mxc_fbi->alpha_chan_en = false;
            }
            ...
            if (la.enable) {
                dev_dbg(&fbi->dev, "Enable DP local alpha for %s\n", fbi->fix.id);
            }
        ...
        case MXCFB_SET_LOC_ALP_BUF:
        {
            uint64_t base;
            uint32_t ipu_alp_ch_irq;
            uint64_t timeout = HZ / 2;

            if (!(((mxc_fbi->ipu_ch == MEM_FG_SYNC) || (mxc_fbi->ipu_ch == MEM_BG_SYNC)) && (mxc_fbi->alpha_chan_en))) {
                dev_err(
            ...
            retval = vmm_completion_wait_timeout(&mxc_fbi->alpha_flip_complete, &timeout);
            ...
            if (ipu_update_channel_buffer(mxc_fbi->ipu, mxc_fbi->ipu_ch, IPU_ALPHA_IN_BUFFER, mxc_fbi->cur_ipu_alpha_buf, base) == 0) {
                ipu_select_buffer(mxc_fbi->ipu, mxc_fbi->ipu_ch, IPU_ALPHA_IN_BUFFER, mxc_fbi->cur_ipu_alpha_buf);
                ...
                dev_err(
                    ...
                    "to address=0x%08lX\n",
                    fbi->fix.id, mxc_fbi->cur_ipu_alpha_buf, base);
            }
        ...
        case MXCFB_SET_CLR_KEY:
        {
            ...
            retval = ipu_disp_set_color_key(mxc_fbi->ipu, mxc_fbi->ipu_ch, key.enable, key.color_key);
        ...
        case MXCFB_SET_GAMMA:
        {
            ...
            retval = ipu_disp_set_gamma_correction(mxc_fbi->ipu, mxc_fbi->ipu_ch, gamma.enable, gamma.constk, gamma.slopek);
            break;
        }

        case MXCFB_WAIT_FOR_VSYNC:
        {
            uint64_t timeout = 1 * HZ;

            if (mxc_fbi->ipu_ch == MEM_FG_SYNC) {
                /* BG should poweron */
                struct mxcframe_buffer_info *bg_mxcfbi = NULL;
                struct frame_buffer_info    *fbi_tmp;

                fbi_tmp = found_registered_fb(MEM_BG_SYNC, mxc_fbi->ipu_id);

                if (fbi_tmp) {
                    bg_mxcfbi = ((struct mxcframe_buffer_info *)(fbi_tmp->par));
                }
            ...
            retval = vmm_completion_wait_timeout(&mxc_fbi->vsync_complete, &timeout);
        ...
        case FBIO_ALLOC:
        {
            ...
            mem->cpu_addr = dma_alloc_coherent(&fbi->dev, size, &mem->phy_addr, GFP_KERNEL);
        ...
        case FBIO_FREE:
        {
            ...
            list_for_each_entry(mem, &fb_alloc_list, list)
            {
                if (mem->phy_addr == offset) {
                    list_del(&mem->list);
                    dma_free_coherent(&fbi->dev, mem->size, mem->cpu_addr, mem->phy_addr);
                    vmm_free(mem);
                    retval = 0;
                    break;
                }
        ...
        case MXCFB_SET_OVERLAY_POS:
        {
            ...
            struct frame_buffer_info *bg_fbi = NULL;
 ...
 * @param               info    Framebuffer information pointer
 */
static int mxcfb_pan_display(struct frame_buffer_var_screeninfo *var, struct frame_buffer_info *info)
{
    struct mxcframe_buffer_info *mxc_fbi = (struct mxcframe_buffer_info *)info->par, *mxc_graphic_fbi = NULL;
    u_int                        y_bottom;
    uint32_t                     fr_xoff, fr_yoff, fr_w, fr_h;
    uint64_t                     base, active_alpha_phy_addr = 0;
    ...
    if (!(var->vmode & FB_VMODE_YWRAP)) {
        ...
        fr_h    = info->var.yres;
        base += info->fix.line_length * info->var.yres * udiv32(var->yoffset, info->var.yres);
    } else {
    ...
    if (mxc_fbi->ipu_ch == MEM_BG_SYNC || mxc_fbi->ipu_ch == MEM_FG_SYNC) {
        for (i = 0; i < fb_count(); i++) {
            ...
            if ((strcmp(idstr, bg_id) == 0 || strcmp(idstr, fg_id) == 0) && ((struct mxcframe_buffer_info *)(fb_get(i)->par))->alpha_chan_en) {
                loc_alpha_en          = true;
                mxc_graphic_fbi       = (struct mxcframe_buffer_info *)(fb_get(i)->par);
                active_alpha_phy_addr = mxc_fbi->cur_ipu_alpha_buf ? mxc_graphic_fbi->alpha_phy_addr1 : mxc_graphic_fbi->alpha_phy_addr0;
                dev_dbg(
                    &info->dev,
                    "Updating SDC alpha "
                    "buf %d address=0x%08lX\n",
                    !mxc_fbi->cur_ipu_alpha_buf, active_alpha_phy_addr);
                break;
            }
        }
    }
    ...
    if (ipu_update_channel_buffer(mxc_fbi->ipu, mxc_fbi->ipu_ch, IPU_INPUT_BUFFER, mxc_fbi->cur_ipu_buf, base) == 0) {
        ...
        if (loc_alpha_en && mxc_graphic_fbi == mxc_fbi &&
            ...
            ipu_select_buffer(mxc_graphic_fbi->ipu, mxc_graphic_fbi->ipu_ch, IPU_ALPHA_IN_BUFFER, mxc_fbi->cur_ipu_alpha_buf);
        }

        /* update u/v offset */
        ipu_update_channel_offset(mxc_fbi->ipu, mxc_fbi->ipu_ch, IPU_INPUT_BUFFER, fbi_to_pixfmt(info), fr_w, fr_h, fr_w, 0, 0, fr_yoff, fr_xoff);

        ipu_select_buffer(mxc_fbi->ipu, mxc_fbi->ipu_ch, IPU_INPUT_BUFFER, mxc_fbi->cur_ipu_buf);
        ipu_clear_irq(mxc_fbi->ipu, mxc_fbi->ipu_ch_irq);
        ...
        dev_err(
            ...
            "buf2 ready %d\n",
            mxc_fbi->cur_ipu_buf, base, ipu_get_cur_buffer_idx(mxc_fbi->ipu, mxc_fbi->ipu_ch, IPU_INPUT_BUFFER),
            ipu_check_buffer_ready(mxc_fbi->ipu, mxc_fbi->ipu_ch, IPU_INPUT_BUFFER, 0),
            ipu_check_buffer_ready(mxc_fbi->ipu, mxc_fbi->ipu_ch, IPU_INPUT_BUFFER, 1),
            ipu_check_buffer_ready(mxc_fbi->ipu, mxc_fbi->ipu_ch, IPU_INPUT_BUFFER, 2));
        ++mxc_fbi->cur_ipu_buf;
        mxc_fbi->cur_ipu_buf %= 3;
        ++mxc_fbi->cur_ipu_buf;
        mxc_fbi->cur_ipu_buf %= 3;
 ...
 */
static int mxcfb_mmap(struct frame_buffer_info *fbi, struct vm_area_struct *vma)
{
    bool found = false;
    uint32_t len;
...
static struct fb_ops mxcfb_ops = {
    ...
    .fb_mmap = mxcfb_mmap,
#endif /* 0 */
    .frame_buffer_fillrect  = cframe_buffer_fillrect,
    .frame_buffer_copyarea  = cframe_buffer_copyarea,
    .frame_buffer_imageblit = cframe_buffer_imageblit,
    .fb_blank               = mxcfb_blank,
};

static vmm_irq_return_t mxcfb_irq_handler(int irq, void *dev_id)
{
    struct frame_buffer_info    *fbi     = dev_id;
    struct mxcframe_buffer_info *mxc_fbi = fbi->par;
...
{
    struct frame_buffer_info *fbi = platform_get_drvdata(pdev);
    struct mxcframe_buffer_info *mxc_fbi = (struct mxcframe_buffer_info *)fbi->par;
    int saved_blank;
    ...
    if (mxc_fbi->ovfbi) {
        struct mxcframe_buffer_info *mxc_fbi_fg =
            (struct mxcframe_buffer_info *)mxc_fbi->ovfbi->par;
...
static int mxcfb_map_video_memory(struct frame_buffer_info *fbi)
{
    ...
    fbi->screen_base = dma_alloc_writecombine(&fbi->dev, fbi->fix.smem_len, (dma_addr_t *)&fbi->fix.smem_start, GFP_DMA | GFP_KERNEL);
...
static int mxcfb_unmap_video_memory(struct frame_buffer_info *fbi)
{
    dma_free_writecombine(&fbi->dev, fbi->fix.smem_len, fbi->screen_base, fbi->fix.smem_start);
    fbi->screen_base    = 0;
    fbi->fix.smem_start = 0;
    fbi->fix.smem_len   = 0;
    return 0;
 ...
 */
static struct frame_buffer_info *mxcfb_init_fbinfo(vmm_device_t *dev, struct fb_ops *ops)
{
    struct frame_buffer_info    *fbi;
    struct mxcframe_buffer_info *mxcfbi;

    /*
     * Allocate sufficient memory for the fb structure
     */
    fbi = fb_alloc(sizeof(struct mxcframe_buffer_info), dev);

    if (!fbi) {
        return NULL;
    }

    mxcfbi            = (struct mxcframe_buffer_info *)fbi->par;
...
#if 0
static ssize_t show_disp_chan(vmm_device_t *dev,
                              struct device_attribute *attr, char *buf)
{
    struct frame_buffer_info *info = dev_get_drvdata(dev);
    struct mxcframe_buffer_info *mxcfbi = (struct mxcframe_buffer_info *)info->par;
                              ...
                              const char *buf, size_t count)
{
    struct frame_buffer_info *info = dev_get_drvdata(dev);
    struct mxcframe_buffer_info *mxcfbi = (struct mxcframe_buffer_info *)info->par;
    struct mxcframe_buffer_info *fg_mxcfbi = NULL;
    ...
    if (((mxcfbi->ipu_ch == MEM_BG_SYNC) &&
        ...
        struct frame_buffer_info *fbi_fg;

        fbi_fg = found_registered_fb(MEM_FG_SYNC, mxcfbi->ipu_id);

        if (fbi_fg)
            fg_mxcfbi = (struct mxcframe_buffer_info *)fbi_fg->par;
...
static int mxcfb_dispdrv_init(vmm_device_t *dev, struct frame_buffer_info *fbi)
{
    struct mxcframe_buffer_info   *mxcfbi    = (struct mxcframe_buffer_info *)fbi->par;
    ...
    if (!strlen(plat_data->disp_dev)) {
        ...
        memcpy(disp_dev, plat_data->disp_dev, strlen(plat_data->disp_dev));
    ...
    if (VMM_IS_ERR_OR_NULL(mxcfbi->dispdrv)) {
        ...
        dev_dbg(dev, "di_pixfmt:0x%x, bpp:0x%x, di:%d, ipu:%d\n", setting.if_fmt, setting.default_bpp, setting.disp_id, setting.dev_id);
    }
 ...
 */
static int mxcfb_option_setup(vmm_device_t *dev, struct frame_buffer_info *fbi)
{
    char *options_save, *options, *opt, *fb_mode_str = NULL;
    struct mxcframe_buffer_info *mxcfbi = fbi->par;
    ...
    while ((opt = strsep(&options, ",")) != NULL) {
        ...
        } else if (!strncmp(opt, "int_clock", 7)) {
            pdata->int_clock = true;
            continue;
...
static int mxcfb_register(struct frame_buffer_info *fbi)
{
    struct mxcframe_buffer_info *mxcfbi = (struct mxcframe_buffer_info *)fbi->par;
    ...
    frame_buffer_var_to_videomode(&m, &fbi->var);
    ...
    if (mxcfbi->ipu_alp_ch_irq != -1) {
        if (ipu_request_irq(mxcfbi->ipu, mxcfbi->ipu_alp_ch_irq, mxcfb_alpha_irq_handler, IPU_IRQF_ONESHOT, MXCFB_NAME, fbi) != 0) {
            dev_err(
    ...
    if (!mxcfbi->late_init) {
        ...
        if (mxcfbi->next_blank == FB_BLANK_UNBLANK) {
            ...
            if (ret < 0) {
                dev_err(&fbi->dev, "Error fb_blank ret:%d\n", ret);
                goto err4;
            }
        }
    } else {
...
err5:
    ...
    if (mxcfbi->next_blank == FB_BLANK_UNBLANK) {
        ...
        if (!mxcfbi->late_init) {
            ...
            ipu_disable_channel(mxcfbi->ipu, mxcfbi->ipu_ch, true);
...
static void mxcfb_unregister(struct frame_buffer_info *fbi)
{
    struct mxcframe_buffer_info *mxcfbi = (struct mxcframe_buffer_info *)fbi->par;
...
static int mxcfb_setup_overlay(vmm_device_t *dev, struct frame_buffer_info *fbi_bg, virtual_addr_t start, virtual_size_t size)
{
    struct frame_buffer_info    *ovfbi;
    struct mxcframe_buffer_info *mxcfbi_bg = (struct mxcframe_buffer_info *)fbi_bg->par;
    struct mxcframe_buffer_info *mxcfbi_fg;
    int                          ret = 0;
    ...
    mxcfbi_fg      = (struct mxcframe_buffer_info *)ovfbi->par;
    ...
    if (start && size) {
        ...
        ovfbi->screen_base    = (char *)vmm_host_iomap(ovfbi->fix.smem_start, ovfbi->fix.smem_len);
    }
...
static void mxcfb_unsetup_overlay(struct frame_buffer_info *fbi_bg)
{
    struct mxcframe_buffer_info *mxcfbi_bg = (struct mxcframe_buffer_info *)fbi_bg->par;
    struct frame_buffer_info    *ovfbi     = mxcfbi_bg->ovfbi;
...
static int mxcfb_get_of_property(vmm_device_t *dev, struct ipuv3_fb_platform_data *plat_data)
{
    ...
    int                     err;
    int                     len;
    uint32_t                bpp, int_clock;
    uint32_t                late_init;

    err = vmm_device_tree_read_string(np, "disp_dev", &disp_dev);
    ...
    err = vmm_device_tree_read_string(np, "mode_str", &mode_str);
    ...
    err = vmm_device_tree_read_string(np, "interface_pix_fmt", &pixfmt);
    ...
    err = vmm_device_tree_read_u32(np, "default_bpp", &bpp);
    ...
    err = vmm_device_tree_read_u32(np, "int_clock", &int_clock);

    if (err) {
        dev_dbg(dev, "get of property int_clock fail\n");
        return err;
    }

    err = vmm_device_tree_read_u32(np, "late_init", &late_init);
    ...
    plat_data->int_clock     = (bool)int_clock;
 ...
 */
static int mxcfb_probe(vmm_device_t *dev)
{
    ...
    mxcfbi->ipu_int_clock = plat_data->int_clock;
    ...
    if (mxcfbi->dispdrv->drv->post_init) {
        ret = mxcfbi->dispdrv->drv->post_init(mxcfbi->dispdrv, mxcfbi->ipu_id, mxcfbi->ipu_di);
    ...
    if (VMM_OK != vmm_device_tree_regsize(dev->of_node, &size, 0)) {
        size = 0;
    }

    if (VMM_OK != vmm_device_tree_regaddr(dev->of_node, &start, 0)) {
        start = 0;
    }
    ...
    if (start && size) {
        ...
        fbi->fix.smem_start = start;
        fbi->screen_base    = (char *)vmm_host_iomap(fbi->fix.smem_start, fbi->fix.smem_len);
    ...
    if (!g_dp_in_use[mxcfbi->ipu_id]) {
        ...
        if (VMM_OK != vmm_device_tree_regsize(dev->of_node, &size, 1)) {
            size = 0;
        }

        if (VMM_OK != vmm_device_tree_regaddr(dev->of_node, &start, 1)) {
            start = 0;
        }
    ...
    vmm_device_driver_set_data(dev, fbi);
...
static int mxcfb_remove(vmm_device_t *dev)
{
    struct frame_buffer_info    *fbi     = vmm_device_driver_get_data(dev);
    struct mxcframe_buffer_info *mxc_fbi = fbi->par;
...
static const struct vmm_device_tree_nodeid imx_mxcfb_device_tree_ids[] = {{.compatible = "fsl,mxc_sdc_fb"}, {/* sentinel */}};

/*!
 * This structure contains pointers to the power management callback functions.
 */
static vmm_driver_t mxcfb_driver                                       = {
                                          .name        = MXCFB_NAME,
                                          .match_table = imx_mxcfb_device_tree_ids,
                                          .probe       = mxcfb_probe,
                                          .remove      = mxcfb_remove,
#if 0
...
{
    return vmm_device_driver_register_driver(&mxcfb_driver);
}

void mxcfb_exit(void)
{
    vmm_device_driver_unregister_driver(&mxcfb_driver);
}
