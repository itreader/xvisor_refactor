...
static const struct fb_videomode vga_mode = {
    ...
    NULL, 60, 640, 480, 39721, 48, 16, 33, 10, 96, 2, 0, FB_VMODE_NONINTERLACED | FB_VMODE_ASPECT_4_3, FB_MODE_IS_VESA,
};
...
struct hdmi_vmode {
    ...
    uint32_t mPixelRepetitionInput;
    uint32_t mPixelRepetitionOutput;
};

struct hdmi_data_info {
    uint32_t          enc_in_format;
    uint32_t          enc_out_format;
    uint32_t          enc_color_depth;
    uint32_t          colorimetry;
    uint32_t          pix_repet_factor;
    uint32_t          hdcp_enable;
    uint32_t          rgb_out_enable;
...
struct hdmi_phy_reg_config {
    ...
    uint16_t reg_vlev;
    uint16_t reg_cksymtx;
};

struct mxc_hdmi {
    vmm_device_t              *dev;
    struct mxc_dispdrv_handle *disp_mxc_hdmi;
    struct frame_buffer_info  *fbi;
    struct clk                *hdmi_isfr_clock;
    struct clk                *hdmi_iahb_clock;
    ...
    uint8_t               edid[HDMI_EDID_LEN];
    uint8_t              *override_edid;
...
extern void                      mxc_hdmi_cec_handle(uint16_t cec_stat);

static void mxc_hdmi_setup(struct mxc_hdmi *hdmi, uint64_t event);
...
static const struct of_device_id imx_hdmi_device_tree_ids[] = {
    {
     .compatible = "fsl,imx6dl-hdmi-video",
     .data       = &imx_hdmi_devtype[IMX6DL_HDMI],
     },
    {
     .compatible = "fsl,imx6q-hdmi-video",
     .data       = &imx_hdmi_devtype[IMX6Q_HDMI],
     },
    {/* sentinel */ }
};
...
#ifdef DEBUG
    pr_debug(
        "fb_videomode = %d %d %d %d %d %d %d %d %d %d %d %d %d\n", m->refresh, m->xres, m->yres, m->pixclock, m->left_margin, m->right_margin,
        m->upper_margin, m->lower_margin, m->hsync_len, m->vsync_len, m->sync, m->vmode, m->flag);
#endif
}
...
{
    return (
        (hdmi->hdmi_data.enc_out_format == YCBCR422_8BITS) && (hdmi->hdmi_data.enc_in_format == RGB || hdmi->hdmi_data.enc_in_format == YCBCR444));
}

static int isColorSpaceInterpolation(struct mxc_hdmi *hdmi)
{
    return (
        (hdmi->hdmi_data.enc_in_format == YCBCR422_8BITS) && (hdmi->hdmi_data.enc_out_format == RGB || hdmi->hdmi_data.enc_out_format == YCBCR444));
}
...
static void update_csc_coeffs(struct mxc_hdmi *hdmi)
{
    ...
    hdmi_writeb((unsigned char)(csc_coeff[0][0] & 0xFF), HDMI_CSC_COEF_A1_LSB);
    hdmi_writeb((unsigned char)(csc_coeff[0][0] >> 8), HDMI_CSC_COEF_A1_MSB);
    hdmi_writeb((unsigned char)(csc_coeff[0][1] & 0xFF), HDMI_CSC_COEF_A2_LSB);
    hdmi_writeb((unsigned char)(csc_coeff[0][1] >> 8), HDMI_CSC_COEF_A2_MSB);
    hdmi_writeb((unsigned char)(csc_coeff[0][2] & 0xFF), HDMI_CSC_COEF_A3_LSB);
    hdmi_writeb((unsigned char)(csc_coeff[0][2] >> 8), HDMI_CSC_COEF_A3_MSB);
    hdmi_writeb((unsigned char)(csc_coeff[0][3] & 0xFF), HDMI_CSC_COEF_A4_LSB);
    hdmi_writeb((unsigned char)(csc_coeff[0][3] >> 8), HDMI_CSC_COEF_A4_MSB);

    hdmi_writeb((unsigned char)(csc_coeff[1][0] & 0xFF), HDMI_CSC_COEF_B1_LSB);
    hdmi_writeb((unsigned char)(csc_coeff[1][0] >> 8), HDMI_CSC_COEF_B1_MSB);
    hdmi_writeb((unsigned char)(csc_coeff[1][1] & 0xFF), HDMI_CSC_COEF_B2_LSB);
    hdmi_writeb((unsigned char)(csc_coeff[1][1] >> 8), HDMI_CSC_COEF_B2_MSB);
    hdmi_writeb((unsigned char)(csc_coeff[1][2] & 0xFF), HDMI_CSC_COEF_B3_LSB);
    hdmi_writeb((unsigned char)(csc_coeff[1][2] >> 8), HDMI_CSC_COEF_B3_MSB);
    hdmi_writeb((unsigned char)(csc_coeff[1][3] & 0xFF), HDMI_CSC_COEF_B4_LSB);
    hdmi_writeb((unsigned char)(csc_coeff[1][3] >> 8), HDMI_CSC_COEF_B4_MSB);

    hdmi_writeb((unsigned char)(csc_coeff[2][0] & 0xFF), HDMI_CSC_COEF_C1_LSB);
    hdmi_writeb((unsigned char)(csc_coeff[2][0] >> 8), HDMI_CSC_COEF_C1_MSB);
    hdmi_writeb((unsigned char)(csc_coeff[2][1] & 0xFF), HDMI_CSC_COEF_C2_LSB);
    hdmi_writeb((unsigned char)(csc_coeff[2][1] >> 8), HDMI_CSC_COEF_C2_MSB);
    hdmi_writeb((unsigned char)(csc_coeff[2][2] & 0xFF), HDMI_CSC_COEF_C3_LSB);
    hdmi_writeb((unsigned char)(csc_coeff[2][2] >> 8), HDMI_CSC_COEF_C3_MSB);
    hdmi_writeb((unsigned char)(csc_coeff[2][3] & 0xFF), HDMI_CSC_COEF_C4_LSB);
    hdmi_writeb((unsigned char)(csc_coeff[2][3] >> 8), HDMI_CSC_COEF_C4_MSB);
...
static void hdmi_video_packetize(struct mxc_hdmi *hdmi)
{
    ...
    uint32_t               remap_size    = HDMI_VP_REMAP_YCC422_16bit;
    uint32_t               output_select = HDMI_VP_CONF_OUTPUT_SELECTOR_PP;
    struct hdmi_data_info *hdmi_data     = &hdmi->hdmi_data;
    uint8_t                val;

    if (hdmi_data->enc_out_format == RGB || hdmi_data->enc_out_format == YCBCR444) {
        ...
        if (hdmi_data->enc_color_depth == 0 || hdmi_data->enc_color_depth == 8) {
    ...
    val         = ((color_depth << HDMI_VP_PR_CD_COLOR_DEPTH_OFFSET) & HDMI_VP_PR_CD_COLOR_DEPTH_MASK) |
          ((hdmi_data->pix_repet_factor << HDMI_VP_PR_CD_DESIRED_PR_FACTOR_OFFSET) & HDMI_VP_PR_CD_DESIRED_PR_FACTOR_MASK);
    ...
    if (hdmi_data->pix_repet_factor > 1) {
        ...
        val |= HDMI_VP_CONF_PR_EN_ENABLE | HDMI_VP_CONF_BYPASS_SELECT_PIX_REPEATER;
        hdmi_writeb(val, HDMI_VP_CONF);
    } else { /* data from packetizer block */
        val = hdmi_readb(HDMI_VP_CONF);
        val &= ~(HDMI_VP_CONF_PR_EN_MASK | HDMI_VP_CONF_BYPASS_SELECT_MASK);
        val |= HDMI_VP_CONF_PR_EN_DISABLE | HDMI_VP_CONF_BYPASS_SELECT_VID_PACKETIZER;
        hdmi_writeb(val, HDMI_VP_CONF);
    }
    ...
    if (output_select == HDMI_VP_CONF_OUTPUT_SELECTOR_PP) {
        val = hdmi_readb(HDMI_VP_CONF);
        val &= ~(HDMI_VP_CONF_BYPASS_EN_MASK | HDMI_VP_CONF_PP_EN_ENMASK | HDMI_VP_CONF_YCC422_EN_MASK);
        val |= HDMI_VP_CONF_BYPASS_EN_DISABLE | HDMI_VP_CONF_PP_EN_ENABLE | HDMI_VP_CONF_YCC422_EN_DISABLE;
        hdmi_writeb(val, HDMI_VP_CONF);
    } else if (output_select == HDMI_VP_CONF_OUTPUT_SELECTOR_YCC422) {
        val = hdmi_readb(HDMI_VP_CONF);
        val &= ~(HDMI_VP_CONF_BYPASS_EN_MASK | HDMI_VP_CONF_PP_EN_ENMASK | HDMI_VP_CONF_YCC422_EN_MASK);
        val |= HDMI_VP_CONF_BYPASS_EN_DISABLE | HDMI_VP_CONF_PP_EN_DISABLE | HDMI_VP_CONF_YCC422_EN_ENABLE;
        hdmi_writeb(val, HDMI_VP_CONF);
    } else if (output_select == HDMI_VP_CONF_OUTPUT_SELECTOR_BYPASS) {
        val = hdmi_readb(HDMI_VP_CONF);
        val &= ~(HDMI_VP_CONF_BYPASS_EN_MASK | HDMI_VP_CONF_PP_EN_ENMASK | HDMI_VP_CONF_YCC422_EN_MASK);
        val |= HDMI_VP_CONF_BYPASS_EN_ENABLE | HDMI_VP_CONF_PP_EN_DISABLE | HDMI_VP_CONF_YCC422_EN_DISABLE;
        hdmi_writeb(val, HDMI_VP_CONF);
    } else {
        return;
    }
    ...
    val = hdmi_readb(HDMI_VP_STUFF);
    val &= ~(HDMI_VP_STUFF_PP_STUFFING_MASK | HDMI_VP_STUFF_YCC422_STUFFING_MASK);
    val |= HDMI_VP_STUFF_PP_STUFFING_STUFFING_MODE | HDMI_VP_STUFF_YCC422_STUFFING_STUFFING_MODE;
    hdmi_writeb(val, HDMI_VP_STUFF);
...
static inline void hdmi_phy_test_clear(struct mxc_hdmi *hdmi, unsigned char bit)
{
    uint8_t val = hdmi_readb(HDMI_PHY_TST0);
    val &= ~HDMI_PHY_TST0_TSTCLR_MASK;
    val |= (bit << HDMI_PHY_TST0_TSTCLR_OFFSET) & HDMI_PHY_TST0_TSTCLR_MASK;
    hdmi_writeb(val, HDMI_PHY_TST0);
}

static inline void hdmi_phy_test_enable(struct mxc_hdmi *hdmi, unsigned char bit)
{
    uint8_t val = hdmi_readb(HDMI_PHY_TST0);
    val &= ~HDMI_PHY_TST0_TSTEN_MASK;
    val |= (bit << HDMI_PHY_TST0_TSTEN_OFFSET) & HDMI_PHY_TST0_TSTEN_MASK;
    hdmi_writeb(val, HDMI_PHY_TST0);
}

static inline void hdmi_phy_test_clock(struct mxc_hdmi *hdmi, unsigned char bit)
{
    uint8_t val = hdmi_readb(HDMI_PHY_TST0);
    val &= ~HDMI_PHY_TST0_TSTCLK_MASK;
    val |= (bit << HDMI_PHY_TST0_TSTCLK_OFFSET) & HDMI_PHY_TST0_TSTCLK_MASK;
    hdmi_writeb(val, HDMI_PHY_TST0);
}

static inline void hdmi_phy_test_din(struct mxc_hdmi *hdmi, unsigned char bit)
{
    hdmi_writeb(bit, HDMI_PHY_TST1);
}

static inline void hdmi_phy_test_dout(struct mxc_hdmi *hdmi, unsigned char bit)
{
...
static void hdmi_phy_i2c_write(struct mxc_hdmi *hdmi, unsigned short data, unsigned char addr)
{
    hdmi_writeb(0xFF, HDMI_IH_I2CMPHY_STAT0);
    hdmi_writeb(addr, HDMI_PHY_I2CM_ADDRESS_ADDR);
    hdmi_writeb((unsigned char)(data >> 8), HDMI_PHY_I2CM_DATAO_1_ADDR);
    hdmi_writeb((unsigned char)(data >> 0), HDMI_PHY_I2CM_DATAO_0_ADDR);
    hdmi_writeb(HDMI_PHY_I2CM_OPERATION_ADDR_WRITE, HDMI_PHY_I2CM_OPERATION_ADDR);
...
static bool hdmi_edid_wait_i2c_done(struct mxc_hdmi *hdmi, int msec)
{
    ...
    while (val == 0) {
        ...
        if (msec-- == 0) {
            dev_dbg(hdmi->dev, "HDMI EDID i2c operation time out!!\n");
            return false;
        }
...
static uint8_t hdmi_edid_i2c_read(struct mxc_hdmi *hdmi, uint8_t addr, uint8_t blockno)
{
    uint8_t spointer    = blockno / 2;
    uint8_t edidaddress = ((blockno % 2) * 0x80) + addr;
    uint8_t data;
    ...
    if (spointer == 0) {
        ...
        hdmi_writeb(HDMI_I2CM_OPERATION_READ_EXT, HDMI_I2CM_OPERATION);
    }
...
static void mxc_hdmi_phy_enable_power(uint8_t enable)
{
    hdmi_mask_writeb(enable, HDMI_PHY_CONF0, HDMI_PHY_CONF0_PDZ_OFFSET, HDMI_PHY_CONF0_PDZ_MASK);
}

static void mxc_hdmi_phy_enable_tmds(uint8_t enable)
{
    hdmi_mask_writeb(enable, HDMI_PHY_CONF0, HDMI_PHY_CONF0_ENTMDS_OFFSET, HDMI_PHY_CONF0_ENTMDS_MASK);
}

static void mxc_hdmi_phy_gen2_pddq(uint8_t enable)
{
    hdmi_mask_writeb(enable, HDMI_PHY_CONF0, HDMI_PHY_CONF0_GEN2_PDDQ_OFFSET, HDMI_PHY_CONF0_GEN2_PDDQ_MASK);
}

static void mxc_hdmi_phy_gen2_txpwron(uint8_t enable)
{
    hdmi_mask_writeb(enable, HDMI_PHY_CONF0, HDMI_PHY_CONF0_GEN2_TXPWRON_OFFSET, HDMI_PHY_CONF0_GEN2_TXPWRON_MASK);
}

#if 0
static void mxc_hdmi_phy_gen2_enhpdrxsense(uint8_t enable)
{
...
static void mxc_hdmi_phy_sel_data_en_pol(uint8_t enable)
{
    hdmi_mask_writeb(enable, HDMI_PHY_CONF0, HDMI_PHY_CONF0_SELDATAENPOL_OFFSET, HDMI_PHY_CONF0_SELDATAENPOL_MASK);
}

static void mxc_hdmi_phy_sel_interface_control(uint8_t enable)
{
    hdmi_mask_writeb(enable, HDMI_PHY_CONF0, HDMI_PHY_CONF0_SELDIPIF_OFFSET, HDMI_PHY_CONF0_SELDIPIF_MASK);
}

static int hdmi_phy_configure(struct mxc_hdmi *hdmi, unsigned char pRep, unsigned char cRes, int cscOn)
{
    uint8_t val;
    uint8_t msec;
    ...
    hdmi_writeb(HDMI_PHY_I2CM_SLAVE_ADDR_PHY_GEN2, HDMI_PHY_I2CM_SLAVE_ADDR);
...
static void mxc_hdmi_phy_init(struct mxc_hdmi *hdmi)
{
    ...
    cscon = (isColorSpaceConversion(hdmi) && !hdmi->hdmi_data.video_mode.mDVI);
...
static void hdmi_config_AVI(struct mxc_hdmi *hdmi)
{
    ...
    uint8_t                    pix_fmt;
    uint8_t                    under_scan;
    uint8_t                    act_ratio, coded_ratio, colorimetry, ext_colorimetry;
    ...
    frame_buffer_var_to_videomode(&mode, &hdmi->fbi->var);
     ...
     */
    val = pix_fmt | under_scan | HDMI_FC_AVICONF0_ACTIVE_FMT_INFO_PRESENT | HDMI_FC_AVICONF0_BAR_DATA_NO_DATA;
    ...
    if (hdmi->hdmi_data.enc_out_format == XVYCC444) {
        ...
        if (hdmi->hdmi_data.colorimetry == eITU601) {
            ...
            ext_colorimetry = HDMI_FC_AVICONF2_EXT_COLORIMETRY_XVYCC709;
        }
    ...
    val = HDMI_FC_AVICONF2_IT_CONTENT_NO_DATA | ext_colorimetry | HDMI_FC_AVICONF2_RGB_QUANT_DEFAULT | HDMI_FC_AVICONF2_SCALING_NONE;
    ...
    val = (((hdmi->hdmi_data.video_mode.mPixelRepetitionInput + 1) << HDMI_FC_PRCONF_INCOMING_PR_FACTOR_OFFSET) &
          ...
          ((hdmi->hdmi_data.video_mode.mPixelRepetitionOutput << HDMI_FC_PRCONF_OUTPUT_PR_FACTOR_OFFSET) & HDMI_FC_PRCONF_OUTPUT_PR_FACTOR_MASK);
    hdmi_writeb(val, HDMI_FC_PRCONF);

    /* IT Content and quantization range = don't care */
    val = HDMI_FC_AVICONF3_IT_CONTENT_TYPE_GRAPHICS | HDMI_FC_AVICONF3_QUANT_RANGE_LIMITED;
...
{
    uint8_t                   inv_val;
    struct frame_buffer_info *fbi = hdmi->fbi;
    struct fb_videomode       fb_mode;
    struct hdmi_vmode        *vmode = &hdmi->hdmi_data.video_mode;
    int                       hblank, vblank;

    frame_buffer_var_to_videomode(&fb_mode, &fbi->var);
    ...
    vmode->mPixelClock    = (fb_mode.xres + fb_mode.left_margin + fb_mode.right_margin + fb_mode.hsync_len) *
                         (fb_mode.yres + fb_mode.upper_margin + fb_mode.lower_margin + fb_mode.vsync_len) * fb_mode.refresh;

    dev_dbg(hdmi->dev, "final pixclk = %d\n", vmode->mPixelClock);

    /* Set up HDMI_FC_INVIDCONF */
    inv_val = (vmode->mVSyncPolarity ? HDMI_FC_INVIDCONF_VSYNC_IN_POLARITY_ACTIVE_HIGH : HDMI_FC_INVIDCONF_VSYNC_IN_POLARITY_ACTIVE_LOW);

    inv_val |= (vmode->mHSyncPolarity ? HDMI_FC_INVIDCONF_HSYNC_IN_POLARITY_ACTIVE_HIGH : HDMI_FC_INVIDCONF_HSYNC_IN_POLARITY_ACTIVE_LOW);
    ...
    if (hdmi->vic == 39) {
        ...
        inv_val |= (vmode->mInterlaced ? HDMI_FC_INVIDCONF_R_V_BLANK_IN_OSC_ACTIVE_HIGH : HDMI_FC_INVIDCONF_R_V_BLANK_IN_OSC_ACTIVE_LOW);
    }

    inv_val |= (vmode->mInterlaced ? HDMI_FC_INVIDCONF_IN_I_P_INTERLACED : HDMI_FC_INVIDCONF_IN_I_P_PROGRESSIVE);

    inv_val |= (vmode->mDVI ? HDMI_FC_INVIDCONF_DVI_MODEZ_DVI_MODE : HDMI_FC_INVIDCONF_DVI_MODEZ_HDMI_MODE);
    ...
    hblank = fb_mode.left_margin + fb_mode.right_margin + fb_mode.hsync_len;
    hdmi_writeb(hblank >> 8, HDMI_FC_INHBLANK1);
    hdmi_writeb(hblank, HDMI_FC_INHBLANK0);

    /* Set up vertical blanking pixel region width */
    vblank = fb_mode.upper_margin + fb_mode.lower_margin + fb_mode.vsync_len;
...
static int mxc_edid_override(
    struct mxc_hdmi *hdmi, unsigned char *edid, struct mxc_edid_cfg *cfg, struct frame_buffer_info *fbi, unsigned char *override, int override_length)
{
    int extblknum;
    int j, ret;
    ...
    if ((override_length < 128) || (override[0] != 0x00) || (override[1] != 0xFF) || (override[2] != 0xFF)) {
    ...
    if (extblknum) {
        ret = mxc_edid_parse_ext_block(edid + EDID_LENGTH, cfg, &fbi->monspecs);
    ...
    if (extblknum > 1) {
        for (j = 2; j <= extblknum; j++) {
            ...
            ret = mxc_edid_parse_ext_block(&override[j * 128], cfg, &fbi->monspecs);
...
static int mxc_edid_read_internal(struct mxc_hdmi *hdmi, unsigned char *edid, struct mxc_edid_cfg *cfg, struct frame_buffer_info *fbi)
{
    int            extblknum;
    int            i, j, ret;
    ...
    hdmi_writeb(HDMI_I2CM_CTLINT_NAC_POL | HDMI_I2CM_CTLINT_ARBITRATION_POL, HDMI_I2CM_CTLINT);
    ...
    /* Check first three byte of EDID head */
    if (!(hdmi_edid_i2c_read(hdmi, 0, 0) == 0x00) || !(hdmi_edid_i2c_read(hdmi, 1, 0) == 0xFF) || !(hdmi_edid_i2c_read(hdmi, 2, 0) == 0xFF)) {
        dev_info(hdmi->dev, "EDID head check failed!");
        return -ENOENT;
    }
    ...
    if (extblknum > 1) {
        for (j = 1; j <= extblknum; j++) {
            ...
            /* edid ext block parsing */
            ret = mxc_edid_parse_ext_block(tmpedid + EDID_LENGTH, cfg, &fbi->monspecs);
...
{
    int                      ret = -EINVAL;
    uint8_t                  edid_old[HDMI_EDID_LEN];
    uint8_t                  clkdis;
    ...
    if (hdmi->override_edid) {
        ret = mxc_edid_override(hdmi, hdmi->edid, &hdmi->edid_cfg, hdmi->fbi, hdmi->override_edid, hdmi->override_edid_length);
    }

    if (!ret) {
    } else if (!hdcp_init) {
        ret = mxc_edid_read(hdmi_i2c->adapter, hdmi_i2c->addr, hdmi->edid, &hdmi->edid_cfg, hdmi->fbi);
    } else {
        ...
        ret = mxc_edid_read_internal(hdmi, hdmi->edid, &hdmi->edid_cfg, hdmi->fbi);
...
static void hdmi_enable_audio_clock(struct mxc_hdmi *hdmi)
{
    uint8_t clkdis;
...
static void mxc_hdmi_clear_overflow(struct mxc_hdmi *hdmi)
{
    ...
    /* TMDS software reset */
    hdmi_writeb((uint8_t)~HDMI_MC_SWRSTZ_TMDSSWRST_REQ, HDMI_MC_SWRSTZ);
...
{
    pr_debug("%s\n", __func__);
    hdmi_writeb(HDMI_IH_MUTE_FC_STAT2_OVERFLOW_MASK, HDMI_IH_MUTE_FC_STAT2);
...
static void mxc_hdmi_edid_rebuild_modelist(struct mxc_hdmi *hdmi)
{
    ...
    for (i = 0; i < hdmi->fbi->monspecs.modedb_len; i++) {
        ...
        if (!(mode->vmode & FB_VMODE_INTERLACED) && (!only_cea || mxc_edid_mode_to_vic(mode))) {

            dev_dbg(hdmi->dev, "Added mode %d:", i);
            dev_dbg(
                hdmi->dev, "xres = %d, yres = %d, freq = %d, vmode = %d, flag = %d\n", hdmi->fbi->monspecs.modedb[i].xres,
                hdmi->fbi->monspecs.modedb[i].yres, hdmi->fbi->monspecs.modedb[i].refresh, hdmi->fbi->monspecs.modedb[i].vmode,
...
static void mxc_hdmi_set_mode(struct mxc_hdmi *hdmi)
{
    ...
    frame_buffer_var_to_videomode(&m, &var);
    dump_fb_videomode(&m);
    ...
    if (fb_mode_is_equal(&hdmi->previous_non_vga_mode, mode)) {
        dev_dbg(hdmi->dev, "%s: Video mode same as previous\n", __func__);
...
static void mxc_hdmi_cable_connected(struct mxc_hdmi *hdmi)
{
    ...
    if (edid_status == HDMI_EDID_NO_MODES || edid_status == HDMI_EDID_FAIL) {
...
static void hotplug_worker(struct work_struct *work)
{
    ...
    struct mxc_hdmi     *hdmi       = container_of(delay_work, struct mxc_hdmi, hotplug_work);
    uint32_t             hdmi_phy_stat0;
    uint32_t             hdmi_phy_mask0;
    uint64_t             flags;
    int                  isalive = 0;
    ...
    spin_lock_irq_save(&hdmi->irq_lock, flags);
    ...
    spin_unlock_irq_restore(&hdmi->irq_lock, flags);
}
...
static void mxc_hdmi_setup(struct mxc_hdmi *hdmi, uint64_t event)
{
    ...
    frame_buffer_var_to_videomode(&m, &hdmi->fbi->var);
    dump_fb_videomode(&m);
    ...
    if (!hdmi->requesting_vga_for_initialization) {
         ...
         * vga default. */
        memcpy(&hdmi->previous_non_vga_mode, &m, sizeof(struct fb_videomode));
    ...
    if ((hdmi->vic == 6) || (hdmi->vic == 7) || (hdmi->vic == 21) || (hdmi->vic == 22) || (hdmi->vic == 2) || (hdmi->vic == 3) || (hdmi->vic == 17) ||
        (hdmi->vic == 18)) {
    ...
    if ((hdmi->vic == 10) || (hdmi->vic == 11) || (hdmi->vic == 12) || (hdmi->vic == 13) || (hdmi->vic == 14) || (hdmi->vic == 15) ||
        (hdmi->vic == 25) || (hdmi->vic == 26) || (hdmi->vic == 27) || (hdmi->vic == 28) || (hdmi->vic == 29) || (hdmi->vic == 30) ||
        (hdmi->vic == 35) || (hdmi->vic == 36) || (hdmi->vic == 37) || (hdmi->vic == 38)) {
    ...
    if (!hdmi->hdmi_data.video_mode.mDVI && !hdmi->hdmi_data.rgb_out_enable) {
    ...
    if (hdmi->hdmi_data.video_mode.mDVI) {
        ...
        hdmi_clock_regenerator_update_pixel_clock(hdmi->fbi->var.pixclock);
        hdmi_enable_audio_clock(hdmi);
...
static void mxc_hdmi_fb_registered(struct mxc_hdmi *hdmi)
{
    ...
    hdmi_writeb(HDMI_PHY_I2CM_INT_ADDR_DONE_POL, HDMI_PHY_I2CM_INT_ADDR);

    hdmi_writeb(HDMI_PHY_I2CM_CTLINT_ADDR_NAC_POL | HDMI_PHY_I2CM_CTLINT_ADDR_ARBITRATION_POL, HDMI_PHY_I2CM_CTLINT_ADDR);

    /* enable cable hot plug irq */
    hdmi_writeb((uint8_t)~HDMI_DVI_STAT, HDMI_PHY_MASK0);
...
static int mxc_hdmi_fb_event(struct notifier_block *nb, uint64_t val, void *v)
{
    ...
    switch (val) {
        ...
        case FB_EVENT_BLANK:
            if ((*((int *)event->data) == FB_BLANK_UNBLANK) && (*((int *)event->data) != hdmi->blank)) {
                dev_dbg(hdmi->dev, "event=FB_EVENT_BLANK - UNBLANK\n");
                ...
                dev_dbg(hdmi->dev, "event=FB_EVENT_BLANK - BLANK\n");
                ...
                dev_dbg(hdmi->dev, "FB BLANK state no changed!\n");
            }

            break;

        case FB_EVENT_SUSPEND:
            dev_dbg(hdmi->dev, "event=FB_EVENT_SUSPEND\n");

            if (hdmi->blank == FB_BLANK_UNBLANK) {
                mxc_hdmi_phy_disable(hdmi);
                clock_disable(hdmi->hdmi_iahb_clock);
                clock_disable(hdmi->hdmi_isfr_clock);
            }

            break;

        case FB_EVENT_RESUME:
            dev_dbg(hdmi->dev, "event=FB_EVENT_RESUME\n");

            if (hdmi->blank == FB_BLANK_UNBLANK) {
                clock_enable(hdmi->hdmi_iahb_clock);
                clock_enable(hdmi->hdmi_isfr_clock);
                vmm_lnotice(NULL, "%s()\n", __func__);
                mxc_hdmi_phy_init(hdmi);
            }
...
static inline void hdmi_hdcp_get_property(const vmm_device_tree_node_t *node)
{
    uint8_t init = 0;

    /* Check hdcp enable by dts */
    vmm_device_tree_read_u8_atindex(node, "fsl,hdcp", &init, 0);
    hdcp_init = !!init;
}
...
{
    int      ret;
    uint32_t phy_reg_vlev = 0, phy_reg_cksymtx = 0;
...
static int mxc_hdmi_disp_init(struct mxc_dispdrv_handle *disp, struct mxc_dispdrv_setting *setting)
{
    int                        ret = 0;
    uint32_t                   i;
    const struct fb_videomode *mode;
    ...
    hdmi->hdmi_isfr_clock = devm_clock_get(hdmi->dev, "hdmi_isfr");

    if (IS_ERR(hdmi->hdmi_isfr_clock)) {
        ret = PTR_ERR(hdmi->hdmi_isfr_clock);
        dev_err(hdmi->dev, "Unable to get HDMI clk: %d\n", ret);
        goto egetclk1;
    }

    ret = clock_prepare_enable(hdmi->hdmi_isfr_clock);
    ...
    hdmi->hdmi_iahb_clock = devm_clock_get(hdmi->dev, "hdmi_iahb");

    if (IS_ERR(hdmi->hdmi_iahb_clock)) {
        ret = PTR_ERR(hdmi->hdmi_iahb_clock);
        dev_err(hdmi->dev, "Unable to get HDMI clk: %d\n", ret);
        goto egetclk2;
    }

    ret = clock_prepare_enable(hdmi->hdmi_iahb_clock);
    ...
    dev_info(
        hdmi->dev, "Detected HDMI controller 0x%x:0x%x:0x%x:0x%x\n", hdmi_readb(HDMI_DESIGN_ID), hdmi_readb(HDMI_REVISION_ID),
        hdmi_readb(HDMI_PRODUCT_ID0), hdmi_readb(HDMI_PRODUCT_ID1));

    /* To prevent overflows in HDMI_IH_FC_STAT2, set the clk regenerator
     * N and cts values before enabling phy */
    hdmi_init_clock_regenerator();
    ...
    fb_find_mode(&hdmi->fbi->var, hdmi->fbi, hdmi->dft_mode_str, NULL, 0, NULL, hdmi->default_bpp);
...
ereqirq:
    clock_disable_unprepare(hdmi->hdmi_iahb_clock);
erate2:
    clock_put(hdmi->hdmi_iahb_clock);
egetclk2:
    clock_disable_unprepare(hdmi->hdmi_isfr_clock);
erate1:
    clock_put(hdmi->hdmi_isfr_clock);
egetclk1:
...
static void mxc_hdmi_disp_deinit(struct mxc_dispdrv_handle *disp)
{
    ...
    vmm_device_driver_unregister_device(hdmi->dev);
}
...
static int mxc_hdmi_probe(vmm_device_t *dev)
{
    struct mxc_hdmi *hdmi;
    int              ret = 0;
    ...
    ret       = vmm_device_tree_request_regmap(dev->of_node, &hdmi->gpr_base, 0, "MXC HDMI");
    ...
    hdmi->irq = vmm_device_tree_irq_parse_map(dev->of_node, 0);
    ...
    hdmi->gpr_hdmi_base = (void *)(hdmi->gpr_base + (3 * sizeof(uint32_t)));
    ...
    vmm_device_driver_set_data(dev, hdmi);
...
static int mxc_hdmi_remove(vmm_device_t *dev)
{
    struct mxc_hdmi *hdmi = vmm_device_driver_get_data(dev);

    fb_unregister_client(&hdmi->nb);

    mxc_dispdrv_puthandle(hdmi->disp_mxc_hdmi);
    mxc_dispdrv_unregister(hdmi->disp_mxc_hdmi);
    vmm_device_tree_regunmap_release(dev->of_node, hdmi->gpr_base, 0);
...
static vmm_driver_t mxc_hdmi_driver = {
    .name        = "mxc_hdmi",
    .probe       = mxc_hdmi_probe,
    .remove      = mxc_hdmi_remove,
    .match_table = imx_hdmi_device_tree_ids,
};

static int __init mxc_hdmi_init(void)
{
    return vmm_device_driver_register_driver(&mxc_hdmi_driver);
}

static void __exit mxc_hdmi_exit(void)
{
    vmm_device_driver_unregister_driver(&mxc_hdmi_driver);
}
