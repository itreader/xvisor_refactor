...
static inline void color_imageblit(
    const struct frame_buffer_image *image, struct frame_buffer_info *p, uint8_t *dst1, uint32_t start_index, uint32_t pitch_index)
{
    /* Draw the penguin */
    uint32_t      *dst, *dst2;
    ...
    for (i = image->height; i--;) {
        ...
        if (start_index) {
            uint32_t start_mask = ~fb_shifted_pixels_mask_u32(p, start_index, bswapmask);
        ...
        if (shift) {
            uint32_t end_mask = fb_shifted_pixels_mask_u32(p, shift, bswapmask);
...
static inline void slow_imageblit(
    const struct frame_buffer_image *image, struct frame_buffer_info *p, uint8_t *dst1, uint32_t fgcolor, uint32_t bgcolor, uint32_t start_index,
    uint32_t pitch_index)
{
    ...
    const uint8_t *src       = (uint8_t *)image->data, *s;
    uint32_t       i, j, l;
    uint32_t       bswapmask = fb_compute_bswapmask(p);
...
static inline void fast_imageblit(
    const struct frame_buffer_image *image, struct frame_buffer_info *p, uint8_t *dst1, uint32_t fgcolor, uint32_t bgcolor)
{
...
void cframe_buffer_imageblit(struct frame_buffer_info *p, const struct frame_buffer_image *image)
{
    ...
    bitstart /= 8;
    bitstart &= ~(bpl - 1);
    dst1 = (uint8_t *)(p->screen_base + bitstart);
