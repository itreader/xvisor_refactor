...
const struct fb_videomode mxc_cea_mode[64] = {
    ...
    [1] =
        {
             ...
             10, 96,
             2, 0,
             FB_VMODE_NONINTERLACED | FB_VMODE_ASPECT_4_3,
             0, },
    ...
    [3] =
        {
             ...
             9, 62,
             6, 0,
             FB_VMODE_NONINTERLACED | FB_VMODE_ASPECT_16_9,
             0, },
    /* #4: 1280x720p@59.94/60Hz 16:9 */
    [4] =
        {NULL, 60, 1280, 720, 13468, 220, 110, 20, 5, 40, 5, FB_SYNC_HOR_HIGH_ACT | FB_SYNC_VERT_HIGH_ACT,
    ...
    [5] =
        {
             ...
             FB_VMODE_INTERLACED | FB_VMODE_ASPECT_16_9,
             0, },
    ...
    [6] =
        {
             ...
             15, 4,
             124, 3,
             0, FB_VMODE_INTERLACED | FB_VMODE_ASPECT_4_3,
             0, },
    ...
    [7] =
        {
             ...
             15, 4,
             124, 3,
             0, FB_VMODE_INTERLACED | FB_VMODE_ASPECT_16_9,
             0, },
    ...
    [41] =
        {NULL, 100, 1280, 720, 6734, 220, 440, 20, 5, 40, 5, FB_SYNC_HOR_HIGH_ACT | FB_SYNC_VERT_HIGH_ACT,
             FB_VMODE_NONINTERLACED | FB_VMODE_ASPECT_16_9, 0},
    /* #47: 1280x720p@119.88/120Hz 16:9 */
    [47] =
        {NULL, 120, 1280, 720, 6734, 220, 110, 20, 5, 40, 5, FB_SYNC_HOR_HIGH_ACT | FB_SYNC_VERT_HIGH_ACT,
             FB_VMODE_NONINTERLACED | FB_VMODE_ASPECT_16_9, 0},
};
 ...
 */
int mxc_edid_fb_mode_is_equal(bool use_aspect, const struct fb_videomode *mode1, const struct fb_videomode *mode2)
{
    uint32_t mask;
    ...
    return (
        mode1->xres == mode2->xres && mode1->yres == mode2->yres && mode1->hsync_len == mode2->hsync_len && mode1->vsync_len == mode2->vsync_len &&
        mode1->left_margin == mode2->left_margin && mode1->right_margin == mode2->right_margin && mode1->upper_margin == mode2->upper_margin &&
        mode1->lower_margin == mode2->lower_margin && mode1->sync == mode2->sync &&
        /* refresh check, 59.94Hz and 60Hz have the same parameter
         * in struct of mxc_cea_mode */
        abs(mode1->refresh - mode2->refresh) <= 1 && (mode1->vmode & mask) == (mode2->vmode & mask));
}
...
int mxc_edid_parse_ext_block(unsigned char *edid, struct mxc_edid_cfg *cfg, struct fb_monspecs *specs)
{
            ...
            switch (tagcode) {
                case 0x2: /*Video data block*/
                {
                    ...
                    while (i < blklen) {
                        ...
                        if (cea_idx < ARRAY_SIZE(mxc_cea_mode) && (mxc_cea_mode[cea_idx].xres)) {
                ...
                case 0x3: /*Vendor specific data*/
                {
                    ...
                    if ((IEEE_reg_iden[0] == 0x03) && (IEEE_reg_iden[1] == 0x0c) && (IEEE_reg_iden[2] == 0x00)) {
                        cfg->hdmi_cap = 1;
                    }
                    ...
                    if (blklen > 6) {
                        cfg->vsd_max_tmdsclock_rate = edid[index + 7] * 5;
                    }

                    DPRINTK("VSD MAX TMDS CLOCK RATE %d\n", cfg->vsd_max_tmdsclock_rate);
                        ...
                        if (hdmi_3d_len > 0) {
                            ...
                            if ((hdmi_3d_len - index_inc) > 0) {
                                ...
                                while ((hdmi_3d_len - index_inc) > 0) {
                                    ...
                                    DPRINTK(
                                        "vic_order_2d=%d, 3d_struct=%d, 3d_detail=0x%x\n", cfg->hdmi_3d_format[i].vic_order_2d,
                                        cfg->hdmi_3d_format[i].struct_3d, cfg->hdmi_3d_format[i].detail_3d);
                                    i++;
                                }
                            }
                ...
                {
                    uint8_t audio_format, max_ch, byte1, byte2, byte3;
    ...
    m = kmalloc((num + specs->modedb_len) * sizeof(struct fb_videomode), GFP_KERNEL);
...
EXPORT_SYMBOL(mxc_edid_parse_ext_block);

static int mxc_edid_readblk(struct i2c_adapter *adp, unsigned short addr, unsigned char *edid)
{
...
static int mxc_edid_readsegblk(struct i2c_adapter *adp, unsigned short addr, unsigned char *edid, int seg_num)
{
    int            ret     = 0;
...
int mxc_edid_var_to_vic(struct frame_buffer_var_screeninfo *var)
{
    int                 i;
...
int mxc_edid_read(struct i2c_adapter *adp, unsigned short addr, unsigned char *edid, struct mxc_edid_cfg *cfg, struct frame_buffer_info *fbi)
{
    int ret = 0, extblknum;
    ...
    if (extblknum) {
        ...
        if (extblknum > 1) {
            ret = mxc_edid_readsegblk(adp, addr, edid + EDID_LENGTH * 2, extblknum - 1);
        ...
        for (i = 1; i <= extblknum; i++) {
            ...
            mxc_edid_parse_ext_block(edid + i * EDID_LENGTH, cfg, &fbi->monspecs);
        }
    }
