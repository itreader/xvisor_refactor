 ...
 * @brief Frame buffer management framework
 *
...
static void get_frame_buffer_info(struct frame_buffer_info *frame_buffer_info)
{
    if (!frame_buffer_info) {
        return;
    }

    arch_atomic_add(&frame_buffer_info->count, 1);
}

static void put_frame_buffer_info(struct frame_buffer_info *frame_buffer_info)
{
    if (arch_atomic_sub_return(&frame_buffer_info->count, 1)) {
        return;
    }

    if (frame_buffer_info->fbops->fb_destroy) {
        frame_buffer_info->fbops->fb_destroy(frame_buffer_info);
    }
}

static int fb_check_foreignness(struct frame_buffer_info *fi)
{
...
static bool fb_do_apertures_overlap(struct apertures_struct *gena, struct apertures_struct *hwa)
{
    ...
    for (i = 0; i < hwa->count; ++i) {
        ...
        for (j = 0; j < gena->count; ++j) {
            ...
            vmm_printf(
                "checking generic (%llx %llx) vs hw (%llx %llx)\n", (uint64_t)g->base, (uint64_t)g->size, (uint64_t)h->base, (uint64_t)h->size);
...
static int fb_check_caps(struct frame_buffer_info *info, struct frame_buffer_var_screeninfo *var, uint32_t activate)
{
...
int fb_set_var(struct frame_buffer_info *info, struct frame_buffer_var_screeninfo *var)
{
    ...
    if (var->activate & FB_ACTIVATE_INV_MODE) {
        ...
        frame_buffer_var_to_videomode(&mode1, var);
        frame_buffer_var_to_videomode(&mode2, &info->var);
    ...
    if ((var->activate & FB_ACTIVATE_FORCE) || memcmp(&info->var, var, sizeof(struct frame_buffer_var_screeninfo))) {
        uint32_t activate = var->activate;
        ...
        if ((var->activate & FB_ACTIVATE_MASK) == FB_ACTIVATE_NOW) {
            struct frame_buffer_var_screeninfo old_var;
            ...
            frame_buffer_var_to_videomode(&mode, &info->var);
            ...
            if (!ret && (flags & FBINFO_MISC_USEREVENT)) {
                struct fb_event event;
                int             evnt = (activate & FB_ACTIVATE_ALL) ? FB_EVENT_MODE_CHANGE_ALL : FB_EVENT_MODE_CHANGE;
...
{
    if (!lock_frame_buffer_info(info)) {
        return;
    }
...
int fb_get_color_depth(struct frame_buffer_var_screeninfo *var, struct frame_buffer_fix_screeninfo *fix)
{
    ...
    if (fix->visual == FB_VISUAL_MONO01 || fix->visual == FB_VISUAL_MONO10) {
        ...
        if (var->green.length == var->blue.length && var->green.length == var->red.length && var->green.offset == var->blue.offset &&
    ...
    void *private)
{
    ...
    if (!list_empty(&info->user_list)) {
        ...
        if (user->save) {
            user->save(info, user->private);
        }
    }

    /* Increment open refcount */
    get_frame_buffer_info(info);
...
int fb_release(struct frame_buffer_info *info)
{
    ...
    if (!list_empty(&info->user_list)) {
        user = list_entry(list_pop(&info->user_list), struct fb_user, head);
        vmm_free(user);
    }
    ...
    if (!list_empty(&info->user_list)) {
        ...
        if (user->restore) {
            user->restore(info, user->private);
        }
    }
...
#define PADDING        (BYTES_PER_LONG - (sizeof(struct frame_buffer_info) % BYTES_PER_LONG))
    int                       frame_buffer_info_size = sizeof(struct frame_buffer_info);
    struct frame_buffer_info *info;
    char                     *p;

    if (size) {
        frame_buffer_info_size += PADDING;
    }

    p = vmm_zalloc(frame_buffer_info_size + size);

    if (!p) {
        return NULL;
    }

    info = (struct frame_buffer_info *)p;

    if (size) {
        info->par = p + frame_buffer_info_size;
    }
...
void fb_remove_conflicting_framebuffers(struct apertures_struct *a, const char *name, bool primary)
{
    ...
    while (1) {
        ...
        for (i = 0; i < count; i++) {
            ...
            if (fb_do_apertures_overlap(gen_aper, a) || (primary && gen_aper && gen_aper->count && gen_aper->ranges[0].base == VGA_FB_PHYS)) {
...
int fb_register(struct frame_buffer_info *info)
{
    ...
    fb_remove_conflicting_framebuffers(info->apertures, info->fix.id, FALSE);
    ...
    if (!info->pixmap.blit_x) {
        info->pixmap.blit_x = ~(uint32_t)0;
    }

    if (!info->pixmap.blit_y) {
        info->pixmap.blit_y = ~(uint32_t)0;
    }
    ...
    vmm_snprintf(info->name, sizeof(info->name), "fb%d", vmm_device_driver_class_device_count(&fb_class));
    ...
    vmm_device_driver_set_data(&info->dev, info);

    rc = vmm_device_driver_register_device(&info->dev);
...
int fb_unregister(struct frame_buffer_info *info)
{
    ...
    if (info->pixmap.addr && (info->pixmap.flags & FB_PIXMAP_DEFAULT)) {
        vmm_free(info->pixmap.addr);
    }
...
struct frame_buffer_info *fb_find(const char *name)
{
    ...
    dev = vmm_device_driver_class_find_device_by_name(&fb_class, name);
...
struct fb_iterate_priv {
    void *data;
    int (*fn)(struct frame_buffer_info *info, void *data);
};

static int __fb_iterate(vmm_device_t *dev, void *data)
{
    struct fb_iterate_priv   *p    = data;
    struct frame_buffer_info *info = vmm_device_driver_get_data(dev);
...
static int __fb_get(struct frame_buffer_info *info, void *data)
{
...
{
    return vmm_device_driver_register_class(&fb_class);
}

static void __exit fb_exit(void)
{
    vmm_device_driver_unregister_class(&fb_class);
}
