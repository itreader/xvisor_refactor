...
struct simple_frame_buffer_info {
...
struct simple_fb_data {
    uint32_t reg, width, height, depth, stride;
    char     status[8];
    char     format[16];
};

struct simple_fb {
    struct frame_buffer_info        fb;
    struct platform_device         *dev;
    struct simple_fb_data           data;
    struct simple_frame_buffer_info info;
    virtual_addr_t                  dma;
    uint32_t                        cmap[16];
};
...
static int simple_fb_set_bitfields(struct frame_buffer_var_screeninfo *var)
{
    int ret = 0;
...
static int simple_fb_check_var(struct frame_buffer_var_screeninfo *var, struct frame_buffer_info *info)
{
    ...
    /* info input, var output */
    DPRINTF(
        "%s: info(%p) %dx%d (%dx%d), %d, %d\n", __func__, info, info->var.xres, info->var.yres, info->var.xres_virtual, info->var.yres_virtual,
        (int)info->screen_size, info->var.bits_per_pixel);
    DPRINTF("%s: var(%p) %dx%d (%dx%d), %d\n", __func__, var, var->xres, var->yres, var->xres_virtual, var->yres_virtual, var->bits_per_pixel);
...
static int simple_fb_set_par(struct frame_buffer_info *info)
{
    struct simple_fb                *fb     = to_simple(info);
    struct simple_frame_buffer_info *fbinfo = &fb->info;
    ...
    DPRINTF(
        "%s: start= %p,%p width=%d, height=%d, bpp=%d, "
        "pitch=%d size=%d\n",
        __func__, (void *)fb->fb.screen_base, (void *)fb->fb.fix.smem_start, fbinfo->xres, fbinfo->yres, fbinfo->bpp, fbinfo->pitch,
        fb->fb.screen_size);
...
static int simple_fb_setcolreg(uint32_t regno, uint32_t red, uint32_t green, uint32_t blue, uint32_t transp, struct frame_buffer_info *info)
{
    struct simple_fb *fb = to_simple(info);

    DPRINTF("%s: setcolreg %d:(%02x,%02x,%02x,%02x) %x\n", __func__, regno, red, green, blue, transp, fb->fb.fix.visual);

    if (fb->fb.var.bits_per_pixel <= 8) {
        if (regno < 256) {
            /* blue [0:4], green [5:10], red [11:15] */
            fb->info.cmap[regno] = ((red >> (16 - 5)) & 0x1f) << 11 | ((green >> (16 - 6)) & 0x3f) << 5 | ((blue >> (16 - 5)) & 0x1f) << 0;
        }
...
static int simple_fb_blank(int blank_mode, struct frame_buffer_info *info)
{
    uint32_t i, screen_size;
...
static int simple_fb_probe(vmm_device_t *dev)
{
    int               ret = VMM_OK;
    struct simple_fb *fb;
    const char       *format;
    ...
    ret     = vmm_device_tree_request_regmap(dev->of_node, &fb->dma, 0, "SIMPLE_FB");
...
static int simple_fb_remove(vmm_device_t *dev)
{
...
static struct vmm_device_tree_nodeid simple_fb_devid_table[] = {
    {.compatible = "simple-framebuffer"},
    {/* end of list */},
};

static vmm_driver_t simple_fb_driver = {
    .name        = "simple_fb",
...
{
    return vmm_device_driver_register_driver(&simple_fb_driver);
}

static void __exit simple_fb_exit(void)
{
    vmm_device_driver_unregister_driver(&simple_fb_driver);
}
