...
static int sram_probe(vmm_device_t *dev)
{
    void            *virt_base = NULL;
    ...
    ret                        = vmm_device_tree_regaddr(dev->of_node, &start, 0);
    ...
    ret = vmm_device_tree_regsize(dev->of_node, &size, 0);
    ...
    sram->clk = devm_clock_get(dev, NULL);

    if (VMM_IS_ERR(sram->clk)) {
        sram->clk = NULL;
    } else {
        clock_prepare_enable(sram->clk);
    }
    ...
    vmm_device_driver_set_data(dev, sram);

    vmm_printf("%s: SRAM pool: %" PRISIZE " KiB @ 0x%p\n", dev->name, size / 1024, virt_base);
...
err_out:
    ...
    if (sram->clk)
        clock_disable_unprepare(sram->clk);
...
static int sram_remove(vmm_device_t *dev)
{
    struct sram_dev *sram = vmm_device_driver_get_data(dev);
...
static struct vmm_device_tree_nodeid sram_device_tree_ids[] = {{.compatible = "mmio-sram"}, {}};

static vmm_driver_t sram_driver                             = {
                                .name        = "sram",
                                .match_table = sram_device_tree_ids,
                                .probe       = sram_probe,
                                .remove      = sram_remove,
};
...
{
    return vmm_device_driver_register_driver(&sram_driver);
}

static void __exit sram_exit(void)
{
    vmm_device_driver_unregister_driver(&sram_driver);
}
