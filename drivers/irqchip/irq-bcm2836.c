...
static void bcm2836_arm_irqchip_mask_per_cpu_irq(uint32_t reg_offset, uint32_t bit, int cpu)
{
...
static void bcm2836_arm_irqchip_unmask_per_cpu_irq(uint32_t reg_offset, uint32_t bit, int cpu)
{
    void *reg = intc.base + reg_offset + 4 * cpu;
...
static void bcm2836_arm_irqchip_register_irq(uint32_t hwirq, bool is_ipi, struct vmm_host_irq_chip *chip)
{
    ...
    vmm_host_irq_set_handler(irq, vmm_handle_per_cpu_irq);
}
...
static struct vmm_host_irq_domain_ops bcm2836_intc_ops = {
    .xlate = vmm_host_irq_domain_xlate_onecell,
};

static int __init bcm2836_intc_init(vmm_device_tree_node_t *node)
{
    int      rc, i;
    uint32_t irq_start = 0;
    ...
    bcm2836_arm_irqchip_register_irq(LOCAL_IRQ_CNTPSIRQ, FALSE, &bcm2836_arm_irqchip_timer);
    bcm2836_arm_irqchip_register_irq(LOCAL_IRQ_CNTPNSIRQ, FALSE, &bcm2836_arm_irqchip_timer);
    bcm2836_arm_irqchip_register_irq(LOCAL_IRQ_CNTHPIRQ, FALSE, &bcm2836_arm_irqchip_timer);
    bcm2836_arm_irqchip_register_irq(LOCAL_IRQ_CNTVIRQ, FALSE, &bcm2836_arm_irqchip_timer);
    /* Mailbox0 used for SMP spin loop so we don't use it for IPIs */
    bcm2836_arm_irqchip_register_irq(LOCAL_IRQ_MAILBOX0, FALSE, &bcm2836_arm_irqchip_mbox);
    bcm2836_arm_irqchip_register_irq(LOCAL_IRQ_MAILBOX1, TRUE, &bcm2836_arm_irqchip_mbox);
    bcm2836_arm_irqchip_register_irq(LOCAL_IRQ_MAILBOX2, TRUE, &bcm2836_arm_irqchip_mbox);
    bcm2836_arm_irqchip_register_irq(LOCAL_IRQ_MAILBOX3, TRUE, &bcm2836_arm_irqchip_mbox);
    bcm2836_arm_irqchip_register_irq(LOCAL_IRQ_GPU_FAST, FALSE, &bcm2836_arm_irqchip_gpu);
    bcm2836_arm_irqchip_register_irq(LOCAL_IRQ_PMU_FAST, FALSE, &bcm2836_arm_irqchip_pmu);

    /* Mask timer and mailbox interrupts */
    for (i = 0; i < 4; i++) {
        bcm2836_arm_irqchip_mask_per_cpu_irq(LOCAL_TIMER_INT_CONTROL0, LOCAL_IRQ_CNTPSIRQ - LOCAL_IRQ_CNTPSIRQ, i);
        bcm2836_arm_irqchip_mask_per_cpu_irq(LOCAL_TIMER_INT_CONTROL0, LOCAL_IRQ_CNTPNSIRQ - LOCAL_IRQ_CNTPSIRQ, i);
        bcm2836_arm_irqchip_mask_per_cpu_irq(LOCAL_TIMER_INT_CONTROL0, LOCAL_IRQ_CNTHPIRQ - LOCAL_IRQ_CNTPSIRQ, i);
        bcm2836_arm_irqchip_mask_per_cpu_irq(LOCAL_TIMER_INT_CONTROL0, LOCAL_IRQ_CNTVIRQ - LOCAL_IRQ_CNTPSIRQ, i);
        bcm2836_arm_irqchip_mask_per_cpu_irq(LOCAL_MAILBOX_INT_CONTROL0, (LOCAL_IRQ_MAILBOX0 - LOCAL_IRQ_MAILBOX0), i);
        bcm2836_arm_irqchip_mask_per_cpu_irq(LOCAL_MAILBOX_INT_CONTROL0, (LOCAL_IRQ_MAILBOX1 - LOCAL_IRQ_MAILBOX0), i);
        bcm2836_arm_irqchip_mask_per_cpu_irq(LOCAL_MAILBOX_INT_CONTROL0, (LOCAL_IRQ_MAILBOX2 - LOCAL_IRQ_MAILBOX0), i);
        bcm2836_arm_irqchip_mask_per_cpu_irq(LOCAL_MAILBOX_INT_CONTROL0, (LOCAL_IRQ_MAILBOX3 - LOCAL_IRQ_MAILBOX0), i);
    }
...
VMM_HOST_IRQ_INIT_DECLARE(bcm2836l1intc, "brcm,bcm2836-l1-intc", bcm2836_intc_init);
