...
static struct vmm_host_irq_domain_ops bcm283x_intc_ops = {
    .xlate = bcm283x_intc_xlate,
};

static int __init bcm283x_intc_init(vmm_device_tree_node_t *node, bool is_bcm2836)
{
    int      rc, irq;
    uint32_t b, i = 0, irq_start = 0;

    intc.parent_irq = vmm_device_tree_irq_parse_map(node, 0);
    ...
    for (b = 0; b < NR_BANKS; b++) {
        ...
        for (i = 0; i < intc.irqs[b]; i++) {
            irq = vmm_host_irq_domain_create_mapping(intc.domain, MAKE_HWIRQ(b, i));
            BUG_ON(irq < 0);
    ...
    if (intc.parent_irq != UINT_MAX) {
        if (vmm_host_irq_register(intc.parent_irq, "BCM2836 INTC", bcm2836_intc_cascade_irq, &intc)) {
            BUG();
        }
    } else {
...
static int __init bcm2835_intc_init(vmm_device_tree_node_t *node)
{
    return bcm283x_intc_init(node, FALSE);
}

VMM_HOST_IRQ_INIT_DECLARE(bcm2835intc, "brcm,bcm2835-armctrl-ic", bcm2835_intc_init);
...
VMM_HOST_IRQ_INIT_DECLARE(bcm2836intc, "brcm,bcm2836-armctrl-ic", bcm2836_intc_init);
