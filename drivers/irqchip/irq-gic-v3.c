...
#define gic_data_rdist()          (&gic_data.rdists.rdist[vmm_smp_processor_id()])
...
static void gic_udelay(uint64_t usecs)
{
    uint32_t i;
...
static inline uint32_t gic_irq(vmm_host_irq_t *d)
{
    return d->hw_irq_num;
}

static inline int gic_irq_in_rdist(vmm_host_irq_t *d)
{
    return gic_irq(d) < 32;
}

static inline void *gic_dist_base(vmm_host_irq_t *d)
{
    if (gic_irq_in_rdist(d)) { /* SGI+PPI -> SGI_base for this CPU */
        return gic_data_rdist_sgi_base();
    }

    if (d->hw_irq_num <= 1023) { /* SPI -> dist_base */
 ...
 */
static int gic_peek_irq(vmm_host_irq_t *d, uint32_t offset)
{
    uint32_t mask = 1 << (gic_irq(d) % 32);
    void    *base;
...
static void gic_poke_irq(vmm_host_irq_t *d, uint32_t offset)
{
    uint32_t mask = 1 << (gic_irq(d) % 32);
    void (*rwp_wait)(void);
    void *base;
...
static void gic_irq_set_routed_state(vmm_host_irq_t *d, uint32_t val, uint32_t mask)
{
    ...
    if (mask & VMM_ROUTED_IRQ_STATE_PENDING) {
        gic_poke_irq(d, (val & VMM_ROUTED_IRQ_STATE_PENDING) ? GICD_ISPENDR : GICD_ICPENDR);
    }

    if (mask & VMM_ROUTED_IRQ_STATE_ACTIVE) {
        gic_poke_irq(d, (val & VMM_ROUTED_IRQ_STATE_ACTIVE) ? GICD_ISACTIVER : GICD_ICACTIVER);
    }

    if (mask & VMM_ROUTED_IRQ_STATE_MASKED) {
        gic_poke_irq(d, (val & VMM_ROUTED_IRQ_STATE_MASKED) ? GICD_ICENABLER : GICD_ISENABLER);
    }
}
...
static uint32_t gic_irq_get_routed_state(vmm_host_irq_t *d, uint32_t mask)
{
    ...
    if ((mask & VMM_ROUTED_IRQ_STATE_PENDING) && gic_peek_irq(d, GICD_ISPENDR)) {
        val |= VMM_ROUTED_IRQ_STATE_PENDING;
    }

    if ((mask & VMM_ROUTED_IRQ_STATE_ACTIVE) && gic_peek_irq(d, GICD_ISACTIVER)) {
        val |= VMM_ROUTED_IRQ_STATE_ACTIVE;
    }

    if ((mask & VMM_ROUTED_IRQ_STATE_MASKED) && !gic_peek_irq(d, GICD_ISENABLER)) {
...
{
    uint32_t irqnr = gic_read_iar();

    if (irqnr == ICC_IAR1_EL1_SPURIOUS) {
        return UINT_MAX;
    }

    return vmm_host_irq_domain_find_mapping(gic_data.domain, irqnr);
}
...
static int gic_configure_irq(uint32_t irq, uint32_t type, void *base, void (*sync_access)(void))
{
    uint32_t confmask = 0x2 << ((irq % 16) * 2);
    uint32_t confoff  = (irq / 16) * 4;
    uint32_t val, oldval;
    int      ret = 0;
...
static int gic_set_type(vmm_host_irq_t *d, uint32_t type)
{
    ...
    if (irq >= 32 && type != VMM_IRQ_TYPE_LEVEL_HIGH && type != VMM_IRQ_TYPE_EDGE_RISING) {
        return VMM_ERR_INVALID;
    }
...
static uint64_t gic_mpidr_to_affinity(uint64_t mpidr)
{
    ...
    aff =
        ((uint64_t)MPIDR_AFFINITY_LEVEL(mpidr, 3) << 32 | MPIDR_AFFINITY_LEVEL(mpidr, 2) << 16 | MPIDR_AFFINITY_LEVEL(mpidr, 1) << 8 |
...
static uint16_t gic_compute_target_list(int *base_cpu, const vmm_cpumask_t *mask, uint64_t cluster_id)
{
    int      rc, cpu = *base_cpu;
    uint64_t mpidr;
    uint16_t tlist = 0;
...
static void gic_send_sgi(uint64_t cluster_id, uint16_t tlist, uint32_t irq)
{
    uint64_t val;

    val =
        (MPIDR_TO_SGI_AFFINITY(cluster_id, 3) | MPIDR_TO_SGI_AFFINITY(cluster_id, 2) | irq << ICC_SGI1R_SGI_ID_SHIFT |
         MPIDR_TO_SGI_AFFINITY(cluster_id, 1) | tlist << ICC_SGI1R_TARGET_LIST_SHIFT);
...
static int gic_set_affinity(vmm_host_irq_t *d, const vmm_cpumask_t *mask_val, bool force)
{
    ...
    int      rc, enabled;
    uint64_t mpidr;
    uint32_t cpu = vmm_cpumask_any_and(mask_val, cpu_online_mask);
...
static void gic_dist_config(void *base, int gic_irqs, void (*sync_access)(void))
{
    ...
    for (i = 32; i < gic_irqs; i += 16) {
        gic_writel(GICD_INT_ACTLOW_LVLTRIG, base + GICD_ICFGR + i / 4);
    }
    ...
    for (i = 32; i < gic_irqs; i += 32) {
        gic_writel(GICD_INT_EN_CLR_X32, base + GICD_ICACTIVER + i / 8);
        gic_writel(GICD_INT_EN_CLR_X32, base + GICD_ICENABLER + i / 8);
    }
...
static void __init gic_dist_init(void)
{
    ...
    for (i = 0; i < gic_data.domain_irq_nr; i++) {
        hirq = vmm_host_irq_domain_create_mapping(gic_data.domain, i);
        BUG_ON(hirq < 0);
    ...
    gic_writel(GICD_CTLR_ARE_NS | GICD_CTLR_ENABLE_G1A | GICD_CTLR_ENABLE_G1, base + GICD_CTLR);
...
static int gic_populate_rdist(void)
{
     ...
     */
    aff =
        (MPIDR_AFFINITY_LEVEL(mpidr, 3) << 24 | MPIDR_AFFINITY_LEVEL(mpidr, 2) << 16 | MPIDR_AFFINITY_LEVEL(mpidr, 1) << 8 |
         MPIDR_AFFINITY_LEVEL(mpidr, 0));
    ...
    for (i = 0; i < gic_data.nr_redist_regions; i++) {
        ...
        do {
            ...
            if ((typer >> 32) == aff) {
                uint64_t offset             = ptr - gic_data.redist_regions[i].redist_base;
                gic_data_rdist_rd_base()    = ptr;
                gic_data_rdist()->phys_base = gic_data.redist_regions[i].phys_base + offset;
                vmm_printf(
                    "CPU%d: GICv3: found redistributor"
                    " %lx region %d:0x%" PRIPADDR "\n",
                    cpu, mpidr, i, gic_data_rdist()->phys_base);
                return 0;
            }
...
static void gic_cpu_config(void *base, void (*sync_access)(void))
{
     ...
     */
    for (i = 0; i < 32; i += 4) {
        gic_writel(GICD_INT_DEF_PRI_X4, base + GICD_IPRIORITYR + i * 4 / 4);
    }
...
static struct vmm_host_irq_domain_ops gic_ops = {
    .xlate = gic_of_xlate,
};
...
static int __init
gic_init_bases(void *dist_base, struct redist_region *rdist_regs, uint32_t nr_redist_regions, uint64_t redist_stride, vmm_device_tree_node_t *node)
{
    uint32_t typer;
    int      gic_irqs;
    ...
    gic_data.domain_irq_nr = (gic_data.irq_nr < CONFIG_HOST_IRQ_COUNT) ? gic_data.irq_nr : CONFIG_HOST_IRQ_COUNT;

    gic_data.domain        = vmm_host_irq_domain_add(node, 0x0, gic_data.domain_irq_nr, &gic_ops, &gic_data);
...
static int gic_startup(vmm_cpu_hotplug_notify_t *cpu_hotplug, uint32_t cpu)
{
    gic_cpu_init();
...
static vmm_cpu_hotplug_notify_t gic_cpu_hotplug = {
    .name    = "GICv3",
...
{
    uint32_t reg = gic_readl(dist_base + GICD_PIDR2) & GIC_PIDR2_ARCH_MASK;
...
static int __init gic_of_init(vmm_device_tree_node_t *node)
{
    virtual_addr_t        va;
    void                 *dist_base;
    ...
    err = vmm_device_tree_request_regmap(node, &va, 0, "GICv3 Dist");
    ...
    if (vmm_device_tree_read_u32(node, "#redistributor-regions", &nr_redist_regions)) {
        nr_redist_regions = 1;
    }
    ...
    for (i = 0; i < nr_redist_regions; i++) {
        ...
        vmm_snprintf(rdist_regs[i].name, sizeof(rdist_regs[i].name), "GICv3 Redist%d", i);

        err = vmm_device_tree_regaddr(node, &pa, 1 + i);

        if (err) {
            vmm_printf(
                "GICv3: %s: unable to get address of %s "
                "regs\n",
                node->name, rdist_regs[i].name);
            goto out_unmap_rdist;
        }

        err = vmm_device_tree_request_regmap(node, &va, 1 + i, rdist_regs[i].name);
    ...
    if (vmm_device_tree_read_u64(node, "redistributor-stride", &redist_stride)) {
        redist_stride = 0;
    }

    err = gic_init_bases(dist_base, rdist_regs, nr_redist_regions, redist_stride, node);

    if (err) {
        goto out_unmap_rdist;
    }

    return vmm_cpu_hotplug_register(&gic_cpu_hotplug, FALSE);
...
out_unmap_rdist:
    ...
    for (i = 0; i < nr_redist_regions; i++) {
        ...
        vmm_device_tree_regunmap_release(node, (virtual_addr_t)rdist_regs[i].redist_base, 1 + i);
        rdist_regs[i].redist_base = NULL;
    }

out_unmap_dist:
    vmm_device_tree_regunmap_release(node, (virtual_addr_t)dist_base, 0);
    return err;
}
