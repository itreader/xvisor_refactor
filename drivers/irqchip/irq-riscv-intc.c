...
static uint32_t riscv_intc_aia_active_irq(uint32_t cpu_irq_no, uint32_t prev_irq)
{
    uint64_t topi = csr_read(CSR_STOPI);
    return (topi) ? topi >> TOPI_IID_SHIFT : UINT_MAX;
}

static uint32_t riscv_intc_active_irq(uint32_t cpu_irq_no, uint32_t prev_irq)
{
...
static struct vmm_host_irq_domain_ops riscv_intc_ops = {
    .xlate = vmm_host_irq_domain_xlate_onecell,
};

static int riscv_intc_startup(vmm_cpu_hotplug_notify_t *cpu_hotplug, uint32_t cpu)
{
...
static vmm_cpu_hotplug_notify_t riscv_intc_cpu_hotplug = {
    .name    = "RISCV_INTC",
    .state   = VMM_CPU_HOTPLUG_STATE_HOST_IRQ,
    .startup = riscv_intc_startup,
};

static int __init riscv_intc_init(vmm_device_tree_node_t *node)
{
    int      i, irq, rc;
    uint32_t nr_irqs, hart_id = 0;

    /* Get hart_id of associated HART */
    rc = riscv_node_to_hartid(node->parent, &hart_id);

    if (rc) {
        vmm_lerror("riscv-intc", "can't find hart_id of asociated HART\n");
        return rc;
    }
    ...
    /* Register IRQ domain */
    intc_domain = vmm_host_irq_domain_add(node, 0, nr_irqs, &riscv_intc_ops, NULL);
    ...
    for (i = 0; i < RISCV_IRQ_COUNT; i++) {
        irq = vmm_host_irq_domain_create_mapping(intc_domain, i);
    ...
    rc = vmm_cpu_hotplug_register(&riscv_intc_cpu_hotplug, TRUE);

    if (rc) {
        vmm_lerror("riscv-intc", "failed to register cpu_hotplug\n");
        vmm_host_irq_domain_remove(intc_domain);
        intc_domain = NULL;
        return rc;
    }
