...
static bool     plic_cpu_hotplug_setup_done;
static uint32_t plic_parent_irq;
static DEFINE_PER_CPU(struct plic_context *, handlers);

static void plic_context_disable_irq(struct plic_context *cntx, int hw_irq_num)
{
    uint32_t    mask, *reg;
    irq_flags_t flags;
    ...
    reg  = cntx->reg_enable_base + (hw_irq_num / 32) * sizeof(uint32_t);
    mask = ~(1 << (hw_irq_num % 32));

    vmm_spin_lock_irq_save_lite(&cntx->reg_enable_lock, flags);
    vmm_writel(vmm_readl(reg) & mask, reg);
    vmm_spin_unlock_irq_restore_lite(&cntx->reg_enable_lock, flags);
}

static void plic_context_enable_irq(struct plic_context *cntx, int hw_irq_num)
{
    uint32_t    bit, *reg;
    irq_flags_t flags;
...
static int plic_irq_enable_with_mask(vmm_host_irq_t *d, const vmm_cpumask_t *mask)
{
    int                  cpu;
    vmm_cpumask_t        amask;
    ...
    vmm_writel(1, cntx->hw->reg_priority_base + d->hw_irq_num * PRIORITY_PER_ID);
    plic_context_enable_irq(cntx, d->hw_irq_num);

    return VMM_OK;
}

static void plic_irq_enable(vmm_host_irq_t *d)
{
    plic_irq_enable_with_mask(d, vmm_host_irq_get_affinity(d));
}

static void plic_irq_disable(vmm_host_irq_t *d)
{
    int             i;
    struct plic_hw *hw = vmm_host_irq_get_chip_data(d);

    vmm_writel(0, hw->reg_priority_base + d->hw_irq_num * PRIORITY_PER_ID);

    for (i = 0; i < hw->ncontexts; ++i) {
        plic_context_disable_irq(&hw->contexts[i], d->hw_irq_num);
    }
}

static int plic_irq_set_affinity(vmm_host_irq_t *d, const vmm_cpumask_t *mask, bool force)
{
...
static vmm_host_irq_chip_t plic_chip = {
    .name             = "riscv-plic",
...
static vmm_irq_return_t plic_chained_handle_irq(int irq, void *dev)
{
     ...
     */
    while ((hw_irq_num = vmm_readl(cntx->reg_base + CONTEXT_CLAIM))) {
        hirq = vmm_host_irq_domain_find_mapping(cntx->hw->domain, hw_irq_num);
        vmm_host_generic_irq_exec(hirq);
        vmm_writel(hw_irq_num, cntx->reg_base + CONTEXT_CLAIM);
        have_irq = TRUE;
    }
...
static int plic_irq_domain_map(struct vmm_host_irq_domain *dom, uint32_t hirq, uint32_t hw_irq_num)
{
...
static struct vmm_host_irq_domain_ops plic_ops = {
    .xlate = vmm_host_irq_domain_xlate_onecell,
    .map   = plic_irq_domain_map,
};
...
static void plic_context_init(struct plic_context *cntx)
{
    ...
    if (vmm_host_irq_register(plic_parent_irq, "riscv-plic", plic_chained_handle_irq, cntx)) {
        return;
    }
...
static int plic_cpu_init(vmm_cpu_hotplug_notify_t *cpu_hotplug, uint32_t cpu)
{
...
static vmm_cpu_hotplug_notify_t plic_cpu_hotplug = {
    .name    = "PLIC",
    .state   = VMM_CPU_HOTPLUG_STATE_HOST_IRQ,
    .startup = plic_cpu_init,
};

static int __init plic_init(vmm_device_tree_node_t *node)
{
    uint32_t                            cpu;
    int                                 i, rc, hw_irq_num;
    ...
    if (vmm_device_tree_read_u32(node, "riscv,ndev", &hw->ndev)) {
        hw->ndev = MAX_DEVICES;
    }
    ...
    hw->ncontexts       = vmm_device_tree_irq_count(node);
    ...
    rc = vmm_device_tree_regaddr(node, &hw->reg_phys, 0);
    ...
    for (i = 0; i < hw->ncontexts; ++i) {
        ...
        cntx->reg_base        = hw->reg_base + CONTEXT_BASE + CONTEXT_PER_HART * cntx->contextid;
        cntx->reg_enable_base = hw->reg_base + ENABLE_BASE + ENABLE_PER_HART * cntx->contextid;

        /* Parse interrupt entry */
        rc                    = vmm_device_tree_irq_parse_one(node, i, &oirq);

        if (rc || !oirq.np || !oirq.np->parent || !oirq.args_count) {
            vmm_lerror("plic", "%s: failed to parse irq for context=%d\n", node->name, i);
            continue;
        }
        ...
        if (!plic_parent_irq) {
            plic_parent_irq = vmm_device_tree_irq_parse_map(node, i);
        }
        ...
        for (hw_irq_num = 1; hw_irq_num <= cntx->hw->ndev; ++hw_irq_num) {
            plic_context_disable_irq(cntx, hw_irq_num);
        }
    ...
    /* Create IRQ domain */
    hw->domain = vmm_host_irq_domain_add(node, -1, hw->ndev, &plic_ops, hw);

    if (!hw->domain) {
        vmm_lerror("plic", "%s: failed to add irq_domain\n", node->name);
    ...
    if (this_cpu(handlers) && !plic_cpu_hotplug_setup_done) {
        rc = vmm_cpu_hotplug_register(&plic_cpu_hotplug, TRUE);

        if (rc) {
            vmm_lerror("plic", "%s: failed to setup cpu_hotplug\n", node->name);
            vmm_host_irq_domain_remove(hw->domain);
            vmm_host_iounmap(hw->reg_virt);
            vmm_free(hw->contexts);
            vmm_free(hw);
            return rc;
        ...
        plic_cpu_hotplug_setup_done = TRUE;
    }

    /* Print details */
    vmm_init_printf("plic: %s: devices=%d contexts=%d/%d\n", node->name, hw->ndev, hw->ncontexts_avail, hw->ncontexts);
