...
static uint32_t aw_intc_irq_active(uint32_t cpu_irq_no, uint32_t prev_irq)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        return vmm_host_irq_domain_find_mapping(awvic.domain, i);
    }
...
static struct vmm_host_irq_domain_ops aw_intc_ops = {
    .xlate = aw_intc_xlate,
};
...
static int __init aw_intc_device_tree_init(vmm_device_tree_node_t *node)
{
    ...
    /* Map registers */
    rc = vmm_device_tree_request_regmap(node, &awvic.base, 0, "Allwinner INTC");

    if (rc) {
        vmm_host_irq_domain_remove(awvic.domain);
        return rc;
    }
...
VMM_HOST_IRQ_INIT_DECLARE(sunxiintc, "allwinner,sun4i-ic", aw_intc_device_tree_init);
