...
struct gic_chip_data {
    ...
    uint32_t                    hwirq_base; /* Starting physical IRQ number */
    uint32_t                    max_irqs;   /* Total IRQs */
...
{
    uint32_t ret = gic_read(gic_data[0].cpu_base + GICC_INTACK) & 0x3FF;

    if (ret < 1021) {
        ret = vmm_host_irq_domain_find_mapping(gic_data[0].domain, ret);
    } else {
        ret = UINT_MAX;
    }
...
static uint32_t gic_irq_get_routed_state(struct vmm_host_irq *d, uint32_t mask)
{
    ...
    if ((mask & VMM_ROUTED_IRQ_STATE_PENDING) && gic_peek_irq(gic, d, GICD_ENABLE_SET)) {
        val |= VMM_ROUTED_IRQ_STATE_PENDING;
    }

    if ((mask & VMM_ROUTED_IRQ_STATE_ACTIVE) && gic_peek_irq(gic, d, GICD_ACTIVE_SET)) {
        val |= VMM_ROUTED_IRQ_STATE_ACTIVE;
    }

    if ((mask & VMM_ROUTED_IRQ_STATE_MASKED) && !gic_peek_irq(gic, d, GICD_ENABLE_SET)) {
...
static void gic_irq_set_routed_state(struct vmm_host_irq *d, uint32_t val, uint32_t mask)
{
    ...
    if (mask & VMM_ROUTED_IRQ_STATE_PENDING) {
        gic_poke_irq(gic, d, (val & VMM_ROUTED_IRQ_STATE_PENDING) ? GICD_ENABLE_SET : GICD_ENABLE_CLEAR);
    }

    if (mask & VMM_ROUTED_IRQ_STATE_ACTIVE) {
        gic_poke_irq(gic, d, (val & VMM_ROUTED_IRQ_STATE_ACTIVE) ? GICD_ACTIVE_SET : GICD_ACTIVE_CLEAR);
    }

    if (mask & VMM_ROUTED_IRQ_STATE_MASKED) {
        gic_poke_irq(gic, d, (val & VMM_ROUTED_IRQ_STATE_MASKED) ? GICD_ENABLE_CLEAR : GICD_ENABLE_SET);
    }
}
...
static vmm_irq_return_t gic_handle_cascade_irq(int irq, void *dev)
{
    ...
    cascade_irq = vmm_host_irq_domain_find_mapping(gic->domain, gic_irq);
...
static void __init gic_cascade_irq(uint32_t gic_nr, uint32_t irq)
{
    if (gic_nr >= GIC_MAX_NR) {
        BUG();
    }

    if (vmm_host_irq_register(irq, "GIC-CHILD", gic_handle_cascade_irq, &gic_data[gic_nr])) {
        BUG();
    }
}
...
static void __init gic_dist_init(struct gic_chip_data *gic)
{
    int            hirq;
    uint32_t       i;
    uint32_t       cpumask = 1 << vmm_smp_processor_id();
    virtual_addr_t base    = gic->dist_base;
    ...
    for (i = 0; i < gic->max_irqs; i += 32) {
        gic_write(0xffffffff, base + GICD_ENABLE_CLEAR + i * 4 / 32);
    }
    ...
    for (i = 0; i < gic->max_irqs; i++) {
        hirq = vmm_host_irq_domain_create_mapping(gic->domain, i);
        BUG_ON(hirq < 0);
...
static void gic_cpu_init(struct gic_chip_data *gic)
{
     ...
     */
    for (i = 0; i < 32; i += 4) {
        gic_write(0xa0a0a0a0, gic->dist_base + GICD_PRI + i * 4 / 4);
    }
...
static int __init gic_init_bases(
    vmm_device_tree_node_t *node, uint32_t gic_nr, bool eoimode, uint32_t irq_start, virtual_addr_t cpu_base, virtual_addr_t cpu2_base,
    virtual_addr_t dist_base)
{
    uint32_t              max_irqs;
...
static int __init gic_device_tree_init(vmm_device_tree_node_t *node, vmm_device_tree_node_t *parent, bool eoimode)
{
    int             rc;
    uint32_t        irq, irq_start = 0;
    ...
    rc = vmm_device_tree_request_regmap(node, &cpu_base, 1, "GIC CPU");
    WARN(rc, "unable to map gic cpu registers\n");

    rc = vmm_device_tree_request_regmap(node, &cpu2_base, 4, "GIC CPU2");

    if (rc) {
        rc = vmm_device_tree_regsize(node, &cpu_sz, 1);
    ...
    rc = gic_init_bases(node, gic_cnt, eoimode, irq_start, cpu_base, cpu2_base, dist_base);
    ...
    if (parent) {
        if (vmm_device_tree_read_u32(node, "parent_irq", &irq)) {
            irq = 1020;
        }
...
static int __init gic_init(vmm_device_tree_node_t *node)
{
    return gic_device_tree_init(node, NULL, FALSE);
}

static int __init gic_eoimode_init(vmm_device_tree_node_t *node)
{
    return gic_device_tree_init(node, NULL, TRUE);
}
