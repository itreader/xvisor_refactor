...
#define MODULE_IPRIORITY   (SCSI_DISK_IPRIORITY + USB_CORE_IPRIORITY + 1)
...
static int usb_storage_alloc_disk_num(void)
{
    ...
    vmm_spin_unlock_irq_restore(&us_disk_bmap_lock, flags);
...
static int usb_storage_BBB_reset(struct scsi_transport *tr, void *private)
{
    int                 rc;
    struct usb_storage *us = private;
     ...
     */
    rc                     = usb_control_msg(
        us->dev, usb_sndctrlpipe(us->dev, 0), US_BBB_RESET, USB_TYPE_CLASS | USB_RECIP_INTERFACE, 0, us->intf->desc.bInterfaceNumber, NULL, 0, NULL,
        USB_CNTL_TIMEOUT * 5);
 ...
 */
static int usb_storage_BBB_comdat(struct scsi_request *srb, struct usb_storage *us)
{
    int                                            actlen;
    uint32_t                                       pipe;
    ...
    cbw.bCBWFlags              = SCSI_CMD_DIRECTION(srb->cmd[0]) ? CBWFLAGS_IN : CBWFLAGS_OUT;
    cbw.bCBWLUN                = srb->lun;
    ...
    return usb_bulk_msg(us->dev, pipe, &cbw, UMASS_BBB_CBW_SIZE, &actlen, USB_CNTL_TIMEOUT * 5);
}

/* clear a stall on an endpoint - special for BBB devices */
static int usb_storage_BBB_clear_endpt_stall(struct usb_storage *us, uint8_t endpt)
{
    /* ENDPOINT_HALT = 0, so set value to 0 */
    return usb_control_msg(
        us->dev, usb_sndctrlpipe(us->dev, 0), USB_REQ_CLEAR_FEATURE, USB_RECIP_ENDPOINT, 0, endpt, NULL, 0, NULL, USB_CNTL_TIMEOUT * 5);
}

static int usb_storage_BBB_transport(struct scsi_request *srb, struct scsi_transport *tr, void *private)
{
    int                                            rc, retry;
    ...
    pipe = SCSI_CMD_DIRECTION(srb->cmd[0]) ? usb_rcvbulkpipe(us->dev, us->ep_in) : usb_sndbulkpipe(us->dev, us->ep_out);
    ep   = SCSI_CMD_DIRECTION(srb->cmd[0]) ? us->ep_in : us->ep_out;
    rc   = usb_bulk_msg(us->dev, pipe, srb->data, srb->datalen, &data_actlen, USB_CNTL_TIMEOUT * 5);
...
st:
    retry = 0;
again:
    rc = usb_bulk_msg(us->dev, usb_rcvbulkpipe(us->dev, us->ep_in), &csw, UMASS_BBB_CSW_SIZE, &actlen, USB_CNTL_TIMEOUT * 5);
...
static void usb_storage_info_fixup(struct scsi_info *info, struct scsi_transport *tr, void *private)
{
    struct usb_storage *us = private;

    if ((us->dev->descriptor.idVendor == 0x0424) && (us->dev->descriptor.idProduct == 0x223a)) {
        strncpy(info->vendor, "SMSC", sizeof(info->vendor));
        strncpy(info->product, "Flash Controller", sizeof(info->product));
    }
}

static int usb_storage_max_luns(struct usb_device *dev, int ifnum, unsigned char *max_luns)
{
    int                               rc, len;
    unsigned char __cacheline_aligned result;

    rc = usb_control_msg(
        dev, usb_rcvctrlpipe(dev, 0), US_BBB_GET_MAX_LUN, USB_TYPE_CLASS | USB_RECIP_INTERFACE | USB_DIR_IN, 0, ifnum, &result, sizeof(result), &len,
        USB_CNTL_TIMEOUT * 5);
...
static int usb_storage_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
    ...
    for (i = 0; i < intf->desc.bNumEndpoints; i++) {
        ...
        if ((ep_desc->bmAttributes & USB_ENDPOINT_XFERTYPE_MASK) == USB_ENDPOINT_XFER_BULK) {
            if (ep_desc->bEndpointAddress & USB_DIR_IN) {
                us->ep_in = ep_desc->bEndpointAddress & USB_ENDPOINT_NUMBER_MASK;
            } else {
                us->ep_out = ep_desc->bEndpointAddress & USB_ENDPOINT_NUMBER_MASK;
            }
        }

        /* is it an interrupt endpoint? */
        if ((ep_desc->bmAttributes & USB_ENDPOINT_XFERTYPE_MASK) == USB_ENDPOINT_XFER_INT) {
            us->ep_int      = ep_desc->bEndpointAddress & USB_ENDPOINT_NUMBER_MASK;
    ...
    for (lun = 0; lun < us->luns_count; lun++) {
        ...
        us->luns[lun].lun      = lun;
        vmm_snprintf(name, sizeof(name), "usbdisk%d", us->luns[lun].disk_num);
        disk = scsi_create_disk(name, lun, US_MAX_PENDING, US_BLKS_PER_XFER, &us->intf->dev, us->tr, us);
