...
static int usb_hcd_request_irqs(struct usb_hcd *hcd, uint32_t irqnum, uint64_t irqflags)
{
    ...
    if (hcd->driver->irq) {
        vmm_snprintf(hcd->irq_descr, sizeof(hcd->irq_descr), "%s:%s", hcd->dev->name, hcd->root_hub->dev.name);
        rc = vmm_host_irq_register(irqnum, hcd->irq_descr, &usb_hcd_irq, hcd);
        ...
        hcd->irq = irqnum;
        vmm_printf("%s: %s 0x%08lx\n", hcd->dev->name, (hcd->driver->flags & HCD_MEMORY) ? "IO mem" : "IO base", (uint64_t)hcd->resource_addr_start);
    } else {
        hcd->irq = 0;

        if (hcd->resource_addr_start) {
            vmm_printf(
                "%s: %s 0x%08lx\n", hcd->dev->name, (hcd->driver->flags & HCD_MEMORY) ? "IO mem" : "IO base", (uint64_t)hcd->resource_addr_start);
        }
    }
...
void usb_hcd_died(struct usb_hcd *hcd)
{
    ...
    vmm_spin_lock_irq_save(&hcd_root_hub_lock, flags);
    ...
    if (hcd->rh_registered) {
        ...
        usb_set_device_state(hcd->root_hub, USB_STATE_NOTATTACHED);
        usb_notify_remove_device(hcd->root_hub);
        hcd->root_hub      = NULL;
        hcd->rh_registered = 0;
    }

    vmm_spin_unlock_irq_restore(&hcd_root_hub_lock, flags);
