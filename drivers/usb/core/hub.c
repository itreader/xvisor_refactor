...
static vmm_thread_t *usb_hub_worker_thread;
...
static void usb_hub_init_work(struct usb_hub_work *work, int (*func)(struct usb_hub_work *), bool freeup, struct usb_device *dev)
{
...
static struct usb_hub_work *usb_hub_alloc_work(int (*func)(struct usb_hub_work *), struct usb_device *dev)
{
...
static void usb_hub_queue_work(struct usb_hub_work *work)
{
    ...
    vmm_spin_lock_irq_save(&usb_hub_work_list_lock, flags);

    INIT_LIST_HEAD(&work->head);
    list_add_tail(&work->head, &usb_hub_work_list);

    vmm_spin_unlock_irq_restore(&usb_hub_work_list_lock, flags);
...
static struct usb_hub_device *usb_hub_find(struct usb_device *dev, struct usb_interface *intf)
{
    double_list_t         *l;
...
static uint32_t usb_hub_count(void)
{
    uint32_t       ret = 0;
    double_list_t *l;
...
static void usb_announce_device(struct usb_device *udev)
{
    ...
    vmm_printf(
        "%s: New USB %s device found, "
        "idVendor=%04x, idProduct=%04x\n",
        udev->dev.name, speed_str, vmm_le16_to_cpu(udev->descriptor.idVendor), vmm_le16_to_cpu(udev->descriptor.idProduct));
...
static int usb_set_address(struct usb_device *dev, uint8_t addr)
{
    DPRINTF("%s: set address %d\n", __func__, addr);
    return usb_control_msg(dev, usb_snddefctrl(dev), USB_REQ_SET_ADDRESS, 0, addr, 0, NULL, 0, NULL, USB_CNTL_TIMEOUT);
}
...
static int usb_set_configuration(struct usb_device *dev, int configuration)
{
    ...
    /* set setup command */
    res = usb_control_msg(dev, usb_sndctrlpipe(dev, 0), USB_REQ_SET_CONFIGURATION, 0, configuration, 0, NULL, 0, NULL, USB_CNTL_TIMEOUT);
...
static int usb_set_maxpacket(struct usb_device *dev)
{
    ...
    for (i = 0; i < dev->config.desc.bNumInterfaces; i++) {
        for (ii = 0; ii < dev->config.intf[i].desc.bNumEndpoints; ii++) {
 ...
 */
static int usb_parse_config(struct usb_device *dev, uint8_t *buffer, int cfgno)
{
    uint16_t                        ep_wMaxPacketSize;
    ...
    dev->config.desc.wTotalLength = vmm_le16_to_cpu(dev->config.desc.wTotalLength);
        ...
        switch (head->bDescriptorType) {
            case USB_DT_INTERFACE:
                if (((struct usb_interface_descriptor *)&buffer[index])->bInterfaceNumber != curr_if_num) {
                    ...
                    vmm_device_driver_initialize_device(&ifp->dev);
                    vmm_snprintf(ifp->dev.name, sizeof(ifp->dev.name), "%s-intf%d", dev->dev.name, ifno);
                    ifp->dev.parent = &dev->dev;
                    ifp->dev.bus    = &usb_bus_type;
                    ifp->dev.type   = &usb_interface_type;
                    dev->config.no_of_intf++;
                    memcpy(&ifp->desc, &buffer[index], sizeof(ifp->desc));
                    ifp->no_of_ep       = 0;
            ...
            case USB_DT_ENDPOINT:
                ...
                ep_wMaxPacketSize = get_unaligned(&ep_desc->wMaxPacketSize);
                put_unaligned(vmm_le16_to_cpu(ep_wMaxPacketSize), &ep_desc->wMaxPacketSize);
                DPRINTF("%s: ifnum=%d ep=%d\n", __func__, ifno, epno);
                break;

            case USB_DT_SS_ENDPOINT_COMP:
                ifp = &dev->config.intf[ifno];
                memcpy(&ifp->ss_ep_comp_desc[epno], &buffer[index], sizeof(ifp->ss_ep_comp_desc[epno]));
                break;
...
static int usb_hub_get_descriptor(struct usb_device *dev, void *data, int size)
{
    return usb_control_msg(
        dev, usb_rcvctrlpipe(dev, 0), USB_REQ_GET_DESCRIPTOR, USB_DIR_IN | USB_RT_HUB, USB_DT_HUB << 8, 0, data, size, NULL, USB_CNTL_TIMEOUT);
}

static int usb_hub_clear_port_feature(struct usb_device *dev, int port, int feature)
{
    return usb_control_msg(dev, usb_sndctrlpipe(dev, 0), USB_REQ_CLEAR_FEATURE, USB_RT_PORT, feature, port, NULL, 0, NULL, USB_CNTL_TIMEOUT);
}

static int usb_hub_set_port_feature(struct usb_device *dev, int port, int feature)
{
    return usb_control_msg(dev, usb_sndctrlpipe(dev, 0), USB_REQ_SET_FEATURE, USB_RT_PORT, feature, port, NULL, 0, NULL, USB_CNTL_TIMEOUT);
}

static int usb_hub_get_status(struct usb_device *dev, void *data)
{
    return usb_control_msg(
        dev, usb_rcvctrlpipe(dev, 0), USB_REQ_GET_STATUS, USB_DIR_IN | USB_RT_HUB, 0, 0, data, sizeof(struct usb_hub_status), NULL, USB_CNTL_TIMEOUT);
}

static int usb_hub_get_port_status(struct usb_device *dev, int port, void *data)
{
    return usb_control_msg(
        dev, usb_rcvctrlpipe(dev, 0), USB_REQ_GET_STATUS, USB_DIR_IN | USB_RT_PORT, 0, port, data, sizeof(struct usb_hub_status), NULL,
        USB_CNTL_TIMEOUT);
}
...
static int usb_hub_port_reset(struct usb_device *dev, int port, unsigned short *portstat)
{
#define MAX_TRIES 5
    int                     err, tries;
    ...
    for (tries = 0; tries < MAX_TRIES; tries++) {
        ...
        DPRINTF("%s: portstatus 0x%x, change 0x%x, %s\n", __func__, portstatus, portchange, usb_hub_portspeed(portstatus));

        DPRINTF(
            "%s: STAT_C_CONNECTION = %d STAT_CONNECTION = %d"
            " USB_PORT_STAT_ENABLE %d\n",
            __func__, (portchange & USB_PORT_STAT_C_CONNECTION) ? 1 : 0, (portstatus & USB_PORT_STAT_CONNECTION) ? 1 : 0,
    ...
    if (tries == MAX_TRIES) {
        vmm_printf(
            "%s: Cannot enable port %i after %i retries, "
            "disabling port.\n",
...
static void usb_hub_power_on(struct usb_hub_device *hub)
{
    ...
    uint32_t                pgood_delay = hub->desc.bPwrOn2PwrGood * 2;
    struct usb_port_status *portsts;
    ...
    for (i = 0; i < dev->maxchild; i++) {
        ret = usb_hub_clear_port_feature(dev, i + 1, USB_PORT_FEAT_POWER);
    ...
    for (i = 0; i < dev->maxchild; i++) {
        ret = usb_hub_set_port_feature(dev, i + 1, USB_PORT_FEAT_POWER);
        DPRINTF("%s: port %d returns %d\n", __func__, i + 1, ret);
    }
...
static int usb_hub_configure(struct usb_device *dev, struct usb_interface *intf)
{
    ...
    put_unaligned(vmm_le16_to_cpu(hubCharacteristics), &hub->desc.wHubCharacteristics);

    /* Set the bitmap */
    /* Devices not removable by default */
    bitmap = (uint8_t *)&hub->desc.u.hs.DeviceRemovable[0];
    memset(bitmap, 0xff, (USB_MAXCHILDREN + 1 + 7) / 8);
    bitmap = (uint8_t *)&hub->desc.u.hs.PortPwrCtrlMask[0];
    ...
    for (i = 0; i < dev->maxchild; i++) {
        DPRINTF("%s: port %d is%s removable\n", __func__, i + 1, hub->desc.u.hs.DeviceRemovable[(i + 1) / 8] & (1 << ((i + 1) % 8)) ? " not" : "");
    }
    ...
    DPRINTF(
        "%s: get_hub_status returned status 0x%x, change 0x%x\n", __func__, vmm_le16_to_cpu(hubsts->wHubStatus), vmm_le16_to_cpu(hubsts->wHubChange));
    DPRINTF("%s: local power source is %s\n", __func__, (vmm_le16_to_cpu(hubsts->wHubStatus) & HUB_STATUS_LOCAL_POWER) ? "lost (inactive)" : "good");
    DPRINTF("%s: %sover-current condition exists\n", __func__, (vmm_le16_to_cpu(hubsts->wHubStatus) & HUB_STATUS_OVERCURRENT) ? "" : "no ");
...
static int usb_hub_detect_new_device(struct usb_device *parent, struct usb_device *dev)
{
    uint32_t              i;
    uint8_t              *tmpbuf, addr;
    uint16_t              portstatus;
    ...
    if (parent) {
        ...
        vmm_spin_lock_irq_save(&parent->children_lock, flags);
        ...
        vmm_spin_unlock_irq_restore(&parent->children_lock, flags);
    ...
    if (err < 0) {
        vmm_printf(
            "%s: device not accepting new address %" PRIu32 " "
            "(err=%d)\n",
            __func__, addr, err);
        goto done;
    }
    ...
    if (err) {
        vmm_printf(
            "%s: unable to get device descriptor "
            "(error=%d)\n",
            __func__, err);
        goto done;
    }
    ...
    if (err) {
        vmm_printf(
            "%s: failed to set default configuration "
            "error=%d\n",
            __func__, err);
        goto done;
    }
    ...
    if (dev->descriptor.iManufacturer) {
        usb_string(dev, dev->descriptor.iManufacturer, dev->manufacturer, sizeof(dev->manufacturer));
    }
    ...
    vmm_device_driver_register_device(&dev->dev);

    /* Register interface devices to devdrv */
    for (i = 0; i < dev->config.no_of_intf; i++) {
        vmm_device_driver_register_device(&dev->config.intf[i].dev);
    }
...
static void usb_recursively_disconnect(struct usb_device *dev)
{
    ...
    vmm_spin_lock_irq_save(&dev->children_lock, flags);
    ...
    for (i = 0; i < dev->maxchild; ++i) {
        ...
        vmm_spin_unlock_irq_restore(&dev->children_lock, flags);
    ...
    for (i = 0; i < dev->config.no_of_intf; i++) {
        ...
        vmm_device_driver_unregister_device(&intf->dev);
    }
    ...
    vmm_device_driver_unregister_device(&dev->dev);
...
static void usb_hub_port_connect_change(struct usb_hub_device *hub, struct usb_port_status *portsts, int port)
{
    uint16_t           portstatus, portchange;
    uint16_t           portmask = (USB_PORT_STAT_LOW_SPEED | USB_PORT_STAT_HIGH_SPEED);
    irq_flags_t        flags;
    ...
    if (!(portchange & USB_PORT_STAT_C_CONNECTION) && !(portchange & USB_PORT_STAT_C_ENABLE)) {
        goto done;
    }
...
static int usb_hub_poll_status(struct usb_hub_device *hub)
{
    ...
    for (i = 0; i < dev->maxchild; i++) {
        ...
        DPRINTF("%s: dev %s port %d status 0x%x change 0x%x\n", __func__, dev->dev.name, i + 1, portstatus, portchange);
        ...
        if (portchange & USB_PORT_STAT_C_ENABLE) {
            ...
            usb_hub_clear_port_feature(dev, i + 1, USB_PORT_FEAT_C_ENABLE);
        }

        if (portstatus & USB_PORT_STAT_SUSPEND) {
            DPRINTF("%s: dev %s port %d suspend change\n", __func__, dev->dev.name, i + 1);
            usb_hub_clear_port_feature(dev, i + 1, USB_PORT_FEAT_SUSPEND);
        }

        if (portchange & USB_PORT_STAT_C_OVERCURRENT) {
            DPRINTF("%s: dev %s port %d over-current change\n", __func__, dev->dev.name, i + 1);
            usb_hub_clear_port_feature(dev, i + 1, USB_PORT_FEAT_C_OVER_CURRENT);
            usb_hub_power_on(hub);
        }

        if (portchange & USB_PORT_STAT_C_RESET) {
            DPRINTF("%s: dev %s port %d reset change\n", __func__, dev->dev.name, i + 1);
            usb_hub_clear_port_feature(dev, i + 1, USB_PORT_FEAT_C_RESET);
        }
    }
...
static vmm_timer_event_t   usb_hub_mon_event;

static int usb_hub_mon_work_func(struct usb_hub_work *work)
{
    int                    i, err;
    uint32_t               count = usb_hub_count();
    struct usb_hub_device *hub;
...
static void usb_hub_mon_event_func(vmm_timer_event_t *ev)
{
...
static int usb_hub_driver_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
...
{
    struct usb_hub_device *hub = usb_hub_find(interface_to_usbdev(intf), intf);
...
static const struct usb_device_id usb_hub_driver_id_table[] = {
    {.match_flags = USB_DEVICE_ID_MATCH_DEVICE_CLASS, .bDeviceClass = USB_CLASS_HUB},
    {.match_flags = USB_DEVICE_ID_MATCH_INT_CLASS, .bInterfaceClass = USB_CLASS_HUB},
    {}  /* Terminating entry */
};
...
static int usb_new_device_work(struct usb_hub_work *work)
{
    ...
    struct usb_device *parent = (dev->dev.parent) ? to_usb_device(dev->dev.parent) : NULL;
...
static int usb_hub_notifier_call(vmm_notifier_block_t *nb, uint64_t event, void *data)
{
    int                ret = NOTIFY_DONE;
    struct usb_device *dev;
...
static vmm_notifier_block_t usb_hub_nb = {
...
int __init usb_hub_init(void)
{
    ...
    usb_hub_init_work(&usb_hub_mon_work, usb_hub_mon_work_func, FALSE, NULL);
