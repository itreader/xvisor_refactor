    ...
    int *actual_length, int timeout)
{
    ...
    DPRINTF(
        "%s: pipe=0x%X request=0x%X requesttype=0x%X value=0x%X "
        "index=0x%X data=0x%p size=0x%X timeout=0x%x\n",
        __func__, pipe, request, requesttype, value, index, data, size, timeout);
    ...
    /* Fill URB */
    usb_fill_control_urb(&u, dev, pipe, (uint8_t *)&setup, data, size, urb_request_complete, &uc);
...
int usb_interrupt_msg(struct usb_device *dev, uint32_t pipe, void *data, int len, int interval)
{
    ...
    /* Fill Interrupt URB */
    usb_fill_int_urb(&u, dev, pipe, data, len, urb_request_complete, &uc, interval);
...
int usb_bulk_msg(struct usb_device *dev, uint32_t pipe, void *data, int len, int *actual_length, int timeout)
{
    ...
    /* Fill Bulk URB */
    usb_fill_bulk_urb(&u, dev, pipe, data, len, urb_request_complete, &uc);
...
static int usb_get_string(struct usb_device *dev, unsigned short langid, unsigned char index, void *buf, int size)
{
    ...
    for (i = 0; i < 3; ++i) {
        ...
        result = usb_control_msg(
            dev, usb_rcvctrlpipe(dev, 0), USB_REQ_GET_DESCRIPTOR, USB_DIR_IN, (USB_DT_STRING << 8) + index, langid, buf, size, &len,
            USB_CNTL_TIMEOUT);
...
static void usb_try_string_workarounds(unsigned char *buf, int *length)
{
    ...
    for (newlength = 2; newlength + 1 < oldlength; newlength += 2) {
        if (!vmm_is_printable(buf[newlength]) || buf[newlength + 1]) {
            break;
        }
    }
...
static int usb_string_sub(struct usb_device *dev, uint32_t langid, uint32_t index, unsigned char *buf)
{
    int rc;
...
int usb_string(struct usb_device *dev, int index, char *buf, size_t size)
{
    ...
    if (!dev->have_langid) {
        ...
        if (err < 0) {
            DPRINTF(
                "%s: error getting string descriptor 0 "
                "(error=%d)\n",
                __func__, err);
            goto done;
        } else if (tbuf[0] < 4) {
            ...
            DPRINTF(
                "%s: USB device number %d default "
                "language ID 0x%x\n",
                __func__, dev->devnum, dev->string_langid);
        }
    }
...
int usb_set_interface(struct usb_device *dev, int ifnum, int alternate)
{
    ...
    ret = usb_control_msg(
        dev, usb_sndctrlpipe(dev, 0), USB_REQ_SET_INTERFACE, USB_RECIP_INTERFACE, alternate, ifnum, NULL, 0, NULL, USB_CNTL_TIMEOUT * 5);
...
{
    return usb_control_msg(
        dev, usb_rcvctrlpipe(dev, 0), USB_REQ_GET_DESCRIPTOR, USB_RECIP_INTERFACE | USB_DIR_IN, (type << 8) + id, ifnum, buf, size, NULL,
        USB_CNTL_TIMEOUT);
}
...
int usb_clear_halt(struct usb_device *dev, uint32_t pipe)
{
    ...
    result   = usb_control_msg(dev, usb_sndctrlpipe(dev, 0), USB_REQ_CLEAR_FEATURE, USB_RECIP_ENDPOINT, 0, endp, NULL, 0, NULL, USB_CNTL_TIMEOUT * 3);
