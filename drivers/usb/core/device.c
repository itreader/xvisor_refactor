...
enum usb_device_state usb_get_device_state(struct usb_device *udev)
{
    ...
    vmm_spin_lock_irq_save(&device_state_lock, flags);
    ret = udev->state;
...
static void recursively_mark_NOTATTACHED(struct usb_device *udev)
{
    ...
    vmm_spin_lock_irq_save(&udev->children_lock, flags);
    ...
    for (i = 0; i < udev->maxchild; ++i) {
        ...
        vmm_spin_unlock_irq_restore(&udev->children_lock, flags);
...
void usb_set_device_state(struct usb_device *udev, enum usb_device_state new_state)
{
    ...
    if (udev->state == USB_STATE_NOTATTACHED) {
        ...
        if (udev->state == USB_STATE_SUSPENDED && new_state != USB_STATE_SUSPENDED) {
            udev->active_duration -= vmm_timer_timestamp();
        } else if (new_state == USB_STATE_SUSPENDED && udev->state != USB_STATE_SUSPENDED) {
            udev->active_duration += vmm_timer_timestamp();
        }
...
static void usb_release_device(vmm_device_t *ddev)
{
    ...
    struct usb_device *parent = (ddev->parent) ? to_usb_device(ddev->parent) : NULL;

    /* Update HCD device number bitmap */
    vmm_spin_lock_irq_save(&dev->hcd->devicemap_lock, flags);
    __clear_bit(dev->devnum - 1, dev->hcd->devicemap);
    vmm_spin_unlock_irq_restore(&dev->hcd->devicemap_lock, flags);
...
static void usb_release_interface(vmm_device_t *ddev)
{
...
struct usb_device *usb_alloc_device(struct usb_device *parent, struct usb_hcd *hcd, unsigned port)
{
    ...
    if (hcd->driver->alloc_dev && parent && !hcd->driver->alloc_dev(hcd, dev)) {
        usb_dref_hcd(hcd);
        vmm_free(dev);
        return NULL;
    }
    ...
    if (unlikely(!parent)) {
        ...
        dev->route      = 0;
        dev->level      = 0;
        vmm_snprintf(dev->dev.name, sizeof(dev->dev.name), "usb%d", hcd->bus_num);
    } else {
        if (parent->level == 0) {
            /* Root hub port is not counted in route string
             * because it is always zero.
             */
            vmm_snprintf(dev->devpath, sizeof(dev->devpath), "%d", port);
        } else {
            vmm_snprintf(dev->devpath, sizeof(dev->devpath), "%s.%d", parent->devpath, port);
        }

        /* Route string assumes hubs have less than 16 ports */
        if (port < 15) {
            dev->route = parent->route + (port << (parent->level * 4));
        } else {
            dev->route = parent->route + (15 << (parent->level * 4));
        }

        dev->level = parent->level + 1;
        vmm_snprintf(dev->dev.name, sizeof(dev->dev.name), "usb%d-%s", hcd->bus_num, dev->devpath);
     ...
     */
    vmm_spin_lock_irq_save(&hcd->devicemap_lock, flags);
    dev->devnum = 0;
    ...
    i = dev->devnum;
    vmm_spin_unlock_irq_restore(&hcd->devicemap_lock, flags);
...
{
    if (dev) {
        vmm_device_driver_ref_device(&dev->dev);
    }
}
...
{
    if (dev) {
        vmm_device_driver_dref_device(&dev->dev);
    }
}
...
struct usb_device *usb_find_child(struct usb_device *hdev, int port1)
{
    ...
    vmm_spin_lock_irq_save(&hdev->children_lock, flags);
    ret = hdev->children[port1 - 1];
    vmm_spin_unlock_irq_restore(&hdev->children_lock, flags);
