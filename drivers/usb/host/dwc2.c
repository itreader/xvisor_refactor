 ...
 * @host_ls_low_power_phy_clock: Specifies the PHY clock rate in low power mode
...
struct dwc2_core_params {
    ...
    int      host_ls_low_power_phy_clock;
    int      ts_dline;
    int      reload_ctl;
    int      ahbcfg;
    int      uframe_sched;
...
struct dwc2_hc {
    ...
    struct urb          *urb_int;
    double_list_t        urb_pending_list;
...
struct dwc2_control {
    ...
    uint8_t in_data_toggle[DWC2_MAX_DEVICE][DWC2_MAX_ENDPOINT];
    uint8_t out_data_toggle[DWC2_MAX_DEVICE][DWC2_MAX_ENDPOINT];

    vmm_spinlock_t host_regs_lock;

    vmm_completion_t worker_urb_pending;
    vmm_completion_t worker_irq_wait;
    vmm_thread_t    *worker_thread;
...
static void dwc2_init_fslspclksel(struct dwc2_control *dwc2)
{
    ...
    if (dwc2->params->ulpi_fs_ls) {
        uint32_t ghwcfg2 = vmm_readl(&dwc2->regs->ghwcfg2);
        uint32_t hval    = (ghwcfg2 & DWC2_HWCFG2_HS_PHY_TYPE_MASK) >> DWC2_HWCFG2_HS_PHY_TYPE_OFFSET;
        uint32_t fval    = (ghwcfg2 & DWC2_HWCFG2_FS_PHY_TYPE_MASK) >> DWC2_HWCFG2_FS_PHY_TYPE_OFFSET;
    ...
    vmm_clrsetbits_le32(&dwc2->regs->host_regs.hcfg, DWC2_HCFG_FSLSPCLKSEL_MASK, phyclk << DWC2_HCFG_FSLSPCLKSEL_OFFSET);
}
...
static void dwc2_flush_tx_fifo(struct dwc2_control *dwc2, const int num)
{
    ...
    vmm_writel(DWC2_GRSTCTL_TXFFLSH | (num << DWC2_GRSTCTL_TXFNUM_OFFSET), &dwc2->regs->grstctl);
...
static uint32_t dwc2_hc_count(struct dwc2_control *dwc2)
{
...
static void dwc2_core_host_init(struct dwc2_control *dwc2)
{
    ...
    uint32_t nptxfifosize = 0, ptxfifosize = 0, hprt0 = 0;
    ...
    if (dwc2->params->speed == 1) {
        vmm_setbits_le32(&dwc2->regs->host_regs.hcfg, DWC2_HCFG_FSLSSUPP);
    }

    /* Configure data FIFO sizes */
    if (dwc2->params->enable_dynamic_fifo && (vmm_readl(&dwc2->regs->ghwcfg2) & DWC2_HWCFG2_DYNAMIC_FIFO)) {
        /* Rx FIFO */
        vmm_writel(dwc2->params->host_rx_fifo_size, &dwc2->regs->grxfsiz);

        /* Non-periodic Tx FIFO */
        nptxfifosize |= dwc2->params->host_nperio_tx_fifo_size << DWC2_FIFOSIZE_DEPTH_OFFSET;
        nptxfifosize |= dwc2->params->host_rx_fifo_size << DWC2_FIFOSIZE_STARTADDR_OFFSET;
        vmm_writel(nptxfifosize, &dwc2->regs->gnptxfsiz);

        /* Periodic Tx FIFO */
        ptxfifosize |= dwc2->params->host_perio_tx_fifo_size << DWC2_FIFOSIZE_DEPTH_OFFSET;
        ptxfifosize |= (dwc2->params->host_rx_fifo_size + dwc2->params->host_nperio_tx_fifo_size) << DWC2_FIFOSIZE_STARTADDR_OFFSET;
    ...
    for (i = 0; i < num_channels; i++) {
        vmm_clrsetbits_le32(&dwc2->regs->hc_regs[i].hcchar, DWC2_HCCHAR_CHEN | DWC2_HCCHAR_EPDIR, DWC2_HCCHAR_CHDIS);
    }

    /* Halt all channels to put them into a known state. */
    for (i = 0; i < num_channels; i++) {
        vmm_clrsetbits_le32(&dwc2->regs->hc_regs[i].hcchar, DWC2_HCCHAR_EPDIR, DWC2_HCCHAR_CHEN | DWC2_HCCHAR_CHDIS);
        ret = wait_for_bit(&dwc2->regs->hc_regs[i].hcchar, DWC2_HCCHAR_CHEN, 0);
...
{
    uint32_t ahbcfg = 0, usbcfg = 0;
    uint8_t  brst_sz = dwc2->params->dma_burst_size;
    ...
    if (dwc2->params->phy_ulpi_ext_vbus) {
        ...
        if (!dwc2->params->oc_disable) {
            usbcfg |= DWC2_GUSBCFG_ULPI_INT_VBUS_INDICATOR | DWC2_GUSBCFG_INDICATOR_PASSTHROUGH;
        }
    } else {
        usbcfg &= ~DWC2_GUSBCFG_ULPI_EXT_VBUS_DRV;
    }
     ...
     */
    if ((dwc2->params->speed == 1) && (dwc2->params->phy_type == 0)) {
        ...
        if (dwc2->params->i2c_enable) {
            ...
            vmm_setbits_le32(&dwc2->regs->gusbcfg, DWC2_GUSBCFG_OTGUTMIFSSEL);

            /* Program GI2CCTL.I2CEn */
            vmm_clrsetbits_le32(&dwc2->regs->gi2cctl, DWC2_GI2CCTL_I2CEN | DWC2_GI2CCTL_I2CDEVADDR_MASK, 1 << DWC2_GI2CCTL_I2CDEVADDR_OFFSET);
            vmm_setbits_le32(&dwc2->regs->gi2cctl, DWC2_GI2CCTL_I2CEN);
        }
    } else {
        /* High speed PHY. */
        ...
        usbcfg |= dwc2->params->phy_type << DWC2_GUSBCFG_ULPI_UTMI_SEL_OFFSET;
    ...
    switch (vmm_readl(&dwc2->regs->ghwcfg2) & DWC2_HWCFG2_ARCHITECTURE_MASK) {
    ...
    vmm_setbits_le32(&dwc2->regs->gusbcfg, DWC2_GUSBCFG_HNPCAP | DWC2_GUSBCFG_SRPCAP);
 ...
 */
static void dwc2_hc_init(
    struct dwc2_hc_regs *hc_regs, uint8_t dev_addr, uint8_t ep_num, uint8_t ep_is_in, uint8_t ep_type, uint16_t max_packet, int speed)
{
    uint32_t hcchar = (dev_addr << DWC2_HCCHAR_DEVADDR_OFFSET) | (ep_num << DWC2_HCCHAR_EPNUM_OFFSET) | (ep_is_in << DWC2_HCCHAR_EPDIR_OFFSET) |
                      (ep_type << DWC2_HCCHAR_EPTYPE_OFFSET) | (max_packet << DWC2_HCCHAR_MPS_OFFSET);
...
static void dwc2_hc_init_split(struct dwc2_hc_regs *hc_regs, uint8_t hub_devnum, uint8_t hub_port)
{
    uint32_t hcsplt = 0;
...
static int dwc2_rh_msg_in_status(struct dwc2_control *dwc2, struct urb *u, struct usb_ctrlrequest *cmd)
{
    int      len = 0, rc = VMM_OK;
    uint32_t hprt0 = 0, port_status = 0, port_change = 0;
    void    *buffer     = u->transfer_buffer;
    int      buffer_len = u->transfer_buffer_length;

    switch (cmd->bRequestType & ~USB_DIR_IN) {
        case 0:
            *(uint16_t *)buffer = vmm_cpu_to_le16(1);
            len                 = 2;
            break;

        case USB_RECIP_INTERFACE:
        case USB_RECIP_ENDPOINT:
            *(uint16_t *)buffer = vmm_cpu_to_le16(0);
            len                 = 2;
            break;

        case USB_TYPE_CLASS:
            *(uint32_t *)buffer = vmm_cpu_to_le32(0);
            len                 = 4;
            break;
        ...
        case USB_RECIP_OTHER | USB_TYPE_CLASS:
            ...
            *(uint32_t *)buffer = vmm_cpu_to_le32(port_status | (port_change << 16));
            len                 = 4;
            break;
...
static int dwc2_rh_msg_in_descriptor(struct dwc2_control *dwc2, struct urb *u, struct usb_ctrlrequest *cmd)
{
    uint32_t dsc;
    uint8_t  data[32];
    int      len = 0, rc = VMM_OK;
    uint16_t wValue     = vmm_cpu_to_le16(cmd->wValue);
    uint16_t wLength    = vmm_cpu_to_le16(cmd->wLength);
    void    *buffer     = u->transfer_buffer;
    int      buffer_len = u->transfer_buffer_length;
        ...
        case 0:
            switch (wValue & 0xff00) {
                case 0x0100: /* device descriptor */
                    len = min3(buffer_len, (int)sizeof(root_hub_dev_desc), (int)wLength);
                    memcpy(buffer, root_hub_dev_desc, len);
                    break;

                case 0x0200: /* configuration descriptor */
                    len = min3(buffer_len, (int)sizeof(root_hub_config_desc), (int)wLength);
                ...
                case 0x0300: /* string descriptors */
                    switch (wValue & 0xff) {
                        case 0x00:
                            len = min3(buffer_len, (int)sizeof(root_hub_str_index0), (int)wLength);
                            memcpy(buffer, root_hub_str_index0, len);
                            break;

                        case 0x01:
                            len = min3(buffer_len, (int)sizeof(root_hub_str_index1), (int)wLength);
                            memcpy(buffer, root_hub_str_index1, len);
                            break;

                        case 0x02:
                            len = min3(buffer_len, (int)sizeof(root_hub_str_index2), (int)wLength);
                            memcpy(buffer, root_hub_str_index2, len);
                            break;

                        case 0x03:
                            len = min3(buffer_len, (int)sizeof(root_hub_str_index3), (int)wLength);
...
static int dwc2_rh_msg_in_configuration(struct dwc2_control *dwc2, struct urb *u, struct usb_ctrlrequest *cmd)
{
    int   len = 0, rc = VMM_OK;
    void *buffer     = u->transfer_buffer;
    int   buffer_len = u->transfer_buffer_length;
...
/* Direction: In */
static int dwc2_rh_msg_in(struct dwc2_control *dwc2, struct urb *u, struct usb_ctrlrequest *cmd)
{
    switch (cmd->bRequest) {
...
/* Direction: Out */
static int dwc2_rh_msg_out(struct dwc2_control *dwc2, struct urb *u, struct usb_ctrlrequest *cmd)
{
    int      rc           = VMM_OK;
    uint16_t bmrtype_breq = cmd->bRequestType | (cmd->bRequest << 8);
    uint16_t wValue       = vmm_cpu_to_le16(cmd->wValue);
        ...
        case (USB_REQ_CLEAR_FEATURE << 8) | USB_RECIP_OTHER | USB_TYPE_CLASS:
            switch (wValue) {
                case USB_PORT_FEAT_C_CONNECTION:
                    vmm_setbits_le32(&dwc2->regs->hprt0, DWC2_HPRT0_PRTCONNDET);
                    break;
            };
        ...
        case (USB_REQ_SET_FEATURE << 8) | USB_RECIP_OTHER | USB_TYPE_CLASS:
            switch (wValue) {
                ...
                case USB_PORT_FEAT_RESET:
                    vmm_clrsetbits_le32(
                        &dwc2->regs->hprt0, DWC2_HPRT0_PRTENA | DWC2_HPRT0_PRTCONNDET | DWC2_HPRT0_PRTENCHNG | DWC2_HPRT0_PRTOVRCURRCHNG,
                        DWC2_HPRT0_PRTRST);
                    vmm_msleep(50);
                    vmm_clrbits_le32(&dwc2->regs->hprt0, DWC2_HPRT0_PRTRST);
                    break;
...
static int dwc2_control_rh_msg(struct dwc2_control *dwc2, struct urb *u)
{
    int                     rc;
    struct usb_ctrlrequest *cmd = (struct usb_ctrlrequest *)u->setup_packet;
...
{
    irq_flags_t          flags;
    uint32_t             i, wake, chnmsk, haintmsk;
    struct dwc2_control *dwc2 = usb_hcd_private(hcd);

    vmm_spin_lock_irq_save(&dwc2->host_regs_lock, flags);
...
static int wait_for_chhltd(struct dwc2_hc *hc, uint32_t *sub, uint8_t *toggle)
{
    int      ret;
...
static int transfer_chunk(
    struct dwc2_control *dwc2, struct dwc2_hc *hc, uint8_t *pid, int in, void *buffer, int num_packets, int xfer_len, int *actual_len, int odd_frame,
    int xfer_intr)
{
    int                    ret = 0;
    uint32_t               sub, haintmsk;
    physical_addr_t        pa;
    uint64_t               timeout;
    irq_flags_t            flags;
    ...
    vmm_writel(
        (xfer_len << DWC2_HCTSIZ_XFERSIZE_OFFSET) | (num_packets << DWC2_HCTSIZ_PKTCNT_OFFSET) | (*pid << DWC2_HCTSIZ_PID_OFFSET), &hc->regs->hctsiz);

    pa = vmm_dma_map((virtual_addr_t)buffer, xfer_len, in ? DMA_FROM_DEVICE : DMA_TO_DEVICE);

    vmm_writel((uint32_t)pa + dwc2->params->dma_offset, &hc->regs->hcdma);
    ...
    vmm_clrsetbits_le32(
        &hc->regs->hcchar, DWC2_HCCHAR_MULTICNT_MASK | DWC2_HCCHAR_CHEN | DWC2_HCCHAR_CHDIS | DWC2_HCCHAR_ODDFRM,
        (1 << DWC2_HCCHAR_MULTICNT_OFFSET) | (odd_frame << DWC2_HCCHAR_ODDFRM_OFFSET) | DWC2_HCCHAR_CHEN);
    ...
    *actual_len = xfer_len;
    ...
    vmm_dma_unmap(pa, xfer_len, in ? DMA_FROM_DEVICE : DMA_TO_DEVICE);

    return ret;
}

static int chunk_msg(struct dwc2_control *dwc2, struct dwc2_hc *hc, struct urb *u, uint8_t *pid, int in, void *buffer, int len)
{
    int                    ret            = 0;
    struct dwc2_host_regs *host_regs      = &dwc2->regs->host_regs;
    ...
    if (u->dev->speed != USB_SPEED_HIGH) {
        uint8_t  hub_addr = 0;
        uint8_t  hub_port = 0;
        uint32_t hprt0    = vmm_readl(&dwc2->regs->hprt0);

        if ((hprt0 & DWC2_HPRT0_PRTSPD_MASK) == DWC2_HPRT0_PRTSPD_HIGH) {
            usb_get_usb2_hub_address_port(u->dev, &hub_addr, &hub_port);
    ...
    do {
        ...
        ret   = transfer_chunk(dwc2, hc, pid, in, (char *)buffer + done, num_packets, xfer_len, &actual_len, odd_frame, xfer_intr);
        ...
        if (complete_split) {
            ...
            if (hcint & DWC2_HCINT_NYET) {
                ...
                uint32_t frame_num;
                vmm_spin_lock_irq_save(&dwc2->host_regs_lock, flags);
                frame_num = DWC2_HFNUM_MAX_FRNUM & vmm_readl(&host_regs->hfnum);
                vmm_spin_unlock_irq_restore(&dwc2->host_regs_lock, flags);
            ...
            if (hcint & DWC2_HCINT_ACK) {
                vmm_spin_lock_irq_save(&dwc2->host_regs_lock, flags);
                ssplit_frame_num = DWC2_HFNUM_MAX_FRNUM & vmm_readl(&host_regs->hfnum);
                vmm_spin_unlock_irq_restore(&dwc2->host_regs_lock, flags);
                ret            = 0;
                complete_split = 1;
            }
...
static int dwc2_control_msg(struct dwc2_control *dwc2, struct dwc2_hc *hc, struct urb *u)
{
    int     ret, act_len;
    ...
    if (buffer) {
        ...
        do {
            ret = chunk_msg(dwc2, hc, u, &pid, usb_pipein(u->pipe), buffer, len);
...
static int dwc2_bulk_msg(struct dwc2_control *dwc2, struct dwc2_hc *hc, struct urb *u)
{
...
static int dwc2_int_msg_start(struct dwc2_control *dwc2, struct dwc2_hc *hc, struct urb *u)
{
    uint64_t timeout;
    int      ret;

    timeout = USB_TIMEOUT_MS(u->pipe) * (uint64_t)1000000;
...
static void dwc2_int_msg_stop(struct dwc2_control *dwc2, struct dwc2_hc *hc, struct urb *u, bool urb_int_active)
{
    if (urb_int_active) {
...
static void dwc2_flush_work(struct usb_hcd *hcd)
{
    ...
    irq_flags_t          f;
    struct dwc2_hc      *hc;
    struct dwc2_control *dwc2 = (struct dwc2_control *)usb_hcd_private(hcd);
...
static int dwc2_start(struct usb_hcd *hcd)
{
    ...
    /* Reset port0 */
    vmm_clrsetbits_le32(
        &dwc2->regs->hprt0, DWC2_HPRT0_PRTENA | DWC2_HPRT0_PRTCONNDET | DWC2_HPRT0_PRTENCHNG | DWC2_HPRT0_PRTOVRCURRCHNG, DWC2_HPRT0_PRTRST);
    vmm_msleep(50);
    vmm_clrbits_le32(
        &dwc2->regs->hprt0, DWC2_HPRT0_PRTENA | DWC2_HPRT0_PRTCONNDET | DWC2_HPRT0_PRTENCHNG | DWC2_HPRT0_PRTOVRCURRCHNG | DWC2_HPRT0_PRTRST);
...
static int dwc2_urb_enqueue(struct usb_hcd *hcd, struct urb *urb)
{
    ...
    if (usb_pipetype(urb->pipe) == USB_PIPE_INTERRUPT) {
        i = bitmap_find_free_region(dwc2->hcs_int_bmap, dwc2->hc_count / 2, 0);
    ...
    if ((usb_pipetype(urb->pipe) == USB_PIPE_INTERRUPT) && (hc->urb_int == NULL)) {
        hc->urb_int = urb;
    }
...
static int dwc2_urb_dequeue(struct usb_hcd *hcd, struct urb *urb, int status)
{
    ...
    if (usb_pipetype(urb->pipe) == USB_PIPE_INTERRUPT) {
        ...
        if (hc) {
            bitmap_release_region(dwc2->hcs_int_bmap, hc->index - dwc2->hc_count / 2, 0);
...
{
    uint32_t ahbcfg = vmm_readl(&dwc2->regs->gahbcfg);
...
static int dwc2_driver_probe(vmm_device_t *dev)
{
    int                                  rc = VMM_OK;
    uint32_t                             i, snpsid;
    virtual_addr_t                       regs;
    ...
    dwc2         = (struct dwc2_control *)usb_hcd_private(hcd);
    dwc2->hcd    = hcd;
    dwc2->params = params;

    rc           = vmm_device_tree_regaddr(dev->of_node, &hcd->resource_addr_start, 0);

    if (rc) {
        goto fail_destroy_hcd;
    }

    rc = vmm_device_tree_regsize(dev->of_node, &hcd->resource_addr_len, 0);

    if (rc) {
        goto fail_destroy_hcd;
    }

    rc = vmm_device_tree_request_regmap(dev->of_node, &regs, 0, "DWC2");
...
fail_unmap_regs:
    vmm_device_tree_regunmap_release(dev->of_node, (virtual_addr_t)dwc2->regs, 0);
fail_destroy_hcd:
    usb_dref_hcd(hcd);
fail:
    return rc;
...
static int dwc2_driver_remove(vmm_device_t *dev)
{
    uint32_t             i;
    struct dwc2_hc      *hc;
    struct usb_hcd      *hcd  = dev->private;
    struct dwc2_control *dwc2 = usb_hcd_private(hcd);
...
static const struct dwc2_core_params params_bcm2835 = {
    ...
    .host_ls_low_power_phy_clock  = 0, /* 48 MHz */
    .ts_dline                     = 0,
    .reload_ctl                   = 0,
    .ahbcfg                       = 0x10,
    .uframe_sched                 = 0,
...
static struct vmm_device_tree_nodeid dwc2_devid_table[] = {
    {.compatible = "brcm,bcm2835-usb", .data = &params_bcm2835},
    {.compatible = "brcm,bcm2836-usb", .data = &params_bcm2836},
    {/* end of list */},
};

static vmm_driver_t dwc2_driver = {
    .name        = "dwc2",
...
{
    return vmm_device_driver_register_driver(&dwc2_driver);
}

static void __exit dwc2_driver_exit(void)
{
    vmm_device_driver_unregister_driver(&dwc2_driver);
}
