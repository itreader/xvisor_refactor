...
static int __init realview_reboot_driver_probe(vmm_device_t *dev)
{
...
static int __exit realview_reboot_driver_remove(vmm_device_t *dev)
{
    /* Nothing to do here. */
    return VMM_OK;
}

static struct vmm_device_tree_nodeid realview_reboot_devid_table[] = {
    {.compatible = "arm,realview-reboot"},
    {/* end of list */},
};

static vmm_driver_t realview_reboot_driver = {
...
{
    return vmm_device_driver_register_driver(&realview_reboot_driver);
}

static void __exit realview_reboot_exit(void)
{
    vmm_device_driver_unregister_driver(&realview_reboot_driver);
}
