...
static int __init sun4i_reboot_driver_probe(vmm_device_t *dev)
{
    int rc;

    /* Map timer registers */
    rc = vmm_device_tree_regmap(dev->of_node, &aw_base, 0);
...
static int __exit sun4i_reboot_driver_remove(vmm_device_t *dev)
{
    int rc;

    /* Unmap registers */
    rc = vmm_device_tree_regunmap(dev->of_node, aw_base, 0);
...
static struct vmm_device_tree_nodeid sun4i_reboot_devid_table[] = {
    {.compatible = "allwinner,sun4i-reboot"},
    {/* end of list */},
};

static vmm_driver_t sun4i_reboot_driver = {
    .name        = "sun4i-reboot",
...
{
    return vmm_device_driver_register_driver(&sun4i_reboot_driver);
}

static void __exit sun4i_reboot_exit(void)
{
    vmm_device_driver_unregister_driver(&sun4i_reboot_driver);
}
