...
static int bcm2835_pm_poweroff(void)
{
     ...
     * a hard reset
     */
    uint32_t pm_rsts = vmm_readl(PM_RSTS);
...
static int __init bcm2835_poweroff_driver_probe(vmm_device_t *dev)
{
    int rc;
    ...
    /* Map registers */
    rc = vmm_device_tree_regmap(dev->of_node, &pm_base_va, 0);
...
static int __exit bcm2835_poweroff_driver_remove(vmm_device_t *dev)
{
    int rc;

    /* Unmap registers */
    rc = vmm_device_tree_regunmap(dev->of_node, pm_base_va, 0);
...
static struct vmm_device_tree_nodeid bcm2835_poweroff_devid_table[] = {
    {.compatible = "brcm,bcm2835-poweroff"},
    {/* end of list */},
};

static vmm_driver_t bcm2835_poweroff_driver = {
...
{
    return vmm_device_driver_register_driver(&bcm2835_poweroff_driver);
}

static void __exit bcm2835_poweroff_exit(void)
{
    vmm_device_driver_unregister_driver(&bcm2835_poweroff_driver);
}
