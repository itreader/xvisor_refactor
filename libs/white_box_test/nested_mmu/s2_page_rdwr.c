...
static int s2_page_rdwr_run(struct white_box_test *test, vmm_char_device_t *cdev, uint32_t test_hcpu)
{
    int                    rc = VMM_OK;
    ...
    nested_mmu_test_alloc_page_table(cdev, test, rc, fail_free_host_page, MMU_STAGE2, &s2_page_table);
    ...
    nested_mmu_test_find_free_addr(
        cdev, test, rc, fail_free_s2_page_table, s2_page_table, map_guest_pa + VMM_PAGE_SIZE, VMM_PAGE_SHIFT, &nomap_guest_pa);
...
#define chunk_end   (chunk_start + (VMM_PAGE_SIZE / 4))
    ...
    nested_mmu_test_execute(
        cdev, test, rc, fail_free_s2_page_table, s2_page_table, NULL, nomap_guest_pa + chunk_start + sizeof(uint8_t), MMU_TEST_WIDTH_8BIT,
        nomap_guest_pa + chunk_start + sizeof(uint8_t), MMU_TEST_FAULT_NOMAP | MMU_TEST_FAULT_READ);
    ...
    nested_mmu_test_execute(
        cdev, test, rc, fail_free_s2_page_table, s2_page_table, NULL, nomap_guest_pa + chunk_end - sizeof(uint8_t),
        MMU_TEST_WIDTH_8BIT | MMU_TEST_WRITE, nomap_guest_pa + chunk_end - sizeof(uint8_t), MMU_TEST_FAULT_NOMAP | MMU_TEST_FAULT_WRITE);

#undef chunk_start
#undef chunk_end

    nested_mmu_test_find_free_addr(
        cdev, test, rc, fail_free_s2_page_table, s2_page_table, nomap_guest_pa + VMM_PAGE_SIZE, VMM_PAGE_SHIFT, &nomap_guest_pa);
...
#define chunk_end   (chunk_start + (VMM_PAGE_SIZE / 4))
    ...
    nested_mmu_test_execute(
        cdev, test, rc, fail_free_s2_page_table, s2_page_table, NULL, nomap_guest_pa + chunk_start + sizeof(uint16_t), MMU_TEST_WIDTH_16BIT,
        nomap_guest_pa + chunk_start + sizeof(uint16_t), MMU_TEST_FAULT_NOMAP | MMU_TEST_FAULT_READ);
    ...
    nested_mmu_test_execute(
        cdev, test, rc, fail_free_s2_page_table, s2_page_table, NULL, nomap_guest_pa + chunk_end - sizeof(uint16_t),
        MMU_TEST_WIDTH_16BIT | MMU_TEST_WRITE, nomap_guest_pa + chunk_end - sizeof(uint16_t), MMU_TEST_FAULT_NOMAP | MMU_TEST_FAULT_WRITE);
...
#define chunk_end   (chunk_start + (VMM_PAGE_SIZE / 4))
    ...
    nested_mmu_test_execute(
        cdev, test, rc, fail_free_s2_page_table, s2_page_table, NULL, nomap_guest_pa + chunk_start + sizeof(uint32_t), MMU_TEST_WIDTH_32BIT,
        nomap_guest_pa + chunk_start + sizeof(uint32_t), MMU_TEST_FAULT_NOMAP | MMU_TEST_FAULT_READ);
    ...
    nested_mmu_test_execute(
        cdev, test, rc, fail_free_s2_page_table, s2_page_table, NULL, nomap_guest_pa + chunk_end - sizeof(uint32_t),
        MMU_TEST_WIDTH_32BIT | MMU_TEST_WRITE, nomap_guest_pa + chunk_end - sizeof(uint32_t), MMU_TEST_FAULT_NOMAP | MMU_TEST_FAULT_WRITE);
...
static struct white_box_test s2_page_rdwr = {
    .name = "s2_page_rdwr",
