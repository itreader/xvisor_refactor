...
static int s2_page_nordwr_run(struct white_box_test *test, vmm_char_device_t *cdev, uint32_t test_hcpu)
{
    int                    rc = VMM_OK;
    ...
    nested_mmu_test_map_page_table(
        cdev, test, rc, fail_free_s2_page_table, s2_page_table, map_guest_pa, 0, VMM_PAGE_SIZE, NESTED_MMU_TEST_NORDWR_REG_FLAGS);
...
static struct white_box_test s2_page_nordwr = {
    .name = "s2_page_nordwr",
