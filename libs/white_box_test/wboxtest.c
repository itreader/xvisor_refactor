...
/* Note: Must be called with wtc.lock held */
static struct white_box_test *__wboxtest_find(struct wboxtest_group *group, const char *name)
{
    bool                   found;
    struct white_box_test *test = NULL;
...
/* Note: Must be called with wtc.lock held */
int __wboxtest_group_add_test(const char *group_name, struct white_box_test *test)
{
    struct white_box_test *t;
...
/* Note: Must be called with wtc.lock held */
void __wboxtest_group_del_test(struct white_box_test *test)
{
    struct white_box_test *t;
    struct wboxtest_group *group;
...
/* Note: Must be called with wtc.lock held */
int __wboxtest_run_test(struct white_box_test *test, uint32_t test_hcpu, vmm_char_device_t *cdev, uint32_t iterations)
{
    int            rc;
    uint64_t       leak;
    uint32_t       iter, fail_count = 0;
    uint64_t       tstamp = vmm_timer_timestamp();
    uint32_t       vp     = vmm_host_virtual_address_pool_free_page_count();
    virtual_size_t nh     = vmm_normal_heap_free_size();
    virtual_size_t dh     = vmm_dma_heap_free_size();

    vmm_cdev_printf(cdev, "white_box_test: test=%s test_hcpu=%d start\n", test->name, test_hcpu);
    ...
    if (test->setup) {
        ...
        if (rc) {
            vmm_cdev_printf(
                cdev,
                "white_box_test: test=%s setup failed "
                "(error %d)\n",
                test->name, rc);
        }
    }
    ...
    for (iter = 0; iter < iterations; iter++) {
        ...
        vmm_cdev_printf(
            cdev,
            "white_box_test: test=%s iteration=%d %s"
            " (error %d)\n",
            test->name, iter, (rc) ? "failed" : "passed", rc);
    }
    ...
    leak = vp - vmm_host_virtual_address_pool_free_page_count();

    if (leak) {
        vmm_cdev_printf(cdev, "white_box_test: test=%s virtual_address_pool leakage %lu pages\n", test->name, leak);
    }

    leak = nh - vmm_normal_heap_free_size();

    if (leak) {
        vmm_cdev_printf(cdev, "white_box_test: test=%s normal heap leakage %lu bytes\n", test->name, leak);
    }

    leak = dh - vmm_dma_heap_free_size();

    if (leak) {
        vmm_cdev_printf(cdev, "white_box_test: test=%s dma heap leakage %lu bytes\n", test->name, leak);
    }

    vmm_cdev_printf(cdev, "white_box_test: test=%s time taken %" PRIu64 " nanoseconds\n", test->name, vmm_timer_timestamp() - tstamp);
...
static void wboxtest_run_groups_iter(struct white_box_test *test, void *data)
{
    ...
    __wboxtest_run_test(test, args->test_hcpu, args->cdev, args->iterations);
}

void wboxtest_run_groups(vmm_char_device_t *cdev, uint32_t iterations, int group_count, char **group_names)
{
    int                             g;
    uint32_t                        test_hcpu = vmm_smp_processor_id();
...
static void wboxtest_run_tests_iter(struct white_box_test *test, void *data)
{
...
static void wboxtest_run_all_iter(struct white_box_test *test, void *data)
{
