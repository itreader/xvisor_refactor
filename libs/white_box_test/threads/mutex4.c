...
static vmm_completion_t work_done;
...
static int mutex4_do_test(vmm_char_device_t *cdev)
{
    int done_count = 0;
...
static int mutex4_run(struct white_box_test *test, vmm_char_device_t *cdev, uint32_t test_hcpu)
{
    int     i, ret = VMM_OK;
    ...
    for (i = 0; i < NUM_THREADS; i++) {
        vmm_snprintf(wname, VMM_FIELD_NAME_SIZE, "mutex4_worker%d", i);
        workers[i] = vmm_threads_create(wname, mutex4_worker_thread_main, (void *)(uint64_t)i, current_priority, VMM_THREAD_DEF_TIME_SLICE);
...
static struct white_box_test mutex4 = {
    .name = "mutex4",
    .run  = mutex4_run,
};
