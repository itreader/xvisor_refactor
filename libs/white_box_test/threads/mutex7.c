...
static int mutex7_do_test(vmm_char_device_t *cdev)
{
    int rc, failures = 0;
    ...
    if (rc == VMM_OK) {
        vmm_cdev_printf(cdev, "error: unlocking mutex worked\n");
        failures++;
    }
...
static int mutex7_run(struct white_box_test *test, vmm_char_device_t *cdev, uint32_t test_hcpu)
{
    int     i, ret = VMM_OK;
    ...
    for (i = 0; i < NUM_THREADS; i++) {
        vmm_snprintf(wname, VMM_FIELD_NAME_SIZE, "mutex7_worker%d", i);
        workers[i] = vmm_threads_create(wname, mutex7_worker_thread_main, (void *)(uint64_t)i, current_priority, VMM_THREAD_DEF_TIME_SLICE);
...
static struct white_box_test mutex7 = {
    .name = "mutex7",
    .run  = mutex7_run,
};
