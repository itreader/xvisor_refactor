...
static int semaphore2_do_test(vmm_char_device_t *cdev)
{
    int failures = 0;
    ...
    if (vmm_semaphore_avail(&s1) != 5) {
        vmm_cdev_printf(cdev, "error: worker0 semaphore mismatch\n");
        failures++;
    }
    ...
    if (vmm_semaphore_avail(&s1) != 3) {
        vmm_cdev_printf(cdev, "error: worker1 semaphore mismatch\n");
        failures++;
    }
    ...
    if (vmm_semaphore_avail(&s1) != 0) {
        vmm_cdev_printf(cdev, "error: worker2 semaphore mismatch\n");
        failures++;
    }
    ...
    if (vmm_semaphore_avail(&s1) != 2) {
        vmm_cdev_printf(cdev, "error: worker1 auto-release mismatch\n");
        failures++;
    }
    ...
    /* check s1 semaphore available count */
    if (vmm_semaphore_avail(&s1) != 3) {
        vmm_cdev_printf(cdev, "error: worker0 auto-release mismatch\n");
        failures++;
    }
    ...
    /* check s1 semaphore available count */
    if (vmm_semaphore_avail(&s1) != 6) {
        vmm_cdev_printf(cdev, "error: worker2 auto-release mismatch\n");
        failures++;
    }
...
static int semaphore2_run(struct white_box_test *test, vmm_char_device_t *cdev, uint32_t test_hcpu)
{
    int                  i, ret = VMM_OK;
    ...
    for (i = 0; i < NUM_THREADS; i++) {
        vmm_snprintf(wname, VMM_FIELD_NAME_SIZE, "semaphore2_worker%d", i);
        workers[i] = vmm_threads_create(wname, semaphore2_worker_thread_main, (void *)(uint64_t)i, current_priority, VMM_THREAD_DEF_TIME_SLICE);
...
static struct white_box_test semaphore2 = {
    .name = "semaphore2",
    .run  = semaphore2_run,
};
