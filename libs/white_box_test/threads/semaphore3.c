...
static int semaphore3_do_test(vmm_char_device_t *cdev)
{
    int i, failures = 0;
    ...
    if (shared_data[1] != 0) {
        vmm_cdev_printf(cdev, "error: worker1 shared data updated\n");
        failures++;
    }
    ...
    /* Check worker1 shared data */
    if (shared_data[1] != 1) {
        vmm_cdev_printf(cdev, "error: worker1 shared data not updated\n");
        failures++;
    }
...
static int semaphore3_run(struct white_box_test *test, vmm_char_device_t *cdev, uint32_t test_hcpu)
{
    ...
    for (i = 0; i < NUM_THREADS; i++) {
        vmm_snprintf(wname, VMM_FIELD_NAME_SIZE, "semaphore3_worker%d", i);
...
static struct white_box_test semaphore3 = {
    .name = "semaphore3",
    .run  = semaphore3_run,
};
