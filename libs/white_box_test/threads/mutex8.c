...
static int mutex8_do_test(vmm_char_device_t *cdev)
{
    int failures   = 0;
    ...
    if (shared_data[0] != 1) {
        vmm_cdev_printf(cdev, "error: worker0 shared data unmodified\n");
        failures++;
    }
    ...
    if (shared_data[1] != 1) {
        vmm_cdev_printf(cdev, "error: worker1 shared data unmodified\n");
        failures++;
    }

    /* Check shared data for worker2. It should be one. */
    if (shared_data[2] != 1) {
        vmm_cdev_printf(cdev, "error: worker2 shared data unmodified\n");
        failures++;
    }

    /* Check shared data for worker3. It should be one. */
    if (shared_data[3] != 1) {
        vmm_cdev_printf(cdev, "error: worker3 shared data unmodified\n");
        failures++;
    }
...
static int mutex8_run(struct white_box_test *test, vmm_char_device_t *cdev, uint32_t test_hcpu)
{
    int     i, ret = VMM_OK;
    ...
    for (i = 0; i < NUM_THREADS; i++) {
        vmm_snprintf(wname, VMM_FIELD_NAME_SIZE, "mutex8_worker%d", i);
        workers[i] = vmm_threads_create(wname, mutex8_worker_thread_main, (void *)(uint64_t)i, current_priority, VMM_THREAD_DEF_TIME_SLICE);
...
static struct white_box_test mutex8 = {
    .name = "mutex8",
    .run  = mutex8_run,
};
