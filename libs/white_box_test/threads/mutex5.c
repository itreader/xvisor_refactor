...
static int mutex5_do_test(vmm_char_device_t *cdev)
{
    int i, failures = 0;
    ...
    for (i = 0; i < 4; i++) {
         ...
         */
        if (shared_data != 0) {
            vmm_cdev_printf(cdev, "error: shared data modified\n");
            failures++;
        }
    }
    ...
    if (failures == 0) {
         ...
         */
        if (shared_data != 1) {
            vmm_cdev_printf(cdev, "error: shared data unmodified\n");
            failures++;
        }
        ...
        for (i = 0; i < 4; i++) {
             ...
             */
            if (shared_data != 0) {
                vmm_cdev_printf(cdev, "error: worker modifying\n");
                failures++;
            }
...
static int mutex5_run(struct white_box_test *test, vmm_char_device_t *cdev, uint32_t test_hcpu)
{
    int     i, ret = VMM_OK;
    ...
    for (i = 0; i < NUM_THREADS; i++) {
        vmm_snprintf(wname, VMM_FIELD_NAME_SIZE, "mutex5_worker%d", i);
        workers[i] = vmm_threads_create(wname, mutex5_worker_thread_main, (void *)(uint64_t)i, current_priority, VMM_THREAD_DEF_TIME_SLICE);
...
static struct white_box_test mutex5 = {
    .name = "mutex5",
    .run  = mutex5_run,
};
