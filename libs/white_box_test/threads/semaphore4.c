...
static int semaphore4_do_test(vmm_char_device_t *cdev)
{
    int i, rc, failures = 0;
...
static int semaphore4_run(struct white_box_test *test, vmm_char_device_t *cdev, uint32_t test_hcpu)
{
    int                  i, ret = VMM_OK;
    ...
    for (i = 0; i < NUM_THREADS; i++) {
        vmm_snprintf(wname, VMM_FIELD_NAME_SIZE, "semaphore4_worker%d", i);
        workers[i] = vmm_threads_create(wname, semaphore4_worker_thread_main, (void *)(uint64_t)i, current_priority, VMM_THREAD_DEF_TIME_SLICE);
...
static struct white_box_test semaphore4 = {
    .name = "semaphore4",
    .run  = semaphore4_run,
};
