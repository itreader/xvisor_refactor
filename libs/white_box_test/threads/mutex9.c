...
static int mutex9_do_test(vmm_char_device_t *cdev)
{
    uint64_t i, timeout, etimeout, tstamp;
    int      rc, failures = 0;
...
static int mutex9_run(struct white_box_test *test, vmm_char_device_t *cdev, uint32_t test_hcpu)
{
    ...
    for (i = 0; i < NUM_THREADS; i++) {
        vmm_snprintf(wname, VMM_FIELD_NAME_SIZE, "mutex9_worker%d", i);
...
static struct white_box_test mutex9 = {
    .name = "mutex9",
    .run  = mutex9_run,
};
