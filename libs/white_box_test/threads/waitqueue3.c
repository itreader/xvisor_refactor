...
static int waitqueue3_do_test(vmm_char_device_t *cdev)
{
    int i, w, failures = 0;
    ...
    for (i = 0; i < 10; i++) {
        ...
        /* Check wakeup order */
        if ((wake_order[0] != 0) || (wake_order[1] != 1) || (wake_order[2] != 2) || (wake_order[3] != 3)) {
            vmm_cdev_printf(cdev, "error: wake order %d %d %d %d\n", wake_order[0], wake_order[1], wake_order[2], wake_order[3]);
            failures++;
        }
    }
...
static int waitqueue3_run(struct white_box_test *test, vmm_char_device_t *cdev, uint32_t test_hcpu)
{
    int     i, ret = VMM_OK;
    ...
    for (i = 0; i < NUM_THREADS; i++) {
        vmm_snprintf(wname, VMM_FIELD_NAME_SIZE, "waitqueue3_worker%d", i);
        workers[i] = vmm_threads_create(wname, waitqueue3_worker_thread_main, (void *)(uint64_t)i, current_priority, VMM_THREAD_DEF_TIME_SLICE);
...
static struct white_box_test waitqueue3 = {
    .name = "waitqueue3",
    .run  = waitqueue3_run,
};
