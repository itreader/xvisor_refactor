...
static int mutex6_do_test(vmm_char_device_t *cdev)
{
    int i, rc, failures = 0;
    ...
    for (i = 0; i < TEST_LOCK_CNT; i++) {
        ...
        if (rc) {
            vmm_cdev_printf(cdev, "error: i=%d locking mutex error %d\n", i, rc);
            failures++;
            break;
        }
    }
    ...
    for (i = 0; i < TEST_LOCK_CNT - 1; i++) {
        ...
        if (rc) {
            vmm_cdev_printf(cdev, "error: i=%d unlocking mutex error %d\n", i, rc);
            failures++;
            break;
        }
    }
    ...
    if (failures == 0) {
        ...
        if (rc) {
            vmm_cdev_printf(cdev, "error: unlocking mutex error %d\n", rc);
            failures++;
        }
    ...
    if (rc == VMM_OK) {
        vmm_cdev_printf(cdev, "error: unlock worked fine\n");
        failures++;
    }
...
static int mutex6_run(struct white_box_test *test, vmm_char_device_t *cdev, uint32_t test_hcpu)
{
    ...
    for (i = 0; i < NUM_THREADS; i++) {
        vmm_snprintf(wname, VMM_FIELD_NAME_SIZE, "mutex6_worker%d", i);
...
static struct white_box_test mutex6 = {
    .name = "mutex6",
    .run  = mutex6_run,
};
