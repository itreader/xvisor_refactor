...
struct vsdaemon_control {
    vmm_mutex_t          vsd_list_lock;
    double_list_t        vsd_list;
    double_list_t        vsd_trans_list;
    vmm_notifier_block_t vser_client;
};
...
int vsdaemon_create(const char *transport_name, const char *vserial_name, const char *daemon_name, int argc, char **argv)
{
    int                        rc = VMM_OK;
    bool                       found;
    struct vsdaemon           *vsd;
...
static int __vsdaemon_destroy(struct vsdaemon *vsd)
{
    ...
    vmm_vserial_unregister_receiver(vsd->vser, &vsdaemon_vserial_recv, vsd);
