...
#define MODULE_IPRIORITY        (VSDAEMON_IPRIORITY + NETSTACK_IPRIORITY + 1)
...
struct vsdaemon_telnet {
    ...
    /* tx buffer */
    uint8_t        tx_buf[VSDAEMON_TXBUF_SIZE];
...
static bool vsdaemon_valid_port(uint32_t port)
{
    if (port < 1024) {
...
static void vsdaemon_flush_tx_buffer(struct vsdaemon_telnet *tnet)
{
    ...
    uint32_t    tx_count;
    irq_flags_t flags;
    uint8_t     tx_buf[VSDAEMON_MAX_FLUSH_SIZE];

    while (1) {
        /* Lock connection state */
        vmm_spin_lock_irq_save(&tnet->tx_buf_lock, flags);

        /* Get data from Tx buffer */
        tx_count = 0;

        while (tnet->tx_buf_count && (tx_count < VSDAEMON_MAX_FLUSH_SIZE)) {
        ...
        vmm_spin_unlock_irq_restore(&tnet->tx_buf_lock, flags);

        /* Transmit the pending Tx data */
        if (tx_count && tnet->active_sk) {
            rc = netstack_socket_write(tnet->active_sk, &tx_buf[0], tx_count);
...
static void vsdaemon_telnet_receive_char(struct vsdaemon *vsd, uint8_t ch)
{
    irq_flags_t             flags;
...
static int vsdaemon_telnet_main_loop(struct vsdaemon *vsd)
{
    ...
    while (1) {
        ...
        while (1) {
            ...
            do {
                vmm_vserial_send(vsd->vser, (uint8_t *)buf.data, buf.len);
