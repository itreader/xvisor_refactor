...
#define MODULE_DESC      "virtual_screen library"
#define MODULE_AUTHOR    "Anup Patel"
#define MODULE_LICENSE   "GPL"
#define MODULE_IPRIORITY (VSCREEN_IPRIORITY)
#define MODULE_INIT      virtual_screen_init
#define MODULE_EXIT      virtual_screen_exit
...
typedef enum {
    VSCREEN_WORK_EXIT,
    VSCREEN_MAX_WORK,
} virtual_screen_work_type;

struct virtual_screen_work {
    double_list_t head;
    uint32_t      type;
};

struct virtual_screen_context {
    /* Parameters */
    bool                               is_hard;
    uint32_t                           refresh_rate;
    uint32_t                           esc_key_code[3];
    ...
    struct frame_buffer_var_screeninfo hard_var;
    physical_addr_t                    hard_smem_start;
    uint32_t                           hard_smem_len;
    /* Work queue */
    uint64_t                           work_timeout;
    vmm_spinlock_t                     work_list_lock;
    double_list_t                      work_list;
    vmm_completion_t                   work_avail;
    /* Notifier clients */
    vmm_notifier_block_t               vdis_client;
    vmm_notifier_block_t               vinp_client;
};

struct virtual_screen_ctrl {
    vmm_spinlock_t virtual_screen_list_lock;
    double_list_t  virtual_screen_list;
};

struct virtual_screen_list_elt {
    double_list_t                  head;
    vmm_thread_t                  *thread;
    struct virtual_screen_context *cntx;
    vmm_work_t                     cleanup_work;
};

static struct virtual_screen_ctrl virtual_screenctrl;

static void virtual_screen_enqueue_work(struct virtual_screen_context *cntx, uint32_t type)
{
    irq_flags_t                 flags;
    struct virtual_screen_work *work;
    ...
    work = vmm_zalloc(sizeof(struct virtual_screen_work));
    ...
    vmm_spin_lock_irq_save(&cntx->work_list_lock, flags);
    list_add_tail(&work->head, &cntx->work_list);
    vmm_spin_unlock_irq_restore(&cntx->work_list_lock, flags);

    vmm_completion_complete(&cntx->work_avail);
}

static void virtual_screen_blank_display(struct virtual_screen_context *cntx)
{
    ...
    cntx->info->fbops->frame_buffer_fillrect(cntx->info, &rect);
}

static int virtual_screen_code2vkey(uint32_t code)
{
    int ret;
...
static void virtual_screen_keyboard_event(struct virtual_screen_context *cntx, uint32_t code, int value)
{
    int i, vkey, vkeycode;
    ...
    if (cntx->esc_key_state == ((1 << array_size(cntx->esc_key_code)) - 1)) {
        /* All escape keys pressed hence signal exit */
        virtual_screen_enqueue_work(cntx, VSCREEN_WORK_EXIT);
    }
    ...
    vkey = virtual_screen_code2vkey(code);
    ...
    if (value) {
        ...
        vmm_vkeyboard_event(cntx->vkbd, vkeycode & SCANCODE_KEYCODEMASK, vkey);
    } else {
        if (vkeycode & SCANCODE_GREY) {
            vmm_vkeyboard_event(cntx->vkbd, SCANCODE_EMUL0, -1);
        }
...
static void virtual_screen_mouse_event(struct virtual_screen_context *cntx, int btn, int dx, int dy, int dz)
{
...
static int virtual_screen_event(struct input_handler *ihnd, input_device_t *idev, uint32_t type, uint32_t code, int value)
{
    struct virtual_screen_context *cntx    = ihnd->private;
    ...
    switch (type) {
        case EV_SYN:
            ...
            if (cntx->key_event) {
                virtual_screen_keyboard_event(cntx, cntx->key_code, cntx->key_value);
            }

            /* Process mouse event */
            if (cntx->mouse_event) {
                virtual_screen_mouse_event(cntx, cntx->mouse_btn, cntx->mouse_dx, cntx->mouse_dy, cntx->mouse_dz);
            }
...
static int virtual_screen_soft_refresh(struct virtual_screen_context *cntx)
{
    int                    rc;
    uint32_t               rows, cols;
    ...
    /* Do nothing if virtual_display is NULL */
    if (!cntx->vdis) {
        return VMM_OK;
    }

    /* Try to get current pixeldata of virtual display */
    rc = vmm_virtual_display_get_pixeldata(cntx->vdis, &pf, &rows, &cols, &pa);
    ...
    if (!cntx->hard_vdis && (cntx->info->var.xres_virtual == cols) && (cntx->info->var.yres_virtual == rows) &&
        (cntx->info->var.bits_per_pixel == pf.bits_per_pixel)) {
        screen_size = (cols * rows * pf.bits_per_pixel) >> 3;
        vmm_host_memory_read(pa, cntx->info->screen_base, screen_size, TRUE);
        return VMM_OK;
    }
...
static int virtual_screen_hard_refresh(struct virtual_screen_context *cntx)
{
    int                    rc;
    uint32_t               rows, cols;
    physical_addr_t        pa;
    ...
    if (rc) {
        virtual_screen_hard_switch_back(cntx);
        return VMM_OK;
    }

    /* If we are already using appropriate settings then do nothing */
    if (cntx->hard_vdis && (cntx->info->var.xres_virtual == cols) && (cntx->info->var.yres_virtual == rows) &&
        (cntx->info->var.bits_per_pixel == pf.bits_per_pixel) && (cntx->hard_smem_start == pa)) {
        return VMM_OK;
    }
    ...
    rc = fb_set_smem(cntx->info, cntx->hard_smem_start, cntx->hard_smem_len);
...
hard_bind_error:
    vmm_printf("virtual_screen: %s: rows=%d\n", cntx->info->name, cntx->info->var.yres_virtual);
    vmm_printf("virtual_screen: %s: cols=%d\n", cntx->info->name, cntx->info->var.xres_virtual);
    vmm_printf("virtual_screen: %s: bits_per_pixel=%d\n", cntx->info->name, cntx->info->var.bits_per_pixel);
    vmm_printf("virtual_screen: %s: sem_start=0x%lx\n", cntx->info->name, cntx->smem_start);
    vmm_printf("virtual_screen: %s: sem_len=0x%x\n", cntx->info->name, cntx->smem_len);
    vmm_printf("virtual_screen: %s: rows=%d\n", cntx->vdis->name, rows);
    vmm_printf("virtual_screen: %s: cols=%d\n", cntx->vdis->name, cols);
    vmm_printf("virtual_screen: %s: bits_per_pixel=%d\n", cntx->vdis->name, pf.bits_per_pixel);
    vmm_printf("virtual_screen: %s: hard_sem_start=0x%" PRIPADDR "\n", cntx->vdis->name, cntx->hard_smem_start);
    vmm_printf("virtual_screen: %s: sem_len=0x%x\n", cntx->vdis->name, cntx->hard_smem_len);
    return rc;
}

static int virtual_screen_virtual_display_notification(vmm_notifier_block_t *nb, uint64_t evt, void *data)
{
    struct vmm_virtual_display_event *event = data;
    struct virtual_screen_context    *cntx  = container_of(nb, struct virtual_screen_context, vdis_client);

    if (evt == VMM_VDISPLAY_EVENT_DESTROY) {
        if (cntx->vdis == event->data) {
            /* Signal Exit */
            virtual_screen_enqueue_work(cntx, VSCREEN_WORK_EXIT);
            /* Clear virtual_display pointer */
            cntx->vdis = NULL;
        }
...
static int virtual_screen_virtual_input_notification(vmm_notifier_block_t *nb, uint64_t evt, void *data)
{
    struct vmm_virtual_input_event *event = data;
    struct virtual_screen_context  *cntx  = container_of(nb, struct virtual_screen_context, vinp_client);
...
static void virtual_screen_save(struct frame_buffer_info *info, void *private)
{
    struct virtual_screen_context *cntx = private;
    ...
    /* Erase display */
    virtual_screen_blank_display(cntx);
}

static void virtual_screen_restore(struct frame_buffer_info *info, void *private)
{
    struct virtual_screen_context *cntx = private;
...
static void virtual_screen_process(struct virtual_screen_context *cntx)
{
    int                         rc;
    uint64_t                    timeout;
    irq_flags_t                 flags;
    ...
    while (1) {
        ...
        if (rc == VMM_ERR_TIMEDOUT) {
            if (cntx->is_hard) {
                ...
                rc = virtual_screen_soft_refresh(cntx);
            }
        }
        ...
        work = list_entry(l, struct virtual_screen_work, head);
        vmm_spin_unlock_irq_restore(&cntx->work_list_lock, flags);
...
static int virtual_screen_setup(struct virtual_screen_context *cntx)
{
    int             rc;
    ...
    vmm_snprintf(cntx->name, sizeof(cntx->name), "virtual_screen-%d", (uint32_t)arch_atomic_read(&setup_count));
    ...
    cntx->hndl.evbit[0] = BIT_MASK(EV_SYN) | BIT_MASK(EV_KEY) | BIT_MASK(EV_REL) | BIT_MASK(EV_ABS);
    cntx->hndl.event    = virtual_screen_event;
    cntx->hndl.private  = cntx;
    ...
    /* Open frame buffer*/
    rc = fb_open(cntx->info, virtual_screen_save, virtual_screen_restore, cntx);
    ...
    /* Register virtual_display notifier client */
    cntx->vdis_client.notifier_call = &virtual_screen_virtual_display_notification;
    cntx->vdis_client.priority      = 0;
    rc                              = vmm_virtual_display_register_client(&cntx->vdis_client);

    if (rc) {
        goto dealloc_cmap;
    }

    /* Register virtual_input notifier client */
    cntx->vinp_client.notifier_call = &virtual_screen_virtual_input_notification;
    cntx->vinp_client.priority      = 0;
    rc                              = vmm_virtual_input_register_client(&cntx->vinp_client);

    if (rc) {
        goto unreg_virtual_display_client;
    }

    return VMM_OK;

unreg_virtual_display_client:
    vmm_virtual_display_unregister_client(&cntx->vdis_client);
dealloc_cmap:
...
static int virtual_screen_cleanup(struct virtual_screen_context *cntx)
{
    int rc, rc1, rc2;
    ...
    /* Unregister virtual_input notifier client */
    vmm_virtual_input_unregister_client(&cntx->vinp_client);

    /* Unregister virtual_display notifier client */
    vmm_virtual_display_unregister_client(&cntx->vdis_client);
...
static void virtual_screen_cleanup_work(vmm_work_t *work)
{
    struct virtual_screen_list_elt *velt = container_of(work, struct virtual_screen_list_elt, cleanup_work);
    irq_flags_t                     flags;

    /* Remove framebuffer from list */
    vmm_spin_lock_irq_save(&virtual_screenctrl.virtual_screen_list_lock, flags);
    list_del(&velt->head);
    vmm_spin_unlock_irq_restore(&virtual_screenctrl.virtual_screen_list_lock, flags);

    /* Free virtual_screen context */
    vmm_free(velt->cntx);

    /* Destroy thread */
    vmm_threads_destroy(velt->thread);

    /* Free virtual_screen list element */
    vmm_free(velt);
}

static int virtual_screen_thread(void *data)
{
    int                             rc   = VMM_OK;
    struct virtual_screen_list_elt *velt = data;
    struct virtual_screen_context  *cntx = velt->cntx;

    /* Process work until some one signals exit */
    virtual_screen_process(cntx);

    /* Cleanup virtual_screen */
    rc = virtual_screen_cleanup(cntx);
...
static struct virtual_screen_list_elt *virtual_screen_frame_buffer_info_find(struct frame_buffer_info *info)
{
    irq_flags_t                     flags;
    struct virtual_screen_list_elt *velt = NULL;

    vmm_spin_lock_irq_save(&virtual_screenctrl.virtual_screen_list_lock, flags);
    list_for_each_entry(velt, &virtual_screenctrl.virtual_screen_list, head)
    {
    ...
    struct vmm_virtual_display *vdis, struct vmm_vkeyboard *vkbd, struct vmm_vmouse *vmou)
{
    ...
    /* Setup and intialize virtual_screen */
    rc                    = virtual_screen_setup(cntx);
    ...
    INIT_WORK(&velt->cleanup_work, virtual_screen_cleanup_work);

    vmm_spin_lock_irq_save(&virtual_screenctrl.virtual_screen_list_lock, flags);
    list_add(&velt->head, &virtual_screenctrl.virtual_screen_list);
    vmm_spin_unlock_irq_restore(&virtual_screenctrl.virtual_screen_list_lock, flags);

    /* Create and run virtual_screen thread */
    vmm_snprintf(name, sizeof(name), "virtual_screen/%s", info->name);
...
{
    INIT_SPIN_LOCK(&virtual_screenctrl.virtual_screen_list_lock);
    INIT_LIST_HEAD(&virtual_screenctrl.virtual_screen_list);

    return 0;
}

static void __exit virtual_screen_exit(void)
{
    irq_flags_t                     flags;
    struct virtual_screen_list_elt *velt = NULL;

    /* TODO: The module memory might be freed before exit work
     * is executed by virtual_screen thread.
     */
    vmm_spin_lock_irq_save(&virtual_screenctrl.virtual_screen_list_lock, flags);
    list_for_each_entry(velt, &virtual_screenctrl.virtual_screen_list, head)
    {
