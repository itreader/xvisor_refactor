...
#define VFS_IPRIORITY  (VMM_BLOCK_DEVICE_CLASS_IPRIORITY + 1)
...
struct stat {
    ...
    uint32_t st_uid;   /* user ID of the file owner */
    uint32_t st_gid;   /* group ID of the file's group */
    uint64_t st_ctime; /* file create time */
    uint64_t st_atime; /* file access time */
    uint64_t st_mtime; /* file modify time */
};
...
struct dirent {
    ...
    uint16_t         d_reclen;             /* length of directory entry */
...
/** vnode structure */
struct vnode {
    double_list_t   v_link;               /* link for hash list */
    ...
    vmm_mutex_t v_lock;                   /* lock to protect members below
                                           * v_lock and vnode operations
                                           */
    uint64_t    v_ctime;                  /* Create timestamp
                                           * (updated by filesystem create())
                                           */
    uint64_t    v_atime;                  /* Access timestamp
                                           * (last permission change time)
                                           * (updated by filesystem setattr())
                                           */
    uint64_t    v_mtime;                  /* Modify timestamp
                                           * (last write time)
                                           * (updated by filesystem write())
                                           */
    uint32_t    v_mode;                   /* vnode permissions
                                           * (set once by filesystem lookup())
                                           * (updated by filesystem setattr())
                                           */
...
struct filesystem {
    ...
    int (*mount)(struct mount *, const char *, uint32_t);
