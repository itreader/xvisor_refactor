 ...
 */
static inline __always_inline int __ffs(uint64_t word)
{
    int num = 0;
 ...
 */
static inline __always_inline uint64_t __fls(uint64_t word)
{
 ...
 */
#if BITS_PER_LONG == 32
static inline __always_inline int fls64(uint64_t x)
{
    uint32_t h = x >> 32;
...
static inline unsigned fls_long(uint64_t l)
{
    if (sizeof(l) == 4) {
        return fls(l);
    }
...
static inline int get_count_order(uint32_t count)
{
    if (count == 0) {
        return -1;
    }
 ...
 */
static inline int get_count_order_long(uint64_t l)
{
    if (l == 0UL) {
        return -1;
    }
 ...
 */
static inline uint64_t __ffs64(uint64_t word)
{
#if BITS_PER_LONG == 32
 ...
 * @shift: bits to roll
 */
static inline uint16_t rol16(uint16_t word, uint32_t shift)
{
 ...
 * @shift: bits to roll
 */
static inline uint16_t ror16(uint16_t word, uint32_t shift)
{
    return (word >> shift) | (word << (16 - shift));
}
 ...
 * @shift: bits to roll
 */
static inline uint8_t rol8(uint8_t word, uint32_t shift)
{
...
#define ATOMIC_HASH(a)   (&(__atomic_hash[(((uint64_t)a) / ARCH_CACHE_LINE_SIZE) & (ATOMIC_HASH_SIZE - 1)]))
...
static inline void set_bit(int nr, volatile uint64_t *addr)
{
    ...
    _atomic_spin_lock_irq_save(p, flags);
    *p |= mask;
    _atomic_spin_unlock_irq_restore(p, flags);
}
 ...
 * @addr: Address to start counting from
 */
static inline void clear_bits(int sbit, int len, volatile uint64_t *addr)
{
    uint64_t  ebit = (sbit + len);
    uint64_t  mask = (BIT_MASK(ebit - sbit) - 1) << sbit;
    uint64_t *p    = ((uint64_t *)addr) + BIT_WORD(sbit);
    uint64_t  flags;
 ...
 * restricted to acting on a single-word quantity.
 */
static inline void change_bit(int nr, volatile uint64_t *addr)
{
    uint64_t    mask = BIT_MASK(nr);
    uint64_t   *p    = ((uint64_t *)addr) + BIT_WORD(nr);
    irq_flags_t flags;
 ...
 */
static inline int test_and_set_bit(int nr, volatile uint64_t *addr)
{
    uint64_t    mask = BIT_MASK(nr);
    uint64_t   *p    = ((uint64_t *)addr) + BIT_WORD(nr);
    uint64_t    old;
 ...
 * It also implies a memory barrier.
 */
static inline int test_and_clear_bit(int nr, volatile uint64_t *addr)
{
    uint64_t    mask = BIT_MASK(nr);
    uint64_t   *p    = ((uint64_t *)addr) + BIT_WORD(nr);
    uint64_t    old;
 ...
 * It also implies a memory barrier.
 */
static inline int test_and_change_bit(int nr, volatile uint64_t *addr)
{
    uint64_t    mask = BIT_MASK(nr);
    uint64_t   *p    = ((uint64_t *)addr) + BIT_WORD(nr);
    uint64_t    old;
 ...
 */
static inline void __set_bit(int nr, volatile uint64_t *addr)
{
    uint64_t  mask = BIT_MASK(nr);
    uint64_t *p    = ((uint64_t *)addr) + BIT_WORD(nr);
...
static inline void __clear_bit(int nr, volatile uint64_t *addr)
{
    uint64_t  mask = BIT_MASK(nr);
    uint64_t *p    = ((uint64_t *)addr) + BIT_WORD(nr);
 ...
 * may be that only one operation succeeds.
 */
static inline void __change_bit(int nr, volatile uint64_t *addr)
{
    uint64_t  mask = BIT_MASK(nr);
    uint64_t *p    = ((uint64_t *)addr) + BIT_WORD(nr);
 ...
 */
static inline int __test_and_set_bit(int nr, volatile uint64_t *addr)
{
    uint64_t  mask = BIT_MASK(nr);
    uint64_t *p    = ((uint64_t *)addr) + BIT_WORD(nr);
    uint64_t  old  = *p;
 ...
 * but actually fail.  You must protect multiple accesses with a lock.
 */
static inline int __test_and_clear_bit(int nr, volatile uint64_t *addr)
{
    uint64_t  mask = BIT_MASK(nr);
    uint64_t *p    = ((uint64_t *)addr) + BIT_WORD(nr);
    uint64_t  old  = *p;
...
static inline int __test_and_change_bit(int nr, volatile uint64_t *addr)
{
    uint64_t  mask = BIT_MASK(nr);
    uint64_t *p    = ((uint64_t *)addr) + BIT_WORD(nr);
    uint64_t  old  = *p;
