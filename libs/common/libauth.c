...
extern const uint64_t libs_common_libauth_passwd_data_size;
#endif

#ifdef CONFIG_LIBAUTH_USE_MD5_PASSWD
#include <libs/md5.h>
typedef uint8_t hash_digest_t[16];
#define HASH_LEN 16
#endif
...
static int string_to_digest(const char *digest_str, size_t digest_str_len, uint8_t *digest, uint32_t dlen)
{
    int     i, j;
    uint8_t dval;
...
static int process_auth_entry(char *auth_entry, const char *user, uint8_t *dst_hash, uint32_t dst_len)
{
    char       *auth_token, *auth_save;
    const char *auth_delim = ":";
    uint32_t    centry = 0, found = 0, auth_tok_len;

    for (auth_token = strtok_r(auth_entry, auth_delim, &auth_save); auth_token; auth_token = strtok_r(NULL, auth_delim, &auth_save)) {
        ...
        if (!centry) {
            ...
            if (found) {
                return string_to_digest(auth_token, strlen(auth_token), dst_hash, dst_len);
            }
        }
...
static int get_user_hash(const char *user, uint8_t *dst_hash, uint32_t dst_len)
{
    ...
    while (len) {
        ...
        for (token = strtok_r(buf, delim, &save); token; token = strtok_r(NULL, delim, &save)) {
            tok_len = strlen(token);

            if (*token != '#' && *token != '\n') {
                if (process_auth_entry(token, user, dst_hash, dst_len) == VMM_OK) {
                    return VMM_OK;
                }
            }
...
static void calculate_hash(char *str, hash_digest_t sig)
{
    ...
    md5_init(&md5c);
    md5_update(&md5c, (uint8_t *)str, strlen(str));
    md5_final(sig, &md5c);
}
...
static void calculate_hash(char *str, hash_digest_t sig)
{
    ...
    sha256_init(&sha256c);
    sha256_update(&sha256c, (uint8_t *)str, strlen(str));
