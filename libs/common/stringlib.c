...
size_t strlcpy(char *dest, const char *src, size_t n)
{
    ...
    if (n > 0) {
        /* copy string with limit */
        for (n--; n && ((*dest = *src) != '\0'); dest++, src++, ret++, n--)
            ;
...
int strncasecmp(const char *a, const char *b, size_t n)
{
    if (n > 0) {
        /* search first diff or end of string */
        for (n--; n != 0 && tolower(*a) == tolower(*b) && *a != '\0'; a++, b++, n--)
            ;
...
uint64_t strtoull(const char *s, char **endptr, int base)
{
    uint64_t val = 0;
    uint32_t digit;
...
const char *_parse_integer_fixup_radix(const char *s, uint32_t *base)
{
    if (*base == 0) {
        if (s[0] == '0') {
...
int u64_to_size_str(uint64_t val, char *out, size_t out_len)
{
    const char *suffix = "";
...
int vsscanf(const char *buf, const char *fmt, va_list args)
{
    ...
    while (*fmt) {
        ...
        if (is_sign) {
            val.s = qualifier != 'L' ? strtol(str, &next, base) : strtoll(str, &next, base);
        } else {
            val.u = qualifier != 'L' ? strtoul(str, &next, base) : strtoull(str, &next, base);
        }
