...
struct mempool *mempool_ram_create(uint32_t entity_size, uint32_t page_count, enum vmm_page_pool_type page_type)
{
    ...
    if (!entity_size || ((VMM_PAGE_SIZE * page_count) < entity_size)) {
        return NULL;
    }
    ...
    mp->entity_size  = entity_size;
    mp->entity_count = udiv64((VMM_PAGE_SIZE * page_count), entity_size);
    ...
    mp->entity_base = vmm_page_pool_alloc(page_type, page_count);
...
struct mempool *mempool_heap_create(uint32_t entity_size, uint32_t entity_count)
{
    uint32_t        e;
    virtual_addr_t  va;
    struct mempool *mp;
    ...
    mp->entity_base = (virtual_addr_t)vmm_malloc(entity_size * entity_count);
...
int mempool_destroy(struct mempool *mp)
{
    ...
    switch (mp->type) {
        ...
        case MEMPOOL_TYPE_RAM:
            rc = vmm_page_pool_free(mp->d.ram.page_type, mp->entity_base, mp->d.ram.page_count);
            break;
...
bool mempool_check_ptr(struct mempool *mp, void *entity)
{
    ...
    if ((entity_va < mp->entity_base) || ((mp->entity_base + (mp->entity_count * mp->entity_size)) < entity_va)) {
        return FALSE;
    }
...
uint32_t mempool_total_entities(struct mempool *mp)
{
    return (mp) ? mp->entity_count : 0;
}

uint32_t mempool_free_entities(struct mempool *mp)
{
