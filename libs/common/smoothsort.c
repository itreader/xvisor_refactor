...
typedef struct {
    ...
    uint64_t p;    /**< Concatenation codification */
} stretch;
...
#if DEBUG_SMOOTHSORT
static char const *binary(uint64_t p)
{
 ...
 */
int libsort_smoothsort(void *base, size_t r, size_t N, int (*do_less)(void *m, size_t a, size_t b), void (*do_swap)(void *m, size_t a, size_t b))
{
    stretch s = {1, 1, 1};
    size_t  q;
