...
typedef int (*parser_func_t)(int fd, struct frame_buffer_image *image, struct image_format *fmt);
...
static uint32_t get_number(int fd)
{
    int  rc;
    char val[128];
...
static uint32_t get_number255(int fd)
{
    return get_number(fd) & 0xFF;
}

static int ppm_header(int fd, struct frame_buffer_image *image, int *color_bytes)
{
    char val[128];
    int  rc, colors = 0;
...
static int ppm_parser(int fd, struct frame_buffer_image *image, struct image_format *fmt)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        outv  = red << fmt->red.offset | green << fmt->green.offset | blue << fmt->blue.offset;
        DPRINTF("pos=%d red=0x%x green=0x%x blue=0x%x outv=0x%x\n", i, red, green, blue, outv);

        switch (fmt->bits_per_pixel) {
            case 8:
                ((uint8_t *)out)[i] = (uint8_t)outv;
                break;

            case 16:
                ((uint16_t *)out)[i] = (uint16_t)outv;
                break;

            case 24:
                ((uint32_t *)out)[i] = (uint32_t)outv;
                break;
...
int image_draw(struct frame_buffer_info *info, const struct frame_buffer_image *image, uint32_t x, uint32_t y, uint32_t w, uint32_t h)
{
    ...
    uint32_t    img_stride    = image->width * image->depth / 8;
    uint32_t    screen_stride = info->fix.line_length;
