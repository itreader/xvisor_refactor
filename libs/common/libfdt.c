...
#define LIBFDT_DATA32(ptr) vmm_be32_to_cpu(*((uint32_t *)ptr))
#define LIBFDT_DATA64(ptr) vmm_be64_to_cpu(*((uint64_t *)ptr))

static void libfdt_property_read(const char *prop, void *dst, void *src, uint32_t address_cells, uint32_t size_cells, uint32_t len)
{
    int      tlen;
    uint32_t pos, type, lsz, val32, reg, reg_cells, reg_count;
    uint64_t val64;
    ...
    if (strcmp(prop, "reg") == 0) {
        ...
        for (reg = 0; reg < reg_count; reg++) {
            if (address_cells == 2) {
                *((physical_addr_t *)dst) = (physical_addr_t)LIBFDT_DATA64(src);
            } else {
                *((physical_addr_t *)dst) = (physical_addr_t)LIBFDT_DATA32(src);
            }
            ...
            if (size_cells == 2) {
                *((physical_size_t *)dst) = (physical_size_t)LIBFDT_DATA64(src);
            } else {
                *((physical_size_t *)dst) = (physical_size_t)LIBFDT_DATA32(src);
            }
    ...
    type = vmm_device_tree_estimate_attrtype(prop);

    /* Special way of handling non-literal property */
    if (!vmm_device_tree_isliteral(type)) {
        memcpy(dst, src, len);
        return;
    }

    /* Special way of handling literal property */
    lsz = vmm_device_tree_literal_size(type);
    ...
    if (lsz == 4) {
        ...
        while (tlen > 0) {
            ...
            val32                  = ((uint32_t *)src)[pos];
            ((uint32_t *)dst)[pos] = LIBFDT_DATA32(&val32);
            pos++;
            tlen -= 4;
        }
    } else if (lsz == 8) {
        ...
        while (tlen > 0) {
            if (tlen < 4) {
                ...
                ((uint64_t *)dst)[pos] = LIBFDT_DATA32(&((uint32_t *)src)[pos * 2]);
                break;
            }

            val64                  = ((uint64_t *)src)[pos];
            ((uint64_t *)dst)[pos] = LIBFDT_DATA64(&val64);
            pos++;
            tlen -= 8;
        }
    }
...
int libfdt_parse_fileinfo(virtual_addr_t fdt_addr, struct fdt_fileinfo *fdt)
{
    ...
    fdt->header.off_device_tree_struct   = LIBFDT_DATA32(&fdt->header.off_device_tree_struct);
    fdt->header.off_device_tree_strings  = LIBFDT_DATA32(&fdt->header.off_device_tree_strings);
    ...
    fdt->header.size_device_tree_strings = LIBFDT_DATA32(&fdt->header.size_device_tree_strings);
    fdt->header.size_device_tree_struct  = LIBFDT_DATA32(&fdt->header.size_device_tree_struct);
    ...
    fdt->data       = (char *)fdt_addr + fdt->header.off_device_tree_struct;
    fdt->data_size  = fdt->header.size_device_tree_struct;

    /* Compute strings location & size */
    fdt->str        = (char *)fdt_addr + fdt->header.off_device_tree_strings;
    fdt->str_size   = fdt->header.size_device_tree_strings;
...
static void libfdt_parse_device_tree_recursive(struct fdt_fileinfo *fdt, vmm_device_tree_node_t *node, char **data)
{
    const char             *name;
    uint32_t                type, len;
    ...
    while (LIBFDT_DATA32(*data) != FDT_END_NODE) {
        switch (LIBFDT_DATA32(*data)) {
            case FDT_PROP:
                ...
                *data += sizeof(fdt_cell_t);
                type = vmm_device_tree_estimate_attrtype(name);
                vmm_device_tree_setattr(node, name, *data, type, len, TRUE);
                *data += len;
            ...
            case FDT_BEGIN_NODE:
                *data += sizeof(fdt_cell_t);
                child = vmm_device_tree_addnode(node, *data);
                *data += strlen(*data) + 1;
                ...
                libfdt_parse_device_tree_recursive(fdt, child, data);
                break;
...
int libfdt_parse_device_tree(struct fdt_fileinfo *fdt, vmm_device_tree_node_t **root, const char *root_name, vmm_device_tree_node_t *root_parent)
{
    ...
    libfdt_parse_device_tree_recursive(fdt, node, &data);

    return VMM_OK;
}

static struct fdt_node_header *libfdt_find_matching_node_recursive(
    char **data, char *str, int level, int (*match)(struct fdt_node_header *, int, void *), void *private)
{
    uint32_t                len = 0x0;
    ...
    if (match((struct fdt_node_header *)(*data), level, private)) {
    ...
    *data += len + 1;
    ...
    while (LIBFDT_DATA32(*data) != FDT_END_NODE) {
        switch (LIBFDT_DATA32(*data)) {
            ...
            case FDT_BEGIN_NODE:
                ret = libfdt_find_matching_node_recursive(data, str, level + 1, match, private);
...
struct fdt_node_header *libfdt_find_matching_node(struct fdt_fileinfo *fdt, int (*match)(struct fdt_node_header *, int, void *), void *private)
{
    ...
    data = fdt->data;
    return libfdt_find_matching_node_recursive(&data, fdt->str, 0, match, private);
}

static struct fdt_node_header *libfdt_find_node_recursive(char **data, char *str, const char *node_path)
{
    uint32_t                i, valid, len = 0x0;
    struct fdt_node_header *ret = NULL;

    while ((*node_path == ' ') || (*node_path == '\t') || (*node_path == '\r') || (*node_path == '\n')) {
        node_path++;
    }
    ...
    void *property_value, uint32_t property_len)
{
    ...
    while (LIBFDT_DATA32(data) == FDT_PROP) {
        ...
        if (!strcmp(&fdt->str[LIBFDT_DATA32(data)], property)) {
    ...
    libfdt_property_read(property, property_value, &ret->data[0], address_cells, size_cells, len);
