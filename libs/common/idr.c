...
int idr_alloc(struct idr *idr, void *ptr, int start, int end, unsigned gfp_mask)
{
    uint64_t id = 0;
...
int ida_simple_get(struct ida *ida, uint32_t start, uint32_t end, unsigned gfp_mask)
{
    ...
    return idr_alloc(&ida->idr, (void *)0xDCDCDCDC, start, end, gfp_mask);
}

void ida_simple_remove(struct ida *ida, uint32_t id)
{
    if (!ida) {
        return;
    }
