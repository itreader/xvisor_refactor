...
int ext4fs_mount(struct mount *m, const char *dev, uint32_t flags)
{
    int                    rc;
    uint16_t               rootmode;
    ...
    /* Setup control info */
    rc = ext4fs_control_init(ctrl, m->m_device);
...
static int ext4fs_create(struct vnode *dv, const char *name, uint32_t mode)
{
    int                 rc;
    uint16_t            filemode;
    uint32_t            inode_no;
...
static int ext4fs_rename(struct vnode *sv, const char *sname, struct vnode *v, struct vnode *dv, const char *dname)
{
    int                 rc;
    struct ext2_dirent  dent;
...
static int ext4fs_mkdir(struct vnode *dv, const char *name, uint32_t mode)
{
    int                    rc;
    uint16_t               filemode;
    uint32_t               i, inode_no, blkno;
    char                   buf[64];
    struct ext2_dirent     dent;
    struct ext2_inode      inode;
    struct ext4fs_node    *dnode = dv->v_data;
...
static int ext4fs_chmod(struct vnode *v, uint32_t mode)
{
    uint16_t            filemode;
    struct ext4fs_node *node = v->v_data;
