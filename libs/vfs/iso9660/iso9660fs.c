...
struct dec_datetime {
    ...
    uint8_t minute[2];
    uint8_t second[2];
    uint8_t hsecond[2]; /* hundreths of a second (0-99) */
    /*
...
struct primary_vol_desc {
    ...
    INT16_LSB_MSB       logical_block_size; /* Means blk size can be > 2KiB! */
    INT32_LSB_MSB       path_table_size;    /* in bytes */
    INT32_LSB           typel_path_table_loc;
    INT32_LSB           typel_opt_path_table_loc;
    INT32_MSB           typem_path_table_loc;
    INT32_MSB           typem_opt_path_table_loc;
...
/* Rockridge PX data */
struct rrip_px_data {
    uint8_t  signature[2]; /* PX (50,58) */
    uint8_t  len;          /* PX entry len */
    uint8_t  sus_version;
    uint64_t f_mode;       /* POSIX file mode (st_mode) */
    uint64_t f_links;      /* POSIX file links (st_nlink) */
    uint64_t f_user;       /* POSIX user id */
    uint64_t f_grid;       /* POSIX group id */
    uint64_t f_sernum;     /* POSIX file serial number */
} __packed;
...
struct iso9660_mount_data {
    ...
    vmm_block_device_t     *mdev;
};

/* Mount operation */
static int iso9660fs_mount(struct mount *m, const char *dev, uint32_t flags)
{
    uint64_t                   read_count;
    ...
    mdata->root_dir_entry  = (struct dir_entry *)(&mdata->vol_desc.root_dir_entry[0]);
...
static struct dir_entry *lookup_dentry(const char *dir_name, struct dir_entry *pdentry)
{
    ...
    uint32_t          toffs               = 0, dentry_id_len, nlen;
    ...
    while (1) {
        ...
        nlen          = (dentry_id_len >= VFS_MAX_NAME ? VFS_MAX_NAME : dentry_id_len);
...
static struct dir_entry *path_to_dentry(vmm_block_device_t *mdev, const char *path, struct dir_entry *pdentry)
{
    ...
    if (dentry) {
        ...
        rd     = vmm_block_device_read(mdev, (uint8_t *)d_root, (dentry->start_lba.lsb * 2048), dentry->dlen.lsb);
...
static int iso9660fs_readdir(struct vnode *dv, loff_t off, struct dirent *d)
{
    ...
    uint32_t                   dentry_id_len, nlen;
    uint64_t                   mode = 0, toffs = 0;
    struct rrip_px_data       *px_data = NULL;

    BUG_ON(mdata == NULL);
    BUG_ON(mdata->root_dir == NULL);

    root_dir = (uint8_t *)path_to_dentry(mdata->mdev, dv->v_path, mdata->root_dir);
    ...
    while (1) {
        ...
        if (dentry_id_len % 2) {
            ...
            px_data = (struct rrip_px_data *)(&dentry->vdata[nlen + 1]);
        }

        if (px_data->signature[0] == 'P' && px_data->signature[1] == 'X') {
            mode = px_data->f_mode;
        }
...
static uint32_t iso9660_pack_timestamp(struct dir_entry_datetime *dt)
{
    uint32_t packed_time;
    packed_time = (uint32_t)vmm_wall_clock_mktime(1900 + dt->year, dt->month, dt->day, dt->hour, dt->minute, dt->second);
...
static int iso9660fs_lookup(struct vnode *dv, const char *name, struct vnode *v)
{
    ...
    v->v_mtime = v->v_ctime = v->v_atime = iso9660_pack_timestamp(&dentry->datetime);
    ...
    v->v_data = (void *)((uint64_t)(dentry->start_lba.lsb * 2048));
