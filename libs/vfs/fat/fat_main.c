...
static int fatfs_mount(struct mount *m, const char *dev, uint32_t flags)
{
    int                rc;
    ...
    /* Setup control info */
    rc = fatfs_control_init(ctrl, m->m_device);
    ...
    root->parent_dent_len = sizeof(struct fat_directory_entry);
    memset(&root->parent_dent, 0, sizeof(struct fat_directory_entry));
    root->parent_dent.file_attributes = FAT_DIRENT_SUBDIR;

    if (ctrl->type == FAT_TYPE_32) {
        root->first_cluster                = ctrl->first_root_cluster;
        root->parent_dent.first_cluster_hi = __le16((root->first_cluster >> 16) & 0xFFFF);
        root->parent_dent.first_cluster_lo = __le16(root->first_cluster & 0xFFFF);
...
{
    struct fatfs_node *node     = v->v_data;
    uint32_t           filesize = fatfs_node_get_size(node);

    if (filesize <= (uint32_t)off) {
        return 0;
    }

    if (filesize < (uint32_t)(len + off)) {
        len = filesize - off;
    }

    return fatfs_node_read(node, (uint32_t)off, len, buf);
}
...
static size_t fatfs_write(struct vnode *v, loff_t off, void *buf, size_t len)
{
    ...
    wlen                    = fatfs_node_write(node, (uint32_t)off, len, buf);

    /* Size and mtime might have changed */
    v->v_size               = fatfs_node_get_size(node);
    v->v_mtime              = fatfs_pack_timestamp(
        node->parent_dent.lmodify_date_year, node->parent_dent.lmodify_date_month, node->parent_dent.lmodify_date_day,
...
static int fatfs_lookup(struct vnode *dv, const char *name, struct vnode *v)
{
    ...
    memcpy(&node->parent_dent, &dent, sizeof(struct fat_directory_entry));
    ...
    v->v_ctime = fatfs_pack_timestamp(
        dent.create_date_year, dent.create_date_month, dent.create_date_day, dent.create_time_hours, dent.create_time_minutes,
    ...
    v->v_mtime = fatfs_pack_timestamp(
        dent.lmodify_date_year, dent.lmodify_date_month, dent.lmodify_date_day, dent.lmodify_time_hours, dent.lmodify_time_minutes,
...
static int fatfs_create(struct vnode *dv, const char *name, uint32_t mode)
{
    int                        rc;
    uint8_t                    fileattr;
    uint32_t                   off, len;
...
static int fatfs_rename(struct vnode *sv, const char *sname, struct vnode *v, struct vnode *dv, const char *dname)
{
    int                        rc;
    uint32_t                   off, len;
    struct fat_directory_entry dent;
...
static int fatfs_mkdir(struct vnode *dv, const char *name, uint32_t mode)
{
    int                        rc;
    uint8_t                    fileattr;
    uint32_t                   off, len;
...
static int fatfs_chmod(struct vnode *v, uint32_t mode)
{
    uint8_t            fileattr;
