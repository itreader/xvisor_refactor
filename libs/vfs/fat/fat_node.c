...
static int fatfs_node_find_lookup_dirent(struct fatfs_node *dnode, const char *name, struct fat_directory_entry *dent, uint32_t *off, uint32_t *len)
{
    int i, idx;
...
static void fatfs_node_add_lookup_dirent(struct fatfs_node *dnode, const char *name, struct fat_directory_entry *dent, uint32_t off, uint32_t len)
{
    int  idx;
    bool found = FALSE;
    ...
    for (idx = 0; idx < FAT_NODE_LOOKUP_SIZE; idx++) {
        if (!strncasecmp(dnode->lookup_name[idx], name, VFS_MAX_NAME)) {
            found = TRUE;
            break;
        }
    }
...
static void fatfs_node_del_lookup_dirent(struct fatfs_node *dnode, const char *name)
{
    int idx;
...
{
    uint64_t         wlen, woff;
    fatfs_control_t *ctrl = node->ctrl;

    if (node->cached_dirty && fatfs_control_valid_cluster(ctrl, node->cached_clust)) {
        woff = (uint64_t)ctrl->first_data_sector * ctrl->bytes_per_sector;
        woff += (uint64_t)(node->cached_clust - 2) * ctrl->bytes_per_cluster;
...
uint32_t fatfs_node_read(struct fatfs_node *node, uint32_t pos, uint32_t len, uint8_t *buf)
{
    int              rc;
    uint64_t         rlen, roff;
    uint32_t         r, cl_pos, cl_off, cl_num, cl_len;
    fatfs_control_t *ctrl = node->ctrl;

    if (!node->parent && ctrl->type != FAT_TYPE_32) {
        rlen = (uint64_t)ctrl->bytes_per_sector * ctrl->root_sectors;
        ...
        roff = (uint64_t)ctrl->first_root_sector * ctrl->bytes_per_sector;
        roff += pos;
        return vmm_block_device_read(ctrl->block_device, (uint8_t *)buf, roff, rlen);
    }
    ...
    while (r < len) {
        ...
        if (node->cached_clust != cl_num) {
            ...
            roff               = (uint64_t)ctrl->first_data_sector * ctrl->bytes_per_sector;
            roff += (uint64_t)(cl_num - 2) * ctrl->bytes_per_cluster;
            rlen = vmm_block_device_read(ctrl->block_device, node->cached_data, roff, ctrl->bytes_per_cluster);
...
uint32_t fatfs_node_write(struct fatfs_node *node, uint32_t pos, uint32_t len, uint8_t *buf)
{
    int              rc;
    uint64_t         woff, wlen;
    uint32_t         w, wstartcl, wendcl;
    uint32_t         cl_off, cl_num, cl_len;
    uint32_t         year, mon, day, hour, min, sec;
    fatfs_control_t *ctrl = node->ctrl;

    if (!node->parent && ctrl->type != FAT_TYPE_32) {
        wlen = (uint64_t)ctrl->bytes_per_sector * ctrl->root_sectors;
        ...
        woff = (uint64_t)ctrl->first_root_sector * ctrl->bytes_per_sector;
        woff += pos;
        return vmm_block_device_write(ctrl->block_device, (uint8_t *)buf, woff, wlen);
    }
    ...
    for (w = 0; w <= (wendcl - wstartcl); w++) {
        ...
        woff = (uint64_t)ctrl->first_data_sector * ctrl->bytes_per_sector;
        woff += (uint64_t)(cl_num - 2) * ctrl->bytes_per_cluster;
        wlen = vmm_block_device_write(ctrl->block_device, node->cached_data, woff, ctrl->bytes_per_cluster);
    ...
    while (w < len) {
        ...
        woff += (uint64_t)(cl_num - 2) * ctrl->bytes_per_cluster;
        woff += cl_off;
        wlen = vmm_block_device_write(ctrl->block_device, buf, woff, cl_len);
...
int fatfs_node_init(fatfs_control_t *ctrl, struct fatfs_node *node)
{
    ...
    memset(&node->parent_dent, 0, sizeof(struct fat_directory_entry));
    node->parent_dent_dirty = FALSE;
    node->first_cluster     = 0;
...
static uint8_t fatfs_node_lfn_checksum(const uint8_t *name)
{
    int     i;
    uint8_t sum = 0;
...
{
    uint32_t                   i, rlen, len;
    uint8_t                    lcsum = 0, dcsum = 0, check[11];
    char                       lname[VFS_MAX_NAME];
    struct fat_directory_entry dent;
    ...
    while (1) {
        ...
        fileoff += sizeof(struct fat_directory_entry);
...
{
    uint8_t             lcsum = 0, dcsum = 0, check[11];
    uint32_t            i, off, rlen, len, lfn_off, lfn_len;
    struct fat_longname lfn;
    char                lname[VFS_MAX_NAME];
    ...
    while (1) {
        ...
        if (dent->file_attributes == FAT_LONGNAME_ATTRIBUTE) {
            ...
            if (FAT_LONGNAME_LASTSEQ(lfn.seqno)) {
                lfn.seqno = FAT_LONGNAME_SEQNO(lfn.seqno);
                lfn_off   = off - sizeof(struct fat_directory_entry);
        ...
        if (!strncmp(lname, name, VFS_MAX_NAME) && (lcsum == dcsum)) {
            *dent_off = lfn_off;
            *dent_len = sizeof(struct fat_directory_entry) + lfn_len;
...
{
    bool                       found;
    uint8_t                    dcsum, check[11];
    uint32_t                   i, len, off, cnt, dent_cnt, dent_off;
    struct fat_directory_entry dent;
    struct fat_longname        lfn;
    ...
    while (1) {
        len = fatfs_node_read(dnode, dent_off, sizeof(struct fat_directory_entry), (uint8_t *)&dent);

        if (len != sizeof(struct fat_directory_entry)) {
            cnt   = 0;
            found = TRUE;
            break;
        }
    ...
    i = (uint32_t)vmm_timer_timestamp(); /* Use timestamp for random bytes */
    ...
    for (cnt = 0; cnt < (dent_cnt - 1); cnt++) {
        ...
        for (i = 0; i < 13; i++) {
            ...
            if (i < 5) {
                lfn.name_utf16_1[i] = __le16((uint16_t)name[off + i]);
            } else if (i < 11) {
                lfn.name_utf16_2[i - 5] = __le16((uint16_t)name[off + i]);
            } else {
                lfn.name_utf16_3[i - 11] = __le16((uint16_t)name[off + i]);
            }
        }
