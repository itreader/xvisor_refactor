...
struct sys_mbox {
    ...
    vmm_completion_t    not_empty;
    vmm_completion_t    not_full;
};
...
err_t sys_mbox_trypost(sys_mbox_t *mb, void *msg)
{
    ...
    vmm_spin_lock_irq_save(&mbox->lock, flags);

    LWIP_DEBUGF(SYS_DEBUG, ("sys_mbox_trypost: mbox %p msg %p\n", (void *)mbox, (void *)msg));

    if (mbox->avail == mbox->size) {
        vmm_spin_unlock_irq_restore(&mbox->lock, flags);
        return ERR_MEM;
    }
...
u32_t sys_arch_mbox_fetch(sys_mbox_t *mb, void **msg, u32_t timeout)
{
    ...
    return ((uint64_t)timeout * 1000000ULL) - time_avail;
}
...
sys_thread_t sys_thread_new(const char *name, lwip_thread_fn function, void *arg, int stacksize, int prio)
{
    ...
    st->thread    = vmm_threads_create(st->name, &sys_thread_main, st, VMM_THREAD_DEF_PRIORITY, VMM_THREAD_DEF_TIME_SLICE);

    if (!st->thread) {
        vmm_panic("Failed to create thread.\n");
    }

    vmm_spin_lock_irq_save(&st_list_lock, flags);
    list_add_tail(&st->head, &st_list);
    vmm_spin_unlock_irq_restore(&st_list_lock, flags);
